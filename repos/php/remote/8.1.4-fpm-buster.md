...
```console
$ docker pull php@sha256:289afd2d1732f9774a355b0c1d09e021d8dcc50e51e5be76c9f138190aa938ee
```
...
```console
$ docker pull php@sha256:7e4f1742fec019b57e40c222a1a0fc13fba644880b9f6a59d37307401290776e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **141.6 MB (141572270 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b201c56ac10f342e1d4f75f55aabf23693d22b2c0d74f7bde1ce8a4622b9b0d3`
...
```console
$ docker pull php@sha256:108fb0b333b289c3b1098d6587af71247e591d67559e4949f582dbfcfcb31493
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **120.0 MB (120018507 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1243dfc1339e6670c0d0918370020e6e0ec7a1f450d059d3c069cfaded7a8ad5`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["php-fpm"]`
...
COPY file:ce57c04b70896f77cc11eb2766417d8a1240fcffe5bba92179ec78c458844110 in /usr/local/bin/ 
# Tue, 29 Mar 2022 01:59:01 GMT
RUN set -eux; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get update; 	apt-get install -y --no-install-recommends 		libargon2-dev 		libcurl4-openssl-dev 		libonig-dev 		libreadline-dev 		libsodium-dev 		libsqlite3-dev 		libssl-dev 		libxml2-dev 		zlib1g-dev 	; 		export 		CFLAGS="$PHP_CFLAGS" 		CPPFLAGS="$PHP_CPPFLAGS" 		LDFLAGS="$PHP_LDFLAGS" 	; 	docker-php-source extract; 	cd /usr/src/php; 	gnuArch="$(dpkg-architecture --query DEB_BUILD_GNU_TYPE)"; 	debMultiarch="$(dpkg-architecture --query DEB_BUILD_MULTIARCH)"; 	if [ ! -d /usr/include/curl ]; then 		ln -sT "/usr/include/$debMultiarch/curl" /usr/local/include/curl; 	fi; 	./configure 		--build="$gnuArch" 		--with-config-file-path="$PHP_INI_DIR" 		--with-config-file-scan-dir="$PHP_INI_DIR/conf.d" 				--enable-option-checking=fatal 				--with-mhash 				--with-pic 				--enable-ftp 		--enable-mbstring 		--enable-mysqlnd 		--with-password-argon2 		--with-sodium=shared 		--with-pdo-sqlite=/usr 		--with-sqlite3=/usr 				--with-curl 		--with-iconv 		--with-openssl 		--with-readline 		--with-zlib 				--disable-phpdbg 				--with-pear 				$(test "$gnuArch" = 's390x-linux-gnu' && echo '--without-pcre-jit') 		--with-libdir="lib/$debMultiarch" 				--disable-cgi 				--enable-fpm 		--with-fpm-user=www-data 		--with-fpm-group=www-data 	; 	make -j "$(nproc)"; 	find -type f -name '*.a' -delete; 	make install; 	find 		/usr/local 		-type f 		-perm '/0111' 		-exec sh -euxc ' 			strip --strip-all "$@" || : 		' -- '{}' + 	; 	make clean; 		cp -v php.ini-* "$PHP_INI_DIR/"; 		cd /; 	docker-php-source delete; 		apt-mark auto '.*' > /dev/null; 	[ -z "$savedAptMark" ] || apt-mark manual $savedAptMark; 	find /usr/local -type f -executable -exec ldd '{}' ';' 		| awk '/=>/ { print $(NF-1) }' 		| sort -u 		| xargs -r dpkg-query --search 		| cut -d: -f1 		| sort -u 		| xargs -r apt-mark manual 	; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 	rm -rf /var/lib/apt/lists/*; 		pecl update-channels; 	rm -rf /tmp/pear ~/.pearrc; 		php --version
# Tue, 29 Mar 2022 01:59:02 GMT
COPY multi:869bde9dbeae74886a05c9e2107b3e3b4877116db8c6d9adbaff2719f9fb5262 in /usr/local/bin/ 
# Tue, 29 Mar 2022 01:59:04 GMT
RUN docker-php-ext-enable sodium
# Tue, 29 Mar 2022 01:59:04 GMT
ENTRYPOINT ["docker-php-entrypoint"]
# Tue, 29 Mar 2022 01:59:04 GMT
WORKDIR /var/www/html
# Tue, 29 Mar 2022 01:59:06 GMT
RUN set -eux; 	cd /usr/local/etc; 	if [ -d php-fpm.d ]; then 		sed 's!=NONE/!=!g' php-fpm.conf.default | tee php-fpm.conf > /dev/null; 		cp php-fpm.d/www.conf.default php-fpm.d/www.conf; 	else 		mkdir php-fpm.d; 		cp php-fpm.conf.default php-fpm.d/www.conf; 		{ 			echo '[global]'; 			echo 'include=etc/php-fpm.d/*.conf'; 		} | tee php-fpm.conf; 	fi; 	{ 		echo '[global]'; 		echo 'error_log = /proc/self/fd/2'; 		echo; echo '; https://github.com/docker-library/php/pull/725#issuecomment-443540114'; echo 'log_limit = 8192'; 		echo; 		echo '[www]'; 		echo '; if we send this to /proc/self/fd/1, it never appears'; 		echo 'access.log = /proc/self/fd/2'; 		echo; 		echo 'clear_env = no'; 		echo; 		echo '; Ensure worker stdout and stderr are sent to the main error log.'; 		echo 'catch_workers_output = yes'; 		echo 'decorate_workers_output = no'; 	} | tee php-fpm.d/docker.conf; 	{ 		echo '[global]'; 		echo 'daemonize = no'; 		echo; 		echo '[www]'; 		echo 'listen = 9000'; 	} | tee php-fpm.d/zz-docker.conf
# Tue, 29 Mar 2022 01:59:07 GMT
STOPSIGNAL SIGQUIT
# Tue, 29 Mar 2022 01:59:07 GMT
EXPOSE 9000
# Tue, 29 Mar 2022 01:59:07 GMT
...
-	Layers:
	...
	-	`sha256:e2faaa81425cf856c46175327607c23524053b511ac3aac1953a78b06c997cef`  
		...
		Size: 493.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:792e00f8611b6cf7396b6f16c0e7fe733597cc7cb7d9d1328d146106941c469c`  
		Last Modified: Tue, 29 Mar 2022 03:43:48 GMT  
		Size: 24.3 MB (24272507 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:eb250c9820916fb7cdba03e50694e07ce00cac616c50659727fb932f3c1587d1`  
		Last Modified: Tue, 29 Mar 2022 03:43:32 GMT  
		Size: 2.5 KB (2452 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:32fbf82b76c2fea4f692327d942cba705caff91b8815a2efcd3b39548e7e0c99`  
		Last Modified: Tue, 29 Mar 2022 03:43:32 GMT  
		Size: 248.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:127a74eaa6597a94d88d3c5095e06fe308beabfcc456253e2b2c0ae29f00e088`  
		Last Modified: Tue, 29 Mar 2022 03:43:32 GMT  
		Size: 8.6 KB (8622 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
...
```console
$ docker pull php@sha256:99dab4556e2beb5d3477a077c3abe777a0c7370bebff0902b2457a0a419905b5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **133.6 MB (133639875 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d12cf32e689d6ee215c58796834fcde41381c199f5d0e7ba90acaec8c0a7e89`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["php-fpm"]`
...
```console
$ docker pull php@sha256:e43c033dac9f71260be2aa7f4e60bbbd3184e51b99e3dcb512ccef7b03a69578
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **146.8 MB (146802052 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c4d3e8b0fad267e6f6d82b6bc575e44dcb24d70e4e490b3f76933ccb54e78d2`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["php-fpm"]`
...
```console
$ docker pull php@sha256:54c370470ad04e64c0e0e1404596549aa4c6de987227aee35e32536a532cfe70
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **127.2 MB (127246506 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2237a38c503dd1b3f302cd4c39bf9a524c5a8d96cb0e03e0e2fe9c8cb6fa9cb0`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["php-fpm"]`
