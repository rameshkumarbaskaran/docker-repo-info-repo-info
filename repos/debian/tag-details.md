<!-- THIS FILE IS GENERATED VIA './update-remote.sh' -->

# Tags of `debian`

-	[`debian:10`](#debian10)
-	[`debian:10-slim`](#debian10-slim)
-	[`debian:10.12`](#debian1012)
-	[`debian:10.12-slim`](#debian1012-slim)
-	[`debian:11`](#debian11)
-	[`debian:11-slim`](#debian11-slim)
-	[`debian:11.3`](#debian113)
-	[`debian:11.3-slim`](#debian113-slim)
-	[`debian:9`](#debian9)
-	[`debian:9-slim`](#debian9-slim)
-	[`debian:9.13`](#debian913)
-	[`debian:9.13-slim`](#debian913-slim)
-	[`debian:bookworm`](#debianbookworm)
-	[`debian:bookworm-20220418`](#debianbookworm-20220418)
-	[`debian:bookworm-20220418-slim`](#debianbookworm-20220418-slim)
-	[`debian:bookworm-backports`](#debianbookworm-backports)
-	[`debian:bookworm-slim`](#debianbookworm-slim)
-	[`debian:bullseye`](#debianbullseye)
-	[`debian:bullseye-20220418`](#debianbullseye-20220418)
-	[`debian:bullseye-20220418-slim`](#debianbullseye-20220418-slim)
-	[`debian:bullseye-backports`](#debianbullseye-backports)
-	[`debian:bullseye-slim`](#debianbullseye-slim)
-	[`debian:buster`](#debianbuster)
-	[`debian:buster-20220418`](#debianbuster-20220418)
-	[`debian:buster-20220418-slim`](#debianbuster-20220418-slim)
-	[`debian:buster-backports`](#debianbuster-backports)
-	[`debian:buster-slim`](#debianbuster-slim)
-	[`debian:experimental`](#debianexperimental)
-	[`debian:experimental-20220418`](#debianexperimental-20220418)
-	[`debian:latest`](#debianlatest)
-	[`debian:oldoldstable`](#debianoldoldstable)
-	[`debian:oldoldstable-20220418`](#debianoldoldstable-20220418)
-	[`debian:oldoldstable-20220418-slim`](#debianoldoldstable-20220418-slim)
-	[`debian:oldoldstable-backports`](#debianoldoldstable-backports)
-	[`debian:oldoldstable-slim`](#debianoldoldstable-slim)
-	[`debian:oldstable`](#debianoldstable)
-	[`debian:oldstable-20220418`](#debianoldstable-20220418)
-	[`debian:oldstable-20220418-slim`](#debianoldstable-20220418-slim)
-	[`debian:oldstable-backports`](#debianoldstable-backports)
-	[`debian:oldstable-slim`](#debianoldstable-slim)
-	[`debian:rc-buggy`](#debianrc-buggy)
-	[`debian:rc-buggy-20220418`](#debianrc-buggy-20220418)
-	[`debian:sid`](#debiansid)
-	[`debian:sid-20220418`](#debiansid-20220418)
-	[`debian:sid-20220418-slim`](#debiansid-20220418-slim)
-	[`debian:sid-slim`](#debiansid-slim)
-	[`debian:stable`](#debianstable)
-	[`debian:stable-20220418`](#debianstable-20220418)
-	[`debian:stable-20220418-slim`](#debianstable-20220418-slim)
-	[`debian:stable-backports`](#debianstable-backports)
-	[`debian:stable-slim`](#debianstable-slim)
-	[`debian:stretch`](#debianstretch)
-	[`debian:stretch-20220418`](#debianstretch-20220418)
-	[`debian:stretch-20220418-slim`](#debianstretch-20220418-slim)
-	[`debian:stretch-backports`](#debianstretch-backports)
-	[`debian:stretch-slim`](#debianstretch-slim)
-	[`debian:testing`](#debiantesting)
-	[`debian:testing-20220418`](#debiantesting-20220418)
-	[`debian:testing-20220418-slim`](#debiantesting-20220418-slim)
-	[`debian:testing-backports`](#debiantesting-backports)
-	[`debian:testing-slim`](#debiantesting-slim)
-	[`debian:unstable`](#debianunstable)
-	[`debian:unstable-20220418`](#debianunstable-20220418)
-	[`debian:unstable-20220418-slim`](#debianunstable-20220418-slim)
-	[`debian:unstable-slim`](#debianunstable-slim)

## `debian:10`

```console
$ docker pull debian@sha256:7979afbcb351f4704aa52fe88f0a40937cabd660895b83e33dea992dec85c3e1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10` - linux; amd64

```console
$ docker pull debian@sha256:40dd1c1b1c36eac161ab63b6ce3a57d56ad79a667a37717a31721bac3f30aaf9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50436985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26a2b081e03207d26a105340161109ba0f00e857cbb0ff85aaeeeadd46b709c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:37 GMT
ADD file:7c5789fb822bda2652d7addee832c5a3d71733f0f94f97d89b0c5570c0840829 in / 
# Wed, 20 Apr 2022 04:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85bed84afb9a834cf090b55d2e584abd55b4792d93b750db896f486680638344`  
		Last Modified: Wed, 20 Apr 2022 04:48:49 GMT  
		Size: 50.4 MB (50436985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v5

```console
$ docker pull debian@sha256:92f008e12d46bf2e18e54af85d4f606a2664746288211093cb79deb2c349d538
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3258a131168592bbcc73ee3bff8628e8831ab30efbfb45deb28886921bf5ab40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:11 GMT
ADD file:ad329516525d9a2e9eecb4bd6263809faafa65bbd9e6deebcda8196b0ea24234 in / 
# Wed, 20 Apr 2022 07:17:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d426a13b8d217ec7cc2eceea9d798d79a1c36c60cb9363d2ef2d86abcf895e88`  
		Last Modified: Wed, 20 Apr 2022 07:33:13 GMT  
		Size: 48.2 MB (48153607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e9f24e8010ba995daf25c6fafe8ea5c1914428cc29df867ffc034375d769a43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b12c05639277048cd1ee9a1764a3946118f4dbd48e2c6dd7d36abe127500640`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:49 GMT
ADD file:fbbe7a4ec12b0fdfa82879ff73d49ba760c5b6cc47b4c8ecabe64f7e8f4e6340 in / 
# Wed, 20 Apr 2022 13:27:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca53ad6266ed4ba4323f1553d8b226d0a180384cd290a8361ab19347f6d761fa`  
		Last Modified: Wed, 20 Apr 2022 13:44:25 GMT  
		Size: 45.9 MB (45908143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bf508d976de85857fd69e5da1f67d89f59ab3bc9f60b578ce24eee43047faf23
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de70a5f020527dc50128dcfe45bc76352337371d985d92dbeb1f826f5b462d08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:22 GMT
ADD file:5286ff5db23c42ac6a437e8f2f9ef01b6ee6e523d7866c752428d54f177c7108 in / 
# Wed, 20 Apr 2022 04:29:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9f330b3a7e234282d632d95bd19a655cf0e06c1a76a51d0f73b9581be8f851b`  
		Last Modified: Wed, 20 Apr 2022 04:36:12 GMT  
		Size: 49.2 MB (49227738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; 386

```console
$ docker pull debian@sha256:373c53aee6f022db1a970f7d8631f633d63810d51e466edb682427093874fc10
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e379afe86ea1b6d5434a1193a58277d88f2275200efacfe33fee80b295855706`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:47 GMT
ADD file:f66ba5e7b2801d19041661506f244a3bb89fc613e2e27891bdba3183bd6ee097 in / 
# Wed, 20 Apr 2022 07:37:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e8f333ddc9f58dd9e74b1f250e3812ba3942ef13abee1d6b4e3d3131a7e29ff`  
		Last Modified: Wed, 20 Apr 2022 07:45:17 GMT  
		Size: 51.2 MB (51195868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; mips64le

```console
$ docker pull debian@sha256:1fd521fb0c639ec1c7a9cf64eb55b485e5d2177fa53cc0ee2f3fd2851df4459a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079891 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fffd7182b90e6f67b05c4ad7efaed98eb074b678b22345c014814758cba833a0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:01 GMT
ADD file:c86010b52f782f910842586e6abc81e64c7a659d001b89799314a59f4fb96573 in / 
# Tue, 29 Mar 2022 07:43:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8e0bee9c4b9257c0ad7469830be5e890536feef4e8ffe6c2613b7e8be7e060f6`  
		Last Modified: Tue, 29 Mar 2022 07:53:41 GMT  
		Size: 49.1 MB (49079891 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; ppc64le

```console
$ docker pull debian@sha256:2e9fdeb6153b85d54d8e6a6dccc44faef10b9b33478a6af45114e35b6c56f86f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169874 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f419f2a07e659f09f4d3f9892bc82edd223abd5c9357e17bd896416a203f099`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:08 GMT
ADD file:0e047f9a02092945498103cc7b6483b67ad3a6d4945ac61230d539e4b6e14663 in / 
# Wed, 20 Apr 2022 09:47:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9d81baca7c32c0c0c5ac628e027c3c89760343d8a3469fbf7c764d60badfa23`  
		Last Modified: Wed, 20 Apr 2022 09:57:27 GMT  
		Size: 54.2 MB (54169874 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; s390x

```console
$ docker pull debian@sha256:3930c6d27a18a742104fc0ef8d49fdd180999f80104dbbe9e1d6fd526eac7d99
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98d4667bcfc2f6d7c588eb4c5160bc3234cff1e44888e375fe4ba6291e3d9d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:05 GMT
ADD file:fedc64967d4810188e8bd8289de1b1848a9501e7c68ae5bd4af83377fb9e3108 in / 
# Wed, 20 Apr 2022 08:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2799a5996f1eb24bb7f4c0219facf7e95c780314bb9e9b62594f1fe56cd511`  
		Last Modified: Wed, 20 Apr 2022 08:49:56 GMT  
		Size: 49.0 MB (48999225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10-slim`

```console
$ docker pull debian@sha256:253082b13b593e7f3b7a82800fa7f7f44de0f7895f5c4996c030be0206e6d4d0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10-slim` - linux; amd64

```console
$ docker pull debian@sha256:dfa8be0c158e556532b023717650ee2ab52ed2b1b792a5810db138708101548a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77eeed0828a424e6b36cbde8c519381037702610d8e2f883872d7ba7198fb4e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:48 GMT
ADD file:011a43ee23214c201afb7f3b5be592f374b89a4c71aea82ca66146bbbc31b959 in / 
# Wed, 20 Apr 2022 04:43:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4be315f6562fccf08fd6c749557e31e45ab6d987370e20e2c4933ddb04ddd5ff`  
		Last Modified: Wed, 20 Apr 2022 04:49:15 GMT  
		Size: 27.1 MB (27140664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:87ac356f0eb760b1b5114218ea1148f9e93d1371dd0a15cef28894e48b0dca0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889592 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a8356a7ebaad8c554f999a9556ab637482de9fa4f117a6a31b16567b1282338`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:47 GMT
ADD file:5b740716d202d2f3494df5e108c0ba999125920ed9c97803fe0ccdfeba5cf518 in / 
# Wed, 20 Apr 2022 07:17:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fc8704cfa531d4583c5d4d571b836a82c02a812dae88792c4397cf1c1770405`  
		Last Modified: Wed, 20 Apr 2022 07:33:54 GMT  
		Size: 24.9 MB (24889592 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:79c373b3a8b1543cb91ff909eec8c5b6b2751d8ed1806063aba4bea77b83799e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747865 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfe2eb31a188f4af3f49515a22bd251f26b1351ae64056cb418759da984df972`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:26 GMT
ADD file:2a755b6e7bf01ec959b8bb848b6d46f292821a0bf08e04d565991ec4bdbea029 in / 
# Wed, 20 Apr 2022 13:28:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:88467ac22b4358b1b60009f6332ba9e12b7a82a36bef334d91d68dbf8b58d96f`  
		Last Modified: Wed, 20 Apr 2022 13:45:04 GMT  
		Size: 22.7 MB (22747865 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:09d07b5771076a6acf574e578ade2f64f4f4d44845d5fee34a6814beb4601d98
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908349 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a2d0ef9e5ad17086e08346533255f9296d445e66bad3c5b9601c9758c878524`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:36 GMT
ADD file:5de4397c1295f7249c93907be599d96efbd9cd25909340d245ec8d1c9770f444 in / 
# Wed, 20 Apr 2022 04:29:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c3e5b218d6477ff77053280968d5bdeaa5dcf16a803e917d98dd0ea8b93ac82`  
		Last Modified: Wed, 20 Apr 2022 04:36:38 GMT  
		Size: 25.9 MB (25908349 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; 386

```console
$ docker pull debian@sha256:589a7c019a77ef6f47c72807cf92ab71e09d208212db1703fb28b3432722689d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:41e984298ef845f5111c417dd3c3a5446f7fb9f06549018f81be3337421972eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:03 GMT
ADD file:602a25173054242f635a5a299845b7f1b56864ac5d3b8af1ae29dec3a9da119f in / 
# Wed, 20 Apr 2022 07:38:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a13b13d818745c4d2234aab71df26e4a01bfe59f396ab62f20d156b94803650c`  
		Last Modified: Wed, 20 Apr 2022 07:45:46 GMT  
		Size: 27.8 MB (27799825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; mips64le

```console
$ docker pull debian@sha256:af7ca58714f59c0cfc55282e11d3c7aab0ab34d30ae6228b2c962a0c3fa83184
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25818066 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ac7583a1184fcf172d760efca521e0ea2a669111393a826992b236a324657a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:36 GMT
ADD file:7827dc4070f09537c063b825b0b7a5b076fac295f9afa415dee6c5346c38d46b in / 
# Tue, 29 Mar 2022 07:43:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a3e27ca4ec7041c36d0e9cbc672ce565851ce1a0cc90120d7a923a938fb3c18`  
		Last Modified: Tue, 29 Mar 2022 07:54:19 GMT  
		Size: 25.8 MB (25818066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:c9b116e64e9f03088780c4da7456aea50d24047340f18f50e80416173eed96d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98e35f6574d2a25af8e8be68282f6c7fa6b571faa4404e75c09e913d23973469`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:47 GMT
ADD file:69078e7ae6d887f7e36ae048f98f784678fbd8b7a8ec27e3c2bce0d68a47236d in / 
# Wed, 20 Apr 2022 09:47:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aacfa28cbcef999a852ce9c261b67f15f0ccd8450cd1dbfe7b0dc88e6b5aba2`  
		Last Modified: Wed, 20 Apr 2022 09:58:02 GMT  
		Size: 30.6 MB (30560296 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; s390x

```console
$ docker pull debian@sha256:f8a77bc2f84442b3708edeef82c7c787b95aae8a82e746a73a81b2c3e5abe0db
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7716345ae3e3eff8664c28b2abe1284a6e2b186186aede487146ac809fa58cd2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:41 GMT
ADD file:e9840ab2ce3aeff61b68bf4837f65471a86dcba02c5023863f3e7493098b07ba in / 
# Wed, 20 Apr 2022 08:40:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b59fdcf2cb29d2ee4dd08aa08a467372e33525431c4caba9b01e9eb28f76425f`  
		Last Modified: Wed, 20 Apr 2022 08:50:14 GMT  
		Size: 25.8 MB (25758600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.12`

```console
$ docker pull debian@sha256:7979afbcb351f4704aa52fe88f0a40937cabd660895b83e33dea992dec85c3e1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10.12` - linux; amd64

```console
$ docker pull debian@sha256:40dd1c1b1c36eac161ab63b6ce3a57d56ad79a667a37717a31721bac3f30aaf9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50436985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26a2b081e03207d26a105340161109ba0f00e857cbb0ff85aaeeeadd46b709c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:37 GMT
ADD file:7c5789fb822bda2652d7addee832c5a3d71733f0f94f97d89b0c5570c0840829 in / 
# Wed, 20 Apr 2022 04:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85bed84afb9a834cf090b55d2e584abd55b4792d93b750db896f486680638344`  
		Last Modified: Wed, 20 Apr 2022 04:48:49 GMT  
		Size: 50.4 MB (50436985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; arm variant v5

```console
$ docker pull debian@sha256:92f008e12d46bf2e18e54af85d4f606a2664746288211093cb79deb2c349d538
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3258a131168592bbcc73ee3bff8628e8831ab30efbfb45deb28886921bf5ab40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:11 GMT
ADD file:ad329516525d9a2e9eecb4bd6263809faafa65bbd9e6deebcda8196b0ea24234 in / 
# Wed, 20 Apr 2022 07:17:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d426a13b8d217ec7cc2eceea9d798d79a1c36c60cb9363d2ef2d86abcf895e88`  
		Last Modified: Wed, 20 Apr 2022 07:33:13 GMT  
		Size: 48.2 MB (48153607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e9f24e8010ba995daf25c6fafe8ea5c1914428cc29df867ffc034375d769a43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b12c05639277048cd1ee9a1764a3946118f4dbd48e2c6dd7d36abe127500640`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:49 GMT
ADD file:fbbe7a4ec12b0fdfa82879ff73d49ba760c5b6cc47b4c8ecabe64f7e8f4e6340 in / 
# Wed, 20 Apr 2022 13:27:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca53ad6266ed4ba4323f1553d8b226d0a180384cd290a8361ab19347f6d761fa`  
		Last Modified: Wed, 20 Apr 2022 13:44:25 GMT  
		Size: 45.9 MB (45908143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bf508d976de85857fd69e5da1f67d89f59ab3bc9f60b578ce24eee43047faf23
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de70a5f020527dc50128dcfe45bc76352337371d985d92dbeb1f826f5b462d08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:22 GMT
ADD file:5286ff5db23c42ac6a437e8f2f9ef01b6ee6e523d7866c752428d54f177c7108 in / 
# Wed, 20 Apr 2022 04:29:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9f330b3a7e234282d632d95bd19a655cf0e06c1a76a51d0f73b9581be8f851b`  
		Last Modified: Wed, 20 Apr 2022 04:36:12 GMT  
		Size: 49.2 MB (49227738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; 386

```console
$ docker pull debian@sha256:373c53aee6f022db1a970f7d8631f633d63810d51e466edb682427093874fc10
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e379afe86ea1b6d5434a1193a58277d88f2275200efacfe33fee80b295855706`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:47 GMT
ADD file:f66ba5e7b2801d19041661506f244a3bb89fc613e2e27891bdba3183bd6ee097 in / 
# Wed, 20 Apr 2022 07:37:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e8f333ddc9f58dd9e74b1f250e3812ba3942ef13abee1d6b4e3d3131a7e29ff`  
		Last Modified: Wed, 20 Apr 2022 07:45:17 GMT  
		Size: 51.2 MB (51195868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; mips64le

```console
$ docker pull debian@sha256:1fd521fb0c639ec1c7a9cf64eb55b485e5d2177fa53cc0ee2f3fd2851df4459a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079891 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fffd7182b90e6f67b05c4ad7efaed98eb074b678b22345c014814758cba833a0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:01 GMT
ADD file:c86010b52f782f910842586e6abc81e64c7a659d001b89799314a59f4fb96573 in / 
# Tue, 29 Mar 2022 07:43:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8e0bee9c4b9257c0ad7469830be5e890536feef4e8ffe6c2613b7e8be7e060f6`  
		Last Modified: Tue, 29 Mar 2022 07:53:41 GMT  
		Size: 49.1 MB (49079891 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; ppc64le

```console
$ docker pull debian@sha256:2e9fdeb6153b85d54d8e6a6dccc44faef10b9b33478a6af45114e35b6c56f86f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169874 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f419f2a07e659f09f4d3f9892bc82edd223abd5c9357e17bd896416a203f099`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:08 GMT
ADD file:0e047f9a02092945498103cc7b6483b67ad3a6d4945ac61230d539e4b6e14663 in / 
# Wed, 20 Apr 2022 09:47:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9d81baca7c32c0c0c5ac628e027c3c89760343d8a3469fbf7c764d60badfa23`  
		Last Modified: Wed, 20 Apr 2022 09:57:27 GMT  
		Size: 54.2 MB (54169874 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12` - linux; s390x

```console
$ docker pull debian@sha256:3930c6d27a18a742104fc0ef8d49fdd180999f80104dbbe9e1d6fd526eac7d99
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98d4667bcfc2f6d7c588eb4c5160bc3234cff1e44888e375fe4ba6291e3d9d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:05 GMT
ADD file:fedc64967d4810188e8bd8289de1b1848a9501e7c68ae5bd4af83377fb9e3108 in / 
# Wed, 20 Apr 2022 08:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2799a5996f1eb24bb7f4c0219facf7e95c780314bb9e9b62594f1fe56cd511`  
		Last Modified: Wed, 20 Apr 2022 08:49:56 GMT  
		Size: 49.0 MB (48999225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.12-slim`

```console
$ docker pull debian@sha256:253082b13b593e7f3b7a82800fa7f7f44de0f7895f5c4996c030be0206e6d4d0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10.12-slim` - linux; amd64

```console
$ docker pull debian@sha256:dfa8be0c158e556532b023717650ee2ab52ed2b1b792a5810db138708101548a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77eeed0828a424e6b36cbde8c519381037702610d8e2f883872d7ba7198fb4e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:48 GMT
ADD file:011a43ee23214c201afb7f3b5be592f374b89a4c71aea82ca66146bbbc31b959 in / 
# Wed, 20 Apr 2022 04:43:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4be315f6562fccf08fd6c749557e31e45ab6d987370e20e2c4933ddb04ddd5ff`  
		Last Modified: Wed, 20 Apr 2022 04:49:15 GMT  
		Size: 27.1 MB (27140664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:87ac356f0eb760b1b5114218ea1148f9e93d1371dd0a15cef28894e48b0dca0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889592 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a8356a7ebaad8c554f999a9556ab637482de9fa4f117a6a31b16567b1282338`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:47 GMT
ADD file:5b740716d202d2f3494df5e108c0ba999125920ed9c97803fe0ccdfeba5cf518 in / 
# Wed, 20 Apr 2022 07:17:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fc8704cfa531d4583c5d4d571b836a82c02a812dae88792c4397cf1c1770405`  
		Last Modified: Wed, 20 Apr 2022 07:33:54 GMT  
		Size: 24.9 MB (24889592 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:79c373b3a8b1543cb91ff909eec8c5b6b2751d8ed1806063aba4bea77b83799e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747865 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfe2eb31a188f4af3f49515a22bd251f26b1351ae64056cb418759da984df972`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:26 GMT
ADD file:2a755b6e7bf01ec959b8bb848b6d46f292821a0bf08e04d565991ec4bdbea029 in / 
# Wed, 20 Apr 2022 13:28:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:88467ac22b4358b1b60009f6332ba9e12b7a82a36bef334d91d68dbf8b58d96f`  
		Last Modified: Wed, 20 Apr 2022 13:45:04 GMT  
		Size: 22.7 MB (22747865 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:09d07b5771076a6acf574e578ade2f64f4f4d44845d5fee34a6814beb4601d98
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908349 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a2d0ef9e5ad17086e08346533255f9296d445e66bad3c5b9601c9758c878524`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:36 GMT
ADD file:5de4397c1295f7249c93907be599d96efbd9cd25909340d245ec8d1c9770f444 in / 
# Wed, 20 Apr 2022 04:29:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c3e5b218d6477ff77053280968d5bdeaa5dcf16a803e917d98dd0ea8b93ac82`  
		Last Modified: Wed, 20 Apr 2022 04:36:38 GMT  
		Size: 25.9 MB (25908349 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; 386

```console
$ docker pull debian@sha256:589a7c019a77ef6f47c72807cf92ab71e09d208212db1703fb28b3432722689d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:41e984298ef845f5111c417dd3c3a5446f7fb9f06549018f81be3337421972eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:03 GMT
ADD file:602a25173054242f635a5a299845b7f1b56864ac5d3b8af1ae29dec3a9da119f in / 
# Wed, 20 Apr 2022 07:38:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a13b13d818745c4d2234aab71df26e4a01bfe59f396ab62f20d156b94803650c`  
		Last Modified: Wed, 20 Apr 2022 07:45:46 GMT  
		Size: 27.8 MB (27799825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; mips64le

```console
$ docker pull debian@sha256:af7ca58714f59c0cfc55282e11d3c7aab0ab34d30ae6228b2c962a0c3fa83184
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25818066 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ac7583a1184fcf172d760efca521e0ea2a669111393a826992b236a324657a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:36 GMT
ADD file:7827dc4070f09537c063b825b0b7a5b076fac295f9afa415dee6c5346c38d46b in / 
# Tue, 29 Mar 2022 07:43:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a3e27ca4ec7041c36d0e9cbc672ce565851ce1a0cc90120d7a923a938fb3c18`  
		Last Modified: Tue, 29 Mar 2022 07:54:19 GMT  
		Size: 25.8 MB (25818066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:c9b116e64e9f03088780c4da7456aea50d24047340f18f50e80416173eed96d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98e35f6574d2a25af8e8be68282f6c7fa6b571faa4404e75c09e913d23973469`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:47 GMT
ADD file:69078e7ae6d887f7e36ae048f98f784678fbd8b7a8ec27e3c2bce0d68a47236d in / 
# Wed, 20 Apr 2022 09:47:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aacfa28cbcef999a852ce9c261b67f15f0ccd8450cd1dbfe7b0dc88e6b5aba2`  
		Last Modified: Wed, 20 Apr 2022 09:58:02 GMT  
		Size: 30.6 MB (30560296 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.12-slim` - linux; s390x

```console
$ docker pull debian@sha256:f8a77bc2f84442b3708edeef82c7c787b95aae8a82e746a73a81b2c3e5abe0db
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7716345ae3e3eff8664c28b2abe1284a6e2b186186aede487146ac809fa58cd2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:41 GMT
ADD file:e9840ab2ce3aeff61b68bf4837f65471a86dcba02c5023863f3e7493098b07ba in / 
# Wed, 20 Apr 2022 08:40:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b59fdcf2cb29d2ee4dd08aa08a467372e33525431c4caba9b01e9eb28f76425f`  
		Last Modified: Wed, 20 Apr 2022 08:50:14 GMT  
		Size: 25.8 MB (25758600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11`

```console
$ docker pull debian@sha256:c9cc571d4eee85529d4764aedaa177ea42d2d5cf29e43f69eab2e690f1ec1e11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11` - linux; amd64

```console
$ docker pull debian@sha256:bdf44f19d09b558203306836a612cc8e42a1106b2f731fbeb000e2696c04f9c8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a11311205db1d20ae0c5373473525e45ff36ec83f472b276f283b34173af5513`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v5

```console
$ docker pull debian@sha256:183c9884139c6f80cc2b41e5b6f0832ef7fe1c42a927801b6278a0b6e18d0333
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e0968d51d6984762a26b7bd501cc11518fc8006dd1f78c754ee7e3cdc15c2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v7

```console
$ docker pull debian@sha256:f5965ab2ab9c4163f95c716a5524797dfb61300140e97aaa6821a762d0096c0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133696 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaabdee60d04a8992fcef8727e5d2e99d97fc0efc15c1bf5e28e466360c3033b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3c0acad2a24228465b3273129fe5bd5c3b122925a491f24e0308999fa5532d48
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633096 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f8dd32c4c981fb56702f6effe49f64226ac124415109befc63d15ec19a12dabd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; 386

```console
$ docker pull debian@sha256:ea5a721ee0cf89d1cd7ec768dbdf3d719b37bb3d2d6b4c89b027f9bfa56da966
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940721 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40aa2edee2c7f9abcdcb40d249854f199ec91602a19da6d7d1a645492f233893`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; mips64le

```console
$ docker pull debian@sha256:8495d8e4715051871244692e7b96666ba4af68e25a6cf9d8072f98836c927bc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199386 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0ccd6826906b5b70da7c48a74b23763d1d48a85f2495f3582faa87a95c42116`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:51 GMT
ADD file:40fc6f06ccce55e9889d1c94556c29242d1ac62870cb9895da64d37bac45ab5e in / 
# Tue, 29 Mar 2022 07:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb90a274e1b19eec263e44f7fbc1f9c9c6c393210c27b03ebde7c2b0a2b6482`  
		Last Modified: Tue, 29 Mar 2022 07:52:16 GMT  
		Size: 53.2 MB (53199386 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; ppc64le

```console
$ docker pull debian@sha256:50bc3cfc65c5b7386d0ef3b67512d703d02da0d682e9e346f279ab9922ebe14f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bed7dbfb0379bc4cb4fb0509badb61ec1da3b439a4c0f1e1c2b69acc8693e678`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; s390x

```console
$ docker pull debian@sha256:132c0434d1a999c95433e8bb9bf4f0bc2943d0185b60711d581530769fd5b7e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207374 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f579f688d5503f84dabfbce1af0e85fa391ae288878e09a07b55353d88eb508b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11-slim`

```console
$ docker pull debian@sha256:a25bdade954a2255408020517a980244b272c3863f2e5b48075bd4a78a62cb55
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11-slim` - linux; amd64

```console
$ docker pull debian@sha256:b771c35d1e6ecf2556718ad3c0f481b4a04c1fbc133c609643acc9dd6743ead2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfbec70f84886eb3760616aa408f60fe6f753ef414a0f40938020b673230126b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:27 GMT
ADD file:8b1e79f91081eb527b455431af58e823d8b84d9d0c8e5c47cb7bda7507954ae4 in / 
# Wed, 20 Apr 2022 04:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fe172e4850f03bb45d41a20174112bc119fbfec42a650edbbd8491aee32e3c3`  
		Last Modified: Wed, 20 Apr 2022 04:48:27 GMT  
		Size: 31.4 MB (31378979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:7e99b539841e02be174aa3dca177073bd3c3c183ba473b2aede0ecfc440d5dd5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921085 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03dfd2315bcac484fe4ec9bc76bf58b6644c7087594777d3cd2b0300021a915a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:42 GMT
ADD file:f20c57847025604e1ea1a848af3f090f71d78b5fc2eb2ac795822da94d6e6a0a in / 
# Wed, 20 Apr 2022 07:16:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3edd9d296fa023e00f76828c9d6fd6cc5b03e626341a6a045c40d2ea16c32d08`  
		Last Modified: Wed, 20 Apr 2022 07:32:27 GMT  
		Size: 28.9 MB (28921085 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ea3841109522787a901913924be99040ecbd4ec7a757446dd971f2e7065aba3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575758 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26f43b966e43ab4ea5bcf8dccf3ff2438b2f0a3f40661200716734a2e9a4ef26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:18 GMT
ADD file:1a0e290eb32b3533f9d1684c683ad866b5003579790b25ad61b2044dc6c20bbb in / 
# Wed, 20 Apr 2022 13:27:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6b2963233cd14b47391f0e25f2a485bbeabb87e771a71b73e5cde7c013963da`  
		Last Modified: Wed, 20 Apr 2022 13:43:41 GMT  
		Size: 26.6 MB (26575758 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f82fd419b104813e7ba6374e2e6946d4314e6e8ab90ddd565371ba04b2a61ae6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f21e23dc6e6ed773301f6d2f71ecae4bb3f73c72ec26df347815574871529b9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:10 GMT
ADD file:84523b5820128293b713ef35d8db618921162250da1e4c72bf9923f34ad7d87d in / 
# Wed, 20 Apr 2022 04:29:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d4a449ac69c579312443ded09f57c4894e7adb42f7406abd364f95982fafc59`  
		Last Modified: Wed, 20 Apr 2022 04:35:49 GMT  
		Size: 30.1 MB (30065801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; 386

```console
$ docker pull debian@sha256:82ab01b1fafba70f80af0bec310cb62f0b60bf1558fde388502e6c9cb26e227a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389597 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f06ba4c886838d848a2c08cc8828db043336099af17bf20d66606ebb0a30e4ca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:35 GMT
ADD file:460761e2baaea91893a907ce122ff7d585ef5704f48c03454835986028a1d842 in / 
# Wed, 20 Apr 2022 07:37:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e5c4eb151e267d273883db6e9aee3ff01be07e098ca33f0d1d65bb0e0416921`  
		Last Modified: Wed, 20 Apr 2022 07:44:54 GMT  
		Size: 32.4 MB (32389597 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; mips64le

```console
$ docker pull debian@sha256:0b6b37bb7ea352f1cdc67d0019d6eb6d42a4249dc249419e8707507c21cef6d2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29641474 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b79c0d2777fcb480d3d72c13ad8afd9ffd671ab3d984fa184ab0c28f694dd61`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:42:27 GMT
ADD file:32aa9fd7ee5c64e4bd49459e801e3e5dc50138590bbfca671e336a197aa7fa92 in / 
# Tue, 29 Mar 2022 07:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2a8045f9de06328ab3d0ff505d990892219b7faee393bc9ac342347fc83d04`  
		Last Modified: Tue, 29 Mar 2022 07:52:59 GMT  
		Size: 29.6 MB (29641474 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:75efc4f12ff7ada74b636bef34e96b7905370c19f1ff5b8e7794e8cbed880aca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285249 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d3c24fc64ca70b09590e5a585748e1687cac9cec688c2a07ec4254c37dbf308`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:46:36 GMT
ADD file:8d406ebce4d9b0884d46ee25ec31fe7714726024b80aba9b408d81d39e2f6f8c in / 
# Wed, 20 Apr 2022 09:46:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e0d035960b14409a1ddb839de80aa08677b71addd5e94264ff9ee89a2523e5b`  
		Last Modified: Wed, 20 Apr 2022 09:56:59 GMT  
		Size: 35.3 MB (35285249 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; s390x

```console
$ docker pull debian@sha256:c628fea72069afa77a21e18266c95b24c0e82d73108178cf680915f717b479f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09a58d3a39a373f45e7dab1ae57b01ae6f12de01b1f70b1f83daa474c65cd443`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:37 GMT
ADD file:c3381330644d6d9fc585301395580b564cdf7c880fb2dc2d8e48992673184f7d in / 
# Wed, 20 Apr 2022 08:39:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3179e1eccb473ae22a70e8ac3e9ddb4660ebdfd977989d115bd4d322ecc12b89`  
		Last Modified: Wed, 20 Apr 2022 08:49:39 GMT  
		Size: 29.7 MB (29654805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.3`

```console
$ docker pull debian@sha256:c9cc571d4eee85529d4764aedaa177ea42d2d5cf29e43f69eab2e690f1ec1e11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.3` - linux; amd64

```console
$ docker pull debian@sha256:bdf44f19d09b558203306836a612cc8e42a1106b2f731fbeb000e2696c04f9c8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a11311205db1d20ae0c5373473525e45ff36ec83f472b276f283b34173af5513`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; arm variant v5

```console
$ docker pull debian@sha256:183c9884139c6f80cc2b41e5b6f0832ef7fe1c42a927801b6278a0b6e18d0333
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e0968d51d6984762a26b7bd501cc11518fc8006dd1f78c754ee7e3cdc15c2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; arm variant v7

```console
$ docker pull debian@sha256:f5965ab2ab9c4163f95c716a5524797dfb61300140e97aaa6821a762d0096c0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133696 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaabdee60d04a8992fcef8727e5d2e99d97fc0efc15c1bf5e28e466360c3033b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3c0acad2a24228465b3273129fe5bd5c3b122925a491f24e0308999fa5532d48
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633096 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f8dd32c4c981fb56702f6effe49f64226ac124415109befc63d15ec19a12dabd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; 386

```console
$ docker pull debian@sha256:ea5a721ee0cf89d1cd7ec768dbdf3d719b37bb3d2d6b4c89b027f9bfa56da966
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940721 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40aa2edee2c7f9abcdcb40d249854f199ec91602a19da6d7d1a645492f233893`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; mips64le

```console
$ docker pull debian@sha256:8495d8e4715051871244692e7b96666ba4af68e25a6cf9d8072f98836c927bc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199386 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0ccd6826906b5b70da7c48a74b23763d1d48a85f2495f3582faa87a95c42116`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:51 GMT
ADD file:40fc6f06ccce55e9889d1c94556c29242d1ac62870cb9895da64d37bac45ab5e in / 
# Tue, 29 Mar 2022 07:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb90a274e1b19eec263e44f7fbc1f9c9c6c393210c27b03ebde7c2b0a2b6482`  
		Last Modified: Tue, 29 Mar 2022 07:52:16 GMT  
		Size: 53.2 MB (53199386 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; ppc64le

```console
$ docker pull debian@sha256:50bc3cfc65c5b7386d0ef3b67512d703d02da0d682e9e346f279ab9922ebe14f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bed7dbfb0379bc4cb4fb0509badb61ec1da3b439a4c0f1e1c2b69acc8693e678`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3` - linux; s390x

```console
$ docker pull debian@sha256:132c0434d1a999c95433e8bb9bf4f0bc2943d0185b60711d581530769fd5b7e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207374 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f579f688d5503f84dabfbce1af0e85fa391ae288878e09a07b55353d88eb508b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.3-slim`

```console
$ docker pull debian@sha256:a25bdade954a2255408020517a980244b272c3863f2e5b48075bd4a78a62cb55
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.3-slim` - linux; amd64

```console
$ docker pull debian@sha256:b771c35d1e6ecf2556718ad3c0f481b4a04c1fbc133c609643acc9dd6743ead2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfbec70f84886eb3760616aa408f60fe6f753ef414a0f40938020b673230126b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:27 GMT
ADD file:8b1e79f91081eb527b455431af58e823d8b84d9d0c8e5c47cb7bda7507954ae4 in / 
# Wed, 20 Apr 2022 04:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fe172e4850f03bb45d41a20174112bc119fbfec42a650edbbd8491aee32e3c3`  
		Last Modified: Wed, 20 Apr 2022 04:48:27 GMT  
		Size: 31.4 MB (31378979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:7e99b539841e02be174aa3dca177073bd3c3c183ba473b2aede0ecfc440d5dd5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921085 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03dfd2315bcac484fe4ec9bc76bf58b6644c7087594777d3cd2b0300021a915a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:42 GMT
ADD file:f20c57847025604e1ea1a848af3f090f71d78b5fc2eb2ac795822da94d6e6a0a in / 
# Wed, 20 Apr 2022 07:16:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3edd9d296fa023e00f76828c9d6fd6cc5b03e626341a6a045c40d2ea16c32d08`  
		Last Modified: Wed, 20 Apr 2022 07:32:27 GMT  
		Size: 28.9 MB (28921085 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ea3841109522787a901913924be99040ecbd4ec7a757446dd971f2e7065aba3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575758 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26f43b966e43ab4ea5bcf8dccf3ff2438b2f0a3f40661200716734a2e9a4ef26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:18 GMT
ADD file:1a0e290eb32b3533f9d1684c683ad866b5003579790b25ad61b2044dc6c20bbb in / 
# Wed, 20 Apr 2022 13:27:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6b2963233cd14b47391f0e25f2a485bbeabb87e771a71b73e5cde7c013963da`  
		Last Modified: Wed, 20 Apr 2022 13:43:41 GMT  
		Size: 26.6 MB (26575758 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f82fd419b104813e7ba6374e2e6946d4314e6e8ab90ddd565371ba04b2a61ae6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f21e23dc6e6ed773301f6d2f71ecae4bb3f73c72ec26df347815574871529b9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:10 GMT
ADD file:84523b5820128293b713ef35d8db618921162250da1e4c72bf9923f34ad7d87d in / 
# Wed, 20 Apr 2022 04:29:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d4a449ac69c579312443ded09f57c4894e7adb42f7406abd364f95982fafc59`  
		Last Modified: Wed, 20 Apr 2022 04:35:49 GMT  
		Size: 30.1 MB (30065801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; 386

```console
$ docker pull debian@sha256:82ab01b1fafba70f80af0bec310cb62f0b60bf1558fde388502e6c9cb26e227a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389597 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f06ba4c886838d848a2c08cc8828db043336099af17bf20d66606ebb0a30e4ca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:35 GMT
ADD file:460761e2baaea91893a907ce122ff7d585ef5704f48c03454835986028a1d842 in / 
# Wed, 20 Apr 2022 07:37:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e5c4eb151e267d273883db6e9aee3ff01be07e098ca33f0d1d65bb0e0416921`  
		Last Modified: Wed, 20 Apr 2022 07:44:54 GMT  
		Size: 32.4 MB (32389597 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; mips64le

```console
$ docker pull debian@sha256:0b6b37bb7ea352f1cdc67d0019d6eb6d42a4249dc249419e8707507c21cef6d2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29641474 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b79c0d2777fcb480d3d72c13ad8afd9ffd671ab3d984fa184ab0c28f694dd61`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:42:27 GMT
ADD file:32aa9fd7ee5c64e4bd49459e801e3e5dc50138590bbfca671e336a197aa7fa92 in / 
# Tue, 29 Mar 2022 07:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2a8045f9de06328ab3d0ff505d990892219b7faee393bc9ac342347fc83d04`  
		Last Modified: Tue, 29 Mar 2022 07:52:59 GMT  
		Size: 29.6 MB (29641474 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:75efc4f12ff7ada74b636bef34e96b7905370c19f1ff5b8e7794e8cbed880aca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285249 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d3c24fc64ca70b09590e5a585748e1687cac9cec688c2a07ec4254c37dbf308`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:46:36 GMT
ADD file:8d406ebce4d9b0884d46ee25ec31fe7714726024b80aba9b408d81d39e2f6f8c in / 
# Wed, 20 Apr 2022 09:46:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e0d035960b14409a1ddb839de80aa08677b71addd5e94264ff9ee89a2523e5b`  
		Last Modified: Wed, 20 Apr 2022 09:56:59 GMT  
		Size: 35.3 MB (35285249 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.3-slim` - linux; s390x

```console
$ docker pull debian@sha256:c628fea72069afa77a21e18266c95b24c0e82d73108178cf680915f717b479f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09a58d3a39a373f45e7dab1ae57b01ae6f12de01b1f70b1f83daa474c65cd443`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:37 GMT
ADD file:c3381330644d6d9fc585301395580b564cdf7c880fb2dc2d8e48992673184f7d in / 
# Wed, 20 Apr 2022 08:39:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3179e1eccb473ae22a70e8ac3e9ddb4660ebdfd977989d115bd4d322ecc12b89`  
		Last Modified: Wed, 20 Apr 2022 08:49:39 GMT  
		Size: 29.7 MB (29654805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9`

```console
$ docker pull debian@sha256:cebe6e1c30384958d471467e231f740e8f0fd92cbfd2a435a186e9bada3aee1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9` - linux; amd64

```console
$ docker pull debian@sha256:75383204de7da8eb4bb2724e5fb8dae2910ee78938ade4d35ed8afec19a90b0b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3dbb17ee70fa3383907ebfc75e737e975d73b3172eb433a1a132766aa8a11f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:14 GMT
ADD file:6ed691b65385dede44a92f9de9e1428af431197c66461aa0f9c61e2f7c8bade5 in / 
# Wed, 20 Apr 2022 04:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5196cdf25181bc7e4411865a2e002932b7b6b0ffce787c04c1bdeaf1f204f20`  
		Last Modified: Wed, 20 Apr 2022 04:52:01 GMT  
		Size: 45.4 MB (45427434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm variant v5

```console
$ docker pull debian@sha256:fbf4b8d8ccbfc5b2142939a75df4983c60065732a75393a8b8ce39ca2cf98794
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d74fea7db7861cec6731fee36ad5dc1c8a616414000db685b5d5de374595fd6f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:06 GMT
ADD file:407e28120c3ae0a1012433a9787cf812e0ecc360e63ace9fe21f6728470b4db1 in / 
# Wed, 20 Apr 2022 07:22:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6849d3c2fa1fe7c2c78db1c26ca92e44c119dd3e92d93450115c927ecb6dce6`  
		Last Modified: Wed, 20 Apr 2022 07:39:41 GMT  
		Size: 44.1 MB (44122673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm variant v7

```console
$ docker pull debian@sha256:820547110898f52616039d3876738c0fca8a475be9290a4b77f5abaeb25045a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150830 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43cfe0e985e30b2fb4c56df2258c4f5ee922b7365ded10938ff7c2a7985c69a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:15 GMT
ADD file:d237e5cd139706988e7bbc50ae542898a9b1fa1539f00322f21a71714975ccd8 in / 
# Wed, 20 Apr 2022 13:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef9cca547ca93783f3168dc26ce42494066c74a8d5b51e766ac90a3fb5ad2931`  
		Last Modified: Wed, 20 Apr 2022 13:50:30 GMT  
		Size: 42.2 MB (42150830 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fb3a805a4536a028bbec14eca5ee1e37fca67af1b1ab517d830bdcc3e0db2bcd
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212018 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d1693f5b3dd3cb0a26e7bae2c9190f6cf40aa5134990b06748c551da00579dd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:19 GMT
ADD file:73f1db8536438ca891f4b507a569e6c561364db0f05914ebea9d3fa97e1bf837 in / 
# Wed, 20 Apr 2022 04:31:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a41fbedfa4b1547a2b4fea33de48af745d66a94741d3cf344cb2766f0e80211b`  
		Last Modified: Wed, 20 Apr 2022 04:39:38 GMT  
		Size: 43.2 MB (43212018 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; 386

```console
$ docker pull debian@sha256:29ee5bf0bf0aa44f872f3a7df77ce220c90d3749932b950f65e0b21c332111f2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fcdce55a50efd98b7534c78cde22133b58b6112028b144ef677ef78d1c183883`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:57 GMT
ADD file:e8c6a60834ed931ccc3c93b1f2c9b9ca2bf190b0d8d2474382ee8e96a7e35b5a in / 
# Wed, 20 Apr 2022 07:39:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cd343d4c0c2fbc67ddea33155621bdf36074fc30ffb6f917e716fbb81645f79a`  
		Last Modified: Wed, 20 Apr 2022 07:48:48 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9-slim`

```console
$ docker pull debian@sha256:e317b647e7b7df5229116215896f591917b95a8be9a0ba0ccfec83feff3861c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9-slim` - linux; amd64

```console
$ docker pull debian@sha256:ef76a14f299ecc38bc00cd075f230dc69f8ff5aaad07a13b7cfb92fd3601e7de
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567235 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd3475f2eda5b0088a2164878c70031d5ae6d0304580f86449d867934fd6e8e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:25 GMT
ADD file:0fafb7f95dc18699227154856361058bc9aba65de2ff88bf55cc66363142a05e in / 
# Wed, 20 Apr 2022 04:45:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8bd3f5a20b90c0e84cc65ea4a9172a51b7b5c0b9a097fa88d7102cac91cddf2c`  
		Last Modified: Wed, 20 Apr 2022 04:52:27 GMT  
		Size: 22.6 MB (22567235 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:371cd0b21fab642901d9e51dd04f095571c2b85eef693a0c209c376f722340fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240552 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37e0f3d66f25288d6c7cb7e8ee7487a8a098e33df878873ea22a96bf541e723d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:40 GMT
ADD file:8caa5e7ca2ab4b420bed155bd2190baa73ca3c4e9378db02b0c0cede15974d69 in / 
# Wed, 20 Apr 2022 07:22:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d96febf107fb8f8f8c5d8bd55514488bd2c5f9ca3c49cb1c015381d7aa816d6`  
		Last Modified: Wed, 20 Apr 2022 07:40:19 GMT  
		Size: 21.2 MB (21240552 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ea27ab375b847954717cfeff2736d39679501579d38b3dc2f71ae6254735a64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:898e8f79b5495e8f2c00b96f8c438b474227379a436b9f510cfda019b8c753a6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:51 GMT
ADD file:096692331564a30a9a9c6b24e681d99d68b54eeeabb7c5ac7d4073de06e050b2 in / 
# Wed, 20 Apr 2022 13:33:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08da560f0b65b004ba95f9a431d3ac51b8807b16ec6ef170e2e00dc55330246d`  
		Last Modified: Wed, 20 Apr 2022 13:51:07 GMT  
		Size: 19.4 MB (19359807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:13a6a318bc4e4200435e0f3ac16c65e3a60fa45511a472bd47e81c5bac41710b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3e0d7663d4764771c2ba52276e17e355f57961f2cd98863d1b29ebc3bf0f52e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:34 GMT
ADD file:02ad9214474cf0e6d278d7015dee30e6a8e24cdc699812f5fb3915491177e92b in / 
# Wed, 20 Apr 2022 04:31:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fca491399855be4c9b780dca621b48440831235d7916e5695b876698df84db7`  
		Last Modified: Wed, 20 Apr 2022 04:40:05 GMT  
		Size: 20.4 MB (20424115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; 386

```console
$ docker pull debian@sha256:33d586c9ff1c3dee1135a843b10b7c620107ef269e3ae83aa2b9d88719b91a30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b77216ff0bf0b007fd5cac2c9b36dcc3cb0fa2469d6d198dbffff8270facf9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:12 GMT
ADD file:f1394937f911449ed9fb7103f2bfb5b299d7779e98c4f0f34bcee08f8c2bf5db in / 
# Wed, 20 Apr 2022 07:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da9db5d720427742cbccdd8a4a37dfaa81873ab9db6de4935623ca79ce881ee4`  
		Last Modified: Wed, 20 Apr 2022 07:49:16 GMT  
		Size: 23.2 MB (23202179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9.13`

```console
$ docker pull debian@sha256:cebe6e1c30384958d471467e231f740e8f0fd92cbfd2a435a186e9bada3aee1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9.13` - linux; amd64

```console
$ docker pull debian@sha256:75383204de7da8eb4bb2724e5fb8dae2910ee78938ade4d35ed8afec19a90b0b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3dbb17ee70fa3383907ebfc75e737e975d73b3172eb433a1a132766aa8a11f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:14 GMT
ADD file:6ed691b65385dede44a92f9de9e1428af431197c66461aa0f9c61e2f7c8bade5 in / 
# Wed, 20 Apr 2022 04:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5196cdf25181bc7e4411865a2e002932b7b6b0ffce787c04c1bdeaf1f204f20`  
		Last Modified: Wed, 20 Apr 2022 04:52:01 GMT  
		Size: 45.4 MB (45427434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm variant v5

```console
$ docker pull debian@sha256:fbf4b8d8ccbfc5b2142939a75df4983c60065732a75393a8b8ce39ca2cf98794
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d74fea7db7861cec6731fee36ad5dc1c8a616414000db685b5d5de374595fd6f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:06 GMT
ADD file:407e28120c3ae0a1012433a9787cf812e0ecc360e63ace9fe21f6728470b4db1 in / 
# Wed, 20 Apr 2022 07:22:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6849d3c2fa1fe7c2c78db1c26ca92e44c119dd3e92d93450115c927ecb6dce6`  
		Last Modified: Wed, 20 Apr 2022 07:39:41 GMT  
		Size: 44.1 MB (44122673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm variant v7

```console
$ docker pull debian@sha256:820547110898f52616039d3876738c0fca8a475be9290a4b77f5abaeb25045a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150830 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43cfe0e985e30b2fb4c56df2258c4f5ee922b7365ded10938ff7c2a7985c69a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:15 GMT
ADD file:d237e5cd139706988e7bbc50ae542898a9b1fa1539f00322f21a71714975ccd8 in / 
# Wed, 20 Apr 2022 13:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef9cca547ca93783f3168dc26ce42494066c74a8d5b51e766ac90a3fb5ad2931`  
		Last Modified: Wed, 20 Apr 2022 13:50:30 GMT  
		Size: 42.2 MB (42150830 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fb3a805a4536a028bbec14eca5ee1e37fca67af1b1ab517d830bdcc3e0db2bcd
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212018 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d1693f5b3dd3cb0a26e7bae2c9190f6cf40aa5134990b06748c551da00579dd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:19 GMT
ADD file:73f1db8536438ca891f4b507a569e6c561364db0f05914ebea9d3fa97e1bf837 in / 
# Wed, 20 Apr 2022 04:31:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a41fbedfa4b1547a2b4fea33de48af745d66a94741d3cf344cb2766f0e80211b`  
		Last Modified: Wed, 20 Apr 2022 04:39:38 GMT  
		Size: 43.2 MB (43212018 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; 386

```console
$ docker pull debian@sha256:29ee5bf0bf0aa44f872f3a7df77ce220c90d3749932b950f65e0b21c332111f2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fcdce55a50efd98b7534c78cde22133b58b6112028b144ef677ef78d1c183883`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:57 GMT
ADD file:e8c6a60834ed931ccc3c93b1f2c9b9ca2bf190b0d8d2474382ee8e96a7e35b5a in / 
# Wed, 20 Apr 2022 07:39:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cd343d4c0c2fbc67ddea33155621bdf36074fc30ffb6f917e716fbb81645f79a`  
		Last Modified: Wed, 20 Apr 2022 07:48:48 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9.13-slim`

```console
$ docker pull debian@sha256:e317b647e7b7df5229116215896f591917b95a8be9a0ba0ccfec83feff3861c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9.13-slim` - linux; amd64

```console
$ docker pull debian@sha256:ef76a14f299ecc38bc00cd075f230dc69f8ff5aaad07a13b7cfb92fd3601e7de
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567235 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd3475f2eda5b0088a2164878c70031d5ae6d0304580f86449d867934fd6e8e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:25 GMT
ADD file:0fafb7f95dc18699227154856361058bc9aba65de2ff88bf55cc66363142a05e in / 
# Wed, 20 Apr 2022 04:45:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8bd3f5a20b90c0e84cc65ea4a9172a51b7b5c0b9a097fa88d7102cac91cddf2c`  
		Last Modified: Wed, 20 Apr 2022 04:52:27 GMT  
		Size: 22.6 MB (22567235 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:371cd0b21fab642901d9e51dd04f095571c2b85eef693a0c209c376f722340fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240552 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37e0f3d66f25288d6c7cb7e8ee7487a8a098e33df878873ea22a96bf541e723d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:40 GMT
ADD file:8caa5e7ca2ab4b420bed155bd2190baa73ca3c4e9378db02b0c0cede15974d69 in / 
# Wed, 20 Apr 2022 07:22:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d96febf107fb8f8f8c5d8bd55514488bd2c5f9ca3c49cb1c015381d7aa816d6`  
		Last Modified: Wed, 20 Apr 2022 07:40:19 GMT  
		Size: 21.2 MB (21240552 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ea27ab375b847954717cfeff2736d39679501579d38b3dc2f71ae6254735a64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:898e8f79b5495e8f2c00b96f8c438b474227379a436b9f510cfda019b8c753a6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:51 GMT
ADD file:096692331564a30a9a9c6b24e681d99d68b54eeeabb7c5ac7d4073de06e050b2 in / 
# Wed, 20 Apr 2022 13:33:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08da560f0b65b004ba95f9a431d3ac51b8807b16ec6ef170e2e00dc55330246d`  
		Last Modified: Wed, 20 Apr 2022 13:51:07 GMT  
		Size: 19.4 MB (19359807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:13a6a318bc4e4200435e0f3ac16c65e3a60fa45511a472bd47e81c5bac41710b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3e0d7663d4764771c2ba52276e17e355f57961f2cd98863d1b29ebc3bf0f52e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:34 GMT
ADD file:02ad9214474cf0e6d278d7015dee30e6a8e24cdc699812f5fb3915491177e92b in / 
# Wed, 20 Apr 2022 04:31:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fca491399855be4c9b780dca621b48440831235d7916e5695b876698df84db7`  
		Last Modified: Wed, 20 Apr 2022 04:40:05 GMT  
		Size: 20.4 MB (20424115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; 386

```console
$ docker pull debian@sha256:33d586c9ff1c3dee1135a843b10b7c620107ef269e3ae83aa2b9d88719b91a30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b77216ff0bf0b007fd5cac2c9b36dcc3cb0fa2469d6d198dbffff8270facf9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:12 GMT
ADD file:f1394937f911449ed9fb7103f2bfb5b299d7779e98c4f0f34bcee08f8c2bf5db in / 
# Wed, 20 Apr 2022 07:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da9db5d720427742cbccdd8a4a37dfaa81873ab9db6de4935623ca79ce881ee4`  
		Last Modified: Wed, 20 Apr 2022 07:49:16 GMT  
		Size: 23.2 MB (23202179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm`

```console
$ docker pull debian@sha256:2c44fc330a2275fcd147e584af781d18acb308dc9d725ff88c40a07c97b90bf1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm` - linux; amd64

```console
$ docker pull debian@sha256:4b5eda06c1020b4b0c34b605a3d778b5717ed877ba0eae8a21b7c1374f8a65c1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55578729 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1e4cfbdda3e54c1a32f89e91ee1fd5b883380d60adf9a6d00b264421a31f3df6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:42:54 GMT
ADD file:b5180fc4d45b984afa69f3cdfa95980bcdfd76d75f704f74f1aa60e416272f1a in / 
# Wed, 20 Apr 2022 04:42:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cb01d170a54a4c7fd7c1969e79df0a453468ebabfe1d860b863f7a3b6fbbc2f`  
		Last Modified: Wed, 20 Apr 2022 04:47:18 GMT  
		Size: 55.6 MB (55578729 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v5

```console
$ docker pull debian@sha256:346cee1f8607c6b31fc7ab88a2074fce5ef6139ac40b3cf1290b16927cf63622
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001097 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11e387f33656be6173839570e483dcc4d83f98e7519a45ce1e30d89c4bfae865`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:15:05 GMT
ADD file:e283a0d8ab69b94018679c485439a2b5700bcbd73218180b0a334a00df340093 in / 
# Wed, 20 Apr 2022 07:15:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e7b4944a983e7a423568be713ae200d96e80ca1893c45b7758ec97be00ab710b`  
		Last Modified: Wed, 20 Apr 2022 07:30:20 GMT  
		Size: 53.0 MB (53001097 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v7

```console
$ docker pull debian@sha256:a9f2ab5ce0373efa08cd8851a2ab89ac04be233e84640fa66f82d8d1c31b5d0c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620332 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a83829fa6208d45cc4dc234d977ead06b2f43606620a268844682d48f278e3a7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:25:33 GMT
ADD file:b489c04ffff2353f6ceea0c110a1e81cc311de6662b48af844e74c82fc7b8155 in / 
# Wed, 20 Apr 2022 13:25:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:092c836e0e284efdd221da0378b0c96d6671f0ecf169a2e2a6c40b20606d657b`  
		Last Modified: Wed, 20 Apr 2022 13:41:43 GMT  
		Size: 50.6 MB (50620332 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e5b1c175555f065f1fac70bddf8c50fd51d91ec2415ec15029552d7c8983a0e2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493289 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a9d35c53f2d7d9f5338304831cf80d66fcd1bbef1f8aa0657ce92e66c75c966`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:29 GMT
ADD file:23397ad30e34521aa2c0881ab09c9c75f58316594a1c14f01cfcb212161c32fc in / 
# Wed, 20 Apr 2022 04:28:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6d996fb7c2fde9d95056213e2cc0a2ea025cc86ce1ceeac46b8c51b2d458d84`  
		Last Modified: Wed, 20 Apr 2022 04:34:39 GMT  
		Size: 54.5 MB (54493289 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; 386

```console
$ docker pull debian@sha256:ce1516ffeff8b8d2c903e50edfd9cd6be0101d36b32f143d1ea44e6ccd029d24
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:410b2e6a2d7a7e1941650fe0398546da35c325709f28b1689055f4047b671054`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:36:51 GMT
ADD file:469d404c80a1f5720a0a00e4cd116adfd490349ede3a368fae44c6409add4819 in / 
# Wed, 20 Apr 2022 07:36:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c3d3d77c2c71a18bcd4eead6143eb6d36f5096a31d00bfdb4cbd88058ba444c8`  
		Last Modified: Wed, 20 Apr 2022 07:43:35 GMT  
		Size: 56.6 MB (56624540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; mips64le

```console
$ docker pull debian@sha256:56589d9ca94dfb5591a3dc8347c981cdd2222549a3204d16382181c6d19fea67
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54240334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e461bdc5aad3aa8b7373324f58f4d4b899efb7272872147fa2b330b3dcf03185`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:40:39 GMT
ADD file:88e75745fdbfd0784c1c18134a0a52a5534df18201a6f49555a5c618b4531804 in / 
# Tue, 29 Mar 2022 07:40:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bdc3f4c024aea887030bcf477ee808da6aa3e6683fb111845d94ab4f2480ff24`  
		Last Modified: Tue, 29 Mar 2022 07:50:58 GMT  
		Size: 54.2 MB (54240334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; ppc64le

```console
$ docker pull debian@sha256:91382f1288e5a119111a7cf3161e1e5b4f3e32609e4428fedafffa3de312f997
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982654 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e39df5abaec0fde5413c004606bb89cdfb4207605787d91596a794518e82c0b8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:44:36 GMT
ADD file:b40661820352e5ff94b93927c06b3375d894c06269350d442169c7a7b3952388 in / 
# Wed, 20 Apr 2022 09:44:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9f7d9b4a813a5ec1ec0b3475149a2c942961debb65bc15ebf43901246ca6a9a3`  
		Last Modified: Wed, 20 Apr 2022 09:55:28 GMT  
		Size: 60.0 MB (59982654 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; s390x

```console
$ docker pull debian@sha256:b72a041fdd537e357e72c8654ed1b6d17dbbde959db62ad6a13e13018c654493
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de83dfeb97a642ffbde0a5e9302ca21c10344b409b1daeb9c80c0e49e55491e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:37:57 GMT
ADD file:dd66488219818707ab57e2b9e87dcec876513326c64a733dcf95396ad5f22cd9 in / 
# Wed, 20 Apr 2022 08:38:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b24c47356e8597f2e75ec110237cb3350c6d41c44b019ed531f1d9069ab1a82`  
		Last Modified: Wed, 20 Apr 2022 08:48:48 GMT  
		Size: 53.8 MB (53813432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20220418`

```console
$ docker pull debian@sha256:c039b8a93753ba2e7e97b66168cb32b152c35706cd9ba48fcd35bebb4637f3ab
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20220418` - linux; amd64

```console
$ docker pull debian@sha256:4b5eda06c1020b4b0c34b605a3d778b5717ed877ba0eae8a21b7c1374f8a65c1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55578729 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1e4cfbdda3e54c1a32f89e91ee1fd5b883380d60adf9a6d00b264421a31f3df6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:42:54 GMT
ADD file:b5180fc4d45b984afa69f3cdfa95980bcdfd76d75f704f74f1aa60e416272f1a in / 
# Wed, 20 Apr 2022 04:42:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cb01d170a54a4c7fd7c1969e79df0a453468ebabfe1d860b863f7a3b6fbbc2f`  
		Last Modified: Wed, 20 Apr 2022 04:47:18 GMT  
		Size: 55.6 MB (55578729 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:346cee1f8607c6b31fc7ab88a2074fce5ef6139ac40b3cf1290b16927cf63622
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001097 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11e387f33656be6173839570e483dcc4d83f98e7519a45ce1e30d89c4bfae865`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:15:05 GMT
ADD file:e283a0d8ab69b94018679c485439a2b5700bcbd73218180b0a334a00df340093 in / 
# Wed, 20 Apr 2022 07:15:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e7b4944a983e7a423568be713ae200d96e80ca1893c45b7758ec97be00ab710b`  
		Last Modified: Wed, 20 Apr 2022 07:30:20 GMT  
		Size: 53.0 MB (53001097 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:a9f2ab5ce0373efa08cd8851a2ab89ac04be233e84640fa66f82d8d1c31b5d0c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620332 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a83829fa6208d45cc4dc234d977ead06b2f43606620a268844682d48f278e3a7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:25:33 GMT
ADD file:b489c04ffff2353f6ceea0c110a1e81cc311de6662b48af844e74c82fc7b8155 in / 
# Wed, 20 Apr 2022 13:25:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:092c836e0e284efdd221da0378b0c96d6671f0ecf169a2e2a6c40b20606d657b`  
		Last Modified: Wed, 20 Apr 2022 13:41:43 GMT  
		Size: 50.6 MB (50620332 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e5b1c175555f065f1fac70bddf8c50fd51d91ec2415ec15029552d7c8983a0e2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493289 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a9d35c53f2d7d9f5338304831cf80d66fcd1bbef1f8aa0657ce92e66c75c966`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:29 GMT
ADD file:23397ad30e34521aa2c0881ab09c9c75f58316594a1c14f01cfcb212161c32fc in / 
# Wed, 20 Apr 2022 04:28:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6d996fb7c2fde9d95056213e2cc0a2ea025cc86ce1ceeac46b8c51b2d458d84`  
		Last Modified: Wed, 20 Apr 2022 04:34:39 GMT  
		Size: 54.5 MB (54493289 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; 386

```console
$ docker pull debian@sha256:ce1516ffeff8b8d2c903e50edfd9cd6be0101d36b32f143d1ea44e6ccd029d24
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:410b2e6a2d7a7e1941650fe0398546da35c325709f28b1689055f4047b671054`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:36:51 GMT
ADD file:469d404c80a1f5720a0a00e4cd116adfd490349ede3a368fae44c6409add4819 in / 
# Wed, 20 Apr 2022 07:36:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c3d3d77c2c71a18bcd4eead6143eb6d36f5096a31d00bfdb4cbd88058ba444c8`  
		Last Modified: Wed, 20 Apr 2022 07:43:35 GMT  
		Size: 56.6 MB (56624540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:91382f1288e5a119111a7cf3161e1e5b4f3e32609e4428fedafffa3de312f997
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982654 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e39df5abaec0fde5413c004606bb89cdfb4207605787d91596a794518e82c0b8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:44:36 GMT
ADD file:b40661820352e5ff94b93927c06b3375d894c06269350d442169c7a7b3952388 in / 
# Wed, 20 Apr 2022 09:44:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9f7d9b4a813a5ec1ec0b3475149a2c942961debb65bc15ebf43901246ca6a9a3`  
		Last Modified: Wed, 20 Apr 2022 09:55:28 GMT  
		Size: 60.0 MB (59982654 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418` - linux; s390x

```console
$ docker pull debian@sha256:b72a041fdd537e357e72c8654ed1b6d17dbbde959db62ad6a13e13018c654493
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de83dfeb97a642ffbde0a5e9302ca21c10344b409b1daeb9c80c0e49e55491e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:37:57 GMT
ADD file:dd66488219818707ab57e2b9e87dcec876513326c64a733dcf95396ad5f22cd9 in / 
# Wed, 20 Apr 2022 08:38:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b24c47356e8597f2e75ec110237cb3350c6d41c44b019ed531f1d9069ab1a82`  
		Last Modified: Wed, 20 Apr 2022 08:48:48 GMT  
		Size: 53.8 MB (53813432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20220418-slim`

```console
$ docker pull debian@sha256:20bddf433a9d5b5d943c7e15e06944d7192dd7ad05d1cc2dd3b41be73255d5e9
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:071c9166330f9512b864772d2eb76e8a671af41ab926dae8d193ea8be2d2e3d3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31993245 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9548e22a990d47195d4cdf663594da9f8196dca10341f5b6c392c4de6c0c3b5b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:06 GMT
ADD file:c7e7681ab722883f8b3c7efdb189f040e3153614056e18c151aa784988408394 in / 
# Wed, 20 Apr 2022 04:43:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fbc00f9dc92991c646a173e8f7d87a80938ae0ffc53414ef568db1951dcb0298`  
		Last Modified: Wed, 20 Apr 2022 04:47:40 GMT  
		Size: 32.0 MB (31993245 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2785152737ba27e27af6375c52aae211d2c07689b0a9e0676f2d622c6e89c246
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.4 MB (29416618 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e0370330d3bdd55f8dd88472cbb4472c3310cd604ebb8fcbf82db05329724c36`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:15:38 GMT
ADD file:be96a02b9a3b55e3c6b17b1aac493f3629b7aaa6cdf78286de52622da2ab9a42 in / 
# Wed, 20 Apr 2022 07:15:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:acfef501855cc9121e8aa70da56cb7a1c2be842720f5b23af6df5149cc8ec726`  
		Last Modified: Wed, 20 Apr 2022 07:30:58 GMT  
		Size: 29.4 MB (29416618 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:60285e3c97c92d01741818fb8c10ada710051eb6a69bf6156e97693b3ede5c67
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (27035008 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:088bd10d17f91da6bbf9f390caea6db375d055cf6b8e19a4d29c9b16a4ba8112`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:09 GMT
ADD file:1e4988c09fec9701f6e0ef78cbb41f7b3638fe33f163e6f970f71b2dd9f04379 in / 
# Wed, 20 Apr 2022 13:26:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4e0365a5c5b7b5d061ed3b224488ae34059b6adf1cc4f8a6ef51d0a18310d506`  
		Last Modified: Wed, 20 Apr 2022 13:42:16 GMT  
		Size: 27.0 MB (27035008 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:28401b676cbed133c4829ee6ca6c05367b2e84c54f30b043c3703388e10b4262
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30914378 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:422efdc51f3bf577af529e17d978b57afe52ec927a0214cbf0dd03a27b975e50`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:45 GMT
ADD file:fad132548f579e1f4d6e182541f6cd3f5ed2b0926db0c4222637e7cb85a281ce in / 
# Wed, 20 Apr 2022 04:28:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f20082961cd0afe1f32085e3fb57ecc6ddba529c7257038b18ef1be5fb6ad4a6`  
		Last Modified: Wed, 20 Apr 2022 04:35:01 GMT  
		Size: 30.9 MB (30914378 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:3e0ee389f7d571d50cda11d6b09476287f92fa8559f187f11daba186c95de31c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.0 MB (33042725 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b498e1d8a2589999ad4212b6314585215c9b15bc88fd726814d5ca0373b54b93`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:06 GMT
ADD file:54c5c2c45ba283d3327db71638b428e7b853a447b4346bd859fb08d2c1c73013 in / 
# Wed, 20 Apr 2022 07:37:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e30ff5d8bdd1d3bd986d4f9a9d338966465675dc5b4ae5993edf3fccc28255d6`  
		Last Modified: Wed, 20 Apr 2022 07:44:02 GMT  
		Size: 33.0 MB (33042725 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:835ae5f0581734feb7d92ffed04a8491c9baec30ba17dbd1aafbff039bfe90b8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.4 MB (36403434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b32491f0985700475b6155824f911e7edf9e50d6ed5a5a8ad94764416a4952fe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:22 GMT
ADD file:f14832beca200b9a57cc4e3f081a4b00e2bdc2bd93ebb7c945e750fc8f4eb12f in / 
# Wed, 20 Apr 2022 09:45:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:473c0e8a1f0d1c28e6fdddf6060fa42d9aa96cb54bb84ca22136d08de18821df`  
		Last Modified: Wed, 20 Apr 2022 09:55:57 GMT  
		Size: 36.4 MB (36403434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:0b74b990a448ba103eca6c071f0ee87d1ece0615e2c61439f2359b764adbf39c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.2 MB (30229876 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f47710a64a89396e2e9d3f7361fe34cd82b4fb97c961e360ae073e1b24c6f2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:38:30 GMT
ADD file:69023645ee8445197318ee6f8248a13eac1778ee18384794bd72239307a242df in / 
# Wed, 20 Apr 2022 08:38:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9921a7bbd10afaaa8670c971b54cef6ef06c7b9497b863feec94900d5b95ed01`  
		Last Modified: Wed, 20 Apr 2022 08:49:04 GMT  
		Size: 30.2 MB (30229876 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-backports`

```console
$ docker pull debian@sha256:d9841d83fa2644de0f05eb4a0458c78696ad02a204aa5a459fbf82d11b77f029
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-backports` - linux; amd64

```console
$ docker pull debian@sha256:b1f28eaf2b0f6e1c4fcb2de8d2e55f1500310cd6f8c702a34add85c9ab3031d8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55578956 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:69efe536855be0685b2bb92a85e7729b27558103d276a9517abee930f368caef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:42:54 GMT
ADD file:b5180fc4d45b984afa69f3cdfa95980bcdfd76d75f704f74f1aa60e416272f1a in / 
# Wed, 20 Apr 2022 04:42:54 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:42:58 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8cb01d170a54a4c7fd7c1969e79df0a453468ebabfe1d860b863f7a3b6fbbc2f`  
		Last Modified: Wed, 20 Apr 2022 04:47:18 GMT  
		Size: 55.6 MB (55578729 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:02313a896ab9477f41a3f1fdb81b645fc2ff3eeebe183d0249e0f7a7ff945c17`  
		Last Modified: Wed, 20 Apr 2022 04:47:28 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:f9aa794f100bb865ba0671b9e7fa5472c5b98cf48542a2cea87a70e3711088e6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001322 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb9b70998c55de1abfc130a0e3a9c2f2912467dec649f4cb8fed40f243f41058`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:15:05 GMT
ADD file:e283a0d8ab69b94018679c485439a2b5700bcbd73218180b0a334a00df340093 in / 
# Wed, 20 Apr 2022 07:15:06 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:15:18 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e7b4944a983e7a423568be713ae200d96e80ca1893c45b7758ec97be00ab710b`  
		Last Modified: Wed, 20 Apr 2022 07:30:20 GMT  
		Size: 53.0 MB (53001097 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:425c158578cc53fa219a5b15e47609f914eb51269df1330bd97ad98a76b6dfc3`  
		Last Modified: Wed, 20 Apr 2022 07:30:32 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:a286988dece8d08430f4b0d6438c85b920d51e90713ff02375d2978abf417e3f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620557 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7566543b7d166a80bffab8d1a5265478e06fa7194706e54c3149934cb146a983`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:25:33 GMT
ADD file:b489c04ffff2353f6ceea0c110a1e81cc311de6662b48af844e74c82fc7b8155 in / 
# Wed, 20 Apr 2022 13:25:35 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:25:47 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:092c836e0e284efdd221da0378b0c96d6671f0ecf169a2e2a6c40b20606d657b`  
		Last Modified: Wed, 20 Apr 2022 13:41:43 GMT  
		Size: 50.6 MB (50620332 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d80ef599294305b947a3a3f481e68819b7650b6245cf68a6f597b01537dc5a81`  
		Last Modified: Wed, 20 Apr 2022 13:41:54 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a7ce268a65810bc0a624aa6f8c2252b1898767a3b9951cad9258803b3a366919
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493515 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bac6fdfbac43d0d5177740962c16caac4303ff648ed775e86bd445f839dba1ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:29 GMT
ADD file:23397ad30e34521aa2c0881ab09c9c75f58316594a1c14f01cfcb212161c32fc in / 
# Wed, 20 Apr 2022 04:28:30 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:28:36 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c6d996fb7c2fde9d95056213e2cc0a2ea025cc86ce1ceeac46b8c51b2d458d84`  
		Last Modified: Wed, 20 Apr 2022 04:34:39 GMT  
		Size: 54.5 MB (54493289 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bbf32b5d72b6a2412051bc228e2f98ceb6276b3c68dcfaf7b951d518ae387c31`  
		Last Modified: Wed, 20 Apr 2022 04:34:49 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; 386

```console
$ docker pull debian@sha256:0ca5c9c0fefccf8f4879086d2d8fedb0a7561a5b6da6a9da69f6ff261bb3050f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624764 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:90185cee737e64c059da56cd54770d2893dbe87ab190ad0bad25088bb47a0729`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:36:51 GMT
ADD file:469d404c80a1f5720a0a00e4cd116adfd490349ede3a368fae44c6409add4819 in / 
# Wed, 20 Apr 2022 07:36:52 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:36:58 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c3d3d77c2c71a18bcd4eead6143eb6d36f5096a31d00bfdb4cbd88058ba444c8`  
		Last Modified: Wed, 20 Apr 2022 07:43:35 GMT  
		Size: 56.6 MB (56624540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e438fe680fd64a42ebf4a25e6e6e716baa6faaa6e3296b8cf3fbe8567a303450`  
		Last Modified: Wed, 20 Apr 2022 07:43:46 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; mips64le

```console
$ docker pull debian@sha256:718685db3cd4f848161727a37e8452a499e8ebb8fdbc4bd98662e8a9085c0f5d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54240562 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:595d136793bb4f998f309a5990bb19fbbd7014f1738ce8775e136c6a0c99c17b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:40:39 GMT
ADD file:88e75745fdbfd0784c1c18134a0a52a5534df18201a6f49555a5c618b4531804 in / 
# Tue, 29 Mar 2022 07:40:44 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:40:55 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:bdc3f4c024aea887030bcf477ee808da6aa3e6683fb111845d94ab4f2480ff24`  
		Last Modified: Tue, 29 Mar 2022 07:50:58 GMT  
		Size: 54.2 MB (54240334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:43c073b220886baed5bbefb457357d80edad91c1cb4c1f6285abd66f85bcfa6d`  
		Last Modified: Tue, 29 Mar 2022 07:51:08 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:2cd7223e27cf66a3c8792c95aae14e23bd260e64c1efcd14579e9b7aafafe48d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982881 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:940d95447bba21279b3c6f0f5d6cec558c36821bbb754071e5c3a0cda6a65c25`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:44:36 GMT
ADD file:b40661820352e5ff94b93927c06b3375d894c06269350d442169c7a7b3952388 in / 
# Wed, 20 Apr 2022 09:44:42 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:45:08 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:9f7d9b4a813a5ec1ec0b3475149a2c942961debb65bc15ebf43901246ca6a9a3`  
		Last Modified: Wed, 20 Apr 2022 09:55:28 GMT  
		Size: 60.0 MB (59982654 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d5f5e48829c886141228a6d3cd15ccd8091ce900fb2ba7f1018c907181b192ae`  
		Last Modified: Wed, 20 Apr 2022 09:55:41 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; s390x

```console
$ docker pull debian@sha256:152f1c32652e2647df5621f0e756f2ff8f756af222ac1169d99d0c42cf883843
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813658 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a5f8e6acd8eee5a1459a198c28b9a985434eaaf11159c198963ddf489606db08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:37:57 GMT
ADD file:dd66488219818707ab57e2b9e87dcec876513326c64a733dcf95396ad5f22cd9 in / 
# Wed, 20 Apr 2022 08:38:03 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:38:12 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2b24c47356e8597f2e75ec110237cb3350c6d41c44b019ed531f1d9069ab1a82`  
		Last Modified: Wed, 20 Apr 2022 08:48:48 GMT  
		Size: 53.8 MB (53813432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:36a403ace6777bce26af1254889ec8d1e1ea0df2437dc2ce68f3d784caaa4c9c`  
		Last Modified: Wed, 20 Apr 2022 08:48:56 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-slim`

```console
$ docker pull debian@sha256:45b4aeda768241d0469cf3e02e3ed41aeee3454b739cbcf544e6a1d689f47e43
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-slim` - linux; amd64

```console
$ docker pull debian@sha256:071c9166330f9512b864772d2eb76e8a671af41ab926dae8d193ea8be2d2e3d3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31993245 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9548e22a990d47195d4cdf663594da9f8196dca10341f5b6c392c4de6c0c3b5b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:06 GMT
ADD file:c7e7681ab722883f8b3c7efdb189f040e3153614056e18c151aa784988408394 in / 
# Wed, 20 Apr 2022 04:43:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fbc00f9dc92991c646a173e8f7d87a80938ae0ffc53414ef568db1951dcb0298`  
		Last Modified: Wed, 20 Apr 2022 04:47:40 GMT  
		Size: 32.0 MB (31993245 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2785152737ba27e27af6375c52aae211d2c07689b0a9e0676f2d622c6e89c246
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.4 MB (29416618 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e0370330d3bdd55f8dd88472cbb4472c3310cd604ebb8fcbf82db05329724c36`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:15:38 GMT
ADD file:be96a02b9a3b55e3c6b17b1aac493f3629b7aaa6cdf78286de52622da2ab9a42 in / 
# Wed, 20 Apr 2022 07:15:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:acfef501855cc9121e8aa70da56cb7a1c2be842720f5b23af6df5149cc8ec726`  
		Last Modified: Wed, 20 Apr 2022 07:30:58 GMT  
		Size: 29.4 MB (29416618 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:60285e3c97c92d01741818fb8c10ada710051eb6a69bf6156e97693b3ede5c67
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (27035008 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:088bd10d17f91da6bbf9f390caea6db375d055cf6b8e19a4d29c9b16a4ba8112`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:09 GMT
ADD file:1e4988c09fec9701f6e0ef78cbb41f7b3638fe33f163e6f970f71b2dd9f04379 in / 
# Wed, 20 Apr 2022 13:26:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4e0365a5c5b7b5d061ed3b224488ae34059b6adf1cc4f8a6ef51d0a18310d506`  
		Last Modified: Wed, 20 Apr 2022 13:42:16 GMT  
		Size: 27.0 MB (27035008 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:28401b676cbed133c4829ee6ca6c05367b2e84c54f30b043c3703388e10b4262
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30914378 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:422efdc51f3bf577af529e17d978b57afe52ec927a0214cbf0dd03a27b975e50`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:45 GMT
ADD file:fad132548f579e1f4d6e182541f6cd3f5ed2b0926db0c4222637e7cb85a281ce in / 
# Wed, 20 Apr 2022 04:28:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f20082961cd0afe1f32085e3fb57ecc6ddba529c7257038b18ef1be5fb6ad4a6`  
		Last Modified: Wed, 20 Apr 2022 04:35:01 GMT  
		Size: 30.9 MB (30914378 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; 386

```console
$ docker pull debian@sha256:3e0ee389f7d571d50cda11d6b09476287f92fa8559f187f11daba186c95de31c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.0 MB (33042725 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b498e1d8a2589999ad4212b6314585215c9b15bc88fd726814d5ca0373b54b93`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:06 GMT
ADD file:54c5c2c45ba283d3327db71638b428e7b853a447b4346bd859fb08d2c1c73013 in / 
# Wed, 20 Apr 2022 07:37:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e30ff5d8bdd1d3bd986d4f9a9d338966465675dc5b4ae5993edf3fccc28255d6`  
		Last Modified: Wed, 20 Apr 2022 07:44:02 GMT  
		Size: 33.0 MB (33042725 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; mips64le

```console
$ docker pull debian@sha256:f86349d0bcf5ae27f3f825e47756f792138709d4a0635c89f838e9889da65abe
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30715773 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c8a8608f8ca13764fee2a6bba85a2cc14b8a228b0257c5c7965a0a434f0360b5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:16 GMT
ADD file:964e2cdd2fb2f97f790348219c9733ab638c04dce00b490987d06de65eb4da20 in / 
# Tue, 29 Mar 2022 07:41:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d00491a91df8f9bd6b12e09694fa80f076230c3c4c22bfc44b6caeaaf9f77681`  
		Last Modified: Tue, 29 Mar 2022 07:51:35 GMT  
		Size: 30.7 MB (30715773 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:835ae5f0581734feb7d92ffed04a8491c9baec30ba17dbd1aafbff039bfe90b8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.4 MB (36403434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b32491f0985700475b6155824f911e7edf9e50d6ed5a5a8ad94764416a4952fe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:22 GMT
ADD file:f14832beca200b9a57cc4e3f081a4b00e2bdc2bd93ebb7c945e750fc8f4eb12f in / 
# Wed, 20 Apr 2022 09:45:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:473c0e8a1f0d1c28e6fdddf6060fa42d9aa96cb54bb84ca22136d08de18821df`  
		Last Modified: Wed, 20 Apr 2022 09:55:57 GMT  
		Size: 36.4 MB (36403434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; s390x

```console
$ docker pull debian@sha256:0b74b990a448ba103eca6c071f0ee87d1ece0615e2c61439f2359b764adbf39c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.2 MB (30229876 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f47710a64a89396e2e9d3f7361fe34cd82b4fb97c961e360ae073e1b24c6f2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:38:30 GMT
ADD file:69023645ee8445197318ee6f8248a13eac1778ee18384794bd72239307a242df in / 
# Wed, 20 Apr 2022 08:38:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9921a7bbd10afaaa8670c971b54cef6ef06c7b9497b863feec94900d5b95ed01`  
		Last Modified: Wed, 20 Apr 2022 08:49:04 GMT  
		Size: 30.2 MB (30229876 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye`

```console
$ docker pull debian@sha256:c9cc571d4eee85529d4764aedaa177ea42d2d5cf29e43f69eab2e690f1ec1e11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye` - linux; amd64

```console
$ docker pull debian@sha256:bdf44f19d09b558203306836a612cc8e42a1106b2f731fbeb000e2696c04f9c8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a11311205db1d20ae0c5373473525e45ff36ec83f472b276f283b34173af5513`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v5

```console
$ docker pull debian@sha256:183c9884139c6f80cc2b41e5b6f0832ef7fe1c42a927801b6278a0b6e18d0333
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e0968d51d6984762a26b7bd501cc11518fc8006dd1f78c754ee7e3cdc15c2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v7

```console
$ docker pull debian@sha256:f5965ab2ab9c4163f95c716a5524797dfb61300140e97aaa6821a762d0096c0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133696 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaabdee60d04a8992fcef8727e5d2e99d97fc0efc15c1bf5e28e466360c3033b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3c0acad2a24228465b3273129fe5bd5c3b122925a491f24e0308999fa5532d48
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633096 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f8dd32c4c981fb56702f6effe49f64226ac124415109befc63d15ec19a12dabd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; 386

```console
$ docker pull debian@sha256:ea5a721ee0cf89d1cd7ec768dbdf3d719b37bb3d2d6b4c89b027f9bfa56da966
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940721 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40aa2edee2c7f9abcdcb40d249854f199ec91602a19da6d7d1a645492f233893`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; mips64le

```console
$ docker pull debian@sha256:8495d8e4715051871244692e7b96666ba4af68e25a6cf9d8072f98836c927bc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199386 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0ccd6826906b5b70da7c48a74b23763d1d48a85f2495f3582faa87a95c42116`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:51 GMT
ADD file:40fc6f06ccce55e9889d1c94556c29242d1ac62870cb9895da64d37bac45ab5e in / 
# Tue, 29 Mar 2022 07:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb90a274e1b19eec263e44f7fbc1f9c9c6c393210c27b03ebde7c2b0a2b6482`  
		Last Modified: Tue, 29 Mar 2022 07:52:16 GMT  
		Size: 53.2 MB (53199386 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; ppc64le

```console
$ docker pull debian@sha256:50bc3cfc65c5b7386d0ef3b67512d703d02da0d682e9e346f279ab9922ebe14f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bed7dbfb0379bc4cb4fb0509badb61ec1da3b439a4c0f1e1c2b69acc8693e678`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; s390x

```console
$ docker pull debian@sha256:132c0434d1a999c95433e8bb9bf4f0bc2943d0185b60711d581530769fd5b7e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207374 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f579f688d5503f84dabfbce1af0e85fa391ae288878e09a07b55353d88eb508b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20220418`

```console
$ docker pull debian@sha256:018a504db34ed0a8d2e4dc2a908d73e205c45205fd7e565512842620c3d2ec12
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20220418` - linux; amd64

```console
$ docker pull debian@sha256:bdf44f19d09b558203306836a612cc8e42a1106b2f731fbeb000e2696c04f9c8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a11311205db1d20ae0c5373473525e45ff36ec83f472b276f283b34173af5513`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:183c9884139c6f80cc2b41e5b6f0832ef7fe1c42a927801b6278a0b6e18d0333
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e0968d51d6984762a26b7bd501cc11518fc8006dd1f78c754ee7e3cdc15c2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:f5965ab2ab9c4163f95c716a5524797dfb61300140e97aaa6821a762d0096c0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133696 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaabdee60d04a8992fcef8727e5d2e99d97fc0efc15c1bf5e28e466360c3033b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3c0acad2a24228465b3273129fe5bd5c3b122925a491f24e0308999fa5532d48
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633096 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f8dd32c4c981fb56702f6effe49f64226ac124415109befc63d15ec19a12dabd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; 386

```console
$ docker pull debian@sha256:ea5a721ee0cf89d1cd7ec768dbdf3d719b37bb3d2d6b4c89b027f9bfa56da966
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940721 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40aa2edee2c7f9abcdcb40d249854f199ec91602a19da6d7d1a645492f233893`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:50bc3cfc65c5b7386d0ef3b67512d703d02da0d682e9e346f279ab9922ebe14f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bed7dbfb0379bc4cb4fb0509badb61ec1da3b439a4c0f1e1c2b69acc8693e678`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418` - linux; s390x

```console
$ docker pull debian@sha256:132c0434d1a999c95433e8bb9bf4f0bc2943d0185b60711d581530769fd5b7e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207374 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f579f688d5503f84dabfbce1af0e85fa391ae288878e09a07b55353d88eb508b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20220418-slim`

```console
$ docker pull debian@sha256:b781625da5a1c4e3c61515bbc129f979ee5fa50e552d42855ad339cf8ff256df
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:b771c35d1e6ecf2556718ad3c0f481b4a04c1fbc133c609643acc9dd6743ead2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfbec70f84886eb3760616aa408f60fe6f753ef414a0f40938020b673230126b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:27 GMT
ADD file:8b1e79f91081eb527b455431af58e823d8b84d9d0c8e5c47cb7bda7507954ae4 in / 
# Wed, 20 Apr 2022 04:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fe172e4850f03bb45d41a20174112bc119fbfec42a650edbbd8491aee32e3c3`  
		Last Modified: Wed, 20 Apr 2022 04:48:27 GMT  
		Size: 31.4 MB (31378979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:7e99b539841e02be174aa3dca177073bd3c3c183ba473b2aede0ecfc440d5dd5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921085 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03dfd2315bcac484fe4ec9bc76bf58b6644c7087594777d3cd2b0300021a915a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:42 GMT
ADD file:f20c57847025604e1ea1a848af3f090f71d78b5fc2eb2ac795822da94d6e6a0a in / 
# Wed, 20 Apr 2022 07:16:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3edd9d296fa023e00f76828c9d6fd6cc5b03e626341a6a045c40d2ea16c32d08`  
		Last Modified: Wed, 20 Apr 2022 07:32:27 GMT  
		Size: 28.9 MB (28921085 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ea3841109522787a901913924be99040ecbd4ec7a757446dd971f2e7065aba3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575758 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26f43b966e43ab4ea5bcf8dccf3ff2438b2f0a3f40661200716734a2e9a4ef26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:18 GMT
ADD file:1a0e290eb32b3533f9d1684c683ad866b5003579790b25ad61b2044dc6c20bbb in / 
# Wed, 20 Apr 2022 13:27:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6b2963233cd14b47391f0e25f2a485bbeabb87e771a71b73e5cde7c013963da`  
		Last Modified: Wed, 20 Apr 2022 13:43:41 GMT  
		Size: 26.6 MB (26575758 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f82fd419b104813e7ba6374e2e6946d4314e6e8ab90ddd565371ba04b2a61ae6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f21e23dc6e6ed773301f6d2f71ecae4bb3f73c72ec26df347815574871529b9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:10 GMT
ADD file:84523b5820128293b713ef35d8db618921162250da1e4c72bf9923f34ad7d87d in / 
# Wed, 20 Apr 2022 04:29:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d4a449ac69c579312443ded09f57c4894e7adb42f7406abd364f95982fafc59`  
		Last Modified: Wed, 20 Apr 2022 04:35:49 GMT  
		Size: 30.1 MB (30065801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:82ab01b1fafba70f80af0bec310cb62f0b60bf1558fde388502e6c9cb26e227a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389597 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f06ba4c886838d848a2c08cc8828db043336099af17bf20d66606ebb0a30e4ca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:35 GMT
ADD file:460761e2baaea91893a907ce122ff7d585ef5704f48c03454835986028a1d842 in / 
# Wed, 20 Apr 2022 07:37:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e5c4eb151e267d273883db6e9aee3ff01be07e098ca33f0d1d65bb0e0416921`  
		Last Modified: Wed, 20 Apr 2022 07:44:54 GMT  
		Size: 32.4 MB (32389597 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:75efc4f12ff7ada74b636bef34e96b7905370c19f1ff5b8e7794e8cbed880aca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285249 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d3c24fc64ca70b09590e5a585748e1687cac9cec688c2a07ec4254c37dbf308`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:46:36 GMT
ADD file:8d406ebce4d9b0884d46ee25ec31fe7714726024b80aba9b408d81d39e2f6f8c in / 
# Wed, 20 Apr 2022 09:46:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e0d035960b14409a1ddb839de80aa08677b71addd5e94264ff9ee89a2523e5b`  
		Last Modified: Wed, 20 Apr 2022 09:56:59 GMT  
		Size: 35.3 MB (35285249 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:c628fea72069afa77a21e18266c95b24c0e82d73108178cf680915f717b479f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09a58d3a39a373f45e7dab1ae57b01ae6f12de01b1f70b1f83daa474c65cd443`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:37 GMT
ADD file:c3381330644d6d9fc585301395580b564cdf7c880fb2dc2d8e48992673184f7d in / 
# Wed, 20 Apr 2022 08:39:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3179e1eccb473ae22a70e8ac3e9ddb4660ebdfd977989d115bd4d322ecc12b89`  
		Last Modified: Wed, 20 Apr 2022 08:49:39 GMT  
		Size: 29.7 MB (29654805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-backports`

```console
$ docker pull debian@sha256:9a0d5bd58e060a1ac147e31dd776768446fddc790b51620a6328f15a76e7a6bc
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-backports` - linux; amd64

```console
$ docker pull debian@sha256:8e537bd87ce1db0283e6574577bdc61ef57fb7ce73f178fdbb69a90c569d0502
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54942004 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:71ad039cec8f1ebcec7fa8036d1aed21a1d17ee0bddf6cd7987384b1521ff2c6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:43:19 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9e039d6f4a08015070e85a9abf46f6133f47a35b90d5cbf3d61c0979e9abe41a`  
		Last Modified: Wed, 20 Apr 2022 04:48:16 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:ae7bd4fd8ea079a3179eb6f18d2e6b95ab6692b4bf69a35c83151283804cad7f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52475094 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0524ddfeae4b0c149381d12265722125def01788f3664b50b70ed6cc08943c6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:16:22 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:662f95d1ade54dd023eeba89f509dcc07b1a15511885d4ec602f297354021f74`  
		Last Modified: Wed, 20 Apr 2022 07:32:01 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:63f71f177b81effa8428fe9b48705f49fa6c16205e3e58f1bd54b216c6f06c1c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133923 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f327f0aa4e3c1b28d39808f65ae0e9727d6626c1dc4533193f1b30b809306d1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:26:56 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:38c506110af71156f1de46457aeb7eba0cb5a54241438a1fbefb3bce94f3d6a4`  
		Last Modified: Wed, 20 Apr 2022 13:43:18 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e0e73ebac447b048749639ffcd4f53e4cd8bb4a2f9abe5b6c259ce7012cf563d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633319 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:88661b991c2e33f8c52c0e286923f37dba092523a95fa42ae3a5adf650d5db4f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:29:02 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b8f62352badf9a0f962790e6fd7f921e93911eb5643361b1ea3a73cccca6237d`  
		Last Modified: Wed, 20 Apr 2022 04:35:38 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; 386

```console
$ docker pull debian@sha256:c4398e1e88771afea7eeb7600945a6ebc03d94b3a88e856cc8a739ed26002f18
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940945 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e16d46e42a5abb2035bd0575ab59df147257b0632d41ba6c3c15a458e3d642cf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:37:26 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:91216565f5b55f1eb08576cd574486baf91f11d004c2a1222ebedc6774a1373f`  
		Last Modified: Wed, 20 Apr 2022 07:44:43 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; mips64le

```console
$ docker pull debian@sha256:b1d8e26633bc3bd9bcb967f20212c935870f321b0eddb5a457d41141a72f83f6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199613 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1f43eccaff4cdf00d92fa2a040673f311237a00de7a0a06e0ea97e9c72f81dc3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:51 GMT
ADD file:40fc6f06ccce55e9889d1c94556c29242d1ac62870cb9895da64d37bac45ab5e in / 
# Tue, 29 Mar 2022 07:41:56 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:42:07 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:efb90a274e1b19eec263e44f7fbc1f9c9c6c393210c27b03ebde7c2b0a2b6482`  
		Last Modified: Tue, 29 Mar 2022 07:52:16 GMT  
		Size: 53.2 MB (53199386 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d5f3607c822b6087d91a8e246d263f4d0226b8bf2237680aab28b0f5df563784`  
		Last Modified: Tue, 29 Mar 2022 07:52:33 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:6bd5a0b038824a8795dafd8534ee276c63404305aedf8d272116900093ba3751
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835561 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8128369f787c43a816682088c9f00ae949837a14a328fb9b53f543a5e94332cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:46:19 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9c4a790d78cda5f995ef3d7cf2b6a40d50b0ca5da573801bb9c2b7f1d2d0ceca`  
		Last Modified: Wed, 20 Apr 2022 09:56:42 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; s390x

```console
$ docker pull debian@sha256:c952d0601045d64b602ca80b027613c236c033b661f6de05f7783b24553e8ca0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207601 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ccd90758145f47e4b42c7e35b3c6b87d49c40eb82fa7bd3d2d46fe2f61576318`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:39:18 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8d829b9e4f82b4e1f88df842c36d133c1290c6e2f5794f518006a93b2d0d5ddb`  
		Last Modified: Wed, 20 Apr 2022 08:49:30 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-slim`

```console
$ docker pull debian@sha256:a25bdade954a2255408020517a980244b272c3863f2e5b48075bd4a78a62cb55
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-slim` - linux; amd64

```console
$ docker pull debian@sha256:b771c35d1e6ecf2556718ad3c0f481b4a04c1fbc133c609643acc9dd6743ead2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfbec70f84886eb3760616aa408f60fe6f753ef414a0f40938020b673230126b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:27 GMT
ADD file:8b1e79f91081eb527b455431af58e823d8b84d9d0c8e5c47cb7bda7507954ae4 in / 
# Wed, 20 Apr 2022 04:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fe172e4850f03bb45d41a20174112bc119fbfec42a650edbbd8491aee32e3c3`  
		Last Modified: Wed, 20 Apr 2022 04:48:27 GMT  
		Size: 31.4 MB (31378979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:7e99b539841e02be174aa3dca177073bd3c3c183ba473b2aede0ecfc440d5dd5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921085 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03dfd2315bcac484fe4ec9bc76bf58b6644c7087594777d3cd2b0300021a915a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:42 GMT
ADD file:f20c57847025604e1ea1a848af3f090f71d78b5fc2eb2ac795822da94d6e6a0a in / 
# Wed, 20 Apr 2022 07:16:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3edd9d296fa023e00f76828c9d6fd6cc5b03e626341a6a045c40d2ea16c32d08`  
		Last Modified: Wed, 20 Apr 2022 07:32:27 GMT  
		Size: 28.9 MB (28921085 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ea3841109522787a901913924be99040ecbd4ec7a757446dd971f2e7065aba3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575758 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26f43b966e43ab4ea5bcf8dccf3ff2438b2f0a3f40661200716734a2e9a4ef26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:18 GMT
ADD file:1a0e290eb32b3533f9d1684c683ad866b5003579790b25ad61b2044dc6c20bbb in / 
# Wed, 20 Apr 2022 13:27:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6b2963233cd14b47391f0e25f2a485bbeabb87e771a71b73e5cde7c013963da`  
		Last Modified: Wed, 20 Apr 2022 13:43:41 GMT  
		Size: 26.6 MB (26575758 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f82fd419b104813e7ba6374e2e6946d4314e6e8ab90ddd565371ba04b2a61ae6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f21e23dc6e6ed773301f6d2f71ecae4bb3f73c72ec26df347815574871529b9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:10 GMT
ADD file:84523b5820128293b713ef35d8db618921162250da1e4c72bf9923f34ad7d87d in / 
# Wed, 20 Apr 2022 04:29:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d4a449ac69c579312443ded09f57c4894e7adb42f7406abd364f95982fafc59`  
		Last Modified: Wed, 20 Apr 2022 04:35:49 GMT  
		Size: 30.1 MB (30065801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; 386

```console
$ docker pull debian@sha256:82ab01b1fafba70f80af0bec310cb62f0b60bf1558fde388502e6c9cb26e227a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389597 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f06ba4c886838d848a2c08cc8828db043336099af17bf20d66606ebb0a30e4ca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:35 GMT
ADD file:460761e2baaea91893a907ce122ff7d585ef5704f48c03454835986028a1d842 in / 
# Wed, 20 Apr 2022 07:37:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e5c4eb151e267d273883db6e9aee3ff01be07e098ca33f0d1d65bb0e0416921`  
		Last Modified: Wed, 20 Apr 2022 07:44:54 GMT  
		Size: 32.4 MB (32389597 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; mips64le

```console
$ docker pull debian@sha256:0b6b37bb7ea352f1cdc67d0019d6eb6d42a4249dc249419e8707507c21cef6d2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29641474 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b79c0d2777fcb480d3d72c13ad8afd9ffd671ab3d984fa184ab0c28f694dd61`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:42:27 GMT
ADD file:32aa9fd7ee5c64e4bd49459e801e3e5dc50138590bbfca671e336a197aa7fa92 in / 
# Tue, 29 Mar 2022 07:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2a8045f9de06328ab3d0ff505d990892219b7faee393bc9ac342347fc83d04`  
		Last Modified: Tue, 29 Mar 2022 07:52:59 GMT  
		Size: 29.6 MB (29641474 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:75efc4f12ff7ada74b636bef34e96b7905370c19f1ff5b8e7794e8cbed880aca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285249 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d3c24fc64ca70b09590e5a585748e1687cac9cec688c2a07ec4254c37dbf308`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:46:36 GMT
ADD file:8d406ebce4d9b0884d46ee25ec31fe7714726024b80aba9b408d81d39e2f6f8c in / 
# Wed, 20 Apr 2022 09:46:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e0d035960b14409a1ddb839de80aa08677b71addd5e94264ff9ee89a2523e5b`  
		Last Modified: Wed, 20 Apr 2022 09:56:59 GMT  
		Size: 35.3 MB (35285249 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; s390x

```console
$ docker pull debian@sha256:c628fea72069afa77a21e18266c95b24c0e82d73108178cf680915f717b479f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09a58d3a39a373f45e7dab1ae57b01ae6f12de01b1f70b1f83daa474c65cd443`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:37 GMT
ADD file:c3381330644d6d9fc585301395580b564cdf7c880fb2dc2d8e48992673184f7d in / 
# Wed, 20 Apr 2022 08:39:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3179e1eccb473ae22a70e8ac3e9ddb4660ebdfd977989d115bd4d322ecc12b89`  
		Last Modified: Wed, 20 Apr 2022 08:49:39 GMT  
		Size: 29.7 MB (29654805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster`

```console
$ docker pull debian@sha256:7979afbcb351f4704aa52fe88f0a40937cabd660895b83e33dea992dec85c3e1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster` - linux; amd64

```console
$ docker pull debian@sha256:40dd1c1b1c36eac161ab63b6ce3a57d56ad79a667a37717a31721bac3f30aaf9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50436985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26a2b081e03207d26a105340161109ba0f00e857cbb0ff85aaeeeadd46b709c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:37 GMT
ADD file:7c5789fb822bda2652d7addee832c5a3d71733f0f94f97d89b0c5570c0840829 in / 
# Wed, 20 Apr 2022 04:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85bed84afb9a834cf090b55d2e584abd55b4792d93b750db896f486680638344`  
		Last Modified: Wed, 20 Apr 2022 04:48:49 GMT  
		Size: 50.4 MB (50436985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v5

```console
$ docker pull debian@sha256:92f008e12d46bf2e18e54af85d4f606a2664746288211093cb79deb2c349d538
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3258a131168592bbcc73ee3bff8628e8831ab30efbfb45deb28886921bf5ab40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:11 GMT
ADD file:ad329516525d9a2e9eecb4bd6263809faafa65bbd9e6deebcda8196b0ea24234 in / 
# Wed, 20 Apr 2022 07:17:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d426a13b8d217ec7cc2eceea9d798d79a1c36c60cb9363d2ef2d86abcf895e88`  
		Last Modified: Wed, 20 Apr 2022 07:33:13 GMT  
		Size: 48.2 MB (48153607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e9f24e8010ba995daf25c6fafe8ea5c1914428cc29df867ffc034375d769a43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b12c05639277048cd1ee9a1764a3946118f4dbd48e2c6dd7d36abe127500640`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:49 GMT
ADD file:fbbe7a4ec12b0fdfa82879ff73d49ba760c5b6cc47b4c8ecabe64f7e8f4e6340 in / 
# Wed, 20 Apr 2022 13:27:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca53ad6266ed4ba4323f1553d8b226d0a180384cd290a8361ab19347f6d761fa`  
		Last Modified: Wed, 20 Apr 2022 13:44:25 GMT  
		Size: 45.9 MB (45908143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bf508d976de85857fd69e5da1f67d89f59ab3bc9f60b578ce24eee43047faf23
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de70a5f020527dc50128dcfe45bc76352337371d985d92dbeb1f826f5b462d08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:22 GMT
ADD file:5286ff5db23c42ac6a437e8f2f9ef01b6ee6e523d7866c752428d54f177c7108 in / 
# Wed, 20 Apr 2022 04:29:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9f330b3a7e234282d632d95bd19a655cf0e06c1a76a51d0f73b9581be8f851b`  
		Last Modified: Wed, 20 Apr 2022 04:36:12 GMT  
		Size: 49.2 MB (49227738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; 386

```console
$ docker pull debian@sha256:373c53aee6f022db1a970f7d8631f633d63810d51e466edb682427093874fc10
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e379afe86ea1b6d5434a1193a58277d88f2275200efacfe33fee80b295855706`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:47 GMT
ADD file:f66ba5e7b2801d19041661506f244a3bb89fc613e2e27891bdba3183bd6ee097 in / 
# Wed, 20 Apr 2022 07:37:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e8f333ddc9f58dd9e74b1f250e3812ba3942ef13abee1d6b4e3d3131a7e29ff`  
		Last Modified: Wed, 20 Apr 2022 07:45:17 GMT  
		Size: 51.2 MB (51195868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; mips64le

```console
$ docker pull debian@sha256:1fd521fb0c639ec1c7a9cf64eb55b485e5d2177fa53cc0ee2f3fd2851df4459a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079891 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fffd7182b90e6f67b05c4ad7efaed98eb074b678b22345c014814758cba833a0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:01 GMT
ADD file:c86010b52f782f910842586e6abc81e64c7a659d001b89799314a59f4fb96573 in / 
# Tue, 29 Mar 2022 07:43:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8e0bee9c4b9257c0ad7469830be5e890536feef4e8ffe6c2613b7e8be7e060f6`  
		Last Modified: Tue, 29 Mar 2022 07:53:41 GMT  
		Size: 49.1 MB (49079891 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; ppc64le

```console
$ docker pull debian@sha256:2e9fdeb6153b85d54d8e6a6dccc44faef10b9b33478a6af45114e35b6c56f86f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169874 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f419f2a07e659f09f4d3f9892bc82edd223abd5c9357e17bd896416a203f099`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:08 GMT
ADD file:0e047f9a02092945498103cc7b6483b67ad3a6d4945ac61230d539e4b6e14663 in / 
# Wed, 20 Apr 2022 09:47:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9d81baca7c32c0c0c5ac628e027c3c89760343d8a3469fbf7c764d60badfa23`  
		Last Modified: Wed, 20 Apr 2022 09:57:27 GMT  
		Size: 54.2 MB (54169874 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; s390x

```console
$ docker pull debian@sha256:3930c6d27a18a742104fc0ef8d49fdd180999f80104dbbe9e1d6fd526eac7d99
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98d4667bcfc2f6d7c588eb4c5160bc3234cff1e44888e375fe4ba6291e3d9d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:05 GMT
ADD file:fedc64967d4810188e8bd8289de1b1848a9501e7c68ae5bd4af83377fb9e3108 in / 
# Wed, 20 Apr 2022 08:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2799a5996f1eb24bb7f4c0219facf7e95c780314bb9e9b62594f1fe56cd511`  
		Last Modified: Wed, 20 Apr 2022 08:49:56 GMT  
		Size: 49.0 MB (48999225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20220418`

```console
$ docker pull debian@sha256:b337fc53783de3ec4c3225974e1d273f161fd0b79fe5ccca8e9e03b5f0a9a560
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-20220418` - linux; amd64

```console
$ docker pull debian@sha256:40dd1c1b1c36eac161ab63b6ce3a57d56ad79a667a37717a31721bac3f30aaf9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50436985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:26a2b081e03207d26a105340161109ba0f00e857cbb0ff85aaeeeadd46b709c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:37 GMT
ADD file:7c5789fb822bda2652d7addee832c5a3d71733f0f94f97d89b0c5570c0840829 in / 
# Wed, 20 Apr 2022 04:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85bed84afb9a834cf090b55d2e584abd55b4792d93b750db896f486680638344`  
		Last Modified: Wed, 20 Apr 2022 04:48:49 GMT  
		Size: 50.4 MB (50436985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:92f008e12d46bf2e18e54af85d4f606a2664746288211093cb79deb2c349d538
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3258a131168592bbcc73ee3bff8628e8831ab30efbfb45deb28886921bf5ab40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:11 GMT
ADD file:ad329516525d9a2e9eecb4bd6263809faafa65bbd9e6deebcda8196b0ea24234 in / 
# Wed, 20 Apr 2022 07:17:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d426a13b8d217ec7cc2eceea9d798d79a1c36c60cb9363d2ef2d86abcf895e88`  
		Last Modified: Wed, 20 Apr 2022 07:33:13 GMT  
		Size: 48.2 MB (48153607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e9f24e8010ba995daf25c6fafe8ea5c1914428cc29df867ffc034375d769a43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b12c05639277048cd1ee9a1764a3946118f4dbd48e2c6dd7d36abe127500640`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:49 GMT
ADD file:fbbe7a4ec12b0fdfa82879ff73d49ba760c5b6cc47b4c8ecabe64f7e8f4e6340 in / 
# Wed, 20 Apr 2022 13:27:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca53ad6266ed4ba4323f1553d8b226d0a180384cd290a8361ab19347f6d761fa`  
		Last Modified: Wed, 20 Apr 2022 13:44:25 GMT  
		Size: 45.9 MB (45908143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bf508d976de85857fd69e5da1f67d89f59ab3bc9f60b578ce24eee43047faf23
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de70a5f020527dc50128dcfe45bc76352337371d985d92dbeb1f826f5b462d08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:22 GMT
ADD file:5286ff5db23c42ac6a437e8f2f9ef01b6ee6e523d7866c752428d54f177c7108 in / 
# Wed, 20 Apr 2022 04:29:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9f330b3a7e234282d632d95bd19a655cf0e06c1a76a51d0f73b9581be8f851b`  
		Last Modified: Wed, 20 Apr 2022 04:36:12 GMT  
		Size: 49.2 MB (49227738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; 386

```console
$ docker pull debian@sha256:373c53aee6f022db1a970f7d8631f633d63810d51e466edb682427093874fc10
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e379afe86ea1b6d5434a1193a58277d88f2275200efacfe33fee80b295855706`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:47 GMT
ADD file:f66ba5e7b2801d19041661506f244a3bb89fc613e2e27891bdba3183bd6ee097 in / 
# Wed, 20 Apr 2022 07:37:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e8f333ddc9f58dd9e74b1f250e3812ba3942ef13abee1d6b4e3d3131a7e29ff`  
		Last Modified: Wed, 20 Apr 2022 07:45:17 GMT  
		Size: 51.2 MB (51195868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:2e9fdeb6153b85d54d8e6a6dccc44faef10b9b33478a6af45114e35b6c56f86f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169874 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f419f2a07e659f09f4d3f9892bc82edd223abd5c9357e17bd896416a203f099`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:08 GMT
ADD file:0e047f9a02092945498103cc7b6483b67ad3a6d4945ac61230d539e4b6e14663 in / 
# Wed, 20 Apr 2022 09:47:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9d81baca7c32c0c0c5ac628e027c3c89760343d8a3469fbf7c764d60badfa23`  
		Last Modified: Wed, 20 Apr 2022 09:57:27 GMT  
		Size: 54.2 MB (54169874 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418` - linux; s390x

```console
$ docker pull debian@sha256:3930c6d27a18a742104fc0ef8d49fdd180999f80104dbbe9e1d6fd526eac7d99
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98d4667bcfc2f6d7c588eb4c5160bc3234cff1e44888e375fe4ba6291e3d9d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:05 GMT
ADD file:fedc64967d4810188e8bd8289de1b1848a9501e7c68ae5bd4af83377fb9e3108 in / 
# Wed, 20 Apr 2022 08:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c2799a5996f1eb24bb7f4c0219facf7e95c780314bb9e9b62594f1fe56cd511`  
		Last Modified: Wed, 20 Apr 2022 08:49:56 GMT  
		Size: 49.0 MB (48999225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20220418-slim`

```console
$ docker pull debian@sha256:d3a847a54ea25565e81919c30b6ff723f752b99f039b65e162a248446f40892a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:dfa8be0c158e556532b023717650ee2ab52ed2b1b792a5810db138708101548a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77eeed0828a424e6b36cbde8c519381037702610d8e2f883872d7ba7198fb4e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:48 GMT
ADD file:011a43ee23214c201afb7f3b5be592f374b89a4c71aea82ca66146bbbc31b959 in / 
# Wed, 20 Apr 2022 04:43:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4be315f6562fccf08fd6c749557e31e45ab6d987370e20e2c4933ddb04ddd5ff`  
		Last Modified: Wed, 20 Apr 2022 04:49:15 GMT  
		Size: 27.1 MB (27140664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:87ac356f0eb760b1b5114218ea1148f9e93d1371dd0a15cef28894e48b0dca0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889592 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a8356a7ebaad8c554f999a9556ab637482de9fa4f117a6a31b16567b1282338`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:47 GMT
ADD file:5b740716d202d2f3494df5e108c0ba999125920ed9c97803fe0ccdfeba5cf518 in / 
# Wed, 20 Apr 2022 07:17:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fc8704cfa531d4583c5d4d571b836a82c02a812dae88792c4397cf1c1770405`  
		Last Modified: Wed, 20 Apr 2022 07:33:54 GMT  
		Size: 24.9 MB (24889592 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:79c373b3a8b1543cb91ff909eec8c5b6b2751d8ed1806063aba4bea77b83799e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747865 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfe2eb31a188f4af3f49515a22bd251f26b1351ae64056cb418759da984df972`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:26 GMT
ADD file:2a755b6e7bf01ec959b8bb848b6d46f292821a0bf08e04d565991ec4bdbea029 in / 
# Wed, 20 Apr 2022 13:28:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:88467ac22b4358b1b60009f6332ba9e12b7a82a36bef334d91d68dbf8b58d96f`  
		Last Modified: Wed, 20 Apr 2022 13:45:04 GMT  
		Size: 22.7 MB (22747865 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:09d07b5771076a6acf574e578ade2f64f4f4d44845d5fee34a6814beb4601d98
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908349 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a2d0ef9e5ad17086e08346533255f9296d445e66bad3c5b9601c9758c878524`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:36 GMT
ADD file:5de4397c1295f7249c93907be599d96efbd9cd25909340d245ec8d1c9770f444 in / 
# Wed, 20 Apr 2022 04:29:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c3e5b218d6477ff77053280968d5bdeaa5dcf16a803e917d98dd0ea8b93ac82`  
		Last Modified: Wed, 20 Apr 2022 04:36:38 GMT  
		Size: 25.9 MB (25908349 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:589a7c019a77ef6f47c72807cf92ab71e09d208212db1703fb28b3432722689d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:41e984298ef845f5111c417dd3c3a5446f7fb9f06549018f81be3337421972eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:03 GMT
ADD file:602a25173054242f635a5a299845b7f1b56864ac5d3b8af1ae29dec3a9da119f in / 
# Wed, 20 Apr 2022 07:38:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a13b13d818745c4d2234aab71df26e4a01bfe59f396ab62f20d156b94803650c`  
		Last Modified: Wed, 20 Apr 2022 07:45:46 GMT  
		Size: 27.8 MB (27799825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:c9b116e64e9f03088780c4da7456aea50d24047340f18f50e80416173eed96d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98e35f6574d2a25af8e8be68282f6c7fa6b571faa4404e75c09e913d23973469`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:47 GMT
ADD file:69078e7ae6d887f7e36ae048f98f784678fbd8b7a8ec27e3c2bce0d68a47236d in / 
# Wed, 20 Apr 2022 09:47:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aacfa28cbcef999a852ce9c261b67f15f0ccd8450cd1dbfe7b0dc88e6b5aba2`  
		Last Modified: Wed, 20 Apr 2022 09:58:02 GMT  
		Size: 30.6 MB (30560296 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:f8a77bc2f84442b3708edeef82c7c787b95aae8a82e746a73a81b2c3e5abe0db
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7716345ae3e3eff8664c28b2abe1284a6e2b186186aede487146ac809fa58cd2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:41 GMT
ADD file:e9840ab2ce3aeff61b68bf4837f65471a86dcba02c5023863f3e7493098b07ba in / 
# Wed, 20 Apr 2022 08:40:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b59fdcf2cb29d2ee4dd08aa08a467372e33525431c4caba9b01e9eb28f76425f`  
		Last Modified: Wed, 20 Apr 2022 08:50:14 GMT  
		Size: 25.8 MB (25758600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-backports`

```console
$ docker pull debian@sha256:70fa0b367e4843f9b9a45d710779dd601899fb25a29d15538005dd89b31cfcf6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-backports` - linux; amd64

```console
$ docker pull debian@sha256:aa99e15dabc88c67375a5bcd4b0b330678a97fadf536a804ed886f481786c096
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437207 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db6737e6e3305c6605f023abed0975cb0abcceb5c222ed50e51d386abd1333ba`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:37 GMT
ADD file:7c5789fb822bda2652d7addee832c5a3d71733f0f94f97d89b0c5570c0840829 in / 
# Wed, 20 Apr 2022 04:43:37 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:43:40 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:85bed84afb9a834cf090b55d2e584abd55b4792d93b750db896f486680638344`  
		Last Modified: Wed, 20 Apr 2022 04:48:49 GMT  
		Size: 50.4 MB (50436985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4b6bd587522a21952ca26470113c848c07d020fa048297162069cacfcc13d431`  
		Last Modified: Wed, 20 Apr 2022 04:49:04 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:d6e0b137fd859993be272212793c6b224e9862b8486ba55559d3e08dc16d51f9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153831 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:119a27061a4d5330e432df69d2f4bc94650a79a194fc83cf37757fdcebd0974b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:11 GMT
ADD file:ad329516525d9a2e9eecb4bd6263809faafa65bbd9e6deebcda8196b0ea24234 in / 
# Wed, 20 Apr 2022 07:17:12 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:17:28 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d426a13b8d217ec7cc2eceea9d798d79a1c36c60cb9363d2ef2d86abcf895e88`  
		Last Modified: Wed, 20 Apr 2022 07:33:13 GMT  
		Size: 48.2 MB (48153607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:081520d2a94fe831c1eca81aaebb40486e92c1d2ff0c9094d41d8ba579a86e14`  
		Last Modified: Wed, 20 Apr 2022 07:33:32 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:b20d2e24f04079c04e9a39ec01e9e4ba0008b958dd6011b63e9e9aaf03924262
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908369 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a60d696958ed4b474fd446b464f25d6dc0dc82ea6c90deb714a8614bd2a41b9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:27:49 GMT
ADD file:fbbe7a4ec12b0fdfa82879ff73d49ba760c5b6cc47b4c8ecabe64f7e8f4e6340 in / 
# Wed, 20 Apr 2022 13:27:50 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:28:05 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ca53ad6266ed4ba4323f1553d8b226d0a180384cd290a8361ab19347f6d761fa`  
		Last Modified: Wed, 20 Apr 2022 13:44:25 GMT  
		Size: 45.9 MB (45908143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b2b0b86771b0c247f208f44f8c27d1dd1a68614155acca96c36e48683d954cd6`  
		Last Modified: Wed, 20 Apr 2022 13:44:43 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c221b609f44c706d88518d5afa4e25b2a389688cb437b9d52c1f373caabdb57c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ae36706ec6d034436705f6490f83278531fc593c23eb4025103249c71660daf7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:22 GMT
ADD file:5286ff5db23c42ac6a437e8f2f9ef01b6ee6e523d7866c752428d54f177c7108 in / 
# Wed, 20 Apr 2022 04:29:22 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:29:29 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b9f330b3a7e234282d632d95bd19a655cf0e06c1a76a51d0f73b9581be8f851b`  
		Last Modified: Wed, 20 Apr 2022 04:36:12 GMT  
		Size: 49.2 MB (49227738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2fcf61c50bc20ce89acc6da83284c168f8a27dee2f04757a4d43ec7785f79daf`  
		Last Modified: Wed, 20 Apr 2022 04:36:28 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; 386

```console
$ docker pull debian@sha256:73beaf0209cdb2bda81e69cbd97bfeac26411c10dbbaf8bad2b4e023afd2ce54
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51196090 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:12970f12d60844dbc23d54c178bea432ea784490909b718ed6d74e4413f50d9b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:47 GMT
ADD file:f66ba5e7b2801d19041661506f244a3bb89fc613e2e27891bdba3183bd6ee097 in / 
# Wed, 20 Apr 2022 07:37:48 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:37:55 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6e8f333ddc9f58dd9e74b1f250e3812ba3942ef13abee1d6b4e3d3131a7e29ff`  
		Last Modified: Wed, 20 Apr 2022 07:45:17 GMT  
		Size: 51.2 MB (51195868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8f744635066267b8ec0833903081dcd12425841a698105cda576ce096163160b`  
		Last Modified: Wed, 20 Apr 2022 07:45:34 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; mips64le

```console
$ docker pull debian@sha256:6d9d53e320307d3695a9a9786ddd4d6f323043acfeb826bf4c8580787bf56eba
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49080115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e3276a86430777e37c247fbde5dd561abadf31fbb98998459cbefb1c63399b75`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:01 GMT
ADD file:c86010b52f782f910842586e6abc81e64c7a659d001b89799314a59f4fb96573 in / 
# Tue, 29 Mar 2022 07:43:06 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:43:17 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8e0bee9c4b9257c0ad7469830be5e890536feef4e8ffe6c2613b7e8be7e060f6`  
		Last Modified: Tue, 29 Mar 2022 07:53:41 GMT  
		Size: 49.1 MB (49079891 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e07dc846abd481122a2d53a26eefcb104fddd2751120eede429890fc1b83bfa0`  
		Last Modified: Tue, 29 Mar 2022 07:53:56 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:4dbbe597fe1f25894f385d02c2ff67f055244c8ccf0a0164d7c94763e6b77707
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54170099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0bb5483cb1442bf80e1c8f7be7f71c55108770078838458c7d23381a0d68a3e3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:08 GMT
ADD file:0e047f9a02092945498103cc7b6483b67ad3a6d4945ac61230d539e4b6e14663 in / 
# Wed, 20 Apr 2022 09:47:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:47:31 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c9d81baca7c32c0c0c5ac628e027c3c89760343d8a3469fbf7c764d60badfa23`  
		Last Modified: Wed, 20 Apr 2022 09:57:27 GMT  
		Size: 54.2 MB (54169874 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:acb012f4f2706e9479637735267f86262a6a19d9894b78beb2b53049d261db8a`  
		Last Modified: Wed, 20 Apr 2022 09:57:48 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; s390x

```console
$ docker pull debian@sha256:d269d06592391adbea776fcdf4668a495b7ee3fb56f7112d39e9f6d55523d52a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999449 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e1f632b1436c781b44b51848336bb7e325a3ea66c788af7f4602f648312d488b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:05 GMT
ADD file:fedc64967d4810188e8bd8289de1b1848a9501e7c68ae5bd4af83377fb9e3108 in / 
# Wed, 20 Apr 2022 08:40:12 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:40:25 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:5c2799a5996f1eb24bb7f4c0219facf7e95c780314bb9e9b62594f1fe56cd511`  
		Last Modified: Wed, 20 Apr 2022 08:49:56 GMT  
		Size: 49.0 MB (48999225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c3422788b4b795ea88c9c68a41d74c3cc960912d937b142a8bb66ab25f19d7c1`  
		Last Modified: Wed, 20 Apr 2022 08:50:06 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-slim`

```console
$ docker pull debian@sha256:253082b13b593e7f3b7a82800fa7f7f44de0f7895f5c4996c030be0206e6d4d0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-slim` - linux; amd64

```console
$ docker pull debian@sha256:dfa8be0c158e556532b023717650ee2ab52ed2b1b792a5810db138708101548a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77eeed0828a424e6b36cbde8c519381037702610d8e2f883872d7ba7198fb4e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:48 GMT
ADD file:011a43ee23214c201afb7f3b5be592f374b89a4c71aea82ca66146bbbc31b959 in / 
# Wed, 20 Apr 2022 04:43:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4be315f6562fccf08fd6c749557e31e45ab6d987370e20e2c4933ddb04ddd5ff`  
		Last Modified: Wed, 20 Apr 2022 04:49:15 GMT  
		Size: 27.1 MB (27140664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:87ac356f0eb760b1b5114218ea1148f9e93d1371dd0a15cef28894e48b0dca0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889592 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a8356a7ebaad8c554f999a9556ab637482de9fa4f117a6a31b16567b1282338`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:17:47 GMT
ADD file:5b740716d202d2f3494df5e108c0ba999125920ed9c97803fe0ccdfeba5cf518 in / 
# Wed, 20 Apr 2022 07:17:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1fc8704cfa531d4583c5d4d571b836a82c02a812dae88792c4397cf1c1770405`  
		Last Modified: Wed, 20 Apr 2022 07:33:54 GMT  
		Size: 24.9 MB (24889592 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:79c373b3a8b1543cb91ff909eec8c5b6b2751d8ed1806063aba4bea77b83799e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747865 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfe2eb31a188f4af3f49515a22bd251f26b1351ae64056cb418759da984df972`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:26 GMT
ADD file:2a755b6e7bf01ec959b8bb848b6d46f292821a0bf08e04d565991ec4bdbea029 in / 
# Wed, 20 Apr 2022 13:28:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:88467ac22b4358b1b60009f6332ba9e12b7a82a36bef334d91d68dbf8b58d96f`  
		Last Modified: Wed, 20 Apr 2022 13:45:04 GMT  
		Size: 22.7 MB (22747865 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:09d07b5771076a6acf574e578ade2f64f4f4d44845d5fee34a6814beb4601d98
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908349 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a2d0ef9e5ad17086e08346533255f9296d445e66bad3c5b9601c9758c878524`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:36 GMT
ADD file:5de4397c1295f7249c93907be599d96efbd9cd25909340d245ec8d1c9770f444 in / 
# Wed, 20 Apr 2022 04:29:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c3e5b218d6477ff77053280968d5bdeaa5dcf16a803e917d98dd0ea8b93ac82`  
		Last Modified: Wed, 20 Apr 2022 04:36:38 GMT  
		Size: 25.9 MB (25908349 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; 386

```console
$ docker pull debian@sha256:589a7c019a77ef6f47c72807cf92ab71e09d208212db1703fb28b3432722689d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:41e984298ef845f5111c417dd3c3a5446f7fb9f06549018f81be3337421972eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:03 GMT
ADD file:602a25173054242f635a5a299845b7f1b56864ac5d3b8af1ae29dec3a9da119f in / 
# Wed, 20 Apr 2022 07:38:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a13b13d818745c4d2234aab71df26e4a01bfe59f396ab62f20d156b94803650c`  
		Last Modified: Wed, 20 Apr 2022 07:45:46 GMT  
		Size: 27.8 MB (27799825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; mips64le

```console
$ docker pull debian@sha256:af7ca58714f59c0cfc55282e11d3c7aab0ab34d30ae6228b2c962a0c3fa83184
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25818066 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ac7583a1184fcf172d760efca521e0ea2a669111393a826992b236a324657a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:43:36 GMT
ADD file:7827dc4070f09537c063b825b0b7a5b076fac295f9afa415dee6c5346c38d46b in / 
# Tue, 29 Mar 2022 07:43:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a3e27ca4ec7041c36d0e9cbc672ce565851ce1a0cc90120d7a923a938fb3c18`  
		Last Modified: Tue, 29 Mar 2022 07:54:19 GMT  
		Size: 25.8 MB (25818066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:c9b116e64e9f03088780c4da7456aea50d24047340f18f50e80416173eed96d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98e35f6574d2a25af8e8be68282f6c7fa6b571faa4404e75c09e913d23973469`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:47:47 GMT
ADD file:69078e7ae6d887f7e36ae048f98f784678fbd8b7a8ec27e3c2bce0d68a47236d in / 
# Wed, 20 Apr 2022 09:47:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aacfa28cbcef999a852ce9c261b67f15f0ccd8450cd1dbfe7b0dc88e6b5aba2`  
		Last Modified: Wed, 20 Apr 2022 09:58:02 GMT  
		Size: 30.6 MB (30560296 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; s390x

```console
$ docker pull debian@sha256:f8a77bc2f84442b3708edeef82c7c787b95aae8a82e746a73a81b2c3e5abe0db
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7716345ae3e3eff8664c28b2abe1284a6e2b186186aede487146ac809fa58cd2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:40:41 GMT
ADD file:e9840ab2ce3aeff61b68bf4837f65471a86dcba02c5023863f3e7493098b07ba in / 
# Wed, 20 Apr 2022 08:40:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b59fdcf2cb29d2ee4dd08aa08a467372e33525431c4caba9b01e9eb28f76425f`  
		Last Modified: Wed, 20 Apr 2022 08:50:14 GMT  
		Size: 25.8 MB (25758600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental`

```console
$ docker pull debian@sha256:b433c85c3518964328c9b2c880fc1889f20bcdcb29b868a540ba83e2c22e995e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental` - linux; amd64

```console
$ docker pull debian@sha256:220bd5d9a869e9c16f5e40369d4d95f848deb3629bdcd830ce7b245dfd558541
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:baeded9577302da4d155448d989777fbbd01564a09c102e50529f25a71cc5b8f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:55 GMT
ADD file:b8f07060f9d2d7f5c4db9947fd089591ea261222b369890ff85325ec74bc0a6c in / 
# Wed, 20 Apr 2022 04:45:55 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:46:08 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7c26de214b349330f4067f62eaf7fbc9c2e817d82821b54e517e1456eedf3030`  
		Last Modified: Wed, 20 Apr 2022 04:53:29 GMT  
		Size: 56.1 MB (56112682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:19f1ad2473963dd6a11702bae6dd1105ba15fa377ce0ac90a6dc700e73c1089a`  
		Last Modified: Wed, 20 Apr 2022 04:53:53 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v5

```console
$ docker pull debian@sha256:44c6434d93ebb2496ebe96fb272c77d5a609dc0ad2d1907c81700589e296fb51
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518731 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:47b2ea52dd05e175b2e7c3dede5320555657e98a27752d33f14f0c405d871353`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:14 GMT
ADD file:d59b07addbc058dbb2e308592be3cdab758f37e2b392931b9f1cfea1a3a038e6 in / 
# Wed, 20 Apr 2022 07:24:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:24:51 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:cbf05feef61225dcf4d5af2c7d07b77a5fb04c7158b3585c00e9c665a058ee9d`  
		Last Modified: Wed, 20 Apr 2022 07:42:28 GMT  
		Size: 53.5 MB (53518510 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6d1761de9a86b52c467167e6b16adaabe5d91996db6373998306ec2d40567ef2`  
		Last Modified: Wed, 20 Apr 2022 07:43:09 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v7

```console
$ docker pull debian@sha256:8a5ad5c7f5d5efe59dcfb54bca3688847ee83e877fefd1ae5a2db2f79f7d49c1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125919 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2363982ace9e4497f8cef18db07a31a3d635e72389a195c40d3b8a30df2427b4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:30 GMT
ADD file:e72999b097295d9ae9ee09ef8bc01b0923d18fb194da7edfcd2d109356c00735 in / 
# Wed, 20 Apr 2022 13:35:31 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:36:07 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:dccc0a48401f1ff6ee85164169c47fb40d68e6d429c81c0d90ff5c0030cfac46`  
		Last Modified: Wed, 20 Apr 2022 13:53:08 GMT  
		Size: 51.1 MB (51125695 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8e1b0ca941fe8278a84be1eb769ea48c858491d62874dbfee604e5bdc8930d83`  
		Last Modified: Wed, 20 Apr 2022 13:53:48 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6a2de964a9853b6cd0bb86c9d4b131d6eb91de19718c82f94be97b13bccd6db9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55064029 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b13bf1fdf8a672da09c712249542936165a2606d4337a9a7ce95bd052f890e68`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:09 GMT
ADD file:ecf4c1717eb6fdd76f39aa7c85e910a5b33b82a012a3115872223977e906fc6b in / 
# Wed, 20 Apr 2022 04:32:09 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:32:24 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2b3f6be4af088219c3879fb497cd5051ae20697284c5fbf8f47d4d9f017ba3c2`  
		Last Modified: Wed, 20 Apr 2022 04:41:09 GMT  
		Size: 55.1 MB (55063807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bef30381bc5727e5e206a346f30d3d3d0fc4f4adc0d9567fd094b50bf1e419ca`  
		Last Modified: Wed, 20 Apr 2022 04:41:35 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; 386

```console
$ docker pull debian@sha256:38ee325a93ca8631ea38b37bc9e461a77158c8289084636a61b773657011091b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154205 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:94a0973af1d13f231eca19c39345a629d643a71acbb21da9e22901e20309ebf0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:52 GMT
ADD file:76ce7950d3083a262cf5a8a5350e77e81af33edd5696e7bea672c6b56dfba430 in / 
# Wed, 20 Apr 2022 07:40:52 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:41:08 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1b03b8c90a9f4d2a5ce30d33ca3da78514bfc07dbf09d69b25fffd10d5626a40`  
		Last Modified: Wed, 20 Apr 2022 07:50:24 GMT  
		Size: 57.2 MB (57153982 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:60e73fd3a47c092cabc6a0c15e8d36da2a6f5ef49c63065770f4b7e3acef9cd9`  
		Last Modified: Wed, 20 Apr 2022 07:50:53 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; mips64le

```console
$ docker pull debian@sha256:380732e2f87214c28dc3c99bcef41b4a3b79975681bfb04fe37b414e7d374fe0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54453695 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:07ead74b7d8a28f3208a7a6a89d4e79b34e267a0a49f7754baa33267ed57a75a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:48:39 GMT
ADD file:1f36bf26ba6ed5898744d070089c9800f3701bbe25ca8e98ba248a56b990ac09 in / 
# Tue, 29 Mar 2022 07:48:43 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:49:26 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:e7de11e455a9a72d18dbadb02f9098da749f18656d04d5f21c6fbb1ea7ecbfb9`  
		Last Modified: Tue, 29 Mar 2022 08:00:01 GMT  
		Size: 54.5 MB (54453472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ad296dfca336d4fc2f7f9dae99c7ee263e1a5e1d9400df740ad175173f34666b`  
		Last Modified: Tue, 29 Mar 2022 08:00:40 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; ppc64le

```console
$ docker pull debian@sha256:2e30526cb6200e9c25e02b5bb9c5f821263ca32585054b0bf126a8fe9bb7ad32
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558196 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:73d3da83de7fb6dae25272dae46a91d7719f8bac376b85404c6242a8acd2f1d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:14 GMT
ADD file:66e1a3778c3b5bd2a8d17600540923d83a26fc3c9aaa8a9fb3df562018ddf467 in / 
# Wed, 20 Apr 2022 09:52:21 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:52:55 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b0c49142451cdb4390822a432aca8fd4b80b2d7e389807dbe684df7dccc369ab`  
		Last Modified: Wed, 20 Apr 2022 10:02:03 GMT  
		Size: 60.6 MB (60557977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:728e1d0b4de36d3b411e42e002418dae405952cdf11099094206f18cc21f5515`  
		Last Modified: Wed, 20 Apr 2022 10:02:38 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; riscv64

```console
$ docker pull debian@sha256:a1cead4d44ed77c5662eac9f2429556006120420caba2d19a63ec196fc62a185
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757584 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b3dda1884db97d35d90d8cc9d6e29762b5d5a1efaa65abe6947b0ed8470c295`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:19:42 GMT
ADD file:eb73ccec6d534b35c3f0725251b8917d96bcd87d77d960d0a879ca91d2530a47 in / 
# Wed, 20 Apr 2022 00:19:44 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 00:23:27 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:62d036327287ffa181f78db1cedfa9a2d2ddc4c54f3c3838d75dae32e11ff1b1`  
		Last Modified: Wed, 20 Apr 2022 00:38:38 GMT  
		Size: 51.8 MB (51757356 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1e6440a805ff86b05ccf55572b7ebe62afdf80683fe40032330d06f9f2896e63`  
		Last Modified: Wed, 20 Apr 2022 00:41:41 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; s390x

```console
$ docker pull debian@sha256:d69ea7db5f051c494d707dd5de6c8e8689acb9fe066d398b2dac3080aa0086ab
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54331214 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3acb28da9e72d4ec20b94114bdf94421364a665854462661419878eb038003d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:08 GMT
ADD file:c6ea51fdbfa2c4af7547b6b1dc6f96d047344c44c7918374ad035c3d804bc0d0 in / 
# Wed, 20 Apr 2022 08:45:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:45:49 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1aa7bce33c54b3eca1469f66058926ebda6d4300fba1951826d8344a4735d337`  
		Last Modified: Wed, 20 Apr 2022 08:52:43 GMT  
		Size: 54.3 MB (54330991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2f76843361d0f15c43fec6f84d6f45279e0f92ec42851634eae80c57682cea0b`  
		Last Modified: Wed, 20 Apr 2022 08:53:04 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental-20220418`

```console
$ docker pull debian@sha256:c12fca5b0224b5a5b9d272323f52c6231e7f4b00a209e069ae3c566ec484274d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental-20220418` - linux; amd64

```console
$ docker pull debian@sha256:220bd5d9a869e9c16f5e40369d4d95f848deb3629bdcd830ce7b245dfd558541
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:baeded9577302da4d155448d989777fbbd01564a09c102e50529f25a71cc5b8f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:55 GMT
ADD file:b8f07060f9d2d7f5c4db9947fd089591ea261222b369890ff85325ec74bc0a6c in / 
# Wed, 20 Apr 2022 04:45:55 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:46:08 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7c26de214b349330f4067f62eaf7fbc9c2e817d82821b54e517e1456eedf3030`  
		Last Modified: Wed, 20 Apr 2022 04:53:29 GMT  
		Size: 56.1 MB (56112682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:19f1ad2473963dd6a11702bae6dd1105ba15fa377ce0ac90a6dc700e73c1089a`  
		Last Modified: Wed, 20 Apr 2022 04:53:53 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:44c6434d93ebb2496ebe96fb272c77d5a609dc0ad2d1907c81700589e296fb51
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518731 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:47b2ea52dd05e175b2e7c3dede5320555657e98a27752d33f14f0c405d871353`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:14 GMT
ADD file:d59b07addbc058dbb2e308592be3cdab758f37e2b392931b9f1cfea1a3a038e6 in / 
# Wed, 20 Apr 2022 07:24:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:24:51 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:cbf05feef61225dcf4d5af2c7d07b77a5fb04c7158b3585c00e9c665a058ee9d`  
		Last Modified: Wed, 20 Apr 2022 07:42:28 GMT  
		Size: 53.5 MB (53518510 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6d1761de9a86b52c467167e6b16adaabe5d91996db6373998306ec2d40567ef2`  
		Last Modified: Wed, 20 Apr 2022 07:43:09 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:8a5ad5c7f5d5efe59dcfb54bca3688847ee83e877fefd1ae5a2db2f79f7d49c1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125919 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2363982ace9e4497f8cef18db07a31a3d635e72389a195c40d3b8a30df2427b4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:30 GMT
ADD file:e72999b097295d9ae9ee09ef8bc01b0923d18fb194da7edfcd2d109356c00735 in / 
# Wed, 20 Apr 2022 13:35:31 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:36:07 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:dccc0a48401f1ff6ee85164169c47fb40d68e6d429c81c0d90ff5c0030cfac46`  
		Last Modified: Wed, 20 Apr 2022 13:53:08 GMT  
		Size: 51.1 MB (51125695 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8e1b0ca941fe8278a84be1eb769ea48c858491d62874dbfee604e5bdc8930d83`  
		Last Modified: Wed, 20 Apr 2022 13:53:48 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6a2de964a9853b6cd0bb86c9d4b131d6eb91de19718c82f94be97b13bccd6db9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55064029 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b13bf1fdf8a672da09c712249542936165a2606d4337a9a7ce95bd052f890e68`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:09 GMT
ADD file:ecf4c1717eb6fdd76f39aa7c85e910a5b33b82a012a3115872223977e906fc6b in / 
# Wed, 20 Apr 2022 04:32:09 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:32:24 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2b3f6be4af088219c3879fb497cd5051ae20697284c5fbf8f47d4d9f017ba3c2`  
		Last Modified: Wed, 20 Apr 2022 04:41:09 GMT  
		Size: 55.1 MB (55063807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bef30381bc5727e5e206a346f30d3d3d0fc4f4adc0d9567fd094b50bf1e419ca`  
		Last Modified: Wed, 20 Apr 2022 04:41:35 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; 386

```console
$ docker pull debian@sha256:38ee325a93ca8631ea38b37bc9e461a77158c8289084636a61b773657011091b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154205 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:94a0973af1d13f231eca19c39345a629d643a71acbb21da9e22901e20309ebf0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:52 GMT
ADD file:76ce7950d3083a262cf5a8a5350e77e81af33edd5696e7bea672c6b56dfba430 in / 
# Wed, 20 Apr 2022 07:40:52 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:41:08 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1b03b8c90a9f4d2a5ce30d33ca3da78514bfc07dbf09d69b25fffd10d5626a40`  
		Last Modified: Wed, 20 Apr 2022 07:50:24 GMT  
		Size: 57.2 MB (57153982 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:60e73fd3a47c092cabc6a0c15e8d36da2a6f5ef49c63065770f4b7e3acef9cd9`  
		Last Modified: Wed, 20 Apr 2022 07:50:53 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:2e30526cb6200e9c25e02b5bb9c5f821263ca32585054b0bf126a8fe9bb7ad32
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558196 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:73d3da83de7fb6dae25272dae46a91d7719f8bac376b85404c6242a8acd2f1d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:14 GMT
ADD file:66e1a3778c3b5bd2a8d17600540923d83a26fc3c9aaa8a9fb3df562018ddf467 in / 
# Wed, 20 Apr 2022 09:52:21 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:52:55 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b0c49142451cdb4390822a432aca8fd4b80b2d7e389807dbe684df7dccc369ab`  
		Last Modified: Wed, 20 Apr 2022 10:02:03 GMT  
		Size: 60.6 MB (60557977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:728e1d0b4de36d3b411e42e002418dae405952cdf11099094206f18cc21f5515`  
		Last Modified: Wed, 20 Apr 2022 10:02:38 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; riscv64

```console
$ docker pull debian@sha256:a1cead4d44ed77c5662eac9f2429556006120420caba2d19a63ec196fc62a185
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757584 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b3dda1884db97d35d90d8cc9d6e29762b5d5a1efaa65abe6947b0ed8470c295`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:19:42 GMT
ADD file:eb73ccec6d534b35c3f0725251b8917d96bcd87d77d960d0a879ca91d2530a47 in / 
# Wed, 20 Apr 2022 00:19:44 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 00:23:27 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:62d036327287ffa181f78db1cedfa9a2d2ddc4c54f3c3838d75dae32e11ff1b1`  
		Last Modified: Wed, 20 Apr 2022 00:38:38 GMT  
		Size: 51.8 MB (51757356 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1e6440a805ff86b05ccf55572b7ebe62afdf80683fe40032330d06f9f2896e63`  
		Last Modified: Wed, 20 Apr 2022 00:41:41 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20220418` - linux; s390x

```console
$ docker pull debian@sha256:d69ea7db5f051c494d707dd5de6c8e8689acb9fe066d398b2dac3080aa0086ab
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54331214 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3acb28da9e72d4ec20b94114bdf94421364a665854462661419878eb038003d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:08 GMT
ADD file:c6ea51fdbfa2c4af7547b6b1dc6f96d047344c44c7918374ad035c3d804bc0d0 in / 
# Wed, 20 Apr 2022 08:45:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:45:49 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1aa7bce33c54b3eca1469f66058926ebda6d4300fba1951826d8344a4735d337`  
		Last Modified: Wed, 20 Apr 2022 08:52:43 GMT  
		Size: 54.3 MB (54330991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2f76843361d0f15c43fec6f84d6f45279e0f92ec42851634eae80c57682cea0b`  
		Last Modified: Wed, 20 Apr 2022 08:53:04 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:latest`

```console
$ docker pull debian@sha256:c9cc571d4eee85529d4764aedaa177ea42d2d5cf29e43f69eab2e690f1ec1e11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:latest` - linux; amd64

```console
$ docker pull debian@sha256:bdf44f19d09b558203306836a612cc8e42a1106b2f731fbeb000e2696c04f9c8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a11311205db1d20ae0c5373473525e45ff36ec83f472b276f283b34173af5513`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:15 GMT
ADD file:3a81c181c66f226bd6abd48d0c7ed8a9c599c9f521ec7229286c83161afec8c2 in / 
# Wed, 20 Apr 2022 04:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6aefca2dc61dcbcd268b8a9861e552f9cdb69e57242faec64ac120d2355a9c1a`  
		Last Modified: Wed, 20 Apr 2022 04:47:57 GMT  
		Size: 54.9 MB (54941777 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v5

```console
$ docker pull debian@sha256:183c9884139c6f80cc2b41e5b6f0832ef7fe1c42a927801b6278a0b6e18d0333
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e0968d51d6984762a26b7bd501cc11518fc8006dd1f78c754ee7e3cdc15c2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:16:05 GMT
ADD file:d668b425e22c8042fc04ca031d5034b01d7488f8b7adf54485b4025fcb8eea77 in / 
# Wed, 20 Apr 2022 07:16:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:625ca11644bb62d8db7c0b110e4fd87d4b2b14a21e09653f8ea20ef89a5d2663`  
		Last Modified: Wed, 20 Apr 2022 07:31:39 GMT  
		Size: 52.5 MB (52474869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v7

```console
$ docker pull debian@sha256:f5965ab2ab9c4163f95c716a5524797dfb61300140e97aaa6821a762d0096c0e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133696 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaabdee60d04a8992fcef8727e5d2e99d97fc0efc15c1bf5e28e466360c3033b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:26:39 GMT
ADD file:1c05c50014bbd32a4cf1edd085996a8c62abc3c8969b64d2355475827a07475e in / 
# Wed, 20 Apr 2022 13:26:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a2c02101ee4340d4a9ebb962e9331486e417a3835c9adefb9badd2f0c116ab6`  
		Last Modified: Wed, 20 Apr 2022 13:42:55 GMT  
		Size: 50.1 MB (50133696 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3c0acad2a24228465b3273129fe5bd5c3b122925a491f24e0308999fa5532d48
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633096 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f8dd32c4c981fb56702f6effe49f64226ac124415109befc63d15ec19a12dabd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:28:55 GMT
ADD file:ece192802cbdaf1a141d04f0c2e90cfd3479e5e3e82c6a00206970684494cf48 in / 
# Wed, 20 Apr 2022 04:28:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83d5dcfea08e6927083bc886bf182fc39d87bb04b54b63002ac0c4c0b9aab682`  
		Last Modified: Wed, 20 Apr 2022 04:35:19 GMT  
		Size: 53.6 MB (53633096 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; 386

```console
$ docker pull debian@sha256:ea5a721ee0cf89d1cd7ec768dbdf3d719b37bb3d2d6b4c89b027f9bfa56da966
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940721 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40aa2edee2c7f9abcdcb40d249854f199ec91602a19da6d7d1a645492f233893`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:37:18 GMT
ADD file:81b79bac6ad02f2b0e2d30c005dac5f38d58fc7e12d7466c6704bc8a8980a0ef in / 
# Wed, 20 Apr 2022 07:37:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03830d6b0d2ecb0a7f823997ff6629d0ff36a821ec317f6d5644d08b1870d936`  
		Last Modified: Wed, 20 Apr 2022 07:44:23 GMT  
		Size: 55.9 MB (55940721 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; mips64le

```console
$ docker pull debian@sha256:8495d8e4715051871244692e7b96666ba4af68e25a6cf9d8072f98836c927bc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199386 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0ccd6826906b5b70da7c48a74b23763d1d48a85f2495f3582faa87a95c42116`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:41:51 GMT
ADD file:40fc6f06ccce55e9889d1c94556c29242d1ac62870cb9895da64d37bac45ab5e in / 
# Tue, 29 Mar 2022 07:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb90a274e1b19eec263e44f7fbc1f9c9c6c393210c27b03ebde7c2b0a2b6482`  
		Last Modified: Tue, 29 Mar 2022 07:52:16 GMT  
		Size: 53.2 MB (53199386 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; ppc64le

```console
$ docker pull debian@sha256:50bc3cfc65c5b7386d0ef3b67512d703d02da0d682e9e346f279ab9922ebe14f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835334 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bed7dbfb0379bc4cb4fb0509badb61ec1da3b439a4c0f1e1c2b69acc8693e678`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:45:51 GMT
ADD file:2fff7021563ff08bee97c626d3c91410f8f8dc3213b548a7e8b0c351557c7f1d in / 
# Wed, 20 Apr 2022 09:46:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09c77622a2c385ed83fadca4a945b0065fdeaaac427ce2b1a9e795ce2007f8c3`  
		Last Modified: Wed, 20 Apr 2022 09:56:19 GMT  
		Size: 58.8 MB (58835334 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; s390x

```console
$ docker pull debian@sha256:132c0434d1a999c95433e8bb9bf4f0bc2943d0185b60711d581530769fd5b7e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207374 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f579f688d5503f84dabfbce1af0e85fa391ae288878e09a07b55353d88eb508b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:39:00 GMT
ADD file:82d4840cdb4b0211b2191ba71ea2698d8dc1b05554d7ed1499aca9cbafaa3fc8 in / 
# Wed, 20 Apr 2022 08:39:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec7f58dd40ec15934a767db84e5e9b88704cefe60ebe4d7f5abc1583f6c060c`  
		Last Modified: Wed, 20 Apr 2022 08:49:18 GMT  
		Size: 53.2 MB (53207374 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable`

```console
$ docker pull debian@sha256:9b1cbed12a3eb128087c4c34ae083006dae6993144853d02e5ca0b8984002686
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable` - linux; amd64

```console
$ docker pull debian@sha256:b67ab5e7f69bc21ebf5c967b055bfdffe5c8c1aab48c98ebd68e5763b861139a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427461 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:44d9ec10d1ccf6563bba738781b957b3c2b86c91446e63b4317acda2e343fe76`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:57 GMT
ADD file:91c1b90dda98c0f3328e5c763b36b29831467c0a0cf52df5359551154d2e6355 in / 
# Wed, 20 Apr 2022 04:43:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d195d42244aab04cff1a0e88441cc5fd6954f65abf0514e85b72d7c4a631445d`  
		Last Modified: Wed, 20 Apr 2022 04:49:36 GMT  
		Size: 45.4 MB (45427461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:53971613c8b5bec6d8cb993e0aeeb4a22b041d538df6e59ef2918008eee4432e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cc4daf58db8cb10744af92a8ca02f458caf1247d75dcfb1e172b6713cc4b34ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:18:16 GMT
ADD file:e9d593b94101c656c0e16180bbfd484731d77bff96a72ddb0c9434a4c47d574a in / 
# Wed, 20 Apr 2022 07:18:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:011248ff897363bbfbb6391be232b1fe34d171c43e4402a37d13bb16665622b4`  
		Last Modified: Wed, 20 Apr 2022 07:34:41 GMT  
		Size: 44.1 MB (44122702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:a97f4eb34aabcdb7126cbe687c55445d94a078e5971424a12fa506fef2bf84fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150773 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b61cbd9c21b49168dc855cc5e9a2d126084056032122ccf98fbc1f66cc98d0a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:57 GMT
ADD file:75eab034a8a41af1c3746a6596e4b7a7c02c2b4d88c43ae6ee30fe838b58d074 in / 
# Wed, 20 Apr 2022 13:28:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e82ea888fad1689582caf74dfff67eaad7573350b522a40c2e0c3eeefde3b35`  
		Last Modified: Wed, 20 Apr 2022 13:45:48 GMT  
		Size: 42.2 MB (42150773 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7734c71642380c7a021628e69b4abeec707f7d1360d27fbe429cd61f2f899b47
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212037 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:524587322dcec9b3ceacb3adee033224d89cbd887e2504ecb0f086f55dc39745`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:47 GMT
ADD file:6eba819e0e2efda877840d26ec82d97b5cc971ac347cfa420006e49d98af40da in / 
# Wed, 20 Apr 2022 04:29:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3f84d18376d0447d1292f58ace6bd0081a20c00a2682f97974e0bb2caa786dab`  
		Last Modified: Wed, 20 Apr 2022 04:37:01 GMT  
		Size: 43.2 MB (43212037 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; 386

```console
$ docker pull debian@sha256:b147f115d8ade1e350c149bd9cc7dd1184058b4c2ad10c427edab7428f017947
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:28a35e3e2d7c4c55bd4a0e4f10aff948323171594dd5a52b757cf3b2985212ae`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:16 GMT
ADD file:5c34d7058c53f0327bc7b12fc42fd6e752870d7b5b982bad1b9e9d0894d75d04 in / 
# Wed, 20 Apr 2022 07:38:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:79d3d2a2441cfff5e929e595c3a88d3e53fe158136cd9b6443fe762ef335980c`  
		Last Modified: Wed, 20 Apr 2022 07:46:10 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20220418`

```console
$ docker pull debian@sha256:9b1cbed12a3eb128087c4c34ae083006dae6993144853d02e5ca0b8984002686
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20220418` - linux; amd64

```console
$ docker pull debian@sha256:b67ab5e7f69bc21ebf5c967b055bfdffe5c8c1aab48c98ebd68e5763b861139a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427461 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:44d9ec10d1ccf6563bba738781b957b3c2b86c91446e63b4317acda2e343fe76`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:57 GMT
ADD file:91c1b90dda98c0f3328e5c763b36b29831467c0a0cf52df5359551154d2e6355 in / 
# Wed, 20 Apr 2022 04:43:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d195d42244aab04cff1a0e88441cc5fd6954f65abf0514e85b72d7c4a631445d`  
		Last Modified: Wed, 20 Apr 2022 04:49:36 GMT  
		Size: 45.4 MB (45427461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:53971613c8b5bec6d8cb993e0aeeb4a22b041d538df6e59ef2918008eee4432e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cc4daf58db8cb10744af92a8ca02f458caf1247d75dcfb1e172b6713cc4b34ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:18:16 GMT
ADD file:e9d593b94101c656c0e16180bbfd484731d77bff96a72ddb0c9434a4c47d574a in / 
# Wed, 20 Apr 2022 07:18:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:011248ff897363bbfbb6391be232b1fe34d171c43e4402a37d13bb16665622b4`  
		Last Modified: Wed, 20 Apr 2022 07:34:41 GMT  
		Size: 44.1 MB (44122702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:a97f4eb34aabcdb7126cbe687c55445d94a078e5971424a12fa506fef2bf84fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150773 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b61cbd9c21b49168dc855cc5e9a2d126084056032122ccf98fbc1f66cc98d0a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:57 GMT
ADD file:75eab034a8a41af1c3746a6596e4b7a7c02c2b4d88c43ae6ee30fe838b58d074 in / 
# Wed, 20 Apr 2022 13:28:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e82ea888fad1689582caf74dfff67eaad7573350b522a40c2e0c3eeefde3b35`  
		Last Modified: Wed, 20 Apr 2022 13:45:48 GMT  
		Size: 42.2 MB (42150773 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7734c71642380c7a021628e69b4abeec707f7d1360d27fbe429cd61f2f899b47
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212037 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:524587322dcec9b3ceacb3adee033224d89cbd887e2504ecb0f086f55dc39745`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:47 GMT
ADD file:6eba819e0e2efda877840d26ec82d97b5cc971ac347cfa420006e49d98af40da in / 
# Wed, 20 Apr 2022 04:29:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3f84d18376d0447d1292f58ace6bd0081a20c00a2682f97974e0bb2caa786dab`  
		Last Modified: Wed, 20 Apr 2022 04:37:01 GMT  
		Size: 43.2 MB (43212037 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418` - linux; 386

```console
$ docker pull debian@sha256:b147f115d8ade1e350c149bd9cc7dd1184058b4c2ad10c427edab7428f017947
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:28a35e3e2d7c4c55bd4a0e4f10aff948323171594dd5a52b757cf3b2985212ae`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:16 GMT
ADD file:5c34d7058c53f0327bc7b12fc42fd6e752870d7b5b982bad1b9e9d0894d75d04 in / 
# Wed, 20 Apr 2022 07:38:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:79d3d2a2441cfff5e929e595c3a88d3e53fe158136cd9b6443fe762ef335980c`  
		Last Modified: Wed, 20 Apr 2022 07:46:10 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20220418-slim`

```console
$ docker pull debian@sha256:ddea78a1eaa28f583e74c201bca3e6d3d6ff15c4cbc252c0ef9c2016a9d59cbb
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:3bc15e1bee5479b1532269324468bca0e59bf019ef08f7b5144ef0d6241a1ccc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567257 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5a8eec4b1fe315bc0abd18a6d2d7cb7241ae154b01de5d7a468750ceaf380212`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:07 GMT
ADD file:85fd974aab4d79dd8465735c942a30e0ad61da73b79ef5eea753a00139178266 in / 
# Wed, 20 Apr 2022 04:44:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:05867de039c2d6cb04b6c4391bcf93cf3b4ccf883863fc92af5bf97e5cece1cd`  
		Last Modified: Wed, 20 Apr 2022 04:49:56 GMT  
		Size: 22.6 MB (22567257 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:12c69cc4ae2792734bc8f63c9fa5122648dd15be024556997890c1634f8e95eb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240604 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c54c582bb22f6c86506ba4393b5cc223be859afac71a7bf10f02378066005f8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:18:47 GMT
ADD file:78963a3c18c28e7faa7fa3e5ce9469121b1e51737552d7d6f013087d1037d1a8 in / 
# Wed, 20 Apr 2022 07:18:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7db5d0726552dd4b618941567fe168a92dbbef3e54e956165e5d35addbed6291`  
		Last Modified: Wed, 20 Apr 2022 07:35:13 GMT  
		Size: 21.2 MB (21240604 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:13a347a129ea812d2209fef4b421717f237b1d30fccf791b284c17319cce1859
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359811 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:75f5ddf9c9dde7af744caef6afc0f649c898875493c7011aa05f30fa86f073c6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:29:31 GMT
ADD file:6abac199821d507945e290be831ea591731440771b11ca01851092cf965a8334 in / 
# Wed, 20 Apr 2022 13:29:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3625c26de6ae1affffa458d4722477622f5dd72575d551a3404b81ada7ba5d30`  
		Last Modified: Wed, 20 Apr 2022 13:46:19 GMT  
		Size: 19.4 MB (19359811 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e4647ac66c5d9c2ef49c61c9a142b986b633b7772a7ec324b27443975c476c0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424126 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68831a04810d1fdfd4f265a0a466adff10ddb49005abad3fbdf1297701833768`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:02 GMT
ADD file:109383aa434619bda89bb91926133df7afbd867763ce64f1c62e9ffce031097e in / 
# Wed, 20 Apr 2022 04:30:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:de91f9eb7e1650538b6b00d81bf8755ae2d149d6d1a96d7b1cae3a44e48b0e9a`  
		Last Modified: Wed, 20 Apr 2022 04:37:21 GMT  
		Size: 20.4 MB (20424126 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:ec46464be5e55fe6d3f582c2b333091681e85390a7d1bc5401d3397dafbcd916
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202148 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ee2e8be0c9b9787825390d523e72e1947861b9017a951058cec1af3f51d6c0c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:30 GMT
ADD file:d9a957d474bc5a74bbd0b3d6e277d7d300b0b99b2063a3b5ad5c172014447b93 in / 
# Wed, 20 Apr 2022 07:38:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a184e3969934cfd3ae593804765aa3e4a28d46d01a4d07624f57009ebb3a30ad`  
		Last Modified: Wed, 20 Apr 2022 07:46:31 GMT  
		Size: 23.2 MB (23202148 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-backports`

```console
$ docker pull debian@sha256:ba5b2e98f5255e74d0e20ac76dafa8bc05f6e7778b21dd608a44d1c68a2f75b5
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:e99c785c67deec8faf233f804f7b18d787f9f2851fc39297d0dce7e0ca5ff435
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427687 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:096abacdf3e787598a4806f906f8e321342157d38343d4fd281f10e655f8968e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:43:57 GMT
ADD file:91c1b90dda98c0f3328e5c763b36b29831467c0a0cf52df5359551154d2e6355 in / 
# Wed, 20 Apr 2022 04:43:58 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:44:00 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d195d42244aab04cff1a0e88441cc5fd6954f65abf0514e85b72d7c4a631445d`  
		Last Modified: Wed, 20 Apr 2022 04:49:36 GMT  
		Size: 45.4 MB (45427461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:872d25904c5d3caad43216df724674640e8d212683e520aa153dcc575450739e`  
		Last Modified: Wed, 20 Apr 2022 04:49:46 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:22a23b548de996a944d14de8f21c6ed272bbbdd00a07bdd7a655ef22608e1f7a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122930 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66336408614f60d2352905373c02e766809b5c841c75a3c11a001955f1c247ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:18:16 GMT
ADD file:e9d593b94101c656c0e16180bbfd484731d77bff96a72ddb0c9434a4c47d574a in / 
# Wed, 20 Apr 2022 07:18:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:18:29 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:011248ff897363bbfbb6391be232b1fe34d171c43e4402a37d13bb16665622b4`  
		Last Modified: Wed, 20 Apr 2022 07:34:41 GMT  
		Size: 44.1 MB (44122702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3562a03d0a7e26b75f2bb976b97802db99261413529365b6568934c8c2991089`  
		Last Modified: Wed, 20 Apr 2022 07:34:53 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:72ab5e7a3bcffecfa475373311f30faf523ce8c3251f7061f57b004c582082c3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42151003 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:905cf436c215c3fbb733f1f4f480987fe76008e8d7c55ea7dd5de54abe0536b5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:28:57 GMT
ADD file:75eab034a8a41af1c3746a6596e4b7a7c02c2b4d88c43ae6ee30fe838b58d074 in / 
# Wed, 20 Apr 2022 13:28:58 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:29:10 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3e82ea888fad1689582caf74dfff67eaad7573350b522a40c2e0c3eeefde3b35`  
		Last Modified: Wed, 20 Apr 2022 13:45:48 GMT  
		Size: 42.2 MB (42150773 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f490d5e892d1157295f8134b36a617579963f5ec24428f928c322e620b048c95`  
		Last Modified: Wed, 20 Apr 2022 13:46:00 GMT  
		Size: 230.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a99f13b8cb03897ae7b3f384bdf5d77d91060837326b3e956a6062d556e74af7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:709ddd6fce1f7082cb9dd79471ed6d039d554ed669911e1f2f12a8c5a258b00d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:29:47 GMT
ADD file:6eba819e0e2efda877840d26ec82d97b5cc971ac347cfa420006e49d98af40da in / 
# Wed, 20 Apr 2022 04:29:48 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:29:54 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3f84d18376d0447d1292f58ace6bd0081a20c00a2682f97974e0bb2caa786dab`  
		Last Modified: Wed, 20 Apr 2022 04:37:01 GMT  
		Size: 43.2 MB (43212037 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:eb6c10697208d9361ae5b3cc2f57693c758f3cf4c64ed56e86cedf5170c57259`  
		Last Modified: Wed, 20 Apr 2022 04:37:11 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:1a489638f5a66d02fb515f6586fdf06b508045d490a7adb74a4115409c890820
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147958 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2c875f15cb642d65332b4add4022ecb80c3568415cffd12c64135406cb2fb448`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:16 GMT
ADD file:5c34d7058c53f0327bc7b12fc42fd6e752870d7b5b982bad1b9e9d0894d75d04 in / 
# Wed, 20 Apr 2022 07:38:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:38:22 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:79d3d2a2441cfff5e929e595c3a88d3e53fe158136cd9b6443fe762ef335980c`  
		Last Modified: Wed, 20 Apr 2022 07:46:10 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:76ded3c2b5a3a6183ee4bdfc5fecde2a7e759750bedfc100ed008b97a2f26551`  
		Last Modified: Wed, 20 Apr 2022 07:46:20 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-slim`

```console
$ docker pull debian@sha256:ddea78a1eaa28f583e74c201bca3e6d3d6ff15c4cbc252c0ef9c2016a9d59cbb
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:3bc15e1bee5479b1532269324468bca0e59bf019ef08f7b5144ef0d6241a1ccc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567257 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5a8eec4b1fe315bc0abd18a6d2d7cb7241ae154b01de5d7a468750ceaf380212`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:07 GMT
ADD file:85fd974aab4d79dd8465735c942a30e0ad61da73b79ef5eea753a00139178266 in / 
# Wed, 20 Apr 2022 04:44:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:05867de039c2d6cb04b6c4391bcf93cf3b4ccf883863fc92af5bf97e5cece1cd`  
		Last Modified: Wed, 20 Apr 2022 04:49:56 GMT  
		Size: 22.6 MB (22567257 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:12c69cc4ae2792734bc8f63c9fa5122648dd15be024556997890c1634f8e95eb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240604 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c54c582bb22f6c86506ba4393b5cc223be859afac71a7bf10f02378066005f8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:18:47 GMT
ADD file:78963a3c18c28e7faa7fa3e5ce9469121b1e51737552d7d6f013087d1037d1a8 in / 
# Wed, 20 Apr 2022 07:18:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7db5d0726552dd4b618941567fe168a92dbbef3e54e956165e5d35addbed6291`  
		Last Modified: Wed, 20 Apr 2022 07:35:13 GMT  
		Size: 21.2 MB (21240604 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:13a347a129ea812d2209fef4b421717f237b1d30fccf791b284c17319cce1859
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359811 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:75f5ddf9c9dde7af744caef6afc0f649c898875493c7011aa05f30fa86f073c6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:29:31 GMT
ADD file:6abac199821d507945e290be831ea591731440771b11ca01851092cf965a8334 in / 
# Wed, 20 Apr 2022 13:29:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3625c26de6ae1affffa458d4722477622f5dd72575d551a3404b81ada7ba5d30`  
		Last Modified: Wed, 20 Apr 2022 13:46:19 GMT  
		Size: 19.4 MB (19359811 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e4647ac66c5d9c2ef49c61c9a142b986b633b7772a7ec324b27443975c476c0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424126 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68831a04810d1fdfd4f265a0a466adff10ddb49005abad3fbdf1297701833768`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:02 GMT
ADD file:109383aa434619bda89bb91926133df7afbd867763ce64f1c62e9ffce031097e in / 
# Wed, 20 Apr 2022 04:30:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:de91f9eb7e1650538b6b00d81bf8755ae2d149d6d1a96d7b1cae3a44e48b0e9a`  
		Last Modified: Wed, 20 Apr 2022 04:37:21 GMT  
		Size: 20.4 MB (20424126 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:ec46464be5e55fe6d3f582c2b333091681e85390a7d1bc5401d3397dafbcd916
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202148 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ee2e8be0c9b9787825390d523e72e1947861b9017a951058cec1af3f51d6c0c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:30 GMT
ADD file:d9a957d474bc5a74bbd0b3d6e277d7d300b0b99b2063a3b5ad5c172014447b93 in / 
# Wed, 20 Apr 2022 07:38:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a184e3969934cfd3ae593804765aa3e4a28d46d01a4d07624f57009ebb3a30ad`  
		Last Modified: Wed, 20 Apr 2022 07:46:31 GMT  
		Size: 23.2 MB (23202148 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable`

```console
$ docker pull debian@sha256:a0977a726fd1417ed993a410d3746eeb48fcb1a023f21202158b5e4e12633b6f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:fe51ad4875f795f8ed63a206e120873cf9fdd6c45a4f9baa74ec2ba73e9db5bc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437032 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:466361bc01e1c1e836ecf3ce677e21f93c400e8ffb553c45f79eca5a51167503`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:17 GMT
ADD file:8a4f1d7e5e52aa53b75f2808ce11c808a8e77c6404891742f522fb7a7e986f37 in / 
# Wed, 20 Apr 2022 04:44:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c45c7abc4a6b3c318a73fe816c8198b2491c4dc321a9ac23dd5ee43d702623fa`  
		Last Modified: Wed, 20 Apr 2022 04:50:13 GMT  
		Size: 50.4 MB (50437032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:81f0912733368ecd311c8581e2c474bef7293583484f1a968545b2ffe1e3421e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153585 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9aa103758c5c429d75086eb049f8c2b467768f2bf660efbe3740cb9522b80519`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:19:14 GMT
ADD file:4d7d89b6490098f2e1b62cd1f0c77de4312f6ebc5563a0f8a8750c4818f47a2f in / 
# Wed, 20 Apr 2022 07:19:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fc45a926bdf002ad9abd6b8b8688c6fe998933998dc0c4ad013d1479dccac1c2`  
		Last Modified: Wed, 20 Apr 2022 07:35:53 GMT  
		Size: 48.2 MB (48153585 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:82be2cda0dac113ba856c9fcb1770283bb11f9a421262c62af574eb4f241d3cc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908154 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e343a7589dc4639aba88feb4fbd0639257234736e965d301581ae8caf1a39056`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:30:00 GMT
ADD file:0d2ae1f5342ed0819efeef05b93f607aa99aee31678c9038b9f8f020d1e2addd in / 
# Wed, 20 Apr 2022 13:30:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0d3a3a78bf992aea1edcd4dcf1832e362ae3cbbe593fe5694fbf654ac079a221`  
		Last Modified: Wed, 20 Apr 2022 13:46:57 GMT  
		Size: 45.9 MB (45908154 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e9dd1f87b6c7bafd547c1364f152bd2848b3d408f19a5ddae35d38fcfff0a370
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:62647055f4daece1089d528a9d71b0d9c8f44d02a6d01659c21b7ceac82c0be0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:12 GMT
ADD file:55626f1299c943934f8cd38ec9a9cde4d4b8beede74a23774f4fe6ca0b7ec45a in / 
# Wed, 20 Apr 2022 04:30:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d6f4a9b5619dae92e0d604298044fd2a8ed8c396fe01e6ab40d3f2bd5605473`  
		Last Modified: Wed, 20 Apr 2022 04:37:38 GMT  
		Size: 49.2 MB (49227718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:b292634cba5534ded5412290b6dd4903bf8aa2dc0b46ea01d3a59ef037f5eaa7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195838 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b85cc9d2574105fb2a354cfd1b6326c9c4c4a6ab41cb32bb5bba95e3ef4e3056`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:42 GMT
ADD file:19e770beac41792d9baa38c96e3426b5d62b0526bba5ee4c638295c30d0f03f3 in / 
# Wed, 20 Apr 2022 07:38:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:58502976986d74622893d9aa344c104e0e78fb67c58c87fd7088cbe5c6805d35`  
		Last Modified: Wed, 20 Apr 2022 07:46:49 GMT  
		Size: 51.2 MB (51195838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; mips64le

```console
$ docker pull debian@sha256:493667b719427fee80cbf93f1e8c4b8bd9e8bf091284623c85133120d183973f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079873 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ed1b399f78d6447493fedaaf0ef819dae41e15d2e7a2db33ed8173f5a34c83ed`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:44:08 GMT
ADD file:2d8c34802d0b44aabd27e95eff2ce94751098f2cc68aae7f789185fd8aae0d0b in / 
# Tue, 29 Mar 2022 07:44:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bd194f847b7470ac501b6bd3620e76ed1178f748e2cdc221ee013f7d5c74b72d`  
		Last Modified: Tue, 29 Mar 2022 07:55:01 GMT  
		Size: 49.1 MB (49079873 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; ppc64le

```console
$ docker pull debian@sha256:83c90393aef0e87841a20b7492f9d44333a0af291ffd73b80942698ef4af97fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169935 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4792d0c32b18ab0aa01cb3f5d2b42ed525d02c43b0e3db682b27192c8c5d6ab2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:48:12 GMT
ADD file:ed5800f209002e99add8748fd1a1f5701a24178942a2ce172d2bea191a3933a1 in / 
# Wed, 20 Apr 2022 09:48:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e7ad4688b6e90819e677c7353bd462760334d88e75dd149723d5df19b4f8abd2`  
		Last Modified: Wed, 20 Apr 2022 09:58:34 GMT  
		Size: 54.2 MB (54169935 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; s390x

```console
$ docker pull debian@sha256:deb76be7b5d33420738354383ddfdc030e3d602f405ada0a96e442bda92afda3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999263 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15da6ded967e4adabaeb7391ba258b99161c0006b598fa201b59bafebb4af4d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:41:11 GMT
ADD file:6f25f699dbf6ffb4c1440b1d592fca93d523bd9d8db26167cb18b1ab609c5bea in / 
# Wed, 20 Apr 2022 08:41:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e78d245215ba52c813dea79f6c77d96b807db3a4255717eb46d340e59def4bed`  
		Last Modified: Wed, 20 Apr 2022 08:50:34 GMT  
		Size: 49.0 MB (48999263 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20220418`

```console
$ docker pull debian@sha256:4a5ffa8185e687976a8fc1ce5019dd6c00360fe635ab13968b65ccbd7f83f824
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20220418` - linux; amd64

```console
$ docker pull debian@sha256:fe51ad4875f795f8ed63a206e120873cf9fdd6c45a4f9baa74ec2ba73e9db5bc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437032 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:466361bc01e1c1e836ecf3ce677e21f93c400e8ffb553c45f79eca5a51167503`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:17 GMT
ADD file:8a4f1d7e5e52aa53b75f2808ce11c808a8e77c6404891742f522fb7a7e986f37 in / 
# Wed, 20 Apr 2022 04:44:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c45c7abc4a6b3c318a73fe816c8198b2491c4dc321a9ac23dd5ee43d702623fa`  
		Last Modified: Wed, 20 Apr 2022 04:50:13 GMT  
		Size: 50.4 MB (50437032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:81f0912733368ecd311c8581e2c474bef7293583484f1a968545b2ffe1e3421e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153585 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9aa103758c5c429d75086eb049f8c2b467768f2bf660efbe3740cb9522b80519`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:19:14 GMT
ADD file:4d7d89b6490098f2e1b62cd1f0c77de4312f6ebc5563a0f8a8750c4818f47a2f in / 
# Wed, 20 Apr 2022 07:19:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fc45a926bdf002ad9abd6b8b8688c6fe998933998dc0c4ad013d1479dccac1c2`  
		Last Modified: Wed, 20 Apr 2022 07:35:53 GMT  
		Size: 48.2 MB (48153585 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:82be2cda0dac113ba856c9fcb1770283bb11f9a421262c62af574eb4f241d3cc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908154 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e343a7589dc4639aba88feb4fbd0639257234736e965d301581ae8caf1a39056`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:30:00 GMT
ADD file:0d2ae1f5342ed0819efeef05b93f607aa99aee31678c9038b9f8f020d1e2addd in / 
# Wed, 20 Apr 2022 13:30:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0d3a3a78bf992aea1edcd4dcf1832e362ae3cbbe593fe5694fbf654ac079a221`  
		Last Modified: Wed, 20 Apr 2022 13:46:57 GMT  
		Size: 45.9 MB (45908154 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e9dd1f87b6c7bafd547c1364f152bd2848b3d408f19a5ddae35d38fcfff0a370
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:62647055f4daece1089d528a9d71b0d9c8f44d02a6d01659c21b7ceac82c0be0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:12 GMT
ADD file:55626f1299c943934f8cd38ec9a9cde4d4b8beede74a23774f4fe6ca0b7ec45a in / 
# Wed, 20 Apr 2022 04:30:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d6f4a9b5619dae92e0d604298044fd2a8ed8c396fe01e6ab40d3f2bd5605473`  
		Last Modified: Wed, 20 Apr 2022 04:37:38 GMT  
		Size: 49.2 MB (49227718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; 386

```console
$ docker pull debian@sha256:b292634cba5534ded5412290b6dd4903bf8aa2dc0b46ea01d3a59ef037f5eaa7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51195838 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b85cc9d2574105fb2a354cfd1b6326c9c4c4a6ab41cb32bb5bba95e3ef4e3056`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:42 GMT
ADD file:19e770beac41792d9baa38c96e3426b5d62b0526bba5ee4c638295c30d0f03f3 in / 
# Wed, 20 Apr 2022 07:38:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:58502976986d74622893d9aa344c104e0e78fb67c58c87fd7088cbe5c6805d35`  
		Last Modified: Wed, 20 Apr 2022 07:46:49 GMT  
		Size: 51.2 MB (51195838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:83c90393aef0e87841a20b7492f9d44333a0af291ffd73b80942698ef4af97fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54169935 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4792d0c32b18ab0aa01cb3f5d2b42ed525d02c43b0e3db682b27192c8c5d6ab2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:48:12 GMT
ADD file:ed5800f209002e99add8748fd1a1f5701a24178942a2ce172d2bea191a3933a1 in / 
# Wed, 20 Apr 2022 09:48:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e7ad4688b6e90819e677c7353bd462760334d88e75dd149723d5df19b4f8abd2`  
		Last Modified: Wed, 20 Apr 2022 09:58:34 GMT  
		Size: 54.2 MB (54169935 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418` - linux; s390x

```console
$ docker pull debian@sha256:deb76be7b5d33420738354383ddfdc030e3d602f405ada0a96e442bda92afda3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999263 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15da6ded967e4adabaeb7391ba258b99161c0006b598fa201b59bafebb4af4d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:41:11 GMT
ADD file:6f25f699dbf6ffb4c1440b1d592fca93d523bd9d8db26167cb18b1ab609c5bea in / 
# Wed, 20 Apr 2022 08:41:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e78d245215ba52c813dea79f6c77d96b807db3a4255717eb46d340e59def4bed`  
		Last Modified: Wed, 20 Apr 2022 08:50:34 GMT  
		Size: 49.0 MB (48999263 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20220418-slim`

```console
$ docker pull debian@sha256:7e4b0ee87c13397bbaaa8a72498768acdf31171f853d31c641531a788fb632d0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:fcfaf614fb3e521f4cde8ab7c057d97475b525b5d777adbe688a88ac1525ed95
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f69908c1ca1fdf1c1321779a9fada4771d2d4e403d431f042e08b6932d7bbf1e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:27 GMT
ADD file:78cbde2416efb20ac08feec22e7bd8e41f4ff58645046e900d0d992d6dd55f86 in / 
# Wed, 20 Apr 2022 04:44:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:272e2d30fa189711da9e9097a828de4c323b728ef2b2afc5bc25a41f02f038a5`  
		Last Modified: Wed, 20 Apr 2022 04:50:34 GMT  
		Size: 27.1 MB (27140682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6925aed715ed9c0469cb3d1e7e462ef4be4d8433911a0eb80f1269c0e2c5d006
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec3c54b7c0d102288049cfec922401bea8da8088cc36ebcc400b485830bbb044`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:19:47 GMT
ADD file:5bee6255d49c9cfddbecbefcdc104b4306ece4acd1d0fc2743b30610779d0794 in / 
# Wed, 20 Apr 2022 07:19:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:78805a772fbb69f9bb4050c0bfb7a25d733daaf14d3e953ebfcb38caba84b31d`  
		Last Modified: Wed, 20 Apr 2022 07:36:26 GMT  
		Size: 24.9 MB (24889607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e5342f5368e115250c5cc4b6fd8cfba51608f3d92a891ab9eef7195d3b18535e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747856 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ecfde816f44d3d4165a9a7c12bf2ed69e28fd601ef89676fe3b54d610e46f384`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:30:48 GMT
ADD file:a23171df0236105cafd0b81abee1e58eda43453ab412639387e201a668eee153 in / 
# Wed, 20 Apr 2022 13:30:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a1cc08d8247264a1a34812857c808024bbd42ec7e7eba6113d19df7f83c6e916`  
		Last Modified: Wed, 20 Apr 2022 13:47:29 GMT  
		Size: 22.7 MB (22747856 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e0ac43876c707b92ed26a6a06e2b8445266aae9c35434f2aff86dbae4a52e58
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908344 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3fea65231107b1f04fd3ea691222daa8b7d0914915e346acc0465200329663d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:27 GMT
ADD file:953c182230db8c10c658b8fbb6943bf8a90648381dbb2807155a32efdba70d68 in / 
# Wed, 20 Apr 2022 04:30:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:72ceb0f3a15007a2f0eafa1a6c47f9762d9825985ee6ba1b0280dd94312be509`  
		Last Modified: Wed, 20 Apr 2022 04:38:00 GMT  
		Size: 25.9 MB (25908344 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:4c1775b4d7373e8d594cf466f97b9d16d5cdb70f592f18f3c027dfb5ac638be6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aa26d17d1280cfde0345ce97331b65dd34382176affc63a54e34fb0f1576d5d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:57 GMT
ADD file:8a554bac3bb871609f25c7fd7275d3df3d3f3f534fe9d8608a7a98d93ddd5a68 in / 
# Wed, 20 Apr 2022 07:38:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3693f705d788ad5617ef77629d6767e24da8f5f540c7faedb5e8b4504387482c`  
		Last Modified: Wed, 20 Apr 2022 07:47:12 GMT  
		Size: 27.8 MB (27799819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:18a185089a20ecce23fd7d354f64da58eed8bb6efd0ff5ae0efd976b97596bf4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65ce1b643da9683e2904023382acc11ae1f5f68939a54187a57cb9ba84596b99`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:48:49 GMT
ADD file:db40596dddefc62ffbcd087a42101a2dd7635fff46b22e32cf787e5f55bd060c in / 
# Wed, 20 Apr 2022 09:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:48e37c1d7497ed0081a5cb107b67d0a6b823f4fa27e3f36dfc6e32db04a852fc`  
		Last Modified: Wed, 20 Apr 2022 09:59:05 GMT  
		Size: 30.6 MB (30560300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:5a5b38b8d2dbc70ca2ad898d8d5d4c64597aec1ca85d124db39a4c9c68f514f6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758584 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:866a084d156398ecab7f332407db58923d8424d589192b5d1644ba7bc2ac30bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:41:44 GMT
ADD file:9a04b73a6cba49d084c6a9370246fd1892448e8ca7e7b5d032139c3ae622532e in / 
# Wed, 20 Apr 2022 08:41:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56bd0746d9bc73e9a47c4641e596cc658dfd21712896d6c4b834db6aa02c890c`  
		Last Modified: Wed, 20 Apr 2022 08:50:50 GMT  
		Size: 25.8 MB (25758584 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-backports`

```console
$ docker pull debian@sha256:687c9700e73299c5eaf489b417098cc7f3bd4449c7b2a20b86ed659f3ff44668
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:96cbe179bda137453706c2fb1bcff9791bf7954502e728fa76eb12b4a61f3726
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437256 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b8d00394c540c916c45931972a9321db681bbcb0066c0954d6f37727b94f7280`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:17 GMT
ADD file:8a4f1d7e5e52aa53b75f2808ce11c808a8e77c6404891742f522fb7a7e986f37 in / 
# Wed, 20 Apr 2022 04:44:17 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:44:20 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c45c7abc4a6b3c318a73fe816c8198b2491c4dc321a9ac23dd5ee43d702623fa`  
		Last Modified: Wed, 20 Apr 2022 04:50:13 GMT  
		Size: 50.4 MB (50437032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5b310eb2a533b74f3b684fae98b6af97bd425384fc31bc31856ef9557a5ffd74`  
		Last Modified: Wed, 20 Apr 2022 04:50:23 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:9eef1bfa84a63b32add0f6fb0d3eb368376705d1a0b0f5d13d76ae8acb39e680
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48153811 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7a6ddd233e439acef3129bf25994a738fe0479e34c70a12c1e1a5a691b166971`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:19:14 GMT
ADD file:4d7d89b6490098f2e1b62cd1f0c77de4312f6ebc5563a0f8a8750c4818f47a2f in / 
# Wed, 20 Apr 2022 07:19:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:19:28 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:fc45a926bdf002ad9abd6b8b8688c6fe998933998dc0c4ad013d1479dccac1c2`  
		Last Modified: Wed, 20 Apr 2022 07:35:53 GMT  
		Size: 48.2 MB (48153585 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f236f4cc3ae736f495e1ec7a18233f6ceb35d6a5db334267cf17900f61895926`  
		Last Modified: Wed, 20 Apr 2022 07:36:04 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:bfeb4a737667f2935acceabe4d3d3b3c48189b677395e9ea29711ab668579118
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45908382 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:71f6d35c27089486b5493aa29cd8fd44c5c33ac37bd77e1e16d88b345fdbfb7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:30:00 GMT
ADD file:0d2ae1f5342ed0819efeef05b93f607aa99aee31678c9038b9f8f020d1e2addd in / 
# Wed, 20 Apr 2022 13:30:10 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:30:25 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0d3a3a78bf992aea1edcd4dcf1832e362ae3cbbe593fe5694fbf654ac079a221`  
		Last Modified: Wed, 20 Apr 2022 13:46:57 GMT  
		Size: 45.9 MB (45908154 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a9991e4d531ad7482f4e2e0445c0d59dea23f1822b3c466cce40c08a8e15526b`  
		Last Modified: Wed, 20 Apr 2022 13:47:08 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2cc0a75cc76b25edd5f8f5125fb35300d64aa8c3bbe5623f25d471c640f3a726
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49227940 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:04c2a64e9ebb6d4acc67c28de26324c208f6a40e50a89548a9209d30261e08ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:12 GMT
ADD file:55626f1299c943934f8cd38ec9a9cde4d4b8beede74a23774f4fe6ca0b7ec45a in / 
# Wed, 20 Apr 2022 04:30:13 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:30:19 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6d6f4a9b5619dae92e0d604298044fd2a8ed8c396fe01e6ab40d3f2bd5605473`  
		Last Modified: Wed, 20 Apr 2022 04:37:38 GMT  
		Size: 49.2 MB (49227718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0a0d06947fd270084fb3f4d8cbe2dd6f4b7b7e3ea4b15b9ddce40569f59960f5`  
		Last Modified: Wed, 20 Apr 2022 04:37:49 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:e48dcd70d79303ea68def419ed805d2be9cd9c7fe944b6c7e036b70f715f1b14
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51196061 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66e73c39534eccffd38212da259cf541310ab2843a0456c94d9cb17135f6cbf1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:42 GMT
ADD file:19e770beac41792d9baa38c96e3426b5d62b0526bba5ee4c638295c30d0f03f3 in / 
# Wed, 20 Apr 2022 07:38:43 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:38:49 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:58502976986d74622893d9aa344c104e0e78fb67c58c87fd7088cbe5c6805d35`  
		Last Modified: Wed, 20 Apr 2022 07:46:49 GMT  
		Size: 51.2 MB (51195838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ef6d1feb32dc131b86ad765ef22b9b5d40bf7f834b6189442563ae49404f4fa9`  
		Last Modified: Wed, 20 Apr 2022 07:47:00 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:195cfc4c88e902d9fd7799b55c56a54dc4440d0756a646378d2a4c17b581b763
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49080099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef4f643d413d25802e663ee90b69ccba4a8b00d1502dcde33ebda8e98951edf3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:44:08 GMT
ADD file:2d8c34802d0b44aabd27e95eff2ce94751098f2cc68aae7f789185fd8aae0d0b in / 
# Tue, 29 Mar 2022 07:44:13 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:44:23 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:bd194f847b7470ac501b6bd3620e76ed1178f748e2cdc221ee013f7d5c74b72d`  
		Last Modified: Tue, 29 Mar 2022 07:55:01 GMT  
		Size: 49.1 MB (49079873 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c9a3c96700fbe540f5b46f50c924617ad1e1b8807c5af7c41c263a598e31488f`  
		Last Modified: Tue, 29 Mar 2022 07:55:17 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:d23065ab45378951f00db916eb4aba0d4bace807f7447c3ce41d1703e247abca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54170162 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:561ac87780f529176713452b3f3f0e67e5d5cb08f7529d938be52cbf9d79bc2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:48:12 GMT
ADD file:ed5800f209002e99add8748fd1a1f5701a24178942a2ce172d2bea191a3933a1 in / 
# Wed, 20 Apr 2022 09:48:20 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:48:35 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e7ad4688b6e90819e677c7353bd462760334d88e75dd149723d5df19b4f8abd2`  
		Last Modified: Wed, 20 Apr 2022 09:58:34 GMT  
		Size: 54.2 MB (54169935 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:44e0d9921edb11b71468cbb29cab97b4395614d904fafa3c1392593e8e9b6bb8`  
		Last Modified: Wed, 20 Apr 2022 09:58:50 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; s390x

```console
$ docker pull debian@sha256:8bc60865b92ae4ed66f9aea5c2b7fcddd226a9d2ff7084623f3c44a450f4ba59
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (48999487 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c36bec06d9249882acf48ac9348e6da4eea39fc34a60da5d84a4d40305986165`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:41:11 GMT
ADD file:6f25f699dbf6ffb4c1440b1d592fca93d523bd9d8db26167cb18b1ab609c5bea in / 
# Wed, 20 Apr 2022 08:41:18 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:41:29 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e78d245215ba52c813dea79f6c77d96b807db3a4255717eb46d340e59def4bed`  
		Last Modified: Wed, 20 Apr 2022 08:50:34 GMT  
		Size: 49.0 MB (48999263 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3a528526ff08e607b03c9bb88ea282d83f01947cbf231612b0e4e8d551af5e14`  
		Last Modified: Wed, 20 Apr 2022 08:50:42 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-slim`

```console
$ docker pull debian@sha256:7abfb6c78ecb28872340f0ba3c07ac9e04ff0d6cbb982624b01e1264ca7c9838
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:fcfaf614fb3e521f4cde8ab7c057d97475b525b5d777adbe688a88ac1525ed95
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27140682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f69908c1ca1fdf1c1321779a9fada4771d2d4e403d431f042e08b6932d7bbf1e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:27 GMT
ADD file:78cbde2416efb20ac08feec22e7bd8e41f4ff58645046e900d0d992d6dd55f86 in / 
# Wed, 20 Apr 2022 04:44:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:272e2d30fa189711da9e9097a828de4c323b728ef2b2afc5bc25a41f02f038a5`  
		Last Modified: Wed, 20 Apr 2022 04:50:34 GMT  
		Size: 27.1 MB (27140682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6925aed715ed9c0469cb3d1e7e462ef4be4d8433911a0eb80f1269c0e2c5d006
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24889607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec3c54b7c0d102288049cfec922401bea8da8088cc36ebcc400b485830bbb044`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:19:47 GMT
ADD file:5bee6255d49c9cfddbecbefcdc104b4306ece4acd1d0fc2743b30610779d0794 in / 
# Wed, 20 Apr 2022 07:19:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:78805a772fbb69f9bb4050c0bfb7a25d733daaf14d3e953ebfcb38caba84b31d`  
		Last Modified: Wed, 20 Apr 2022 07:36:26 GMT  
		Size: 24.9 MB (24889607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e5342f5368e115250c5cc4b6fd8cfba51608f3d92a891ab9eef7195d3b18535e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747856 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ecfde816f44d3d4165a9a7c12bf2ed69e28fd601ef89676fe3b54d610e46f384`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:30:48 GMT
ADD file:a23171df0236105cafd0b81abee1e58eda43453ab412639387e201a668eee153 in / 
# Wed, 20 Apr 2022 13:30:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a1cc08d8247264a1a34812857c808024bbd42ec7e7eba6113d19df7f83c6e916`  
		Last Modified: Wed, 20 Apr 2022 13:47:29 GMT  
		Size: 22.7 MB (22747856 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e0ac43876c707b92ed26a6a06e2b8445266aae9c35434f2aff86dbae4a52e58
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25908344 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3fea65231107b1f04fd3ea691222daa8b7d0914915e346acc0465200329663d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:27 GMT
ADD file:953c182230db8c10c658b8fbb6943bf8a90648381dbb2807155a32efdba70d68 in / 
# Wed, 20 Apr 2022 04:30:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:72ceb0f3a15007a2f0eafa1a6c47f9762d9825985ee6ba1b0280dd94312be509`  
		Last Modified: Wed, 20 Apr 2022 04:38:00 GMT  
		Size: 25.9 MB (25908344 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:4c1775b4d7373e8d594cf466f97b9d16d5cdb70f592f18f3c027dfb5ac638be6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27799819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aa26d17d1280cfde0345ce97331b65dd34382176affc63a54e34fb0f1576d5d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:38:57 GMT
ADD file:8a554bac3bb871609f25c7fd7275d3df3d3f3f534fe9d8608a7a98d93ddd5a68 in / 
# Wed, 20 Apr 2022 07:38:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3693f705d788ad5617ef77629d6767e24da8f5f540c7faedb5e8b4504387482c`  
		Last Modified: Wed, 20 Apr 2022 07:47:12 GMT  
		Size: 27.8 MB (27799819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:1dab58231b0e0205c927862086c5b0cb87c1b1770b83e7193170586ac1ef713a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25818056 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fd5aa256be94693865ea1645c2404ca33c67d35612e2679c99f39de1989aeab6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:44:43 GMT
ADD file:e3a4243ec1871f07861ddd9157c9fe8d1f174d1cbe305a212d953e5c75202af7 in / 
# Tue, 29 Mar 2022 07:44:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e908cfaa72d3700cf03f139d17be027e5b18628cc771ea8ec63cade0a3456322`  
		Last Modified: Tue, 29 Mar 2022 07:55:40 GMT  
		Size: 25.8 MB (25818056 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:18a185089a20ecce23fd7d354f64da58eed8bb6efd0ff5ae0efd976b97596bf4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30560300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65ce1b643da9683e2904023382acc11ae1f5f68939a54187a57cb9ba84596b99`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:48:49 GMT
ADD file:db40596dddefc62ffbcd087a42101a2dd7635fff46b22e32cf787e5f55bd060c in / 
# Wed, 20 Apr 2022 09:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:48e37c1d7497ed0081a5cb107b67d0a6b823f4fa27e3f36dfc6e32db04a852fc`  
		Last Modified: Wed, 20 Apr 2022 09:59:05 GMT  
		Size: 30.6 MB (30560300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:5a5b38b8d2dbc70ca2ad898d8d5d4c64597aec1ca85d124db39a4c9c68f514f6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25758584 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:866a084d156398ecab7f332407db58923d8424d589192b5d1644ba7bc2ac30bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:41:44 GMT
ADD file:9a04b73a6cba49d084c6a9370246fd1892448e8ca7e7b5d032139c3ae622532e in / 
# Wed, 20 Apr 2022 08:41:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56bd0746d9bc73e9a47c4641e596cc658dfd21712896d6c4b834db6aa02c890c`  
		Last Modified: Wed, 20 Apr 2022 08:50:50 GMT  
		Size: 25.8 MB (25758584 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy`

```console
$ docker pull debian@sha256:db021594d6b58c77ff249ae6e928c9c50f75b88558a89c5862f7c94e853ff145
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy` - linux; amd64

```console
$ docker pull debian@sha256:44263d20df1fc634877f59256459bf6ac75502a1e17da3a99e0229be04b11fba
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112788 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de96b6ea729e5e4feca37bbd1123cd9d51b5024a3907505f027ff059ec1f566b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:37 GMT
ADD file:859abf2d579747b132742454ad96e32dc879955afff8af84fab63dc41312a0e0 in / 
# Wed, 20 Apr 2022 04:44:37 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:46:10 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:212be0dc3a8ffdc400158a5e3a9812f7413dbb5a86269ff50e41b84b37fdb9f7`  
		Last Modified: Wed, 20 Apr 2022 04:50:51 GMT  
		Size: 56.1 MB (56112566 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c7dd6831f9568bd76d2215a05bb28aeb4f27d47c57302f6f79005a3cca616fcc`  
		Last Modified: Wed, 20 Apr 2022 04:54:02 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v5

```console
$ docker pull debian@sha256:be943ec362a510134e7a610997179b4eeab895d37eca1d3892057311013fe366
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:081ff399031684549d25ad4cd357406851873b8b1987ef2e8ff6d5e3b8b244a1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:15 GMT
ADD file:5cf288f7326d2adfff5d4d3ad0b8951ab86c23ab83cbd1d7799d5c3f7e16a857 in / 
# Wed, 20 Apr 2022 07:20:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:25:04 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:8db2a46143e157b04953cf05194b0c408afa491a70876185538ed8dca91f9f55`  
		Last Modified: Wed, 20 Apr 2022 07:37:08 GMT  
		Size: 53.5 MB (53518487 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ac3edae2b955816dd61420df6f8651c24acb6625bd1dca09a2f2175d2adadf0c`  
		Last Modified: Wed, 20 Apr 2022 07:43:20 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v7

```console
$ docker pull debian@sha256:b8ba5d0c8373da0e6c3733da46dc5ae711e6974478b55de6dcfb4b29e07d8ff3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125893 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:38af82d290611f53d346af21193260ef62a3030754afe471bab1c3d93bee6d73`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:17 GMT
ADD file:0bbaf7ead1d59ceb682d490a4942a357b01272b17a3eb062c89673ee4e166052 in / 
# Wed, 20 Apr 2022 13:31:18 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:36:20 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2bdfb6c7c7f66a4f6d01a8add17283f952a25f6c31c2267fd53d1ff9148edc16`  
		Last Modified: Wed, 20 Apr 2022 13:48:09 GMT  
		Size: 51.1 MB (51125664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:11a0b3d3b2e91dcbab7183261c268de2e4e250580a9431cc3c4ca0372cab0ec0`  
		Last Modified: Wed, 20 Apr 2022 13:53:59 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f2c882c8c2fa7ca3e90ae4d4330e68553c48c42043e5537c185412bc31895fa1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55064030 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15628421858686cee7f25a0ec8d68dc32ffccf4706c58be07f6ca579e7029de8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:37 GMT
ADD file:21175f4b5f80ba5d755041ff362152a8991b53f89e3c6699275e0c99f6ff6acd in / 
# Wed, 20 Apr 2022 04:30:38 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:32:30 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:414d522b7454847f294a8c61536f57f36cb195df33ab1fce8c838de3d5ae109e`  
		Last Modified: Wed, 20 Apr 2022 04:38:18 GMT  
		Size: 55.1 MB (55063805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c0618986ec147bd863b6a8cea7ee692a66f12d587dd47d451895ae7c0ab65baf`  
		Last Modified: Wed, 20 Apr 2022 04:41:45 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; 386

```console
$ docker pull debian@sha256:3c546b9abfe22250450a44bcf01f5f2c42c5b21788f329d61809230e8f8fbcc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154243 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c4ec8e9655311040fafed267a624b16e994247c75cb868493e92fdb077a2026c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:08 GMT
ADD file:92a4df746589380f0ee875ec89574f67fa2f56164bbf20da79eaa902a778eff6 in / 
# Wed, 20 Apr 2022 07:39:09 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:41:14 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b42fb3c96c9db2aa67a5b3b033791b735ef01e8c8f15321f44a085532b703190`  
		Last Modified: Wed, 20 Apr 2022 07:47:30 GMT  
		Size: 57.2 MB (57154017 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6f5559635446e91cbf6742e4a4085e246cc0814a5163f6d729020cab3f169216`  
		Last Modified: Wed, 20 Apr 2022 07:51:04 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; mips64le

```console
$ docker pull debian@sha256:85b7dca648ccc4eb71e265768da1f1fd2fb96752ce24ce7fd481f06567714da6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54453699 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4301c00afbf7eb736c97acfc8c4e5ee32c9dbf6cbbfcc5a8a149a8991373530d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:45:15 GMT
ADD file:8f554d1fa0414c8c5592687851c7ff4646547038f7d464423b75f57ecade0e16 in / 
# Tue, 29 Mar 2022 07:45:20 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:49:36 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:024c5d1fff33205caaf39b2903eafbff594af3ca19d825c46db9f726a37cdcd1`  
		Last Modified: Tue, 29 Mar 2022 07:56:21 GMT  
		Size: 54.5 MB (54453471 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8cda226572e359177f0b24505d16e51c4eb6540796bb0b6465dcfe9f3ddf0d2d`  
		Last Modified: Tue, 29 Mar 2022 08:00:50 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; ppc64le

```console
$ docker pull debian@sha256:bb9f528b66bafce1fc13b4e7a7ec7f4ea731d96baf69d140d84f14fbe6f0d244
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558229 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c8968f4824cb98430a28f23c5618a28e7503aabab2d0a5b9e1c40d4ef2d07971`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:13 GMT
ADD file:fe481980541905937ff6238feab8ead20ee273dfc616bf9e647446ec0f98b510 in / 
# Wed, 20 Apr 2022 09:49:19 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:53:08 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7a4e99688ea8909140343db9b1a83e87f7ddcee1e1ba6d630b79487e5e720199`  
		Last Modified: Wed, 20 Apr 2022 09:59:29 GMT  
		Size: 60.6 MB (60558000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0515ac2c161210aa1b518129517bf62841f211b9608b3d8b946ee301acbc25d4`  
		Last Modified: Wed, 20 Apr 2022 10:02:53 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; s390x

```console
$ docker pull debian@sha256:e0ce76cb0f35098ed00f2fbdce5e98241669e75d37856dafccb6d0aab2aa92f4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54331196 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:536c75ccd7ef158fee7a049176b94ed29052e8eeb69c6117da42e6f99d02da8c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:10 GMT
ADD file:39be572f6c0b3f6f84497807d4bcb370f80b4ac4167f1857002b5571a4dba6b2 in / 
# Wed, 20 Apr 2022 08:42:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:45:59 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2d9529e5f0bb592a3bb453a841aee9e6a765fede232bccdc538ec2c8b96421d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:05 GMT  
		Size: 54.3 MB (54330969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bcd965c6427627772edfce6c727e7a17cef230fa08f03eed51055d19429c5f44`  
		Last Modified: Wed, 20 Apr 2022 08:53:11 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy-20220418`

```console
$ docker pull debian@sha256:368f6dcba099d797febaba8e1eea7390b95dc6304edccc186c0b926e36f1b173
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy-20220418` - linux; amd64

```console
$ docker pull debian@sha256:44263d20df1fc634877f59256459bf6ac75502a1e17da3a99e0229be04b11fba
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112788 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de96b6ea729e5e4feca37bbd1123cd9d51b5024a3907505f027ff059ec1f566b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:37 GMT
ADD file:859abf2d579747b132742454ad96e32dc879955afff8af84fab63dc41312a0e0 in / 
# Wed, 20 Apr 2022 04:44:37 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:46:10 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:212be0dc3a8ffdc400158a5e3a9812f7413dbb5a86269ff50e41b84b37fdb9f7`  
		Last Modified: Wed, 20 Apr 2022 04:50:51 GMT  
		Size: 56.1 MB (56112566 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c7dd6831f9568bd76d2215a05bb28aeb4f27d47c57302f6f79005a3cca616fcc`  
		Last Modified: Wed, 20 Apr 2022 04:54:02 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:be943ec362a510134e7a610997179b4eeab895d37eca1d3892057311013fe366
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:081ff399031684549d25ad4cd357406851873b8b1987ef2e8ff6d5e3b8b244a1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:15 GMT
ADD file:5cf288f7326d2adfff5d4d3ad0b8951ab86c23ab83cbd1d7799d5c3f7e16a857 in / 
# Wed, 20 Apr 2022 07:20:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:25:04 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:8db2a46143e157b04953cf05194b0c408afa491a70876185538ed8dca91f9f55`  
		Last Modified: Wed, 20 Apr 2022 07:37:08 GMT  
		Size: 53.5 MB (53518487 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ac3edae2b955816dd61420df6f8651c24acb6625bd1dca09a2f2175d2adadf0c`  
		Last Modified: Wed, 20 Apr 2022 07:43:20 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:b8ba5d0c8373da0e6c3733da46dc5ae711e6974478b55de6dcfb4b29e07d8ff3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125893 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:38af82d290611f53d346af21193260ef62a3030754afe471bab1c3d93bee6d73`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:17 GMT
ADD file:0bbaf7ead1d59ceb682d490a4942a357b01272b17a3eb062c89673ee4e166052 in / 
# Wed, 20 Apr 2022 13:31:18 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:36:20 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2bdfb6c7c7f66a4f6d01a8add17283f952a25f6c31c2267fd53d1ff9148edc16`  
		Last Modified: Wed, 20 Apr 2022 13:48:09 GMT  
		Size: 51.1 MB (51125664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:11a0b3d3b2e91dcbab7183261c268de2e4e250580a9431cc3c4ca0372cab0ec0`  
		Last Modified: Wed, 20 Apr 2022 13:53:59 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f2c882c8c2fa7ca3e90ae4d4330e68553c48c42043e5537c185412bc31895fa1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55064030 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15628421858686cee7f25a0ec8d68dc32ffccf4706c58be07f6ca579e7029de8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:37 GMT
ADD file:21175f4b5f80ba5d755041ff362152a8991b53f89e3c6699275e0c99f6ff6acd in / 
# Wed, 20 Apr 2022 04:30:38 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:32:30 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:414d522b7454847f294a8c61536f57f36cb195df33ab1fce8c838de3d5ae109e`  
		Last Modified: Wed, 20 Apr 2022 04:38:18 GMT  
		Size: 55.1 MB (55063805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c0618986ec147bd863b6a8cea7ee692a66f12d587dd47d451895ae7c0ab65baf`  
		Last Modified: Wed, 20 Apr 2022 04:41:45 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; 386

```console
$ docker pull debian@sha256:3c546b9abfe22250450a44bcf01f5f2c42c5b21788f329d61809230e8f8fbcc2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154243 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c4ec8e9655311040fafed267a624b16e994247c75cb868493e92fdb077a2026c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:08 GMT
ADD file:92a4df746589380f0ee875ec89574f67fa2f56164bbf20da79eaa902a778eff6 in / 
# Wed, 20 Apr 2022 07:39:09 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:41:14 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b42fb3c96c9db2aa67a5b3b033791b735ef01e8c8f15321f44a085532b703190`  
		Last Modified: Wed, 20 Apr 2022 07:47:30 GMT  
		Size: 57.2 MB (57154017 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6f5559635446e91cbf6742e4a4085e246cc0814a5163f6d729020cab3f169216`  
		Last Modified: Wed, 20 Apr 2022 07:51:04 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:bb9f528b66bafce1fc13b4e7a7ec7f4ea731d96baf69d140d84f14fbe6f0d244
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558229 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c8968f4824cb98430a28f23c5618a28e7503aabab2d0a5b9e1c40d4ef2d07971`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:13 GMT
ADD file:fe481980541905937ff6238feab8ead20ee273dfc616bf9e647446ec0f98b510 in / 
# Wed, 20 Apr 2022 09:49:19 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:53:08 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7a4e99688ea8909140343db9b1a83e87f7ddcee1e1ba6d630b79487e5e720199`  
		Last Modified: Wed, 20 Apr 2022 09:59:29 GMT  
		Size: 60.6 MB (60558000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0515ac2c161210aa1b518129517bf62841f211b9608b3d8b946ee301acbc25d4`  
		Last Modified: Wed, 20 Apr 2022 10:02:53 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20220418` - linux; s390x

```console
$ docker pull debian@sha256:e0ce76cb0f35098ed00f2fbdce5e98241669e75d37856dafccb6d0aab2aa92f4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54331196 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:536c75ccd7ef158fee7a049176b94ed29052e8eeb69c6117da42e6f99d02da8c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:10 GMT
ADD file:39be572f6c0b3f6f84497807d4bcb370f80b4ac4167f1857002b5571a4dba6b2 in / 
# Wed, 20 Apr 2022 08:42:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:45:59 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2d9529e5f0bb592a3bb453a841aee9e6a765fede232bccdc538ec2c8b96421d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:05 GMT  
		Size: 54.3 MB (54330969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bcd965c6427627772edfce6c727e7a17cef230fa08f03eed51055d19429c5f44`  
		Last Modified: Wed, 20 Apr 2022 08:53:11 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid`

```console
$ docker pull debian@sha256:40d7499c456923c4178119360f894b4f4ffeb38b89212a3fad21643902fe12be
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid` - linux; amd64

```console
$ docker pull debian@sha256:8f1d4e94f67959e833de4287c05177d46a4e9715a5372728ad2640299661b8d5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112566 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0160468109b1f378dfc726e16cdae51a1e58d35c4e8b0d0a28302d40094d53ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:37 GMT
ADD file:859abf2d579747b132742454ad96e32dc879955afff8af84fab63dc41312a0e0 in / 
# Wed, 20 Apr 2022 04:44:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:212be0dc3a8ffdc400158a5e3a9812f7413dbb5a86269ff50e41b84b37fdb9f7`  
		Last Modified: Wed, 20 Apr 2022 04:50:51 GMT  
		Size: 56.1 MB (56112566 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v5

```console
$ docker pull debian@sha256:807a4ef3b027800774a6063deaa1b20114a97c392a1309e1baf1864a4b129cf8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518487 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a31547fc7cae9bda9410a0b464be9f8f8883bfdc3f24f34cadc308e2ec17d423`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:15 GMT
ADD file:5cf288f7326d2adfff5d4d3ad0b8951ab86c23ab83cbd1d7799d5c3f7e16a857 in / 
# Wed, 20 Apr 2022 07:20:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8db2a46143e157b04953cf05194b0c408afa491a70876185538ed8dca91f9f55`  
		Last Modified: Wed, 20 Apr 2022 07:37:08 GMT  
		Size: 53.5 MB (53518487 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v7

```console
$ docker pull debian@sha256:85d832c06c2a670b741eb859d9005f06ff6d15a4aa9392632cb034758b2ac5a5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b37145103304d6a126b19b9e17c72d05fb9f336c261b1a9038126f52974656f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:17 GMT
ADD file:0bbaf7ead1d59ceb682d490a4942a357b01272b17a3eb062c89673ee4e166052 in / 
# Wed, 20 Apr 2022 13:31:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bdfb6c7c7f66a4f6d01a8add17283f952a25f6c31c2267fd53d1ff9148edc16`  
		Last Modified: Wed, 20 Apr 2022 13:48:09 GMT  
		Size: 51.1 MB (51125664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:42c459ce79ae5c289cd08a3a367d0aad1e63c3dad36822e95e7378deec273571
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55063805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bebe2d8f5565145776afb75ce660b1d8f171e912ca1bd3e7336fd18c3f4bb8e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:37 GMT
ADD file:21175f4b5f80ba5d755041ff362152a8991b53f89e3c6699275e0c99f6ff6acd in / 
# Wed, 20 Apr 2022 04:30:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:414d522b7454847f294a8c61536f57f36cb195df33ab1fce8c838de3d5ae109e`  
		Last Modified: Wed, 20 Apr 2022 04:38:18 GMT  
		Size: 55.1 MB (55063805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; 386

```console
$ docker pull debian@sha256:b0b409f4709f92da7a3491c23af3a8a45c6c17254856b0640d97c4fb803eb856
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154017 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6479c6a51fcf6d1aeaa7b4d912ff11aac451d9b1d6dab25925bf06882fc584e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:08 GMT
ADD file:92a4df746589380f0ee875ec89574f67fa2f56164bbf20da79eaa902a778eff6 in / 
# Wed, 20 Apr 2022 07:39:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b42fb3c96c9db2aa67a5b3b033791b735ef01e8c8f15321f44a085532b703190`  
		Last Modified: Wed, 20 Apr 2022 07:47:30 GMT  
		Size: 57.2 MB (57154017 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; mips64le

```console
$ docker pull debian@sha256:90f40b91d0c70a24101548616328f68e1960f4debc5f86b513dbe159748d854b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54453471 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a86bf92e099c692380adb2a182835f00dc83f35a9243a0d19c34dee27fd41cf8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:45:15 GMT
ADD file:8f554d1fa0414c8c5592687851c7ff4646547038f7d464423b75f57ecade0e16 in / 
# Tue, 29 Mar 2022 07:45:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:024c5d1fff33205caaf39b2903eafbff594af3ca19d825c46db9f726a37cdcd1`  
		Last Modified: Tue, 29 Mar 2022 07:56:21 GMT  
		Size: 54.5 MB (54453471 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; ppc64le

```console
$ docker pull debian@sha256:19b71939fd4ba487fdb0604aae62fb4955ce6d459ae31a0bd4ad2883ca82c7e4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:60636c3dc49d419d81acf35232f5d8f96e63753370db53a14c1e4991dd7822ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:13 GMT
ADD file:fe481980541905937ff6238feab8ead20ee273dfc616bf9e647446ec0f98b510 in / 
# Wed, 20 Apr 2022 09:49:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7a4e99688ea8909140343db9b1a83e87f7ddcee1e1ba6d630b79487e5e720199`  
		Last Modified: Wed, 20 Apr 2022 09:59:29 GMT  
		Size: 60.6 MB (60558000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; riscv64

```console
$ docker pull debian@sha256:28a29382cd983a55b9111468116f9e220f5166cd2b802bb0a274c873a62801ab
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757327 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e3ce48a76dcd9ee6f31b7ac4c219efbf48ad76ae06692c4b0d649a2dc3ae2356`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:15:39 GMT
ADD file:250fc868a440657403e5cf8fde95da30be6fa0968ce4385656b837e923d678b0 in / 
# Wed, 20 Apr 2022 00:15:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5d7000bee08e5edd84dede58b856acde7ae5b24268b5109b3fe7e2260ac19618`  
		Last Modified: Wed, 20 Apr 2022 00:34:42 GMT  
		Size: 51.8 MB (51757327 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; s390x

```console
$ docker pull debian@sha256:69e0b72778bf55cde0ea95768727a28c470d3798306ee59ca524e45f07dbcc69
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54330969 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a0de4f4ab1a34f4412180df64940150b0e3eae608314bcd4573bac248847f0de`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:10 GMT
ADD file:39be572f6c0b3f6f84497807d4bcb370f80b4ac4167f1857002b5571a4dba6b2 in / 
# Wed, 20 Apr 2022 08:42:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d9529e5f0bb592a3bb453a841aee9e6a765fede232bccdc538ec2c8b96421d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:05 GMT  
		Size: 54.3 MB (54330969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20220418`

```console
$ docker pull debian@sha256:1670f813c9a5bd75a75f664e460f6400cfaf60751a446323d52bcf4e293df257
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20220418` - linux; amd64

```console
$ docker pull debian@sha256:8f1d4e94f67959e833de4287c05177d46a4e9715a5372728ad2640299661b8d5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112566 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0160468109b1f378dfc726e16cdae51a1e58d35c4e8b0d0a28302d40094d53ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:37 GMT
ADD file:859abf2d579747b132742454ad96e32dc879955afff8af84fab63dc41312a0e0 in / 
# Wed, 20 Apr 2022 04:44:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:212be0dc3a8ffdc400158a5e3a9812f7413dbb5a86269ff50e41b84b37fdb9f7`  
		Last Modified: Wed, 20 Apr 2022 04:50:51 GMT  
		Size: 56.1 MB (56112566 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:807a4ef3b027800774a6063deaa1b20114a97c392a1309e1baf1864a4b129cf8
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518487 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a31547fc7cae9bda9410a0b464be9f8f8883bfdc3f24f34cadc308e2ec17d423`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:15 GMT
ADD file:5cf288f7326d2adfff5d4d3ad0b8951ab86c23ab83cbd1d7799d5c3f7e16a857 in / 
# Wed, 20 Apr 2022 07:20:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8db2a46143e157b04953cf05194b0c408afa491a70876185538ed8dca91f9f55`  
		Last Modified: Wed, 20 Apr 2022 07:37:08 GMT  
		Size: 53.5 MB (53518487 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:85d832c06c2a670b741eb859d9005f06ff6d15a4aa9392632cb034758b2ac5a5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b37145103304d6a126b19b9e17c72d05fb9f336c261b1a9038126f52974656f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:17 GMT
ADD file:0bbaf7ead1d59ceb682d490a4942a357b01272b17a3eb062c89673ee4e166052 in / 
# Wed, 20 Apr 2022 13:31:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bdfb6c7c7f66a4f6d01a8add17283f952a25f6c31c2267fd53d1ff9148edc16`  
		Last Modified: Wed, 20 Apr 2022 13:48:09 GMT  
		Size: 51.1 MB (51125664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:42c459ce79ae5c289cd08a3a367d0aad1e63c3dad36822e95e7378deec273571
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55063805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bebe2d8f5565145776afb75ce660b1d8f171e912ca1bd3e7336fd18c3f4bb8e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:37 GMT
ADD file:21175f4b5f80ba5d755041ff362152a8991b53f89e3c6699275e0c99f6ff6acd in / 
# Wed, 20 Apr 2022 04:30:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:414d522b7454847f294a8c61536f57f36cb195df33ab1fce8c838de3d5ae109e`  
		Last Modified: Wed, 20 Apr 2022 04:38:18 GMT  
		Size: 55.1 MB (55063805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; 386

```console
$ docker pull debian@sha256:b0b409f4709f92da7a3491c23af3a8a45c6c17254856b0640d97c4fb803eb856
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57154017 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6479c6a51fcf6d1aeaa7b4d912ff11aac451d9b1d6dab25925bf06882fc584e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:08 GMT
ADD file:92a4df746589380f0ee875ec89574f67fa2f56164bbf20da79eaa902a778eff6 in / 
# Wed, 20 Apr 2022 07:39:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b42fb3c96c9db2aa67a5b3b033791b735ef01e8c8f15321f44a085532b703190`  
		Last Modified: Wed, 20 Apr 2022 07:47:30 GMT  
		Size: 57.2 MB (57154017 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:19b71939fd4ba487fdb0604aae62fb4955ce6d459ae31a0bd4ad2883ca82c7e4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60558000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:60636c3dc49d419d81acf35232f5d8f96e63753370db53a14c1e4991dd7822ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:13 GMT
ADD file:fe481980541905937ff6238feab8ead20ee273dfc616bf9e647446ec0f98b510 in / 
# Wed, 20 Apr 2022 09:49:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7a4e99688ea8909140343db9b1a83e87f7ddcee1e1ba6d630b79487e5e720199`  
		Last Modified: Wed, 20 Apr 2022 09:59:29 GMT  
		Size: 60.6 MB (60558000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; riscv64

```console
$ docker pull debian@sha256:28a29382cd983a55b9111468116f9e220f5166cd2b802bb0a274c873a62801ab
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757327 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e3ce48a76dcd9ee6f31b7ac4c219efbf48ad76ae06692c4b0d649a2dc3ae2356`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:15:39 GMT
ADD file:250fc868a440657403e5cf8fde95da30be6fa0968ce4385656b837e923d678b0 in / 
# Wed, 20 Apr 2022 00:15:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5d7000bee08e5edd84dede58b856acde7ae5b24268b5109b3fe7e2260ac19618`  
		Last Modified: Wed, 20 Apr 2022 00:34:42 GMT  
		Size: 51.8 MB (51757327 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418` - linux; s390x

```console
$ docker pull debian@sha256:69e0b72778bf55cde0ea95768727a28c470d3798306ee59ca524e45f07dbcc69
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54330969 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a0de4f4ab1a34f4412180df64940150b0e3eae608314bcd4573bac248847f0de`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:10 GMT
ADD file:39be572f6c0b3f6f84497807d4bcb370f80b4ac4167f1857002b5571a4dba6b2 in / 
# Wed, 20 Apr 2022 08:42:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d9529e5f0bb592a3bb453a841aee9e6a765fede232bccdc538ec2c8b96421d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:05 GMT  
		Size: 54.3 MB (54330969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20220418-slim`

```console
$ docker pull debian@sha256:3e104515e1ea1c3394eb0764020286d0ee4ec7a7f63a953cddfb690bf9e2b9cb
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:bae83cd8ad4ae9eb37b34d26370075dcbfa1155543e38b2b438cec53b4d9d72b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.2 MB (32229755 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f3c25fbe72372a098a23fcf5836ea7eca49eed1b09d55ea8c3255173c58bc29`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:45 GMT
ADD file:6feb3003e71266521ab75fe061b41af68d3ce168b3459f6d9ad58bbd75c7ed39 in / 
# Wed, 20 Apr 2022 04:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d51b003dba9b98f93b085567ad765e426335016a04dc584840486da860b9c83`  
		Last Modified: Wed, 20 Apr 2022 04:51:06 GMT  
		Size: 32.2 MB (32229755 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:e5f76a43f4484b2d9fc7ec44f0e596e8b4700d7787138dfcdd9d8c0fbf13d211
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29633290 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2cd023e2fb7ccde1df480ea4461962466838ff622c5072d8e1463c1607898cbf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:38 GMT
ADD file:0af9838b7d361060931691124f58b1eb67995b6aa1c5385e8ea8d28a5bfdd643 in / 
# Wed, 20 Apr 2022 07:20:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0d924fa318b059a58c51ff417f44dd722772773b9301030f0dabe73804be43e`  
		Last Modified: Wed, 20 Apr 2022 07:37:40 GMT  
		Size: 29.6 MB (29633290 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:39d0b0866c8093e73322c5b3c43f735f91397fe8ce8b5c28400e840901d91374
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.3 MB (27250842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:839154d5e125ad758a0b424ca51944d666a01e55627eb72acd39e65cb52dda19`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:41 GMT
ADD file:7a9182c3a9dc0cfadc6f4409a47c6a68cf6f213c9ef78d5ed9625ef683a3bd1d in / 
# Wed, 20 Apr 2022 13:31:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5f47795e81e27598f5a6641caa2da01a051b306b27a836407d12b6cf4a7ca0dc`  
		Last Modified: Wed, 20 Apr 2022 13:48:43 GMT  
		Size: 27.3 MB (27250842 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5b647eae176e2a786d1d6ff1f5409158a08894bc4c3bf0ca1242be8c64a528f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.2 MB (31184594 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0ca0075266113f84a0bb70afffb9dc046f78fab1f090b0ca90f6c0d3e68ed8d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:46 GMT
ADD file:0222fdc08b0e563db9e90a08f45b505356534ddb51251727ea0167050967c0e7 in / 
# Wed, 20 Apr 2022 04:30:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:edacba25feb5ee41b0b047125e3bdc26a1e9b54c25819517878d319475759b7c`  
		Last Modified: Wed, 20 Apr 2022 04:38:43 GMT  
		Size: 31.2 MB (31184594 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:10b0f47c6b4e5ec42073735de0a11bcd23eef9b9fdb9341be4645280b14551cf
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.3 MB (33286466 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de3e95eadb581638b17101c87f0f8cfbfa75eca4b323dacdc55ee77b7472becf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:19 GMT
ADD file:5f235bee0359b7345eb5ce7bea111abccb71d2c1b469d9d6454e157dfe129958 in / 
# Wed, 20 Apr 2022 07:39:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31e0d5103f90b003665961c1fca9564be4950bd96c51082eeed0bed443a07aaa`  
		Last Modified: Wed, 20 Apr 2022 07:47:45 GMT  
		Size: 33.3 MB (33286466 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:7b29085a4feb2dce16d6d3662a44902c378f2487c79986a9bd92aa89f6bac469
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.7 MB (36677766 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f002769e3a56ff7493530f1ad47767a8a3b0a6dc2efdb928a658349f5366bdd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:37 GMT
ADD file:119cca2acdf3216c38b2de64fb8de451027e61f435bb28a769c45c8adcb1a6c6 in / 
# Wed, 20 Apr 2022 09:49:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ad0f89c79d2959775608892e04819f0c4bde532c3eb7173e7b1c0f1155d417d1`  
		Last Modified: Wed, 20 Apr 2022 09:59:48 GMT  
		Size: 36.7 MB (36677766 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; riscv64

```console
$ docker pull debian@sha256:1a91f39d0b8b70486c5b4ffbcfc17b3dcaaeabf9266ea408724018f984c5fc25
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.9 MB (27887244 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:101822d3ead181a09f218eed716f48ae51e294ce85569aa4b0b702181fd18546`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:17:29 GMT
ADD file:726d1b5f0064bd5d8f0f8424272d41708cb7c89f914c9ddb16cd5cee805da523 in / 
# Wed, 20 Apr 2022 00:17:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:07ae6011d59b98cabc9f01ff8275da94006875083a970aedc909d14f76285f9e`  
		Last Modified: Wed, 20 Apr 2022 00:36:29 GMT  
		Size: 27.9 MB (27887244 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:51f05a386b138005db20c547dbec177127e5bc7acac1c6cd45724f918adcf6e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30453665 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b026e6916393c35a3418e746871e198d10f4e6ae27e82df7569200938e3f30e9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:35 GMT
ADD file:86c0c46158b0792857b688a356ec06469ae155de4f88fd1176531729443e28d3 in / 
# Wed, 20 Apr 2022 08:42:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c78bdacf45c70f85a592dda51f72d9d256ab58958eea7cd720c53cbce409277f`  
		Last Modified: Wed, 20 Apr 2022 08:51:23 GMT  
		Size: 30.5 MB (30453665 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-slim`

```console
$ docker pull debian@sha256:83e31edae5615ba905fcddce3cec206a101bdfe4775fa48b2084be14eb0eeec2
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-slim` - linux; amd64

```console
$ docker pull debian@sha256:bae83cd8ad4ae9eb37b34d26370075dcbfa1155543e38b2b438cec53b4d9d72b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.2 MB (32229755 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f3c25fbe72372a098a23fcf5836ea7eca49eed1b09d55ea8c3255173c58bc29`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:45 GMT
ADD file:6feb3003e71266521ab75fe061b41af68d3ce168b3459f6d9ad58bbd75c7ed39 in / 
# Wed, 20 Apr 2022 04:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d51b003dba9b98f93b085567ad765e426335016a04dc584840486da860b9c83`  
		Last Modified: Wed, 20 Apr 2022 04:51:06 GMT  
		Size: 32.2 MB (32229755 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:e5f76a43f4484b2d9fc7ec44f0e596e8b4700d7787138dfcdd9d8c0fbf13d211
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29633290 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2cd023e2fb7ccde1df480ea4461962466838ff622c5072d8e1463c1607898cbf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:20:38 GMT
ADD file:0af9838b7d361060931691124f58b1eb67995b6aa1c5385e8ea8d28a5bfdd643 in / 
# Wed, 20 Apr 2022 07:20:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0d924fa318b059a58c51ff417f44dd722772773b9301030f0dabe73804be43e`  
		Last Modified: Wed, 20 Apr 2022 07:37:40 GMT  
		Size: 29.6 MB (29633290 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:39d0b0866c8093e73322c5b3c43f735f91397fe8ce8b5c28400e840901d91374
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.3 MB (27250842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:839154d5e125ad758a0b424ca51944d666a01e55627eb72acd39e65cb52dda19`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:31:41 GMT
ADD file:7a9182c3a9dc0cfadc6f4409a47c6a68cf6f213c9ef78d5ed9625ef683a3bd1d in / 
# Wed, 20 Apr 2022 13:31:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5f47795e81e27598f5a6641caa2da01a051b306b27a836407d12b6cf4a7ca0dc`  
		Last Modified: Wed, 20 Apr 2022 13:48:43 GMT  
		Size: 27.3 MB (27250842 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5b647eae176e2a786d1d6ff1f5409158a08894bc4c3bf0ca1242be8c64a528f7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.2 MB (31184594 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0ca0075266113f84a0bb70afffb9dc046f78fab1f090b0ca90f6c0d3e68ed8d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:46 GMT
ADD file:0222fdc08b0e563db9e90a08f45b505356534ddb51251727ea0167050967c0e7 in / 
# Wed, 20 Apr 2022 04:30:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:edacba25feb5ee41b0b047125e3bdc26a1e9b54c25819517878d319475759b7c`  
		Last Modified: Wed, 20 Apr 2022 04:38:43 GMT  
		Size: 31.2 MB (31184594 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; 386

```console
$ docker pull debian@sha256:10b0f47c6b4e5ec42073735de0a11bcd23eef9b9fdb9341be4645280b14551cf
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.3 MB (33286466 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:de3e95eadb581638b17101c87f0f8cfbfa75eca4b323dacdc55ee77b7472becf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:19 GMT
ADD file:5f235bee0359b7345eb5ce7bea111abccb71d2c1b469d9d6454e157dfe129958 in / 
# Wed, 20 Apr 2022 07:39:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31e0d5103f90b003665961c1fca9564be4950bd96c51082eeed0bed443a07aaa`  
		Last Modified: Wed, 20 Apr 2022 07:47:45 GMT  
		Size: 33.3 MB (33286466 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; mips64le

```console
$ docker pull debian@sha256:972cc02beef7806a0e61c5c22932a5008c4a82bd35b492fdf1cca2180ca004b3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30727494 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1677f9c11f850d0e11cb315bd391f5f55526fd30072a5ec908593d314736b77a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:45:46 GMT
ADD file:7f2ba719932bd88c18a3ef17e2621286d58ae41c583dce2215fc098d966e2d1f in / 
# Tue, 29 Mar 2022 07:45:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:777e317f80b53ff55432c383ef807f2e69a7c8288412890bedf245f65c66e759`  
		Last Modified: Tue, 29 Mar 2022 07:56:50 GMT  
		Size: 30.7 MB (30727494 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:7b29085a4feb2dce16d6d3662a44902c378f2487c79986a9bd92aa89f6bac469
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.7 MB (36677766 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f002769e3a56ff7493530f1ad47767a8a3b0a6dc2efdb928a658349f5366bdd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:49:37 GMT
ADD file:119cca2acdf3216c38b2de64fb8de451027e61f435bb28a769c45c8adcb1a6c6 in / 
# Wed, 20 Apr 2022 09:49:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ad0f89c79d2959775608892e04819f0c4bde532c3eb7173e7b1c0f1155d417d1`  
		Last Modified: Wed, 20 Apr 2022 09:59:48 GMT  
		Size: 36.7 MB (36677766 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; riscv64

```console
$ docker pull debian@sha256:1a91f39d0b8b70486c5b4ffbcfc17b3dcaaeabf9266ea408724018f984c5fc25
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.9 MB (27887244 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:101822d3ead181a09f218eed716f48ae51e294ce85569aa4b0b702181fd18546`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:17:29 GMT
ADD file:726d1b5f0064bd5d8f0f8424272d41708cb7c89f914c9ddb16cd5cee805da523 in / 
# Wed, 20 Apr 2022 00:17:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:07ae6011d59b98cabc9f01ff8275da94006875083a970aedc909d14f76285f9e`  
		Last Modified: Wed, 20 Apr 2022 00:36:29 GMT  
		Size: 27.9 MB (27887244 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; s390x

```console
$ docker pull debian@sha256:51f05a386b138005db20c547dbec177127e5bc7acac1c6cd45724f918adcf6e9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30453665 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b026e6916393c35a3418e746871e198d10f4e6ae27e82df7569200938e3f30e9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:42:35 GMT
ADD file:86c0c46158b0792857b688a356ec06469ae155de4f88fd1176531729443e28d3 in / 
# Wed, 20 Apr 2022 08:42:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c78bdacf45c70f85a592dda51f72d9d256ab58958eea7cd720c53cbce409277f`  
		Last Modified: Wed, 20 Apr 2022 08:51:23 GMT  
		Size: 30.5 MB (30453665 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable`

```console
$ docker pull debian@sha256:5468f11a4928700ad01973ee791e155caa49d3f18a420b2ca8e5193f8ca3893e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable` - linux; amd64

```console
$ docker pull debian@sha256:5a12d4d61ffc727a1343bfed7ef1327e7dfa499483e9458ef9c48e7a990b3e64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4c8dfa39e6ffc4c09dfeb3486014ec97244b14e09d45a7de9e30b74e5b18e5d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:55 GMT
ADD file:70d54120d8cfb74e196d7f06205a5afbb6d666c607d98bb6fb288403d27cac5b in / 
# Wed, 20 Apr 2022 04:44:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e4019cb8009a11f4d0c0305817ebafd8aabc5efdd2ccfb680768c686fbf59f1`  
		Last Modified: Wed, 20 Apr 2022 04:51:23 GMT  
		Size: 54.9 MB (54941807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v5

```console
$ docker pull debian@sha256:fa1084ca58476b178f480e4cc19a58b56c1b28e922409a9fb6981d735b55bce3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474813 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:88c4d8ecc919f19f2cf8a8e97bd1897452b1388e87d336a8b108f4da78262b10`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:21:06 GMT
ADD file:68b75e09ec08e461705ec78b5040cf58c621ea9c79c137de9a8112de3fdbe5f6 in / 
# Wed, 20 Apr 2022 07:21:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:59cbbab28dfd690490cdf7eabe5ef0336e0cd6081d0e42156e2a07a3910bfa47`  
		Last Modified: Wed, 20 Apr 2022 07:38:21 GMT  
		Size: 52.5 MB (52474813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v7

```console
$ docker pull debian@sha256:e217c4e38223efb77b83ca6fb7053fa19204a1190e2f2256c7135b63a3fa11f4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133717 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c1c65b8483966f9428665c8e6079a9e3edb9efb040e6433c985af933d498582e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:32:11 GMT
ADD file:ed430d73187a99b2caa4e7b1596b9ebcd4662344b0610aafccdc9178a9e803ac in / 
# Wed, 20 Apr 2022 13:32:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:567476beb4c5ffe964b4f185e7884956f7af30a95f00283da4daf1684841b17b`  
		Last Modified: Wed, 20 Apr 2022 13:49:21 GMT  
		Size: 50.1 MB (50133717 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:782195e80e9c31533a11cf9167353238160dfa51a166c5b42c51d9c4c91edfbe
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633103 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:51e109fd2851225ed85abff1100635f641ee99daf6648313232a3ec88e0232c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:56 GMT
ADD file:9cb2ef4274be4c1a318ca57c12ca95f467f143eca0bac4c199e2a86f11c10eb9 in / 
# Wed, 20 Apr 2022 04:30:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f9d085fb4027d698e9010339730647a2cafcd6b562f259def78de7b561267965`  
		Last Modified: Wed, 20 Apr 2022 04:39:00 GMT  
		Size: 53.6 MB (53633103 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; 386

```console
$ docker pull debian@sha256:6cb35036642342b922fb0337437b876279f4515061b2716acd0654dad8317a44
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940688 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e0c16e5ffc4df5bc6e13ee412708bbb9f015a96497b2d4252f4d80e45031a15f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:30 GMT
ADD file:92b44faca8628d83825b927748987e74da58dffc328fc6c5613c27749255e877 in / 
# Wed, 20 Apr 2022 07:39:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:61b9c3fd915ab1c74b16d1e1369048684cd36eed0ead6d0ef99401069fd2fb42`  
		Last Modified: Wed, 20 Apr 2022 07:48:03 GMT  
		Size: 55.9 MB (55940688 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; mips64le

```console
$ docker pull debian@sha256:3a0cc659a7ae6cf6a1e0f28e7a3d0f8b2564a1d1cf7f70e918df4c19dc6e13d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199330 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43697233d680c92d8d15b0637a324a1c8a3eaafeb820b806c1f900a525e701a7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:46:21 GMT
ADD file:0fbef85df7c69585e70b9596838509ae1c00562f24fa354eb32e872202dfa7d6 in / 
# Tue, 29 Mar 2022 07:46:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fb4a12948a42065e860c13e45b3238ccb73f0c66e8c6cc4ad8b23ba7879f522`  
		Last Modified: Tue, 29 Mar 2022 07:57:30 GMT  
		Size: 53.2 MB (53199330 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; ppc64le

```console
$ docker pull debian@sha256:b9e15b87ce09dab1d81936b4d27623219163717df628c3e975bd6f7b565486df
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835260 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:571924899617b331a7ba2b715522529bfbc16be9795966cb246fb7df1be8affd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:50:04 GMT
ADD file:bfd1cdc74762b2fd575c1887fc1ee1830df56a7327ec69522f616240418491be in / 
# Wed, 20 Apr 2022 09:50:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cb41263da1dc6eda181580f28c1b3d2d552197140b58337fdec1318f27f01274`  
		Last Modified: Wed, 20 Apr 2022 10:00:12 GMT  
		Size: 58.8 MB (58835260 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; s390x

```console
$ docker pull debian@sha256:b67c2b5641ae25c7a67e23f5eec71d8bafac6ef0349d524e90eadfa674d7ce68
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207391 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:25df4ea8f8e776a75ef68fa2198703f502f4f299ae12048176d9fe0b4897871b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:43:05 GMT
ADD file:a1728962611fc2fb01d4c24d61347ae9f5053b293d5d8650bcd4c196f25744e1 in / 
# Wed, 20 Apr 2022 08:43:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:97f6537fb8406b5d5515b31cb525bb261828bced06956c6ad38e825a1c7131d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:39 GMT  
		Size: 53.2 MB (53207391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20220418`

```console
$ docker pull debian@sha256:2bf1394e55145ea975139b7095b80c8ab065acc662dbc6efa2a83996abc708f2
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20220418` - linux; amd64

```console
$ docker pull debian@sha256:5a12d4d61ffc727a1343bfed7ef1327e7dfa499483e9458ef9c48e7a990b3e64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54941807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4c8dfa39e6ffc4c09dfeb3486014ec97244b14e09d45a7de9e30b74e5b18e5d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:55 GMT
ADD file:70d54120d8cfb74e196d7f06205a5afbb6d666c607d98bb6fb288403d27cac5b in / 
# Wed, 20 Apr 2022 04:44:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e4019cb8009a11f4d0c0305817ebafd8aabc5efdd2ccfb680768c686fbf59f1`  
		Last Modified: Wed, 20 Apr 2022 04:51:23 GMT  
		Size: 54.9 MB (54941807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:fa1084ca58476b178f480e4cc19a58b56c1b28e922409a9fb6981d735b55bce3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52474813 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:88c4d8ecc919f19f2cf8a8e97bd1897452b1388e87d336a8b108f4da78262b10`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:21:06 GMT
ADD file:68b75e09ec08e461705ec78b5040cf58c621ea9c79c137de9a8112de3fdbe5f6 in / 
# Wed, 20 Apr 2022 07:21:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:59cbbab28dfd690490cdf7eabe5ef0336e0cd6081d0e42156e2a07a3910bfa47`  
		Last Modified: Wed, 20 Apr 2022 07:38:21 GMT  
		Size: 52.5 MB (52474813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:e217c4e38223efb77b83ca6fb7053fa19204a1190e2f2256c7135b63a3fa11f4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133717 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c1c65b8483966f9428665c8e6079a9e3edb9efb040e6433c985af933d498582e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:32:11 GMT
ADD file:ed430d73187a99b2caa4e7b1596b9ebcd4662344b0610aafccdc9178a9e803ac in / 
# Wed, 20 Apr 2022 13:32:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:567476beb4c5ffe964b4f185e7884956f7af30a95f00283da4daf1684841b17b`  
		Last Modified: Wed, 20 Apr 2022 13:49:21 GMT  
		Size: 50.1 MB (50133717 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:782195e80e9c31533a11cf9167353238160dfa51a166c5b42c51d9c4c91edfbe
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633103 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:51e109fd2851225ed85abff1100635f641ee99daf6648313232a3ec88e0232c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:56 GMT
ADD file:9cb2ef4274be4c1a318ca57c12ca95f467f143eca0bac4c199e2a86f11c10eb9 in / 
# Wed, 20 Apr 2022 04:30:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f9d085fb4027d698e9010339730647a2cafcd6b562f259def78de7b561267965`  
		Last Modified: Wed, 20 Apr 2022 04:39:00 GMT  
		Size: 53.6 MB (53633103 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; 386

```console
$ docker pull debian@sha256:6cb35036642342b922fb0337437b876279f4515061b2716acd0654dad8317a44
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940688 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e0c16e5ffc4df5bc6e13ee412708bbb9f015a96497b2d4252f4d80e45031a15f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:30 GMT
ADD file:92b44faca8628d83825b927748987e74da58dffc328fc6c5613c27749255e877 in / 
# Wed, 20 Apr 2022 07:39:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:61b9c3fd915ab1c74b16d1e1369048684cd36eed0ead6d0ef99401069fd2fb42`  
		Last Modified: Wed, 20 Apr 2022 07:48:03 GMT  
		Size: 55.9 MB (55940688 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:b9e15b87ce09dab1d81936b4d27623219163717df628c3e975bd6f7b565486df
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835260 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:571924899617b331a7ba2b715522529bfbc16be9795966cb246fb7df1be8affd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:50:04 GMT
ADD file:bfd1cdc74762b2fd575c1887fc1ee1830df56a7327ec69522f616240418491be in / 
# Wed, 20 Apr 2022 09:50:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cb41263da1dc6eda181580f28c1b3d2d552197140b58337fdec1318f27f01274`  
		Last Modified: Wed, 20 Apr 2022 10:00:12 GMT  
		Size: 58.8 MB (58835260 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418` - linux; s390x

```console
$ docker pull debian@sha256:b67c2b5641ae25c7a67e23f5eec71d8bafac6ef0349d524e90eadfa674d7ce68
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207391 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:25df4ea8f8e776a75ef68fa2198703f502f4f299ae12048176d9fe0b4897871b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:43:05 GMT
ADD file:a1728962611fc2fb01d4c24d61347ae9f5053b293d5d8650bcd4c196f25744e1 in / 
# Wed, 20 Apr 2022 08:43:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:97f6537fb8406b5d5515b31cb525bb261828bced06956c6ad38e825a1c7131d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:39 GMT  
		Size: 53.2 MB (53207391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20220418-slim`

```console
$ docker pull debian@sha256:bb5410b24502e6600cacce771dcb990067862114d80cea9263281f8f3c184717
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:1086096bcb743c4d04ad1edc0fe729fe536442049d76894172c8ed1fdbb4a48b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378939 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b20702a8e984799edcfafc4f1dde2fabfb232b9d39478250f443020de9c72c20`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:06 GMT
ADD file:90915646c689c88399524cc601499906ec16097b7397ee90ac3b40478fa7b770 in / 
# Wed, 20 Apr 2022 04:45:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fbeed35392b132d1b5f6043ee2415968f904105e71325d783ea4b2c7ab2f2e91`  
		Last Modified: Wed, 20 Apr 2022 04:51:45 GMT  
		Size: 31.4 MB (31378939 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c12eefb2f102884c4847e6eb9b4942a9bde897bfc813d8f6c3545105bacec992
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921032 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ee0deec2bbcb9cae6cc5eef6f728a3a2cd8bce27b3ec9221bda1c6c0d81e86f2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:21:39 GMT
ADD file:8dd05879817d88292cb00b90b967476f3a2eacc46c41d6cebc54c55d4926b71d in / 
# Wed, 20 Apr 2022 07:21:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:db664cb3067dc23b9aa91b788dbefb9db9c557ff9230dc237967a56b2caf9b2e`  
		Last Modified: Wed, 20 Apr 2022 07:39:02 GMT  
		Size: 28.9 MB (28921032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:a0307b758e4361f53cf9ede75acf51c04fe975c1dd7231395edee76f0eddb92b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3fe2df5a25261c32cf31d5dd1d16f3404c1e5846c250bd58a8e6e3103470e21b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:32:47 GMT
ADD file:6201a43405f412da82e5b471994d51cbf09597a688993cd77e319fc78a830abf in / 
# Wed, 20 Apr 2022 13:32:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b3739def0712f8cdca97823309d9167d6e33b03ff52a10ce4a3b1cf88ef5741e`  
		Last Modified: Wed, 20 Apr 2022 13:49:55 GMT  
		Size: 26.6 MB (26575738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5eefd59d0bb016310169d86ce3af5b089579ba2fac05a3dbfa7041fb730e05cb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065792 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:865e6c40b214ee774c1003c9231cadd70f56befc762303f45d52268209087e9f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:09 GMT
ADD file:3db3f51df2dda01c09cb6bc32c9e5a0ae4aa0d3fb5a0ae2c29aa5fe06f1086da in / 
# Wed, 20 Apr 2022 04:31:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6737670e4cd4778507176a1feae98fabaf4fa0d61bc9f95d92236835688fb176`  
		Last Modified: Wed, 20 Apr 2022 04:39:22 GMT  
		Size: 30.1 MB (30065792 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:49c3e77db2f72c22db5850c698d51695738ff6da58a0ad75bac3036d0830d3a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389573 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a572401b4d74d00ddbfe76987dcb16a9ebd7432fe68232328550c265f04a47ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:46 GMT
ADD file:ab72417f2d700d7ab64d62210f4de97fef9012ca30da71ca9e647f5b29890ed6 in / 
# Wed, 20 Apr 2022 07:39:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2672eb7e923aa96d194b5e085e96f03dfac7ff1a392819e322323d08567ab403`  
		Last Modified: Wed, 20 Apr 2022 07:48:25 GMT  
		Size: 32.4 MB (32389573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:b4f49d1fa2356f75d00fee7cd05dffa71b6366d4e06c4de675b565f9f2eaa65f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285262 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4db5e37be10e277cf604b17901cca4c9e665221d25cd91f72a8ac31c88c584ce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:50:43 GMT
ADD file:f9ff44f15dd33912917ecc060b574424770cd049760083a3bc7bf6a1c3073a02 in / 
# Wed, 20 Apr 2022 09:50:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c7fb14f29eacb296e2baa8e1e7adfad1c45184f75ab07bd6f9e6a548283e471f`  
		Last Modified: Wed, 20 Apr 2022 10:00:44 GMT  
		Size: 35.3 MB (35285262 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:2078acd8c84fd5b67b97843bc931a8cd6620cabedcdc43a4689a9ddbd18d84a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7bbb191f0b0210b69db56b466980111b0edbd4d14768dc96dae0591cfe79ee6a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:43:41 GMT
ADD file:9721155ebd020f1a0ef257842479e3774f156baa64085785d9409c068bbcb311 in / 
# Wed, 20 Apr 2022 08:43:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:128bcb4e5a755b6b0ba00d9e7f9200d3e0bc654d76b90825e0a749ba225f3401`  
		Last Modified: Wed, 20 Apr 2022 08:51:57 GMT  
		Size: 29.7 MB (29654842 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-backports`

```console
$ docker pull debian@sha256:5f353bc56eeae8d29a6db2702157488c57493a3b12ca4829d0cf37456a989b96
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-backports` - linux; amd64

```console
$ docker pull debian@sha256:e48257b4c3c946ffd8e961c7ff05895eecf707a5ff53102e778e693d0976bdf4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54942031 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6432b06c60d3cdce30a05e1158c966c1d80fe60b2456b7b3488412599cda77f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:44:55 GMT
ADD file:70d54120d8cfb74e196d7f06205a5afbb6d666c607d98bb6fb288403d27cac5b in / 
# Wed, 20 Apr 2022 04:44:55 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:44:58 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7e4019cb8009a11f4d0c0305817ebafd8aabc5efdd2ccfb680768c686fbf59f1`  
		Last Modified: Wed, 20 Apr 2022 04:51:23 GMT  
		Size: 54.9 MB (54941807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e8fbc92289b20574efbecaffcafecd68353d8c7d8b0bdcca29247eda94c2bc50`  
		Last Modified: Wed, 20 Apr 2022 04:51:34 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:1e651b6407e5d02fd96a46b16aa4b7c237a2ebf13944cfc1e29fc0ddca0b1a96
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52475036 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4876f4114ade2bb394bdc90c917c01697143e3bdd098b9c45852653f7624aea5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:21:06 GMT
ADD file:68b75e09ec08e461705ec78b5040cf58c621ea9c79c137de9a8112de3fdbe5f6 in / 
# Wed, 20 Apr 2022 07:21:07 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:21:20 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:59cbbab28dfd690490cdf7eabe5ef0336e0cd6081d0e42156e2a07a3910bfa47`  
		Last Modified: Wed, 20 Apr 2022 07:38:21 GMT  
		Size: 52.5 MB (52474813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:43e0a118d3e9d5e5bc2650067587607b96d25b5855f9a937eb02d733d1bc57fd`  
		Last Modified: Wed, 20 Apr 2022 07:38:37 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:4fe98bd09fc7cbfffe8e3ea24a53c851778469c037fdc8d051421bbb2cfa2a54
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50133942 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4d9ac26ebc67aacd57469e44ba70c8bd0e03e675e277ae253de2a97e8cdeae0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:32:11 GMT
ADD file:ed430d73187a99b2caa4e7b1596b9ebcd4662344b0610aafccdc9178a9e803ac in / 
# Wed, 20 Apr 2022 13:32:12 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:32:25 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:567476beb4c5ffe964b4f185e7884956f7af30a95f00283da4daf1684841b17b`  
		Last Modified: Wed, 20 Apr 2022 13:49:21 GMT  
		Size: 50.1 MB (50133717 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:68f00418b6b6f607491af856868b8692cd348b48161cef5809e8879b2f4203d9`  
		Last Modified: Wed, 20 Apr 2022 13:49:33 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fafc23ff1c9860f1757010c16c774cea89c1456f81f9ba000f667f2f2e4a5e54
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53633323 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:607729f0e0eb8fc21b82823c7c5f3dcc60b24b2dc03c551a751f7b965cdb900e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:30:56 GMT
ADD file:9cb2ef4274be4c1a318ca57c12ca95f467f143eca0bac4c199e2a86f11c10eb9 in / 
# Wed, 20 Apr 2022 04:30:56 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:31:02 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:f9d085fb4027d698e9010339730647a2cafcd6b562f259def78de7b561267965`  
		Last Modified: Wed, 20 Apr 2022 04:39:00 GMT  
		Size: 53.6 MB (53633103 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:763140a5f75386d34014d2975b6f1f53443a8cd19a9c507901afd8ea9aeee9fd`  
		Last Modified: Wed, 20 Apr 2022 04:39:10 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; 386

```console
$ docker pull debian@sha256:95c0b4fc58bb5f54ef1942385955fb47b28114efc1dd92ec0f1c48dfe15ef55e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55940909 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d06ea66de2b3b53a2390915a3c999b80a7129cb2a50426238389953ba5d0f3f1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:30 GMT
ADD file:92b44faca8628d83825b927748987e74da58dffc328fc6c5613c27749255e877 in / 
# Wed, 20 Apr 2022 07:39:31 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:39:37 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:61b9c3fd915ab1c74b16d1e1369048684cd36eed0ead6d0ef99401069fd2fb42`  
		Last Modified: Wed, 20 Apr 2022 07:48:03 GMT  
		Size: 55.9 MB (55940688 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7e5467fe4e9503027a0a01e4b1a28e4aa3f74678d92048518fbbdd539a40248f`  
		Last Modified: Wed, 20 Apr 2022 07:48:13 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:4deb166e82551aab1b3db3af4fdfc1a17ea0444c80e6b6074d1ece9f78638a8b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53199555 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11d6948bbcc454f8859ea7e1db3a470670a78e85161d36771329bbdfaa456d81`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:46:21 GMT
ADD file:0fbef85df7c69585e70b9596838509ae1c00562f24fa354eb32e872202dfa7d6 in / 
# Tue, 29 Mar 2022 07:46:26 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:46:36 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0fb4a12948a42065e860c13e45b3238ccb73f0c66e8c6cc4ad8b23ba7879f522`  
		Last Modified: Tue, 29 Mar 2022 07:57:30 GMT  
		Size: 53.2 MB (53199330 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b79dad7ad6060aae57a9d550ef733aa04579b3644ccf436f534f6a8f45fd7284`  
		Last Modified: Tue, 29 Mar 2022 07:57:39 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:5420de5f237cacd006e5a6787bea346401e445dfc47567be4d422321b34d146f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58835483 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a38a601136715449ccb26eef56dbe693f07410ab3694c3b86dd86ac5858d3d0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:50:04 GMT
ADD file:bfd1cdc74762b2fd575c1887fc1ee1830df56a7327ec69522f616240418491be in / 
# Wed, 20 Apr 2022 09:50:11 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:50:28 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:cb41263da1dc6eda181580f28c1b3d2d552197140b58337fdec1318f27f01274`  
		Last Modified: Wed, 20 Apr 2022 10:00:12 GMT  
		Size: 58.8 MB (58835260 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:765c2fbe4c7936986609f407607d3ffe54f8e73e0479b5c8d1bb286692baec85`  
		Last Modified: Wed, 20 Apr 2022 10:00:27 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; s390x

```console
$ docker pull debian@sha256:5b2fe8607e9248809058400783c75f604abe474199a79ab1dcf7ad10776d5822
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207615 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3b9463a3d80a6987deae4b3b5a3cecbb6dd0139e808be60e5516e55c23a4910c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:43:05 GMT
ADD file:a1728962611fc2fb01d4c24d61347ae9f5053b293d5d8650bcd4c196f25744e1 in / 
# Wed, 20 Apr 2022 08:43:11 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:43:23 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:97f6537fb8406b5d5515b31cb525bb261828bced06956c6ad38e825a1c7131d2`  
		Last Modified: Wed, 20 Apr 2022 08:51:39 GMT  
		Size: 53.2 MB (53207391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:db4014d2da36b5fdc20f6cf9378e048e3e3643879e3efbd6c971e39bdf9b8125`  
		Last Modified: Wed, 20 Apr 2022 08:51:47 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-slim`

```console
$ docker pull debian@sha256:2c7f0e40c489c4b73fe9a20b8ebc79eaa68739551f381e9bc212fd533469fba4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-slim` - linux; amd64

```console
$ docker pull debian@sha256:1086096bcb743c4d04ad1edc0fe729fe536442049d76894172c8ed1fdbb4a48b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31378939 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b20702a8e984799edcfafc4f1dde2fabfb232b9d39478250f443020de9c72c20`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:06 GMT
ADD file:90915646c689c88399524cc601499906ec16097b7397ee90ac3b40478fa7b770 in / 
# Wed, 20 Apr 2022 04:45:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fbeed35392b132d1b5f6043ee2415968f904105e71325d783ea4b2c7ab2f2e91`  
		Last Modified: Wed, 20 Apr 2022 04:51:45 GMT  
		Size: 31.4 MB (31378939 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c12eefb2f102884c4847e6eb9b4942a9bde897bfc813d8f6c3545105bacec992
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28921032 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ee0deec2bbcb9cae6cc5eef6f728a3a2cd8bce27b3ec9221bda1c6c0d81e86f2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:21:39 GMT
ADD file:8dd05879817d88292cb00b90b967476f3a2eacc46c41d6cebc54c55d4926b71d in / 
# Wed, 20 Apr 2022 07:21:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:db664cb3067dc23b9aa91b788dbefb9db9c557ff9230dc237967a56b2caf9b2e`  
		Last Modified: Wed, 20 Apr 2022 07:39:02 GMT  
		Size: 28.9 MB (28921032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:a0307b758e4361f53cf9ede75acf51c04fe975c1dd7231395edee76f0eddb92b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26575738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3fe2df5a25261c32cf31d5dd1d16f3404c1e5846c250bd58a8e6e3103470e21b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:32:47 GMT
ADD file:6201a43405f412da82e5b471994d51cbf09597a688993cd77e319fc78a830abf in / 
# Wed, 20 Apr 2022 13:32:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b3739def0712f8cdca97823309d9167d6e33b03ff52a10ce4a3b1cf88ef5741e`  
		Last Modified: Wed, 20 Apr 2022 13:49:55 GMT  
		Size: 26.6 MB (26575738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5eefd59d0bb016310169d86ce3af5b089579ba2fac05a3dbfa7041fb730e05cb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30065792 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:865e6c40b214ee774c1003c9231cadd70f56befc762303f45d52268209087e9f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:09 GMT
ADD file:3db3f51df2dda01c09cb6bc32c9e5a0ae4aa0d3fb5a0ae2c29aa5fe06f1086da in / 
# Wed, 20 Apr 2022 04:31:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6737670e4cd4778507176a1feae98fabaf4fa0d61bc9f95d92236835688fb176`  
		Last Modified: Wed, 20 Apr 2022 04:39:22 GMT  
		Size: 30.1 MB (30065792 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; 386

```console
$ docker pull debian@sha256:49c3e77db2f72c22db5850c698d51695738ff6da58a0ad75bac3036d0830d3a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32389573 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a572401b4d74d00ddbfe76987dcb16a9ebd7432fe68232328550c265f04a47ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:46 GMT
ADD file:ab72417f2d700d7ab64d62210f4de97fef9012ca30da71ca9e647f5b29890ed6 in / 
# Wed, 20 Apr 2022 07:39:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2672eb7e923aa96d194b5e085e96f03dfac7ff1a392819e322323d08567ab403`  
		Last Modified: Wed, 20 Apr 2022 07:48:25 GMT  
		Size: 32.4 MB (32389573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:0e74dd3e94f147c9dffefd79d6fe680f77eb4c2db98e752afd8a8f243a0a8b2d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29641481 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:19fc0bf8dd2f9acaa5f735dc6237ce7b35779caecdf747c9e83dd16dce2aac2b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:46:57 GMT
ADD file:a5486ab7b33d64cb8135983a8ba55ac598076f9eb2e7cefcc0c4bc3a2903f17b in / 
# Tue, 29 Mar 2022 07:47:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dfdd0c2f109c10b50033c76b9820d10683a1792c4843e8366999a8e8576f5517`  
		Last Modified: Tue, 29 Mar 2022 07:58:06 GMT  
		Size: 29.6 MB (29641481 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:b4f49d1fa2356f75d00fee7cd05dffa71b6366d4e06c4de675b565f9f2eaa65f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35285262 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4db5e37be10e277cf604b17901cca4c9e665221d25cd91f72a8ac31c88c584ce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:50:43 GMT
ADD file:f9ff44f15dd33912917ecc060b574424770cd049760083a3bc7bf6a1c3073a02 in / 
# Wed, 20 Apr 2022 09:50:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c7fb14f29eacb296e2baa8e1e7adfad1c45184f75ab07bd6f9e6a548283e471f`  
		Last Modified: Wed, 20 Apr 2022 10:00:44 GMT  
		Size: 35.3 MB (35285262 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; s390x

```console
$ docker pull debian@sha256:2078acd8c84fd5b67b97843bc931a8cd6620cabedcdc43a4689a9ddbd18d84a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29654842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7bbb191f0b0210b69db56b466980111b0edbd4d14768dc96dae0591cfe79ee6a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:43:41 GMT
ADD file:9721155ebd020f1a0ef257842479e3774f156baa64085785d9409c068bbcb311 in / 
# Wed, 20 Apr 2022 08:43:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:128bcb4e5a755b6b0ba00d9e7f9200d3e0bc654d76b90825e0a749ba225f3401`  
		Last Modified: Wed, 20 Apr 2022 08:51:57 GMT  
		Size: 29.7 MB (29654842 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch`

```console
$ docker pull debian@sha256:cebe6e1c30384958d471467e231f740e8f0fd92cbfd2a435a186e9bada3aee1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch` - linux; amd64

```console
$ docker pull debian@sha256:75383204de7da8eb4bb2724e5fb8dae2910ee78938ade4d35ed8afec19a90b0b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3dbb17ee70fa3383907ebfc75e737e975d73b3172eb433a1a132766aa8a11f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:14 GMT
ADD file:6ed691b65385dede44a92f9de9e1428af431197c66461aa0f9c61e2f7c8bade5 in / 
# Wed, 20 Apr 2022 04:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5196cdf25181bc7e4411865a2e002932b7b6b0ffce787c04c1bdeaf1f204f20`  
		Last Modified: Wed, 20 Apr 2022 04:52:01 GMT  
		Size: 45.4 MB (45427434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm variant v5

```console
$ docker pull debian@sha256:fbf4b8d8ccbfc5b2142939a75df4983c60065732a75393a8b8ce39ca2cf98794
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d74fea7db7861cec6731fee36ad5dc1c8a616414000db685b5d5de374595fd6f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:06 GMT
ADD file:407e28120c3ae0a1012433a9787cf812e0ecc360e63ace9fe21f6728470b4db1 in / 
# Wed, 20 Apr 2022 07:22:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6849d3c2fa1fe7c2c78db1c26ca92e44c119dd3e92d93450115c927ecb6dce6`  
		Last Modified: Wed, 20 Apr 2022 07:39:41 GMT  
		Size: 44.1 MB (44122673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm variant v7

```console
$ docker pull debian@sha256:820547110898f52616039d3876738c0fca8a475be9290a4b77f5abaeb25045a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150830 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43cfe0e985e30b2fb4c56df2258c4f5ee922b7365ded10938ff7c2a7985c69a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:15 GMT
ADD file:d237e5cd139706988e7bbc50ae542898a9b1fa1539f00322f21a71714975ccd8 in / 
# Wed, 20 Apr 2022 13:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef9cca547ca93783f3168dc26ce42494066c74a8d5b51e766ac90a3fb5ad2931`  
		Last Modified: Wed, 20 Apr 2022 13:50:30 GMT  
		Size: 42.2 MB (42150830 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fb3a805a4536a028bbec14eca5ee1e37fca67af1b1ab517d830bdcc3e0db2bcd
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212018 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d1693f5b3dd3cb0a26e7bae2c9190f6cf40aa5134990b06748c551da00579dd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:19 GMT
ADD file:73f1db8536438ca891f4b507a569e6c561364db0f05914ebea9d3fa97e1bf837 in / 
# Wed, 20 Apr 2022 04:31:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a41fbedfa4b1547a2b4fea33de48af745d66a94741d3cf344cb2766f0e80211b`  
		Last Modified: Wed, 20 Apr 2022 04:39:38 GMT  
		Size: 43.2 MB (43212018 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; 386

```console
$ docker pull debian@sha256:29ee5bf0bf0aa44f872f3a7df77ce220c90d3749932b950f65e0b21c332111f2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fcdce55a50efd98b7534c78cde22133b58b6112028b144ef677ef78d1c183883`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:57 GMT
ADD file:e8c6a60834ed931ccc3c93b1f2c9b9ca2bf190b0d8d2474382ee8e96a7e35b5a in / 
# Wed, 20 Apr 2022 07:39:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cd343d4c0c2fbc67ddea33155621bdf36074fc30ffb6f917e716fbb81645f79a`  
		Last Modified: Wed, 20 Apr 2022 07:48:48 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-20220418`

```console
$ docker pull debian@sha256:cebe6e1c30384958d471467e231f740e8f0fd92cbfd2a435a186e9bada3aee1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-20220418` - linux; amd64

```console
$ docker pull debian@sha256:75383204de7da8eb4bb2724e5fb8dae2910ee78938ade4d35ed8afec19a90b0b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3dbb17ee70fa3383907ebfc75e737e975d73b3172eb433a1a132766aa8a11f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:14 GMT
ADD file:6ed691b65385dede44a92f9de9e1428af431197c66461aa0f9c61e2f7c8bade5 in / 
# Wed, 20 Apr 2022 04:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5196cdf25181bc7e4411865a2e002932b7b6b0ffce787c04c1bdeaf1f204f20`  
		Last Modified: Wed, 20 Apr 2022 04:52:01 GMT  
		Size: 45.4 MB (45427434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:fbf4b8d8ccbfc5b2142939a75df4983c60065732a75393a8b8ce39ca2cf98794
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d74fea7db7861cec6731fee36ad5dc1c8a616414000db685b5d5de374595fd6f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:06 GMT
ADD file:407e28120c3ae0a1012433a9787cf812e0ecc360e63ace9fe21f6728470b4db1 in / 
# Wed, 20 Apr 2022 07:22:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6849d3c2fa1fe7c2c78db1c26ca92e44c119dd3e92d93450115c927ecb6dce6`  
		Last Modified: Wed, 20 Apr 2022 07:39:41 GMT  
		Size: 44.1 MB (44122673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:820547110898f52616039d3876738c0fca8a475be9290a4b77f5abaeb25045a1
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42150830 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43cfe0e985e30b2fb4c56df2258c4f5ee922b7365ded10938ff7c2a7985c69a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:15 GMT
ADD file:d237e5cd139706988e7bbc50ae542898a9b1fa1539f00322f21a71714975ccd8 in / 
# Wed, 20 Apr 2022 13:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef9cca547ca93783f3168dc26ce42494066c74a8d5b51e766ac90a3fb5ad2931`  
		Last Modified: Wed, 20 Apr 2022 13:50:30 GMT  
		Size: 42.2 MB (42150830 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fb3a805a4536a028bbec14eca5ee1e37fca67af1b1ab517d830bdcc3e0db2bcd
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212018 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d1693f5b3dd3cb0a26e7bae2c9190f6cf40aa5134990b06748c551da00579dd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:19 GMT
ADD file:73f1db8536438ca891f4b507a569e6c561364db0f05914ebea9d3fa97e1bf837 in / 
# Wed, 20 Apr 2022 04:31:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a41fbedfa4b1547a2b4fea33de48af745d66a94741d3cf344cb2766f0e80211b`  
		Last Modified: Wed, 20 Apr 2022 04:39:38 GMT  
		Size: 43.2 MB (43212018 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418` - linux; 386

```console
$ docker pull debian@sha256:29ee5bf0bf0aa44f872f3a7df77ce220c90d3749932b950f65e0b21c332111f2
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fcdce55a50efd98b7534c78cde22133b58b6112028b144ef677ef78d1c183883`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:57 GMT
ADD file:e8c6a60834ed931ccc3c93b1f2c9b9ca2bf190b0d8d2474382ee8e96a7e35b5a in / 
# Wed, 20 Apr 2022 07:39:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cd343d4c0c2fbc67ddea33155621bdf36074fc30ffb6f917e716fbb81645f79a`  
		Last Modified: Wed, 20 Apr 2022 07:48:48 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-20220418-slim`

```console
$ docker pull debian@sha256:e317b647e7b7df5229116215896f591917b95a8be9a0ba0ccfec83feff3861c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:ef76a14f299ecc38bc00cd075f230dc69f8ff5aaad07a13b7cfb92fd3601e7de
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567235 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd3475f2eda5b0088a2164878c70031d5ae6d0304580f86449d867934fd6e8e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:25 GMT
ADD file:0fafb7f95dc18699227154856361058bc9aba65de2ff88bf55cc66363142a05e in / 
# Wed, 20 Apr 2022 04:45:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8bd3f5a20b90c0e84cc65ea4a9172a51b7b5c0b9a097fa88d7102cac91cddf2c`  
		Last Modified: Wed, 20 Apr 2022 04:52:27 GMT  
		Size: 22.6 MB (22567235 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:371cd0b21fab642901d9e51dd04f095571c2b85eef693a0c209c376f722340fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240552 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37e0f3d66f25288d6c7cb7e8ee7487a8a098e33df878873ea22a96bf541e723d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:40 GMT
ADD file:8caa5e7ca2ab4b420bed155bd2190baa73ca3c4e9378db02b0c0cede15974d69 in / 
# Wed, 20 Apr 2022 07:22:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d96febf107fb8f8f8c5d8bd55514488bd2c5f9ca3c49cb1c015381d7aa816d6`  
		Last Modified: Wed, 20 Apr 2022 07:40:19 GMT  
		Size: 21.2 MB (21240552 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ea27ab375b847954717cfeff2736d39679501579d38b3dc2f71ae6254735a64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:898e8f79b5495e8f2c00b96f8c438b474227379a436b9f510cfda019b8c753a6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:51 GMT
ADD file:096692331564a30a9a9c6b24e681d99d68b54eeeabb7c5ac7d4073de06e050b2 in / 
# Wed, 20 Apr 2022 13:33:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08da560f0b65b004ba95f9a431d3ac51b8807b16ec6ef170e2e00dc55330246d`  
		Last Modified: Wed, 20 Apr 2022 13:51:07 GMT  
		Size: 19.4 MB (19359807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:13a6a318bc4e4200435e0f3ac16c65e3a60fa45511a472bd47e81c5bac41710b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3e0d7663d4764771c2ba52276e17e355f57961f2cd98863d1b29ebc3bf0f52e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:34 GMT
ADD file:02ad9214474cf0e6d278d7015dee30e6a8e24cdc699812f5fb3915491177e92b in / 
# Wed, 20 Apr 2022 04:31:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fca491399855be4c9b780dca621b48440831235d7916e5695b876698df84db7`  
		Last Modified: Wed, 20 Apr 2022 04:40:05 GMT  
		Size: 20.4 MB (20424115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:33d586c9ff1c3dee1135a843b10b7c620107ef269e3ae83aa2b9d88719b91a30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b77216ff0bf0b007fd5cac2c9b36dcc3cb0fa2469d6d198dbffff8270facf9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:12 GMT
ADD file:f1394937f911449ed9fb7103f2bfb5b299d7779e98c4f0f34bcee08f8c2bf5db in / 
# Wed, 20 Apr 2022 07:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da9db5d720427742cbccdd8a4a37dfaa81873ab9db6de4935623ca79ce881ee4`  
		Last Modified: Wed, 20 Apr 2022 07:49:16 GMT  
		Size: 23.2 MB (23202179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-backports`

```console
$ docker pull debian@sha256:0eba6fefe70319824c9c505dbe796e550d6b41f76e4d46c172fd916615010153
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-backports` - linux; amd64

```console
$ docker pull debian@sha256:a0415b442feac68daa84c8593f6b8cde23d8ecade4ef46ccc8a3786a12cf8b90
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45427658 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33386c6b36009d7d27bfe0879f50041853630fd16c3f454848232b8ae31eed03`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:14 GMT
ADD file:6ed691b65385dede44a92f9de9e1428af431197c66461aa0f9c61e2f7c8bade5 in / 
# Wed, 20 Apr 2022 04:45:15 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:45:18 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:f5196cdf25181bc7e4411865a2e002932b7b6b0ffce787c04c1bdeaf1f204f20`  
		Last Modified: Wed, 20 Apr 2022 04:52:01 GMT  
		Size: 45.4 MB (45427434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d2394e6d00af092ff7367c7fb33292903653e6cf6aabe32479e312a38bb0cb5b`  
		Last Modified: Wed, 20 Apr 2022 04:52:16 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:37a8411b446d12d110918e96391ed07b36aabf0edec7bb132ea9d77ad181a2bc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44122895 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c3a67b0b313d69591facedfcacb81e41c77531f53965b82e865f19f66001e835`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:06 GMT
ADD file:407e28120c3ae0a1012433a9787cf812e0ecc360e63ace9fe21f6728470b4db1 in / 
# Wed, 20 Apr 2022 07:22:07 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:22:21 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c6849d3c2fa1fe7c2c78db1c26ca92e44c119dd3e92d93450115c927ecb6dce6`  
		Last Modified: Wed, 20 Apr 2022 07:39:41 GMT  
		Size: 44.1 MB (44122673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:98248ff0db11472ad62001823042f2e1f5bdeac0e6a484c74ce326e5ecc0eaba`  
		Last Modified: Wed, 20 Apr 2022 07:39:59 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:33b55d1d0f6c1b9d2566d7e56ee4eca8fca4b0e8873741e3f1f1dff6bbee39ca
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.2 MB (42151055 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:38d2abf4bc3ffb9bdb1111c27de0e4c1eedb0907b98aeed3cc0ffefd8e0c01d2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:15 GMT
ADD file:d237e5cd139706988e7bbc50ae542898a9b1fa1539f00322f21a71714975ccd8 in / 
# Wed, 20 Apr 2022 13:33:16 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:33:30 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ef9cca547ca93783f3168dc26ce42494066c74a8d5b51e766ac90a3fb5ad2931`  
		Last Modified: Wed, 20 Apr 2022 13:50:30 GMT  
		Size: 42.2 MB (42150830 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9cdd579cc812b935db1a3ce31f9118e0bd97271509943e6f218a1f896a17f84c`  
		Last Modified: Wed, 20 Apr 2022 13:50:49 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:69b33411284e960871a5c65303ac7002e770fd086c9d102c39b0759509cc5c74
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43212238 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3af33a0b58fbfa12ec25670dea899cd58d4023aceb7c58b142ae38b7f2612bb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:19 GMT
ADD file:73f1db8536438ca891f4b507a569e6c561364db0f05914ebea9d3fa97e1bf837 in / 
# Wed, 20 Apr 2022 04:31:20 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:31:26 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a41fbedfa4b1547a2b4fea33de48af745d66a94741d3cf344cb2766f0e80211b`  
		Last Modified: Wed, 20 Apr 2022 04:39:38 GMT  
		Size: 43.2 MB (43212018 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5b96a53eb3beb1d44463d8aff37df806ccd9366541010ca13c54f965cf0267e3`  
		Last Modified: Wed, 20 Apr 2022 04:39:55 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; 386

```console
$ docker pull debian@sha256:8e4e2bfd809ff66a5d33f6a8f356c4889968e30ee4f0f52a5029b62060db9945
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46147950 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f444c2a94f9431981aee1e2ed6b26aa68aed33aaa74258ea84a735bf6905799b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:39:57 GMT
ADD file:e8c6a60834ed931ccc3c93b1f2c9b9ca2bf190b0d8d2474382ee8e96a7e35b5a in / 
# Wed, 20 Apr 2022 07:39:57 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:40:04 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:cd343d4c0c2fbc67ddea33155621bdf36074fc30ffb6f917e716fbb81645f79a`  
		Last Modified: Wed, 20 Apr 2022 07:48:48 GMT  
		Size: 46.1 MB (46147730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4d4a0a39c21cf6cf5a561cede12d53c959a96644267af6576c061c575c761786`  
		Last Modified: Wed, 20 Apr 2022 07:49:05 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-slim`

```console
$ docker pull debian@sha256:e317b647e7b7df5229116215896f591917b95a8be9a0ba0ccfec83feff3861c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-slim` - linux; amd64

```console
$ docker pull debian@sha256:ef76a14f299ecc38bc00cd075f230dc69f8ff5aaad07a13b7cfb92fd3601e7de
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.6 MB (22567235 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd3475f2eda5b0088a2164878c70031d5ae6d0304580f86449d867934fd6e8e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:25 GMT
ADD file:0fafb7f95dc18699227154856361058bc9aba65de2ff88bf55cc66363142a05e in / 
# Wed, 20 Apr 2022 04:45:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8bd3f5a20b90c0e84cc65ea4a9172a51b7b5c0b9a097fa88d7102cac91cddf2c`  
		Last Modified: Wed, 20 Apr 2022 04:52:27 GMT  
		Size: 22.6 MB (22567235 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:371cd0b21fab642901d9e51dd04f095571c2b85eef693a0c209c376f722340fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21240552 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37e0f3d66f25288d6c7cb7e8ee7487a8a098e33df878873ea22a96bf541e723d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:22:40 GMT
ADD file:8caa5e7ca2ab4b420bed155bd2190baa73ca3c4e9378db02b0c0cede15974d69 in / 
# Wed, 20 Apr 2022 07:22:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d96febf107fb8f8f8c5d8bd55514488bd2c5f9ca3c49cb1c015381d7aa816d6`  
		Last Modified: Wed, 20 Apr 2022 07:40:19 GMT  
		Size: 21.2 MB (21240552 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ea27ab375b847954717cfeff2736d39679501579d38b3dc2f71ae6254735a64
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.4 MB (19359807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:898e8f79b5495e8f2c00b96f8c438b474227379a436b9f510cfda019b8c753a6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:33:51 GMT
ADD file:096692331564a30a9a9c6b24e681d99d68b54eeeabb7c5ac7d4073de06e050b2 in / 
# Wed, 20 Apr 2022 13:33:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08da560f0b65b004ba95f9a431d3ac51b8807b16ec6ef170e2e00dc55330246d`  
		Last Modified: Wed, 20 Apr 2022 13:51:07 GMT  
		Size: 19.4 MB (19359807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:13a6a318bc4e4200435e0f3ac16c65e3a60fa45511a472bd47e81c5bac41710b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20424115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3e0d7663d4764771c2ba52276e17e355f57961f2cd98863d1b29ebc3bf0f52e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:34 GMT
ADD file:02ad9214474cf0e6d278d7015dee30e6a8e24cdc699812f5fb3915491177e92b in / 
# Wed, 20 Apr 2022 04:31:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fca491399855be4c9b780dca621b48440831235d7916e5695b876698df84db7`  
		Last Modified: Wed, 20 Apr 2022 04:40:05 GMT  
		Size: 20.4 MB (20424115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; 386

```console
$ docker pull debian@sha256:33d586c9ff1c3dee1135a843b10b7c620107ef269e3ae83aa2b9d88719b91a30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23202179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b77216ff0bf0b007fd5cac2c9b36dcc3cb0fa2469d6d198dbffff8270facf9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:12 GMT
ADD file:f1394937f911449ed9fb7103f2bfb5b299d7779e98c4f0f34bcee08f8c2bf5db in / 
# Wed, 20 Apr 2022 07:40:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da9db5d720427742cbccdd8a4a37dfaa81873ab9db6de4935623ca79ce881ee4`  
		Last Modified: Wed, 20 Apr 2022 07:49:16 GMT  
		Size: 23.2 MB (23202179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing`

```console
$ docker pull debian@sha256:27a44f1596c36bc056154d9b6c72ec9ae99a569be8d17b97ea34f97e12fa9873
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:d028a76a73a5de693a4377141318ef20910abbf9f74f9dad86edfe98731adc68
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55578869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:48bb56efe1c89e203ffe7f782a56cba6e6d1cb12f60968de71633fbcfbf43ced`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:35 GMT
ADD file:fb66d619557384a14385fa7b5371be954594c4aff1700edcbd9cff422c3498f0 in / 
# Wed, 20 Apr 2022 04:45:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:536039adbcd59ca1bf50e1393d4c116319d1f29168898c56d21c192d219c4832`  
		Last Modified: Wed, 20 Apr 2022 04:52:49 GMT  
		Size: 55.6 MB (55578869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:6b5fc2dfc22a8ba83a0d101c83714e6aac6c50762f74c81c73a2457a8cbf29bb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001119 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8a05a685f035bc435afeef6d4fb66d684228a3f2c42e5483895a9ad7e7edf9df`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:23:11 GMT
ADD file:1a78dbadec303b303272bf60526069e90e5b10883a8d0400749940066e1366ca in / 
# Wed, 20 Apr 2022 07:23:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0966e12cf6b5ce5f837d6a4be5f22fc3bf1a9f73f8f399eda6ba93087d2fab69`  
		Last Modified: Wed, 20 Apr 2022 07:41:08 GMT  
		Size: 53.0 MB (53001119 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ed82d705ed318712a52d192d3dec55a083c96383e83b3bc97930f84048e5182
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620399 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:378ee5d4f72b9c0c0db6f0636b0a7c6960107be7888ddc1c16ed52a904c73052`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:34:23 GMT
ADD file:b4562b1984de2c46857d6e34c4a861fc9196690576727617cde894e3a0dd3f06 in / 
# Wed, 20 Apr 2022 13:34:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7b9a6a6547fd5374c2052305582d00416f4cd56fd55435b8d0044a6bfda0fba8`  
		Last Modified: Wed, 20 Apr 2022 13:51:53 GMT  
		Size: 50.6 MB (50620399 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:88c0860b848b6daa597e697d4c6bfaf351799294319a4f703ced9ade44bfaeb9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493338 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb0dad668e5272fb346215435f570a4a382da26784c85356b5d06878f7b58e84`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:45 GMT
ADD file:273f985806cb79b6f6b619ba5b9db50ba151174fde33bffc81c25187b0999170 in / 
# Wed, 20 Apr 2022 04:31:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3124af2dc12fec2edfce6b676869cb7dbcea85f39bf473efd3b5674765b68351`  
		Last Modified: Wed, 20 Apr 2022 04:40:29 GMT  
		Size: 54.5 MB (54493338 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:28a0b86050533d202e3a2c13e6cb003bc0820b832e9d183d8e25beee00b0dd06
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624517 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:76f1efb72f1520eea3d19ce4131826a67a0036dcd864b19854341d3c54584915`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:25 GMT
ADD file:990bedbbc12a848e4ddb2e9645f1c6a5e7d1b282f26d2ce1112fa7f0765e84cc in / 
# Wed, 20 Apr 2022 07:40:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:25dd2dde37c9ed5b4c67bd973631862a34b0b2b837c36d134cc8116840ec0a1a`  
		Last Modified: Wed, 20 Apr 2022 07:49:40 GMT  
		Size: 56.6 MB (56624517 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:8d9e8b2a9ccffe2c0e44a6df7e123c156c67a0f4e0e23f21bbdc952b3ea6cc40
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54240178 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:011a556d1eb503142329860e6793bf3c9b9ef086f1c47166a9afcdcfabc3198f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:47:32 GMT
ADD file:024d3880d7b57925a179be264896b146d4cfeac87003866bb0e336c33400106e in / 
# Tue, 29 Mar 2022 07:47:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:246ac5e155c140a71ba5195b835c78579b686fb0d9cbe2cf062e39583ddeac22`  
		Last Modified: Tue, 29 Mar 2022 07:58:46 GMT  
		Size: 54.2 MB (54240178 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:b7428f73a4f8442f6971864feeed6b5caead0827547d7ee664bcf47efc68e6fe
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982556 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7edeee2b43a74656b01130b2bce59c8cb4e14f94ade4015e086e262b0a0eaea3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:51:07 GMT
ADD file:0441cbd5b80c3154a95cf5f152472d9f238a42a1d874898b18c876944e830675 in / 
# Wed, 20 Apr 2022 09:51:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6a660090735cb86ef009831259b3363aa899b4b1f765ffad3047dd93f2146807`  
		Last Modified: Wed, 20 Apr 2022 10:01:09 GMT  
		Size: 60.0 MB (59982556 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:4424ebda5a4834fc82c585f3f5d7b640a49b67e911f1cadbfec0f471da5bdd39
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813515 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:52f5ecafd6a8c168c765fe28755de9ab5c8f09dc97071b163327ba3ea2f450c1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:44:07 GMT
ADD file:23684e7af9a096cf83a114c361e7879ed97ef5fbf7cb9607e40ffaf19dd0bed7 in / 
# Wed, 20 Apr 2022 08:44:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:362ce0a85fc3f682e0adac2df3a00133068c959c8dbf9d7872eb24b1b68f55a3`  
		Last Modified: Wed, 20 Apr 2022 08:52:12 GMT  
		Size: 53.8 MB (53813515 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20220418`

```console
$ docker pull debian@sha256:2ac07d2678bd3bd9e0cbdaa7d1cd0f4b20d6fdd8306daa0dddcb0ceb787df680
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20220418` - linux; amd64

```console
$ docker pull debian@sha256:d028a76a73a5de693a4377141318ef20910abbf9f74f9dad86edfe98731adc68
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55578869 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:48bb56efe1c89e203ffe7f782a56cba6e6d1cb12f60968de71633fbcfbf43ced`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:35 GMT
ADD file:fb66d619557384a14385fa7b5371be954594c4aff1700edcbd9cff422c3498f0 in / 
# Wed, 20 Apr 2022 04:45:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:536039adbcd59ca1bf50e1393d4c116319d1f29168898c56d21c192d219c4832`  
		Last Modified: Wed, 20 Apr 2022 04:52:49 GMT  
		Size: 55.6 MB (55578869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:6b5fc2dfc22a8ba83a0d101c83714e6aac6c50762f74c81c73a2457a8cbf29bb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001119 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8a05a685f035bc435afeef6d4fb66d684228a3f2c42e5483895a9ad7e7edf9df`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:23:11 GMT
ADD file:1a78dbadec303b303272bf60526069e90e5b10883a8d0400749940066e1366ca in / 
# Wed, 20 Apr 2022 07:23:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0966e12cf6b5ce5f837d6a4be5f22fc3bf1a9f73f8f399eda6ba93087d2fab69`  
		Last Modified: Wed, 20 Apr 2022 07:41:08 GMT  
		Size: 53.0 MB (53001119 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:9ed82d705ed318712a52d192d3dec55a083c96383e83b3bc97930f84048e5182
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620399 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:378ee5d4f72b9c0c0db6f0636b0a7c6960107be7888ddc1c16ed52a904c73052`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:34:23 GMT
ADD file:b4562b1984de2c46857d6e34c4a861fc9196690576727617cde894e3a0dd3f06 in / 
# Wed, 20 Apr 2022 13:34:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7b9a6a6547fd5374c2052305582d00416f4cd56fd55435b8d0044a6bfda0fba8`  
		Last Modified: Wed, 20 Apr 2022 13:51:53 GMT  
		Size: 50.6 MB (50620399 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:88c0860b848b6daa597e697d4c6bfaf351799294319a4f703ced9ade44bfaeb9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493338 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb0dad668e5272fb346215435f570a4a382da26784c85356b5d06878f7b58e84`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:45 GMT
ADD file:273f985806cb79b6f6b619ba5b9db50ba151174fde33bffc81c25187b0999170 in / 
# Wed, 20 Apr 2022 04:31:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3124af2dc12fec2edfce6b676869cb7dbcea85f39bf473efd3b5674765b68351`  
		Last Modified: Wed, 20 Apr 2022 04:40:29 GMT  
		Size: 54.5 MB (54493338 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; 386

```console
$ docker pull debian@sha256:28a0b86050533d202e3a2c13e6cb003bc0820b832e9d183d8e25beee00b0dd06
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624517 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:76f1efb72f1520eea3d19ce4131826a67a0036dcd864b19854341d3c54584915`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:25 GMT
ADD file:990bedbbc12a848e4ddb2e9645f1c6a5e7d1b282f26d2ce1112fa7f0765e84cc in / 
# Wed, 20 Apr 2022 07:40:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:25dd2dde37c9ed5b4c67bd973631862a34b0b2b837c36d134cc8116840ec0a1a`  
		Last Modified: Wed, 20 Apr 2022 07:49:40 GMT  
		Size: 56.6 MB (56624517 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:b7428f73a4f8442f6971864feeed6b5caead0827547d7ee664bcf47efc68e6fe
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982556 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7edeee2b43a74656b01130b2bce59c8cb4e14f94ade4015e086e262b0a0eaea3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:51:07 GMT
ADD file:0441cbd5b80c3154a95cf5f152472d9f238a42a1d874898b18c876944e830675 in / 
# Wed, 20 Apr 2022 09:51:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6a660090735cb86ef009831259b3363aa899b4b1f765ffad3047dd93f2146807`  
		Last Modified: Wed, 20 Apr 2022 10:01:09 GMT  
		Size: 60.0 MB (59982556 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418` - linux; s390x

```console
$ docker pull debian@sha256:4424ebda5a4834fc82c585f3f5d7b640a49b67e911f1cadbfec0f471da5bdd39
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813515 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:52f5ecafd6a8c168c765fe28755de9ab5c8f09dc97071b163327ba3ea2f450c1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:44:07 GMT
ADD file:23684e7af9a096cf83a114c361e7879ed97ef5fbf7cb9607e40ffaf19dd0bed7 in / 
# Wed, 20 Apr 2022 08:44:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:362ce0a85fc3f682e0adac2df3a00133068c959c8dbf9d7872eb24b1b68f55a3`  
		Last Modified: Wed, 20 Apr 2022 08:52:12 GMT  
		Size: 53.8 MB (53813515 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20220418-slim`

```console
$ docker pull debian@sha256:455ea9a92e2dee3257f5b0ccf4e37db35c5a70afb080eb9fdca460be650e25f1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 7
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:a68b5b424b01c67f1f4e4b288f7268f338f93a36c22f96297c182b8007cbcee4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31993134 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9c3edf315b3dd09cef91b6a2d01ff7d5aef5833d2c0bb580e4cffdf5a67279b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:45 GMT
ADD file:cd719eb98903f234a74da1be3a06bf71fc354c5f7eaaa15fcf5538565fd8870a in / 
# Wed, 20 Apr 2022 04:45:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40d46f201a2a06e8ab2a5f80e4c1b6074553938f26ce7936b1e2fe6810615ff2`  
		Last Modified: Wed, 20 Apr 2022 04:53:11 GMT  
		Size: 32.0 MB (31993134 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b22cca0aa11c4688550084c35cd185d3cd69a8ef271963c25c9dcac1aa7f19a3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.4 MB (29416612 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf755dc1eaff910b5033eac70587e23338d5205be821195568573342cbb25b77`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:23:46 GMT
ADD file:ccb1fc64bdbea34d071aec713b93e5dcc1c2477efe24a62597666a384ffba3a6 in / 
# Wed, 20 Apr 2022 07:23:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bb6f39990a16c1ff1d852497649a2109fba81b39b1da12f57f05c3d6063c2b1`  
		Last Modified: Wed, 20 Apr 2022 07:41:45 GMT  
		Size: 29.4 MB (29416612 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:4c21415671a2b47115663213abc466f8d069fb98ca8234acfddb4cba613fbd36
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (27034951 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c0dbb5a6dac629f3a3b25533f3aac3963321882b2619493b962c3830f36cc68`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:34:59 GMT
ADD file:1d1a80681aaee18701649fd147b12647b896d1a4370d25bfc8eee7408182bb6c in / 
# Wed, 20 Apr 2022 13:35:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a68edbdcf8418c3219090b6e124dcefd9c56c344a18cb465fbdb711104c8c94e`  
		Last Modified: Wed, 20 Apr 2022 13:52:28 GMT  
		Size: 27.0 MB (27034951 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:dbca0296405c60e75f327c478ec3c65cccd86693e4a6a71cf2c6fb69e2361692
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30914331 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:166e50fab213422be5b763f9f049bc151ffcc7dbbfe34d64058fce1c01da738a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:58 GMT
ADD file:ba285b1c6f5796c3d027a3333a5abaac0122ab699079dee4ff435c4394677703 in / 
# Wed, 20 Apr 2022 04:31:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef4019fe8d551899d8874125916041c91745d7cd1e218e018c2bafdb64b2780c`  
		Last Modified: Wed, 20 Apr 2022 04:40:51 GMT  
		Size: 30.9 MB (30914331 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:91f450d7e08cd0eafbade6313d6ba8bd6cf05a10efe7dade230e3482d5e5f605
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.0 MB (33042652 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:037489cc41203f5ca3584ed3341dd198b2868849715c8508651d71d5a298f826`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:40 GMT
ADD file:46351c98839f6a547b9a6adf55ebf535b254a90469f50c4f74468cabd2032175 in / 
# Wed, 20 Apr 2022 07:40:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f839a25ff88548319b8c6bd665f0459521341c802d8e950b22f943407d603776`  
		Last Modified: Wed, 20 Apr 2022 07:50:03 GMT  
		Size: 33.0 MB (33042652 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:3e4f40e38dc56e645c614f1134b9013f68b0d83b92175c193284485689f6e36f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.4 MB (36403326 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9830545e9796cb6030a76b26fe643dacd65577fc3dbc2e6f23cf8cc14e2cf65`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:51:51 GMT
ADD file:dea7cbcbfe343785f9675f4c4e812862b2db5f6159ea9e57cc403ccc47439428 in / 
# Wed, 20 Apr 2022 09:51:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f212c8643a4c6cc7e9c1331292cb160171801f091aacc8cb27b53a20d3ea8128`  
		Last Modified: Wed, 20 Apr 2022 10:01:40 GMT  
		Size: 36.4 MB (36403326 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:2c3438e4a9cda7c763c9d44a11c45eb007d7d1050e2b44c117d941179d480a1b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.2 MB (30230015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b2c023a60f87e02657cb8cf6a62098c2fd49a8044f90f7a819a0adc368d1d9f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:44:41 GMT
ADD file:b634fc268a842a4b154d76c8a07294c6a67301ad576b574b874ee44db49b7320 in / 
# Wed, 20 Apr 2022 08:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8fa69140642e0b7147ad9dbea6b031f707ffa6b8605265bf84ac4ca763f1651c`  
		Last Modified: Wed, 20 Apr 2022 08:52:29 GMT  
		Size: 30.2 MB (30230015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-backports`

```console
$ docker pull debian@sha256:c31b9b3370197b88fbb57ca4be3501a359d809a3ba530b6056abe0cc5aaaee68
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-backports` - linux; amd64

```console
$ docker pull debian@sha256:f637f11a2ff4380f65f935775cc7c41e96a32cbdb675e15635cbe101104d5da9
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.6 MB (55579088 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3480a9f7a4bca34b5b6cd26603deb0bea01bc2791686f7bca25b936c77208dfe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:35 GMT
ADD file:fb66d619557384a14385fa7b5371be954594c4aff1700edcbd9cff422c3498f0 in / 
# Wed, 20 Apr 2022 04:45:35 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:45:38 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:536039adbcd59ca1bf50e1393d4c116319d1f29168898c56d21c192d219c4832`  
		Last Modified: Wed, 20 Apr 2022 04:52:49 GMT  
		Size: 55.6 MB (55578869 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2954f5cd8207b7dac4a769e9a5d2bd826aef31f018a912501ee32b6244b1aca7`  
		Last Modified: Wed, 20 Apr 2022 04:53:00 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:c7eba5c344db35398a31101df7ae4187ab10ea6d6b2c09c6e10d68a7dc84c648
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (53001342 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:614cb3e15d39d2e228804b8cb67076650eae5d36da74daa08bf5d352a4c9c24e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:23:11 GMT
ADD file:1a78dbadec303b303272bf60526069e90e5b10883a8d0400749940066e1366ca in / 
# Wed, 20 Apr 2022 07:23:12 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:23:25 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0966e12cf6b5ce5f837d6a4be5f22fc3bf1a9f73f8f399eda6ba93087d2fab69`  
		Last Modified: Wed, 20 Apr 2022 07:41:08 GMT  
		Size: 53.0 MB (53001119 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6a5719d2b8c2420a309123ce8c0999b249c9740db341e9540ecda4eb80245d33`  
		Last Modified: Wed, 20 Apr 2022 07:41:20 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:01302be54c79de8fd349a12be5b7c6b7834b50a4b05f05f50bdafdc965bf54fc
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50620624 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:252ebc870b769800529916c5804f367adb34fdafe2de8627ec4fa1b13c9accc5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:34:23 GMT
ADD file:b4562b1984de2c46857d6e34c4a861fc9196690576727617cde894e3a0dd3f06 in / 
# Wed, 20 Apr 2022 13:34:24 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 13:34:37 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7b9a6a6547fd5374c2052305582d00416f4cd56fd55435b8d0044a6bfda0fba8`  
		Last Modified: Wed, 20 Apr 2022 13:51:53 GMT  
		Size: 50.6 MB (50620399 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:98e25ce69c3ffd7c6250e0d1505d5577b6c0485f38952781ed8f34ab4d3db637`  
		Last Modified: Wed, 20 Apr 2022 13:52:05 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5268989e7331d9b0996eccbb735bbd8e604f70430511f83f31eec32ae4591e07
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54493560 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:489d522af0e00f742c3ffaddc107729942676665752c2284b9d972f13579ecd3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:45 GMT
ADD file:273f985806cb79b6f6b619ba5b9db50ba151174fde33bffc81c25187b0999170 in / 
# Wed, 20 Apr 2022 04:31:45 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 04:31:51 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3124af2dc12fec2edfce6b676869cb7dbcea85f39bf473efd3b5674765b68351`  
		Last Modified: Wed, 20 Apr 2022 04:40:29 GMT  
		Size: 54.5 MB (54493338 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:eeec0554206fe7d868b12cfaff256babefc48ca0d621611263220d3124283725`  
		Last Modified: Wed, 20 Apr 2022 04:40:39 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; 386

```console
$ docker pull debian@sha256:19cc5083c3fb725a1f6cbc932bcdfb2f7ee5ff47fdb057f11b1d4b7021ccc2cb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.6 MB (56624736 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c8a8be1d997035a4f58ce782764809bc96c5a1e761223938dcdec1aa54b3550b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:25 GMT
ADD file:990bedbbc12a848e4ddb2e9645f1c6a5e7d1b282f26d2ce1112fa7f0765e84cc in / 
# Wed, 20 Apr 2022 07:40:26 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 07:40:31 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:25dd2dde37c9ed5b4c67bd973631862a34b0b2b837c36d134cc8116840ec0a1a`  
		Last Modified: Wed, 20 Apr 2022 07:49:40 GMT  
		Size: 56.6 MB (56624517 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b6eea383b106de4cac855efd53f667442c6201715451a60af96b9984df748957`  
		Last Modified: Wed, 20 Apr 2022 07:49:50 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; mips64le

```console
$ docker pull debian@sha256:a4b0dc4649f5a934e9c29eed98303f1d885376e2256cab6ebbea6650192fdb6e
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54240403 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72e2109bea6f18f9ef024529c2d611bf31023588a94e0c0512e3525c3704735e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:47:32 GMT
ADD file:024d3880d7b57925a179be264896b146d4cfeac87003866bb0e336c33400106e in / 
# Tue, 29 Mar 2022 07:47:36 GMT
CMD ["bash"]
# Tue, 29 Mar 2022 07:47:46 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:246ac5e155c140a71ba5195b835c78579b686fb0d9cbe2cf062e39583ddeac22`  
		Last Modified: Tue, 29 Mar 2022 07:58:46 GMT  
		Size: 54.2 MB (54240178 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:09255a198d562b80a7f219aab7d72c19e26b492ba0d9b5107b45640f913a4a8b`  
		Last Modified: Tue, 29 Mar 2022 07:58:55 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:f4e236df8ccde81037d5de775616442fdc2f5dd0dc9f4f9f6c49a294271f0c58
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (59982778 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2ef5ebc4fa88dfcfaaad716dbb46e736918ff20a7692a6c425bce888aab766c7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:51:07 GMT
ADD file:0441cbd5b80c3154a95cf5f152472d9f238a42a1d874898b18c876944e830675 in / 
# Wed, 20 Apr 2022 09:51:19 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 09:51:37 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6a660090735cb86ef009831259b3363aa899b4b1f765ffad3047dd93f2146807`  
		Last Modified: Wed, 20 Apr 2022 10:01:09 GMT  
		Size: 60.0 MB (59982556 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:39fa69df81c97ac7a3ad7411e6e28a3fb92b13f378a5187e454931200bbf563a`  
		Last Modified: Wed, 20 Apr 2022 10:01:22 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; s390x

```console
$ docker pull debian@sha256:ada62ead09a70633c9247a230e237007fdbba4ec9a52d86a902951d2591801ba
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.8 MB (53813739 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f683e1c642a85d7f8feac85a7ecacaed5666269318809e4bb5dc92fde13a2f86`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:44:07 GMT
ADD file:23684e7af9a096cf83a114c361e7879ed97ef5fbf7cb9607e40ffaf19dd0bed7 in / 
# Wed, 20 Apr 2022 08:44:14 GMT
CMD ["bash"]
# Wed, 20 Apr 2022 08:44:23 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:362ce0a85fc3f682e0adac2df3a00133068c959c8dbf9d7872eb24b1b68f55a3`  
		Last Modified: Wed, 20 Apr 2022 08:52:12 GMT  
		Size: 53.8 MB (53813515 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:dab3ab942c84a5ca094429e8c1fa42731e06a77aaed1d68117d52a60c8f16fcb`  
		Last Modified: Wed, 20 Apr 2022 08:52:20 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-slim`

```console
$ docker pull debian@sha256:7c74da90c8cfc9545ced00346960f69c955996c1c2339a458ed925f343fc857d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-slim` - linux; amd64

```console
$ docker pull debian@sha256:a68b5b424b01c67f1f4e4b288f7268f338f93a36c22f96297c182b8007cbcee4
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31993134 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9c3edf315b3dd09cef91b6a2d01ff7d5aef5833d2c0bb580e4cffdf5a67279b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:45 GMT
ADD file:cd719eb98903f234a74da1be3a06bf71fc354c5f7eaaa15fcf5538565fd8870a in / 
# Wed, 20 Apr 2022 04:45:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40d46f201a2a06e8ab2a5f80e4c1b6074553938f26ce7936b1e2fe6810615ff2`  
		Last Modified: Wed, 20 Apr 2022 04:53:11 GMT  
		Size: 32.0 MB (31993134 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b22cca0aa11c4688550084c35cd185d3cd69a8ef271963c25c9dcac1aa7f19a3
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.4 MB (29416612 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf755dc1eaff910b5033eac70587e23338d5205be821195568573342cbb25b77`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:23:46 GMT
ADD file:ccb1fc64bdbea34d071aec713b93e5dcc1c2477efe24a62597666a384ffba3a6 in / 
# Wed, 20 Apr 2022 07:23:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bb6f39990a16c1ff1d852497649a2109fba81b39b1da12f57f05c3d6063c2b1`  
		Last Modified: Wed, 20 Apr 2022 07:41:45 GMT  
		Size: 29.4 MB (29416612 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:4c21415671a2b47115663213abc466f8d069fb98ca8234acfddb4cba613fbd36
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (27034951 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c0dbb5a6dac629f3a3b25533f3aac3963321882b2619493b962c3830f36cc68`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:34:59 GMT
ADD file:1d1a80681aaee18701649fd147b12647b896d1a4370d25bfc8eee7408182bb6c in / 
# Wed, 20 Apr 2022 13:35:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a68edbdcf8418c3219090b6e124dcefd9c56c344a18cb465fbdb711104c8c94e`  
		Last Modified: Wed, 20 Apr 2022 13:52:28 GMT  
		Size: 27.0 MB (27034951 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:dbca0296405c60e75f327c478ec3c65cccd86693e4a6a71cf2c6fb69e2361692
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30914331 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:166e50fab213422be5b763f9f049bc151ffcc7dbbfe34d64058fce1c01da738a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:31:58 GMT
ADD file:ba285b1c6f5796c3d027a3333a5abaac0122ab699079dee4ff435c4394677703 in / 
# Wed, 20 Apr 2022 04:31:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef4019fe8d551899d8874125916041c91745d7cd1e218e018c2bafdb64b2780c`  
		Last Modified: Wed, 20 Apr 2022 04:40:51 GMT  
		Size: 30.9 MB (30914331 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; 386

```console
$ docker pull debian@sha256:91f450d7e08cd0eafbade6313d6ba8bd6cf05a10efe7dade230e3482d5e5f605
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.0 MB (33042652 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:037489cc41203f5ca3584ed3341dd198b2868849715c8508651d71d5a298f826`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:40 GMT
ADD file:46351c98839f6a547b9a6adf55ebf535b254a90469f50c4f74468cabd2032175 in / 
# Wed, 20 Apr 2022 07:40:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f839a25ff88548319b8c6bd665f0459521341c802d8e950b22f943407d603776`  
		Last Modified: Wed, 20 Apr 2022 07:50:03 GMT  
		Size: 33.0 MB (33042652 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; mips64le

```console
$ docker pull debian@sha256:9adb73829cee1266e407f6d1381c6fbda3bcee2110f5c54e4c0c99c273c9b626
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30715770 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:62c66ff4d08a74584b4aca1e24a3fcf5f02b496a395956a1d8b31f6a996a081e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:48:07 GMT
ADD file:58b9d9e3ac0c7baa795bae272288a1ec5911490cd064a219538e32300495fe23 in / 
# Tue, 29 Mar 2022 07:48:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:daa619241b534ce7cb56297b8247eb61cb0ab181dc4dddc790495b7cf425b991`  
		Last Modified: Tue, 29 Mar 2022 07:59:22 GMT  
		Size: 30.7 MB (30715770 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:3e4f40e38dc56e645c614f1134b9013f68b0d83b92175c193284485689f6e36f
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.4 MB (36403326 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9830545e9796cb6030a76b26fe643dacd65577fc3dbc2e6f23cf8cc14e2cf65`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:51:51 GMT
ADD file:dea7cbcbfe343785f9675f4c4e812862b2db5f6159ea9e57cc403ccc47439428 in / 
# Wed, 20 Apr 2022 09:51:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f212c8643a4c6cc7e9c1331292cb160171801f091aacc8cb27b53a20d3ea8128`  
		Last Modified: Wed, 20 Apr 2022 10:01:40 GMT  
		Size: 36.4 MB (36403326 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; s390x

```console
$ docker pull debian@sha256:2c3438e4a9cda7c763c9d44a11c45eb007d7d1050e2b44c117d941179d480a1b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.2 MB (30230015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b2c023a60f87e02657cb8cf6a62098c2fd49a8044f90f7a819a0adc368d1d9f0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:44:41 GMT
ADD file:b634fc268a842a4b154d76c8a07294c6a67301ad576b574b874ee44db49b7320 in / 
# Wed, 20 Apr 2022 08:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8fa69140642e0b7147ad9dbea6b031f707ffa6b8605265bf84ac4ca763f1651c`  
		Last Modified: Wed, 20 Apr 2022 08:52:29 GMT  
		Size: 30.2 MB (30230015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable`

```console
$ docker pull debian@sha256:67c0aa5ef9856b9861b300410f922756d8fd3700ec1ad0ee7dd39520730fce46
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable` - linux; amd64

```console
$ docker pull debian@sha256:faf622482fa732fe9ff059ef714e8956586ca6d70fb46725c5d8ab55eb86ed03
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:23a90b8e3c3009effa839d7142402670c5ceb35a3b113873de20628f3ef336f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:55 GMT
ADD file:b8f07060f9d2d7f5c4db9947fd089591ea261222b369890ff85325ec74bc0a6c in / 
# Wed, 20 Apr 2022 04:45:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7c26de214b349330f4067f62eaf7fbc9c2e817d82821b54e517e1456eedf3030`  
		Last Modified: Wed, 20 Apr 2022 04:53:29 GMT  
		Size: 56.1 MB (56112682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:b4c82e9b235fc94741191f870c4293a172a0080696be4abfba96417187a64b53
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518510 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d7858322f0f561604f815372e0e12e3e488b3f0647e7be61c6810d6731fe5ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:14 GMT
ADD file:d59b07addbc058dbb2e308592be3cdab758f37e2b392931b9f1cfea1a3a038e6 in / 
# Wed, 20 Apr 2022 07:24:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cbf05feef61225dcf4d5af2c7d07b77a5fb04c7158b3585c00e9c665a058ee9d`  
		Last Modified: Wed, 20 Apr 2022 07:42:28 GMT  
		Size: 53.5 MB (53518510 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:56ed5677d045c7f52592b68a42991d1046590a872af2ee2308ccfbd74803d2d7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125695 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f7fc5bd88bbb84d4ac2055805a80cb120135d59a76e7f52ea1088cec0958dba5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:30 GMT
ADD file:e72999b097295d9ae9ee09ef8bc01b0923d18fb194da7edfcd2d109356c00735 in / 
# Wed, 20 Apr 2022 13:35:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dccc0a48401f1ff6ee85164169c47fb40d68e6d429c81c0d90ff5c0030cfac46`  
		Last Modified: Wed, 20 Apr 2022 13:53:08 GMT  
		Size: 51.1 MB (51125695 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5e4f89944ac10a37522ba92696562acf6fe1233d1b2377a47a162bd5c7da88a6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55063807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e5b4be918551a63ac0f84b035c888bd20f087b0edf354383318d4799061354b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:09 GMT
ADD file:ecf4c1717eb6fdd76f39aa7c85e910a5b33b82a012a3115872223977e906fc6b in / 
# Wed, 20 Apr 2022 04:32:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b3f6be4af088219c3879fb497cd5051ae20697284c5fbf8f47d4d9f017ba3c2`  
		Last Modified: Wed, 20 Apr 2022 04:41:09 GMT  
		Size: 55.1 MB (55063807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; 386

```console
$ docker pull debian@sha256:71f60f9ff7e004b6b57f8ed13c08deff3ff20c13568420d0135024606ae0919b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57153982 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fd477463d66aaa2e9b436f0ec4efc9113bfe76fde88f0b36c7520185c77feca9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:52 GMT
ADD file:76ce7950d3083a262cf5a8a5350e77e81af33edd5696e7bea672c6b56dfba430 in / 
# Wed, 20 Apr 2022 07:40:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b03b8c90a9f4d2a5ce30d33ca3da78514bfc07dbf09d69b25fffd10d5626a40`  
		Last Modified: Wed, 20 Apr 2022 07:50:24 GMT  
		Size: 57.2 MB (57153982 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; mips64le

```console
$ docker pull debian@sha256:41dceb7dd70dda936b972a464de9d8235406aa6e77367255d0ec80f93d49b642
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54453472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96477e67b7b2cde0d7c72db08d19e716579392b9e9578ff7af729eab6992132f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:48:39 GMT
ADD file:1f36bf26ba6ed5898744d070089c9800f3701bbe25ca8e98ba248a56b990ac09 in / 
# Tue, 29 Mar 2022 07:48:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e7de11e455a9a72d18dbadb02f9098da749f18656d04d5f21c6fbb1ea7ecbfb9`  
		Last Modified: Tue, 29 Mar 2022 08:00:01 GMT  
		Size: 54.5 MB (54453472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; ppc64le

```console
$ docker pull debian@sha256:1e10c83513467e8f6e6828cc563e6c6174df28802c9a5f2d058b77662e01ac30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60557977 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4e1224c220c6831f6d3f5ba8ecb5dd9e09769b8641f1a03243c8fb6f30a7163`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:14 GMT
ADD file:66e1a3778c3b5bd2a8d17600540923d83a26fc3c9aaa8a9fb3df562018ddf467 in / 
# Wed, 20 Apr 2022 09:52:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0c49142451cdb4390822a432aca8fd4b80b2d7e389807dbe684df7dccc369ab`  
		Last Modified: Wed, 20 Apr 2022 10:02:03 GMT  
		Size: 60.6 MB (60557977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; riscv64

```console
$ docker pull debian@sha256:445600370c72af65a79fd309cac66d3f1f55c96ae09f9371bb09cda6ab13839a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757356 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b67eb36b007d31c495623df07d78226aa0c7b747b9233da3ee612c5263c334e1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:19:42 GMT
ADD file:eb73ccec6d534b35c3f0725251b8917d96bcd87d77d960d0a879ca91d2530a47 in / 
# Wed, 20 Apr 2022 00:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:62d036327287ffa181f78db1cedfa9a2d2ddc4c54f3c3838d75dae32e11ff1b1`  
		Last Modified: Wed, 20 Apr 2022 00:38:38 GMT  
		Size: 51.8 MB (51757356 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; s390x

```console
$ docker pull debian@sha256:5b1274cb9b8f6f5cf9acffca5159856a6b73a8fef815e4da21ccfdb5cd861acb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54330991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:216158c7e87a40595e9ea93e755ec9c7eb51075ca70eb39ce7f2cd1aad41df55`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:08 GMT
ADD file:c6ea51fdbfa2c4af7547b6b1dc6f96d047344c44c7918374ad035c3d804bc0d0 in / 
# Wed, 20 Apr 2022 08:45:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aa7bce33c54b3eca1469f66058926ebda6d4300fba1951826d8344a4735d337`  
		Last Modified: Wed, 20 Apr 2022 08:52:43 GMT  
		Size: 54.3 MB (54330991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20220418`

```console
$ docker pull debian@sha256:4e27c293440da6435c802e497da61927f173dbc2f2201ddc6995a3bcc1b8e0e0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20220418` - linux; amd64

```console
$ docker pull debian@sha256:faf622482fa732fe9ff059ef714e8956586ca6d70fb46725c5d8ab55eb86ed03
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56112682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:23a90b8e3c3009effa839d7142402670c5ceb35a3b113873de20628f3ef336f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:45:55 GMT
ADD file:b8f07060f9d2d7f5c4db9947fd089591ea261222b369890ff85325ec74bc0a6c in / 
# Wed, 20 Apr 2022 04:45:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7c26de214b349330f4067f62eaf7fbc9c2e817d82821b54e517e1456eedf3030`  
		Last Modified: Wed, 20 Apr 2022 04:53:29 GMT  
		Size: 56.1 MB (56112682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; arm variant v5

```console
$ docker pull debian@sha256:b4c82e9b235fc94741191f870c4293a172a0080696be4abfba96417187a64b53
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53518510 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d7858322f0f561604f815372e0e12e3e488b3f0647e7be61c6810d6731fe5ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:14 GMT
ADD file:d59b07addbc058dbb2e308592be3cdab758f37e2b392931b9f1cfea1a3a038e6 in / 
# Wed, 20 Apr 2022 07:24:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cbf05feef61225dcf4d5af2c7d07b77a5fb04c7158b3585c00e9c665a058ee9d`  
		Last Modified: Wed, 20 Apr 2022 07:42:28 GMT  
		Size: 53.5 MB (53518510 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; arm variant v7

```console
$ docker pull debian@sha256:56ed5677d045c7f52592b68a42991d1046590a872af2ee2308ccfbd74803d2d7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.1 MB (51125695 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f7fc5bd88bbb84d4ac2055805a80cb120135d59a76e7f52ea1088cec0958dba5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:30 GMT
ADD file:e72999b097295d9ae9ee09ef8bc01b0923d18fb194da7edfcd2d109356c00735 in / 
# Wed, 20 Apr 2022 13:35:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dccc0a48401f1ff6ee85164169c47fb40d68e6d429c81c0d90ff5c0030cfac46`  
		Last Modified: Wed, 20 Apr 2022 13:53:08 GMT  
		Size: 51.1 MB (51125695 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5e4f89944ac10a37522ba92696562acf6fe1233d1b2377a47a162bd5c7da88a6
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55063807 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e5b4be918551a63ac0f84b035c888bd20f087b0edf354383318d4799061354b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:09 GMT
ADD file:ecf4c1717eb6fdd76f39aa7c85e910a5b33b82a012a3115872223977e906fc6b in / 
# Wed, 20 Apr 2022 04:32:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b3f6be4af088219c3879fb497cd5051ae20697284c5fbf8f47d4d9f017ba3c2`  
		Last Modified: Wed, 20 Apr 2022 04:41:09 GMT  
		Size: 55.1 MB (55063807 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; 386

```console
$ docker pull debian@sha256:71f60f9ff7e004b6b57f8ed13c08deff3ff20c13568420d0135024606ae0919b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57153982 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fd477463d66aaa2e9b436f0ec4efc9113bfe76fde88f0b36c7520185c77feca9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:40:52 GMT
ADD file:76ce7950d3083a262cf5a8a5350e77e81af33edd5696e7bea672c6b56dfba430 in / 
# Wed, 20 Apr 2022 07:40:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b03b8c90a9f4d2a5ce30d33ca3da78514bfc07dbf09d69b25fffd10d5626a40`  
		Last Modified: Wed, 20 Apr 2022 07:50:24 GMT  
		Size: 57.2 MB (57153982 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; ppc64le

```console
$ docker pull debian@sha256:1e10c83513467e8f6e6828cc563e6c6174df28802c9a5f2d058b77662e01ac30
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.6 MB (60557977 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4e1224c220c6831f6d3f5ba8ecb5dd9e09769b8641f1a03243c8fb6f30a7163`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:14 GMT
ADD file:66e1a3778c3b5bd2a8d17600540923d83a26fc3c9aaa8a9fb3df562018ddf467 in / 
# Wed, 20 Apr 2022 09:52:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0c49142451cdb4390822a432aca8fd4b80b2d7e389807dbe684df7dccc369ab`  
		Last Modified: Wed, 20 Apr 2022 10:02:03 GMT  
		Size: 60.6 MB (60557977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; riscv64

```console
$ docker pull debian@sha256:445600370c72af65a79fd309cac66d3f1f55c96ae09f9371bb09cda6ab13839a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.8 MB (51757356 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b67eb36b007d31c495623df07d78226aa0c7b747b9233da3ee612c5263c334e1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:19:42 GMT
ADD file:eb73ccec6d534b35c3f0725251b8917d96bcd87d77d960d0a879ca91d2530a47 in / 
# Wed, 20 Apr 2022 00:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:62d036327287ffa181f78db1cedfa9a2d2ddc4c54f3c3838d75dae32e11ff1b1`  
		Last Modified: Wed, 20 Apr 2022 00:38:38 GMT  
		Size: 51.8 MB (51757356 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418` - linux; s390x

```console
$ docker pull debian@sha256:5b1274cb9b8f6f5cf9acffca5159856a6b73a8fef815e4da21ccfdb5cd861acb
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.3 MB (54330991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:216158c7e87a40595e9ea93e755ec9c7eb51075ca70eb39ce7f2cd1aad41df55`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:08 GMT
ADD file:c6ea51fdbfa2c4af7547b6b1dc6f96d047344c44c7918374ad035c3d804bc0d0 in / 
# Wed, 20 Apr 2022 08:45:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aa7bce33c54b3eca1469f66058926ebda6d4300fba1951826d8344a4735d337`  
		Last Modified: Wed, 20 Apr 2022 08:52:43 GMT  
		Size: 54.3 MB (54330991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20220418-slim`

```console
$ docker pull debian@sha256:0178aca3fac9a2bbba2040c6fccf3678403de79b252a7b04500121200214c857
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20220418-slim` - linux; amd64

```console
$ docker pull debian@sha256:137f6ee947f153b241f4e72d0ecdd37c3321a36f41cb655f62c3cacf46b54011
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.2 MB (32229749 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2342973c9d040e4a552bfccf5a426206bf396a7ca75e68e130f11c251765eb34`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:46:05 GMT
ADD file:66d4a8c613879ada814c4416bbe4e6d3ccbc6b0ea75a03067f8378fe0240c7d3 in / 
# Wed, 20 Apr 2022 04:46:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b0cb78170dbbf7031e6261517b87dfd32b94ebfe8ef47c5df480f34a3739cb6`  
		Last Modified: Wed, 20 Apr 2022 04:53:43 GMT  
		Size: 32.2 MB (32229749 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6ba8b72d5ddc168bacea61c57cf04e2f6dce3e7c54fd1fa68f55b4b119e41443
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29633303 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:289f962be40deb45148aca07517faf274d8eacce7795179b91c4ae8542d7ceb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:37 GMT
ADD file:9dce454b4f02591a22ce9a034fa800654d2408e6083f8ce18eae689d45400a45 in / 
# Wed, 20 Apr 2022 07:24:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef6f60d2d2d241fcb87b99e4d41035bab8e0f47d5a8f39f886c7971ea93b746c`  
		Last Modified: Wed, 20 Apr 2022 07:42:58 GMT  
		Size: 29.6 MB (29633303 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:908d0c48bbcf433d9556407ba78bd971a938666eb7ec75292283f5ff10cf0a11
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.3 MB (27250902 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c91ae1962add336e7321ad7e3f6c14b677ffdb882965e5c0c316e3557f6b7220`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:54 GMT
ADD file:743399e137e39dae6620d187402e710e2f966d7dab63a065535f155dd140778c in / 
# Wed, 20 Apr 2022 13:35:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a09d17d8c3b0ed0a3e161e0607949a2674f7127e4f7a4844f9331c0b9634917`  
		Last Modified: Wed, 20 Apr 2022 13:53:35 GMT  
		Size: 27.3 MB (27250902 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b998b044adfd383d66f0655865154cadef15cd722ac98038eb0a035f9684ec11
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.2 MB (31184535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1b873bc8afdda0dc2f7dc5f4f1ae3d53c840cc829e21e21392ce22b9460dded`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:18 GMT
ADD file:9e79b776ec24f843cdd21d707283111a9dc89a97165a110daa892b445b7b39c7 in / 
# Wed, 20 Apr 2022 04:32:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4980db26fedb2a65774a4d406de333f539570c3d21f4fc8f5edd381391841e04`  
		Last Modified: Wed, 20 Apr 2022 04:41:24 GMT  
		Size: 31.2 MB (31184535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; 386

```console
$ docker pull debian@sha256:8214e1000cc3605e318dc3e9752ef9e829600a88e4cf0ffc44f48c40f43c569c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.3 MB (33286504 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c7e196b16fd82581f0ebf4dddaa981f530070b45bf04d5d7b882ace71817de58`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:41:02 GMT
ADD file:2a66fd30e67d6b2f1bc2bf89eeb696077958b49c0aa80cc179f1cba3d7a475db in / 
# Wed, 20 Apr 2022 07:41:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:adad59a8414b3686855b5ab14b6e48e3ebeb00d6c7e07c93b7455992f20895a2`  
		Last Modified: Wed, 20 Apr 2022 07:50:41 GMT  
		Size: 33.3 MB (33286504 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:901578ff67f03bc1b9b7ba33ff6503feecd64fc6ed73dd65a33a7963ee1bcc43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.7 MB (36677817 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:08a77a55dd11f99fa44f923a120f22d1d1b4a5b3b7e5382e56396bf19773ccaa`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:36 GMT
ADD file:c4b75293ff946dc3f3bd118e298d3998761a454482d93defcdc36e85b8627b38 in / 
# Wed, 20 Apr 2022 09:52:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:11087f2bafaa939a2323669ba4629df9dfb8491ec9e4793a6e92daf647a41312`  
		Last Modified: Wed, 20 Apr 2022 10:02:23 GMT  
		Size: 36.7 MB (36677817 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; riscv64

```console
$ docker pull debian@sha256:d852fe72d3a1b7e7ce65efae8877314f1469069034d31b8679b9871d6f2c2e81
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.9 MB (27887283 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:035ab24d66a6351c99cc4500660d13187548ac124aded7ffadc7d62ab8acfef9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:21:33 GMT
ADD file:abd539428cde7e4098c8ca877165ceb40ff3c200d5f187a2a15d0086aff9e856 in / 
# Wed, 20 Apr 2022 00:21:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2069855bbef5541a86d00976156e5a8c9f1dbb9f433051afae264cf7992d6d1d`  
		Last Modified: Wed, 20 Apr 2022 00:40:24 GMT  
		Size: 27.9 MB (27887283 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20220418-slim` - linux; s390x

```console
$ docker pull debian@sha256:0ffc637e5411623be3fe3efa0151f4f839a6ff85cf7937377b12fb7449b481e7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30453649 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d7129aace406e57bea5255306767947e837bcbd68bd0caa359a9a2c9a6d79ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:35 GMT
ADD file:ab2d454fe445a93e3cd5b8281c54cc9db2450cf4fa81c5a2d8645f0ddf2d57f1 in / 
# Wed, 20 Apr 2022 08:45:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a5e20b42e934b4ebf84a22ca3a2ccd2121e5fb9622673ff29c9824c7497d7810`  
		Last Modified: Wed, 20 Apr 2022 08:52:56 GMT  
		Size: 30.5 MB (30453649 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-slim`

```console
$ docker pull debian@sha256:94d81fc13b4976772739771cad1d1fbae129133703ec3aebc82e1c9c0e1160d8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:137f6ee947f153b241f4e72d0ecdd37c3321a36f41cb655f62c3cacf46b54011
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.2 MB (32229749 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2342973c9d040e4a552bfccf5a426206bf396a7ca75e68e130f11c251765eb34`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:46:05 GMT
ADD file:66d4a8c613879ada814c4416bbe4e6d3ccbc6b0ea75a03067f8378fe0240c7d3 in / 
# Wed, 20 Apr 2022 04:46:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b0cb78170dbbf7031e6261517b87dfd32b94ebfe8ef47c5df480f34a3739cb6`  
		Last Modified: Wed, 20 Apr 2022 04:53:43 GMT  
		Size: 32.2 MB (32229749 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6ba8b72d5ddc168bacea61c57cf04e2f6dce3e7c54fd1fa68f55b4b119e41443
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29633303 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:289f962be40deb45148aca07517faf274d8eacce7795179b91c4ae8542d7ceb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:24:37 GMT
ADD file:9dce454b4f02591a22ce9a034fa800654d2408e6083f8ce18eae689d45400a45 in / 
# Wed, 20 Apr 2022 07:24:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ef6f60d2d2d241fcb87b99e4d41035bab8e0f47d5a8f39f886c7971ea93b746c`  
		Last Modified: Wed, 20 Apr 2022 07:42:58 GMT  
		Size: 29.6 MB (29633303 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:908d0c48bbcf433d9556407ba78bd971a938666eb7ec75292283f5ff10cf0a11
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.3 MB (27250902 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c91ae1962add336e7321ad7e3f6c14b677ffdb882965e5c0c316e3557f6b7220`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 13:35:54 GMT
ADD file:743399e137e39dae6620d187402e710e2f966d7dab63a065535f155dd140778c in / 
# Wed, 20 Apr 2022 13:35:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a09d17d8c3b0ed0a3e161e0607949a2674f7127e4f7a4844f9331c0b9634917`  
		Last Modified: Wed, 20 Apr 2022 13:53:35 GMT  
		Size: 27.3 MB (27250902 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b998b044adfd383d66f0655865154cadef15cd722ac98038eb0a035f9684ec11
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.2 MB (31184535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1b873bc8afdda0dc2f7dc5f4f1ae3d53c840cc829e21e21392ce22b9460dded`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 04:32:18 GMT
ADD file:9e79b776ec24f843cdd21d707283111a9dc89a97165a110daa892b445b7b39c7 in / 
# Wed, 20 Apr 2022 04:32:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4980db26fedb2a65774a4d406de333f539570c3d21f4fc8f5edd381391841e04`  
		Last Modified: Wed, 20 Apr 2022 04:41:24 GMT  
		Size: 31.2 MB (31184535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; 386

```console
$ docker pull debian@sha256:8214e1000cc3605e318dc3e9752ef9e829600a88e4cf0ffc44f48c40f43c569c
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.3 MB (33286504 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c7e196b16fd82581f0ebf4dddaa981f530070b45bf04d5d7b882ace71817de58`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 07:41:02 GMT
ADD file:2a66fd30e67d6b2f1bc2bf89eeb696077958b49c0aa80cc179f1cba3d7a475db in / 
# Wed, 20 Apr 2022 07:41:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:adad59a8414b3686855b5ab14b6e48e3ebeb00d6c7e07c93b7455992f20895a2`  
		Last Modified: Wed, 20 Apr 2022 07:50:41 GMT  
		Size: 33.3 MB (33286504 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:70bc0e0d01e930d54d2d440ab21537039ac9190c9cfb499dcf62e04ac38900d0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30727488 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:051d2440ec6bdc0853192c8edd79a7d541a7388ee10e0251c2e536f4898e023a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 29 Mar 2022 07:49:09 GMT
ADD file:0eb34296fa92e50e56fba54954152bab897f9a07fb9f8f710e099d32bb9613f3 in / 
# Tue, 29 Mar 2022 07:49:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2c56ed5eec02f286ea19b1a5885d155f6fa57eac59d03ad2ae1dd5bc8a2afc20`  
		Last Modified: Tue, 29 Mar 2022 08:00:31 GMT  
		Size: 30.7 MB (30727488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:901578ff67f03bc1b9b7ba33ff6503feecd64fc6ed73dd65a33a7963ee1bcc43
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.7 MB (36677817 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:08a77a55dd11f99fa44f923a120f22d1d1b4a5b3b7e5382e56396bf19773ccaa`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 09:52:36 GMT
ADD file:c4b75293ff946dc3f3bd118e298d3998761a454482d93defcdc36e85b8627b38 in / 
# Wed, 20 Apr 2022 09:52:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:11087f2bafaa939a2323669ba4629df9dfb8491ec9e4793a6e92daf647a41312`  
		Last Modified: Wed, 20 Apr 2022 10:02:23 GMT  
		Size: 36.7 MB (36677817 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; riscv64

```console
$ docker pull debian@sha256:d852fe72d3a1b7e7ce65efae8877314f1469069034d31b8679b9871d6f2c2e81
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.9 MB (27887283 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:035ab24d66a6351c99cc4500660d13187548ac124aded7ffadc7d62ab8acfef9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 00:21:33 GMT
ADD file:abd539428cde7e4098c8ca877165ceb40ff3c200d5f187a2a15d0086aff9e856 in / 
# Wed, 20 Apr 2022 00:21:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2069855bbef5541a86d00976156e5a8c9f1dbb9f433051afae264cf7992d6d1d`  
		Last Modified: Wed, 20 Apr 2022 00:40:24 GMT  
		Size: 27.9 MB (27887283 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:0ffc637e5411623be3fe3efa0151f4f839a6ff85cf7937377b12fb7449b481e7
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30453649 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d7129aace406e57bea5255306767947e837bcbd68bd0caa359a9a2c9a6d79ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 20 Apr 2022 08:45:35 GMT
ADD file:ab2d454fe445a93e3cd5b8281c54cc9db2450cf4fa81c5a2d8645f0ddf2d57f1 in / 
# Wed, 20 Apr 2022 08:45:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a5e20b42e934b4ebf84a22ca3a2ccd2121e5fb9622673ff29c9824c7497d7810`  
		Last Modified: Wed, 20 Apr 2022 08:52:56 GMT  
		Size: 30.5 MB (30453649 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
