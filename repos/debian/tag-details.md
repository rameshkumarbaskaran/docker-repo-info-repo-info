<!-- THIS FILE IS GENERATED VIA './update-remote.sh' -->

# Tags of `debian`

-	[`debian:10`](#debian10)
-	[`debian:10-slim`](#debian10-slim)
-	[`debian:10.13`](#debian1013)
-	[`debian:10.13-slim`](#debian1013-slim)
-	[`debian:11`](#debian11)
-	[`debian:11-slim`](#debian11-slim)
-	[`debian:11.7`](#debian117)
-	[`debian:11.7-slim`](#debian117-slim)
-	[`debian:bookworm`](#debianbookworm)
-	[`debian:bookworm-20230502`](#debianbookworm-20230502)
-	[`debian:bookworm-20230502-slim`](#debianbookworm-20230502-slim)
-	[`debian:bookworm-backports`](#debianbookworm-backports)
-	[`debian:bookworm-slim`](#debianbookworm-slim)
-	[`debian:bullseye`](#debianbullseye)
-	[`debian:bullseye-20230502`](#debianbullseye-20230502)
-	[`debian:bullseye-20230502-slim`](#debianbullseye-20230502-slim)
-	[`debian:bullseye-backports`](#debianbullseye-backports)
-	[`debian:bullseye-slim`](#debianbullseye-slim)
-	[`debian:buster`](#debianbuster)
-	[`debian:buster-20230502`](#debianbuster-20230502)
-	[`debian:buster-20230502-slim`](#debianbuster-20230502-slim)
-	[`debian:buster-backports`](#debianbuster-backports)
-	[`debian:buster-slim`](#debianbuster-slim)
-	[`debian:experimental`](#debianexperimental)
-	[`debian:experimental-20230502`](#debianexperimental-20230502)
-	[`debian:latest`](#debianlatest)
-	[`debian:oldstable`](#debianoldstable)
-	[`debian:oldstable-20230502`](#debianoldstable-20230502)
-	[`debian:oldstable-20230502-slim`](#debianoldstable-20230502-slim)
-	[`debian:oldstable-backports`](#debianoldstable-backports)
-	[`debian:oldstable-slim`](#debianoldstable-slim)
-	[`debian:rc-buggy`](#debianrc-buggy)
-	[`debian:rc-buggy-20230502`](#debianrc-buggy-20230502)
-	[`debian:sid`](#debiansid)
-	[`debian:sid-20230502`](#debiansid-20230502)
-	[`debian:sid-20230502-slim`](#debiansid-20230502-slim)
-	[`debian:sid-slim`](#debiansid-slim)
-	[`debian:stable`](#debianstable)
-	[`debian:stable-20230502`](#debianstable-20230502)
-	[`debian:stable-20230502-slim`](#debianstable-20230502-slim)
-	[`debian:stable-backports`](#debianstable-backports)
-	[`debian:stable-slim`](#debianstable-slim)
-	[`debian:testing`](#debiantesting)
-	[`debian:testing-20230502`](#debiantesting-20230502)
-	[`debian:testing-20230502-slim`](#debiantesting-20230502-slim)
-	[`debian:testing-backports`](#debiantesting-backports)
-	[`debian:testing-slim`](#debiantesting-slim)
-	[`debian:unstable`](#debianunstable)
-	[`debian:unstable-20230502`](#debianunstable-20230502)
-	[`debian:unstable-20230502-slim`](#debianunstable-20230502-slim)
-	[`debian:unstable-slim`](#debianunstable-slim)

## `debian:10`

```console
$ docker pull debian@sha256:173f0b19d2ac5ee17194dbe9a888a4a617dbf905ee666c9e780afad55eaf1d8c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10` - linux; amd64

```console
$ docker pull debian@sha256:2803ab27dfe4f2e23b58d351e80e8d433942b6fb830225e6d034098ed450a169
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448726 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b277f5e168f773e3dba38bca291a5fd138d56fcdd5008a2f0bf1866d311483b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:15 GMT
ADD file:40953ed6e6f96703b2e0c13288437c2aaf8b3df33dbc423686290cbe0e595a5e in / 
# Wed, 12 Apr 2023 00:20:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2ff1d7c41c74a25258bfa6f0b8adb0a727f84518f55f65ca845ebc747976c408`  
		Last Modified: Wed, 12 Apr 2023 00:24:01 GMT  
		Size: 50.4 MB (50448726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v7

```console
$ docker pull debian@sha256:3e45d273ad15d7b648212c8faa5e236f6fdc694af7272b8c4bd25a5d90c27fe1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:966f48305f6385972a055032836be86bf1af1f7b601a7b8271ef7b695ab566b0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:53 GMT
ADD file:3f6ca36ec4fbf50ab85c907c089dbb0618c73393c07a5841b922145b17795552 in / 
# Tue, 11 Apr 2023 23:59:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:166ef90dbaeab03f451f526d325c57795001e25f10c984c334a82d174b3c554b`  
		Last Modified: Wed, 12 Apr 2023 00:03:36 GMT  
		Size: 45.9 MB (45916201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:55cd39723a057f730e9c33487fea9fe151bbe86dce51e92a73ccfd12206a1f43
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238632 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0604d0d2dc821d9cfef7619166adc659049f3ef4092aabd9bfb268d86869e56`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:55 GMT
ADD file:93facc669dd63b37fb5dde18f3b3a1cb5621aa396e1960ea85facdd1c619a3f0 in / 
# Wed, 12 Apr 2023 00:39:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4910c31031a0301ea4f8b7155269014925aeb17c71b869dea3ff907ba294b55`  
		Last Modified: Wed, 12 Apr 2023 00:42:52 GMT  
		Size: 49.2 MB (49238632 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; 386

```console
$ docker pull debian@sha256:91e8c113a94dfe5daf935a7d943a8ed9e500bce23540c57bd91153eb1d9b8c15
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68659ee584b238b4b4811ba3fbb00f097987eba15c8a0f3beeefc5ddb1ca7a63`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:11 GMT
ADD file:8299e58c7e43af1f3b9fdb9bd37e3f37cf619a97ef1b87d60ca66860d646d526 in / 
# Wed, 12 Apr 2023 00:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d15e1c2a3da7d675c2513d7ceab1d0f33ced9a82bed63d9a11ce71f1218d232`  
		Last Modified: Wed, 12 Apr 2023 00:43:07 GMT  
		Size: 51.2 MB (51206167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10-slim`

```console
$ docker pull debian@sha256:4d208d79338beaa197912496d0791921a31d9c58fa6fbd299787fab37cc893dd
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10-slim` - linux; amd64

```console
$ docker pull debian@sha256:603e7e0b2c5ee5dcfd1b1ba07b4598dd481b6afa6dcf348399428e72b0370a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138920 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92a9b50a62b45ede9878156909ea76de9d04716aa0cdfe175bd2d28e34b87149`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:25 GMT
ADD file:e614539607055bdbde0cc1a94ef9783fe3627c8553ef1b21071ecb5c27ea27e4 in / 
# Wed, 12 Apr 2023 00:20:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9fbefa3370776b7ec7633cf07efc14cc24e0c0cd53893ad0e7e3f44ffdc1bedb`  
		Last Modified: Wed, 12 Apr 2023 00:24:22 GMT  
		Size: 27.1 MB (27138920 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:afaf84f53aff159d638edc52279285cc4310214dfa612247748f05653dbf70f6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:688cfdcf3400ea8753eb981828a4e85b493091379bbc0f641a0e231aa6cb247f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:02 GMT
ADD file:72785c92abeacda7808e9e65787e604ed7fbc159c53eb4b2337628501418e9ce in / 
# Wed, 12 Apr 2023 00:00:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2f777f2c33b3c8a258e4118224a280169c60be31db805388525da1f620f59a86`  
		Last Modified: Wed, 12 Apr 2023 00:03:58 GMT  
		Size: 22.7 MB (22747664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5889e7bed85fdc3fd2c124b4fc0b8de6b2201c26ddb7b2d886e0d42b00d74334
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25922011 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92d2f037b9ac2076ddd4d19cbf795887a83cbd9d179c3d18a656ac62b6e880a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:02 GMT
ADD file:39d4ef5cc48487567b857a8527ab3c106b5099eafffbeaa8b70df450fb8409e3 in / 
# Wed, 12 Apr 2023 00:40:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:27bec964ec41cf69bd5df2ffd45c27623ae4bb0eb6f9f366179f7ac9b47d9840`  
		Last Modified: Wed, 12 Apr 2023 00:43:11 GMT  
		Size: 25.9 MB (25922011 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; 386

```console
$ docker pull debian@sha256:a9b9702cddec97594b2a09947e77a615f2e78c17c12d319233592a9ef994b707
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797581 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e93730b5b7b45f61b02834bfc1f973ce7f27f98f58b3522f29c84c78485b31b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:21 GMT
ADD file:0f4c5475f636be52b419806a7ea7d8d81fc1203c9228544c1324e79277739a3f in / 
# Wed, 12 Apr 2023 00:39:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ae6c02ab25c72d784ae8832cdcb583320ab9a3046938adc3c659ae607da19b43`  
		Last Modified: Wed, 12 Apr 2023 00:43:29 GMT  
		Size: 27.8 MB (27797581 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.13`

```console
$ docker pull debian@sha256:173f0b19d2ac5ee17194dbe9a888a4a617dbf905ee666c9e780afad55eaf1d8c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10.13` - linux; amd64

```console
$ docker pull debian@sha256:2803ab27dfe4f2e23b58d351e80e8d433942b6fb830225e6d034098ed450a169
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448726 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b277f5e168f773e3dba38bca291a5fd138d56fcdd5008a2f0bf1866d311483b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:15 GMT
ADD file:40953ed6e6f96703b2e0c13288437c2aaf8b3df33dbc423686290cbe0e595a5e in / 
# Wed, 12 Apr 2023 00:20:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2ff1d7c41c74a25258bfa6f0b8adb0a727f84518f55f65ca845ebc747976c408`  
		Last Modified: Wed, 12 Apr 2023 00:24:01 GMT  
		Size: 50.4 MB (50448726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; arm variant v7

```console
$ docker pull debian@sha256:3e45d273ad15d7b648212c8faa5e236f6fdc694af7272b8c4bd25a5d90c27fe1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:966f48305f6385972a055032836be86bf1af1f7b601a7b8271ef7b695ab566b0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:53 GMT
ADD file:3f6ca36ec4fbf50ab85c907c089dbb0618c73393c07a5841b922145b17795552 in / 
# Tue, 11 Apr 2023 23:59:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:166ef90dbaeab03f451f526d325c57795001e25f10c984c334a82d174b3c554b`  
		Last Modified: Wed, 12 Apr 2023 00:03:36 GMT  
		Size: 45.9 MB (45916201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:55cd39723a057f730e9c33487fea9fe151bbe86dce51e92a73ccfd12206a1f43
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238632 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0604d0d2dc821d9cfef7619166adc659049f3ef4092aabd9bfb268d86869e56`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:55 GMT
ADD file:93facc669dd63b37fb5dde18f3b3a1cb5621aa396e1960ea85facdd1c619a3f0 in / 
# Wed, 12 Apr 2023 00:39:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4910c31031a0301ea4f8b7155269014925aeb17c71b869dea3ff907ba294b55`  
		Last Modified: Wed, 12 Apr 2023 00:42:52 GMT  
		Size: 49.2 MB (49238632 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; 386

```console
$ docker pull debian@sha256:91e8c113a94dfe5daf935a7d943a8ed9e500bce23540c57bd91153eb1d9b8c15
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68659ee584b238b4b4811ba3fbb00f097987eba15c8a0f3beeefc5ddb1ca7a63`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:11 GMT
ADD file:8299e58c7e43af1f3b9fdb9bd37e3f37cf619a97ef1b87d60ca66860d646d526 in / 
# Wed, 12 Apr 2023 00:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d15e1c2a3da7d675c2513d7ceab1d0f33ced9a82bed63d9a11ce71f1218d232`  
		Last Modified: Wed, 12 Apr 2023 00:43:07 GMT  
		Size: 51.2 MB (51206167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.13-slim`

```console
$ docker pull debian@sha256:4d208d79338beaa197912496d0791921a31d9c58fa6fbd299787fab37cc893dd
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10.13-slim` - linux; amd64

```console
$ docker pull debian@sha256:603e7e0b2c5ee5dcfd1b1ba07b4598dd481b6afa6dcf348399428e72b0370a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138920 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92a9b50a62b45ede9878156909ea76de9d04716aa0cdfe175bd2d28e34b87149`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:25 GMT
ADD file:e614539607055bdbde0cc1a94ef9783fe3627c8553ef1b21071ecb5c27ea27e4 in / 
# Wed, 12 Apr 2023 00:20:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9fbefa3370776b7ec7633cf07efc14cc24e0c0cd53893ad0e7e3f44ffdc1bedb`  
		Last Modified: Wed, 12 Apr 2023 00:24:22 GMT  
		Size: 27.1 MB (27138920 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:afaf84f53aff159d638edc52279285cc4310214dfa612247748f05653dbf70f6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:688cfdcf3400ea8753eb981828a4e85b493091379bbc0f641a0e231aa6cb247f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:02 GMT
ADD file:72785c92abeacda7808e9e65787e604ed7fbc159c53eb4b2337628501418e9ce in / 
# Wed, 12 Apr 2023 00:00:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2f777f2c33b3c8a258e4118224a280169c60be31db805388525da1f620f59a86`  
		Last Modified: Wed, 12 Apr 2023 00:03:58 GMT  
		Size: 22.7 MB (22747664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5889e7bed85fdc3fd2c124b4fc0b8de6b2201c26ddb7b2d886e0d42b00d74334
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25922011 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92d2f037b9ac2076ddd4d19cbf795887a83cbd9d179c3d18a656ac62b6e880a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:02 GMT
ADD file:39d4ef5cc48487567b857a8527ab3c106b5099eafffbeaa8b70df450fb8409e3 in / 
# Wed, 12 Apr 2023 00:40:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:27bec964ec41cf69bd5df2ffd45c27623ae4bb0eb6f9f366179f7ac9b47d9840`  
		Last Modified: Wed, 12 Apr 2023 00:43:11 GMT  
		Size: 25.9 MB (25922011 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; 386

```console
$ docker pull debian@sha256:a9b9702cddec97594b2a09947e77a615f2e78c17c12d319233592a9ef994b707
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797581 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e93730b5b7b45f61b02834bfc1f973ce7f27f98f58b3522f29c84c78485b31b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:21 GMT
ADD file:0f4c5475f636be52b419806a7ea7d8d81fc1203c9228544c1324e79277739a3f in / 
# Wed, 12 Apr 2023 00:39:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ae6c02ab25c72d784ae8832cdcb583320ab9a3046938adc3c659ae607da19b43`  
		Last Modified: Wed, 12 Apr 2023 00:43:29 GMT  
		Size: 27.8 MB (27797581 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11`

```console
$ docker pull debian@sha256:251456f07b3fcc936c6ebf5218e6a8ffae63e4a86d23f16fb1b023f160eca7e6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11` - linux; amd64

```console
$ docker pull debian@sha256:4effa16ae79ab36347d55ebb6bb4aad411077b72ad735054e684b3eeba373a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052736 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca1a8403096efb7da1512b0ae008b16f2dae553deed435163b137447b90fa8b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:55 GMT
ADD file:f2d012660f882f319a5878a3f9ce285f488b8f90fad49ad238541cf72089e035 in / 
# Wed, 12 Apr 2023 00:19:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0248cf3e63c73d0e496a67807d056ca41d5e968b61087e8eca2cf4b9b4d7b99`  
		Last Modified: Wed, 12 Apr 2023 00:23:20 GMT  
		Size: 55.1 MB (55052736 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v5

```console
$ docker pull debian@sha256:d170adce3d14567fad94acb11df1b8811da1a742668a2289778be573cb9a7ce8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb6d0a852fe4800d15c6c1b1fce5de90be94f044e74cabdfd5418db829529565`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab0ce510343d33415fc4eaea0102da2d24cb720b185aaee0b411c48a07abb0de
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50216882 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b29663cecd1d9b95d625512cc33ad2c8d970653700f863175d7fdf5b022fc129`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:35 GMT
ADD file:75c57df33c95251a80549b7949853df282a42bc4e5f19176764907a54b74caa9 in / 
# Tue, 11 Apr 2023 23:59:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4feff9b88735325634445419eaaec2ed47500b63027cfd2de34b8bc6d55ee85c`  
		Last Modified: Wed, 12 Apr 2023 00:02:57 GMT  
		Size: 50.2 MB (50216882 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c0f6422141bb031238126fbd535c0df24db4252a2d9876216a94082df74c8fda
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705529 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6d9c9c3e100a13622816226e63a3faa83da315ae01216b1ac02c4ba5cc91683`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:42 GMT
ADD file:dd3d4e5fe819950e7d01b44a29dbd790438cd722ba76198910e2597448ab0c6f in / 
# Wed, 12 Apr 2023 00:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dc80b8cdbfd36cb20231d807a50d704945d1df4da8f6e23197ccfcb629970491`  
		Last Modified: Wed, 12 Apr 2023 00:42:14 GMT  
		Size: 53.7 MB (53705529 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; 386

```console
$ docker pull debian@sha256:8b0dc8f7cb7e6d31777127549096000aec141df162e86e1380198ca285f3c0d2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56032818 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3dc4945dcbf0d0c1254194f3d8bb6b886853fc2c10a7f0a8bab307a301e9b928`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:51 GMT
ADD file:4da7e6af0782b21849e5f6385de3589ac5b8d81d173721f9990d32da754df2cb in / 
# Wed, 12 Apr 2023 00:38:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2bef40925391866903279768b0ae1e8233ef6a1aae64509a922f4aa96fe42a6`  
		Last Modified: Wed, 12 Apr 2023 00:42:19 GMT  
		Size: 56.0 MB (56032818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; mips64le

```console
$ docker pull debian@sha256:fa84b0675f70f0665f2d3f4523eaf612c84f92633741d43947dcb826fddec017
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272023 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1c3fa623ac9031ff737d79a791a64eb03b9c7c30f1d5b3a283d692df505c728`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:26 GMT
ADD file:5d2c839055739fe92d64098c09607e7ec9123101d15f837c54f3688755af0c23 in / 
# Wed, 12 Apr 2023 00:09:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:80243e355b05bee53c6acc45ace2cde4f7495dafe9f3f8d7d299ed51d04928d2`  
		Last Modified: Wed, 12 Apr 2023 00:16:48 GMT  
		Size: 53.3 MB (53272023 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; ppc64le

```console
$ docker pull debian@sha256:186d0c8e5cd4abbdaa10fc9cc24757948bcd32666b9e9bd8acac15f800a5cb40
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b7d3fcd14d537d007a485ba25431b3dec0eeb5ed936a77507e18fe712e5d26ce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:58 GMT
ADD file:c3c2a10473ddaa3d8a30ca99b19ad3599d0e60d826c4e0315bdb7463352ebc09 in / 
# Wed, 12 Apr 2023 00:08:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d3ffdd4610a019889b845cc82b3d956ad85cca78ad4bff2d5e5522bd02c17e7`  
		Last Modified: Wed, 12 Apr 2023 00:12:37 GMT  
		Size: 58.9 MB (58926600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; s390x

```console
$ docker pull debian@sha256:659187525ed5d0088941a635408757a22f6133dc4b464290878d0e52d8c99e26
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e56d91c8c4483e11810a8b7f71d9f36d39e97dffd22d8856cab268ad64d7ff74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:25 GMT
ADD file:ab4e28724d04724a57e0f5bbe299d3361103f05ad1978dc3bfef243c1f9d6476 in / 
# Wed, 12 Apr 2023 00:00:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:042263756d7dc47bf98a243871107aa3a2b41acda91d104d8b4e148749a43dda`  
		Last Modified: Wed, 12 Apr 2023 00:05:03 GMT  
		Size: 53.3 MB (53286682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11-slim`

```console
$ docker pull debian@sha256:b50d0e48a5699f258ba721e65217c4b0928238613da8606e17f70fc22df05133
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11-slim` - linux; amd64

```console
$ docker pull debian@sha256:b29ef9073dba7475c4eb37e071b4e92e3409cb25988faa566db12942be2b741e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31418228 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0cf0e3d07c63f2652698583592be46f2708c49c36b0cfaacf2fa29e49821921`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:06 GMT
ADD file:11b1acca3f68b5c5787e292ff8dbdd114964a7272bf3519ab07710cbc01a0838 in / 
# Wed, 12 Apr 2023 00:20:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:26c5c85e47da3022f1bdb9a112103646c5c29517d757e95426f16e4bd9533405`  
		Last Modified: Wed, 12 Apr 2023 00:23:43 GMT  
		Size: 31.4 MB (31418228 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c86d41703f5caa3aef506c7d136d8894dd8a0cf065ed1b92da551e0cb7c96f45
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903418 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e75f1bda0f94f5a813d56aa484f9552b449d044a167d81010f54228e018671d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:53 GMT
ADD file:ca82c0d9094c1022a00b5f2157a02e1a9032aafc357a41c76c6b60bd74d25395 in / 
# Tue, 02 May 2023 22:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb6ee5d3f82142e70aca665cea92048b1a46ff1aa5c5be47a04c27a471470c07`  
		Last Modified: Tue, 02 May 2023 22:51:25 GMT  
		Size: 28.9 MB (28903418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:5da7f2fd32c314e61c2ac338ac24814be914bc1d83398129fd1337581985b875
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26579772 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:943430bb6515ff5a008c9e89091263e58125eb4b4d5a3ae4e7a207acf25b6d0b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:44 GMT
ADD file:e5f4777456ed4424053e9aa1f3d783f57da094c7a6c6d9d7a2fd315e00b5bbb0 in / 
# Tue, 11 Apr 2023 23:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99c2a28b4a43ce341eb611e82106b886315f30a250473617f2293828e10d8fff`  
		Last Modified: Wed, 12 Apr 2023 00:03:19 GMT  
		Size: 26.6 MB (26579772 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c115e796a6013213e61112da62f6da7f0a3390df894ade3434d0784f56b292cf
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30063826 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f37d1db70154bfdbe470c362201f9378eebd1c6476257f1be037f7d5e20f8453`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:49 GMT
ADD file:7b3c55926db26568f849247e80abdec3cfd6642929a40f0bbee95e4cb176051e in / 
# Wed, 12 Apr 2023 00:39:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ebc3dc5a2d72427c585c8cda7574a75d96e04b9a37572bd3af0bff905abefbb9`  
		Last Modified: Wed, 12 Apr 2023 00:42:35 GMT  
		Size: 30.1 MB (30063826 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; 386

```console
$ docker pull debian@sha256:dbe4b903e0e2c760345a7cdac3769f5cdb3b350e9a088a8c4d095675884539ef
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32398925 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1065af8f4c162dc6f9ff9b9cc37f6da1e82f1b8268d3d4e875e2d87c40a20013`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:01 GMT
ADD file:42fc1b4536cdd6823499b0c94d799e9bfbcb280b7df55d8d6c9f6defd61ecb6e in / 
# Wed, 12 Apr 2023 00:39:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2ee1f87789d52ef09851b4e5c9745fb8aceafa107b0d3452f9973f1abe65042`  
		Last Modified: Wed, 12 Apr 2023 00:42:45 GMT  
		Size: 32.4 MB (32398925 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; mips64le

```console
$ docker pull debian@sha256:75e76d275e2e147827d4071fab2ed95ab180af49c7bd167114bccbb543286350
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29639206 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5362b12a04662a3901f89c57644ab8de61fbf63d36fbec677be60d2c4164172e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:59 GMT
ADD file:2f558bb3cf4bc685f98fcdb12200961e3059267213f0caf7686523305967a663 in / 
# Wed, 12 Apr 2023 00:10:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8b0f62b0d33641c5348fd31abfd06463bdc4213a072d71479a0765756673777b`  
		Last Modified: Wed, 12 Apr 2023 00:17:28 GMT  
		Size: 29.6 MB (29639206 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:3f798f372becc8b10a2d7043c34770902ddf535dc07428f38ae55f3fefe1da4b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291995 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe0266851432b51dd4ae5d09c4e9221315e35cfaf0f1a59c599f1cc64b26b9f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:20 GMT
ADD file:63eb52aaff02c15bceabb87a78eb1b36389066ff4774cf8a754160ca7d509816 in / 
# Wed, 12 Apr 2023 00:08:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5b41d5ec640939cf684959234ad3b80909268a32bfd520a31c6720a91521c2fa`  
		Last Modified: Wed, 12 Apr 2023 00:13:13 GMT  
		Size: 35.3 MB (35291995 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; s390x

```console
$ docker pull debian@sha256:4a614398ac5b726cadb899ccc3bba0b3053a9d691e53c20e9023d77ac8b3e069
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29653156 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:22b6f9f20e8a8fe210b6f72b50c03bdb77b4396f8be929a0f1ac75dd9c115591`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:00 GMT
ADD file:b6463dba97ba9c0a29bacfafc4d67bc603ab57e80b75e23cd42d7ef4b0f8e6ae in / 
# Wed, 12 Apr 2023 00:01:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:97fe10bc7def58e7938e97e41eec4788ec7a45b6ef2cb1770cec02fa831fd19d`  
		Last Modified: Wed, 12 Apr 2023 00:05:18 GMT  
		Size: 29.7 MB (29653156 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.7`

```console
$ docker pull debian@sha256:444ee958e2710e4f705f64a4ef2c1aa7c0446f6b5fa37beac998de437da05758
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:11.7` - linux; arm variant v5

```console
$ docker pull debian@sha256:d170adce3d14567fad94acb11df1b8811da1a742668a2289778be573cb9a7ce8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb6d0a852fe4800d15c6c1b1fce5de90be94f044e74cabdfd5418db829529565`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.7-slim`

```console
$ docker pull debian@sha256:58f7eb49c8f947778d779fdc43676473a571823caf51615916c0d3be1eff3a0f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:11.7-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c86d41703f5caa3aef506c7d136d8894dd8a0cf065ed1b92da551e0cb7c96f45
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903418 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e75f1bda0f94f5a813d56aa484f9552b449d044a167d81010f54228e018671d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:53 GMT
ADD file:ca82c0d9094c1022a00b5f2157a02e1a9032aafc357a41c76c6b60bd74d25395 in / 
# Tue, 02 May 2023 22:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb6ee5d3f82142e70aca665cea92048b1a46ff1aa5c5be47a04c27a471470c07`  
		Last Modified: Tue, 02 May 2023 22:51:25 GMT  
		Size: 28.9 MB (28903418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm`

```console
$ docker pull debian@sha256:d71ed10e632d97169352a14d71e9bfdadd35cfcdefd7f0bc38add2632ba67e7e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm` - linux; amd64

```console
$ docker pull debian@sha256:1f0252afc278fad98c21a1dad40924141d1a5946479f5b81eaf6e632200f22c7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49292938 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:88d8bb6c9a75f0dfc5938525ddfa739244f87bfb60162c5fbf579efb6a3aa8ca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:35 GMT
ADD file:1cb6c660a2e3ea14b2a11bb8b80f53920c3da423a0ad7004391bfab4db4d0177 in / 
# Wed, 12 Apr 2023 00:19:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cc556c281183180fae02d57fa6fb60ed0a99d9040936278743dc4022bfb0c686`  
		Last Modified: Wed, 12 Apr 2023 00:22:48 GMT  
		Size: 49.3 MB (49292938 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v5

```console
$ docker pull debian@sha256:8042866b652f3b0e3411267905c04f20d66b94d4d394e9b39bb7c08b013a1785
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167199 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1d6d05fc851a07cf28729179b3c2567c2e90bb49c2e07851b73496c8a6b58c4b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:29 GMT
ADD file:1ecd266ebf23430d2ea2417b92ebee6029d940fbe99297183c4f811e24fdffb7 in / 
# Tue, 02 May 2023 22:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cbca433c216350213a8b219e118cb55f422acfc53f49c76c44e044b42a5c0aa6`  
		Last Modified: Tue, 02 May 2023 22:50:27 GMT  
		Size: 47.2 MB (47167199 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v7

```console
$ docker pull debian@sha256:8adb140b5e38255156a636cbe13945634e78a772c45afc0fe92a653f5358408e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45883114 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9408c222e93da7bf5ce9754d6e41a7c9a8035a143b3d49c509183084573953d4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:10 GMT
ADD file:aa1c08333544c05fc5317d775ee99af7479c6fa5f35b74652f6126a7a5099958 in / 
# Tue, 11 Apr 2023 23:59:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:81b1dd69f2a38ad14b0919ab9c395aafeb137158539ab6531ac5743690a0ebb7`  
		Last Modified: Wed, 12 Apr 2023 00:02:26 GMT  
		Size: 45.9 MB (45883114 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:38079537fb13e76a7f0ff20c1c0721fc87500d32699839137825ed2e86d09ee4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49345365 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca2373130338fd85b1cc0c8b978244029f273b6503dff12fd74a7227375ab5d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:27 GMT
ADD file:efed18ef2a003bff7e64e7049455216e51876402083581032c35085328e416f6 in / 
# Wed, 12 Apr 2023 00:39:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0f987ce91d920ae0a705ed6c9824f1bbbfa282defb8986614030eb8e2bc0b4e`  
		Last Modified: Wed, 12 Apr 2023 00:41:45 GMT  
		Size: 49.3 MB (49345365 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; 386

```console
$ docker pull debian@sha256:51dc297655832ad72d68807037e201b3d4b6a31fa806e16b9cac6ac81cdc5ddd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50317977 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:86fe0ec694469a483f7475e239b58bae572b304306d8f9466ebdc514615c31b4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:30 GMT
ADD file:7718865253a3489583d40f8c7a7beede0c20cebbab68bb3e97ad2e84082afcb7 in / 
# Wed, 12 Apr 2023 00:38:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a4ce9e81f634822e5166ef3eef875a4140795ac18afe0391433d72c7865bf67b`  
		Last Modified: Wed, 12 Apr 2023 00:41:43 GMT  
		Size: 50.3 MB (50317977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; mips64le

```console
$ docker pull debian@sha256:3b448a3a9d2dc83e824fd76a532d99713a027af201ac3e5873c19c70720c5d65
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49299322 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84cab8fbba0b6e68f97acf0de16053981464b53b3bd5f805d26487042fa7741e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:16 GMT
ADD file:6a525220124da7146a3c356cc57678f3b8d7c6e9299febaaebd5b5a005f1325c in / 
# Wed, 12 Apr 2023 00:08:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b6bdb9410f81554043d19e5ae83249eaa10ffe038db54932946c7d9eaccc8b96`  
		Last Modified: Wed, 12 Apr 2023 00:15:35 GMT  
		Size: 49.3 MB (49299322 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; ppc64le

```console
$ docker pull debian@sha256:550c64aecce2dd0a7f689e0155a05b124d86bb1beab3253f2fc6a5a23d553de6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53299984 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05c5981722915b2cd9d2f7771a580113b28a1f4cbfdac67ca4cfacfc5b3801ab`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:22 GMT
ADD file:b747fc06f7ee1d3e19fc9cf3aa83e6ab4caa056a72d230caf616e67849bfe112 in / 
# Wed, 12 Apr 2023 00:07:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8314f03b76ec928732612641a4f8420e3c4c25719b48503230bf6c03979aeb2e`  
		Last Modified: Wed, 12 Apr 2023 00:11:50 GMT  
		Size: 53.3 MB (53299984 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; s390x

```console
$ docker pull debian@sha256:e018c6280c14cb8dccea43c672d76ebe8680dd6fd530d14803d1783644e64f7a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47670407 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3aeb38a92eed0139989ece5d3d6719cdd6cb8d81cda2f7e3275896e173d34646`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:19 GMT
ADD file:5d2b35b45a0f23e2f2e93f9c311eb34a92e4591ea27979236d801e512d44cb85 in / 
# Tue, 11 Apr 2023 23:59:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8a09cd2374da8355612e7d4c91d55107a3f089816ae52b2c91b9e08e6a895ab1`  
		Last Modified: Wed, 12 Apr 2023 00:04:39 GMT  
		Size: 47.7 MB (47670407 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20230502`

```console
$ docker pull debian@sha256:96f761855da8e3a813705410cb60edc26216a36336303d31f4df4647380dee2e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:bookworm-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:8042866b652f3b0e3411267905c04f20d66b94d4d394e9b39bb7c08b013a1785
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167199 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1d6d05fc851a07cf28729179b3c2567c2e90bb49c2e07851b73496c8a6b58c4b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:29 GMT
ADD file:1ecd266ebf23430d2ea2417b92ebee6029d940fbe99297183c4f811e24fdffb7 in / 
# Tue, 02 May 2023 22:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cbca433c216350213a8b219e118cb55f422acfc53f49c76c44e044b42a5c0aa6`  
		Last Modified: Tue, 02 May 2023 22:50:27 GMT  
		Size: 47.2 MB (47167199 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20230502-slim`

```console
$ docker pull debian@sha256:73d5d8cea8fba0fa2eb89ee08e51e068998dd5bf903ee46931a9dff6158870b0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:bookworm-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:16a3fae448857450519a1b9fae650cf3320c16e45738283bf81a55e734fcb9c0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954741 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:916a01d8c8b4998eb5cc82443db258e2814da2709b37db38155bb1a14939f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:38 GMT
ADD file:b7429171de826e2164d576aacc951f5fb721c3bf9e09efb3e1ce1bd84b3cebe3 in / 
# Tue, 02 May 2023 22:48:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:22864ca607870f2de6105b6ffe5d88701d9af71b3e73ef93223e0c5e078a07fe`  
		Last Modified: Tue, 02 May 2023 22:50:45 GMT  
		Size: 27.0 MB (26954741 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-backports`

```console
$ docker pull debian@sha256:4baaada62295c0836a3afd7720bdb890a528833056da420b9b3c6ba43248032a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-backports` - linux; amd64

```console
$ docker pull debian@sha256:2d21ba1f0e9f55e60aec0e4c391bc1ca90d5ee1801b16b30847e8e9d42b8fe19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49293163 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3b42a3b8f2261d6be25867ff1df1964c392f850cd4a70159ba2b885ec4bc3598`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:35 GMT
ADD file:1cb6c660a2e3ea14b2a11bb8b80f53920c3da423a0ad7004391bfab4db4d0177 in / 
# Wed, 12 Apr 2023 00:19:36 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:19:39 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:cc556c281183180fae02d57fa6fb60ed0a99d9040936278743dc4022bfb0c686`  
		Last Modified: Wed, 12 Apr 2023 00:22:48 GMT  
		Size: 49.3 MB (49292938 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b912467d263f30e17a171b9b45ce9f875a6d1a5b431c9f031644052c2145f27a`  
		Last Modified: Wed, 12 Apr 2023 00:22:56 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:63c0590c4fca207832f2289398b9a7b16607a789126ed57f63d51392efe74d70
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167424 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c33cad8fdf47145896d1114f29b974d9676db8fa76d046cb74ccc8fca6bd04a5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:29 GMT
ADD file:1ecd266ebf23430d2ea2417b92ebee6029d940fbe99297183c4f811e24fdffb7 in / 
# Tue, 02 May 2023 22:48:32 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:48:34 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:cbca433c216350213a8b219e118cb55f422acfc53f49c76c44e044b42a5c0aa6`  
		Last Modified: Tue, 02 May 2023 22:50:27 GMT  
		Size: 47.2 MB (47167199 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f7f12bd05394d7cdb0240b97ff1f8b37890ba3605c9be05a5f0137391f033b4a`  
		Last Modified: Tue, 02 May 2023 22:50:35 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:4f3ae1bbc920a0e25aa1be4afdcec99f8267725fe48d34868307592bb1800e5a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45883342 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ce3550faf1d683b8578f68270eb38d7c0c07f7048548f01d3a047686030a903a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:10 GMT
ADD file:aa1c08333544c05fc5317d775ee99af7479c6fa5f35b74652f6126a7a5099958 in / 
# Tue, 11 Apr 2023 23:59:11 GMT
CMD ["bash"]
# Tue, 11 Apr 2023 23:59:16 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:81b1dd69f2a38ad14b0919ab9c395aafeb137158539ab6531ac5743690a0ebb7`  
		Last Modified: Wed, 12 Apr 2023 00:02:26 GMT  
		Size: 45.9 MB (45883114 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:91279df2be4fc54fa107a1489c352c4e6820235721f6d2fce9770dcd788dd23b`  
		Last Modified: Wed, 12 Apr 2023 00:02:34 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2c2e5b6d8c2df06479ad95ba623cb29ec23bd3b9adadc01ee66f01bd28305610
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49345592 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6814ea763798190eca3987817a70a7bc13eb57913ca423f012a79a5fc8b4e4fb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:27 GMT
ADD file:efed18ef2a003bff7e64e7049455216e51876402083581032c35085328e416f6 in / 
# Wed, 12 Apr 2023 00:39:28 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:39:32 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b0f987ce91d920ae0a705ed6c9824f1bbbfa282defb8986614030eb8e2bc0b4e`  
		Last Modified: Wed, 12 Apr 2023 00:41:45 GMT  
		Size: 49.3 MB (49345365 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:42b263e24395ff79fba3574dbdcda1c6a7449a8f599b233b13d651751ee6c469`  
		Last Modified: Wed, 12 Apr 2023 00:41:53 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; 386

```console
$ docker pull debian@sha256:1b69101031c40b23221892a619f241319692e3bba4d3557778719ebbd302d439
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50318201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f543f79f68c9d270fa7d7810125f7c293424c6c72a1cc2f78c4f201a5d39408c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:30 GMT
ADD file:7718865253a3489583d40f8c7a7beede0c20cebbab68bb3e97ad2e84082afcb7 in / 
# Wed, 12 Apr 2023 00:38:30 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:38:34 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a4ce9e81f634822e5166ef3eef875a4140795ac18afe0391433d72c7865bf67b`  
		Last Modified: Wed, 12 Apr 2023 00:41:43 GMT  
		Size: 50.3 MB (50317977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3a2ebfdfe5f5fe09aaad696aacc27e759afc03f711b3867a755a7e8d60976b31`  
		Last Modified: Wed, 12 Apr 2023 00:41:51 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; mips64le

```console
$ docker pull debian@sha256:51d60254ade2a3f26c148960aa9cc4e84d93e7057c95f7c7d662215de074a040
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49299550 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c85c5fc7fe8efbc02051c3a45ded9ac0c75ed645710021436025b7731d2a5f78`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:16 GMT
ADD file:6a525220124da7146a3c356cc57678f3b8d7c6e9299febaaebd5b5a005f1325c in / 
# Wed, 12 Apr 2023 00:08:21 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:08:34 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b6bdb9410f81554043d19e5ae83249eaa10ffe038db54932946c7d9eaccc8b96`  
		Last Modified: Wed, 12 Apr 2023 00:15:35 GMT  
		Size: 49.3 MB (49299322 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:95a1043ecd820d689db7a68e952536be338fede5f152559185ae52b314992d71`  
		Last Modified: Wed, 12 Apr 2023 00:15:45 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:5e56ccae3b1e7e733e6cecea378c4d0e188de09325248ac762fc060a80ab6c7b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53300212 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:746daca5182cd8102027b20e4f425f2b751ab80c5c23090d7f9da458a5e8c659`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:22 GMT
ADD file:b747fc06f7ee1d3e19fc9cf3aa83e6ab4caa056a72d230caf616e67849bfe112 in / 
# Wed, 12 Apr 2023 00:07:25 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:07:28 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8314f03b76ec928732612641a4f8420e3c4c25719b48503230bf6c03979aeb2e`  
		Last Modified: Wed, 12 Apr 2023 00:11:50 GMT  
		Size: 53.3 MB (53299984 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9785fbbebdcc5c8fd5e34ff7ad1f4a56c3e4defce0fd15af015a6930e52109d8`  
		Last Modified: Wed, 12 Apr 2023 00:12:00 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; s390x

```console
$ docker pull debian@sha256:e86cee0f7cd4dced279fe1b9e2e5af3a9da36b381c0d88e68a4ae2aad33db27f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47670634 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15b1d9f76008c7acea5b310397ea20bff9b9157160f0f1796bab476ab62ebcfe`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:19 GMT
ADD file:5d2b35b45a0f23e2f2e93f9c311eb34a92e4591ea27979236d801e512d44cb85 in / 
# Tue, 11 Apr 2023 23:59:31 GMT
CMD ["bash"]
# Tue, 11 Apr 2023 23:59:39 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8a09cd2374da8355612e7d4c91d55107a3f089816ae52b2c91b9e08e6a895ab1`  
		Last Modified: Wed, 12 Apr 2023 00:04:39 GMT  
		Size: 47.7 MB (47670407 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e0fccd5749b40f4eca9cb96b5e4e343d8ee1bb6f2f86458a77183862f5bb0b59`  
		Last Modified: Wed, 12 Apr 2023 00:04:44 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-slim`

```console
$ docker pull debian@sha256:596317494b6f219bf0be0e108bede4dd55966435ff879dbcf1e735a1cc98c630
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-slim` - linux; amd64

```console
$ docker pull debian@sha256:07c6cb2ae86479dcc1942a89b0a1f4049b6e9415f7de327ff641aed58b8e3100
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29082376 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb5c02e182079d4c798048f9526aadc4cbd48b5e364b0730e58e5554376dc4e7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:46 GMT
ADD file:382dcc0dd39da90030a2de09d6847df082978a1a8ba5f3b2c7f678a1a5e917f3 in / 
# Wed, 12 Apr 2023 00:19:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:143db7d084219658b1d5dc81071b28069505f56c5edd84ce9783d7d1b0689f38`  
		Last Modified: Wed, 12 Apr 2023 00:23:05 GMT  
		Size: 29.1 MB (29082376 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:16a3fae448857450519a1b9fae650cf3320c16e45738283bf81a55e734fcb9c0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954741 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:916a01d8c8b4998eb5cc82443db258e2814da2709b37db38155bb1a14939f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:38 GMT
ADD file:b7429171de826e2164d576aacc951f5fb721c3bf9e09efb3e1ce1bd84b3cebe3 in / 
# Tue, 02 May 2023 22:48:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:22864ca607870f2de6105b6ffe5d88701d9af71b3e73ef93223e0c5e078a07fe`  
		Last Modified: Tue, 02 May 2023 22:50:45 GMT  
		Size: 27.0 MB (26954741 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d28abb2e10eb9e5cf89c3ff8905766036f10a8b6d43845819cce3ad2d83c183b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.7 MB (25672981 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f55f39d4b40317487800fde84c1b8f7803b61cc7722b116a64164ded09ca7d19`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:23 GMT
ADD file:36d02b31daca0f075647db22cc533cb3cc40954d9f5dbb85dca7694dd2112216 in / 
# Tue, 11 Apr 2023 23:59:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e02fe4f8cb8b942d6a16a9477520427d663c18b1385de1e0c62edf5c17037a2`  
		Last Modified: Wed, 12 Apr 2023 00:02:43 GMT  
		Size: 25.7 MB (25672981 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6eb828201cfd94715a81862167141d93c24ddb51f95cc2b0948fd97408b6d918
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29121294 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aba956301a8a8180ddad041d83301953ab05458fff2c5df92c673315e975f029`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:36 GMT
ADD file:f9329dbe55130ad6519986bd39af4c990be00854398667ae7e57b1e77112fa2c in / 
# Wed, 12 Apr 2023 00:39:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2a4e4be009fb4bf3377ee02176cde1465e9aa86b2ca16b6f906d04d62b5e600d`  
		Last Modified: Wed, 12 Apr 2023 00:42:01 GMT  
		Size: 29.1 MB (29121294 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; 386

```console
$ docker pull debian@sha256:de067097a36d8321b06f49fc9dcf8038ffcda135db513192911359e1d63f2b28
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30102772 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c1170cbaaf7c703038d17aaa796ba7085f0eb266d77df4e515703c1da6a82183`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:41 GMT
ADD file:95913b5ae0f08fceb261e02adcee66071562c6366fbf309f33e6b54479abe59a in / 
# Wed, 12 Apr 2023 00:38:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:75d8cbfc608962b83bb059f2130cdb4d8502e95801d291dce6ae1695dd1608bd`  
		Last Modified: Wed, 12 Apr 2023 00:42:02 GMT  
		Size: 30.1 MB (30102772 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; mips64le

```console
$ docker pull debian@sha256:4a8a4177a9d13147be4ef58400cb48a1e3d2ad79538ca75873e64fd0b318137d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29089380 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d00d786762ee9f74884e1ed34614580c2800a10a692b2134a57833b25611045`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:53 GMT
ADD file:be64b09b028cbd539f9299938f8d862ba2c27c1acc744a9db750568be6896924 in / 
# Wed, 12 Apr 2023 00:08:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:02c3ad3440acece54fd8091326a811519a44d6d88195c71e922cc74d06344637`  
		Last Modified: Wed, 12 Apr 2023 00:16:09 GMT  
		Size: 29.1 MB (29089380 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:5dee80ff08182e57edc2537f6b1aa202c1b2ee4012a042fef61fddebc4370518
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33085434 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33618c1bf876d42b7137362e476a67726830c0b3f169a30b3c4135b37e15847f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:40 GMT
ADD file:b101744d31d68668d983c19f88070c5b5a28a4c6f620b4243746e7db9f7a6c30 in / 
# Wed, 12 Apr 2023 00:07:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c617040833a504596d01bbe11c1a4cdd4c5becd5e23312551eae0c10c867944`  
		Last Modified: Wed, 12 Apr 2023 00:12:15 GMT  
		Size: 33.1 MB (33085434 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; s390x

```console
$ docker pull debian@sha256:154bb308723d714791ee704587c9171f67a0b331bb0f9a0b8c1f1da710e73f54
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27458285 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:99b2202e9969f5833441187ac895b414a9e12328bd55302531cef8703d1af3ff`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:53 GMT
ADD file:abb337e814ca7c9a13a8fa76e2afd553a60cafcd7ed5a02375becf2b43cb6ce4 in / 
# Wed, 12 Apr 2023 00:00:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e93565c33c014dab0bd8cea41496513b60191ecff4eb878dd306c3a04cec35e`  
		Last Modified: Wed, 12 Apr 2023 00:04:52 GMT  
		Size: 27.5 MB (27458285 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye`

```console
$ docker pull debian@sha256:251456f07b3fcc936c6ebf5218e6a8ffae63e4a86d23f16fb1b023f160eca7e6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye` - linux; amd64

```console
$ docker pull debian@sha256:4effa16ae79ab36347d55ebb6bb4aad411077b72ad735054e684b3eeba373a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052736 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca1a8403096efb7da1512b0ae008b16f2dae553deed435163b137447b90fa8b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:55 GMT
ADD file:f2d012660f882f319a5878a3f9ce285f488b8f90fad49ad238541cf72089e035 in / 
# Wed, 12 Apr 2023 00:19:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0248cf3e63c73d0e496a67807d056ca41d5e968b61087e8eca2cf4b9b4d7b99`  
		Last Modified: Wed, 12 Apr 2023 00:23:20 GMT  
		Size: 55.1 MB (55052736 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v5

```console
$ docker pull debian@sha256:d170adce3d14567fad94acb11df1b8811da1a742668a2289778be573cb9a7ce8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb6d0a852fe4800d15c6c1b1fce5de90be94f044e74cabdfd5418db829529565`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab0ce510343d33415fc4eaea0102da2d24cb720b185aaee0b411c48a07abb0de
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50216882 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b29663cecd1d9b95d625512cc33ad2c8d970653700f863175d7fdf5b022fc129`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:35 GMT
ADD file:75c57df33c95251a80549b7949853df282a42bc4e5f19176764907a54b74caa9 in / 
# Tue, 11 Apr 2023 23:59:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4feff9b88735325634445419eaaec2ed47500b63027cfd2de34b8bc6d55ee85c`  
		Last Modified: Wed, 12 Apr 2023 00:02:57 GMT  
		Size: 50.2 MB (50216882 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c0f6422141bb031238126fbd535c0df24db4252a2d9876216a94082df74c8fda
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705529 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6d9c9c3e100a13622816226e63a3faa83da315ae01216b1ac02c4ba5cc91683`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:42 GMT
ADD file:dd3d4e5fe819950e7d01b44a29dbd790438cd722ba76198910e2597448ab0c6f in / 
# Wed, 12 Apr 2023 00:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dc80b8cdbfd36cb20231d807a50d704945d1df4da8f6e23197ccfcb629970491`  
		Last Modified: Wed, 12 Apr 2023 00:42:14 GMT  
		Size: 53.7 MB (53705529 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; 386

```console
$ docker pull debian@sha256:8b0dc8f7cb7e6d31777127549096000aec141df162e86e1380198ca285f3c0d2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56032818 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3dc4945dcbf0d0c1254194f3d8bb6b886853fc2c10a7f0a8bab307a301e9b928`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:51 GMT
ADD file:4da7e6af0782b21849e5f6385de3589ac5b8d81d173721f9990d32da754df2cb in / 
# Wed, 12 Apr 2023 00:38:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2bef40925391866903279768b0ae1e8233ef6a1aae64509a922f4aa96fe42a6`  
		Last Modified: Wed, 12 Apr 2023 00:42:19 GMT  
		Size: 56.0 MB (56032818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; mips64le

```console
$ docker pull debian@sha256:fa84b0675f70f0665f2d3f4523eaf612c84f92633741d43947dcb826fddec017
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272023 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1c3fa623ac9031ff737d79a791a64eb03b9c7c30f1d5b3a283d692df505c728`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:26 GMT
ADD file:5d2c839055739fe92d64098c09607e7ec9123101d15f837c54f3688755af0c23 in / 
# Wed, 12 Apr 2023 00:09:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:80243e355b05bee53c6acc45ace2cde4f7495dafe9f3f8d7d299ed51d04928d2`  
		Last Modified: Wed, 12 Apr 2023 00:16:48 GMT  
		Size: 53.3 MB (53272023 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; ppc64le

```console
$ docker pull debian@sha256:186d0c8e5cd4abbdaa10fc9cc24757948bcd32666b9e9bd8acac15f800a5cb40
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b7d3fcd14d537d007a485ba25431b3dec0eeb5ed936a77507e18fe712e5d26ce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:58 GMT
ADD file:c3c2a10473ddaa3d8a30ca99b19ad3599d0e60d826c4e0315bdb7463352ebc09 in / 
# Wed, 12 Apr 2023 00:08:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d3ffdd4610a019889b845cc82b3d956ad85cca78ad4bff2d5e5522bd02c17e7`  
		Last Modified: Wed, 12 Apr 2023 00:12:37 GMT  
		Size: 58.9 MB (58926600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; s390x

```console
$ docker pull debian@sha256:659187525ed5d0088941a635408757a22f6133dc4b464290878d0e52d8c99e26
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e56d91c8c4483e11810a8b7f71d9f36d39e97dffd22d8856cab268ad64d7ff74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:25 GMT
ADD file:ab4e28724d04724a57e0f5bbe299d3361103f05ad1978dc3bfef243c1f9d6476 in / 
# Wed, 12 Apr 2023 00:00:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:042263756d7dc47bf98a243871107aa3a2b41acda91d104d8b4e148749a43dda`  
		Last Modified: Wed, 12 Apr 2023 00:05:03 GMT  
		Size: 53.3 MB (53286682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20230502`

```console
$ docker pull debian@sha256:444ee958e2710e4f705f64a4ef2c1aa7c0446f6b5fa37beac998de437da05758
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:bullseye-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:d170adce3d14567fad94acb11df1b8811da1a742668a2289778be573cb9a7ce8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb6d0a852fe4800d15c6c1b1fce5de90be94f044e74cabdfd5418db829529565`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20230502-slim`

```console
$ docker pull debian@sha256:58f7eb49c8f947778d779fdc43676473a571823caf51615916c0d3be1eff3a0f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:bullseye-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c86d41703f5caa3aef506c7d136d8894dd8a0cf065ed1b92da551e0cb7c96f45
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903418 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e75f1bda0f94f5a813d56aa484f9552b449d044a167d81010f54228e018671d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:53 GMT
ADD file:ca82c0d9094c1022a00b5f2157a02e1a9032aafc357a41c76c6b60bd74d25395 in / 
# Tue, 02 May 2023 22:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb6ee5d3f82142e70aca665cea92048b1a46ff1aa5c5be47a04c27a471470c07`  
		Last Modified: Tue, 02 May 2023 22:51:25 GMT  
		Size: 28.9 MB (28903418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-backports`

```console
$ docker pull debian@sha256:23cf38ea578aae80305b3d9b605c26354ca64f7009a5e7f9d54d4f17a6ce67e5
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-backports` - linux; amd64

```console
$ docker pull debian@sha256:3950fa6c3d923f7929c4e08231d72abaee40420cc7798d78c823a161c2cbf0f5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052961 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d72e890b198a7f9ae8a9907a7f5950cf254bd7c658e9980e88bf9c69c5f74800`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:55 GMT
ADD file:f2d012660f882f319a5878a3f9ce285f488b8f90fad49ad238541cf72089e035 in / 
# Wed, 12 Apr 2023 00:19:56 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:19:59 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b0248cf3e63c73d0e496a67807d056ca41d5e968b61087e8eca2cf4b9b4d7b99`  
		Last Modified: Wed, 12 Apr 2023 00:23:20 GMT  
		Size: 55.1 MB (55052736 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5cd102085f0f28e20f71187d0f003b7d02edd9a13fdee8001aad360df3ca94df`  
		Last Modified: Wed, 12 Apr 2023 00:23:34 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:eb6d3e1e4d0e1f835be420b48a537a82f367827db57a89698f834581fae9c363
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546757 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9f13fee9ad9733a747fc6bad922bf7125275767d91b5fcbd65057e24ca7357e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:48:48 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1827544a6fedf0f8c24aa3074567b30472563d25dfc99582e13dc3bfa2bfa793`  
		Last Modified: Tue, 02 May 2023 22:51:15 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:63e41b11b53d39265994fb5b1b81c43ed0d49320a92f27d5a47ef1ecf4306e0f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50217110 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:31a3d84d0562e2dbcdf040560adbc51181885f294e5f6e9a1bff941f68a92814`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:35 GMT
ADD file:75c57df33c95251a80549b7949853df282a42bc4e5f19176764907a54b74caa9 in / 
# Tue, 11 Apr 2023 23:59:36 GMT
CMD ["bash"]
# Tue, 11 Apr 2023 23:59:38 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:4feff9b88735325634445419eaaec2ed47500b63027cfd2de34b8bc6d55ee85c`  
		Last Modified: Wed, 12 Apr 2023 00:02:57 GMT  
		Size: 50.2 MB (50216882 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:809a1996a22b675eefd3fce9f8b70de051d1433a66b77748d11c2d18115e8a4d`  
		Last Modified: Wed, 12 Apr 2023 00:03:10 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ef0569c515bdcfeb3fb0b31a7e6ef72ef0d331286431cbe658a21daec2e2d671
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705753 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a68f5a36ebe060459bae2685b263bdc37cd7e3000bf600b658fe3985c7c45591`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:42 GMT
ADD file:dd3d4e5fe819950e7d01b44a29dbd790438cd722ba76198910e2597448ab0c6f in / 
# Wed, 12 Apr 2023 00:39:43 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:39:45 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:dc80b8cdbfd36cb20231d807a50d704945d1df4da8f6e23197ccfcb629970491`  
		Last Modified: Wed, 12 Apr 2023 00:42:14 GMT  
		Size: 53.7 MB (53705529 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f50f79822c1334542a2c647bbf2e495cf662e6e25e976764bc9a86e51b6c3411`  
		Last Modified: Wed, 12 Apr 2023 00:42:27 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; 386

```console
$ docker pull debian@sha256:fdcfa7ec57f1bbad816752c0acbd7b49f0163fe243db03def156128a17d9e7c3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56033044 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:93fc77a4ac31486b1355dd091e5132ed1f8ceb5f6c28964699dcdcc5215e2b5a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:51 GMT
ADD file:4da7e6af0782b21849e5f6385de3589ac5b8d81d173721f9990d32da754df2cb in / 
# Wed, 12 Apr 2023 00:38:51 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:38:54 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b2bef40925391866903279768b0ae1e8233ef6a1aae64509a922f4aa96fe42a6`  
		Last Modified: Wed, 12 Apr 2023 00:42:19 GMT  
		Size: 56.0 MB (56032818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:163cbf4c07d8e1ed01c72bdfbbc181381fa7346b0c7d7873d513877905cc0dad`  
		Last Modified: Wed, 12 Apr 2023 00:42:33 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; mips64le

```console
$ docker pull debian@sha256:6bba04ef78172e50a76a1adeae6948d95da13d2b10947194a1e133be255246e6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272249 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:82192e5ae21def8646a916631d34811734eb115466795c16169066406cdb4574`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:26 GMT
ADD file:5d2c839055739fe92d64098c09607e7ec9123101d15f837c54f3688755af0c23 in / 
# Wed, 12 Apr 2023 00:09:31 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:09:40 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:80243e355b05bee53c6acc45ace2cde4f7495dafe9f3f8d7d299ed51d04928d2`  
		Last Modified: Wed, 12 Apr 2023 00:16:48 GMT  
		Size: 53.3 MB (53272023 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:827c557479b9edd028d1b34e5256f9070b200d4fce958cf9e84613c83dea79c0`  
		Last Modified: Wed, 12 Apr 2023 00:17:04 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:0dcf75aa727af3bf39e300f0e936e662682d6243c4e67f3701c9d00501bd3654
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926827 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3b5edd0906c02fc00f201665e4a5b81026c9c7dec72fca0b3a17050d22791895`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:58 GMT
ADD file:c3c2a10473ddaa3d8a30ca99b19ad3599d0e60d826c4e0315bdb7463352ebc09 in / 
# Wed, 12 Apr 2023 00:08:02 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:08:07 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2d3ffdd4610a019889b845cc82b3d956ad85cca78ad4bff2d5e5522bd02c17e7`  
		Last Modified: Wed, 12 Apr 2023 00:12:37 GMT  
		Size: 58.9 MB (58926600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f78baafc7df2db64930490d209b66bee4bd2f7845f03591a93f95be17fe677f1`  
		Last Modified: Wed, 12 Apr 2023 00:12:53 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; s390x

```console
$ docker pull debian@sha256:0e4c0a91f063aa664e5393b1c602a2121fef4fdc1b375e6bac354961019eb77c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286910 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d55d666fcb5965f2338b94392975cb40f61b7a40d568169be8f119d160f5bca9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:25 GMT
ADD file:ab4e28724d04724a57e0f5bbe299d3361103f05ad1978dc3bfef243c1f9d6476 in / 
# Wed, 12 Apr 2023 00:00:38 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:00:48 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:042263756d7dc47bf98a243871107aa3a2b41acda91d104d8b4e148749a43dda`  
		Last Modified: Wed, 12 Apr 2023 00:05:03 GMT  
		Size: 53.3 MB (53286682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5a5f8ea362068477a4e1f3dc8e71b20734f89fdc76fa216834c78d37b4ff337c`  
		Last Modified: Wed, 12 Apr 2023 00:05:11 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-slim`

```console
$ docker pull debian@sha256:b50d0e48a5699f258ba721e65217c4b0928238613da8606e17f70fc22df05133
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-slim` - linux; amd64

```console
$ docker pull debian@sha256:b29ef9073dba7475c4eb37e071b4e92e3409cb25988faa566db12942be2b741e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31418228 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0cf0e3d07c63f2652698583592be46f2708c49c36b0cfaacf2fa29e49821921`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:06 GMT
ADD file:11b1acca3f68b5c5787e292ff8dbdd114964a7272bf3519ab07710cbc01a0838 in / 
# Wed, 12 Apr 2023 00:20:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:26c5c85e47da3022f1bdb9a112103646c5c29517d757e95426f16e4bd9533405`  
		Last Modified: Wed, 12 Apr 2023 00:23:43 GMT  
		Size: 31.4 MB (31418228 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:c86d41703f5caa3aef506c7d136d8894dd8a0cf065ed1b92da551e0cb7c96f45
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903418 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e75f1bda0f94f5a813d56aa484f9552b449d044a167d81010f54228e018671d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:53 GMT
ADD file:ca82c0d9094c1022a00b5f2157a02e1a9032aafc357a41c76c6b60bd74d25395 in / 
# Tue, 02 May 2023 22:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb6ee5d3f82142e70aca665cea92048b1a46ff1aa5c5be47a04c27a471470c07`  
		Last Modified: Tue, 02 May 2023 22:51:25 GMT  
		Size: 28.9 MB (28903418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:5da7f2fd32c314e61c2ac338ac24814be914bc1d83398129fd1337581985b875
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26579772 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:943430bb6515ff5a008c9e89091263e58125eb4b4d5a3ae4e7a207acf25b6d0b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:44 GMT
ADD file:e5f4777456ed4424053e9aa1f3d783f57da094c7a6c6d9d7a2fd315e00b5bbb0 in / 
# Tue, 11 Apr 2023 23:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99c2a28b4a43ce341eb611e82106b886315f30a250473617f2293828e10d8fff`  
		Last Modified: Wed, 12 Apr 2023 00:03:19 GMT  
		Size: 26.6 MB (26579772 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c115e796a6013213e61112da62f6da7f0a3390df894ade3434d0784f56b292cf
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30063826 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f37d1db70154bfdbe470c362201f9378eebd1c6476257f1be037f7d5e20f8453`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:49 GMT
ADD file:7b3c55926db26568f849247e80abdec3cfd6642929a40f0bbee95e4cb176051e in / 
# Wed, 12 Apr 2023 00:39:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ebc3dc5a2d72427c585c8cda7574a75d96e04b9a37572bd3af0bff905abefbb9`  
		Last Modified: Wed, 12 Apr 2023 00:42:35 GMT  
		Size: 30.1 MB (30063826 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; 386

```console
$ docker pull debian@sha256:dbe4b903e0e2c760345a7cdac3769f5cdb3b350e9a088a8c4d095675884539ef
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32398925 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1065af8f4c162dc6f9ff9b9cc37f6da1e82f1b8268d3d4e875e2d87c40a20013`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:01 GMT
ADD file:42fc1b4536cdd6823499b0c94d799e9bfbcb280b7df55d8d6c9f6defd61ecb6e in / 
# Wed, 12 Apr 2023 00:39:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2ee1f87789d52ef09851b4e5c9745fb8aceafa107b0d3452f9973f1abe65042`  
		Last Modified: Wed, 12 Apr 2023 00:42:45 GMT  
		Size: 32.4 MB (32398925 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; mips64le

```console
$ docker pull debian@sha256:75e76d275e2e147827d4071fab2ed95ab180af49c7bd167114bccbb543286350
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29639206 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5362b12a04662a3901f89c57644ab8de61fbf63d36fbec677be60d2c4164172e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:59 GMT
ADD file:2f558bb3cf4bc685f98fcdb12200961e3059267213f0caf7686523305967a663 in / 
# Wed, 12 Apr 2023 00:10:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8b0f62b0d33641c5348fd31abfd06463bdc4213a072d71479a0765756673777b`  
		Last Modified: Wed, 12 Apr 2023 00:17:28 GMT  
		Size: 29.6 MB (29639206 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:3f798f372becc8b10a2d7043c34770902ddf535dc07428f38ae55f3fefe1da4b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291995 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe0266851432b51dd4ae5d09c4e9221315e35cfaf0f1a59c599f1cc64b26b9f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:20 GMT
ADD file:63eb52aaff02c15bceabb87a78eb1b36389066ff4774cf8a754160ca7d509816 in / 
# Wed, 12 Apr 2023 00:08:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5b41d5ec640939cf684959234ad3b80909268a32bfd520a31c6720a91521c2fa`  
		Last Modified: Wed, 12 Apr 2023 00:13:13 GMT  
		Size: 35.3 MB (35291995 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; s390x

```console
$ docker pull debian@sha256:4a614398ac5b726cadb899ccc3bba0b3053a9d691e53c20e9023d77ac8b3e069
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29653156 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:22b6f9f20e8a8fe210b6f72b50c03bdb77b4396f8be929a0f1ac75dd9c115591`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:00 GMT
ADD file:b6463dba97ba9c0a29bacfafc4d67bc603ab57e80b75e23cd42d7ef4b0f8e6ae in / 
# Wed, 12 Apr 2023 00:01:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:97fe10bc7def58e7938e97e41eec4788ec7a45b6ef2cb1770cec02fa831fd19d`  
		Last Modified: Wed, 12 Apr 2023 00:05:18 GMT  
		Size: 29.7 MB (29653156 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster`

```console
$ docker pull debian@sha256:173f0b19d2ac5ee17194dbe9a888a4a617dbf905ee666c9e780afad55eaf1d8c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster` - linux; amd64

```console
$ docker pull debian@sha256:2803ab27dfe4f2e23b58d351e80e8d433942b6fb830225e6d034098ed450a169
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448726 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b277f5e168f773e3dba38bca291a5fd138d56fcdd5008a2f0bf1866d311483b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:15 GMT
ADD file:40953ed6e6f96703b2e0c13288437c2aaf8b3df33dbc423686290cbe0e595a5e in / 
# Wed, 12 Apr 2023 00:20:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2ff1d7c41c74a25258bfa6f0b8adb0a727f84518f55f65ca845ebc747976c408`  
		Last Modified: Wed, 12 Apr 2023 00:24:01 GMT  
		Size: 50.4 MB (50448726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v7

```console
$ docker pull debian@sha256:3e45d273ad15d7b648212c8faa5e236f6fdc694af7272b8c4bd25a5d90c27fe1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:966f48305f6385972a055032836be86bf1af1f7b601a7b8271ef7b695ab566b0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:53 GMT
ADD file:3f6ca36ec4fbf50ab85c907c089dbb0618c73393c07a5841b922145b17795552 in / 
# Tue, 11 Apr 2023 23:59:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:166ef90dbaeab03f451f526d325c57795001e25f10c984c334a82d174b3c554b`  
		Last Modified: Wed, 12 Apr 2023 00:03:36 GMT  
		Size: 45.9 MB (45916201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:55cd39723a057f730e9c33487fea9fe151bbe86dce51e92a73ccfd12206a1f43
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238632 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0604d0d2dc821d9cfef7619166adc659049f3ef4092aabd9bfb268d86869e56`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:55 GMT
ADD file:93facc669dd63b37fb5dde18f3b3a1cb5621aa396e1960ea85facdd1c619a3f0 in / 
# Wed, 12 Apr 2023 00:39:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4910c31031a0301ea4f8b7155269014925aeb17c71b869dea3ff907ba294b55`  
		Last Modified: Wed, 12 Apr 2023 00:42:52 GMT  
		Size: 49.2 MB (49238632 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; 386

```console
$ docker pull debian@sha256:91e8c113a94dfe5daf935a7d943a8ed9e500bce23540c57bd91153eb1d9b8c15
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68659ee584b238b4b4811ba3fbb00f097987eba15c8a0f3beeefc5ddb1ca7a63`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:11 GMT
ADD file:8299e58c7e43af1f3b9fdb9bd37e3f37cf619a97ef1b87d60ca66860d646d526 in / 
# Wed, 12 Apr 2023 00:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d15e1c2a3da7d675c2513d7ceab1d0f33ced9a82bed63d9a11ce71f1218d232`  
		Last Modified: Wed, 12 Apr 2023 00:43:07 GMT  
		Size: 51.2 MB (51206167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20230502`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:buster-20230502-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:buster-backports`

```console
$ docker pull debian@sha256:0c17fc243838d557622b3e7a602b3fa4a681d4ebc36e7162a75158256439eb38
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-backports` - linux; amd64

```console
$ docker pull debian@sha256:e3ac5e3f77f4d5411568944cf93592832a055cfd2ddf67ae07c59d65b71f5a3a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448948 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e209f4e5f3d540c6aeec4f640f22517c6efef06db2dd14e9d319bc343d43c56`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:15 GMT
ADD file:40953ed6e6f96703b2e0c13288437c2aaf8b3df33dbc423686290cbe0e595a5e in / 
# Wed, 12 Apr 2023 00:20:15 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:20:18 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2ff1d7c41c74a25258bfa6f0b8adb0a727f84518f55f65ca845ebc747976c408`  
		Last Modified: Wed, 12 Apr 2023 00:24:01 GMT  
		Size: 50.4 MB (50448726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:111f7e409470bbc7e49a7c71c624c7db3f51198731678f13b630aeae0921c49d`  
		Last Modified: Wed, 12 Apr 2023 00:24:13 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:64c7b44b0b61a1497f7c0843a14f15017b54dbfcbdb3937853eeae0d9aa7411b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916423 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e859bca7fd8cbcc1f66a62cb88c9545143170c7b8ee130f78ecc06ebb4808089`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:53 GMT
ADD file:3f6ca36ec4fbf50ab85c907c089dbb0618c73393c07a5841b922145b17795552 in / 
# Tue, 11 Apr 2023 23:59:54 GMT
CMD ["bash"]
# Tue, 11 Apr 2023 23:59:56 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:166ef90dbaeab03f451f526d325c57795001e25f10c984c334a82d174b3c554b`  
		Last Modified: Wed, 12 Apr 2023 00:03:36 GMT  
		Size: 45.9 MB (45916201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:579cccff48053a246eb15e71e198d78b3461a760b1140d0f4b159eeb17f2c673`  
		Last Modified: Wed, 12 Apr 2023 00:03:49 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:38909b012e9e8c35d0eab4f3338c518017fc77e1a777ae2d2b74956c778d6f1e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238857 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d61f55313c7fd443c984898caf97abcd0259698009e03986fcd18c489f662c39`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:55 GMT
ADD file:93facc669dd63b37fb5dde18f3b3a1cb5621aa396e1960ea85facdd1c619a3f0 in / 
# Wed, 12 Apr 2023 00:39:56 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:39:58 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b4910c31031a0301ea4f8b7155269014925aeb17c71b869dea3ff907ba294b55`  
		Last Modified: Wed, 12 Apr 2023 00:42:52 GMT  
		Size: 49.2 MB (49238632 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:891ba131a68a92f4849170a28e165e333c613ec38c29fd8cee9ae4f4a8e85456`  
		Last Modified: Wed, 12 Apr 2023 00:43:03 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; 386

```console
$ docker pull debian@sha256:1681e901ae07ad9b8d7eaa38006f28b61e26a00869d29265c322a5bd4f7a6c38
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a1dcd56dc5f9433392d61773bae7c52793333adc286e93bcc61171c7080827a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:11 GMT
ADD file:8299e58c7e43af1f3b9fdb9bd37e3f37cf619a97ef1b87d60ca66860d646d526 in / 
# Wed, 12 Apr 2023 00:39:11 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:39:14 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2d15e1c2a3da7d675c2513d7ceab1d0f33ced9a82bed63d9a11ce71f1218d232`  
		Last Modified: Wed, 12 Apr 2023 00:43:07 GMT  
		Size: 51.2 MB (51206167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:cf99dfccba60cad74fd547e7a78dab6288bca72a40deb3882ee89845dade1aed`  
		Last Modified: Wed, 12 Apr 2023 00:43:19 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-slim`

```console
$ docker pull debian@sha256:4d208d79338beaa197912496d0791921a31d9c58fa6fbd299787fab37cc893dd
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-slim` - linux; amd64

```console
$ docker pull debian@sha256:603e7e0b2c5ee5dcfd1b1ba07b4598dd481b6afa6dcf348399428e72b0370a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138920 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92a9b50a62b45ede9878156909ea76de9d04716aa0cdfe175bd2d28e34b87149`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:25 GMT
ADD file:e614539607055bdbde0cc1a94ef9783fe3627c8553ef1b21071ecb5c27ea27e4 in / 
# Wed, 12 Apr 2023 00:20:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9fbefa3370776b7ec7633cf07efc14cc24e0c0cd53893ad0e7e3f44ffdc1bedb`  
		Last Modified: Wed, 12 Apr 2023 00:24:22 GMT  
		Size: 27.1 MB (27138920 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:afaf84f53aff159d638edc52279285cc4310214dfa612247748f05653dbf70f6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:688cfdcf3400ea8753eb981828a4e85b493091379bbc0f641a0e231aa6cb247f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:02 GMT
ADD file:72785c92abeacda7808e9e65787e604ed7fbc159c53eb4b2337628501418e9ce in / 
# Wed, 12 Apr 2023 00:00:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2f777f2c33b3c8a258e4118224a280169c60be31db805388525da1f620f59a86`  
		Last Modified: Wed, 12 Apr 2023 00:03:58 GMT  
		Size: 22.7 MB (22747664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5889e7bed85fdc3fd2c124b4fc0b8de6b2201c26ddb7b2d886e0d42b00d74334
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25922011 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:92d2f037b9ac2076ddd4d19cbf795887a83cbd9d179c3d18a656ac62b6e880a0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:02 GMT
ADD file:39d4ef5cc48487567b857a8527ab3c106b5099eafffbeaa8b70df450fb8409e3 in / 
# Wed, 12 Apr 2023 00:40:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:27bec964ec41cf69bd5df2ffd45c27623ae4bb0eb6f9f366179f7ac9b47d9840`  
		Last Modified: Wed, 12 Apr 2023 00:43:11 GMT  
		Size: 25.9 MB (25922011 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; 386

```console
$ docker pull debian@sha256:a9b9702cddec97594b2a09947e77a615f2e78c17c12d319233592a9ef994b707
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797581 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e93730b5b7b45f61b02834bfc1f973ce7f27f98f58b3522f29c84c78485b31b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:21 GMT
ADD file:0f4c5475f636be52b419806a7ea7d8d81fc1203c9228544c1324e79277739a3f in / 
# Wed, 12 Apr 2023 00:39:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ae6c02ab25c72d784ae8832cdcb583320ab9a3046938adc3c659ae607da19b43`  
		Last Modified: Wed, 12 Apr 2023 00:43:29 GMT  
		Size: 27.8 MB (27797581 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental`

```console
$ docker pull debian@sha256:a34dc28b1c17d0acb56a8df2d51ee5de3e320aaa005d6088bdac0d9524151d43
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental` - linux; amd64

```console
$ docker pull debian@sha256:ca7f035edc086e494d9f2050ecc0618b891e379e98f4fdb1e3230a66ad0b672d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49287875 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cdc81544d8a3971d7f6f506af29909ede4deeb330da5c293a013edcbabbc6c68`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:52 GMT
ADD file:eb948b4bfe0963071c224791a81e9295fa552632c5b185f143e3c42c071c4671 in / 
# Wed, 12 Apr 2023 00:21:53 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:22:05 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:277078978190188dc2be36cde72743cfd4b19903cf7c90ceef8421d87590db29`  
		Last Modified: Wed, 12 Apr 2023 00:26:45 GMT  
		Size: 49.3 MB (49287657 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e24155652d76da641860b0880f1eb77c20820c9755ed8901327813e326271111`  
		Last Modified: Wed, 12 Apr 2023 00:27:06 GMT  
		Size: 218.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v5

```console
$ docker pull debian@sha256:3ec7e8d03d0b24136e45f16749d8e235bf15a53f31933b235167d4a787b5c68f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167286 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e97b434224c2e1b20ddc7accaa6b3353325c2f0a71b144f0b8014381fe091ee1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:42 GMT
ADD file:740dbf70289faed5b254bb33009aaddef87ef73abcfdc2441b61e0be62ac6d62 in / 
# Tue, 02 May 2023 22:49:42 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:50 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:c7b5fff53169be147228f50722828c1946b9253a79b37d14d755411593e715be`  
		Last Modified: Tue, 02 May 2023 22:53:28 GMT  
		Size: 47.2 MB (47167066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0eade863a9332ac1f6ffe4dbd68aaf23ffd777b9c81619aeeccc0bf1aa40899e`  
		Last Modified: Tue, 02 May 2023 22:53:49 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v7

```console
$ docker pull debian@sha256:457923685976ee8c6aaf6bc51b5eba2513f7b372cf1813dfc66f7a2d42d3f5cb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45890459 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9ef959bc2bb82aa080821852f23fc9669fd3daa05a2aa8c61afe1b7eca65041`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:29 GMT
ADD file:a85f9bd069b317fe04f7b0e5e2a6e54a4a94ed304e2487154c520f160f81634f in / 
# Wed, 12 Apr 2023 00:01:30 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:01:39 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:54d799b49c2235300dac823b010337407851814391945952bc9cbe2adfc39d3c`  
		Last Modified: Wed, 12 Apr 2023 00:06:16 GMT  
		Size: 45.9 MB (45890238 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c9f266b9874326d8744696ecb20f43c6a235ce3e66b7ddfb6a034f15e3737e4f`  
		Last Modified: Wed, 12 Apr 2023 00:06:35 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d415cb25dd12c62a6508d76376c8fbc554a296f6e0a647b52d6b91ef8076caac
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49327883 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:be8ed813437ccdf05e7114285d3c8e2935ed2aa0b9ce9cc9612e32a93ab4a61b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:59 GMT
ADD file:eed6ca617d18b91908df79e437985b8da5417ffb4eb0c52b6cec20e46c00414a in / 
# Wed, 12 Apr 2023 00:41:00 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:41:07 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:9a890b97e54717f3d2b342929339a71068f93c352460bf767e21a39e90688b79`  
		Last Modified: Wed, 12 Apr 2023 00:45:24 GMT  
		Size: 49.3 MB (49327659 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4de767cde562b4096331329cc56b1ce0017441c9715bfb559abd67ebcff8fde6`  
		Last Modified: Wed, 12 Apr 2023 00:45:43 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; 386

```console
$ docker pull debian@sha256:160d086cc268d5f0b0c5707abcc7cebea514795e49b3e474185936248551f429
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50311123 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03c073ae6fbf54de0a5f467386fdbc065e18bb2cd1fc6d6ea85a2d3456e8d80f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:50 GMT
ADD file:846fb6727047101baf611eb5e752306b9f0d603546da67314cb4ef21e2f1f54e in / 
# Wed, 12 Apr 2023 00:40:51 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:41:02 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:308acd63a4eaf4bc5f65e274ce996d9cdbcd66aa3e1fce97faf327125eb14e0a`  
		Last Modified: Wed, 12 Apr 2023 00:46:12 GMT  
		Size: 50.3 MB (50310902 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f3e20b6832994573bc5e69363c770a8098c7bb965b3c6dc74989bd928810991c`  
		Last Modified: Wed, 12 Apr 2023 00:46:35 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; mips64le

```console
$ docker pull debian@sha256:21f20e067b8c031a8411cefeb6dbaee8ebc2ee5bef352928e7404e0afb99907a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49296474 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c57b62d7b0bc81496cd6c4f650b7976e091f9170838b072b848d4ead104337c3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:13:36 GMT
ADD file:7175a1b08b8243bc539cc1f758d85ccbe24afbc3df6a1f629c7a43839aee5f62 in / 
# Wed, 12 Apr 2023 00:13:41 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:14:15 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:589b4c3c9d360e937d4513f3ec8af4982019090cf0f68fb06e1e275c06591a76`  
		Last Modified: Wed, 12 Apr 2023 00:21:40 GMT  
		Size: 49.3 MB (49296251 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d3e8aaafc25e00fed4f728c0664478dccdcf9f43ed1708ea8ccbe0fbaed2a1bb`  
		Last Modified: Wed, 12 Apr 2023 00:22:19 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; ppc64le

```console
$ docker pull debian@sha256:1ee69d64a8be19f5bd1e06a74ffc9cf2726fbd4a286c70863f13a57e58b18b88
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53291933 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2c76d7554eabcbe88b5cedc818303dc922fb264b836768deef356515d8d2da76`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:38 GMT
ADD file:930aae2ab6be5d8295a31bf5dd91eee5ee4fa92a08b6159f55cdc6d3bd182e67 in / 
# Wed, 12 Apr 2023 00:10:41 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:10:59 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:4e67fd8b6f31e9a06c3401edfed958df31a0ae78d0692baa41a3d87074423b51`  
		Last Modified: Wed, 12 Apr 2023 00:15:52 GMT  
		Size: 53.3 MB (53291712 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:27112dd79fe0a748333ffc777c2b7353783f06d8bae54c98d2cfa0f7eb90dd40`  
		Last Modified: Wed, 12 Apr 2023 00:16:18 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; riscv64

```console
$ docker pull debian@sha256:848f36bcdc59386757ea640d87b9543b0b0e6974b24753e43c9d62a92cfe49a6
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.5 MB (45495174 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:003a05cc9ec6850682d28a3624bb3de33c33df72a821c873287809c8e36a4c8c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:30 GMT
ADD file:f48330c22d0ecd502b8b46026eeef4081864a8a585160caae93e5700a6524a58 in / 
# Wed, 12 Apr 2023 00:10:32 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:11:18 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1b037784414a5f8d993a1f29169b41dbec924bf914e4e324fdd3209ce4445a32`  
		Last Modified: Wed, 12 Apr 2023 00:13:57 GMT  
		Size: 45.5 MB (45494945 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:91927993230d3dd4786ac01888e74b7a400d90f94093954b120def27101a238d`  
		Last Modified: Wed, 12 Apr 2023 00:14:43 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; s390x

```console
$ docker pull debian@sha256:9bb2165075db3a6509a9447f86beafeac8f7ba75202b764da22b505ec16b170a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47665290 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7a0a7b534b879a5d04f757626e6f6acf5a4dad5380f6c29124d4cce3940a4ccb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:03:33 GMT
ADD file:9280167c5f0a6008baa6e9d84a780823b1cf741a9935a60531fb30279a421a97 in / 
# Wed, 12 Apr 2023 00:03:38 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:03:54 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:967d5de3f09581dfdc9be32037966130e96015074ba7d995ffb521bcb496aedb`  
		Last Modified: Wed, 12 Apr 2023 00:06:36 GMT  
		Size: 47.7 MB (47665070 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:74d61ec497f5f76e5e6a2d2a829e93a2e43b9ad2cbc9586ab0729461604adb3c`  
		Last Modified: Wed, 12 Apr 2023 00:06:49 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental-20230502`

```console
$ docker pull debian@sha256:ee545488a4f6ffe0fd5753dc6228f964a6cd200a9bcf7ac04c1d9ae7c43b2c37
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:experimental-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:3ec7e8d03d0b24136e45f16749d8e235bf15a53f31933b235167d4a787b5c68f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167286 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e97b434224c2e1b20ddc7accaa6b3353325c2f0a71b144f0b8014381fe091ee1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:42 GMT
ADD file:740dbf70289faed5b254bb33009aaddef87ef73abcfdc2441b61e0be62ac6d62 in / 
# Tue, 02 May 2023 22:49:42 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:50 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:c7b5fff53169be147228f50722828c1946b9253a79b37d14d755411593e715be`  
		Last Modified: Tue, 02 May 2023 22:53:28 GMT  
		Size: 47.2 MB (47167066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0eade863a9332ac1f6ffe4dbd68aaf23ffd777b9c81619aeeccc0bf1aa40899e`  
		Last Modified: Tue, 02 May 2023 22:53:49 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:latest`

```console
$ docker pull debian@sha256:251456f07b3fcc936c6ebf5218e6a8ffae63e4a86d23f16fb1b023f160eca7e6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:latest` - linux; amd64

```console
$ docker pull debian@sha256:4effa16ae79ab36347d55ebb6bb4aad411077b72ad735054e684b3eeba373a35
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052736 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca1a8403096efb7da1512b0ae008b16f2dae553deed435163b137447b90fa8b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:19:55 GMT
ADD file:f2d012660f882f319a5878a3f9ce285f488b8f90fad49ad238541cf72089e035 in / 
# Wed, 12 Apr 2023 00:19:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0248cf3e63c73d0e496a67807d056ca41d5e968b61087e8eca2cf4b9b4d7b99`  
		Last Modified: Wed, 12 Apr 2023 00:23:20 GMT  
		Size: 55.1 MB (55052736 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v5

```console
$ docker pull debian@sha256:d170adce3d14567fad94acb11df1b8811da1a742668a2289778be573cb9a7ce8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb6d0a852fe4800d15c6c1b1fce5de90be94f044e74cabdfd5418db829529565`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:48:45 GMT
ADD file:e5816cf9c67a55e94065100a95ed3b269817a643b21e5b9c060bfde394a56889 in / 
# Tue, 02 May 2023 22:48:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a091ba323f94f60ffce46d1dad6471927a6d7136843d7bbc29ede769d6cafd86`  
		Last Modified: Tue, 02 May 2023 22:51:00 GMT  
		Size: 52.5 MB (52546533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab0ce510343d33415fc4eaea0102da2d24cb720b185aaee0b411c48a07abb0de
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50216882 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b29663cecd1d9b95d625512cc33ad2c8d970653700f863175d7fdf5b022fc129`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 11 Apr 2023 23:59:35 GMT
ADD file:75c57df33c95251a80549b7949853df282a42bc4e5f19176764907a54b74caa9 in / 
# Tue, 11 Apr 2023 23:59:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4feff9b88735325634445419eaaec2ed47500b63027cfd2de34b8bc6d55ee85c`  
		Last Modified: Wed, 12 Apr 2023 00:02:57 GMT  
		Size: 50.2 MB (50216882 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c0f6422141bb031238126fbd535c0df24db4252a2d9876216a94082df74c8fda
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705529 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6d9c9c3e100a13622816226e63a3faa83da315ae01216b1ac02c4ba5cc91683`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:42 GMT
ADD file:dd3d4e5fe819950e7d01b44a29dbd790438cd722ba76198910e2597448ab0c6f in / 
# Wed, 12 Apr 2023 00:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dc80b8cdbfd36cb20231d807a50d704945d1df4da8f6e23197ccfcb629970491`  
		Last Modified: Wed, 12 Apr 2023 00:42:14 GMT  
		Size: 53.7 MB (53705529 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; 386

```console
$ docker pull debian@sha256:8b0dc8f7cb7e6d31777127549096000aec141df162e86e1380198ca285f3c0d2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56032818 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3dc4945dcbf0d0c1254194f3d8bb6b886853fc2c10a7f0a8bab307a301e9b928`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:38:51 GMT
ADD file:4da7e6af0782b21849e5f6385de3589ac5b8d81d173721f9990d32da754df2cb in / 
# Wed, 12 Apr 2023 00:38:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2bef40925391866903279768b0ae1e8233ef6a1aae64509a922f4aa96fe42a6`  
		Last Modified: Wed, 12 Apr 2023 00:42:19 GMT  
		Size: 56.0 MB (56032818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; mips64le

```console
$ docker pull debian@sha256:fa84b0675f70f0665f2d3f4523eaf612c84f92633741d43947dcb826fddec017
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272023 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1c3fa623ac9031ff737d79a791a64eb03b9c7c30f1d5b3a283d692df505c728`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:26 GMT
ADD file:5d2c839055739fe92d64098c09607e7ec9123101d15f837c54f3688755af0c23 in / 
# Wed, 12 Apr 2023 00:09:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:80243e355b05bee53c6acc45ace2cde4f7495dafe9f3f8d7d299ed51d04928d2`  
		Last Modified: Wed, 12 Apr 2023 00:16:48 GMT  
		Size: 53.3 MB (53272023 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; ppc64le

```console
$ docker pull debian@sha256:186d0c8e5cd4abbdaa10fc9cc24757948bcd32666b9e9bd8acac15f800a5cb40
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b7d3fcd14d537d007a485ba25431b3dec0eeb5ed936a77507e18fe712e5d26ce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:07:58 GMT
ADD file:c3c2a10473ddaa3d8a30ca99b19ad3599d0e60d826c4e0315bdb7463352ebc09 in / 
# Wed, 12 Apr 2023 00:08:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d3ffdd4610a019889b845cc82b3d956ad85cca78ad4bff2d5e5522bd02c17e7`  
		Last Modified: Wed, 12 Apr 2023 00:12:37 GMT  
		Size: 58.9 MB (58926600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; s390x

```console
$ docker pull debian@sha256:659187525ed5d0088941a635408757a22f6133dc4b464290878d0e52d8c99e26
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286682 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e56d91c8c4483e11810a8b7f71d9f36d39e97dffd22d8856cab268ad64d7ff74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:25 GMT
ADD file:ab4e28724d04724a57e0f5bbe299d3361103f05ad1978dc3bfef243c1f9d6476 in / 
# Wed, 12 Apr 2023 00:00:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:042263756d7dc47bf98a243871107aa3a2b41acda91d104d8b4e148749a43dda`  
		Last Modified: Wed, 12 Apr 2023 00:05:03 GMT  
		Size: 53.3 MB (53286682 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable`

```console
$ docker pull debian@sha256:4bc863527209403825b01ff2b5a492c4a4ae3cf1660a97f07a6e3ac419e10758
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:de217c07152967fb5df2455650cb5a5eed88720e750c21d49b7b8bde6f6a849f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448742 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9327ccf08335b090f014655e6d3b7413588a265b15e22f3cb351a42263574dae`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:35 GMT
ADD file:f847eebe128616577b0c1446a604d83f2e5b0aa0ea6558ac3fd3197acd565450 in / 
# Wed, 12 Apr 2023 00:20:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:86ea788dd30a5e7aa659d05425a26e2e6cc3e28456a86f9a72804a0b20c73a2d`  
		Last Modified: Wed, 12 Apr 2023 00:24:40 GMT  
		Size: 50.4 MB (50448742 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:a465d55767e2dd6e82ba657a50fb3035219838bda9308a327e0b10ea90ce8fbc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916224 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d82f9f70ccbc9c19055bfb4d1aa309caca3d0c3f15d862bb6e9fda989476c168`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:10 GMT
ADD file:cf4b865a9975f91a514a6ac1ea3d16ca0b2168ee0bd34ff1f5ff8608b55ffc7b in / 
# Wed, 12 Apr 2023 00:00:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d7eb6448f66ece8b2e1bdc559da9b93296d41b223b4eb9431ac5d847ea4e5da5`  
		Last Modified: Wed, 12 Apr 2023 00:04:16 GMT  
		Size: 45.9 MB (45916224 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:849e34c981e96cb1deb6f6869bee871c59644af30b3864fadf5ac3bc1f177e34
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238622 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:39a9ee0fdb91fb60529ecf825563dbf9d66130e62a7914ed305e83bd30228af6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:08 GMT
ADD file:128de67bd32a4087af0c3417150a0e6d911dc0d017943fd4c2d7ebfffc549044 in / 
# Wed, 12 Apr 2023 00:40:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:82e3c316dee591406fde11a61d53a1b814e23b19cc3ee139d59cd3caf9125df4`  
		Last Modified: Wed, 12 Apr 2023 00:43:27 GMT  
		Size: 49.2 MB (49238622 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:2af9f6c969ce7630b47183bcc2ff9fb47d76e9c054f453f446bf6aa56c486731
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206154 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:401673e02fdd839d2e13f6f793a2ce21ea7d01a0c4ad960d91564adc0fc7296b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:31 GMT
ADD file:e2125bdc5f0765701ef5411ca3e6052d6f60a35560148a37cf53865b08c5dd87 in / 
# Wed, 12 Apr 2023 00:39:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6077d38359227cc41dcfea2d3c8ce889a701c3c0e430aadf9d9d4ad16d51e55c`  
		Last Modified: Wed, 12 Apr 2023 00:43:49 GMT  
		Size: 51.2 MB (51206154 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20230502`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:oldstable-20230502-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:oldstable-backports`

```console
$ docker pull debian@sha256:4eafb7ce1f7f5a391af927960b3b7e65b1702592c6a9e6de1f93c4b44e30ac77
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:1e2621c4072e1f65d573302510841fd82c72b0101deefe817d7abc7943d0cd97
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448966 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ec95d88d7f4f502f82e0a5befebd371a8f903fa31ff88604e655021f6631c44`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:35 GMT
ADD file:f847eebe128616577b0c1446a604d83f2e5b0aa0ea6558ac3fd3197acd565450 in / 
# Wed, 12 Apr 2023 00:20:35 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:20:38 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:86ea788dd30a5e7aa659d05425a26e2e6cc3e28456a86f9a72804a0b20c73a2d`  
		Last Modified: Wed, 12 Apr 2023 00:24:40 GMT  
		Size: 50.4 MB (50448742 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5e0082b1a88baa4e5489c0f74586f8799157030581fa2a7ad95d350a3a1adde3`  
		Last Modified: Wed, 12 Apr 2023 00:24:48 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:c37ff97d69d0bd9f70b6988dd2e609feb7eff9aab1f9c1faae972a85ded33e42
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916448 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4cf597edfef6cfe7defcd8492e650bad5965b0ee8fd0be47284cd3dc3dc3d254`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:10 GMT
ADD file:cf4b865a9975f91a514a6ac1ea3d16ca0b2168ee0bd34ff1f5ff8608b55ffc7b in / 
# Wed, 12 Apr 2023 00:00:11 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:00:13 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d7eb6448f66ece8b2e1bdc559da9b93296d41b223b4eb9431ac5d847ea4e5da5`  
		Last Modified: Wed, 12 Apr 2023 00:04:16 GMT  
		Size: 45.9 MB (45916224 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4164123b44dafa936ada710e3d59b92e0f3b4e343d4dc1f311f3f5b40afbfceb`  
		Last Modified: Wed, 12 Apr 2023 00:04:24 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:75fc4cc00444e2e76b75065a38c7b8dff968b74f1f692fd210e9205b9950f298
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238849 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6f9c2c5a6dfc3f7e63deb92185f48adef050a061c6404dcf5ea66445ae81798f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:08 GMT
ADD file:128de67bd32a4087af0c3417150a0e6d911dc0d017943fd4c2d7ebfffc549044 in / 
# Wed, 12 Apr 2023 00:40:09 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:40:11 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:82e3c316dee591406fde11a61d53a1b814e23b19cc3ee139d59cd3caf9125df4`  
		Last Modified: Wed, 12 Apr 2023 00:43:27 GMT  
		Size: 49.2 MB (49238622 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:cc5be318b063fb257e05c24aa7708b2cb63fdbccf51f83d659d4eca325ab3f6c`  
		Last Modified: Wed, 12 Apr 2023 00:43:35 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:3a6cfdda22c5128802e3836247a5130fff61025d85e3118d2a42be40093ec1d1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206377 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8cae5dab1fbebcc4541ae9fc0ff84e3896494652aca9c20911a6ca647abc286d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:31 GMT
ADD file:e2125bdc5f0765701ef5411ca3e6052d6f60a35560148a37cf53865b08c5dd87 in / 
# Wed, 12 Apr 2023 00:39:32 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:39:35 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6077d38359227cc41dcfea2d3c8ce889a701c3c0e430aadf9d9d4ad16d51e55c`  
		Last Modified: Wed, 12 Apr 2023 00:43:49 GMT  
		Size: 51.2 MB (51206154 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f24278380bddf5f3969c753a6cb2466ccd5eb30d5e3974ade1c642db6456f7c0`  
		Last Modified: Wed, 12 Apr 2023 00:43:57 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-slim`

```console
$ docker pull debian@sha256:930c43e0a75fe6415649841458e05d4e1dff801d5a166e20b7a68cbf0ed5f549
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:35f6a8e0709819f2e689d3371f4e10e5a54610c733096604c141b3c6c05d0904
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138908 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4cac9257131fc6e982c569cc8da5a57270db40d45a37cc07c5e70fd7d44a25f3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:45 GMT
ADD file:a056118fcc0f46225f518545b7b64bdcd548e2df1e7edae6c1e2f12fdb736a21 in / 
# Wed, 12 Apr 2023 00:20:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:70fd0e3939eae214579ac79c486ee817381b280eed1c6833529b530547f3a905`  
		Last Modified: Wed, 12 Apr 2023 00:24:57 GMT  
		Size: 27.1 MB (27138908 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ef6120733993fb89d18f92f049bbec44cb439779af71278813d2cbbabf968df3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747671 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fc117ce3979a786ac64a8d90897806cb986f3b438d823df2985d7d40cb13bc66`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:19 GMT
ADD file:5b6ce409d7dfeb48300d8386ceb8ce929712c82dbbd75d446a47e757f36bede4 in / 
# Wed, 12 Apr 2023 00:00:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5594825fbfb67c82b5e46fedb6408f85561f2b206d7cf5652bc42355c04ef763`  
		Last Modified: Wed, 12 Apr 2023 00:04:32 GMT  
		Size: 22.7 MB (22747671 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:94dfdcf4506b75e46ce8cae720681d784d7808c2420acdd31d48cbfa13a17e9b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25922015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:727c9383dcd2874bdaf406979b4bf3406a025cba694ca9bea324105c42cf4313`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:15 GMT
ADD file:25d419e469f06e47d76992eb7df6fcf63c87e645ec48e009dd476a94bc4011bc in / 
# Wed, 12 Apr 2023 00:40:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:09305c10e3575dbecd2751e5ae75dfe4268a38c158689176a05975f03e1eb133`  
		Last Modified: Wed, 12 Apr 2023 00:43:43 GMT  
		Size: 25.9 MB (25922015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:12e16a5f92b3a9512ae22b416b02225c17fd34ca95da59a0f18cd845e4e2fe24
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797599 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ed0d65a8ddac44623313474ff17a1ee8a3443e232853ba72727cd904a147b623`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:41 GMT
ADD file:3a5570827dbdad5f8bd747b7951e9588afadfba29a3fde1a59549e1c0f3d912b in / 
# Wed, 12 Apr 2023 00:39:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:415cb8671c25dc8e7764427cf6e81cdba3372640bfc45c32a239eafbd8f27fda`  
		Last Modified: Wed, 12 Apr 2023 00:44:07 GMT  
		Size: 27.8 MB (27797599 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy`

```console
$ docker pull debian@sha256:f500f622e27b4242bc19b96d87538a935adc4f1c4983f4e04b0ed14e75bac7f8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy` - linux; amd64

```console
$ docker pull debian@sha256:d35a285fb08b8722c07733c192753d398cbe27590ee8b4899a4ec18dcb89c88c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49287880 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b09361a3f014b9ffdc3988e728b61cd4862f45deee0905f2acf4e9cb37377e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:55 GMT
ADD file:0936f6fccda991ba27a7f73ca61c23a436b7ca417e504b66d5a68f2283e1e81d in / 
# Wed, 12 Apr 2023 00:20:55 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:22:07 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:003dca91ec623536e08abc4e1bb2849836a84c44f82e24b97fd5423162a4d46e`  
		Last Modified: Wed, 12 Apr 2023 00:25:13 GMT  
		Size: 49.3 MB (49287652 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9edf299448d12355c6805ac3c08f004fef53f15385eb953042929a3ec684674d`  
		Last Modified: Wed, 12 Apr 2023 00:27:13 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v5

```console
$ docker pull debian@sha256:84ae48d1e5c0fd3427c4c104f68c1fad06682f9a1d369a654f236f2bb083eebd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167290 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0acecc292b939ed715158aa04b7403c61f937d056a868e1266f0b533d9383dae`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:00 GMT
ADD file:2228a07c6ec82db1e0719684d77cecf448c3be947c2b1b48347ba5de9e5b57f8 in / 
# Tue, 02 May 2023 22:49:00 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:52 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:90d5a5fdc15637fce928afc279fecd9adb82fe3c476f174458fc6bdd7fb4b57e`  
		Last Modified: Tue, 02 May 2023 22:51:47 GMT  
		Size: 47.2 MB (47167062 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b41c79d2b34a44836a2b17fb9900e62e368aa0ed4b25619c7423e145dfacb92e`  
		Last Modified: Tue, 02 May 2023 22:53:57 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v7

```console
$ docker pull debian@sha256:b0ebe70634e0c690afff84a11f8eb8a4ab1158fc78f3f6109cfb139e06652c9e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45890460 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6aa9c73cd8469f68b83f4ce921fd4f4719d0071f8fd6958ec57132313f9cecfb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:33 GMT
ADD file:f8bcd71f10e7adad706b3a1685fd5c4844e3ebde65bdf9a7c004d7b9dd3520d7 in / 
# Wed, 12 Apr 2023 00:00:34 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:01:41 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7b0aac501774f41dc4e46815077e011ed06b86d9469c6ca28755965123669423`  
		Last Modified: Wed, 12 Apr 2023 00:04:46 GMT  
		Size: 45.9 MB (45890234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:652d2c86befd540f1eda8dfebd2efd59357820a8cf7865b7e11580c20e62e14e`  
		Last Modified: Wed, 12 Apr 2023 00:06:42 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:82d2f7ee13ac76334c78b2f3c96b1c7efb42a61b3904420f9f2a49afe6076ef6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49327880 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:068cfd69d0e398ecc7f50f3385676047333f44629583783f25e7dbe8bf5352de`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:21 GMT
ADD file:2784bc6dba1e1dbea293e074ebf51fa7d89cb5545ea63ba2ed75b74d067387d4 in / 
# Wed, 12 Apr 2023 00:40:22 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:41:09 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:43d214f7ab3ca35a0c9ed4d8cdf62b36248aba2fa1bbac4ecd64e82ba2520b70`  
		Last Modified: Wed, 12 Apr 2023 00:43:58 GMT  
		Size: 49.3 MB (49327656 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b04bd62d44ba32e027825f8869408c8b0295c49fe85344680d94abf86db6c6d6`  
		Last Modified: Wed, 12 Apr 2023 00:45:51 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; 386

```console
$ docker pull debian@sha256:03d4bda50d596a59a92288cafdae6af75c5d0fc9db31a5b46bfe0eac99d415d4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50311126 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2f9a5f3e5bde0c62a30d57edf3500a9cbbe67582306843836074927296b6baa6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:51 GMT
ADD file:31710a830769b5e1f2612e1cd05380f677714bf52f61a54adbc7424087fd1378 in / 
# Wed, 12 Apr 2023 00:39:52 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:41:04 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:35e4af56b8bda58bf958814bc36c8e3a9a1d719c8582c0f33d7d69cc5b416211`  
		Last Modified: Wed, 12 Apr 2023 00:44:24 GMT  
		Size: 50.3 MB (50310899 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e89cb04deba2a42338f73109b3337e09edf3604bfe0c5e9f19f674937a58cd4f`  
		Last Modified: Wed, 12 Apr 2023 00:46:43 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; mips64le

```console
$ docker pull debian@sha256:4efaac374daf67a00881479de182296dbfbaf2c645a1e8d3efeccf06d0d22c71
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49296476 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:edba465505f792b976470866a4803f97d60b35b3769d87cbad1db198fda4bfb5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:29 GMT
ADD file:60b5ce2a691448a1aab43387ba9337b8cc193a2cbb2a01ac71f5f0cb083415ff in / 
# Wed, 12 Apr 2023 00:10:34 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:14:24 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:45f3dc30e8865982b1af34a4bd64597aea34350dd1bf817342cb2901014ff9a2`  
		Last Modified: Wed, 12 Apr 2023 00:18:10 GMT  
		Size: 49.3 MB (49296248 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:897bc05f3645d90b6207202f6d835e763d3053e4c7de843cd7cb9ab95bfb932f`  
		Last Modified: Wed, 12 Apr 2023 00:22:28 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; ppc64le

```console
$ docker pull debian@sha256:d4519be529428ab720a44d25f5dd9f20c1104599d223a81d68e63f17e3a53998
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53291936 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b9c6bb2b7533ed8946304bee172ef4de94ab95a47dce6d325e6c6bcacc21994c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:41 GMT
ADD file:8700846eefa6ef0670106ef8055671479d597d057ee34d1d248dbefa5977753b in / 
# Wed, 12 Apr 2023 00:08:44 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:11:03 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1c14c235d79599f644d6f952e975871669fe8a00e0434e5d26966f11cfb3820b`  
		Last Modified: Wed, 12 Apr 2023 00:13:39 GMT  
		Size: 53.3 MB (53291710 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:631ed35ea15d9ffd6a47c821d9f697df2fea786ea2e328c6911a1dbc8cb8ee22`  
		Last Modified: Wed, 12 Apr 2023 00:16:27 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; s390x

```console
$ docker pull debian@sha256:34880ad087f8d44ade019bc181ba20dbedfcee313ff1bc5958b92ffecdffda62
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47665294 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:76b26e192406c0b2ffdc0c2fd405a8026f126defd65a4dcba5f0140832bf08c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:19 GMT
ADD file:0593fbca11f3d4a9b2b916ee99bb70651af04ea1275262f12e9b0e5c07e81f0e in / 
# Wed, 12 Apr 2023 00:01:29 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:04:01 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:1844a6b5cc32e25703d3a6ac2115866157b1f856d7463d0dca8443fea51061f1`  
		Last Modified: Wed, 12 Apr 2023 00:05:31 GMT  
		Size: 47.7 MB (47665068 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e61f1a1750709a627c6e075515fb08a8ba057dc062a5f19ab977db129e84ef86`  
		Last Modified: Wed, 12 Apr 2023 00:06:54 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy-20230502`

```console
$ docker pull debian@sha256:cf81ed3e9f5f5d52890722debee549eb7aab2d1bfd1f0d4d1495466c1f66e0ab
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:rc-buggy-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:84ae48d1e5c0fd3427c4c104f68c1fad06682f9a1d369a654f236f2bb083eebd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167290 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0acecc292b939ed715158aa04b7403c61f937d056a868e1266f0b533d9383dae`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:00 GMT
ADD file:2228a07c6ec82db1e0719684d77cecf448c3be947c2b1b48347ba5de9e5b57f8 in / 
# Tue, 02 May 2023 22:49:00 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:52 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:90d5a5fdc15637fce928afc279fecd9adb82fe3c476f174458fc6bdd7fb4b57e`  
		Last Modified: Tue, 02 May 2023 22:51:47 GMT  
		Size: 47.2 MB (47167062 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b41c79d2b34a44836a2b17fb9900e62e368aa0ed4b25619c7423e145dfacb92e`  
		Last Modified: Tue, 02 May 2023 22:53:57 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid`

```console
$ docker pull debian@sha256:260e30fd930bf0f4e60dedccb176b6e7ae0b686b2ef39d5c12572c758dfadaeb
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid` - linux; amd64

```console
$ docker pull debian@sha256:ce5fdeec62abf61b8a51d1ce5190999c7f4a1ae36669ea17761f8ab4349a87e9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49287652 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cdecc352b61c3f7311106ce5c4fd10dd55070037612b6551a5643f6a5097710d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:20:55 GMT
ADD file:0936f6fccda991ba27a7f73ca61c23a436b7ca417e504b66d5a68f2283e1e81d in / 
# Wed, 12 Apr 2023 00:20:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:003dca91ec623536e08abc4e1bb2849836a84c44f82e24b97fd5423162a4d46e`  
		Last Modified: Wed, 12 Apr 2023 00:25:13 GMT  
		Size: 49.3 MB (49287652 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v5

```console
$ docker pull debian@sha256:c69027723f16d6577b240522a2d604d1dbf2cb662882b77d315941da50f1a311
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167062 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98c21bede66ffd3027190f0259054b53b328186f8210970497e526d94cd74eee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:00 GMT
ADD file:2228a07c6ec82db1e0719684d77cecf448c3be947c2b1b48347ba5de9e5b57f8 in / 
# Tue, 02 May 2023 22:49:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90d5a5fdc15637fce928afc279fecd9adb82fe3c476f174458fc6bdd7fb4b57e`  
		Last Modified: Tue, 02 May 2023 22:51:47 GMT  
		Size: 47.2 MB (47167062 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v7

```console
$ docker pull debian@sha256:e979558e7c678541188ed98f56fb10f976e60e19b31f6d018f599b48e03223fb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45890234 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7fef0c264abba2f52a27db4f03a04cb22e954e8bce6ff6a1957471a2d4acb9ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:33 GMT
ADD file:f8bcd71f10e7adad706b3a1685fd5c4844e3ebde65bdf9a7c004d7b9dd3520d7 in / 
# Wed, 12 Apr 2023 00:00:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7b0aac501774f41dc4e46815077e011ed06b86d9469c6ca28755965123669423`  
		Last Modified: Wed, 12 Apr 2023 00:04:46 GMT  
		Size: 45.9 MB (45890234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:1d02aa180c4fc4f886257a7d85aaad48ccfccc6fd820fccb432af0045d79de0f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49327656 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:32be991d553ddbd92254806772aaed42155221b4778b8c0d82ca0a42d7017363`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:21 GMT
ADD file:2784bc6dba1e1dbea293e074ebf51fa7d89cb5545ea63ba2ed75b74d067387d4 in / 
# Wed, 12 Apr 2023 00:40:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:43d214f7ab3ca35a0c9ed4d8cdf62b36248aba2fa1bbac4ecd64e82ba2520b70`  
		Last Modified: Wed, 12 Apr 2023 00:43:58 GMT  
		Size: 49.3 MB (49327656 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; 386

```console
$ docker pull debian@sha256:141d8a1ec77b718d360906e0df712f9742d299107fa40d26d30935b33922e507
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50310899 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e630867053c8c254171ace561a1e3adc0f8f49fdecde5734ca489dc5383e39a5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:39:51 GMT
ADD file:31710a830769b5e1f2612e1cd05380f677714bf52f61a54adbc7424087fd1378 in / 
# Wed, 12 Apr 2023 00:39:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:35e4af56b8bda58bf958814bc36c8e3a9a1d719c8582c0f33d7d69cc5b416211`  
		Last Modified: Wed, 12 Apr 2023 00:44:24 GMT  
		Size: 50.3 MB (50310899 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; mips64le

```console
$ docker pull debian@sha256:6a5c69fbd1dd4760f8a459203b459ab0e6e5e7c00e6230355d62bcf600b9b5b8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49296248 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d14dde7bc379c2b079be364c839c85a785aedfcd6550b024d21c601606c1613a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:29 GMT
ADD file:60b5ce2a691448a1aab43387ba9337b8cc193a2cbb2a01ac71f5f0cb083415ff in / 
# Wed, 12 Apr 2023 00:10:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:45f3dc30e8865982b1af34a4bd64597aea34350dd1bf817342cb2901014ff9a2`  
		Last Modified: Wed, 12 Apr 2023 00:18:10 GMT  
		Size: 49.3 MB (49296248 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; ppc64le

```console
$ docker pull debian@sha256:176aa8d96d0fb3b86c145b73747e98b75257bdae9c7705e4cb4e0ea5e9c7a7b8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53291710 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ee376e9fa1924d35d85784a11231def937501296151a5d0916eb74f9afd6828`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:41 GMT
ADD file:8700846eefa6ef0670106ef8055671479d597d057ee34d1d248dbefa5977753b in / 
# Wed, 12 Apr 2023 00:08:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c14c235d79599f644d6f952e975871669fe8a00e0434e5d26966f11cfb3820b`  
		Last Modified: Wed, 12 Apr 2023 00:13:39 GMT  
		Size: 53.3 MB (53291710 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; riscv64

```console
$ docker pull debian@sha256:0d8d75946de488e87624c5daf4d054add4d04ddb55788b81a731ebaac17b8e37
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.5 MB (45494949 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:abd7fe1b312c7840884f66a4746c98ad17e67096554c0d4818ee58366dd6d155`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:05 GMT
ADD file:b8c0cf42f791d92283c2c47cf2ed446fb98a7cde685c9b923c89cadb86ddad31 in / 
# Wed, 12 Apr 2023 00:09:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6178d7c3b8603b1579d72ad9f5bda8fd22627feccccd498dad946eddfe96d27a`  
		Last Modified: Wed, 12 Apr 2023 00:12:25 GMT  
		Size: 45.5 MB (45494949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; s390x

```console
$ docker pull debian@sha256:cdc1934036de517cc44338f5677c0fd96331c5b5cc8256ccc696531955ec67ed
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47665068 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:01bc014758913adcf3b452610f7f2c5f0807bcbb3ab426538e5b9256252d66ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:19 GMT
ADD file:0593fbca11f3d4a9b2b916ee99bb70651af04ea1275262f12e9b0e5c07e81f0e in / 
# Wed, 12 Apr 2023 00:01:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1844a6b5cc32e25703d3a6ac2115866157b1f856d7463d0dca8443fea51061f1`  
		Last Modified: Wed, 12 Apr 2023 00:05:31 GMT  
		Size: 47.7 MB (47665068 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20230502`

```console
$ docker pull debian@sha256:d975808df2ffd75d3cdc0a1c42eb802daace2e6aa2df3ffa25df2d4ec028a4e0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:sid-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:c69027723f16d6577b240522a2d604d1dbf2cb662882b77d315941da50f1a311
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167062 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98c21bede66ffd3027190f0259054b53b328186f8210970497e526d94cd74eee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:00 GMT
ADD file:2228a07c6ec82db1e0719684d77cecf448c3be947c2b1b48347ba5de9e5b57f8 in / 
# Tue, 02 May 2023 22:49:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90d5a5fdc15637fce928afc279fecd9adb82fe3c476f174458fc6bdd7fb4b57e`  
		Last Modified: Tue, 02 May 2023 22:51:47 GMT  
		Size: 47.2 MB (47167062 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20230502-slim`

```console
$ docker pull debian@sha256:e84d674d9b27ac86eceabeeadb35f1a4c9b37f6d0973a87535df3ec0b1785e76
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:sid-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:d5bf07d193654ded36eeeb69b18d7f19481ba6124b0095a07cb9b47d0a532f96
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954672 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:56d915171a5ca1d0370e19539ab5c95399416daf2e0e5ada0dcf5ef3dfc85746`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:05 GMT
ADD file:0526ed42aea39a43f70095df0c140cb7ddd5a9275934b74f217a2deacd8346dc in / 
# Tue, 02 May 2023 22:49:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:047688e6c84b2c1785dc0a838ee0444c71a049e525eb056025722b45ca8cefe8`  
		Last Modified: Tue, 02 May 2023 22:52:01 GMT  
		Size: 27.0 MB (26954672 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-slim`

```console
$ docker pull debian@sha256:5d62d231fdc0afe8d87640308badcc0c708c01f9d60ba69e660229f6db9e9212
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-slim` - linux; amd64

```console
$ docker pull debian@sha256:972ae154ecfe8945b20ef7ee7e49a9d030858c268c065c4ac86b0d1656d0bba3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29085852 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bb517ff3a6cee186635b986ca4adb8bb0ea5abda4b108e6067e645d7c75fbff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:03 GMT
ADD file:ff5d2effcc697f5b4c32f32c1934b7095f98094f13d7f4df5c470b3f2f7872bb in / 
# Wed, 12 Apr 2023 00:21:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6328121207020c132a5dc645a6eab3e0c63a79a02ef06a971fe043029cfcf14a`  
		Last Modified: Wed, 12 Apr 2023 00:25:25 GMT  
		Size: 29.1 MB (29085852 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:d5bf07d193654ded36eeeb69b18d7f19481ba6124b0095a07cb9b47d0a532f96
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954672 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:56d915171a5ca1d0370e19539ab5c95399416daf2e0e5ada0dcf5ef3dfc85746`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:05 GMT
ADD file:0526ed42aea39a43f70095df0c140cb7ddd5a9275934b74f217a2deacd8346dc in / 
# Tue, 02 May 2023 22:49:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:047688e6c84b2c1785dc0a838ee0444c71a049e525eb056025722b45ca8cefe8`  
		Last Modified: Tue, 02 May 2023 22:52:01 GMT  
		Size: 27.0 MB (26954672 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:bcde364b7720618e5ffdd00447ec44e426f4c1fe091c4a759b1e99cdce2707bb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.7 MB (25674764 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40983cedeae319b6af44bbec49c4b0bbe923ced7cb571a54d1672172cb24c6ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:44 GMT
ADD file:5ccc5dea93fe0d3153d28cbd20afaab662261df66b60f5adc7f13f6f7e097047 in / 
# Wed, 12 Apr 2023 00:00:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:60b11026112884d87c72be167d667ef7d712dad54a7b04be01be4fd54688b854`  
		Last Modified: Wed, 12 Apr 2023 00:04:58 GMT  
		Size: 25.7 MB (25674764 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:9f459999c74b63bafd456050d640d6cf4dd3cbbff710ba0e8b3a447b46e953ba
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124199 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6b6564e79616c0bbb79f13c12ffbdf5d906da330128bfd5184a3473fa3b7c8b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:27 GMT
ADD file:990a2f85a20da83b06e38036faa3b5eabdd81a07f18a044717b5bf0c9aee0109 in / 
# Wed, 12 Apr 2023 00:40:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b3cf74c8a7c792d88fc41a2fe6a175554da691b9fbfda375484de18a21bf5285`  
		Last Modified: Wed, 12 Apr 2023 00:44:09 GMT  
		Size: 29.1 MB (29124199 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; 386

```console
$ docker pull debian@sha256:7a91be310b422484a2a8f44e95eda28ad56e79e979403d6e328063d7d75acd19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30105661 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5345724d15033cf98ca442fc4d6ba9704c46367b26ad3ae1bd9e6c58ce9c870d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:00 GMT
ADD file:1b008b669e7cf817c87c7366a8dbe4b017437107b05ed18262df0daf7cbf236f in / 
# Wed, 12 Apr 2023 00:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bc26261a61943735c6b71a1a262b315ebdc5f0b0e89bea5e6b1dc0ad42f9e921`  
		Last Modified: Wed, 12 Apr 2023 00:44:38 GMT  
		Size: 30.1 MB (30105661 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; mips64le

```console
$ docker pull debian@sha256:e4e22fd5550abbc4ea0b48ea6e60807f799ea8c9fb9d3f4dd82562ebc4b94e7b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29090181 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b6f488c3898f59e1f677c036c85aaa48f00cb52d32f7d32860fd78ee00779da5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:53 GMT
ADD file:e572bf6726ccc75451d7aace942ee5bf2f22e589e29be1cc36f86b28a3519744 in / 
# Wed, 12 Apr 2023 00:10:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3d522c89c856f7b3fe175051ec11329b00b9ba08d815c862f27850282ede1a46`  
		Last Modified: Wed, 12 Apr 2023 00:18:39 GMT  
		Size: 29.1 MB (29090181 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:a05cbd8a0776061b3b6b17b34b405f6caf1f94b963798e5441027ddb524ba006
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33089561 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:87ca6661143d522477d1c741811e97b457f37b089bdf4604a158854860494bde`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:08:57 GMT
ADD file:113c8c660b5cd6ce6320839bc806c505c03c23001e9744949bc62e533b4f6143 in / 
# Wed, 12 Apr 2023 00:08:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:349f3146ce145e9eaf50e009ff2c94f3be5661608b9340ee0685793cfd8742aa`  
		Last Modified: Wed, 12 Apr 2023 00:13:58 GMT  
		Size: 33.1 MB (33089561 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; riscv64

```console
$ docker pull debian@sha256:160ca402b5e34ff1a49ded67ae39153d8db1b0c8fd527ee3b65fffe8b709748a
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25308749 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ace5964cad2c1f85529a383b1f7281f4235ec7e0b1c4cfd33f13ee6d8a84393`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:42 GMT
ADD file:4dad95e1fbbc2cf6dda6b4c4b8fd226529afa7b5c720d76f68c388a5c0b241a7 in / 
# Wed, 12 Apr 2023 00:09:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:629092fbb7924ad2a9377a0680a4f84137943ac748c768823052b9c0a173a00b`  
		Last Modified: Wed, 12 Apr 2023 00:13:02 GMT  
		Size: 25.3 MB (25308749 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; s390x

```console
$ docker pull debian@sha256:7065fa84e8bb53f3549fb453844c0f9621e1a89bd2a124e0bb53de5603d7b9ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27455523 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0cdc099d1105395694bd2541f3c6de3fc612a980f74d462044db8ee9f986b975`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:44 GMT
ADD file:794cbf74e625439f6a5b2d35ce6e6b1cb3933f6d8d01e420c564b33b3d91c00e in / 
# Wed, 12 Apr 2023 00:01:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a37d87d58986a0f8f4e3e0f3d22631070f0727636ca0ed335d4bcaccd2ccba3c`  
		Last Modified: Wed, 12 Apr 2023 00:05:39 GMT  
		Size: 27.5 MB (27455523 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable`

```console
$ docker pull debian@sha256:d8c2813b1401f6b31376ef2d4c92e54a9870203f9c075a14195ec27297ea70ee
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable` - linux; amd64

```console
$ docker pull debian@sha256:e822570981e13a6ef1efcf31870726fbd62e72d9abfdcf405a9d8f566e8d7028
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2e1dadc1b3b31522df019067c3543b4517daf72fe2a00960029eaa226aba28cf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:13 GMT
ADD file:369ac96d13405f257c2c05328a0e894fe34c37a4e674f6a51d3c82ac0842aa4f in / 
# Wed, 12 Apr 2023 00:21:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5942866a8d96513aac413957820a0f8951d44dbf27618c7c8d34b8b12221e7b1`  
		Last Modified: Wed, 12 Apr 2023 00:25:39 GMT  
		Size: 55.1 MB (55052738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v5

```console
$ docker pull debian@sha256:bc73527e5b11308d739ce399db2ad7a31877a806b7fe2eb6fe8be3a4c773f7b9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546555 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5b1ccf0248b60b1a48f1187f4326e95bb8e144c51de1feb902f011b99ad9ae50`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:13 GMT
ADD file:c83b51e96ccad1a3bcb3eb66359344f33591342477906e3c6126dc64c2c5e6cb in / 
# Tue, 02 May 2023 22:49:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8f5ba560106b14d65311aa93a26b6d47c0e2897dbd43f82d54c575c8a32e890`  
		Last Modified: Tue, 02 May 2023 22:52:18 GMT  
		Size: 52.5 MB (52546555 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v7

```console
$ docker pull debian@sha256:cbcbe3050391ff21c4dd31753dacffad64cd52b0b69d64658b25167e272c32e3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50216854 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:30fe416cc4c287f243df3b557482527bc70b5d29d322834c4f6a17f516bd8370`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:52 GMT
ADD file:b4bb2f1b3108de77905d6352b1c6446fcdeef536f43aec2500c5d09d92132028 in / 
# Wed, 12 Apr 2023 00:00:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a7c2aaabb66eb576474b0b044c66c12b46741d43d77e19973d3e55d34b10c67a`  
		Last Modified: Wed, 12 Apr 2023 00:05:13 GMT  
		Size: 50.2 MB (50216854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d977bb093bcd2109769b34569c5abaf9482ca9c963f1515baf68d2c530da9d6f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705532 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05511018cbc92a79d7757cb45dccde004e106fbabcb2337c6553c1c001f94e27`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:33 GMT
ADD file:03803c0d18496d0ccb6b3368180d3ad1c425a24cbe434db6378022c82fe1eca6 in / 
# Wed, 12 Apr 2023 00:40:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:de1216f34da7c14b36760b3a58f5c8126ba9aaec8968121ead56dbe5e190c612`  
		Last Modified: Wed, 12 Apr 2023 00:44:23 GMT  
		Size: 53.7 MB (53705532 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; 386

```console
$ docker pull debian@sha256:f6558183d9cc963acdabed67e1c28742c214535d88d280323b8afea82cb8d428
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56032798 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a2b4a7e9586d6a37ce5547ce9b5481d8754d56477f70820df4298415c063e065`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:10 GMT
ADD file:7fbd8b9cfbfe636d889b3ab4bc2720e1f25d78f590b245f407f52de7c435bfed in / 
# Wed, 12 Apr 2023 00:40:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2be01e4197de5042d4ff9d71d4067b99b9860f94b8a4c3a2694c3267add868ae`  
		Last Modified: Wed, 12 Apr 2023 00:44:56 GMT  
		Size: 56.0 MB (56032798 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; mips64le

```console
$ docker pull debian@sha256:13ee4a0bd38c94413c84bd7a107985dc44bb28e7581fa41f6204ac88b3d978a9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272033 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e08dcdbcc5b918766b02764ca24f84f8303460e7b52a3913c3a37aa98be9dd0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:11:26 GMT
ADD file:f97a8376973686baa0bc1882d725fd0017178340fa66fab6b56a5c55c85661e2 in / 
# Wed, 12 Apr 2023 00:11:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5554f032101bb415ee146e371c8c4e52f30d3407a547de63495133b10066fb48`  
		Last Modified: Wed, 12 Apr 2023 00:19:16 GMT  
		Size: 53.3 MB (53272033 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; ppc64le

```console
$ docker pull debian@sha256:c697c37c64207a7775d6fd0ac52732ecd67bbf56d72915704425018d31de2169
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926557 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6336ffc3fafdd4decbb199ce3baeca3c77bb4c26b74a30b2916b5b4a5650a949`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:15 GMT
ADD file:ddb1417647bba31787e1f4f459cfefcdfb20b3fe875b92e1d794de61461baaaf in / 
# Wed, 12 Apr 2023 00:09:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbd3b161013805c0fa5ce44a765b4d5e283b9478fec508653b4b0920631543e0`  
		Last Modified: Wed, 12 Apr 2023 00:14:20 GMT  
		Size: 58.9 MB (58926557 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; s390x

```console
$ docker pull debian@sha256:4c28dd48b3d8c1a41bd22fc26338501dbe350705242bc358124557126f2280be
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286637 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b57c56a18d757079707e2b652ddbadd41262874ff0cd4f5961aadea1d99d388a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:02:12 GMT
ADD file:d908f7342fbc7989444ac877c02229855ecdfb585dcfbaa86226e9e113a75619 in / 
# Wed, 12 Apr 2023 00:02:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:961df1f0f4d8c3e7504abcbd51340fff9c1714f466859ca0077ecbcd6367b5ff`  
		Last Modified: Wed, 12 Apr 2023 00:05:50 GMT  
		Size: 53.3 MB (53286637 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20230502`

```console
$ docker pull debian@sha256:b1f1a4afbb92849e6576deaec589fd1260b71e58306165fafe34356fabb70754
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:stable-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:bc73527e5b11308d739ce399db2ad7a31877a806b7fe2eb6fe8be3a4c773f7b9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546555 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5b1ccf0248b60b1a48f1187f4326e95bb8e144c51de1feb902f011b99ad9ae50`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:13 GMT
ADD file:c83b51e96ccad1a3bcb3eb66359344f33591342477906e3c6126dc64c2c5e6cb in / 
# Tue, 02 May 2023 22:49:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8f5ba560106b14d65311aa93a26b6d47c0e2897dbd43f82d54c575c8a32e890`  
		Last Modified: Tue, 02 May 2023 22:52:18 GMT  
		Size: 52.5 MB (52546555 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20230502-slim`

```console
$ docker pull debian@sha256:90dec235b983adbd179e5dc08bd2b4365257bbcc643882bd23176d7f83fa255f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:stable-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:eb66f790e02834b3eca130ee210bdb2c1b4e798dbfb6f2d1b50c288bfae947c7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903392 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:865d871c9306ee550bff1066fc7520a077deaf001061dde903caeebd737196f8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:20 GMT
ADD file:e981d0b82be4d60dd6a868ab0ac7b3c1218857a8fe92309060baf2195e028989 in / 
# Tue, 02 May 2023 22:49:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0ee1033593dd889e14494624762057ec40e0e857355fc742f808f46fcdf7ab1`  
		Last Modified: Tue, 02 May 2023 22:52:39 GMT  
		Size: 28.9 MB (28903392 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-backports`

```console
$ docker pull debian@sha256:e5b1392b07c1a075145c7317f7183cc04a1e3df201fe6e8d8d54a3d9c70988c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-backports` - linux; amd64

```console
$ docker pull debian@sha256:a8b478d2ce0f702ddc1093b4d04bcd04f6a9f3be0e34ecf5c877631f5d0882f7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55052960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:86b8c0e2f2c54bd4268bac939279c45e88e5cd1a6fca7e90d19df0e363c82bf9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:13 GMT
ADD file:369ac96d13405f257c2c05328a0e894fe34c37a4e674f6a51d3c82ac0842aa4f in / 
# Wed, 12 Apr 2023 00:21:13 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:21:16 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:5942866a8d96513aac413957820a0f8951d44dbf27618c7c8d34b8b12221e7b1`  
		Last Modified: Wed, 12 Apr 2023 00:25:39 GMT  
		Size: 55.1 MB (55052738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:948f98c248f7e4c2d3363b507dffaece6ab160937174f5019ee95ca4f78fd802`  
		Last Modified: Wed, 12 Apr 2023 00:25:47 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:341fd50108bd3aba0518d71cd1b6e70d9539727cfa48a6293a1741ab509eaa30
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52546777 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a4d1def12edd05ed53e354e5bb80dc1f617fd509263e921243835e4db6160988`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:13 GMT
ADD file:c83b51e96ccad1a3bcb3eb66359344f33591342477906e3c6126dc64c2c5e6cb in / 
# Tue, 02 May 2023 22:49:13 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:15 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b8f5ba560106b14d65311aa93a26b6d47c0e2897dbd43f82d54c575c8a32e890`  
		Last Modified: Tue, 02 May 2023 22:52:18 GMT  
		Size: 52.5 MB (52546555 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:46886077603280e7e7b77f0c493193070666352d589c920196d44a7527ee6b2c`  
		Last Modified: Tue, 02 May 2023 22:52:27 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:b8913013a649e8fcce53905decd52caa1e104967fce5ea6324d966d730af376c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50217077 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4fc58fa78e370331ba61d1e9bf43a51a31c4ddd39b17046e633e9d91adaac116`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:00:52 GMT
ADD file:b4bb2f1b3108de77905d6352b1c6446fcdeef536f43aec2500c5d09d92132028 in / 
# Wed, 12 Apr 2023 00:00:53 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:00:55 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a7c2aaabb66eb576474b0b044c66c12b46741d43d77e19973d3e55d34b10c67a`  
		Last Modified: Wed, 12 Apr 2023 00:05:13 GMT  
		Size: 50.2 MB (50216854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bc57f82c11da0c26157ea79613d644ddeeaabe938e1bfe7af49b02924e07f8c6`  
		Last Modified: Wed, 12 Apr 2023 00:05:21 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:117d9a95193164781addd93e07b80757a11c5a47d6bd7a01da2ba378aa88d880
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53705753 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d64a0b35dfbc8dd7213d58084e6aeb18f7a3557163e9eb80f3d5c8e6287ebf75`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:33 GMT
ADD file:03803c0d18496d0ccb6b3368180d3ad1c425a24cbe434db6378022c82fe1eca6 in / 
# Wed, 12 Apr 2023 00:40:33 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:40:35 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:de1216f34da7c14b36760b3a58f5c8126ba9aaec8968121ead56dbe5e190c612`  
		Last Modified: Wed, 12 Apr 2023 00:44:23 GMT  
		Size: 53.7 MB (53705532 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:fe25d6b4cc40c513fd4f1d0b928e5c62e5a85179aa98ce950577fafc360aabac`  
		Last Modified: Wed, 12 Apr 2023 00:44:32 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; 386

```console
$ docker pull debian@sha256:28945dd054c5ab54493854ae48d6e0448ad3d7d42127656003323b8bb796a041
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56033018 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc7b0653c4fd8be1ce9a232435c6f2f0f2b263a39bfa9bd2a5a040fa744cdddd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:10 GMT
ADD file:7fbd8b9cfbfe636d889b3ab4bc2720e1f25d78f590b245f407f52de7c435bfed in / 
# Wed, 12 Apr 2023 00:40:11 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:40:13 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2be01e4197de5042d4ff9d71d4067b99b9860f94b8a4c3a2694c3267add868ae`  
		Last Modified: Wed, 12 Apr 2023 00:44:56 GMT  
		Size: 56.0 MB (56032798 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6a428d8dda767a782952b231a9d9b0be0dc970949dc251998826634a0704c1ae`  
		Last Modified: Wed, 12 Apr 2023 00:45:06 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:bd7f8f2026f5fb36a16d95b7a6140f46f5b1bc9abf71cda7161b0da4ff72a248
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272259 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:81163808fcd93cdd496a696edd78399b69e0c9391958792dafff2166a040e93a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:11:26 GMT
ADD file:f97a8376973686baa0bc1882d725fd0017178340fa66fab6b56a5c55c85661e2 in / 
# Wed, 12 Apr 2023 00:11:31 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:11:40 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:5554f032101bb415ee146e371c8c4e52f30d3407a547de63495133b10066fb48`  
		Last Modified: Wed, 12 Apr 2023 00:19:16 GMT  
		Size: 53.3 MB (53272033 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d9347bda0e43c709344b14c37967995b8fc8e12bd9d6c303a23aaf528beb3ad5`  
		Last Modified: Wed, 12 Apr 2023 00:19:26 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:5b86aef5581f3d0dfd76c4de1f4e2afdb5e659fb55f563079dd313fc0d6b44a0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58926780 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a9da829afe3fb40a3c68dc940af50992321f5dfd2b9cc2a40ffc089181882398`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:15 GMT
ADD file:ddb1417647bba31787e1f4f459cfefcdfb20b3fe875b92e1d794de61461baaaf in / 
# Wed, 12 Apr 2023 00:09:19 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:09:25 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:dbd3b161013805c0fa5ce44a765b4d5e283b9478fec508653b4b0920631543e0`  
		Last Modified: Wed, 12 Apr 2023 00:14:20 GMT  
		Size: 58.9 MB (58926557 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e8850d6e5f5f15afbcaa695be9f33696172febf3b641f74ed3d6b344bae1eaa9`  
		Last Modified: Wed, 12 Apr 2023 00:14:29 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; s390x

```console
$ docker pull debian@sha256:28906673e3c556c941f22beec0fb48999fbc973af331ab89ded0c699de79c6f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53286862 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a9ca89831cd75c91f61e181c73bfb1a3274844d01b88fb439dc949892dd6fb26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:02:12 GMT
ADD file:d908f7342fbc7989444ac877c02229855ecdfb585dcfbaa86226e9e113a75619 in / 
# Wed, 12 Apr 2023 00:02:21 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:02:29 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:961df1f0f4d8c3e7504abcbd51340fff9c1714f466859ca0077ecbcd6367b5ff`  
		Last Modified: Wed, 12 Apr 2023 00:05:50 GMT  
		Size: 53.3 MB (53286637 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a8c9e7a2d5db75aa25aff8e34e39b762190553c497a302af8a391c56c8ac8336`  
		Last Modified: Wed, 12 Apr 2023 00:05:56 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-slim`

```console
$ docker pull debian@sha256:96562bbaf8781b4b948f7a194c804a1e1f76e10dfb6938283fc1da480fcf18b3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-slim` - linux; amd64

```console
$ docker pull debian@sha256:43c7feb5953d47815b7d85d1a3632fc294630136578545f29ba61bb5b8406572
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31418239 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf13a660e07b72a6fed20df1f89ddc822e7721cb4ce6032882d7f0a034ee15bc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:23 GMT
ADD file:237197090b8f6707571c747a0c53e0cafa69eb4ce9768345b8e929099011273a in / 
# Wed, 12 Apr 2023 00:21:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5345eaa127c70e11c48634a94d551b967bc9541c91ee82af09a2350ac7b3ad07`  
		Last Modified: Wed, 12 Apr 2023 00:25:57 GMT  
		Size: 31.4 MB (31418239 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:eb66f790e02834b3eca130ee210bdb2c1b4e798dbfb6f2d1b50c288bfae947c7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28903392 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:865d871c9306ee550bff1066fc7520a077deaf001061dde903caeebd737196f8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:20 GMT
ADD file:e981d0b82be4d60dd6a868ab0ac7b3c1218857a8fe92309060baf2195e028989 in / 
# Tue, 02 May 2023 22:49:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0ee1033593dd889e14494624762057ec40e0e857355fc742f808f46fcdf7ab1`  
		Last Modified: Tue, 02 May 2023 22:52:39 GMT  
		Size: 28.9 MB (28903392 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:7c89baa4260405fb5d874532db50721b39fdd34dd039622be3c5988f23208b08
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26579818 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc6e2f2a89d5027037873919cda33a72cb94969cc7a5c325b64481797294d118`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:01 GMT
ADD file:eb18e096f4d9891f6b19b3dbef34fa5c44a0041a3c27525b38949d80812e7521 in / 
# Wed, 12 Apr 2023 00:01:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:584e613c7f79d0c9601049d58fb191712b597947276a1543a2618ea27d6b8aec`  
		Last Modified: Wed, 12 Apr 2023 00:05:30 GMT  
		Size: 26.6 MB (26579818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e9739b2eb6a410c953e5e79a2b99cf0f4b61dbf9437e625ee34f531fa5692b19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30063832 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4003686d1b690c57153f69e9ee4870c14dc87eda67beffb810fdd6b15e17620d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:40 GMT
ADD file:68fa22321500568bc827e9da89007707e67e5794fcbd0f4fbaa444814fbce23c in / 
# Wed, 12 Apr 2023 00:40:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6e938e33b6c46653da7c54fea5005f84f8c6537b615b6989bed1e8bbb9a6d8d4`  
		Last Modified: Wed, 12 Apr 2023 00:44:40 GMT  
		Size: 30.1 MB (30063832 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; 386

```console
$ docker pull debian@sha256:dda43ad8a0e2999af8c13ca5d62d5aef8cc6a90713f650808d261c580bfcbc3c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32398924 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:491446bb82c16b0bce7325864cc85d157e32136409068d08047c8186b6cb1d93`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:21 GMT
ADD file:440849d8fd3eba731707f1775bbad360a10822443594b8cce06faa53807e7c46 in / 
# Wed, 12 Apr 2023 00:40:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:26a2e03a1e5f6219549948f025f3205d2dda1055b0e80c7726bcac7ed062be0a`  
		Last Modified: Wed, 12 Apr 2023 00:45:18 GMT  
		Size: 32.4 MB (32398924 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:59b425da73ec207e1e1ec295ae65112cedd7c517c161d0902fbbe00198651f86
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29639240 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:94a244254eb91434081af3241cbf32486ac5a5b1f340be641bf71ae934a79e3a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:11:59 GMT
ADD file:5b9a0b8183665b7a3c26a87da4f256db7a1217c9b8e799e9dfca8d46174fc94a in / 
# Wed, 12 Apr 2023 00:12:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:277811b72c7b61b04c10b3f1dd3275a7023e1e86d93fa5e2500ed8c4acd95159`  
		Last Modified: Wed, 12 Apr 2023 00:19:50 GMT  
		Size: 29.6 MB (29639240 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:4b1eb8c0f70b764af673846751d48baa63fdc2bb9381da100e475d22f6250ed6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35292003 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:be20188e63360269c5a2a817273de94a256a0e6a4c31a6c0d481419549009ced`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:09:37 GMT
ADD file:21faa3899a486370cc06c549bbdbce50c65662c150b5975bc2462a6579792638 in / 
# Wed, 12 Apr 2023 00:09:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8ceab1de5e6e0cca17d9dcfbb5bf87871ba476c30ebec146e59397317900a82f`  
		Last Modified: Wed, 12 Apr 2023 00:14:44 GMT  
		Size: 35.3 MB (35292003 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; s390x

```console
$ docker pull debian@sha256:ae2cfc666321e6fed15119168eebd1edb53cac82b612e412e94066dbe2a8cfac
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29653130 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c630d299140c601d308e7c6a703cebb22a41d2d70f315dc8d110daeeffbcec9d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:02:41 GMT
ADD file:43a651e51b66ec23ea5e173b36ea5bafa78526fd703666b460abdacfeb9c593c in / 
# Wed, 12 Apr 2023 00:02:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1e544bd238ef4999bbaa499325584b595f113868bca36d4ecc3d237d103522a0`  
		Last Modified: Wed, 12 Apr 2023 00:06:03 GMT  
		Size: 29.7 MB (29653130 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing`

```console
$ docker pull debian@sha256:ade8426f1f6efdcb9eec8dcb6b759934ba96cd61cafdaf15e3c440c2766c8ac8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:6ecb5d7ca505d7e566fd9c9718739877a83dddb7f6824d7249ae63f82ebac957
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49292939 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c5adbfb05a0be13ad74e2c045927df76228fd1cad7f5a86837f56fa2b8921bfe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:33 GMT
ADD file:28aeff48a52e156907bf710c1e3cd7ac329d157f36816ca755bcea5177269765 in / 
# Wed, 12 Apr 2023 00:21:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c6654873ad3f068a7ff34cc7f44a13003d6eb6f2261ac42d1cac07ac8ed4e0e`  
		Last Modified: Wed, 12 Apr 2023 00:26:11 GMT  
		Size: 49.3 MB (49292939 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:f6e265fa260da897c4274f647129cde2c69eb0ad075b554fb24e3f8e8d83ef93
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167198 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:56bd5b1afad695e8c81036325c3c8405ac2704e472e18ab0cfec6773555c84b0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:27 GMT
ADD file:e453aa4ddf2f5bfa2412dd44d2457067e34bc3a8fc8f811568b37ee5c39828a3 in / 
# Tue, 02 May 2023 22:49:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e2772b4d1fc0f0569b1a8bf114666e528e1e520ff336a1c359016b38622649c3`  
		Last Modified: Tue, 02 May 2023 22:52:54 GMT  
		Size: 47.2 MB (47167198 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:b4b1b3722537aa21e91c6ca26b036b4ca79099aaa7819baeb305898bb4415dbb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45883115 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9de5a5136ca158804b99b6ac6d274a6d453a16f8dfb6f94fc0b0e18eb53a6ac6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:11 GMT
ADD file:644c1d3ee017b7c76344e5dba5070a3e6fdcaaee9c73c7533f5a8cec8df4c6a2 in / 
# Wed, 12 Apr 2023 00:01:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:df24a1290fa20d99e5808e99b1da79324d929f1dd276b232f75c3f5ba9fca816`  
		Last Modified: Wed, 12 Apr 2023 00:05:44 GMT  
		Size: 45.9 MB (45883115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2374b004eb9e572610eb426c9cdef3f34cd34ead3513c2f4305a91d8b5d57613
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49345366 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aac13f87c559dd79fdc9f42c3b931117d1212b4c31c2123b69603f36b0a8af91`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:46 GMT
ADD file:80a4d0d891f9f28bd3948de18579583b6c11a4a8805b6230daaa0ac2a35c453d in / 
# Wed, 12 Apr 2023 00:40:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d1d33b0463b9cb723855fca4aff8cdc62c34a1907b920e8e626bd38d7577387f`  
		Last Modified: Wed, 12 Apr 2023 00:44:53 GMT  
		Size: 49.3 MB (49345366 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:8ce39333f17ec36cc61ca6a2dc03874deeccc6bb4e0b2eb94eadb99d57cdd1e6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50317978 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a0d13dfd495b32d270911f665157ce65cf3367eb520251ff841d4fab07fd227`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:30 GMT
ADD file:2ad0fad15fbda4c0b699b4aaa14445b0322d2e2271a4b3fbd040b3e84b7d2ce6 in / 
# Wed, 12 Apr 2023 00:40:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:61d509c5edb6b6fdcff4f76f6dc24de7a3b1ff8b4c805019cb42cc6720e0ba9a`  
		Last Modified: Wed, 12 Apr 2023 00:45:36 GMT  
		Size: 50.3 MB (50317978 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:3a48e3839e8e00dd3023bcc66501f7e102bf8a638651247e37ffcab99f7a6ee8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49299323 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ffd055efc1413cc93fb4563835bfa9b689c7acaef6c883780d4545f146f9ea4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:12:29 GMT
ADD file:c8ac2734abc43d5debc82cde3d350a41bbdb9839a979a975bcf3c0463e954234 in / 
# Wed, 12 Apr 2023 00:12:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:49cd33eb75d2a83e7f320ce1efc4ac9f56141ad9006032934badc3c04ca429d6`  
		Last Modified: Wed, 12 Apr 2023 00:20:27 GMT  
		Size: 49.3 MB (49299323 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:6c9225bc476435558509ad8951d8ae8d84d97f85d43be21ce4a7c844c41491e1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53299985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb5674fe3ec92911f6bec18240c6d5494cfad83735f824d632fec6703622984f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:00 GMT
ADD file:107d4f159f2bc5b04467e08852422f5c78548e0f92853bd29d5d2b05b387d3a9 in / 
# Wed, 12 Apr 2023 00:10:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a4fca5aa8bf3f9957686fb6705daa645629edb94592f35c71e46e52a1742ed1a`  
		Last Modified: Wed, 12 Apr 2023 00:15:06 GMT  
		Size: 53.3 MB (53299985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:96517a504e773d553fcec768b8c90f34fb0e6ab42ca62dd1dd0ce8cae2af4ef0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47670408 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:24e1220a808c7f422e58ed33150b13013e10d5aab427ad888797476a7b240a96`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:02:57 GMT
ADD file:a04191f521f5135016e837ec503a63f4e8488c1a13030ba67c96cc8f68da9d50 in / 
# Wed, 12 Apr 2023 00:03:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0d66e85e2f078d0a3241d6085f175f28d3ed3f6304a2561cc20fd703234bd4bc`  
		Last Modified: Wed, 12 Apr 2023 00:06:14 GMT  
		Size: 47.7 MB (47670408 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20230502`

```console
$ docker pull debian@sha256:c08a7c252ff7c087fb97d5cd6d4360c578157119932c69055852c50f05da1507
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:testing-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:f6e265fa260da897c4274f647129cde2c69eb0ad075b554fb24e3f8e8d83ef93
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167198 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:56bd5b1afad695e8c81036325c3c8405ac2704e472e18ab0cfec6773555c84b0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:27 GMT
ADD file:e453aa4ddf2f5bfa2412dd44d2457067e34bc3a8fc8f811568b37ee5c39828a3 in / 
# Tue, 02 May 2023 22:49:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e2772b4d1fc0f0569b1a8bf114666e528e1e520ff336a1c359016b38622649c3`  
		Last Modified: Tue, 02 May 2023 22:52:54 GMT  
		Size: 47.2 MB (47167198 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20230502-slim`

```console
$ docker pull debian@sha256:a8cf9f5ebd9207cef047053cd79799b2b3d00c3ae129dfa7b687f4db4b43fd11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:testing-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b4b6724776f37205c2be5eb33047d28e688739caaef0328f70e2ef6a37b648b5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6dc9478268101dc61e1ca84bc9091d2dc3c4f33eb46d467c3d8f754770b9259`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:35 GMT
ADD file:c20890edbc5451260151fc2d2bb7ae5eb0c939c85cca989264cac6284204644c in / 
# Tue, 02 May 2023 22:49:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b89b2bf5d1a66c128ed2bc9fc16a9d7a8ce58411435b33e2856e2bf02086eb31`  
		Last Modified: Tue, 02 May 2023 22:53:13 GMT  
		Size: 27.0 MB (26954740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-backports`

```console
$ docker pull debian@sha256:5bc6880830212fb1df77d34f6bc0c70df08da363c3feee724243b191d005f2e0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-backports` - linux; amd64

```console
$ docker pull debian@sha256:fe19c4b2d059c9ba36f599f56cc788286df0ba9b78325c71c64439ef99a9c360
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49293163 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:608a475ba8f8dced199c0e628ef0b6bfced620469d35bca538293a8c3a4ddabe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:33 GMT
ADD file:28aeff48a52e156907bf710c1e3cd7ac329d157f36816ca755bcea5177269765 in / 
# Wed, 12 Apr 2023 00:21:33 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:21:36 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1c6654873ad3f068a7ff34cc7f44a13003d6eb6f2261ac42d1cac07ac8ed4e0e`  
		Last Modified: Wed, 12 Apr 2023 00:26:11 GMT  
		Size: 49.3 MB (49292939 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0bf6486178d1f01e7f1887374d4f74962578798dd9a755349d4b4bfd7e78af93`  
		Last Modified: Wed, 12 Apr 2023 00:26:21 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:918d74a55e77e38f689b8a89a5432933f9800414607a7e62dc4db75df8006d06
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167421 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af579121f47265597149cf1d4b3096174818eb5633881e970472aba2eaa2e20a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:27 GMT
ADD file:e453aa4ddf2f5bfa2412dd44d2457067e34bc3a8fc8f811568b37ee5c39828a3 in / 
# Tue, 02 May 2023 22:49:28 GMT
CMD ["bash"]
# Tue, 02 May 2023 22:49:30 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e2772b4d1fc0f0569b1a8bf114666e528e1e520ff336a1c359016b38622649c3`  
		Last Modified: Tue, 02 May 2023 22:52:54 GMT  
		Size: 47.2 MB (47167198 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8fdf7669d81f1ef2de32d6f289764ea41019e0224e0eabdb0c99df38b94bdf02`  
		Last Modified: Tue, 02 May 2023 22:53:02 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:5ebb536b168ceae4727bc22e09fee738989afab42dbb3955a2d607ecf587717d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45883337 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9f49c7e6757bfcf5dd6bd0426b8037236fbea6d680c5f0f6f4cffd7b0d5e5f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:11 GMT
ADD file:644c1d3ee017b7c76344e5dba5070a3e6fdcaaee9c73c7533f5a8cec8df4c6a2 in / 
# Wed, 12 Apr 2023 00:01:11 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:01:13 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:df24a1290fa20d99e5808e99b1da79324d929f1dd276b232f75c3f5ba9fca816`  
		Last Modified: Wed, 12 Apr 2023 00:05:44 GMT  
		Size: 45.9 MB (45883115 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c42d26ea6848e711a92bf0082d3e5ec7ee9283a4363324184899219dd26d43bd`  
		Last Modified: Wed, 12 Apr 2023 00:05:52 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b958a8b8c944c4c932a8809c5c9761efe58c0aa9c8fc20798933b3418b50b86f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49345587 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:df509ee94bba241f03867afa48de9f523a716eb39d96422153ed75c2a38844a6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:46 GMT
ADD file:80a4d0d891f9f28bd3948de18579583b6c11a4a8805b6230daaa0ac2a35c453d in / 
# Wed, 12 Apr 2023 00:40:46 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:40:49 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d1d33b0463b9cb723855fca4aff8cdc62c34a1907b920e8e626bd38d7577387f`  
		Last Modified: Wed, 12 Apr 2023 00:44:53 GMT  
		Size: 49.3 MB (49345366 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:824085549218ea0d8a075e376c040022cb0bdb5d2c70f355da9f5b74fdc5d64e`  
		Last Modified: Wed, 12 Apr 2023 00:45:01 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; 386

```console
$ docker pull debian@sha256:7edf9294de1de7b9ad7f6d3c42c534cc8bdb5b8d1f48aede33b65fe2f1ed2b23
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50318202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4194f8228761d780442e0a8debaf02abb06e7bcd6a1f08b3e2d9dff716a15a43`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:30 GMT
ADD file:2ad0fad15fbda4c0b699b4aaa14445b0322d2e2271a4b3fbd040b3e84b7d2ce6 in / 
# Wed, 12 Apr 2023 00:40:31 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:40:34 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:61d509c5edb6b6fdcff4f76f6dc24de7a3b1ff8b4c805019cb42cc6720e0ba9a`  
		Last Modified: Wed, 12 Apr 2023 00:45:36 GMT  
		Size: 50.3 MB (50317978 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:186698fed537745de84b7a675eeef7ba18d97de9b88d00d2cc057ad83d114324`  
		Last Modified: Wed, 12 Apr 2023 00:45:44 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; mips64le

```console
$ docker pull debian@sha256:8f9cba228d7b0642ff42b1ebc812027d7c394ecfa5886abcbdf4b42812a1fc4e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49299545 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b8a82a9a8007b3b23b73a4f3bc93e2875334105d537eaba6f0c076dd410506e1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:12:29 GMT
ADD file:c8ac2734abc43d5debc82cde3d350a41bbdb9839a979a975bcf3c0463e954234 in / 
# Wed, 12 Apr 2023 00:12:34 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:12:43 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:49cd33eb75d2a83e7f320ce1efc4ac9f56141ad9006032934badc3c04ca429d6`  
		Last Modified: Wed, 12 Apr 2023 00:20:27 GMT  
		Size: 49.3 MB (49299323 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ec1d6af93b238ad0986328bb1deb431d254d2df5577559183b3058577a60e634`  
		Last Modified: Wed, 12 Apr 2023 00:20:37 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:96d7b54f0268962c06529af3d7a7e5ff47172d33d6bd7cef62f240b57d688561
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53300211 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:46d22bfb580d41af378c86caac475266f2e64c5d9d53d80df106cbbdac63e0eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:00 GMT
ADD file:107d4f159f2bc5b04467e08852422f5c78548e0f92853bd29d5d2b05b387d3a9 in / 
# Wed, 12 Apr 2023 00:10:03 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:10:09 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a4fca5aa8bf3f9957686fb6705daa645629edb94592f35c71e46e52a1742ed1a`  
		Last Modified: Wed, 12 Apr 2023 00:15:06 GMT  
		Size: 53.3 MB (53299985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:06a64bf96cf803ffe2505fbac1e0c249266e23f4901c9f082ece5ea0af9c8361`  
		Last Modified: Wed, 12 Apr 2023 00:15:15 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; s390x

```console
$ docker pull debian@sha256:8ac017a8c6eb467c9ac5140e50fb9481d203fc191eddc7de9142f439c8ca8e71
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47670629 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:97ccb3922ba228fb9cb3d2d4b1ad27f87fe6a2301c2c337986cf5610d264c207`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:02:57 GMT
ADD file:a04191f521f5135016e837ec503a63f4e8488c1a13030ba67c96cc8f68da9d50 in / 
# Wed, 12 Apr 2023 00:03:03 GMT
CMD ["bash"]
# Wed, 12 Apr 2023 00:03:11 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0d66e85e2f078d0a3241d6085f175f28d3ed3f6304a2561cc20fd703234bd4bc`  
		Last Modified: Wed, 12 Apr 2023 00:06:14 GMT  
		Size: 47.7 MB (47670408 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:014d306832faddd6ad34c518e489793391a11865755e5a2f833e5cb6269f39c1`  
		Last Modified: Wed, 12 Apr 2023 00:06:18 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-slim`

```console
$ docker pull debian@sha256:4a2c667b1d0b5ec569398cbcebd6f3c1f5338a2add17189d33d53b2f8340bb13
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-slim` - linux; amd64

```console
$ docker pull debian@sha256:1c451a38f6d5ef4f60569758c7558b61288b9d6c1985daa05863c8b8093116e2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29082377 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:42e882fa5dceca9ec6e2507f7296174668cee5366003af6d28654c1c98c10dea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:43 GMT
ADD file:013a899e801ca2c7d722c44aa617208ffdbb4838abb9cbbc978a03e5bbd2b357 in / 
# Wed, 12 Apr 2023 00:21:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:39f2c55256eb69b7c7679c94fba7e681db2fec00bf4ecd12c3bf709cf04e0289`  
		Last Modified: Wed, 12 Apr 2023 00:26:31 GMT  
		Size: 29.1 MB (29082377 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b4b6724776f37205c2be5eb33047d28e688739caaef0328f70e2ef6a37b648b5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6dc9478268101dc61e1ca84bc9091d2dc3c4f33eb46d467c3d8f754770b9259`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:35 GMT
ADD file:c20890edbc5451260151fc2d2bb7ae5eb0c939c85cca989264cac6284204644c in / 
# Tue, 02 May 2023 22:49:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b89b2bf5d1a66c128ed2bc9fc16a9d7a8ce58411435b33e2856e2bf02086eb31`  
		Last Modified: Tue, 02 May 2023 22:53:13 GMT  
		Size: 27.0 MB (26954740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:066b53e86faf3a64760e22ec170cf93efd98d93ad8fd8500eb94faf04bcdafea
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.7 MB (25672982 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b9ea28a31a3f0d2116f042185d20b2af08899e3ecf7dc0435044b8666d41f53b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:20 GMT
ADD file:8e106f0fed69534d9ba2ffc546d84766880e78e9610e0d20474885207accbcd1 in / 
# Wed, 12 Apr 2023 00:01:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3f1c37377b6f8737af588dc2136663a886406262531acdedf229188e45dfc84a`  
		Last Modified: Wed, 12 Apr 2023 00:06:01 GMT  
		Size: 25.7 MB (25672982 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7536671e32a816c242aabd7ee8d8a4a150f6132c5b63424981a5291166e0e0ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29121295 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c403544f454b21f6dfa3713e6701bc819d9757d45d100f5e25ac52c9a556d6c3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:53 GMT
ADD file:e09acba12c8b5f7891de7b7b00702b187665c3ad8757451f11fb037471aa8622 in / 
# Wed, 12 Apr 2023 00:40:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3c8cd0a00d7c6c8a270e1cac39ff8f8cfa791a13b9be04c25bbfaa0e130302ba`  
		Last Modified: Wed, 12 Apr 2023 00:45:10 GMT  
		Size: 29.1 MB (29121295 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; 386

```console
$ docker pull debian@sha256:32ea0bd2a987701d5d56855e2040043db252454b4d3a600942981c27e4e6f692
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30102773 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d22d177212cdcbf1f8269de1856d86326b1bea007990078c7f2432bddd9c98f3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:41 GMT
ADD file:52529237e3f5873fc36d8c90c48c34f939bf74c866630b91605610ff7bbd6c8b in / 
# Wed, 12 Apr 2023 00:40:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:884586c66a2d1b6b6259454174a98e0a591d998572d4eb4107425edd62eaef9c`  
		Last Modified: Wed, 12 Apr 2023 00:45:55 GMT  
		Size: 30.1 MB (30102773 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; mips64le

```console
$ docker pull debian@sha256:e5ab64d7f45ade904d669053cfd374ae121de65f06791d821f73a9cdf1df00f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29089381 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3f6359e4a127850c778e378b79521660968cabdce13246a0a9fbd1ecd060dabf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:13:02 GMT
ADD file:87f6f8dab95995644c4557783207d44c12b837de729d356b6b649dec12ba6b98 in / 
# Wed, 12 Apr 2023 00:13:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0faf3f2704de85c89923cad7750eb41f6dfecd7460bb07bccce6c810f50f28aa`  
		Last Modified: Wed, 12 Apr 2023 00:21:03 GMT  
		Size: 29.1 MB (29089381 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:a8f0e338442d036df495d7233e8d1c4a26a238923ce9cb685b2b190a48cfc961
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33085435 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f9f58e23c43468e869cc30d5e1c5910351ec933d3fc3fde8b782ff049c2bd7e9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:21 GMT
ADD file:c093c3afcbc27fa38a4eca0faccad07c223f8ea887616ce0b3f508593fbeef0f in / 
# Wed, 12 Apr 2023 00:10:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d6100968c6918002dd747fb6324bbf250d3c3fed106a853a752192918d4b3704`  
		Last Modified: Wed, 12 Apr 2023 00:15:30 GMT  
		Size: 33.1 MB (33085435 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; s390x

```console
$ docker pull debian@sha256:477a870f0745d5d49ec1b1aaf0cfdd6125a71a9a5e6dd6a53f92f5c273f17c7d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27458286 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8a478e6d3cf1b0845d55c8b863d77aaf54433378cb0c81671dcc6681cf79be2e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:03:20 GMT
ADD file:6a9ca12ae14ef72bb2c7961a3ba0b84393c4e06821819973b871db798bfe8e9b in / 
# Wed, 12 Apr 2023 00:03:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d3081b0b47b8cbff56d6bc9c04a24946e2386ede16632f840f10b98e7858ad13`  
		Last Modified: Wed, 12 Apr 2023 00:06:25 GMT  
		Size: 27.5 MB (27458286 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable`

```console
$ docker pull debian@sha256:4f9c9b9522611d60dbd31465638d25c95c08201dca17a70cd29c0bbb1808065d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable` - linux; amd64

```console
$ docker pull debian@sha256:c266c672c7939ce8a4caf04549e7d282d03f551389eeb0dd70c380561ff9a777
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49287657 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:46289975b87073a1df24dabe3df7eebb8225ad665189c7189e1c28387fad90b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:21:52 GMT
ADD file:eb948b4bfe0963071c224791a81e9295fa552632c5b185f143e3c42c071c4671 in / 
# Wed, 12 Apr 2023 00:21:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:277078978190188dc2be36cde72743cfd4b19903cf7c90ceef8421d87590db29`  
		Last Modified: Wed, 12 Apr 2023 00:26:45 GMT  
		Size: 49.3 MB (49287657 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:6ced5ad5c3c2d37b702fcaebc3ff40c26407d4da5531773ad9cad5029d4401bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167066 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c731822d63f9380f1493d8187b05ea8e1391243260820b2a4cdb2d35bd02f3a8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:42 GMT
ADD file:740dbf70289faed5b254bb33009aaddef87ef73abcfdc2441b61e0be62ac6d62 in / 
# Tue, 02 May 2023 22:49:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c7b5fff53169be147228f50722828c1946b9253a79b37d14d755411593e715be`  
		Last Modified: Tue, 02 May 2023 22:53:28 GMT  
		Size: 47.2 MB (47167066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:b53bfd84aaec69627ea62a342a0b9d77c7ff4e481feb9f287c020c2ad0540f22
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45890238 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:46ece9feae0b684f7624fd9c3b830b855fd4ada57cfb406e4446313f76917a78`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:29 GMT
ADD file:a85f9bd069b317fe04f7b0e5e2a6e54a4a94ed304e2487154c520f160f81634f in / 
# Wed, 12 Apr 2023 00:01:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:54d799b49c2235300dac823b010337407851814391945952bc9cbe2adfc39d3c`  
		Last Modified: Wed, 12 Apr 2023 00:06:16 GMT  
		Size: 45.9 MB (45890238 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:99db3a8bb26ee4699db9c6aaab875a13b7b0ab638b2b35678e71b0d8ea7d7f74
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49327659 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ea3e0d7096a02c23dc11d2924a89f576b190bf0e5a322159fc4852a918dfc4c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:59 GMT
ADD file:eed6ca617d18b91908df79e437985b8da5417ffb4eb0c52b6cec20e46c00414a in / 
# Wed, 12 Apr 2023 00:41:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a890b97e54717f3d2b342929339a71068f93c352460bf767e21a39e90688b79`  
		Last Modified: Wed, 12 Apr 2023 00:45:24 GMT  
		Size: 49.3 MB (49327659 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; 386

```console
$ docker pull debian@sha256:b166ac72be1a008e12e90ed0ef45430f964afaa8d57a986b6b1b65879e406936
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.3 MB (50310902 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96fb04cde1871d58f396c1c7ecdf2bd9c7afeaa5234d130b3556efb26919d0ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:50 GMT
ADD file:846fb6727047101baf611eb5e752306b9f0d603546da67314cb4ef21e2f1f54e in / 
# Wed, 12 Apr 2023 00:40:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:308acd63a4eaf4bc5f65e274ce996d9cdbcd66aa3e1fce97faf327125eb14e0a`  
		Last Modified: Wed, 12 Apr 2023 00:46:12 GMT  
		Size: 50.3 MB (50310902 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; mips64le

```console
$ docker pull debian@sha256:23e2d1014502afb754baed42e74720ab7acbbb2a47e2c008908ffd506bbfd395
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.3 MB (49296251 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:656cf7271750d799d53972aa76197764b6ccc3f0cd7e111c59e85954941c7098`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:13:36 GMT
ADD file:7175a1b08b8243bc539cc1f758d85ccbe24afbc3df6a1f629c7a43839aee5f62 in / 
# Wed, 12 Apr 2023 00:13:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:589b4c3c9d360e937d4513f3ec8af4982019090cf0f68fb06e1e275c06591a76`  
		Last Modified: Wed, 12 Apr 2023 00:21:40 GMT  
		Size: 49.3 MB (49296251 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; ppc64le

```console
$ docker pull debian@sha256:af63afe0a513150de1de9e73bcf9dbfa284d1fa8bde8442a719be79beca60d19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53291712 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3ab57c942338d733748808c79148ed3c193211e7febb1ebb6d3f8d185848fef0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:38 GMT
ADD file:930aae2ab6be5d8295a31bf5dd91eee5ee4fa92a08b6159f55cdc6d3bd182e67 in / 
# Wed, 12 Apr 2023 00:10:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4e67fd8b6f31e9a06c3401edfed958df31a0ae78d0692baa41a3d87074423b51`  
		Last Modified: Wed, 12 Apr 2023 00:15:52 GMT  
		Size: 53.3 MB (53291712 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; riscv64

```console
$ docker pull debian@sha256:f8cda73245651b967552235e0ef428b21a0a5cc29f05264da08261a76799730b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.5 MB (45494945 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:253a02c2584f89ddf97baed13135bf313aa3668e45a56f72881603c6bead3762`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:30 GMT
ADD file:f48330c22d0ecd502b8b46026eeef4081864a8a585160caae93e5700a6524a58 in / 
# Wed, 12 Apr 2023 00:10:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b037784414a5f8d993a1f29169b41dbec924bf914e4e324fdd3209ce4445a32`  
		Last Modified: Wed, 12 Apr 2023 00:13:57 GMT  
		Size: 45.5 MB (45494945 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; s390x

```console
$ docker pull debian@sha256:5e24a48f6e7ffd133b18d017a9890cca823d906e9af24a336658462f71fcea7d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.7 MB (47665070 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd59016cd9679cf407bb086c8b97ce8c0d254ce923c68c3a981a3a3cbad8079e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:03:33 GMT
ADD file:9280167c5f0a6008baa6e9d84a780823b1cf741a9935a60531fb30279a421a97 in / 
# Wed, 12 Apr 2023 00:03:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:967d5de3f09581dfdc9be32037966130e96015074ba7d995ffb521bcb496aedb`  
		Last Modified: Wed, 12 Apr 2023 00:06:36 GMT  
		Size: 47.7 MB (47665070 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20230502`

```console
$ docker pull debian@sha256:8fa86090d9a4e80f1a3ec1ee1e155ddc7792e3e863840eed6ad20f4d9e9ecfec
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:unstable-20230502` - linux; arm variant v5

```console
$ docker pull debian@sha256:6ced5ad5c3c2d37b702fcaebc3ff40c26407d4da5531773ad9cad5029d4401bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.2 MB (47167066 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c731822d63f9380f1493d8187b05ea8e1391243260820b2a4cdb2d35bd02f3a8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:42 GMT
ADD file:740dbf70289faed5b254bb33009aaddef87ef73abcfdc2441b61e0be62ac6d62 in / 
# Tue, 02 May 2023 22:49:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c7b5fff53169be147228f50722828c1946b9253a79b37d14d755411593e715be`  
		Last Modified: Tue, 02 May 2023 22:53:28 GMT  
		Size: 47.2 MB (47167066 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20230502-slim`

```console
$ docker pull debian@sha256:ceb252b363c9f073701a62977f8a2fdb9ae576e32840efd17a16e8ef73e545a8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 1
	-	linux; arm variant v5

### `debian:unstable-20230502-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8fe80014c66cf20c074af9aebc9abf394423e1514b94ec37d31b7d8623c90db9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5011ccb2d0169a03ce720bb3c6aa73d42c183d475ceb872c5b2f1585ec1aa298`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:48 GMT
ADD file:e5e9a77eff3530322574ac8f3ad4717ec398bcbfa2de9d01eed8b21ac9331620 in / 
# Tue, 02 May 2023 22:49:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0277d38ee9762bfcd2d10584d942f19d79ea3e3e6a99e09bae5b597921ec768c`  
		Last Modified: Tue, 02 May 2023 22:53:41 GMT  
		Size: 27.0 MB (26954677 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-slim`

```console
$ docker pull debian@sha256:2d72def81514fc456df18eec1135777a7756b928539f4696f885d833516846c9
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:17e0bb047017694439c32991319f703831fffa86030d9e8ee06374a9f1ac2caf
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29085857 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1804df4193dc534830d1d51860ce5624b9c6e9b82125387c3bad3ce6455780e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:22:01 GMT
ADD file:50654e8e390bfcca0711da0a198a9cacc74035216541095fe4153872a770cb28 in / 
# Wed, 12 Apr 2023 00:22:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0cc372bd9ecf28c415fb05584e95307e6d34e6eedcbd34b143810329587ed19f`  
		Last Modified: Wed, 12 Apr 2023 00:26:58 GMT  
		Size: 29.1 MB (29085857 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8fe80014c66cf20c074af9aebc9abf394423e1514b94ec37d31b7d8623c90db9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26954677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5011ccb2d0169a03ce720bb3c6aa73d42c183d475ceb872c5b2f1585ec1aa298`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 02 May 2023 22:49:48 GMT
ADD file:e5e9a77eff3530322574ac8f3ad4717ec398bcbfa2de9d01eed8b21ac9331620 in / 
# Tue, 02 May 2023 22:49:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0277d38ee9762bfcd2d10584d942f19d79ea3e3e6a99e09bae5b597921ec768c`  
		Last Modified: Tue, 02 May 2023 22:53:41 GMT  
		Size: 27.0 MB (26954677 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:b759e329903979263c0c6e4311e30d8a0a48c19e5f544d434f642ebbe895eede
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.7 MB (25674768 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb926c84e4d872c75c0ca9cb1105121f0a04ecba8127924df683ed4c51e8e879`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:01:37 GMT
ADD file:c9919b4291a9d898239dd36ec8d922c10788809f3aafbf2adeb3e539379e6734 in / 
# Wed, 12 Apr 2023 00:01:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4f149e3a337389d7680ace00396ffe8272f284840253f015b1a9b1f47c6b3e1`  
		Last Modified: Wed, 12 Apr 2023 00:06:27 GMT  
		Size: 25.7 MB (25674768 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fe840632646bcb06fa9a89d52929fc3519d91900cecc364d4d731f92cfb221f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fbee7a566fb09ee8e3d6333b9d80909e4ef417628864abb7243f65a0df899fe2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:41:04 GMT
ADD file:0d866f06ca7bdabecc515b0dc81c27ec44466a23cc454dc0f93177943833a5f1 in / 
# Wed, 12 Apr 2023 00:41:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4f517fa94f5aa4f8a613bc59f82eca3aa039013e8704323c957afc29ecc50479`  
		Last Modified: Wed, 12 Apr 2023 00:45:35 GMT  
		Size: 29.1 MB (29124202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; 386

```console
$ docker pull debian@sha256:827e8e75aacb505fe83ee43a3d2ec94dabbc56fa1e8f3d9837b12fa4701ab888
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30105663 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5a56085ddb1378fb81cc8f823b91d30c7e041bc2c779dcb1d40631572cde0c18`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:40:59 GMT
ADD file:032f1c542350d6511dabc3f6ba1ee6f8815bd1d8d8fff963ab6d6f331a871488 in / 
# Wed, 12 Apr 2023 00:41:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:96f04981aceebb3c577f4ceb2db9616c10f5e6f8fa13222cdeeb47135a1759a2`  
		Last Modified: Wed, 12 Apr 2023 00:46:27 GMT  
		Size: 30.1 MB (30105663 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:a0e6205b71e6533e61aa07c3cc84d45de1cc605fa65cc2b2e277641ff58be309
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29090184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b8a781f06266dfe3c42fc2f5012d7f2bf1bd62a07aedb261bda992c581eae7d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:13:59 GMT
ADD file:d21fa85fc854a3a39e20f6f6c5a2b92791052e9924e473c09a2adaa3f510f634 in / 
# Wed, 12 Apr 2023 00:14:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5573a31eea648ca2a5a5cdb56ce438746ded334cbd0f23114c6415403c057a3e`  
		Last Modified: Wed, 12 Apr 2023 00:22:09 GMT  
		Size: 29.1 MB (29090184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:18a7932a5c904343b155eb919bfe9b0ebb6db6b2fa0144aa5b3021170164bfc1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33089564 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fdc88e0ddb6bc6bdfcc9093c49c559d1d9b616c03255f894b6b5ebc8c36207d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:10:53 GMT
ADD file:0fed6e2778a7e78eb6b4c6ab26b75d6eed1e6face98048197a613301d365e8ee in / 
# Wed, 12 Apr 2023 00:10:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec3b46de7424580563eae9ac2c15a6be1a19191b1f32ceb4385d73366dc4d12c`  
		Last Modified: Wed, 12 Apr 2023 00:16:09 GMT  
		Size: 33.1 MB (33089564 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; riscv64

```console
$ docker pull debian@sha256:66988becd51f9f6d5f7c7c8502f5c0e27e91a419140f813482b5f3130f667716
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25308774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:45f5fe8f55c6d0e0dc376e947f242f645c837838407c527a40693c80e84c481e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:11:06 GMT
ADD file:0092f5352e7696b561d29d2b98493f544678550a77d69e534ac208002d3c8dad in / 
# Wed, 12 Apr 2023 00:11:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c26cf385b366896a35f0aa7da5f4e8554d1e8aa83bf39676e6eaa2ba321043cf`  
		Last Modified: Wed, 12 Apr 2023 00:14:33 GMT  
		Size: 25.3 MB (25308774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:2df8f23189ec3bae377135b06bb735e08f7c51480d2757a77d00131443d3a17e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27455526 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f2e09b4df56a672e34801a8562f17f31dad06aaa2cfc057144452ecd7316d538`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 12 Apr 2023 00:03:46 GMT
ADD file:a4b65ad46b2341f2924bd7b99988512f761729a7a3559af25a2a523c01588279 in / 
# Wed, 12 Apr 2023 00:03:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dca112b473f97e9a2c15a63e1ac45313c8e85d142e6385a7f19ac0b223a756ec`  
		Last Modified: Wed, 12 Apr 2023 00:06:44 GMT  
		Size: 27.5 MB (27455526 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
