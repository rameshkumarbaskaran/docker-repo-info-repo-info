<!-- THIS FILE IS GENERATED VIA './update-remote.sh' -->

# Tags of `debian`

-	[`debian:10`](#debian10)
-	[`debian:10-slim`](#debian10-slim)
-	[`debian:10.13`](#debian1013)
-	[`debian:10.13-slim`](#debian1013-slim)
-	[`debian:11`](#debian11)
-	[`debian:11-slim`](#debian11-slim)
-	[`debian:11.7`](#debian117)
-	[`debian:11.7-slim`](#debian117-slim)
-	[`debian:12`](#debian12)
-	[`debian:12-slim`](#debian12-slim)
-	[`debian:12.0`](#debian120)
-	[`debian:12.0-slim`](#debian120-slim)
-	[`debian:bookworm`](#debianbookworm)
-	[`debian:bookworm-20230703`](#debianbookworm-20230703)
-	[`debian:bookworm-20230703-slim`](#debianbookworm-20230703-slim)
-	[`debian:bookworm-backports`](#debianbookworm-backports)
-	[`debian:bookworm-slim`](#debianbookworm-slim)
-	[`debian:bullseye`](#debianbullseye)
-	[`debian:bullseye-20230703`](#debianbullseye-20230703)
-	[`debian:bullseye-20230703-slim`](#debianbullseye-20230703-slim)
-	[`debian:bullseye-backports`](#debianbullseye-backports)
-	[`debian:bullseye-slim`](#debianbullseye-slim)
-	[`debian:buster`](#debianbuster)
-	[`debian:buster-20230703`](#debianbuster-20230703)
-	[`debian:buster-20230703-slim`](#debianbuster-20230703-slim)
-	[`debian:buster-backports`](#debianbuster-backports)
-	[`debian:buster-slim`](#debianbuster-slim)
-	[`debian:experimental`](#debianexperimental)
-	[`debian:experimental-20230703`](#debianexperimental-20230703)
-	[`debian:latest`](#debianlatest)
-	[`debian:oldoldstable`](#debianoldoldstable)
-	[`debian:oldoldstable-20230703`](#debianoldoldstable-20230703)
-	[`debian:oldoldstable-20230703-slim`](#debianoldoldstable-20230703-slim)
-	[`debian:oldoldstable-backports`](#debianoldoldstable-backports)
-	[`debian:oldoldstable-slim`](#debianoldoldstable-slim)
-	[`debian:oldstable`](#debianoldstable)
-	[`debian:oldstable-20230703`](#debianoldstable-20230703)
-	[`debian:oldstable-20230703-slim`](#debianoldstable-20230703-slim)
-	[`debian:oldstable-backports`](#debianoldstable-backports)
-	[`debian:oldstable-slim`](#debianoldstable-slim)
-	[`debian:rc-buggy`](#debianrc-buggy)
-	[`debian:rc-buggy-20230703`](#debianrc-buggy-20230703)
-	[`debian:sid`](#debiansid)
-	[`debian:sid-20230703`](#debiansid-20230703)
-	[`debian:sid-20230703-slim`](#debiansid-20230703-slim)
-	[`debian:sid-slim`](#debiansid-slim)
-	[`debian:stable`](#debianstable)
-	[`debian:stable-20230703`](#debianstable-20230703)
-	[`debian:stable-20230703-slim`](#debianstable-20230703-slim)
-	[`debian:stable-backports`](#debianstable-backports)
-	[`debian:stable-slim`](#debianstable-slim)
-	[`debian:testing`](#debiantesting)
-	[`debian:testing-20230703`](#debiantesting-20230703)
-	[`debian:testing-20230703-slim`](#debiantesting-20230703-slim)
-	[`debian:testing-backports`](#debiantesting-backports)
-	[`debian:testing-slim`](#debiantesting-slim)
-	[`debian:trixie`](#debiantrixie)
-	[`debian:trixie-20230703`](#debiantrixie-20230703)
-	[`debian:trixie-20230703-slim`](#debiantrixie-20230703-slim)
-	[`debian:trixie-backports`](#debiantrixie-backports)
-	[`debian:trixie-slim`](#debiantrixie-slim)
-	[`debian:unstable`](#debianunstable)
-	[`debian:unstable-20230703`](#debianunstable-20230703)
-	[`debian:unstable-20230703-slim`](#debianunstable-20230703-slim)
-	[`debian:unstable-slim`](#debianunstable-slim)

## `debian:10`

```console
$ docker pull debian@sha256:c21dbb23d41cb3f1c1a7f841e8642bf713934fb4dc5187979bd46f0b4b488616
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10` - linux; amd64

```console
$ docker pull debian@sha256:5d6d56fd575caa578370f3d737116dc1aa3940c0e26f368ca36db72efa576570
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448743 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de12428f6f470169eba22c18e53addae00d60e92ebe96b2c3bdd99c17c34010`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:34 GMT
ADD file:2fa5242038736e48eaca794d061079c1cbd32fcc4336250001523c41b3177276 in / 
# Tue, 04 Jul 2023 01:20:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8eb6dba554cffc072c7a6c696c8a23fc311e543399d84ab3ebc55c07ab54414f`  
		Last Modified: Tue, 04 Jul 2023 01:26:01 GMT  
		Size: 50.4 MB (50448743 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v7

```console
$ docker pull debian@sha256:d829b173cc834be382851eb0e3b6795fd6f3cbf1a47254409525d036de0dc0e7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916488 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8d7573ccdc3e47ecba5eb7d9b18f7dd0d68728e59e6510edd941a09ab38874c7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:31 GMT
ADD file:e1c7c10335e2ac86eba02c2785d0ff530cba87131e91c42924072b4010418f93 in / 
# Tue, 04 Jul 2023 00:58:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b19cf971d5ed51e0bba0bd71ccd7b797ad4873a62fee0f49588f6ef19a58e659`  
		Last Modified: Tue, 04 Jul 2023 01:03:55 GMT  
		Size: 45.9 MB (45916488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3b830c5d210077d297956550caedf93430d5779fd786b4ac8e22a6bfe54ff162
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238644 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9eabbca4bbf36f9c778ae052a9e6796cda58ca5b714378fb81d78703b6abdda2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:59 GMT
ADD file:a61276f534601d7b193af24e35c16b73a2913511d2b1ad997c9a8cb907685257 in / 
# Tue, 04 Jul 2023 01:58:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e2555ae6edde2e7933c533234cb224b6d7ef3a3c90041851e31fe29ab7197c9`  
		Last Modified: Tue, 04 Jul 2023 02:02:18 GMT  
		Size: 49.2 MB (49238644 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; 386

```console
$ docker pull debian@sha256:97fa01fafbac78c2f35b2a126d7716b7a5fe9528f69f712039ba46f3f213935b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206276 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5633184154445083a679086df75c54ab1501fd866fc05950198d5785332842b4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:11 GMT
ADD file:6ecab94f7cb2bc0740a669cfef73f0837a2ecf6475dfb0fc072cbd1a7f60ec41 in / 
# Tue, 04 Jul 2023 01:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9b3c3fd2dd1697ce9fed8a2be0142b2d1294d424c56d0ce6d44bb8055d77e35d`  
		Last Modified: Tue, 04 Jul 2023 01:44:33 GMT  
		Size: 51.2 MB (51206276 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10-slim`

```console
$ docker pull debian@sha256:cddb688e1263b9752275b064171ef6ac9c70ae21a77c774339aecfb53690b9a1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10-slim` - linux; amd64

```console
$ docker pull debian@sha256:bb8aa91b818f9c9d79620db405cdb1834ba9c611d7771ac0df7b1c897c237405
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138502 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:440af094c94c50da90f9758cb86f9c444b533ae1aaa31ae901a90ce3fd502e25`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:47 GMT
ADD file:ca4076bfffab8d09636384070ca253570590554cff76a132afaae5cd89b363b5 in / 
# Tue, 04 Jul 2023 01:20:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90ac1ecaf92c8ae0cb37d29d8cc01b5802612c12edb933c369ad4c586ea94c6c`  
		Last Modified: Tue, 04 Jul 2023 01:26:21 GMT  
		Size: 27.1 MB (27138502 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:65e1480312045435e6bcbe45d6089796a6eed24a3e3dfb690f9de0110adc8180
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747297 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98450657978157841dbe66088f7eccf3e77203a44b6ca7e0e840ecd66a2bcb92`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:42 GMT
ADD file:d2008875df919cb9817e6af97438428c2632113c1ef2b67b96607e184a7af941 in / 
# Tue, 04 Jul 2023 00:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:966944a300a7cf6431aa9fc3454d07eb7764f6b99d43a67de45ba53c30d69010`  
		Last Modified: Tue, 04 Jul 2023 01:04:17 GMT  
		Size: 22.7 MB (22747297 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7fa521a8417b06243b8834cfac50b6e5878cb1a2ac93b30ddf697d03726745cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921455 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d9f0adbe79ff2fdd2578be296271df31982f034355e67d65d114a7db9c05be1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:08 GMT
ADD file:1368964550b1f674dd0365f72bb1d55cfebf5e3c651d49b5e168f9be97a6df76 in / 
# Tue, 04 Jul 2023 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:30bfbe1cefebb80f6f9d5d90c39fcdf9d6367bbc76fa68e33ea3b4391c0f9a77`  
		Last Modified: Tue, 04 Jul 2023 02:02:39 GMT  
		Size: 25.9 MB (25921455 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; 386

```console
$ docker pull debian@sha256:406434a9edee56a3103e89abc4053a3a5b082d0371565f7d4293053bde12ae6e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f24c54dc8c17e5705314df120aeeb1220570ab862b480bf0334811d973bdeb99`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:22 GMT
ADD file:0be890be1c2ff0966cf1b714bd814e6859e1a2cb4a491ebd6c28d551eeedd66b in / 
# Tue, 04 Jul 2023 01:39:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4637b85bb10ca071ae2bfe4ea2e6dd6ca216cecae3d9db6787e67046e0c1f70`  
		Last Modified: Tue, 04 Jul 2023 01:44:56 GMT  
		Size: 27.8 MB (27797000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.13`

```console
$ docker pull debian@sha256:c21dbb23d41cb3f1c1a7f841e8642bf713934fb4dc5187979bd46f0b4b488616
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10.13` - linux; amd64

```console
$ docker pull debian@sha256:5d6d56fd575caa578370f3d737116dc1aa3940c0e26f368ca36db72efa576570
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448743 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de12428f6f470169eba22c18e53addae00d60e92ebe96b2c3bdd99c17c34010`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:34 GMT
ADD file:2fa5242038736e48eaca794d061079c1cbd32fcc4336250001523c41b3177276 in / 
# Tue, 04 Jul 2023 01:20:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8eb6dba554cffc072c7a6c696c8a23fc311e543399d84ab3ebc55c07ab54414f`  
		Last Modified: Tue, 04 Jul 2023 01:26:01 GMT  
		Size: 50.4 MB (50448743 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; arm variant v7

```console
$ docker pull debian@sha256:d829b173cc834be382851eb0e3b6795fd6f3cbf1a47254409525d036de0dc0e7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916488 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8d7573ccdc3e47ecba5eb7d9b18f7dd0d68728e59e6510edd941a09ab38874c7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:31 GMT
ADD file:e1c7c10335e2ac86eba02c2785d0ff530cba87131e91c42924072b4010418f93 in / 
# Tue, 04 Jul 2023 00:58:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b19cf971d5ed51e0bba0bd71ccd7b797ad4873a62fee0f49588f6ef19a58e659`  
		Last Modified: Tue, 04 Jul 2023 01:03:55 GMT  
		Size: 45.9 MB (45916488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3b830c5d210077d297956550caedf93430d5779fd786b4ac8e22a6bfe54ff162
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238644 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9eabbca4bbf36f9c778ae052a9e6796cda58ca5b714378fb81d78703b6abdda2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:59 GMT
ADD file:a61276f534601d7b193af24e35c16b73a2913511d2b1ad997c9a8cb907685257 in / 
# Tue, 04 Jul 2023 01:58:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e2555ae6edde2e7933c533234cb224b6d7ef3a3c90041851e31fe29ab7197c9`  
		Last Modified: Tue, 04 Jul 2023 02:02:18 GMT  
		Size: 49.2 MB (49238644 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13` - linux; 386

```console
$ docker pull debian@sha256:97fa01fafbac78c2f35b2a126d7716b7a5fe9528f69f712039ba46f3f213935b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206276 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5633184154445083a679086df75c54ab1501fd866fc05950198d5785332842b4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:11 GMT
ADD file:6ecab94f7cb2bc0740a669cfef73f0837a2ecf6475dfb0fc072cbd1a7f60ec41 in / 
# Tue, 04 Jul 2023 01:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9b3c3fd2dd1697ce9fed8a2be0142b2d1294d424c56d0ce6d44bb8055d77e35d`  
		Last Modified: Tue, 04 Jul 2023 01:44:33 GMT  
		Size: 51.2 MB (51206276 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.13-slim`

```console
$ docker pull debian@sha256:cddb688e1263b9752275b064171ef6ac9c70ae21a77c774339aecfb53690b9a1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:10.13-slim` - linux; amd64

```console
$ docker pull debian@sha256:bb8aa91b818f9c9d79620db405cdb1834ba9c611d7771ac0df7b1c897c237405
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138502 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:440af094c94c50da90f9758cb86f9c444b533ae1aaa31ae901a90ce3fd502e25`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:47 GMT
ADD file:ca4076bfffab8d09636384070ca253570590554cff76a132afaae5cd89b363b5 in / 
# Tue, 04 Jul 2023 01:20:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90ac1ecaf92c8ae0cb37d29d8cc01b5802612c12edb933c369ad4c586ea94c6c`  
		Last Modified: Tue, 04 Jul 2023 01:26:21 GMT  
		Size: 27.1 MB (27138502 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:65e1480312045435e6bcbe45d6089796a6eed24a3e3dfb690f9de0110adc8180
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747297 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98450657978157841dbe66088f7eccf3e77203a44b6ca7e0e840ecd66a2bcb92`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:42 GMT
ADD file:d2008875df919cb9817e6af97438428c2632113c1ef2b67b96607e184a7af941 in / 
# Tue, 04 Jul 2023 00:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:966944a300a7cf6431aa9fc3454d07eb7764f6b99d43a67de45ba53c30d69010`  
		Last Modified: Tue, 04 Jul 2023 01:04:17 GMT  
		Size: 22.7 MB (22747297 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7fa521a8417b06243b8834cfac50b6e5878cb1a2ac93b30ddf697d03726745cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921455 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d9f0adbe79ff2fdd2578be296271df31982f034355e67d65d114a7db9c05be1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:08 GMT
ADD file:1368964550b1f674dd0365f72bb1d55cfebf5e3c651d49b5e168f9be97a6df76 in / 
# Tue, 04 Jul 2023 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:30bfbe1cefebb80f6f9d5d90c39fcdf9d6367bbc76fa68e33ea3b4391c0f9a77`  
		Last Modified: Tue, 04 Jul 2023 02:02:39 GMT  
		Size: 25.9 MB (25921455 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.13-slim` - linux; 386

```console
$ docker pull debian@sha256:406434a9edee56a3103e89abc4053a3a5b082d0371565f7d4293053bde12ae6e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f24c54dc8c17e5705314df120aeeb1220570ab862b480bf0334811d973bdeb99`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:22 GMT
ADD file:0be890be1c2ff0966cf1b714bd814e6859e1a2cb4a491ebd6c28d551eeedd66b in / 
# Tue, 04 Jul 2023 01:39:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4637b85bb10ca071ae2bfe4ea2e6dd6ca216cecae3d9db6787e67046e0c1f70`  
		Last Modified: Tue, 04 Jul 2023 01:44:56 GMT  
		Size: 27.8 MB (27797000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11`

```console
$ docker pull debian@sha256:a648e10e02af129706b1fb89e1ac9694ae3db7f2b8439aa906321e68cc281bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11` - linux; amd64

```console
$ docker pull debian@sha256:3b6053ca925336c804e2d3f080af177efcdc9f51198a627569bfc7c7e730ef7e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:189a2f977ff1e0aeb33038f380dd9a6ab0355d27ec82f10d23567b95386506ce`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:b7c0be2bb90e88689b1c16da78dea0b85760b55b90ef2e5bc4a529c89e4fa25b in / 
# Tue, 04 Jul 2023 01:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34df401c391c7595044379e04e8ad4856a5a3974cdbf5a160f0a204d761e88aa`  
		Last Modified: Tue, 04 Jul 2023 01:25:21 GMT  
		Size: 55.1 MB (55055300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v5

```console
$ docker pull debian@sha256:ea256bee22dad6b8949beae7c7fa2e40529b8c35451eabe372804c277d6f8f31
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556778 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6db0bcb926823fd1a67160b99db57bff4e133735b9ebc9c945352dd3f1a27b8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:39 GMT
ADD file:fe1b9f9f6c7d67ad23a2ee839774be4bcee33c60c7b34c48df5a08eb33cafd1b in / 
# Tue, 04 Jul 2023 00:48:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:702dd8b607f626c689176debe921fe96009c1ce6dbd66f4f238c7def50080e3d`  
		Last Modified: Tue, 04 Jul 2023 00:52:15 GMT  
		Size: 52.6 MB (52556778 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v7

```console
$ docker pull debian@sha256:819b11bd0ade30fbc72f4b83593d0d126b2f3329b053495833219213fe37714d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218247 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f019634bdb2ec508d690cc1e60eb323089e52863f495460d59f317955c09be44`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:06 GMT
ADD file:17e02296458241d9441f8da6a5dafb747d528a729106b17cec2f4c1c8cfe0ad8 in / 
# Tue, 04 Jul 2023 00:58:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31edf2db9ca1650aa08e2d42e9b5bb7349413d7212110149a1a5d202ac20914b`  
		Last Modified: Tue, 04 Jul 2023 01:03:12 GMT  
		Size: 50.2 MB (50218247 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:de3c0d12dd75f1a47595ff0ce78f2d30d6ca95c3ad66af06c8815d1f9b8e208d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a057fce101fd55e46c91786b7fe2c9e80353b088c83a6eb63d2bcb48dc4b47e9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:43 GMT
ADD file:e626446584d8094b7b58d72a717380ca64d3e9ab924fc625406fe26a83fe1d8b in / 
# Tue, 04 Jul 2023 01:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:29279ac7c19f9c667f1c6b07bfba6fba20ca0d945b9fbc6edad6f75d13361fae`  
		Last Modified: Tue, 04 Jul 2023 02:01:38 GMT  
		Size: 53.7 MB (53703979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; 386

```console
$ docker pull debian@sha256:208ef91ebfbfb49f90dc2917f9c0b82a627ab669d1dab21a29b3bc4598cd2256
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040756 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:985169c5e2dedb21701be6f461b6449ce9c4cc1ff316e5d1e08917d620f6632a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:48 GMT
ADD file:c5739407c2c257fbb85ab4b9dcd2dc07e6fe172d7309aaaaab544c8df6c42b92 in / 
# Tue, 04 Jul 2023 01:38:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b700c8b35944e0d5f6ab9049dbe2262492e26327a8efad518b1011428393653f`  
		Last Modified: Tue, 04 Jul 2023 01:43:49 GMT  
		Size: 56.0 MB (56040756 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; mips64le

```console
$ docker pull debian@sha256:9e54eef8b57fa51064806c0ed4d9ff8f66796190d4cbc7f4bc709fff8b4c70e2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:78934b34e1d526823f453d4f4e8827177b5161276a1ed8fe2f4dac41e3a2e367`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:02 GMT
ADD file:f7ddd7406e3aa24165fc80933583fc2298f9792865511fe65def2f571fed2207 in / 
# Tue, 04 Jul 2023 01:10:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af98e96208d7ce4cff5417a3e8a38f98d2ed3abbca318e1e6c323aa8b8c56690`  
		Last Modified: Tue, 04 Jul 2023 01:20:40 GMT  
		Size: 53.3 MB (53270472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; ppc64le

```console
$ docker pull debian@sha256:bcf5819ab888818cf7eeae793b9e27c6a9221a9625c9e64e2bc785b2232cdaf1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe24e6ff73b4dffad0551a211517a8fafa464e56a39f0e8adf686ec80a4fa968`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:11 GMT
ADD file:ff86c25854c28609c2852c1615270f0acd4c4efaa38ff8ed9c23afe5132f2135 in / 
# Tue, 04 Jul 2023 01:18:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a792749f62225048efdf85841bea6f3999f586371852f3dbce13a6c3d89b1fa7`  
		Last Modified: Tue, 04 Jul 2023 01:25:06 GMT  
		Size: 58.9 MB (58927745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; s390x

```console
$ docker pull debian@sha256:d31138981e215f44472b534c1d30f8dea3eccce1c5e82a730cca2c877e7e0bf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288197 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:251b768a790fd5b8507b454503dd3d193fc688065dfe09b6d3071563ca07651f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:26 GMT
ADD file:231ba6ce8d3ee30318948799a94cb007f1517ea0d14c2b84863012cac37d6c6b in / 
# Tue, 04 Jul 2023 01:32:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1e40ed2a44a8c2786b06584afad765d97e9b1c910f58ae426622ba17fbf3d4c3`  
		Last Modified: Tue, 04 Jul 2023 01:37:21 GMT  
		Size: 53.3 MB (53288197 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11-slim`

```console
$ docker pull debian@sha256:3460d74bec6b88496cd183d7731930be55234c094f581f7dbdd96f56c1fc34d8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11-slim` - linux; amd64

```console
$ docker pull debian@sha256:0781db7f3f74866059cf161d145af0bc16fb13493b6e0f87ca20d7e34498d691
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:02ad3ff027defef6c8c23678fc5aee4c4f7b73af047688850671ccf4469b8641`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:23 GMT
ADD file:4a063d4e089ef10c6806d7042a0040078674ac2db61df02df8bbb8fa4894910a in / 
# Tue, 04 Jul 2023 01:20:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9d21b12d5fab9ab82969054d72411ce627c209257df64b6057016c981e163c30`  
		Last Modified: Tue, 04 Jul 2023 01:25:43 GMT  
		Size: 31.4 MB (31417388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:89e6030fba6589df6fa2faa2c4b8e64d50c412ffc4ebed83f52d9ce66bb1666d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918872 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:087af48210389738e9f2d5f949267d7ba7914194f0278289b82170e274d4209e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:47 GMT
ADD file:2dc75d45982de1ae93d7466556a8d6f806e24a837046bd3445b5df6853b0b5bb in / 
# Tue, 04 Jul 2023 00:48:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e5c23077f8d036e239b1eb6572155131327f3345abd95ce868b997d2e1d969f3`  
		Last Modified: Tue, 04 Jul 2023 00:52:38 GMT  
		Size: 28.9 MB (28918872 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c39bd3c81e12b4e81c32d6d4390b8dbc0c86d2028d452a556f1a8e27d916f8ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578754 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d296fbefde2b415d8164b52defaf05f117e02fb9cbe59e48efbc868e1b70be2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:20 GMT
ADD file:c023c66ee4b7cdae5c542f2ad2dd35aef94ad24e1b3b479a16538c46013ae6a5 in / 
# Tue, 04 Jul 2023 00:58:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d9e6a8b782e380f447723ac26499c5014f2d383b9210819b9e73e97abaf81249`  
		Last Modified: Tue, 04 Jul 2023 01:03:35 GMT  
		Size: 26.6 MB (26578754 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:511660e7e3f8f5d84a6c33710a4e3ccd9bcc19c144631b0b4425203f723ec941
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062957 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bac24d73af3aa85d8ec803623835b945fa61d05f35ad1ef7ed064a2a63a3e4e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:52 GMT
ADD file:83a81aad5cdb80c654a520d913c8bcafe2b8e1062d81c389d4577cde5ad68167 in / 
# Tue, 04 Jul 2023 01:57:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:50eb042e2421869704212f3e076e9088033eb9a5254341fb1b3022e6e2784921`  
		Last Modified: Tue, 04 Jul 2023 02:02:00 GMT  
		Size: 30.1 MB (30062957 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; 386

```console
$ docker pull debian@sha256:50fd2f587932e94cb3ea95e71d9ae31db61d2833e37ededbfe1aacebd255c801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397496 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b900b795d418649b0bce493e8747d485ee86acde60c0d033724cfd4a8a4ec394`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:59 GMT
ADD file:c39e332ccb9cb890ba7896a09e9f074b3c8d67094b4eaf73d5c67e8949c2e0be in / 
# Tue, 04 Jul 2023 01:39:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3466e7f9e8ec982697fc130dbb1706d708f5c53e348a638019a955c15c7395af`  
		Last Modified: Tue, 04 Jul 2023 01:44:12 GMT  
		Size: 32.4 MB (32397496 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; mips64le

```console
$ docker pull debian@sha256:4fc1f150825beac7afd57d3600835a818770a46aa2605c9c030e4e5c597c7484
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eda43c8166bfdc0308049da49e15c03def4c527ec5da9c4ad14fc73275039630`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:44 GMT
ADD file:fa125b6265458c9912ef0f591e20d187cdc4b3a5222cb3f066e997a9baa4d699 in / 
# Tue, 04 Jul 2023 01:10:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5fa30c60fbae4cff4aa6f1626e53313c5fd7d3ed439bb87d067b66af6b1c07fd`  
		Last Modified: Tue, 04 Jul 2023 01:21:20 GMT  
		Size: 29.6 MB (29634364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:381c57ee33b625a5e9f5c4f5629bbc30b881a5248e0a75c45de44f9ecc4bbb14
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291082 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5a0d1f1d6b415be4f9b9f9669c4fc468645ddf3efeaa638129b78c367e64b1d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:33 GMT
ADD file:37fa020aca7253d41d395ed529c38db73caaa4da098754836244552f65fa7d5d in / 
# Tue, 04 Jul 2023 01:18:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:147bb8b5828c99cd6b07d252d2987490463c142099eaa0815025685f8fa4f3d8`  
		Last Modified: Tue, 04 Jul 2023 01:25:40 GMT  
		Size: 35.3 MB (35291082 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; s390x

```console
$ docker pull debian@sha256:e5160c8c64122d433e9e48ea3b95a27c98c1daa9a8d01652064e73af6235d374
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f104358b3f81648e93f98e0d89a739db4e5aa32fcb42bc75d249d06640b6097`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:44 GMT
ADD file:799c0afa70fa3bf4bb7804964481cba79e80aa3fad5c7a25beadde206ed6a8ff in / 
# Tue, 04 Jul 2023 01:32:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbee891481f2623da9c7d37a49947926519c858c2b06773370a6189440d4b4de`  
		Last Modified: Tue, 04 Jul 2023 01:37:37 GMT  
		Size: 29.7 MB (29652540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.7`

```console
$ docker pull debian@sha256:a648e10e02af129706b1fb89e1ac9694ae3db7f2b8439aa906321e68cc281bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.7` - linux; amd64

```console
$ docker pull debian@sha256:3b6053ca925336c804e2d3f080af177efcdc9f51198a627569bfc7c7e730ef7e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:189a2f977ff1e0aeb33038f380dd9a6ab0355d27ec82f10d23567b95386506ce`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:b7c0be2bb90e88689b1c16da78dea0b85760b55b90ef2e5bc4a529c89e4fa25b in / 
# Tue, 04 Jul 2023 01:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34df401c391c7595044379e04e8ad4856a5a3974cdbf5a160f0a204d761e88aa`  
		Last Modified: Tue, 04 Jul 2023 01:25:21 GMT  
		Size: 55.1 MB (55055300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; arm variant v5

```console
$ docker pull debian@sha256:ea256bee22dad6b8949beae7c7fa2e40529b8c35451eabe372804c277d6f8f31
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556778 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6db0bcb926823fd1a67160b99db57bff4e133735b9ebc9c945352dd3f1a27b8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:39 GMT
ADD file:fe1b9f9f6c7d67ad23a2ee839774be4bcee33c60c7b34c48df5a08eb33cafd1b in / 
# Tue, 04 Jul 2023 00:48:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:702dd8b607f626c689176debe921fe96009c1ce6dbd66f4f238c7def50080e3d`  
		Last Modified: Tue, 04 Jul 2023 00:52:15 GMT  
		Size: 52.6 MB (52556778 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; arm variant v7

```console
$ docker pull debian@sha256:819b11bd0ade30fbc72f4b83593d0d126b2f3329b053495833219213fe37714d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218247 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f019634bdb2ec508d690cc1e60eb323089e52863f495460d59f317955c09be44`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:06 GMT
ADD file:17e02296458241d9441f8da6a5dafb747d528a729106b17cec2f4c1c8cfe0ad8 in / 
# Tue, 04 Jul 2023 00:58:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31edf2db9ca1650aa08e2d42e9b5bb7349413d7212110149a1a5d202ac20914b`  
		Last Modified: Tue, 04 Jul 2023 01:03:12 GMT  
		Size: 50.2 MB (50218247 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:de3c0d12dd75f1a47595ff0ce78f2d30d6ca95c3ad66af06c8815d1f9b8e208d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a057fce101fd55e46c91786b7fe2c9e80353b088c83a6eb63d2bcb48dc4b47e9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:43 GMT
ADD file:e626446584d8094b7b58d72a717380ca64d3e9ab924fc625406fe26a83fe1d8b in / 
# Tue, 04 Jul 2023 01:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:29279ac7c19f9c667f1c6b07bfba6fba20ca0d945b9fbc6edad6f75d13361fae`  
		Last Modified: Tue, 04 Jul 2023 02:01:38 GMT  
		Size: 53.7 MB (53703979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; 386

```console
$ docker pull debian@sha256:208ef91ebfbfb49f90dc2917f9c0b82a627ab669d1dab21a29b3bc4598cd2256
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040756 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:985169c5e2dedb21701be6f461b6449ce9c4cc1ff316e5d1e08917d620f6632a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:48 GMT
ADD file:c5739407c2c257fbb85ab4b9dcd2dc07e6fe172d7309aaaaab544c8df6c42b92 in / 
# Tue, 04 Jul 2023 01:38:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b700c8b35944e0d5f6ab9049dbe2262492e26327a8efad518b1011428393653f`  
		Last Modified: Tue, 04 Jul 2023 01:43:49 GMT  
		Size: 56.0 MB (56040756 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; mips64le

```console
$ docker pull debian@sha256:9e54eef8b57fa51064806c0ed4d9ff8f66796190d4cbc7f4bc709fff8b4c70e2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:78934b34e1d526823f453d4f4e8827177b5161276a1ed8fe2f4dac41e3a2e367`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:02 GMT
ADD file:f7ddd7406e3aa24165fc80933583fc2298f9792865511fe65def2f571fed2207 in / 
# Tue, 04 Jul 2023 01:10:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af98e96208d7ce4cff5417a3e8a38f98d2ed3abbca318e1e6c323aa8b8c56690`  
		Last Modified: Tue, 04 Jul 2023 01:20:40 GMT  
		Size: 53.3 MB (53270472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; ppc64le

```console
$ docker pull debian@sha256:bcf5819ab888818cf7eeae793b9e27c6a9221a9625c9e64e2bc785b2232cdaf1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe24e6ff73b4dffad0551a211517a8fafa464e56a39f0e8adf686ec80a4fa968`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:11 GMT
ADD file:ff86c25854c28609c2852c1615270f0acd4c4efaa38ff8ed9c23afe5132f2135 in / 
# Tue, 04 Jul 2023 01:18:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a792749f62225048efdf85841bea6f3999f586371852f3dbce13a6c3d89b1fa7`  
		Last Modified: Tue, 04 Jul 2023 01:25:06 GMT  
		Size: 58.9 MB (58927745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7` - linux; s390x

```console
$ docker pull debian@sha256:d31138981e215f44472b534c1d30f8dea3eccce1c5e82a730cca2c877e7e0bf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288197 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:251b768a790fd5b8507b454503dd3d193fc688065dfe09b6d3071563ca07651f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:26 GMT
ADD file:231ba6ce8d3ee30318948799a94cb007f1517ea0d14c2b84863012cac37d6c6b in / 
# Tue, 04 Jul 2023 01:32:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1e40ed2a44a8c2786b06584afad765d97e9b1c910f58ae426622ba17fbf3d4c3`  
		Last Modified: Tue, 04 Jul 2023 01:37:21 GMT  
		Size: 53.3 MB (53288197 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.7-slim`

```console
$ docker pull debian@sha256:3460d74bec6b88496cd183d7731930be55234c094f581f7dbdd96f56c1fc34d8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.7-slim` - linux; amd64

```console
$ docker pull debian@sha256:0781db7f3f74866059cf161d145af0bc16fb13493b6e0f87ca20d7e34498d691
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:02ad3ff027defef6c8c23678fc5aee4c4f7b73af047688850671ccf4469b8641`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:23 GMT
ADD file:4a063d4e089ef10c6806d7042a0040078674ac2db61df02df8bbb8fa4894910a in / 
# Tue, 04 Jul 2023 01:20:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9d21b12d5fab9ab82969054d72411ce627c209257df64b6057016c981e163c30`  
		Last Modified: Tue, 04 Jul 2023 01:25:43 GMT  
		Size: 31.4 MB (31417388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:89e6030fba6589df6fa2faa2c4b8e64d50c412ffc4ebed83f52d9ce66bb1666d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918872 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:087af48210389738e9f2d5f949267d7ba7914194f0278289b82170e274d4209e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:47 GMT
ADD file:2dc75d45982de1ae93d7466556a8d6f806e24a837046bd3445b5df6853b0b5bb in / 
# Tue, 04 Jul 2023 00:48:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e5c23077f8d036e239b1eb6572155131327f3345abd95ce868b997d2e1d969f3`  
		Last Modified: Tue, 04 Jul 2023 00:52:38 GMT  
		Size: 28.9 MB (28918872 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c39bd3c81e12b4e81c32d6d4390b8dbc0c86d2028d452a556f1a8e27d916f8ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578754 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d296fbefde2b415d8164b52defaf05f117e02fb9cbe59e48efbc868e1b70be2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:20 GMT
ADD file:c023c66ee4b7cdae5c542f2ad2dd35aef94ad24e1b3b479a16538c46013ae6a5 in / 
# Tue, 04 Jul 2023 00:58:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d9e6a8b782e380f447723ac26499c5014f2d383b9210819b9e73e97abaf81249`  
		Last Modified: Tue, 04 Jul 2023 01:03:35 GMT  
		Size: 26.6 MB (26578754 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:511660e7e3f8f5d84a6c33710a4e3ccd9bcc19c144631b0b4425203f723ec941
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062957 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bac24d73af3aa85d8ec803623835b945fa61d05f35ad1ef7ed064a2a63a3e4e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:52 GMT
ADD file:83a81aad5cdb80c654a520d913c8bcafe2b8e1062d81c389d4577cde5ad68167 in / 
# Tue, 04 Jul 2023 01:57:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:50eb042e2421869704212f3e076e9088033eb9a5254341fb1b3022e6e2784921`  
		Last Modified: Tue, 04 Jul 2023 02:02:00 GMT  
		Size: 30.1 MB (30062957 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; 386

```console
$ docker pull debian@sha256:50fd2f587932e94cb3ea95e71d9ae31db61d2833e37ededbfe1aacebd255c801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397496 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b900b795d418649b0bce493e8747d485ee86acde60c0d033724cfd4a8a4ec394`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:59 GMT
ADD file:c39e332ccb9cb890ba7896a09e9f074b3c8d67094b4eaf73d5c67e8949c2e0be in / 
# Tue, 04 Jul 2023 01:39:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3466e7f9e8ec982697fc130dbb1706d708f5c53e348a638019a955c15c7395af`  
		Last Modified: Tue, 04 Jul 2023 01:44:12 GMT  
		Size: 32.4 MB (32397496 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; mips64le

```console
$ docker pull debian@sha256:4fc1f150825beac7afd57d3600835a818770a46aa2605c9c030e4e5c597c7484
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eda43c8166bfdc0308049da49e15c03def4c527ec5da9c4ad14fc73275039630`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:44 GMT
ADD file:fa125b6265458c9912ef0f591e20d187cdc4b3a5222cb3f066e997a9baa4d699 in / 
# Tue, 04 Jul 2023 01:10:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5fa30c60fbae4cff4aa6f1626e53313c5fd7d3ed439bb87d067b66af6b1c07fd`  
		Last Modified: Tue, 04 Jul 2023 01:21:20 GMT  
		Size: 29.6 MB (29634364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:381c57ee33b625a5e9f5c4f5629bbc30b881a5248e0a75c45de44f9ecc4bbb14
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291082 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5a0d1f1d6b415be4f9b9f9669c4fc468645ddf3efeaa638129b78c367e64b1d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:33 GMT
ADD file:37fa020aca7253d41d395ed529c38db73caaa4da098754836244552f65fa7d5d in / 
# Tue, 04 Jul 2023 01:18:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:147bb8b5828c99cd6b07d252d2987490463c142099eaa0815025685f8fa4f3d8`  
		Last Modified: Tue, 04 Jul 2023 01:25:40 GMT  
		Size: 35.3 MB (35291082 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.7-slim` - linux; s390x

```console
$ docker pull debian@sha256:e5160c8c64122d433e9e48ea3b95a27c98c1daa9a8d01652064e73af6235d374
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f104358b3f81648e93f98e0d89a739db4e5aa32fcb42bc75d249d06640b6097`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:44 GMT
ADD file:799c0afa70fa3bf4bb7804964481cba79e80aa3fad5c7a25beadde206ed6a8ff in / 
# Tue, 04 Jul 2023 01:32:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbee891481f2623da9c7d37a49947926519c858c2b06773370a6189440d4b4de`  
		Last Modified: Tue, 04 Jul 2023 01:37:37 GMT  
		Size: 29.7 MB (29652540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12`

```console
$ docker pull debian@sha256:3d868b5eb908155f3784317b3dda2941df87bbbbaa4608f84881de66d9bb297b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12` - linux; amd64

```console
$ docker pull debian@sha256:60774985572749dc3c39147d43089d53e7ce17b844eebcf619d84467160217ab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3676c78a12ad77e170916695d2f192797ddd3eb653d7a1b1ad4c20ab5471891a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm variant v5

```console
$ docker pull debian@sha256:e1eedd931159f74699128b11375ab15a76fed96ea29dedce88588cfaa19b564a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402786 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e836de3c00fa0d0942c5e3ea21d9e0525c92d38c0e8a1dc39a659ec87689a34d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm variant v7

```console
$ docker pull debian@sha256:722595c25d744c903165d26d37fdd2f2ead8d9ceb326deb260173f48725be078
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca910bfe472fc0e9b55301d7ad047098587c598dfd5ac44c010e202d0f7d97bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:345b9265e89c123f994b724b9a4761dbc91c5759c144d01292ca56a338170801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572781 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:886d61a022482574344910f88e436ce64e9218cc8d62bea0cac035c4519ff93e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; 386

```console
$ docker pull debian@sha256:699f29298cb47296a0b0be2d6a1d18b0ab5ca77986862949a665e4524bc0b781
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9743376b1f2144d61495397e2a7f044bbc25d61016cee4dbff8683d31eaed7fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; mips64le

```console
$ docker pull debian@sha256:67103735074efb1301d79591c39140d7362fecc0898dda01a6ab7fcef1500a2f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5f8b2d159b8a89e52d66238252e674cf2d57e11c558c82ab3c4eac593ac893ef`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c051e548fc05b1fa5d532ceffddb1ff227aa57b81b4fcc91cbf627ebb6f0a3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec2232f2e4f8c6c1f0fb3f88f17e3c7c6e3ef4040ebbaa9588f7d2e9de7a07bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; s390x

```console
$ docker pull debian@sha256:aaa4fc6ed87ddc782bba21f6dad6bdeeb0a857c7d5530e19f5d8b5134b8bc9f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921709 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9a90a19fe78ac6ff9bedf17db184107da34ef5c182a64c226910e165d1c11e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12-slim`

```console
$ docker pull debian@sha256:9bd077d2f77c754f4f7f5ee9e6ded9ff1dff92c6dce877754da21b917c122c77
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12-slim` - linux; amd64

```console
$ docker pull debian@sha256:01bd742e2c269abf94e2fefb47b08b5b61c9a880b993417d23a1d0bd9fa60dc4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124829 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc29b27ee12fa143e61ee3c4d0b1eabf114ae5626311ee0a160da5e944abc5f6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:58 GMT
ADD file:bd80a4461150784e5f2f5a1faa720cc347ad3e30ee0969adbfad574c316f5aef in / 
# Tue, 04 Jul 2023 01:19:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:faef57eae888cbe4a5613eca6741b5e48d768b83f6088858aee9a5a2834f8151`  
		Last Modified: Tue, 04 Jul 2023 01:25:03 GMT  
		Size: 29.1 MB (29124829 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:878cb8c0829de1282ccf16bedf888f1cc75036af82876dccd0d63d22cc7a610c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2f063e6f8f6133d209c28499afbd0d55e73a41c0aaa028d0c0342bcdf2c6f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:31 GMT
ADD file:ff5b6029e7e4f2b427f101b31a8dd8d15a3a1204bf9b0576e65270de33ea2f62 in / 
# Tue, 04 Jul 2023 00:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da4ec93bf9d039ea6c48e256f41d88db6aed3ac72074f4cffb3f0c1b41ccac78`  
		Last Modified: Tue, 04 Jul 2023 00:51:54 GMT  
		Size: 27.0 MB (26982160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:f4744abcaaf86307e611493d086e5a6d2a186c7d36f9f63ded725cff9a922cf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801264 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b481106f9ce5f5ef94b896d8c37a6d955ab1e5940345e7c97493096d42385bf7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:54 GMT
ADD file:d168d1710cbca4edb322c26348dafaf2b3a64980f52b8b790f334cdbd6a7047e in / 
# Tue, 04 Jul 2023 00:57:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:340ace0baa422f18db718d7634c1c88a9650f4871dda2630ef15577120aa6816`  
		Last Modified: Tue, 04 Jul 2023 01:02:50 GMT  
		Size: 24.8 MB (24801264 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0ef04748d071c2e631bb3edce8f805cb5512e746b682c83fdae6d8c0b243280b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152458 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc86621bf22e303a42e1f5872b7d07b76d42b1dc7e01121809212641839a0949`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:35 GMT
ADD file:71fd66666294148382f2e6a09ae5e277d4c4e9c74402ab64b693a79387b67a09 in / 
# Tue, 04 Jul 2023 01:57:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3ae0c06b4d3aa97d7e0829233dd36cea1666b87074e55fea6bd1ecae066693c7`  
		Last Modified: Tue, 04 Jul 2023 02:01:20 GMT  
		Size: 29.2 MB (29152458 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; 386

```console
$ docker pull debian@sha256:f00eb751c2d09b95f5eca3c79b0b38fbb3a5a72171460ae56809a20164c7fa11
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140746 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa31d56c510cfe00ea3b0d08b598f9886afa251a17ee3037715aa1aa2eb5bd47`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:36 GMT
ADD file:441addef37edd5da10ca045fe9cc7863b0785f468d83c3884456efd567e97536 in / 
# Tue, 04 Jul 2023 01:38:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d4b098d102e200415126181e2b6aae752738430f2ed6a601f84baf29e2f7d01`  
		Last Modified: Tue, 04 Jul 2023 01:43:28 GMT  
		Size: 30.1 MB (30140746 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; mips64le

```console
$ docker pull debian@sha256:c53575e9b0715d36e43f0ebfe55c30f46f9371b7dbe2c19d437856f05897f613
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fbaec4c66739ebe7fa73ea3959cd0dc5be32570e4a368278cd81d16c1ab17210`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:28 GMT
ADD file:cf4b72a35644a7b9f5f1ca39a54e484822f408f6821f0800f1167ae48ad6e38e in / 
# Tue, 04 Jul 2023 01:09:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:502a81d5004983c2a770a910286271410d8e2b20d0cb61ef7397e664e1e72654`  
		Last Modified: Tue, 04 Jul 2023 01:19:57 GMT  
		Size: 29.1 MB (29118201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:4cd58b1861586c2cb4f2b21789de2d2a0844720ffa22dd00cd597cde4c559726
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116716 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d9dede7363d225c4c296d0ef903b60be1caecbfcb776c47194d83441f355e520`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:51 GMT
ADD file:51887002f5002ccc662adbc6ecf1129e3db375a7a77b93da43cc9a8326b3c4b9 in / 
# Tue, 04 Jul 2023 01:17:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba99a1aa5a92d949d988e66df7723a542fd81bc30eef9d4269e8a899820d4bb2`  
		Last Modified: Tue, 04 Jul 2023 01:24:36 GMT  
		Size: 33.1 MB (33116716 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; s390x

```console
$ docker pull debian@sha256:6ceb743c3afa0c403649e34d7005148bcf5154d84b11342cc74a5230561bc9ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487968 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b251db288e6297bfcb4d93a80d9acdc5f153ecb9ba980f4b946855505c302b6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:13 GMT
ADD file:7c6e123333ed463aaf550f8adb75415706fc8573337b76140d393910a5166731 in / 
# Tue, 04 Jul 2023 01:32:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0df7967bae0170739ec7f61c7bf3adbc2799d4ca7704b10725a46942717891fc`  
		Last Modified: Tue, 04 Jul 2023 01:37:07 GMT  
		Size: 27.5 MB (27487968 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12.0`

```console
$ docker pull debian@sha256:3d868b5eb908155f3784317b3dda2941df87bbbbaa4608f84881de66d9bb297b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12.0` - linux; amd64

```console
$ docker pull debian@sha256:60774985572749dc3c39147d43089d53e7ce17b844eebcf619d84467160217ab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3676c78a12ad77e170916695d2f192797ddd3eb653d7a1b1ad4c20ab5471891a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; arm variant v5

```console
$ docker pull debian@sha256:e1eedd931159f74699128b11375ab15a76fed96ea29dedce88588cfaa19b564a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402786 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e836de3c00fa0d0942c5e3ea21d9e0525c92d38c0e8a1dc39a659ec87689a34d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; arm variant v7

```console
$ docker pull debian@sha256:722595c25d744c903165d26d37fdd2f2ead8d9ceb326deb260173f48725be078
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca910bfe472fc0e9b55301d7ad047098587c598dfd5ac44c010e202d0f7d97bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:345b9265e89c123f994b724b9a4761dbc91c5759c144d01292ca56a338170801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572781 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:886d61a022482574344910f88e436ce64e9218cc8d62bea0cac035c4519ff93e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; 386

```console
$ docker pull debian@sha256:699f29298cb47296a0b0be2d6a1d18b0ab5ca77986862949a665e4524bc0b781
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9743376b1f2144d61495397e2a7f044bbc25d61016cee4dbff8683d31eaed7fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; mips64le

```console
$ docker pull debian@sha256:67103735074efb1301d79591c39140d7362fecc0898dda01a6ab7fcef1500a2f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5f8b2d159b8a89e52d66238252e674cf2d57e11c558c82ab3c4eac593ac893ef`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c051e548fc05b1fa5d532ceffddb1ff227aa57b81b4fcc91cbf627ebb6f0a3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec2232f2e4f8c6c1f0fb3f88f17e3c7c6e3ef4040ebbaa9588f7d2e9de7a07bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0` - linux; s390x

```console
$ docker pull debian@sha256:aaa4fc6ed87ddc782bba21f6dad6bdeeb0a857c7d5530e19f5d8b5134b8bc9f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921709 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9a90a19fe78ac6ff9bedf17db184107da34ef5c182a64c226910e165d1c11e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12.0-slim`

```console
$ docker pull debian@sha256:9bd077d2f77c754f4f7f5ee9e6ded9ff1dff92c6dce877754da21b917c122c77
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12.0-slim` - linux; amd64

```console
$ docker pull debian@sha256:01bd742e2c269abf94e2fefb47b08b5b61c9a880b993417d23a1d0bd9fa60dc4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124829 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc29b27ee12fa143e61ee3c4d0b1eabf114ae5626311ee0a160da5e944abc5f6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:58 GMT
ADD file:bd80a4461150784e5f2f5a1faa720cc347ad3e30ee0969adbfad574c316f5aef in / 
# Tue, 04 Jul 2023 01:19:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:faef57eae888cbe4a5613eca6741b5e48d768b83f6088858aee9a5a2834f8151`  
		Last Modified: Tue, 04 Jul 2023 01:25:03 GMT  
		Size: 29.1 MB (29124829 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:878cb8c0829de1282ccf16bedf888f1cc75036af82876dccd0d63d22cc7a610c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2f063e6f8f6133d209c28499afbd0d55e73a41c0aaa028d0c0342bcdf2c6f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:31 GMT
ADD file:ff5b6029e7e4f2b427f101b31a8dd8d15a3a1204bf9b0576e65270de33ea2f62 in / 
# Tue, 04 Jul 2023 00:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da4ec93bf9d039ea6c48e256f41d88db6aed3ac72074f4cffb3f0c1b41ccac78`  
		Last Modified: Tue, 04 Jul 2023 00:51:54 GMT  
		Size: 27.0 MB (26982160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:f4744abcaaf86307e611493d086e5a6d2a186c7d36f9f63ded725cff9a922cf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801264 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b481106f9ce5f5ef94b896d8c37a6d955ab1e5940345e7c97493096d42385bf7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:54 GMT
ADD file:d168d1710cbca4edb322c26348dafaf2b3a64980f52b8b790f334cdbd6a7047e in / 
# Tue, 04 Jul 2023 00:57:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:340ace0baa422f18db718d7634c1c88a9650f4871dda2630ef15577120aa6816`  
		Last Modified: Tue, 04 Jul 2023 01:02:50 GMT  
		Size: 24.8 MB (24801264 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0ef04748d071c2e631bb3edce8f805cb5512e746b682c83fdae6d8c0b243280b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152458 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc86621bf22e303a42e1f5872b7d07b76d42b1dc7e01121809212641839a0949`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:35 GMT
ADD file:71fd66666294148382f2e6a09ae5e277d4c4e9c74402ab64b693a79387b67a09 in / 
# Tue, 04 Jul 2023 01:57:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3ae0c06b4d3aa97d7e0829233dd36cea1666b87074e55fea6bd1ecae066693c7`  
		Last Modified: Tue, 04 Jul 2023 02:01:20 GMT  
		Size: 29.2 MB (29152458 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; 386

```console
$ docker pull debian@sha256:f00eb751c2d09b95f5eca3c79b0b38fbb3a5a72171460ae56809a20164c7fa11
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140746 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa31d56c510cfe00ea3b0d08b598f9886afa251a17ee3037715aa1aa2eb5bd47`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:36 GMT
ADD file:441addef37edd5da10ca045fe9cc7863b0785f468d83c3884456efd567e97536 in / 
# Tue, 04 Jul 2023 01:38:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d4b098d102e200415126181e2b6aae752738430f2ed6a601f84baf29e2f7d01`  
		Last Modified: Tue, 04 Jul 2023 01:43:28 GMT  
		Size: 30.1 MB (30140746 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; mips64le

```console
$ docker pull debian@sha256:c53575e9b0715d36e43f0ebfe55c30f46f9371b7dbe2c19d437856f05897f613
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fbaec4c66739ebe7fa73ea3959cd0dc5be32570e4a368278cd81d16c1ab17210`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:28 GMT
ADD file:cf4b72a35644a7b9f5f1ca39a54e484822f408f6821f0800f1167ae48ad6e38e in / 
# Tue, 04 Jul 2023 01:09:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:502a81d5004983c2a770a910286271410d8e2b20d0cb61ef7397e664e1e72654`  
		Last Modified: Tue, 04 Jul 2023 01:19:57 GMT  
		Size: 29.1 MB (29118201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:4cd58b1861586c2cb4f2b21789de2d2a0844720ffa22dd00cd597cde4c559726
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116716 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d9dede7363d225c4c296d0ef903b60be1caecbfcb776c47194d83441f355e520`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:51 GMT
ADD file:51887002f5002ccc662adbc6ecf1129e3db375a7a77b93da43cc9a8326b3c4b9 in / 
# Tue, 04 Jul 2023 01:17:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba99a1aa5a92d949d988e66df7723a542fd81bc30eef9d4269e8a899820d4bb2`  
		Last Modified: Tue, 04 Jul 2023 01:24:36 GMT  
		Size: 33.1 MB (33116716 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.0-slim` - linux; s390x

```console
$ docker pull debian@sha256:6ceb743c3afa0c403649e34d7005148bcf5154d84b11342cc74a5230561bc9ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487968 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b251db288e6297bfcb4d93a80d9acdc5f153ecb9ba980f4b946855505c302b6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:13 GMT
ADD file:7c6e123333ed463aaf550f8adb75415706fc8573337b76140d393910a5166731 in / 
# Tue, 04 Jul 2023 01:32:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0df7967bae0170739ec7f61c7bf3adbc2799d4ca7704b10725a46942717891fc`  
		Last Modified: Tue, 04 Jul 2023 01:37:07 GMT  
		Size: 27.5 MB (27487968 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm`

```console
$ docker pull debian@sha256:3d868b5eb908155f3784317b3dda2941df87bbbbaa4608f84881de66d9bb297b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm` - linux; amd64

```console
$ docker pull debian@sha256:60774985572749dc3c39147d43089d53e7ce17b844eebcf619d84467160217ab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3676c78a12ad77e170916695d2f192797ddd3eb653d7a1b1ad4c20ab5471891a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v5

```console
$ docker pull debian@sha256:e1eedd931159f74699128b11375ab15a76fed96ea29dedce88588cfaa19b564a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402786 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e836de3c00fa0d0942c5e3ea21d9e0525c92d38c0e8a1dc39a659ec87689a34d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v7

```console
$ docker pull debian@sha256:722595c25d744c903165d26d37fdd2f2ead8d9ceb326deb260173f48725be078
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca910bfe472fc0e9b55301d7ad047098587c598dfd5ac44c010e202d0f7d97bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:345b9265e89c123f994b724b9a4761dbc91c5759c144d01292ca56a338170801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572781 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:886d61a022482574344910f88e436ce64e9218cc8d62bea0cac035c4519ff93e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; 386

```console
$ docker pull debian@sha256:699f29298cb47296a0b0be2d6a1d18b0ab5ca77986862949a665e4524bc0b781
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9743376b1f2144d61495397e2a7f044bbc25d61016cee4dbff8683d31eaed7fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; mips64le

```console
$ docker pull debian@sha256:67103735074efb1301d79591c39140d7362fecc0898dda01a6ab7fcef1500a2f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5f8b2d159b8a89e52d66238252e674cf2d57e11c558c82ab3c4eac593ac893ef`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c051e548fc05b1fa5d532ceffddb1ff227aa57b81b4fcc91cbf627ebb6f0a3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec2232f2e4f8c6c1f0fb3f88f17e3c7c6e3ef4040ebbaa9588f7d2e9de7a07bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; s390x

```console
$ docker pull debian@sha256:aaa4fc6ed87ddc782bba21f6dad6bdeeb0a857c7d5530e19f5d8b5134b8bc9f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921709 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9a90a19fe78ac6ff9bedf17db184107da34ef5c182a64c226910e165d1c11e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20230703`

```console
$ docker pull debian@sha256:3d868b5eb908155f3784317b3dda2941df87bbbbaa4608f84881de66d9bb297b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20230703` - linux; amd64

```console
$ docker pull debian@sha256:60774985572749dc3c39147d43089d53e7ce17b844eebcf619d84467160217ab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3676c78a12ad77e170916695d2f192797ddd3eb653d7a1b1ad4c20ab5471891a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:e1eedd931159f74699128b11375ab15a76fed96ea29dedce88588cfaa19b564a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402786 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e836de3c00fa0d0942c5e3ea21d9e0525c92d38c0e8a1dc39a659ec87689a34d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:722595c25d744c903165d26d37fdd2f2ead8d9ceb326deb260173f48725be078
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca910bfe472fc0e9b55301d7ad047098587c598dfd5ac44c010e202d0f7d97bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:345b9265e89c123f994b724b9a4761dbc91c5759c144d01292ca56a338170801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572781 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:886d61a022482574344910f88e436ce64e9218cc8d62bea0cac035c4519ff93e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; 386

```console
$ docker pull debian@sha256:699f29298cb47296a0b0be2d6a1d18b0ab5ca77986862949a665e4524bc0b781
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9743376b1f2144d61495397e2a7f044bbc25d61016cee4dbff8683d31eaed7fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:67103735074efb1301d79591c39140d7362fecc0898dda01a6ab7fcef1500a2f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5f8b2d159b8a89e52d66238252e674cf2d57e11c558c82ab3c4eac593ac893ef`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c051e548fc05b1fa5d532ceffddb1ff227aa57b81b4fcc91cbf627ebb6f0a3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec2232f2e4f8c6c1f0fb3f88f17e3c7c6e3ef4040ebbaa9588f7d2e9de7a07bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703` - linux; s390x

```console
$ docker pull debian@sha256:aaa4fc6ed87ddc782bba21f6dad6bdeeb0a857c7d5530e19f5d8b5134b8bc9f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921709 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9a90a19fe78ac6ff9bedf17db184107da34ef5c182a64c226910e165d1c11e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20230703-slim`

```console
$ docker pull debian@sha256:9bd077d2f77c754f4f7f5ee9e6ded9ff1dff92c6dce877754da21b917c122c77
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:01bd742e2c269abf94e2fefb47b08b5b61c9a880b993417d23a1d0bd9fa60dc4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124829 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc29b27ee12fa143e61ee3c4d0b1eabf114ae5626311ee0a160da5e944abc5f6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:58 GMT
ADD file:bd80a4461150784e5f2f5a1faa720cc347ad3e30ee0969adbfad574c316f5aef in / 
# Tue, 04 Jul 2023 01:19:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:faef57eae888cbe4a5613eca6741b5e48d768b83f6088858aee9a5a2834f8151`  
		Last Modified: Tue, 04 Jul 2023 01:25:03 GMT  
		Size: 29.1 MB (29124829 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:878cb8c0829de1282ccf16bedf888f1cc75036af82876dccd0d63d22cc7a610c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2f063e6f8f6133d209c28499afbd0d55e73a41c0aaa028d0c0342bcdf2c6f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:31 GMT
ADD file:ff5b6029e7e4f2b427f101b31a8dd8d15a3a1204bf9b0576e65270de33ea2f62 in / 
# Tue, 04 Jul 2023 00:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da4ec93bf9d039ea6c48e256f41d88db6aed3ac72074f4cffb3f0c1b41ccac78`  
		Last Modified: Tue, 04 Jul 2023 00:51:54 GMT  
		Size: 27.0 MB (26982160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:f4744abcaaf86307e611493d086e5a6d2a186c7d36f9f63ded725cff9a922cf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801264 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b481106f9ce5f5ef94b896d8c37a6d955ab1e5940345e7c97493096d42385bf7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:54 GMT
ADD file:d168d1710cbca4edb322c26348dafaf2b3a64980f52b8b790f334cdbd6a7047e in / 
# Tue, 04 Jul 2023 00:57:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:340ace0baa422f18db718d7634c1c88a9650f4871dda2630ef15577120aa6816`  
		Last Modified: Tue, 04 Jul 2023 01:02:50 GMT  
		Size: 24.8 MB (24801264 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0ef04748d071c2e631bb3edce8f805cb5512e746b682c83fdae6d8c0b243280b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152458 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc86621bf22e303a42e1f5872b7d07b76d42b1dc7e01121809212641839a0949`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:35 GMT
ADD file:71fd66666294148382f2e6a09ae5e277d4c4e9c74402ab64b693a79387b67a09 in / 
# Tue, 04 Jul 2023 01:57:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3ae0c06b4d3aa97d7e0829233dd36cea1666b87074e55fea6bd1ecae066693c7`  
		Last Modified: Tue, 04 Jul 2023 02:01:20 GMT  
		Size: 29.2 MB (29152458 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:f00eb751c2d09b95f5eca3c79b0b38fbb3a5a72171460ae56809a20164c7fa11
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140746 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa31d56c510cfe00ea3b0d08b598f9886afa251a17ee3037715aa1aa2eb5bd47`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:36 GMT
ADD file:441addef37edd5da10ca045fe9cc7863b0785f468d83c3884456efd567e97536 in / 
# Tue, 04 Jul 2023 01:38:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d4b098d102e200415126181e2b6aae752738430f2ed6a601f84baf29e2f7d01`  
		Last Modified: Tue, 04 Jul 2023 01:43:28 GMT  
		Size: 30.1 MB (30140746 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:c53575e9b0715d36e43f0ebfe55c30f46f9371b7dbe2c19d437856f05897f613
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fbaec4c66739ebe7fa73ea3959cd0dc5be32570e4a368278cd81d16c1ab17210`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:28 GMT
ADD file:cf4b72a35644a7b9f5f1ca39a54e484822f408f6821f0800f1167ae48ad6e38e in / 
# Tue, 04 Jul 2023 01:09:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:502a81d5004983c2a770a910286271410d8e2b20d0cb61ef7397e664e1e72654`  
		Last Modified: Tue, 04 Jul 2023 01:19:57 GMT  
		Size: 29.1 MB (29118201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:4cd58b1861586c2cb4f2b21789de2d2a0844720ffa22dd00cd597cde4c559726
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116716 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d9dede7363d225c4c296d0ef903b60be1caecbfcb776c47194d83441f355e520`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:51 GMT
ADD file:51887002f5002ccc662adbc6ecf1129e3db375a7a77b93da43cc9a8326b3c4b9 in / 
# Tue, 04 Jul 2023 01:17:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba99a1aa5a92d949d988e66df7723a542fd81bc30eef9d4269e8a899820d4bb2`  
		Last Modified: Tue, 04 Jul 2023 01:24:36 GMT  
		Size: 33.1 MB (33116716 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:6ceb743c3afa0c403649e34d7005148bcf5154d84b11342cc74a5230561bc9ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487968 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b251db288e6297bfcb4d93a80d9acdc5f153ecb9ba980f4b946855505c302b6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:13 GMT
ADD file:7c6e123333ed463aaf550f8adb75415706fc8573337b76140d393910a5166731 in / 
# Tue, 04 Jul 2023 01:32:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0df7967bae0170739ec7f61c7bf3adbc2799d4ca7704b10725a46942717891fc`  
		Last Modified: Tue, 04 Jul 2023 01:37:07 GMT  
		Size: 27.5 MB (27487968 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-backports`

```console
$ docker pull debian@sha256:45986d7da66cf4211ed0c3f5e5643e28c96709a3aa8051251f78818afb8e8f18
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-backports` - linux; amd64

```console
$ docker pull debian@sha256:b169c2cf97898d33cf0f9732643f2ac049f411fed455e883e0b37b3d4f5b31d8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551656 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f1e7e3eb1e0ab9997e94765d06a353aa08d455dc2aa0b161392335e88fbaf8f3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:19:49 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e80682bca8042a81d167d26bb331b9837b042d8d1934fc707b744f04114ac94a`  
		Last Modified: Tue, 04 Jul 2023 01:24:53 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:685905ae2de8df0c3e164a787377af1585e3630ca14007f00d3f310bcf55e718
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47403011 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa0f960c2e7c48328c1def5b6f4b328a7ee2778738ecc2960146f2f624e24fc3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:48:26 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:111794510e313108d92691330a5947a9996ab861b2c970b2c4423bcde8605172`  
		Last Modified: Tue, 04 Jul 2023 00:51:45 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:3fafd6c274b0fddbfbce6fc9aa88c783c6202c5575d526fbcd2ef8af25af65ef
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236426 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d047ca742e5a12c251806d3e8c69340d31602c584cb9b4a947bd3cf0a81baf6d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:57:46 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b6558e9eb6646f3cc0af1052a3a9f2ed3a88e5a3e33ad0a806ee77d857b26dc5`  
		Last Modified: Tue, 04 Jul 2023 01:02:40 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fa5e22cdd79dc6bdf1e2a65b99fcc74db5c8ab248ef40ad18655b843c734a220
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49573006 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:346b080edc9c2df3610f3601d2cae327fcb638e5ba305439c4535ebd16a526b9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:57:29 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c5ab5ece0fe18118d32daa98b7010ab29d58ed2c65aa823a88ba9ce2356b4d0b`  
		Last Modified: Tue, 04 Jul 2023 02:01:11 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; 386

```console
$ docker pull debian@sha256:818f25271e5c021594d8d69080121ebfb4db876a3945e77a77ce46acc2bec1a5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562614 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:558e35fef2f71b6305496dae0739591d538784bd99a6632b50b741f1f714d27a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:38:28 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b742a20090779a34a45249a3283611726c42c087e0fd7a0488427f0299b4c58a`  
		Last Modified: Tue, 04 Jul 2023 01:43:16 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; mips64le

```console
$ docker pull debian@sha256:25cdd4baaaa2a8d2969a6e492f6732a791f2143840fa9be951eeb49a8c46cca6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542369 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09ffb6186eef8eaedd3436861ddef5b97f7af46d4978fc0fbbec8abdd03e6ceb`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:09:05 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b6e06f7c231fa5a11cd333646ad54b7a628d8cceb2ce48842ba9eae85d279a6f`  
		Last Modified: Tue, 04 Jul 2023 01:19:32 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:56e56a9923f2c480133f840026f3389c29322a42055e51c8cd09632b670da4be
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536966 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:81f14b6596252a15653fff7ab47831720f0e960ad7fd708e055b4314ea3bce74`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:17:38 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a03702ace77d8f2a14b0beae6a5a330dc8271920848ce13a7ace1f7ee85636e8`  
		Last Modified: Tue, 04 Jul 2023 01:24:20 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; s390x

```console
$ docker pull debian@sha256:9ba91622dbeed74acf63570c8e652a612982128a61cd569600379da2b452e807
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921933 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a2fb396d864cbee4dbd4ddfbb842e1f097fce6e93891953ef3777c6600b9d51f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:32:04 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f76bf47dab0a951d9c543cb0843ef93b9097f8957b092be7f808d49e379ef935`  
		Last Modified: Tue, 04 Jul 2023 01:37:00 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-slim`

```console
$ docker pull debian@sha256:9bd077d2f77c754f4f7f5ee9e6ded9ff1dff92c6dce877754da21b917c122c77
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-slim` - linux; amd64

```console
$ docker pull debian@sha256:01bd742e2c269abf94e2fefb47b08b5b61c9a880b993417d23a1d0bd9fa60dc4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124829 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc29b27ee12fa143e61ee3c4d0b1eabf114ae5626311ee0a160da5e944abc5f6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:58 GMT
ADD file:bd80a4461150784e5f2f5a1faa720cc347ad3e30ee0969adbfad574c316f5aef in / 
# Tue, 04 Jul 2023 01:19:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:faef57eae888cbe4a5613eca6741b5e48d768b83f6088858aee9a5a2834f8151`  
		Last Modified: Tue, 04 Jul 2023 01:25:03 GMT  
		Size: 29.1 MB (29124829 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:878cb8c0829de1282ccf16bedf888f1cc75036af82876dccd0d63d22cc7a610c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2f063e6f8f6133d209c28499afbd0d55e73a41c0aaa028d0c0342bcdf2c6f3ee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:31 GMT
ADD file:ff5b6029e7e4f2b427f101b31a8dd8d15a3a1204bf9b0576e65270de33ea2f62 in / 
# Tue, 04 Jul 2023 00:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:da4ec93bf9d039ea6c48e256f41d88db6aed3ac72074f4cffb3f0c1b41ccac78`  
		Last Modified: Tue, 04 Jul 2023 00:51:54 GMT  
		Size: 27.0 MB (26982160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:f4744abcaaf86307e611493d086e5a6d2a186c7d36f9f63ded725cff9a922cf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801264 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b481106f9ce5f5ef94b896d8c37a6d955ab1e5940345e7c97493096d42385bf7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:54 GMT
ADD file:d168d1710cbca4edb322c26348dafaf2b3a64980f52b8b790f334cdbd6a7047e in / 
# Tue, 04 Jul 2023 00:57:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:340ace0baa422f18db718d7634c1c88a9650f4871dda2630ef15577120aa6816`  
		Last Modified: Tue, 04 Jul 2023 01:02:50 GMT  
		Size: 24.8 MB (24801264 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0ef04748d071c2e631bb3edce8f805cb5512e746b682c83fdae6d8c0b243280b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152458 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc86621bf22e303a42e1f5872b7d07b76d42b1dc7e01121809212641839a0949`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:35 GMT
ADD file:71fd66666294148382f2e6a09ae5e277d4c4e9c74402ab64b693a79387b67a09 in / 
# Tue, 04 Jul 2023 01:57:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3ae0c06b4d3aa97d7e0829233dd36cea1666b87074e55fea6bd1ecae066693c7`  
		Last Modified: Tue, 04 Jul 2023 02:01:20 GMT  
		Size: 29.2 MB (29152458 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; 386

```console
$ docker pull debian@sha256:f00eb751c2d09b95f5eca3c79b0b38fbb3a5a72171460ae56809a20164c7fa11
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140746 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa31d56c510cfe00ea3b0d08b598f9886afa251a17ee3037715aa1aa2eb5bd47`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:36 GMT
ADD file:441addef37edd5da10ca045fe9cc7863b0785f468d83c3884456efd567e97536 in / 
# Tue, 04 Jul 2023 01:38:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d4b098d102e200415126181e2b6aae752738430f2ed6a601f84baf29e2f7d01`  
		Last Modified: Tue, 04 Jul 2023 01:43:28 GMT  
		Size: 30.1 MB (30140746 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; mips64le

```console
$ docker pull debian@sha256:c53575e9b0715d36e43f0ebfe55c30f46f9371b7dbe2c19d437856f05897f613
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fbaec4c66739ebe7fa73ea3959cd0dc5be32570e4a368278cd81d16c1ab17210`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:28 GMT
ADD file:cf4b72a35644a7b9f5f1ca39a54e484822f408f6821f0800f1167ae48ad6e38e in / 
# Tue, 04 Jul 2023 01:09:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:502a81d5004983c2a770a910286271410d8e2b20d0cb61ef7397e664e1e72654`  
		Last Modified: Tue, 04 Jul 2023 01:19:57 GMT  
		Size: 29.1 MB (29118201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:4cd58b1861586c2cb4f2b21789de2d2a0844720ffa22dd00cd597cde4c559726
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116716 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d9dede7363d225c4c296d0ef903b60be1caecbfcb776c47194d83441f355e520`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:51 GMT
ADD file:51887002f5002ccc662adbc6ecf1129e3db375a7a77b93da43cc9a8326b3c4b9 in / 
# Tue, 04 Jul 2023 01:17:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba99a1aa5a92d949d988e66df7723a542fd81bc30eef9d4269e8a899820d4bb2`  
		Last Modified: Tue, 04 Jul 2023 01:24:36 GMT  
		Size: 33.1 MB (33116716 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; s390x

```console
$ docker pull debian@sha256:6ceb743c3afa0c403649e34d7005148bcf5154d84b11342cc74a5230561bc9ee
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487968 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b251db288e6297bfcb4d93a80d9acdc5f153ecb9ba980f4b946855505c302b6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:13 GMT
ADD file:7c6e123333ed463aaf550f8adb75415706fc8573337b76140d393910a5166731 in / 
# Tue, 04 Jul 2023 01:32:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0df7967bae0170739ec7f61c7bf3adbc2799d4ca7704b10725a46942717891fc`  
		Last Modified: Tue, 04 Jul 2023 01:37:07 GMT  
		Size: 27.5 MB (27487968 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye`

```console
$ docker pull debian@sha256:a648e10e02af129706b1fb89e1ac9694ae3db7f2b8439aa906321e68cc281bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye` - linux; amd64

```console
$ docker pull debian@sha256:3b6053ca925336c804e2d3f080af177efcdc9f51198a627569bfc7c7e730ef7e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:189a2f977ff1e0aeb33038f380dd9a6ab0355d27ec82f10d23567b95386506ce`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:b7c0be2bb90e88689b1c16da78dea0b85760b55b90ef2e5bc4a529c89e4fa25b in / 
# Tue, 04 Jul 2023 01:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34df401c391c7595044379e04e8ad4856a5a3974cdbf5a160f0a204d761e88aa`  
		Last Modified: Tue, 04 Jul 2023 01:25:21 GMT  
		Size: 55.1 MB (55055300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v5

```console
$ docker pull debian@sha256:ea256bee22dad6b8949beae7c7fa2e40529b8c35451eabe372804c277d6f8f31
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556778 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6db0bcb926823fd1a67160b99db57bff4e133735b9ebc9c945352dd3f1a27b8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:39 GMT
ADD file:fe1b9f9f6c7d67ad23a2ee839774be4bcee33c60c7b34c48df5a08eb33cafd1b in / 
# Tue, 04 Jul 2023 00:48:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:702dd8b607f626c689176debe921fe96009c1ce6dbd66f4f238c7def50080e3d`  
		Last Modified: Tue, 04 Jul 2023 00:52:15 GMT  
		Size: 52.6 MB (52556778 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v7

```console
$ docker pull debian@sha256:819b11bd0ade30fbc72f4b83593d0d126b2f3329b053495833219213fe37714d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218247 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f019634bdb2ec508d690cc1e60eb323089e52863f495460d59f317955c09be44`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:06 GMT
ADD file:17e02296458241d9441f8da6a5dafb747d528a729106b17cec2f4c1c8cfe0ad8 in / 
# Tue, 04 Jul 2023 00:58:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31edf2db9ca1650aa08e2d42e9b5bb7349413d7212110149a1a5d202ac20914b`  
		Last Modified: Tue, 04 Jul 2023 01:03:12 GMT  
		Size: 50.2 MB (50218247 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:de3c0d12dd75f1a47595ff0ce78f2d30d6ca95c3ad66af06c8815d1f9b8e208d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a057fce101fd55e46c91786b7fe2c9e80353b088c83a6eb63d2bcb48dc4b47e9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:43 GMT
ADD file:e626446584d8094b7b58d72a717380ca64d3e9ab924fc625406fe26a83fe1d8b in / 
# Tue, 04 Jul 2023 01:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:29279ac7c19f9c667f1c6b07bfba6fba20ca0d945b9fbc6edad6f75d13361fae`  
		Last Modified: Tue, 04 Jul 2023 02:01:38 GMT  
		Size: 53.7 MB (53703979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; 386

```console
$ docker pull debian@sha256:208ef91ebfbfb49f90dc2917f9c0b82a627ab669d1dab21a29b3bc4598cd2256
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040756 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:985169c5e2dedb21701be6f461b6449ce9c4cc1ff316e5d1e08917d620f6632a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:48 GMT
ADD file:c5739407c2c257fbb85ab4b9dcd2dc07e6fe172d7309aaaaab544c8df6c42b92 in / 
# Tue, 04 Jul 2023 01:38:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b700c8b35944e0d5f6ab9049dbe2262492e26327a8efad518b1011428393653f`  
		Last Modified: Tue, 04 Jul 2023 01:43:49 GMT  
		Size: 56.0 MB (56040756 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; mips64le

```console
$ docker pull debian@sha256:9e54eef8b57fa51064806c0ed4d9ff8f66796190d4cbc7f4bc709fff8b4c70e2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:78934b34e1d526823f453d4f4e8827177b5161276a1ed8fe2f4dac41e3a2e367`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:02 GMT
ADD file:f7ddd7406e3aa24165fc80933583fc2298f9792865511fe65def2f571fed2207 in / 
# Tue, 04 Jul 2023 01:10:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af98e96208d7ce4cff5417a3e8a38f98d2ed3abbca318e1e6c323aa8b8c56690`  
		Last Modified: Tue, 04 Jul 2023 01:20:40 GMT  
		Size: 53.3 MB (53270472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; ppc64le

```console
$ docker pull debian@sha256:bcf5819ab888818cf7eeae793b9e27c6a9221a9625c9e64e2bc785b2232cdaf1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe24e6ff73b4dffad0551a211517a8fafa464e56a39f0e8adf686ec80a4fa968`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:11 GMT
ADD file:ff86c25854c28609c2852c1615270f0acd4c4efaa38ff8ed9c23afe5132f2135 in / 
# Tue, 04 Jul 2023 01:18:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a792749f62225048efdf85841bea6f3999f586371852f3dbce13a6c3d89b1fa7`  
		Last Modified: Tue, 04 Jul 2023 01:25:06 GMT  
		Size: 58.9 MB (58927745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; s390x

```console
$ docker pull debian@sha256:d31138981e215f44472b534c1d30f8dea3eccce1c5e82a730cca2c877e7e0bf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288197 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:251b768a790fd5b8507b454503dd3d193fc688065dfe09b6d3071563ca07651f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:26 GMT
ADD file:231ba6ce8d3ee30318948799a94cb007f1517ea0d14c2b84863012cac37d6c6b in / 
# Tue, 04 Jul 2023 01:32:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1e40ed2a44a8c2786b06584afad765d97e9b1c910f58ae426622ba17fbf3d4c3`  
		Last Modified: Tue, 04 Jul 2023 01:37:21 GMT  
		Size: 53.3 MB (53288197 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20230703`

```console
$ docker pull debian@sha256:a648e10e02af129706b1fb89e1ac9694ae3db7f2b8439aa906321e68cc281bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20230703` - linux; amd64

```console
$ docker pull debian@sha256:3b6053ca925336c804e2d3f080af177efcdc9f51198a627569bfc7c7e730ef7e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055300 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:189a2f977ff1e0aeb33038f380dd9a6ab0355d27ec82f10d23567b95386506ce`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:b7c0be2bb90e88689b1c16da78dea0b85760b55b90ef2e5bc4a529c89e4fa25b in / 
# Tue, 04 Jul 2023 01:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34df401c391c7595044379e04e8ad4856a5a3974cdbf5a160f0a204d761e88aa`  
		Last Modified: Tue, 04 Jul 2023 01:25:21 GMT  
		Size: 55.1 MB (55055300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:ea256bee22dad6b8949beae7c7fa2e40529b8c35451eabe372804c277d6f8f31
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556778 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6db0bcb926823fd1a67160b99db57bff4e133735b9ebc9c945352dd3f1a27b8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:39 GMT
ADD file:fe1b9f9f6c7d67ad23a2ee839774be4bcee33c60c7b34c48df5a08eb33cafd1b in / 
# Tue, 04 Jul 2023 00:48:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:702dd8b607f626c689176debe921fe96009c1ce6dbd66f4f238c7def50080e3d`  
		Last Modified: Tue, 04 Jul 2023 00:52:15 GMT  
		Size: 52.6 MB (52556778 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:819b11bd0ade30fbc72f4b83593d0d126b2f3329b053495833219213fe37714d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218247 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f019634bdb2ec508d690cc1e60eb323089e52863f495460d59f317955c09be44`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:06 GMT
ADD file:17e02296458241d9441f8da6a5dafb747d528a729106b17cec2f4c1c8cfe0ad8 in / 
# Tue, 04 Jul 2023 00:58:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:31edf2db9ca1650aa08e2d42e9b5bb7349413d7212110149a1a5d202ac20914b`  
		Last Modified: Tue, 04 Jul 2023 01:03:12 GMT  
		Size: 50.2 MB (50218247 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:de3c0d12dd75f1a47595ff0ce78f2d30d6ca95c3ad66af06c8815d1f9b8e208d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703979 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a057fce101fd55e46c91786b7fe2c9e80353b088c83a6eb63d2bcb48dc4b47e9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:43 GMT
ADD file:e626446584d8094b7b58d72a717380ca64d3e9ab924fc625406fe26a83fe1d8b in / 
# Tue, 04 Jul 2023 01:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:29279ac7c19f9c667f1c6b07bfba6fba20ca0d945b9fbc6edad6f75d13361fae`  
		Last Modified: Tue, 04 Jul 2023 02:01:38 GMT  
		Size: 53.7 MB (53703979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; 386

```console
$ docker pull debian@sha256:208ef91ebfbfb49f90dc2917f9c0b82a627ab669d1dab21a29b3bc4598cd2256
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040756 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:985169c5e2dedb21701be6f461b6449ce9c4cc1ff316e5d1e08917d620f6632a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:48 GMT
ADD file:c5739407c2c257fbb85ab4b9dcd2dc07e6fe172d7309aaaaab544c8df6c42b92 in / 
# Tue, 04 Jul 2023 01:38:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b700c8b35944e0d5f6ab9049dbe2262492e26327a8efad518b1011428393653f`  
		Last Modified: Tue, 04 Jul 2023 01:43:49 GMT  
		Size: 56.0 MB (56040756 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:9e54eef8b57fa51064806c0ed4d9ff8f66796190d4cbc7f4bc709fff8b4c70e2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:78934b34e1d526823f453d4f4e8827177b5161276a1ed8fe2f4dac41e3a2e367`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:02 GMT
ADD file:f7ddd7406e3aa24165fc80933583fc2298f9792865511fe65def2f571fed2207 in / 
# Tue, 04 Jul 2023 01:10:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af98e96208d7ce4cff5417a3e8a38f98d2ed3abbca318e1e6c323aa8b8c56690`  
		Last Modified: Tue, 04 Jul 2023 01:20:40 GMT  
		Size: 53.3 MB (53270472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:bcf5819ab888818cf7eeae793b9e27c6a9221a9625c9e64e2bc785b2232cdaf1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe24e6ff73b4dffad0551a211517a8fafa464e56a39f0e8adf686ec80a4fa968`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:11 GMT
ADD file:ff86c25854c28609c2852c1615270f0acd4c4efaa38ff8ed9c23afe5132f2135 in / 
# Tue, 04 Jul 2023 01:18:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a792749f62225048efdf85841bea6f3999f586371852f3dbce13a6c3d89b1fa7`  
		Last Modified: Tue, 04 Jul 2023 01:25:06 GMT  
		Size: 58.9 MB (58927745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703` - linux; s390x

```console
$ docker pull debian@sha256:d31138981e215f44472b534c1d30f8dea3eccce1c5e82a730cca2c877e7e0bf7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288197 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:251b768a790fd5b8507b454503dd3d193fc688065dfe09b6d3071563ca07651f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:26 GMT
ADD file:231ba6ce8d3ee30318948799a94cb007f1517ea0d14c2b84863012cac37d6c6b in / 
# Tue, 04 Jul 2023 01:32:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1e40ed2a44a8c2786b06584afad765d97e9b1c910f58ae426622ba17fbf3d4c3`  
		Last Modified: Tue, 04 Jul 2023 01:37:21 GMT  
		Size: 53.3 MB (53288197 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20230703-slim`

```console
$ docker pull debian@sha256:3460d74bec6b88496cd183d7731930be55234c094f581f7dbdd96f56c1fc34d8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:0781db7f3f74866059cf161d145af0bc16fb13493b6e0f87ca20d7e34498d691
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:02ad3ff027defef6c8c23678fc5aee4c4f7b73af047688850671ccf4469b8641`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:23 GMT
ADD file:4a063d4e089ef10c6806d7042a0040078674ac2db61df02df8bbb8fa4894910a in / 
# Tue, 04 Jul 2023 01:20:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9d21b12d5fab9ab82969054d72411ce627c209257df64b6057016c981e163c30`  
		Last Modified: Tue, 04 Jul 2023 01:25:43 GMT  
		Size: 31.4 MB (31417388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:89e6030fba6589df6fa2faa2c4b8e64d50c412ffc4ebed83f52d9ce66bb1666d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918872 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:087af48210389738e9f2d5f949267d7ba7914194f0278289b82170e274d4209e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:47 GMT
ADD file:2dc75d45982de1ae93d7466556a8d6f806e24a837046bd3445b5df6853b0b5bb in / 
# Tue, 04 Jul 2023 00:48:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e5c23077f8d036e239b1eb6572155131327f3345abd95ce868b997d2e1d969f3`  
		Last Modified: Tue, 04 Jul 2023 00:52:38 GMT  
		Size: 28.9 MB (28918872 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c39bd3c81e12b4e81c32d6d4390b8dbc0c86d2028d452a556f1a8e27d916f8ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578754 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d296fbefde2b415d8164b52defaf05f117e02fb9cbe59e48efbc868e1b70be2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:20 GMT
ADD file:c023c66ee4b7cdae5c542f2ad2dd35aef94ad24e1b3b479a16538c46013ae6a5 in / 
# Tue, 04 Jul 2023 00:58:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d9e6a8b782e380f447723ac26499c5014f2d383b9210819b9e73e97abaf81249`  
		Last Modified: Tue, 04 Jul 2023 01:03:35 GMT  
		Size: 26.6 MB (26578754 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:511660e7e3f8f5d84a6c33710a4e3ccd9bcc19c144631b0b4425203f723ec941
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062957 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bac24d73af3aa85d8ec803623835b945fa61d05f35ad1ef7ed064a2a63a3e4e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:52 GMT
ADD file:83a81aad5cdb80c654a520d913c8bcafe2b8e1062d81c389d4577cde5ad68167 in / 
# Tue, 04 Jul 2023 01:57:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:50eb042e2421869704212f3e076e9088033eb9a5254341fb1b3022e6e2784921`  
		Last Modified: Tue, 04 Jul 2023 02:02:00 GMT  
		Size: 30.1 MB (30062957 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:50fd2f587932e94cb3ea95e71d9ae31db61d2833e37ededbfe1aacebd255c801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397496 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b900b795d418649b0bce493e8747d485ee86acde60c0d033724cfd4a8a4ec394`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:59 GMT
ADD file:c39e332ccb9cb890ba7896a09e9f074b3c8d67094b4eaf73d5c67e8949c2e0be in / 
# Tue, 04 Jul 2023 01:39:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3466e7f9e8ec982697fc130dbb1706d708f5c53e348a638019a955c15c7395af`  
		Last Modified: Tue, 04 Jul 2023 01:44:12 GMT  
		Size: 32.4 MB (32397496 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:4fc1f150825beac7afd57d3600835a818770a46aa2605c9c030e4e5c597c7484
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eda43c8166bfdc0308049da49e15c03def4c527ec5da9c4ad14fc73275039630`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:44 GMT
ADD file:fa125b6265458c9912ef0f591e20d187cdc4b3a5222cb3f066e997a9baa4d699 in / 
# Tue, 04 Jul 2023 01:10:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5fa30c60fbae4cff4aa6f1626e53313c5fd7d3ed439bb87d067b66af6b1c07fd`  
		Last Modified: Tue, 04 Jul 2023 01:21:20 GMT  
		Size: 29.6 MB (29634364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:381c57ee33b625a5e9f5c4f5629bbc30b881a5248e0a75c45de44f9ecc4bbb14
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291082 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5a0d1f1d6b415be4f9b9f9669c4fc468645ddf3efeaa638129b78c367e64b1d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:33 GMT
ADD file:37fa020aca7253d41d395ed529c38db73caaa4da098754836244552f65fa7d5d in / 
# Tue, 04 Jul 2023 01:18:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:147bb8b5828c99cd6b07d252d2987490463c142099eaa0815025685f8fa4f3d8`  
		Last Modified: Tue, 04 Jul 2023 01:25:40 GMT  
		Size: 35.3 MB (35291082 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:e5160c8c64122d433e9e48ea3b95a27c98c1daa9a8d01652064e73af6235d374
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f104358b3f81648e93f98e0d89a739db4e5aa32fcb42bc75d249d06640b6097`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:44 GMT
ADD file:799c0afa70fa3bf4bb7804964481cba79e80aa3fad5c7a25beadde206ed6a8ff in / 
# Tue, 04 Jul 2023 01:32:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbee891481f2623da9c7d37a49947926519c858c2b06773370a6189440d4b4de`  
		Last Modified: Tue, 04 Jul 2023 01:37:37 GMT  
		Size: 29.7 MB (29652540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-backports`

```console
$ docker pull debian@sha256:10651984486952dc07e2ad3fa7bbdf7fbf2f95f5489919110898c31e88484d9a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-backports` - linux; amd64

```console
$ docker pull debian@sha256:f5ec299e3f1fc6ba27a2d066becacbbb061cead756898b1a2a6d900542351524
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055525 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:39783992e08db2f199d8f2830b05bca644456b69c1930771bb97449a246abdbc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:b7c0be2bb90e88689b1c16da78dea0b85760b55b90ef2e5bc4a529c89e4fa25b in / 
# Tue, 04 Jul 2023 01:20:10 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:20:14 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:34df401c391c7595044379e04e8ad4856a5a3974cdbf5a160f0a204d761e88aa`  
		Last Modified: Tue, 04 Jul 2023 01:25:21 GMT  
		Size: 55.1 MB (55055300 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:aa19a1e07459e404e7712c9a2f50e0a015759f9ad59341c5884c6284707b1acb`  
		Last Modified: Tue, 04 Jul 2023 01:25:33 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:44fbc710ea5fc0538cacc1ca4d5d192f664aee11815d58f159bd50b136a02004
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52557003 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a9281d9310ac3c181ee076f1b68a43c994205fb260ef016599e4831f2e15595d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:39 GMT
ADD file:fe1b9f9f6c7d67ad23a2ee839774be4bcee33c60c7b34c48df5a08eb33cafd1b in / 
# Tue, 04 Jul 2023 00:48:40 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:48:42 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:702dd8b607f626c689176debe921fe96009c1ce6dbd66f4f238c7def50080e3d`  
		Last Modified: Tue, 04 Jul 2023 00:52:15 GMT  
		Size: 52.6 MB (52556778 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b10a170a2cf9bb1b9df44ef8581288cd8db7c03094b0c792d6b5ebcd6beb1866`  
		Last Modified: Tue, 04 Jul 2023 00:52:28 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:5767bcea30c0706e7083ef279d998f5ce0a11251a1735e9e7efe53b4dd5e3c42
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218471 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:be6b17bb9d70c83abff33da7de1d326f8e26c7538f7ac14565670579c969de9e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:06 GMT
ADD file:17e02296458241d9441f8da6a5dafb747d528a729106b17cec2f4c1c8cfe0ad8 in / 
# Tue, 04 Jul 2023 00:58:07 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:58:10 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:31edf2db9ca1650aa08e2d42e9b5bb7349413d7212110149a1a5d202ac20914b`  
		Last Modified: Tue, 04 Jul 2023 01:03:12 GMT  
		Size: 50.2 MB (50218247 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:72c11575019926ec464a3c162c92a4efa5ff0ac05a431f177207d025823bf40d`  
		Last Modified: Tue, 04 Jul 2023 01:03:24 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5e1357c01ddb0b9cc4073247c39285de2ebb708664d13772b5ea7ee0615bf0e0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53704204 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d769a25c8bdd74fe4d3a76377e0b4462a5a348dee5681a70c378432bf11a7908`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:43 GMT
ADD file:e626446584d8094b7b58d72a717380ca64d3e9ab924fc625406fe26a83fe1d8b in / 
# Tue, 04 Jul 2023 01:57:43 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:57:46 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:29279ac7c19f9c667f1c6b07bfba6fba20ca0d945b9fbc6edad6f75d13361fae`  
		Last Modified: Tue, 04 Jul 2023 02:01:38 GMT  
		Size: 53.7 MB (53703979 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1db10aa9e7b71463f6f33f69145327030478a975d596b6e9447c4a87561819eb`  
		Last Modified: Tue, 04 Jul 2023 02:01:51 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; 386

```console
$ docker pull debian@sha256:7b3531e17ec2b555579a6c11acb473f9910a2936b5eab96c59f804794fa0f144
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040984 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9cb80868b841c625277680ad1a97db4a3daaf87604595ccfceafcc0a119b2f80`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:48 GMT
ADD file:c5739407c2c257fbb85ab4b9dcd2dc07e6fe172d7309aaaaab544c8df6c42b92 in / 
# Tue, 04 Jul 2023 01:38:49 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:38:51 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b700c8b35944e0d5f6ab9049dbe2262492e26327a8efad518b1011428393653f`  
		Last Modified: Tue, 04 Jul 2023 01:43:49 GMT  
		Size: 56.0 MB (56040756 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:445bed08eb7e0c4bc4cc49a1a54db8d379f0c025eb623d36a5576f8ff0a19f7b`  
		Last Modified: Tue, 04 Jul 2023 01:44:00 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; mips64le

```console
$ docker pull debian@sha256:2753db08a096867fc24aca53dafacbaafc1dae6c974f4d2af5847e3a8fcf8a3e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270697 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7505d71f35126ee712f62b7d1caccf4bc39787f8d0b4fdd599aa665604b1efdb`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:02 GMT
ADD file:f7ddd7406e3aa24165fc80933583fc2298f9792865511fe65def2f571fed2207 in / 
# Tue, 04 Jul 2023 01:10:07 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:10:21 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:af98e96208d7ce4cff5417a3e8a38f98d2ed3abbca318e1e6c323aa8b8c56690`  
		Last Modified: Tue, 04 Jul 2023 01:20:40 GMT  
		Size: 53.3 MB (53270472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:60f6fb1c2fcb080f03a61eef6b0e20b57f4684e328f00202d760bfbfd0a8f043`  
		Last Modified: Tue, 04 Jul 2023 01:20:55 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:daa80aed4b5deaf2c5ba864a01b6eca4d3f9c7858df86e0ffd4d4b455e37293b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927973 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd7e8da683dd895282628eb901a0dbf95d47f5744255afe4f39869ff4f77e382`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:11 GMT
ADD file:ff86c25854c28609c2852c1615270f0acd4c4efaa38ff8ed9c23afe5132f2135 in / 
# Tue, 04 Jul 2023 01:18:15 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:18:20 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a792749f62225048efdf85841bea6f3999f586371852f3dbce13a6c3d89b1fa7`  
		Last Modified: Tue, 04 Jul 2023 01:25:06 GMT  
		Size: 58.9 MB (58927745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7c434ee1d4ac272227206ff97f81558cb557407d9ae335b6c4cc1ffa260adfd5`  
		Last Modified: Tue, 04 Jul 2023 01:25:23 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; s390x

```console
$ docker pull debian@sha256:dbae932dc1ef235859b7d74f5ca3310006be38aafefcf4edb56f6b8a4044d7e0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288421 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cb74db0349169eaf0aba25cafb4583437eddad59b34510d75caa3577680525fc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:26 GMT
ADD file:231ba6ce8d3ee30318948799a94cb007f1517ea0d14c2b84863012cac37d6c6b in / 
# Tue, 04 Jul 2023 01:32:29 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:32:35 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1e40ed2a44a8c2786b06584afad765d97e9b1c910f58ae426622ba17fbf3d4c3`  
		Last Modified: Tue, 04 Jul 2023 01:37:21 GMT  
		Size: 53.3 MB (53288197 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:05352767ae08e834acc10c3c08367bb427e51619a865e1f29b04661fbc05dfbc`  
		Last Modified: Tue, 04 Jul 2023 01:37:29 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-slim`

```console
$ docker pull debian@sha256:3460d74bec6b88496cd183d7731930be55234c094f581f7dbdd96f56c1fc34d8
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-slim` - linux; amd64

```console
$ docker pull debian@sha256:0781db7f3f74866059cf161d145af0bc16fb13493b6e0f87ca20d7e34498d691
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:02ad3ff027defef6c8c23678fc5aee4c4f7b73af047688850671ccf4469b8641`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:23 GMT
ADD file:4a063d4e089ef10c6806d7042a0040078674ac2db61df02df8bbb8fa4894910a in / 
# Tue, 04 Jul 2023 01:20:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9d21b12d5fab9ab82969054d72411ce627c209257df64b6057016c981e163c30`  
		Last Modified: Tue, 04 Jul 2023 01:25:43 GMT  
		Size: 31.4 MB (31417388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:89e6030fba6589df6fa2faa2c4b8e64d50c412ffc4ebed83f52d9ce66bb1666d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918872 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:087af48210389738e9f2d5f949267d7ba7914194f0278289b82170e274d4209e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:47 GMT
ADD file:2dc75d45982de1ae93d7466556a8d6f806e24a837046bd3445b5df6853b0b5bb in / 
# Tue, 04 Jul 2023 00:48:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e5c23077f8d036e239b1eb6572155131327f3345abd95ce868b997d2e1d969f3`  
		Last Modified: Tue, 04 Jul 2023 00:52:38 GMT  
		Size: 28.9 MB (28918872 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c39bd3c81e12b4e81c32d6d4390b8dbc0c86d2028d452a556f1a8e27d916f8ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578754 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d296fbefde2b415d8164b52defaf05f117e02fb9cbe59e48efbc868e1b70be2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:20 GMT
ADD file:c023c66ee4b7cdae5c542f2ad2dd35aef94ad24e1b3b479a16538c46013ae6a5 in / 
# Tue, 04 Jul 2023 00:58:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d9e6a8b782e380f447723ac26499c5014f2d383b9210819b9e73e97abaf81249`  
		Last Modified: Tue, 04 Jul 2023 01:03:35 GMT  
		Size: 26.6 MB (26578754 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:511660e7e3f8f5d84a6c33710a4e3ccd9bcc19c144631b0b4425203f723ec941
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062957 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bac24d73af3aa85d8ec803623835b945fa61d05f35ad1ef7ed064a2a63a3e4e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:52 GMT
ADD file:83a81aad5cdb80c654a520d913c8bcafe2b8e1062d81c389d4577cde5ad68167 in / 
# Tue, 04 Jul 2023 01:57:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:50eb042e2421869704212f3e076e9088033eb9a5254341fb1b3022e6e2784921`  
		Last Modified: Tue, 04 Jul 2023 02:02:00 GMT  
		Size: 30.1 MB (30062957 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; 386

```console
$ docker pull debian@sha256:50fd2f587932e94cb3ea95e71d9ae31db61d2833e37ededbfe1aacebd255c801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397496 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b900b795d418649b0bce493e8747d485ee86acde60c0d033724cfd4a8a4ec394`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:59 GMT
ADD file:c39e332ccb9cb890ba7896a09e9f074b3c8d67094b4eaf73d5c67e8949c2e0be in / 
# Tue, 04 Jul 2023 01:39:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3466e7f9e8ec982697fc130dbb1706d708f5c53e348a638019a955c15c7395af`  
		Last Modified: Tue, 04 Jul 2023 01:44:12 GMT  
		Size: 32.4 MB (32397496 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; mips64le

```console
$ docker pull debian@sha256:4fc1f150825beac7afd57d3600835a818770a46aa2605c9c030e4e5c597c7484
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eda43c8166bfdc0308049da49e15c03def4c527ec5da9c4ad14fc73275039630`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:44 GMT
ADD file:fa125b6265458c9912ef0f591e20d187cdc4b3a5222cb3f066e997a9baa4d699 in / 
# Tue, 04 Jul 2023 01:10:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5fa30c60fbae4cff4aa6f1626e53313c5fd7d3ed439bb87d067b66af6b1c07fd`  
		Last Modified: Tue, 04 Jul 2023 01:21:20 GMT  
		Size: 29.6 MB (29634364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:381c57ee33b625a5e9f5c4f5629bbc30b881a5248e0a75c45de44f9ecc4bbb14
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291082 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5a0d1f1d6b415be4f9b9f9669c4fc468645ddf3efeaa638129b78c367e64b1d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:33 GMT
ADD file:37fa020aca7253d41d395ed529c38db73caaa4da098754836244552f65fa7d5d in / 
# Tue, 04 Jul 2023 01:18:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:147bb8b5828c99cd6b07d252d2987490463c142099eaa0815025685f8fa4f3d8`  
		Last Modified: Tue, 04 Jul 2023 01:25:40 GMT  
		Size: 35.3 MB (35291082 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; s390x

```console
$ docker pull debian@sha256:e5160c8c64122d433e9e48ea3b95a27c98c1daa9a8d01652064e73af6235d374
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652540 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f104358b3f81648e93f98e0d89a739db4e5aa32fcb42bc75d249d06640b6097`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:44 GMT
ADD file:799c0afa70fa3bf4bb7804964481cba79e80aa3fad5c7a25beadde206ed6a8ff in / 
# Tue, 04 Jul 2023 01:32:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbee891481f2623da9c7d37a49947926519c858c2b06773370a6189440d4b4de`  
		Last Modified: Tue, 04 Jul 2023 01:37:37 GMT  
		Size: 29.7 MB (29652540 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster`

```console
$ docker pull debian@sha256:c21dbb23d41cb3f1c1a7f841e8642bf713934fb4dc5187979bd46f0b4b488616
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster` - linux; amd64

```console
$ docker pull debian@sha256:5d6d56fd575caa578370f3d737116dc1aa3940c0e26f368ca36db72efa576570
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448743 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de12428f6f470169eba22c18e53addae00d60e92ebe96b2c3bdd99c17c34010`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:34 GMT
ADD file:2fa5242038736e48eaca794d061079c1cbd32fcc4336250001523c41b3177276 in / 
# Tue, 04 Jul 2023 01:20:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8eb6dba554cffc072c7a6c696c8a23fc311e543399d84ab3ebc55c07ab54414f`  
		Last Modified: Tue, 04 Jul 2023 01:26:01 GMT  
		Size: 50.4 MB (50448743 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v7

```console
$ docker pull debian@sha256:d829b173cc834be382851eb0e3b6795fd6f3cbf1a47254409525d036de0dc0e7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916488 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8d7573ccdc3e47ecba5eb7d9b18f7dd0d68728e59e6510edd941a09ab38874c7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:31 GMT
ADD file:e1c7c10335e2ac86eba02c2785d0ff530cba87131e91c42924072b4010418f93 in / 
# Tue, 04 Jul 2023 00:58:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b19cf971d5ed51e0bba0bd71ccd7b797ad4873a62fee0f49588f6ef19a58e659`  
		Last Modified: Tue, 04 Jul 2023 01:03:55 GMT  
		Size: 45.9 MB (45916488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3b830c5d210077d297956550caedf93430d5779fd786b4ac8e22a6bfe54ff162
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238644 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9eabbca4bbf36f9c778ae052a9e6796cda58ca5b714378fb81d78703b6abdda2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:59 GMT
ADD file:a61276f534601d7b193af24e35c16b73a2913511d2b1ad997c9a8cb907685257 in / 
# Tue, 04 Jul 2023 01:58:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e2555ae6edde2e7933c533234cb224b6d7ef3a3c90041851e31fe29ab7197c9`  
		Last Modified: Tue, 04 Jul 2023 02:02:18 GMT  
		Size: 49.2 MB (49238644 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; 386

```console
$ docker pull debian@sha256:97fa01fafbac78c2f35b2a126d7716b7a5fe9528f69f712039ba46f3f213935b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206276 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5633184154445083a679086df75c54ab1501fd866fc05950198d5785332842b4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:11 GMT
ADD file:6ecab94f7cb2bc0740a669cfef73f0837a2ecf6475dfb0fc072cbd1a7f60ec41 in / 
# Tue, 04 Jul 2023 01:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9b3c3fd2dd1697ce9fed8a2be0142b2d1294d424c56d0ce6d44bb8055d77e35d`  
		Last Modified: Tue, 04 Jul 2023 01:44:33 GMT  
		Size: 51.2 MB (51206276 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20230703`

```console
$ docker pull debian@sha256:c21dbb23d41cb3f1c1a7f841e8642bf713934fb4dc5187979bd46f0b4b488616
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-20230703` - linux; amd64

```console
$ docker pull debian@sha256:5d6d56fd575caa578370f3d737116dc1aa3940c0e26f368ca36db72efa576570
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448743 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1de12428f6f470169eba22c18e53addae00d60e92ebe96b2c3bdd99c17c34010`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:34 GMT
ADD file:2fa5242038736e48eaca794d061079c1cbd32fcc4336250001523c41b3177276 in / 
# Tue, 04 Jul 2023 01:20:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8eb6dba554cffc072c7a6c696c8a23fc311e543399d84ab3ebc55c07ab54414f`  
		Last Modified: Tue, 04 Jul 2023 01:26:01 GMT  
		Size: 50.4 MB (50448743 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:d829b173cc834be382851eb0e3b6795fd6f3cbf1a47254409525d036de0dc0e7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916488 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8d7573ccdc3e47ecba5eb7d9b18f7dd0d68728e59e6510edd941a09ab38874c7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:31 GMT
ADD file:e1c7c10335e2ac86eba02c2785d0ff530cba87131e91c42924072b4010418f93 in / 
# Tue, 04 Jul 2023 00:58:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b19cf971d5ed51e0bba0bd71ccd7b797ad4873a62fee0f49588f6ef19a58e659`  
		Last Modified: Tue, 04 Jul 2023 01:03:55 GMT  
		Size: 45.9 MB (45916488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3b830c5d210077d297956550caedf93430d5779fd786b4ac8e22a6bfe54ff162
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238644 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9eabbca4bbf36f9c778ae052a9e6796cda58ca5b714378fb81d78703b6abdda2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:59 GMT
ADD file:a61276f534601d7b193af24e35c16b73a2913511d2b1ad997c9a8cb907685257 in / 
# Tue, 04 Jul 2023 01:58:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5e2555ae6edde2e7933c533234cb224b6d7ef3a3c90041851e31fe29ab7197c9`  
		Last Modified: Tue, 04 Jul 2023 02:02:18 GMT  
		Size: 49.2 MB (49238644 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703` - linux; 386

```console
$ docker pull debian@sha256:97fa01fafbac78c2f35b2a126d7716b7a5fe9528f69f712039ba46f3f213935b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206276 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5633184154445083a679086df75c54ab1501fd866fc05950198d5785332842b4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:11 GMT
ADD file:6ecab94f7cb2bc0740a669cfef73f0837a2ecf6475dfb0fc072cbd1a7f60ec41 in / 
# Tue, 04 Jul 2023 01:39:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9b3c3fd2dd1697ce9fed8a2be0142b2d1294d424c56d0ce6d44bb8055d77e35d`  
		Last Modified: Tue, 04 Jul 2023 01:44:33 GMT  
		Size: 51.2 MB (51206276 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20230703-slim`

```console
$ docker pull debian@sha256:cddb688e1263b9752275b064171ef6ac9c70ae21a77c774339aecfb53690b9a1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:bb8aa91b818f9c9d79620db405cdb1834ba9c611d7771ac0df7b1c897c237405
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138502 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:440af094c94c50da90f9758cb86f9c444b533ae1aaa31ae901a90ce3fd502e25`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:47 GMT
ADD file:ca4076bfffab8d09636384070ca253570590554cff76a132afaae5cd89b363b5 in / 
# Tue, 04 Jul 2023 01:20:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90ac1ecaf92c8ae0cb37d29d8cc01b5802612c12edb933c369ad4c586ea94c6c`  
		Last Modified: Tue, 04 Jul 2023 01:26:21 GMT  
		Size: 27.1 MB (27138502 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:65e1480312045435e6bcbe45d6089796a6eed24a3e3dfb690f9de0110adc8180
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747297 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98450657978157841dbe66088f7eccf3e77203a44b6ca7e0e840ecd66a2bcb92`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:42 GMT
ADD file:d2008875df919cb9817e6af97438428c2632113c1ef2b67b96607e184a7af941 in / 
# Tue, 04 Jul 2023 00:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:966944a300a7cf6431aa9fc3454d07eb7764f6b99d43a67de45ba53c30d69010`  
		Last Modified: Tue, 04 Jul 2023 01:04:17 GMT  
		Size: 22.7 MB (22747297 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7fa521a8417b06243b8834cfac50b6e5878cb1a2ac93b30ddf697d03726745cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921455 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d9f0adbe79ff2fdd2578be296271df31982f034355e67d65d114a7db9c05be1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:08 GMT
ADD file:1368964550b1f674dd0365f72bb1d55cfebf5e3c651d49b5e168f9be97a6df76 in / 
# Tue, 04 Jul 2023 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:30bfbe1cefebb80f6f9d5d90c39fcdf9d6367bbc76fa68e33ea3b4391c0f9a77`  
		Last Modified: Tue, 04 Jul 2023 02:02:39 GMT  
		Size: 25.9 MB (25921455 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:406434a9edee56a3103e89abc4053a3a5b082d0371565f7d4293053bde12ae6e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f24c54dc8c17e5705314df120aeeb1220570ab862b480bf0334811d973bdeb99`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:22 GMT
ADD file:0be890be1c2ff0966cf1b714bd814e6859e1a2cb4a491ebd6c28d551eeedd66b in / 
# Tue, 04 Jul 2023 01:39:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4637b85bb10ca071ae2bfe4ea2e6dd6ca216cecae3d9db6787e67046e0c1f70`  
		Last Modified: Tue, 04 Jul 2023 01:44:56 GMT  
		Size: 27.8 MB (27797000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-backports`

```console
$ docker pull debian@sha256:a7a1de1861ff9acfe976a4cda09ff40c4b6d9aa2349f1a2c348dd6261f35018f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-backports` - linux; amd64

```console
$ docker pull debian@sha256:5cee24386e991d71612ab6603b2aff208c27307120244f056144a829e75e1c95
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448964 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:772a8bf029bb6c561bca9f7fb1b0946ea1bcd183340c2045e1dd1c91c02a5643`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:34 GMT
ADD file:2fa5242038736e48eaca794d061079c1cbd32fcc4336250001523c41b3177276 in / 
# Tue, 04 Jul 2023 01:20:35 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:20:39 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8eb6dba554cffc072c7a6c696c8a23fc311e543399d84ab3ebc55c07ab54414f`  
		Last Modified: Tue, 04 Jul 2023 01:26:01 GMT  
		Size: 50.4 MB (50448743 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:73104bcafd56465a15bf1dc7a6ee33d7f91692b9f20f0dd8a535d1d85d480d37`  
		Last Modified: Tue, 04 Jul 2023 01:26:13 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:5b5a775a224016909354d484cdfd5367605b51e79bf2a136efd22916ab868ee6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916707 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f2fb2b4ea95237b03a7b71ba22054a5d39756dc5eb23ce1ac5b5a9f75763047c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:31 GMT
ADD file:e1c7c10335e2ac86eba02c2785d0ff530cba87131e91c42924072b4010418f93 in / 
# Tue, 04 Jul 2023 00:58:32 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:58:34 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b19cf971d5ed51e0bba0bd71ccd7b797ad4873a62fee0f49588f6ef19a58e659`  
		Last Modified: Tue, 04 Jul 2023 01:03:55 GMT  
		Size: 45.9 MB (45916488 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b4d7a9a6f5f8af9bc7b70d526883758c669829e4729676cb679627d37198c4a8`  
		Last Modified: Tue, 04 Jul 2023 01:04:09 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7a0ecca18790e4da06afa198c5205e35d0468e2584b9aeb7225ccb295a1f25be
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d5838c8e457b798f2dfafa204263242d5c9f5a32456b0a0e16ef1aebd2fc9eac`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:59 GMT
ADD file:a61276f534601d7b193af24e35c16b73a2913511d2b1ad997c9a8cb907685257 in / 
# Tue, 04 Jul 2023 01:58:00 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:58:03 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:5e2555ae6edde2e7933c533234cb224b6d7ef3a3c90041851e31fe29ab7197c9`  
		Last Modified: Tue, 04 Jul 2023 02:02:18 GMT  
		Size: 49.2 MB (49238644 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:55bd34674a07551d61f9ca8bbd42e5eddcf7a519a3b1e5f8c9bc38c0cdc6192b`  
		Last Modified: Tue, 04 Jul 2023 02:02:31 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; 386

```console
$ docker pull debian@sha256:535e1335439a5db5cf391762a69bf2b785df7a464db005bf9a165849ddfdbc07
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206500 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d96aeea16dcce4187c28b7ca7c004bb761002bafe12ae4baf50bde091640f2d9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:11 GMT
ADD file:6ecab94f7cb2bc0740a669cfef73f0837a2ecf6475dfb0fc072cbd1a7f60ec41 in / 
# Tue, 04 Jul 2023 01:39:11 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:39:14 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:9b3c3fd2dd1697ce9fed8a2be0142b2d1294d424c56d0ce6d44bb8055d77e35d`  
		Last Modified: Tue, 04 Jul 2023 01:44:33 GMT  
		Size: 51.2 MB (51206276 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5ddf80dc1aef027de6ef3d70f92e50550c771984d2208d58d8135bf73c589736`  
		Last Modified: Tue, 04 Jul 2023 01:44:45 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-slim`

```console
$ docker pull debian@sha256:cddb688e1263b9752275b064171ef6ac9c70ae21a77c774339aecfb53690b9a1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:buster-slim` - linux; amd64

```console
$ docker pull debian@sha256:bb8aa91b818f9c9d79620db405cdb1834ba9c611d7771ac0df7b1c897c237405
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138502 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:440af094c94c50da90f9758cb86f9c444b533ae1aaa31ae901a90ce3fd502e25`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:47 GMT
ADD file:ca4076bfffab8d09636384070ca253570590554cff76a132afaae5cd89b363b5 in / 
# Tue, 04 Jul 2023 01:20:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:90ac1ecaf92c8ae0cb37d29d8cc01b5802612c12edb933c369ad4c586ea94c6c`  
		Last Modified: Tue, 04 Jul 2023 01:26:21 GMT  
		Size: 27.1 MB (27138502 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:65e1480312045435e6bcbe45d6089796a6eed24a3e3dfb690f9de0110adc8180
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747297 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98450657978157841dbe66088f7eccf3e77203a44b6ca7e0e840ecd66a2bcb92`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:42 GMT
ADD file:d2008875df919cb9817e6af97438428c2632113c1ef2b67b96607e184a7af941 in / 
# Tue, 04 Jul 2023 00:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:966944a300a7cf6431aa9fc3454d07eb7764f6b99d43a67de45ba53c30d69010`  
		Last Modified: Tue, 04 Jul 2023 01:04:17 GMT  
		Size: 22.7 MB (22747297 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7fa521a8417b06243b8834cfac50b6e5878cb1a2ac93b30ddf697d03726745cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921455 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d9f0adbe79ff2fdd2578be296271df31982f034355e67d65d114a7db9c05be1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:08 GMT
ADD file:1368964550b1f674dd0365f72bb1d55cfebf5e3c651d49b5e168f9be97a6df76 in / 
# Tue, 04 Jul 2023 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:30bfbe1cefebb80f6f9d5d90c39fcdf9d6367bbc76fa68e33ea3b4391c0f9a77`  
		Last Modified: Tue, 04 Jul 2023 02:02:39 GMT  
		Size: 25.9 MB (25921455 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; 386

```console
$ docker pull debian@sha256:406434a9edee56a3103e89abc4053a3a5b082d0371565f7d4293053bde12ae6e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f24c54dc8c17e5705314df120aeeb1220570ab862b480bf0334811d973bdeb99`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:22 GMT
ADD file:0be890be1c2ff0966cf1b714bd814e6859e1a2cb4a491ebd6c28d551eeedd66b in / 
# Tue, 04 Jul 2023 01:39:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4637b85bb10ca071ae2bfe4ea2e6dd6ca216cecae3d9db6787e67046e0c1f70`  
		Last Modified: Tue, 04 Jul 2023 01:44:56 GMT  
		Size: 27.8 MB (27797000 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental`

```console
$ docker pull debian@sha256:c4634ddf272060ed2099cebc69e15f8bade63c3f890056cdd892ac916b103e80
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental` - linux; amd64

```console
$ docker pull debian@sha256:ee4b8c2d0a9942a34081ab933f668376bf35d3525a20cd06eed4afb0e403ac43
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49476170 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f61ad82957c77ec101ad1f1f2dd1ddcd8ee8e2b3208954218bcf86f17ff14bb3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:27 GMT
ADD file:30c3621d27a742c3c20865fd70d8aa41d70be0f58eeeb81aaa9dd1298ecb1c1d in / 
# Tue, 04 Jul 2023 01:23:27 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:23:41 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:52ab46b1411e2672033a4f02a5e7df836c372c7d9366d0f10b15751b45a6a217`  
		Last Modified: Tue, 04 Jul 2023 01:29:49 GMT  
		Size: 49.5 MB (49475947 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f0bcd11045bd376dd6392c60ec34a96bc530004eb87177deb7ed1f0fca348007`  
		Last Modified: Tue, 04 Jul 2023 01:30:10 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v5

```console
$ docker pull debian@sha256:8c0a40eb00289d699534f5b060d1c9e9bb61b508c6d5fc662606ba1f250b07c1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322694 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:883bff7d7307e93e117c0640650f6717d50ba33c539fecd83d3e1638c5d7f5a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:15 GMT
ADD file:0d73a7183a38c56858e61e53f0c64ae6eaf9d58311df1be7af3b37f1e1769dd2 in / 
# Tue, 04 Jul 2023 00:50:16 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:50:26 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:eb46e2b513bf023b4297bc46404f96d3a604f8f913dffe3d1f9e5c5c530e9caf`  
		Last Modified: Tue, 04 Jul 2023 00:55:44 GMT  
		Size: 47.3 MB (47322476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b079d533182e62457871a1cdecd8b7307dda1c497fda642b534f1738b904b1aa`  
		Last Modified: Tue, 04 Jul 2023 00:56:07 GMT  
		Size: 218.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e140ce88bd505eebe36c6aebac98f5303b50a067126409f0fc4831ad06b5bc2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45124201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e332aa22a3f6fe878a99005e2a42ca5ff1c89e17e08b81d9010ed364042cab74`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:20 GMT
ADD file:a7c3220804fb0e3fd658c0492b5338ba9fc6c132f28282486bbae9622138fd0e in / 
# Tue, 04 Jul 2023 01:01:21 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:01:31 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:0807315011437359206e59c75af82e26e89ba45ae9281fa33df5e8dda8de17a2`  
		Last Modified: Tue, 04 Jul 2023 01:07:56 GMT  
		Size: 45.1 MB (45123977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e80c69a5003d0ef8f39c11e33430cc1570553f8fa01a641e6fb1694acb22e771`  
		Last Modified: Tue, 04 Jul 2023 01:08:17 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:abe37a32bdce7748581b71305d7931bce1e4abd16e6375cf3c78f7723245dcfb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482796 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03b87baf1354ffa1f14b4908bccf3c4a3860a493a5aea93544a889e0b72bcd05`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:53 GMT
ADD file:bf1bc62117abf0b13f8992acfed17c5505f88edece9bdb42c246d3801869a909 in / 
# Tue, 04 Jul 2023 01:59:53 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 02:00:03 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d63178d4b98b5ec11651f31157085c27b04daa9271f1b4a8be773b8ad11c4264`  
		Last Modified: Tue, 04 Jul 2023 02:05:57 GMT  
		Size: 49.5 MB (49482573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ab8d026073039c25e9ee2a17496721f4ceac9ef9e94a28fd58dfa36d329874a2`  
		Last Modified: Tue, 04 Jul 2023 02:06:17 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; 386

```console
$ docker pull debian@sha256:f040efe89db35f2e92448e942f4ab4233ef1a49a962416c604d5b930e8db2798
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50506050 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b0471ac8119bae30367e8fd4578fc459e80d2028f8d586be4e41431b3e416d96`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:50 GMT
ADD file:2f42b2e397b1e8bc6959dbfa501dddc91fd18f0d3ef0fd9ea0663092286e1429 in / 
# Tue, 04 Jul 2023 01:41:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:42:03 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d6a8ba1eaf3690a5e9b7f2debed87f9cfac7972349e43494cf2b7feffb143629`  
		Last Modified: Tue, 04 Jul 2023 01:48:54 GMT  
		Size: 50.5 MB (50505827 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b1d36e6b4e6c369b3f3ab10d1e9f74b10daac252de8e79569017d2dc1415f745`  
		Last Modified: Tue, 04 Jul 2023 01:49:15 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; mips64le

```console
$ docker pull debian@sha256:75e9871043a230eb22857b46f45c5191a6701ee9ee823e0ba9c973bd75a6a18c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455622 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eeee7aeea20c741a739b5b6e2a1237c759a78b48ae5be20b7dbdafa85ec0d129`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:08 GMT
ADD file:20580fa5e83e1873d44c46cb2f9ac1fe39a42c62c9d4e3756723283f13add821 in / 
# Tue, 04 Jul 2023 01:17:14 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:17:54 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:78d3f505f592dc293a56dbf6460a5f68fd63cf76766388835e32dcbc804fc180`  
		Last Modified: Tue, 04 Jul 2023 01:27:58 GMT  
		Size: 49.5 MB (49455401 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:00b5720ed9039a06c54aba388df180e92b4c556fb4ba1c2f5f088a9a43bb3ce1`  
		Last Modified: Tue, 04 Jul 2023 01:28:36 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; ppc64le

```console
$ docker pull debian@sha256:70d80edab92dc523ddfd8c6b98a75b5e774696097575f336ae3d38bcef02c508
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474637 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4711487b2be5f72a8d3521f4b1c7e786104d126cb169b7004b4accd52543711e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:15 GMT
ADD file:d5d2b24ead06183959c90a6751c80a887c556ca4f0fdc7d287812576eafb1a60 in / 
# Tue, 04 Jul 2023 01:22:18 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:39 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:af5bbdab36f44ac3e6bea7dc4ed1c8b357c4862d3c25a3ec040d72e425ef159c`  
		Last Modified: Tue, 04 Jul 2023 01:30:28 GMT  
		Size: 53.5 MB (53474416 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:11d118ef2a0ad933e33a89e18d137c4a8285f20f4c4e78b17f17003ab68b62e6`  
		Last Modified: Tue, 04 Jul 2023 01:31:00 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; riscv64

```console
$ docker pull debian@sha256:fd8164a7bd9565294d8f433d478adcc1f7371e0ad3431c78beb30d80bbaf2aae
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695712 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3db54503fa0f89e6f3eee133d4dec5dc6027fdaf9f340fdb7affc4e93354b751`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:32 GMT
ADD file:bacf5c7bb4f9db293679962e5936b1037a72a2e667da59236cda267c866ad8e2 in / 
# Tue, 04 Jul 2023 01:10:34 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:11:21 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:892c64510625aa4b95222221c8d62ebbab44da937d39c14956f6e6697a5c4c80`  
		Last Modified: Tue, 04 Jul 2023 01:13:59 GMT  
		Size: 45.7 MB (45695485 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:58595682130886a8914b61a3a37dc1da553893a9cf3815062133cff2dfff8ee6`  
		Last Modified: Tue, 04 Jul 2023 01:14:45 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; s390x

```console
$ docker pull debian@sha256:aeb5c42953c63cbc5db2e38bc861b5d763d6876b3ef94393d7d16063d22239bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890989 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bda98194a8a1698694a72384adfbe1278c95e996163b69b2fbe80c3969745370`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:23 GMT
ADD file:aac2397e3a90e63ce8e13930d00bcbee000d5f021a613a196e495f1c36aeeef1 in / 
# Tue, 04 Jul 2023 01:35:26 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:35:42 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:4385878fa0020ea8459bcfa951b29983c0576d085fba63f0b00e83da8014aa50`  
		Last Modified: Tue, 04 Jul 2023 01:39:52 GMT  
		Size: 47.9 MB (47890770 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:897be0a75f527f567bcbd09694a659cae32e79ab2be66f4dcc6110a9c3d665f9`  
		Last Modified: Tue, 04 Jul 2023 01:40:08 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental-20230703`

```console
$ docker pull debian@sha256:c4634ddf272060ed2099cebc69e15f8bade63c3f890056cdd892ac916b103e80
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental-20230703` - linux; amd64

```console
$ docker pull debian@sha256:ee4b8c2d0a9942a34081ab933f668376bf35d3525a20cd06eed4afb0e403ac43
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49476170 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f61ad82957c77ec101ad1f1f2dd1ddcd8ee8e2b3208954218bcf86f17ff14bb3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:27 GMT
ADD file:30c3621d27a742c3c20865fd70d8aa41d70be0f58eeeb81aaa9dd1298ecb1c1d in / 
# Tue, 04 Jul 2023 01:23:27 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:23:41 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:52ab46b1411e2672033a4f02a5e7df836c372c7d9366d0f10b15751b45a6a217`  
		Last Modified: Tue, 04 Jul 2023 01:29:49 GMT  
		Size: 49.5 MB (49475947 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f0bcd11045bd376dd6392c60ec34a96bc530004eb87177deb7ed1f0fca348007`  
		Last Modified: Tue, 04 Jul 2023 01:30:10 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:8c0a40eb00289d699534f5b060d1c9e9bb61b508c6d5fc662606ba1f250b07c1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322694 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:883bff7d7307e93e117c0640650f6717d50ba33c539fecd83d3e1638c5d7f5a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:15 GMT
ADD file:0d73a7183a38c56858e61e53f0c64ae6eaf9d58311df1be7af3b37f1e1769dd2 in / 
# Tue, 04 Jul 2023 00:50:16 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:50:26 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:eb46e2b513bf023b4297bc46404f96d3a604f8f913dffe3d1f9e5c5c530e9caf`  
		Last Modified: Tue, 04 Jul 2023 00:55:44 GMT  
		Size: 47.3 MB (47322476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b079d533182e62457871a1cdecd8b7307dda1c497fda642b534f1738b904b1aa`  
		Last Modified: Tue, 04 Jul 2023 00:56:07 GMT  
		Size: 218.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:8e140ce88bd505eebe36c6aebac98f5303b50a067126409f0fc4831ad06b5bc2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45124201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e332aa22a3f6fe878a99005e2a42ca5ff1c89e17e08b81d9010ed364042cab74`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:20 GMT
ADD file:a7c3220804fb0e3fd658c0492b5338ba9fc6c132f28282486bbae9622138fd0e in / 
# Tue, 04 Jul 2023 01:01:21 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:01:31 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:0807315011437359206e59c75af82e26e89ba45ae9281fa33df5e8dda8de17a2`  
		Last Modified: Tue, 04 Jul 2023 01:07:56 GMT  
		Size: 45.1 MB (45123977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e80c69a5003d0ef8f39c11e33430cc1570553f8fa01a641e6fb1694acb22e771`  
		Last Modified: Tue, 04 Jul 2023 01:08:17 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:abe37a32bdce7748581b71305d7931bce1e4abd16e6375cf3c78f7723245dcfb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482796 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03b87baf1354ffa1f14b4908bccf3c4a3860a493a5aea93544a889e0b72bcd05`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:53 GMT
ADD file:bf1bc62117abf0b13f8992acfed17c5505f88edece9bdb42c246d3801869a909 in / 
# Tue, 04 Jul 2023 01:59:53 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 02:00:03 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d63178d4b98b5ec11651f31157085c27b04daa9271f1b4a8be773b8ad11c4264`  
		Last Modified: Tue, 04 Jul 2023 02:05:57 GMT  
		Size: 49.5 MB (49482573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ab8d026073039c25e9ee2a17496721f4ceac9ef9e94a28fd58dfa36d329874a2`  
		Last Modified: Tue, 04 Jul 2023 02:06:17 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; 386

```console
$ docker pull debian@sha256:f040efe89db35f2e92448e942f4ab4233ef1a49a962416c604d5b930e8db2798
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50506050 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b0471ac8119bae30367e8fd4578fc459e80d2028f8d586be4e41431b3e416d96`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:50 GMT
ADD file:2f42b2e397b1e8bc6959dbfa501dddc91fd18f0d3ef0fd9ea0663092286e1429 in / 
# Tue, 04 Jul 2023 01:41:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:42:03 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d6a8ba1eaf3690a5e9b7f2debed87f9cfac7972349e43494cf2b7feffb143629`  
		Last Modified: Tue, 04 Jul 2023 01:48:54 GMT  
		Size: 50.5 MB (50505827 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b1d36e6b4e6c369b3f3ab10d1e9f74b10daac252de8e79569017d2dc1415f745`  
		Last Modified: Tue, 04 Jul 2023 01:49:15 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:75e9871043a230eb22857b46f45c5191a6701ee9ee823e0ba9c973bd75a6a18c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455622 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eeee7aeea20c741a739b5b6e2a1237c759a78b48ae5be20b7dbdafa85ec0d129`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:08 GMT
ADD file:20580fa5e83e1873d44c46cb2f9ac1fe39a42c62c9d4e3756723283f13add821 in / 
# Tue, 04 Jul 2023 01:17:14 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:17:54 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:78d3f505f592dc293a56dbf6460a5f68fd63cf76766388835e32dcbc804fc180`  
		Last Modified: Tue, 04 Jul 2023 01:27:58 GMT  
		Size: 49.5 MB (49455401 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:00b5720ed9039a06c54aba388df180e92b4c556fb4ba1c2f5f088a9a43bb3ce1`  
		Last Modified: Tue, 04 Jul 2023 01:28:36 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:70d80edab92dc523ddfd8c6b98a75b5e774696097575f336ae3d38bcef02c508
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474637 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4711487b2be5f72a8d3521f4b1c7e786104d126cb169b7004b4accd52543711e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:15 GMT
ADD file:d5d2b24ead06183959c90a6751c80a887c556ca4f0fdc7d287812576eafb1a60 in / 
# Tue, 04 Jul 2023 01:22:18 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:39 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:af5bbdab36f44ac3e6bea7dc4ed1c8b357c4862d3c25a3ec040d72e425ef159c`  
		Last Modified: Tue, 04 Jul 2023 01:30:28 GMT  
		Size: 53.5 MB (53474416 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:11d118ef2a0ad933e33a89e18d137c4a8285f20f4c4e78b17f17003ab68b62e6`  
		Last Modified: Tue, 04 Jul 2023 01:31:00 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; riscv64

```console
$ docker pull debian@sha256:fd8164a7bd9565294d8f433d478adcc1f7371e0ad3431c78beb30d80bbaf2aae
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695712 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3db54503fa0f89e6f3eee133d4dec5dc6027fdaf9f340fdb7affc4e93354b751`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:32 GMT
ADD file:bacf5c7bb4f9db293679962e5936b1037a72a2e667da59236cda267c866ad8e2 in / 
# Tue, 04 Jul 2023 01:10:34 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:11:21 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:892c64510625aa4b95222221c8d62ebbab44da937d39c14956f6e6697a5c4c80`  
		Last Modified: Tue, 04 Jul 2023 01:13:59 GMT  
		Size: 45.7 MB (45695485 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:58595682130886a8914b61a3a37dc1da553893a9cf3815062133cff2dfff8ee6`  
		Last Modified: Tue, 04 Jul 2023 01:14:45 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20230703` - linux; s390x

```console
$ docker pull debian@sha256:aeb5c42953c63cbc5db2e38bc861b5d763d6876b3ef94393d7d16063d22239bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890989 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bda98194a8a1698694a72384adfbe1278c95e996163b69b2fbe80c3969745370`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:23 GMT
ADD file:aac2397e3a90e63ce8e13930d00bcbee000d5f021a613a196e495f1c36aeeef1 in / 
# Tue, 04 Jul 2023 01:35:26 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:35:42 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:4385878fa0020ea8459bcfa951b29983c0576d085fba63f0b00e83da8014aa50`  
		Last Modified: Tue, 04 Jul 2023 01:39:52 GMT  
		Size: 47.9 MB (47890770 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:897be0a75f527f567bcbd09694a659cae32e79ab2be66f4dcc6110a9c3d665f9`  
		Last Modified: Tue, 04 Jul 2023 01:40:08 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:latest`

```console
$ docker pull debian@sha256:3d868b5eb908155f3784317b3dda2941df87bbbbaa4608f84881de66d9bb297b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:latest` - linux; amd64

```console
$ docker pull debian@sha256:60774985572749dc3c39147d43089d53e7ce17b844eebcf619d84467160217ab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551432 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3676c78a12ad77e170916695d2f192797ddd3eb653d7a1b1ad4c20ab5471891a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:44 GMT
ADD file:5d17ca085473e890bd6eca4abf6d324c3181f80692523b83ef25d1c42576b99f in / 
# Tue, 04 Jul 2023 01:19:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d52e4f012db158bb7c0fe215b98af1facaddcbaee530efd69b1bae07d597b711`  
		Last Modified: Tue, 04 Jul 2023 01:24:39 GMT  
		Size: 49.6 MB (49551432 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v5

```console
$ docker pull debian@sha256:e1eedd931159f74699128b11375ab15a76fed96ea29dedce88588cfaa19b564a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402786 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e836de3c00fa0d0942c5e3ea21d9e0525c92d38c0e8a1dc39a659ec87689a34d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:22 GMT
ADD file:09001854dc5081f907ac2a66d301e4b3d6b4bf2c2483aad5395a535bf068698d in / 
# Tue, 04 Jul 2023 00:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e0a1f476b42bcc214197ec9c5ef753ddf2ab36a314416a6ac96cd921ff0d41d`  
		Last Modified: Tue, 04 Jul 2023 00:51:29 GMT  
		Size: 47.4 MB (47402786 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v7

```console
$ docker pull debian@sha256:722595c25d744c903165d26d37fdd2f2ead8d9ceb326deb260173f48725be078
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca910bfe472fc0e9b55301d7ad047098587c598dfd5ac44c010e202d0f7d97bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:57:42 GMT
ADD file:279ef7213d41f1f7ae76bd76293a5107fd1f8caf557f837cab9c834d2841e031 in / 
# Tue, 04 Jul 2023 00:57:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9bdcf1a9a6361f231594c76a75bfb4c2fef5f9745b9713bbd50efaf37ca11b8`  
		Last Modified: Tue, 04 Jul 2023 01:02:26 GMT  
		Size: 45.2 MB (45236201 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:345b9265e89c123f994b724b9a4761dbc91c5759c144d01292ca56a338170801
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572781 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:886d61a022482574344910f88e436ce64e9218cc8d62bea0cac035c4519ff93e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:57:26 GMT
ADD file:ab1761d899751c4d24e15179fc9e7e7ac3fb83798ee37ca13d6a6ac44fbeded3 in / 
# Tue, 04 Jul 2023 01:57:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cbebf8bc116ba1aed7897e2d0566bf49da9d5c70be71b6a7cb07805d2f5b7a`  
		Last Modified: Tue, 04 Jul 2023 02:00:57 GMT  
		Size: 49.6 MB (49572781 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; 386

```console
$ docker pull debian@sha256:699f29298cb47296a0b0be2d6a1d18b0ab5ca77986862949a665e4524bc0b781
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9743376b1f2144d61495397e2a7f044bbc25d61016cee4dbff8683d31eaed7fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:38:25 GMT
ADD file:e0a25fe261e09183ec383698ab2c800f93c5b0805564d7104ad5119db52cccb7 in / 
# Tue, 04 Jul 2023 01:38:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:381263dab43af8bfb8e85c7a9a32f35492cbdcc3a046b35f947cf3bf3a55f4c1`  
		Last Modified: Tue, 04 Jul 2023 01:43:02 GMT  
		Size: 50.6 MB (50562388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; mips64le

```console
$ docker pull debian@sha256:67103735074efb1301d79591c39140d7362fecc0898dda01a6ab7fcef1500a2f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5f8b2d159b8a89e52d66238252e674cf2d57e11c558c82ab3c4eac593ac893ef`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:08:42 GMT
ADD file:c567fc0f97f36ea531c05e159cd6ac2a8747f14ea5b6ccae39bab9d305da180d in / 
# Tue, 04 Jul 2023 01:08:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76240e493592cbc7ca56583a6e8e03ab12a967b65bbeb1df7828a5099fa271d9`  
		Last Modified: Tue, 04 Jul 2023 01:19:16 GMT  
		Size: 49.5 MB (49542143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c051e548fc05b1fa5d532ceffddb1ff227aa57b81b4fcc91cbf627ebb6f0a3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536740 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ec2232f2e4f8c6c1f0fb3f88f17e3c7c6e3ef4040ebbaa9588f7d2e9de7a07bd`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:23 GMT
ADD file:5aaa642008df5a97014e76042a767f06bbfda91db1d5f336a1b3f98e9b11c7a1 in / 
# Tue, 04 Jul 2023 01:17:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e039cf6634cf83a31e97e2a56b4db40a8c38a62d6857c77e687d5edb3eb0dfde`  
		Last Modified: Tue, 04 Jul 2023 01:23:59 GMT  
		Size: 53.5 MB (53536740 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; s390x

```console
$ docker pull debian@sha256:aaa4fc6ed87ddc782bba21f6dad6bdeeb0a857c7d5530e19f5d8b5134b8bc9f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921709 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9a90a19fe78ac6ff9bedf17db184107da34ef5c182a64c226910e165d1c11e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:31:54 GMT
ADD file:0ad7ffbe777e5367ad6dcbb4a182e1d578739aa7a4853f82f21bd71d824de7ae in / 
# Tue, 04 Jul 2023 01:31:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce9d001c0d31ec9b8072b1a9bd01771a10ea8ddfefa381cf9ce7e2d63538aaeb`  
		Last Modified: Tue, 04 Jul 2023 01:36:50 GMT  
		Size: 47.9 MB (47921709 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable`

```console
$ docker pull debian@sha256:f72844939cb37f4527d59a678f1467cc4a668294934ea57e4da0bb6178a7fcb4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable` - linux; amd64

```console
$ docker pull debian@sha256:efeb7c0c55bbd210ed2e65b6d72124171fbb4c4c7db057d2061747e16cfb3feb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448681 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b597867018c0129281dc89d89c636610f55608898bc13bdd7af7ce92366eef6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:59 GMT
ADD file:3b029f3178b5e1b4334c6406617e14df5bf0ce15ce935acc2cb2dc51b6f151aa in / 
# Tue, 04 Jul 2023 01:20:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6b44c60b4ef8f05f0a7be874cdf80559a40b3584e9146bcef80caf7c3bd47f55`  
		Last Modified: Tue, 04 Jul 2023 01:26:40 GMT  
		Size: 50.4 MB (50448681 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:1bc5899542c257defbedab8c7f732681be44a3f111918b36845a1ff54ffdb6f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916479 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8f4d51dbb67d338f2e4a06b32d7998d73ec8deebf3054036e7270cc850cdfaa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:51 GMT
ADD file:fb0d0efa496216eb4150798e9acf6220f344ec01a00121d2ac5eb8efb61344c5 in / 
# Tue, 04 Jul 2023 00:58:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:003d12612fcea91fa985efa292e97219fbb80e0975883488634bf9f8193e765b`  
		Last Modified: Tue, 04 Jul 2023 01:04:36 GMT  
		Size: 45.9 MB (45916479 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c8bf3bfbba53cb0391f41095488ba744447aca63653afdff8a06dd27ca17b045
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aa30c85951c796babe48c04d0c943725a0d7db90b5ed5bac81028453903eed12`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:16 GMT
ADD file:fc5434a757b40bf7ac5f064c4580674f3f844b4302b8bfb607cbc595c92c9efa in / 
# Tue, 04 Jul 2023 01:58:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:551faad51851c8a8fe8e9faacc89d609b82713385126737d5f6a56acb16ddac0`  
		Last Modified: Tue, 04 Jul 2023 02:02:57 GMT  
		Size: 49.2 MB (49238607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; 386

```console
$ docker pull debian@sha256:18f347b5cca54238bb99b6d20e8b32c3fc5c5197f4780029c4fdc911f3eab247
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206313 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59988bd9372429b8faa54477a3cc3d70581d4e08d146fc0c3a5e92a69a4c909b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:33 GMT
ADD file:a125727a859efc0078fd6104c2d5add32d60bc44caafb60b95835ebdca850b6c in / 
# Tue, 04 Jul 2023 01:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ea65c1e207f0f923cefee5466d92585f5e03f36a470e46a88d80c4bb768818a4`  
		Last Modified: Tue, 04 Jul 2023 01:45:18 GMT  
		Size: 51.2 MB (51206313 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20230703`

```console
$ docker pull debian@sha256:f72844939cb37f4527d59a678f1467cc4a668294934ea57e4da0bb6178a7fcb4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20230703` - linux; amd64

```console
$ docker pull debian@sha256:efeb7c0c55bbd210ed2e65b6d72124171fbb4c4c7db057d2061747e16cfb3feb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448681 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b597867018c0129281dc89d89c636610f55608898bc13bdd7af7ce92366eef6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:59 GMT
ADD file:3b029f3178b5e1b4334c6406617e14df5bf0ce15ce935acc2cb2dc51b6f151aa in / 
# Tue, 04 Jul 2023 01:20:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6b44c60b4ef8f05f0a7be874cdf80559a40b3584e9146bcef80caf7c3bd47f55`  
		Last Modified: Tue, 04 Jul 2023 01:26:40 GMT  
		Size: 50.4 MB (50448681 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:1bc5899542c257defbedab8c7f732681be44a3f111918b36845a1ff54ffdb6f9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916479 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8f4d51dbb67d338f2e4a06b32d7998d73ec8deebf3054036e7270cc850cdfaa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:51 GMT
ADD file:fb0d0efa496216eb4150798e9acf6220f344ec01a00121d2ac5eb8efb61344c5 in / 
# Tue, 04 Jul 2023 00:58:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:003d12612fcea91fa985efa292e97219fbb80e0975883488634bf9f8193e765b`  
		Last Modified: Tue, 04 Jul 2023 01:04:36 GMT  
		Size: 45.9 MB (45916479 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c8bf3bfbba53cb0391f41095488ba744447aca63653afdff8a06dd27ca17b045
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238607 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aa30c85951c796babe48c04d0c943725a0d7db90b5ed5bac81028453903eed12`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:16 GMT
ADD file:fc5434a757b40bf7ac5f064c4580674f3f844b4302b8bfb607cbc595c92c9efa in / 
# Tue, 04 Jul 2023 01:58:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:551faad51851c8a8fe8e9faacc89d609b82713385126737d5f6a56acb16ddac0`  
		Last Modified: Tue, 04 Jul 2023 02:02:57 GMT  
		Size: 49.2 MB (49238607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703` - linux; 386

```console
$ docker pull debian@sha256:18f347b5cca54238bb99b6d20e8b32c3fc5c5197f4780029c4fdc911f3eab247
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206313 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59988bd9372429b8faa54477a3cc3d70581d4e08d146fc0c3a5e92a69a4c909b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:33 GMT
ADD file:a125727a859efc0078fd6104c2d5add32d60bc44caafb60b95835ebdca850b6c in / 
# Tue, 04 Jul 2023 01:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ea65c1e207f0f923cefee5466d92585f5e03f36a470e46a88d80c4bb768818a4`  
		Last Modified: Tue, 04 Jul 2023 01:45:18 GMT  
		Size: 51.2 MB (51206313 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20230703-slim`

```console
$ docker pull debian@sha256:1bc6ca2f31cdee6617795f4c41b469afae16270923e6b04da408613a964c7c8c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:821883bc241809dfdcd52fab6074e448e13866c837e77c0f8fc98d119141e841
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138557 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65ef81e58a24453417d9f0c89d44cb914d5db6c9baf68a82cfab7bbd0c035745`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:13 GMT
ADD file:f8c861794bc9356c93afccdade217410d2de0f1a9f5e739c9e965db11390daae in / 
# Tue, 04 Jul 2023 01:21:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a2d0007802c28f407af424fa43375a522555482f2e668b718adce767050ff52`  
		Last Modified: Tue, 04 Jul 2023 01:26:58 GMT  
		Size: 27.1 MB (27138557 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ce5120c140ac4f83d8e4fdd0337264e10a2808b77c1f1d19acfdecf26622ac88
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747315 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef006db4e1c62a38032e83439a60f1fc665e625ccde7f67d13e17d9a54f92253`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:05 GMT
ADD file:fabf8cc707e11cc2f478a8f281b7ddbddfad58ae12f4e8d321c3a67b13fc1103 in / 
# Tue, 04 Jul 2023 00:59:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eed08a67e3ceb30217d9b34c7dc81a4c3612270511fd3b00780455ea50ed1972`  
		Last Modified: Tue, 04 Jul 2023 01:04:54 GMT  
		Size: 22.7 MB (22747315 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ba616b324dd575209493134e8a29ec21a0eb81d30bcfeb892d27ef7532b729d4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921452 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:acb1d62777c727a9575b653d796b2063b6dc8a5cf897118787d64bdee10c5d55`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:25 GMT
ADD file:87a727c06fc42bff51baa77772f9272277e630b63377431df19144fb8b938543 in / 
# Tue, 04 Jul 2023 01:58:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:27a4a304213cd35e9c77c365966eb4ba4fbdb8c23cf02a55b596155a12f33ce4`  
		Last Modified: Tue, 04 Jul 2023 02:03:14 GMT  
		Size: 25.9 MB (25921452 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:a5aca7c1d27455fbb07fd0c3e6a0008432107ce5ef2a2c4637f344a487de1f9b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797025 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da12a48637f4525acc74e33841acdda2fc032ecb07772ed03ce9bd380b567c1e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:45 GMT
ADD file:a393ecc028bd834e64ec5d0de8332863a7415a3cf9240270187e51a289b7acb7 in / 
# Tue, 04 Jul 2023 01:39:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6ed7733e624d2cbe219f6dec2d92d91624155f2c9a93c476859bca25c0c126c`  
		Last Modified: Tue, 04 Jul 2023 01:45:37 GMT  
		Size: 27.8 MB (27797025 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-backports`

```console
$ docker pull debian@sha256:608eec6a8267663f290e43f81e73840c2366503835e6de177120cd76f09b3f53
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:2826d85a3a1461d057104dbed17a212d0b2e15f3e4f5d5f29f4ab67dbfe9b8ba
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50448905 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:55f0334078bd9a3d3a5805fba265a93a02c60e9eca5b80c011a7a914ad205d01`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:59 GMT
ADD file:3b029f3178b5e1b4334c6406617e14df5bf0ce15ce935acc2cb2dc51b6f151aa in / 
# Tue, 04 Jul 2023 01:20:59 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:21:04 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6b44c60b4ef8f05f0a7be874cdf80559a40b3584e9146bcef80caf7c3bd47f55`  
		Last Modified: Tue, 04 Jul 2023 01:26:40 GMT  
		Size: 50.4 MB (50448681 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b78762e368694aac5183db60432839840fd36b2b774681232bda6ea38c0cc543`  
		Last Modified: Tue, 04 Jul 2023 01:26:48 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:27705c3287f2a99db1dd0812f2cf7f07c6f10f338559136db7bf2b984f1f747a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45916703 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:99c3a89bed79b89b80de79c9612e1da0c327516081357c9562157ecd5f960aab`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:58:51 GMT
ADD file:fb0d0efa496216eb4150798e9acf6220f344ec01a00121d2ac5eb8efb61344c5 in / 
# Tue, 04 Jul 2023 00:58:53 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:58:56 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:003d12612fcea91fa985efa292e97219fbb80e0975883488634bf9f8193e765b`  
		Last Modified: Tue, 04 Jul 2023 01:04:36 GMT  
		Size: 45.9 MB (45916479 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:da294ea69efbe031e32997ebca5cce08305ed429a659b96f9b5e05ac2dac2480`  
		Last Modified: Tue, 04 Jul 2023 01:04:45 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:4931dad40b1a622ada730d6ff26d6dbf2c16e1b39feec3c65b8b3b3de84bf67a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49238832 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:88963793bf0695df5d56b2cbd07befd8d7f95bee13dd2024639c722b2f727853`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:16 GMT
ADD file:fc5434a757b40bf7ac5f064c4580674f3f844b4302b8bfb607cbc595c92c9efa in / 
# Tue, 04 Jul 2023 01:58:17 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:58:19 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:551faad51851c8a8fe8e9faacc89d609b82713385126737d5f6a56acb16ddac0`  
		Last Modified: Tue, 04 Jul 2023 02:02:57 GMT  
		Size: 49.2 MB (49238607 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5034f9f1eaefaa9cb076b68782eb1c922ccd111129a07e041fb44701bef13980`  
		Last Modified: Tue, 04 Jul 2023 02:03:06 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:621e9ac6a4b4e781a7437ddd09926a1b6061263ed30629c0c2778a2ca7422c8d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51206537 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c25277bca6ad77c273623c6d9a92643c5e9f98be0bd38d0ae6d448dc2ad636b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:33 GMT
ADD file:a125727a859efc0078fd6104c2d5add32d60bc44caafb60b95835ebdca850b6c in / 
# Tue, 04 Jul 2023 01:39:34 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:39:37 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ea65c1e207f0f923cefee5466d92585f5e03f36a470e46a88d80c4bb768818a4`  
		Last Modified: Tue, 04 Jul 2023 01:45:18 GMT  
		Size: 51.2 MB (51206313 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9728bf419556d019fcbc6f6a562f2d76219f5cb4e934d02036c8f8b514685fd1`  
		Last Modified: Tue, 04 Jul 2023 01:45:26 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-slim`

```console
$ docker pull debian@sha256:1bc6ca2f31cdee6617795f4c41b469afae16270923e6b04da408613a964c7c8c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:821883bc241809dfdcd52fab6074e448e13866c837e77c0f8fc98d119141e841
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27138557 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65ef81e58a24453417d9f0c89d44cb914d5db6c9baf68a82cfab7bbd0c035745`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:13 GMT
ADD file:f8c861794bc9356c93afccdade217410d2de0f1a9f5e739c9e965db11390daae in / 
# Tue, 04 Jul 2023 01:21:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a2d0007802c28f407af424fa43375a522555482f2e668b718adce767050ff52`  
		Last Modified: Tue, 04 Jul 2023 01:26:58 GMT  
		Size: 27.1 MB (27138557 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ce5120c140ac4f83d8e4fdd0337264e10a2808b77c1f1d19acfdecf26622ac88
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22747315 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef006db4e1c62a38032e83439a60f1fc665e625ccde7f67d13e17d9a54f92253`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:05 GMT
ADD file:fabf8cc707e11cc2f478a8f281b7ddbddfad58ae12f4e8d321c3a67b13fc1103 in / 
# Tue, 04 Jul 2023 00:59:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eed08a67e3ceb30217d9b34c7dc81a4c3612270511fd3b00780455ea50ed1972`  
		Last Modified: Tue, 04 Jul 2023 01:04:54 GMT  
		Size: 22.7 MB (22747315 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ba616b324dd575209493134e8a29ec21a0eb81d30bcfeb892d27ef7532b729d4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25921452 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:acb1d62777c727a9575b653d796b2063b6dc8a5cf897118787d64bdee10c5d55`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:25 GMT
ADD file:87a727c06fc42bff51baa77772f9272277e630b63377431df19144fb8b938543 in / 
# Tue, 04 Jul 2023 01:58:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:27a4a304213cd35e9c77c365966eb4ba4fbdb8c23cf02a55b596155a12f33ce4`  
		Last Modified: Tue, 04 Jul 2023 02:03:14 GMT  
		Size: 25.9 MB (25921452 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:a5aca7c1d27455fbb07fd0c3e6a0008432107ce5ef2a2c4637f344a487de1f9b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27797025 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da12a48637f4525acc74e33841acdda2fc032ecb07772ed03ce9bd380b567c1e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:45 GMT
ADD file:a393ecc028bd834e64ec5d0de8332863a7415a3cf9240270187e51a289b7acb7 in / 
# Tue, 04 Jul 2023 01:39:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6ed7733e624d2cbe219f6dec2d92d91624155f2c9a93c476859bca25c0c126c`  
		Last Modified: Tue, 04 Jul 2023 01:45:37 GMT  
		Size: 27.8 MB (27797025 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable`

```console
$ docker pull debian@sha256:2053cf94aadec2cc167488183a928165313c281b954d042d45ba65cb84459fde
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:5092e10827176e78b93b0ef9c5fa9721000b08bdee57cb103d7ae81d3b9efa17
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055303 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3b1ed8e0d05e3170c599b291075bf7fb201f1510f7f4450f5ca63dec7da9391`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:25 GMT
ADD file:aa8ed94ddb59e4c0538b39dcaa7040aa0aa62297242af9074715df5e49310d5c in / 
# Tue, 04 Jul 2023 01:21:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:70705a13f194adda8510677de1e9a79a43350123f9bd3d7a7be637b9f7084f32`  
		Last Modified: Tue, 04 Jul 2023 01:27:13 GMT  
		Size: 55.1 MB (55055303 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:f8be7aafbc78d47068cac5567bfcb796ea5060a2d02b640ddcad8b7157301739
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556780 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:803dc9bf7772d64648910a07eef759b73f949c53f9317cd78dfc6a206d750bc8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:55 GMT
ADD file:eedd41901674350e1e1469ed0910b376f827555b17b22e3e10f0110f86d62478 in / 
# Tue, 04 Jul 2023 00:48:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d4e822f3c7ca5566f101f8d90ca82f6f749f73c7518367686a1e48cb3d17f459`  
		Last Modified: Tue, 04 Jul 2023 00:52:58 GMT  
		Size: 52.6 MB (52556780 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:c0e897c91a0aa9cc120c6741c95956482fd12b6dd65d040214c0e3ab9ec9ebcb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218268 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a06afbabdaac7807aca5a1e3e1b9305fdd6f4efb4ae1499f73450325220fa17`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:22 GMT
ADD file:ea176a714b2523843bef5db3cdb0b649af02c3656e86f8c2c4f18054fae67225 in / 
# Tue, 04 Jul 2023 00:59:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:172a14bdbe7b3b2f83f795e8c55945aea2def70aa907d371cb1f32a71323d65f`  
		Last Modified: Tue, 04 Jul 2023 01:05:08 GMT  
		Size: 50.2 MB (50218268 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0967123e2a4f66b6c52cfb82843dfc41abd0b080cdcff3c8c71849587502be81
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703976 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37f1aa5c7fe931873d7ddb5f9b9ee02213998f26021bb055df14cc7257cb8f46`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:33 GMT
ADD file:1a5822a96f77d75084868721a1bb8d1591dfd1cd3dc5db29e59956edafa16bdb in / 
# Tue, 04 Jul 2023 01:58:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3311ddc370e1bf7a67ee3323c736fc8d88636bb086836df8086317ecae682df2`  
		Last Modified: Tue, 04 Jul 2023 02:03:28 GMT  
		Size: 53.7 MB (53703976 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:d470b2f3be24c2bf0975f17f294fced0ee04a7e6826626e63a4babcace1de209
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040748 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f922faee2316ec5cfac3728a67b35336bdea8910bc923098fe38938409052bcc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:57 GMT
ADD file:57c69bd29985e87430db91f2af1d239154249b5574a15b60df18114148c5bc16 in / 
# Tue, 04 Jul 2023 01:39:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7367da3b7b4ed975852a58cda403c7be1fb59785e1b07d9f0b3fe0d635117474`  
		Last Modified: Tue, 04 Jul 2023 01:45:54 GMT  
		Size: 56.0 MB (56040748 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; mips64le

```console
$ docker pull debian@sha256:7644e635da7e67beb5725e60a70e081e5f64a07a41485d76fc757f4dd8d7db8b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270477 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6aaaf5509e699202591d4e18852fc5328ea2b56b74f2f6c9be6dfc5071d9dfd6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:17 GMT
ADD file:01e73873458ff017b9060e02c6c6d6e80dd596c5d69932be1af564907154c729 in / 
# Tue, 04 Jul 2023 01:11:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cad33424acd55ddf10c5c761d89e4e342d970ebf80cbd441dafc9c4afaf0224b`  
		Last Modified: Tue, 04 Jul 2023 01:22:03 GMT  
		Size: 53.3 MB (53270477 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; ppc64le

```console
$ docker pull debian@sha256:f4ec1bb44eacd39abdfa16dba9517b23e9910b3c8f88a6ac4ac260a9a847ff87
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927737 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c7ed76179d0ec7671b0dbd2e7fa7885c910d08fe25e54036a3102881b25f41b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:53 GMT
ADD file:28017068c8ae84b6dadb06a194af584a65f0ea1966327d6ee4c66ff6b29bc8cf in / 
# Tue, 04 Jul 2023 01:18:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a92e1f73b9a25672e5117ddfc8a47ace827a65a17ac563c3cef3151fe2f94cd`  
		Last Modified: Tue, 04 Jul 2023 01:26:10 GMT  
		Size: 58.9 MB (58927737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; s390x

```console
$ docker pull debian@sha256:a9150aa122ba6681444f1860649a54acac99e42766a34f9353340c08674ee7dd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288169 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:70f00791bb2effdb250f6deeba35bab03dbff662e01c144d8663475570148708`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:56 GMT
ADD file:9a5b565563f2ef29c23b2ae9528258400fa561c8621431baa41e5586c4481a8b in / 
# Tue, 04 Jul 2023 01:33:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fe6dd3e95702ff7fd336d8943229a96a472888c11decf348b3344b315fb5a372`  
		Last Modified: Tue, 04 Jul 2023 01:37:50 GMT  
		Size: 53.3 MB (53288169 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20230703`

```console
$ docker pull debian@sha256:2053cf94aadec2cc167488183a928165313c281b954d042d45ba65cb84459fde
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20230703` - linux; amd64

```console
$ docker pull debian@sha256:5092e10827176e78b93b0ef9c5fa9721000b08bdee57cb103d7ae81d3b9efa17
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055303 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3b1ed8e0d05e3170c599b291075bf7fb201f1510f7f4450f5ca63dec7da9391`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:25 GMT
ADD file:aa8ed94ddb59e4c0538b39dcaa7040aa0aa62297242af9074715df5e49310d5c in / 
# Tue, 04 Jul 2023 01:21:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:70705a13f194adda8510677de1e9a79a43350123f9bd3d7a7be637b9f7084f32`  
		Last Modified: Tue, 04 Jul 2023 01:27:13 GMT  
		Size: 55.1 MB (55055303 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:f8be7aafbc78d47068cac5567bfcb796ea5060a2d02b640ddcad8b7157301739
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52556780 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:803dc9bf7772d64648910a07eef759b73f949c53f9317cd78dfc6a206d750bc8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:55 GMT
ADD file:eedd41901674350e1e1469ed0910b376f827555b17b22e3e10f0110f86d62478 in / 
# Tue, 04 Jul 2023 00:48:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d4e822f3c7ca5566f101f8d90ca82f6f749f73c7518367686a1e48cb3d17f459`  
		Last Modified: Tue, 04 Jul 2023 00:52:58 GMT  
		Size: 52.6 MB (52556780 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:c0e897c91a0aa9cc120c6741c95956482fd12b6dd65d040214c0e3ab9ec9ebcb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218268 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a06afbabdaac7807aca5a1e3e1b9305fdd6f4efb4ae1499f73450325220fa17`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:22 GMT
ADD file:ea176a714b2523843bef5db3cdb0b649af02c3656e86f8c2c4f18054fae67225 in / 
# Tue, 04 Jul 2023 00:59:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:172a14bdbe7b3b2f83f795e8c55945aea2def70aa907d371cb1f32a71323d65f`  
		Last Modified: Tue, 04 Jul 2023 01:05:08 GMT  
		Size: 50.2 MB (50218268 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0967123e2a4f66b6c52cfb82843dfc41abd0b080cdcff3c8c71849587502be81
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703976 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37f1aa5c7fe931873d7ddb5f9b9ee02213998f26021bb055df14cc7257cb8f46`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:33 GMT
ADD file:1a5822a96f77d75084868721a1bb8d1591dfd1cd3dc5db29e59956edafa16bdb in / 
# Tue, 04 Jul 2023 01:58:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3311ddc370e1bf7a67ee3323c736fc8d88636bb086836df8086317ecae682df2`  
		Last Modified: Tue, 04 Jul 2023 02:03:28 GMT  
		Size: 53.7 MB (53703976 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; 386

```console
$ docker pull debian@sha256:d470b2f3be24c2bf0975f17f294fced0ee04a7e6826626e63a4babcace1de209
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040748 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f922faee2316ec5cfac3728a67b35336bdea8910bc923098fe38938409052bcc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:57 GMT
ADD file:57c69bd29985e87430db91f2af1d239154249b5574a15b60df18114148c5bc16 in / 
# Tue, 04 Jul 2023 01:39:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7367da3b7b4ed975852a58cda403c7be1fb59785e1b07d9f0b3fe0d635117474`  
		Last Modified: Tue, 04 Jul 2023 01:45:54 GMT  
		Size: 56.0 MB (56040748 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:7644e635da7e67beb5725e60a70e081e5f64a07a41485d76fc757f4dd8d7db8b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270477 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6aaaf5509e699202591d4e18852fc5328ea2b56b74f2f6c9be6dfc5071d9dfd6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:17 GMT
ADD file:01e73873458ff017b9060e02c6c6d6e80dd596c5d69932be1af564907154c729 in / 
# Tue, 04 Jul 2023 01:11:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cad33424acd55ddf10c5c761d89e4e342d970ebf80cbd441dafc9c4afaf0224b`  
		Last Modified: Tue, 04 Jul 2023 01:22:03 GMT  
		Size: 53.3 MB (53270477 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:f4ec1bb44eacd39abdfa16dba9517b23e9910b3c8f88a6ac4ac260a9a847ff87
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927737 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c7ed76179d0ec7671b0dbd2e7fa7885c910d08fe25e54036a3102881b25f41b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:53 GMT
ADD file:28017068c8ae84b6dadb06a194af584a65f0ea1966327d6ee4c66ff6b29bc8cf in / 
# Tue, 04 Jul 2023 01:18:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a92e1f73b9a25672e5117ddfc8a47ace827a65a17ac563c3cef3151fe2f94cd`  
		Last Modified: Tue, 04 Jul 2023 01:26:10 GMT  
		Size: 58.9 MB (58927737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703` - linux; s390x

```console
$ docker pull debian@sha256:a9150aa122ba6681444f1860649a54acac99e42766a34f9353340c08674ee7dd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288169 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:70f00791bb2effdb250f6deeba35bab03dbff662e01c144d8663475570148708`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:56 GMT
ADD file:9a5b565563f2ef29c23b2ae9528258400fa561c8621431baa41e5586c4481a8b in / 
# Tue, 04 Jul 2023 01:33:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fe6dd3e95702ff7fd336d8943229a96a472888c11decf348b3344b315fb5a372`  
		Last Modified: Tue, 04 Jul 2023 01:37:50 GMT  
		Size: 53.3 MB (53288169 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20230703-slim`

```console
$ docker pull debian@sha256:86d12b9a2e6e2b70e3ebac3b4e07b2829d4dab502e313425f06b59f5ecde7bff
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:2a5a4eeed86180e188fdde729fd69288968a0dd7567718577a2090ec28325605
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:884079f207a34868572538c1f386cd983c8c085e24cd08d5315d8e1b868c3f01`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:38 GMT
ADD file:57e0de4174b0f1b239eb71c4883f51a6c1de1b8f38b312d7afc87678646ab483 in / 
# Tue, 04 Jul 2023 01:21:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce209994312d4b1dbdad0c561052e3561feb2688b28b943e0f1a1ce801cb12df`  
		Last Modified: Tue, 04 Jul 2023 01:27:31 GMT  
		Size: 31.4 MB (31417358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b40f91bf84ae86aa6b76bb2314811ed0dd818602c70d23d16822924697b52f33
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918867 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2a7e9a0e6f6b77efe7c337e7a1f21dfd8a45fcc01dda20cc8c21b225fd67a42f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:03 GMT
ADD file:f6200ebbbda685f73ac69851a23c660017af3532cb8665f62aff227ab6b11075 in / 
# Tue, 04 Jul 2023 00:49:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:43f1da47ada966113f4fea858aba11bb5672e6653d2cb73b5843d8a9b144d386`  
		Last Modified: Tue, 04 Jul 2023 00:53:17 GMT  
		Size: 28.9 MB (28918867 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:37831a567b079dfe30f749bc44283f2b35cf12a0834d11d4504c6f7312c4d234
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578750 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a0b497b2c84602f91fae9a17a21330c7fb404d4e178704550d8bdfd4d0121468`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:34 GMT
ADD file:bdb5755e16bc4d49ff90191703ff7ca7e82c2a98150d0a8276961df0b4138963 in / 
# Tue, 04 Jul 2023 00:59:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:71e4cfaebaf86c37bd00177823662a676191dd89cad7d26a4ac85fba16fd9173`  
		Last Modified: Tue, 04 Jul 2023 01:05:27 GMT  
		Size: 26.6 MB (26578750 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:60678469b9478953011798e688dcd51e3d5959d3c4cae903babcd6c92e10776d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062955 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e996633b9b5d47fcfc7e45de6dd019819a6306903375f114e5711dc4cb680052`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:42 GMT
ADD file:94645c9eae9cb5b29c18d9e22de4097d1323ca814a608cd4dec1c4de9fc8128c in / 
# Tue, 04 Jul 2023 01:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:318988d8345b1bd463a837edba8dad872aa5e68560a7fa40f672f32ae5e5efe2`  
		Last Modified: Tue, 04 Jul 2023 02:03:46 GMT  
		Size: 30.1 MB (30062955 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:7fc913ed1900745898130804ebadf4d2108dedfab6d5838db2e5049b75157c1d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2b729e795d479545f09117e7c78ad267429a19bce22839ba5a249f0aa30eafaf`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:08 GMT
ADD file:2c546607c976920aa73bf80cfd697f21fd018fd4e02a8843a895d81442cbf7bc in / 
# Tue, 04 Jul 2023 01:40:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:583b4e885e959116a8d05c19c4579a370f3e2b25c9b0b65b5a4322b1b3294ac7`  
		Last Modified: Tue, 04 Jul 2023 01:46:14 GMT  
		Size: 32.4 MB (32397505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:b03cffed8865f238607e41e58cab9943a70ea29d771bab48a7118bb3f5ff90a9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3cf4ab33202309d77d4ce44536b8ecf71bc8d83853b1cb8fd3458f60d822149`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:58 GMT
ADD file:e31404aead99be9dda613c3dce124b205e9094af36a3875d2367bc6e9e450439 in / 
# Tue, 04 Jul 2023 01:12:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0cfe3f8e9fd469459cdd70e51a36421caf3752db315fb9393dc4e1b30189ef03`  
		Last Modified: Tue, 04 Jul 2023 01:22:37 GMT  
		Size: 29.6 MB (29634358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:44fb3663a84518732d23f514102b657f2e2645570c6b29ac31e23cae2b315725
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291074 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:30e642a304f237bf2ee8210371987f793167cd455d9255076abcaa96e5751092`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:14 GMT
ADD file:fc5c4adbe49964131c079e19d7bfff949a0fe6032dd79808c7bd0e0138ec5661 in / 
# Tue, 04 Jul 2023 01:19:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0e63feeb38201393b1122ba79ccf6065ff5edf0c02633fd6464f3cc9b27f7e6`  
		Last Modified: Tue, 04 Jul 2023 01:26:38 GMT  
		Size: 35.3 MB (35291074 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:70f7809e1b2f0b0f78ea4acca6d94db9e8d26c3a8cde735cbced15137fd9ba33
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652541 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:debcc7b5cf81a89bbb608311b90ee97d16488aa33da6f8952848578b8e02a668`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:14 GMT
ADD file:5c8535f6e30f90794b09d5176bc8d0e265bb28db78c8c13d61b8855749ea6765 in / 
# Tue, 04 Jul 2023 01:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c746821e9185637dc199f8ef78e52f0295d7b33c11f2728917588e8e241c2628`  
		Last Modified: Tue, 04 Jul 2023 01:38:05 GMT  
		Size: 29.7 MB (29652541 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-backports`

```console
$ docker pull debian@sha256:4d7181d4dfa073dacb0bfcdccfe00877f8552afdc847b0c98202f126b67c73c0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:ef7812a599a58c346d949028ad5742a121bc4d6316aa745293a3e8d448eb85c6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55055528 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:806d590d2f5b342480c8f6ab41e7cf886ad37bd0fd8cd90e5500cddec69edb1d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:25 GMT
ADD file:aa8ed94ddb59e4c0538b39dcaa7040aa0aa62297242af9074715df5e49310d5c in / 
# Tue, 04 Jul 2023 01:21:25 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:21:29 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:70705a13f194adda8510677de1e9a79a43350123f9bd3d7a7be637b9f7084f32`  
		Last Modified: Tue, 04 Jul 2023 01:27:13 GMT  
		Size: 55.1 MB (55055303 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b66222735b3343b5b6b4d785e66b8f874dfb990b0f598d08cd3081d54660f5b4`  
		Last Modified: Tue, 04 Jul 2023 01:27:21 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:a1c142aaf6010ed412209a6930b30a61454d97e222e475d389c7c11cf3339218
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.6 MB (52557004 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4231655e7e94dd41818c717cf6ad97fade4aa67795febe1247db0de2e781766a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:48:55 GMT
ADD file:eedd41901674350e1e1469ed0910b376f827555b17b22e3e10f0110f86d62478 in / 
# Tue, 04 Jul 2023 00:48:56 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:48:58 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d4e822f3c7ca5566f101f8d90ca82f6f749f73c7518367686a1e48cb3d17f459`  
		Last Modified: Tue, 04 Jul 2023 00:52:58 GMT  
		Size: 52.6 MB (52556780 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:40b7a7789c97a6d50342a32a55eb69b34d57a5a9e65f9dba2787d9b6a7f76d30`  
		Last Modified: Tue, 04 Jul 2023 00:53:07 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:aef5318ba503c33f45cf7a269cfb2e6f670e8175c9b00b0a8f6e16192b2893f5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50218493 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:155602c72582ddb6094901a521ba97c5577a41c962da17679d86d43c4009170c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:22 GMT
ADD file:ea176a714b2523843bef5db3cdb0b649af02c3656e86f8c2c4f18054fae67225 in / 
# Tue, 04 Jul 2023 00:59:23 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:59:26 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:172a14bdbe7b3b2f83f795e8c55945aea2def70aa907d371cb1f32a71323d65f`  
		Last Modified: Tue, 04 Jul 2023 01:05:08 GMT  
		Size: 50.2 MB (50218268 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:36d7c97b4dfbcb704327b6bfaae85ad8b672d6a7846f992b10fb34ce524a5699`  
		Last Modified: Tue, 04 Jul 2023 01:05:18 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f90f6daecb83d31f33666d2e1103cc71189bdf79526e3111f59afed609772170
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53704202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b1c28916dd4ff432e265cbd6c76ba0d705b54f34c9176869bf0e59d96de13d70`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:33 GMT
ADD file:1a5822a96f77d75084868721a1bb8d1591dfd1cd3dc5db29e59956edafa16bdb in / 
# Tue, 04 Jul 2023 01:58:33 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:58:36 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3311ddc370e1bf7a67ee3323c736fc8d88636bb086836df8086317ecae682df2`  
		Last Modified: Tue, 04 Jul 2023 02:03:28 GMT  
		Size: 53.7 MB (53703976 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8048f09539baa115a9c083cc28085c52b8d868967b6db83c8c02f71144f3ffcb`  
		Last Modified: Tue, 04 Jul 2023 02:03:37 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:2592e2bc10273173f750e0ca7dfe48506731d33afd0a5782f7321aad3d819131
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56040970 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:df4e855f5baba675db26cfb1611d146ebcaac1f8248b52f9c80bc7df20f20f73`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:39:57 GMT
ADD file:57c69bd29985e87430db91f2af1d239154249b5574a15b60df18114148c5bc16 in / 
# Tue, 04 Jul 2023 01:39:58 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:40:00 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7367da3b7b4ed975852a58cda403c7be1fb59785e1b07d9f0b3fe0d635117474`  
		Last Modified: Tue, 04 Jul 2023 01:45:54 GMT  
		Size: 56.0 MB (56040748 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:955341ee1324c1f341f307c46792810a457fd7e5af2c43553de2c2cbbb1263d6`  
		Last Modified: Tue, 04 Jul 2023 01:46:02 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:9ed26dd86c73f492bb27b64317b40d92601467eb7c592295eec86d7fd8ed7415
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53270703 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1f509d0d0fe4f01b734f2050b452568f8f751d5648994bae95967965f76ef932`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:17 GMT
ADD file:01e73873458ff017b9060e02c6c6d6e80dd596c5d69932be1af564907154c729 in / 
# Tue, 04 Jul 2023 01:11:21 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:11:36 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:cad33424acd55ddf10c5c761d89e4e342d970ebf80cbd441dafc9c4afaf0224b`  
		Last Modified: Tue, 04 Jul 2023 01:22:03 GMT  
		Size: 53.3 MB (53270477 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:72ce2a6c04567e0c79f2b48c9c4f3d13c777db985e3eae22f07a9289af88866c`  
		Last Modified: Tue, 04 Jul 2023 01:22:12 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:6a99017a0a473945817202052d513e7e2dbf48ca1a272e1bffebf7c58e0e9e0f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58927961 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1046574891621967c2362700f745937d96160d35c2e556689e72c42e0c71bf59`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:18:53 GMT
ADD file:28017068c8ae84b6dadb06a194af584a65f0ea1966327d6ee4c66ff6b29bc8cf in / 
# Tue, 04 Jul 2023 01:18:56 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:19:01 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:4a92e1f73b9a25672e5117ddfc8a47ace827a65a17ac563c3cef3151fe2f94cd`  
		Last Modified: Tue, 04 Jul 2023 01:26:10 GMT  
		Size: 58.9 MB (58927737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ce0704b61031f4c425841aebeb50cbdc2e0e681d3c24fdf2488782cb58109308`  
		Last Modified: Tue, 04 Jul 2023 01:26:21 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; s390x

```console
$ docker pull debian@sha256:f1728a784f67ec26b605733b559378140df26d2a7c08442ecc83807b6368a995
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53288396 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3a838f2edc5fe106d82241bc217d282e0e3e0d5e519432d9afc79c4b1949b883`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:32:56 GMT
ADD file:9a5b565563f2ef29c23b2ae9528258400fa561c8621431baa41e5586c4481a8b in / 
# Tue, 04 Jul 2023 01:33:00 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:33:06 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:fe6dd3e95702ff7fd336d8943229a96a472888c11decf348b3344b315fb5a372`  
		Last Modified: Tue, 04 Jul 2023 01:37:50 GMT  
		Size: 53.3 MB (53288169 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:eac660a6995d12b63b75062ad31a1aa3ea2639ae2c8dbd213107840d67e2390b`  
		Last Modified: Tue, 04 Jul 2023 01:37:56 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-slim`

```console
$ docker pull debian@sha256:86d12b9a2e6e2b70e3ebac3b4e07b2829d4dab502e313425f06b59f5ecde7bff
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:2a5a4eeed86180e188fdde729fd69288968a0dd7567718577a2090ec28325605
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31417358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:884079f207a34868572538c1f386cd983c8c085e24cd08d5315d8e1b868c3f01`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:38 GMT
ADD file:57e0de4174b0f1b239eb71c4883f51a6c1de1b8f38b312d7afc87678646ab483 in / 
# Tue, 04 Jul 2023 01:21:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce209994312d4b1dbdad0c561052e3561feb2688b28b943e0f1a1ce801cb12df`  
		Last Modified: Tue, 04 Jul 2023 01:27:31 GMT  
		Size: 31.4 MB (31417358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:b40f91bf84ae86aa6b76bb2314811ed0dd818602c70d23d16822924697b52f33
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28918867 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2a7e9a0e6f6b77efe7c337e7a1f21dfd8a45fcc01dda20cc8c21b225fd67a42f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:03 GMT
ADD file:f6200ebbbda685f73ac69851a23c660017af3532cb8665f62aff227ab6b11075 in / 
# Tue, 04 Jul 2023 00:49:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:43f1da47ada966113f4fea858aba11bb5672e6653d2cb73b5843d8a9b144d386`  
		Last Modified: Tue, 04 Jul 2023 00:53:17 GMT  
		Size: 28.9 MB (28918867 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:37831a567b079dfe30f749bc44283f2b35cf12a0834d11d4504c6f7312c4d234
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26578750 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a0b497b2c84602f91fae9a17a21330c7fb404d4e178704550d8bdfd4d0121468`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:34 GMT
ADD file:bdb5755e16bc4d49ff90191703ff7ca7e82c2a98150d0a8276961df0b4138963 in / 
# Tue, 04 Jul 2023 00:59:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:71e4cfaebaf86c37bd00177823662a676191dd89cad7d26a4ac85fba16fd9173`  
		Last Modified: Tue, 04 Jul 2023 01:05:27 GMT  
		Size: 26.6 MB (26578750 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:60678469b9478953011798e688dcd51e3d5959d3c4cae903babcd6c92e10776d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30062955 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e996633b9b5d47fcfc7e45de6dd019819a6306903375f114e5711dc4cb680052`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:42 GMT
ADD file:94645c9eae9cb5b29c18d9e22de4097d1323ca814a608cd4dec1c4de9fc8128c in / 
# Tue, 04 Jul 2023 01:58:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:318988d8345b1bd463a837edba8dad872aa5e68560a7fa40f672f32ae5e5efe2`  
		Last Modified: Tue, 04 Jul 2023 02:03:46 GMT  
		Size: 30.1 MB (30062955 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:7fc913ed1900745898130804ebadf4d2108dedfab6d5838db2e5049b75157c1d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32397505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2b729e795d479545f09117e7c78ad267429a19bce22839ba5a249f0aa30eafaf`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:08 GMT
ADD file:2c546607c976920aa73bf80cfd697f21fd018fd4e02a8843a895d81442cbf7bc in / 
# Tue, 04 Jul 2023 01:40:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:583b4e885e959116a8d05c19c4579a370f3e2b25c9b0b65b5a4322b1b3294ac7`  
		Last Modified: Tue, 04 Jul 2023 01:46:14 GMT  
		Size: 32.4 MB (32397505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:b03cffed8865f238607e41e58cab9943a70ea29d771bab48a7118bb3f5ff90a9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29634358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3cf4ab33202309d77d4ce44536b8ecf71bc8d83853b1cb8fd3458f60d822149`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:58 GMT
ADD file:e31404aead99be9dda613c3dce124b205e9094af36a3875d2367bc6e9e450439 in / 
# Tue, 04 Jul 2023 01:12:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0cfe3f8e9fd469459cdd70e51a36421caf3752db315fb9393dc4e1b30189ef03`  
		Last Modified: Tue, 04 Jul 2023 01:22:37 GMT  
		Size: 29.6 MB (29634358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:44fb3663a84518732d23f514102b657f2e2645570c6b29ac31e23cae2b315725
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35291074 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:30e642a304f237bf2ee8210371987f793167cd455d9255076abcaa96e5751092`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:14 GMT
ADD file:fc5c4adbe49964131c079e19d7bfff949a0fe6032dd79808c7bd0e0138ec5661 in / 
# Tue, 04 Jul 2023 01:19:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0e63feeb38201393b1122ba79ccf6065ff5edf0c02633fd6464f3cc9b27f7e6`  
		Last Modified: Tue, 04 Jul 2023 01:26:38 GMT  
		Size: 35.3 MB (35291074 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:70f7809e1b2f0b0f78ea4acca6d94db9e8d26c3a8cde735cbced15137fd9ba33
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29652541 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:debcc7b5cf81a89bbb608311b90ee97d16488aa33da6f8952848578b8e02a668`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:14 GMT
ADD file:5c8535f6e30f90794b09d5176bc8d0e265bb28db78c8c13d61b8855749ea6765 in / 
# Tue, 04 Jul 2023 01:33:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c746821e9185637dc199f8ef78e52f0295d7b33c11f2728917588e8e241c2628`  
		Last Modified: Tue, 04 Jul 2023 01:38:05 GMT  
		Size: 29.7 MB (29652541 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy`

```console
$ docker pull debian@sha256:e60a653e91928537ed40b0e710b40652e2d84eb6983dc30564cc7b6048abf6a0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy` - linux; amd64

```console
$ docker pull debian@sha256:3a4c3cd5c45f6725d6e0b15e200ef0fddea069c6f3435e1e6acda6da063b2caa
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49476172 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea445cd8a5977acf202de4a5dc3713b702db04de630e3d5aa27e284432b501a3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:50 GMT
ADD file:4bfda8f11e59383c37bfa6038bb5d22f58b9724d249a62568a2e7d2908821cd3 in / 
# Tue, 04 Jul 2023 01:21:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:23:44 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:6efa412d16b908043ec01c4ac9ff7323c0f488dd5d7ea4b9b086f1e80350b9b3`  
		Last Modified: Tue, 04 Jul 2023 01:27:46 GMT  
		Size: 49.5 MB (49475944 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d5938779f7041d2eed56d07a832f8b26100c8c792d5c70dbd81ee104db155b96`  
		Last Modified: Tue, 04 Jul 2023 01:30:18 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v5

```console
$ docker pull debian@sha256:8431f23413f063c89ca1c5c6bc53816226370f5d807f28478533251d83a23ca3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322701 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c31435e717296b9e24b2c39649540a77f07be5b3086d293441493344b7841809`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:11 GMT
ADD file:e6181e27889c016519d8d701345aafff7138df96a76fb6a29403c8a33a3365fe in / 
# Tue, 04 Jul 2023 00:49:11 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:50:28 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:667272d8fe1bc81ce33874dbc84976c86d6b95f50198465a6ab672c71d87f666`  
		Last Modified: Tue, 04 Jul 2023 00:53:33 GMT  
		Size: 47.3 MB (47322473 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2190885d65eb97f69c0c7c0c158851e72bdf60e2cfdb2b1136c8de3f6dd6c37f`  
		Last Modified: Tue, 04 Jul 2023 00:56:15 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v7

```console
$ docker pull debian@sha256:7ab5870e0bdbdeeecef461a5b5fbb2906ea9ee2ef4c5da1c1b8998deed1c8a18
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45124201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5bfc027409f8fba31974738384e2dfa1f2a1a243b3f73b6fd5e1b69f93300462`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:44 GMT
ADD file:5be8eee7722dc860882f26018d8a0de49a4db6fcaa0f35d1dfbb74eff22929cb in / 
# Tue, 04 Jul 2023 00:59:45 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:01:33 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:42cc2becb0b735f0260d9f678fb1201bbe47c438212c5faf04879fab6a2dd73f`  
		Last Modified: Tue, 04 Jul 2023 01:05:43 GMT  
		Size: 45.1 MB (45123975 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1ce68afc01eaea6b552daf2fa88d7caafdad8a2a5bd322b779b58e945322376f`  
		Last Modified: Tue, 04 Jul 2023 01:08:25 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:74c35a88542099de6bc301d5b0a41b91f478e0a136693a73b6b828c07870f328
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482793 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c7dc11db86b8fe46faa58399faa8901211dd196a273b814306fc856887243c3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:49 GMT
ADD file:2c2c0588b93f9af7d93f19033795edf9e61a692ff56e65d9e9500915276782c9 in / 
# Tue, 04 Jul 2023 01:58:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 02:00:05 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d1b52151d5bb774f3680f8697c343bae1b64b7308354db57dc6bbeb7d65d0964`  
		Last Modified: Tue, 04 Jul 2023 02:03:59 GMT  
		Size: 49.5 MB (49482570 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d40b228ca99236a2bb77881dfde126d9e56af64935e3319b6656cb719d0eee3f`  
		Last Modified: Tue, 04 Jul 2023 02:06:25 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; 386

```console
$ docker pull debian@sha256:c959765e87be9a204eac6d8182448e6bba3f6609eb0400c88e076fa894e29e19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50506052 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:509ad128766c27fb0dd4f93bcb68388825c995b17b3ad06da40fc6b11fdd05f7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:19 GMT
ADD file:565f8487c71b5de967f1bf16d4bd86291107b68f4152a61e35a8ab86e9f83b7b in / 
# Tue, 04 Jul 2023 01:40:20 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:42:05 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:46685592106ba6554716942e180f6b4cd8a73f1ec934554b40aee89b47118f5e`  
		Last Modified: Tue, 04 Jul 2023 01:46:32 GMT  
		Size: 50.5 MB (50505825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4d74b547e670384f06e7000b09b5fc6ab9f25571d708e00fec2c351140f5d3b4`  
		Last Modified: Tue, 04 Jul 2023 01:49:23 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; mips64le

```console
$ docker pull debian@sha256:e22162bafbe5ff7e199da64bdbb05a5e1a9d73af060a0cec82099bc3e314584e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:551188c58f26c946df80410bde70b326be018cf165af6aaa6c09ab618459e2ed`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:32 GMT
ADD file:ab2873cb088258c79fab5dde2d754d1967ca11e58adcf6a08811206759a323d5 in / 
# Tue, 04 Jul 2023 01:12:35 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:18:03 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:796c2f1009ce9a433d10a1f4708cbb79ef733dcb0ca992041a6aa621e85d8410`  
		Last Modified: Tue, 04 Jul 2023 01:23:15 GMT  
		Size: 49.5 MB (49455398 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:619a4904f147ba7ddbe54aa9fa10e34000a998f58d5372d5f112bd3a1bbdc738`  
		Last Modified: Tue, 04 Jul 2023 01:28:45 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; ppc64le

```console
$ docker pull debian@sha256:7f4da65286d7e5db96f60b1cb22b5c07676ee5b56162370fe299912a934f2d1e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474639 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea78d3f7f329c36df0d2b483eda4e8cb72aed603a61fd9a9408b594257a6e396`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:33 GMT
ADD file:4c1d6f76704d32b9b503bd61c4b485555ec28037fb500403ffb5cbb102cf4509 in / 
# Tue, 04 Jul 2023 01:19:36 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:43 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3e4e6efa69b637e0e8ca00ddcae1aed23aef874a92a2904bdc6c9fa5b3212b8a`  
		Last Modified: Tue, 04 Jul 2023 01:27:02 GMT  
		Size: 53.5 MB (53474414 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7222666516418e1bc453a1d0aa1c322401427d980c317645d0ced94a082bcf88`  
		Last Modified: Tue, 04 Jul 2023 01:31:11 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; s390x

```console
$ docker pull debian@sha256:ab30641b065b5a26db630781063daaa6bee194f8a3d62c163f1f05dd6ec5780a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8f5e583fbbb5738a6f1ec6510b1001913887d33ac5540dbdce71a8a1d000a762`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:27 GMT
ADD file:9c12086273bf86ac1b3f72beebd17cc357bb749a95ac64d0f5ecdbbe7631cec4 in / 
# Tue, 04 Jul 2023 01:33:29 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:35:49 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3dcb3b9eb8abea3e1994db2e9f2073832965384334d3e204d1df1e3dcd09cffa`  
		Last Modified: Tue, 04 Jul 2023 01:38:16 GMT  
		Size: 47.9 MB (47890767 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0e2e9a34da129975469d0fb165471084cb9ad56a688067ce780bfbe786020397`  
		Last Modified: Tue, 04 Jul 2023 01:40:13 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy-20230703`

```console
$ docker pull debian@sha256:e60a653e91928537ed40b0e710b40652e2d84eb6983dc30564cc7b6048abf6a0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy-20230703` - linux; amd64

```console
$ docker pull debian@sha256:3a4c3cd5c45f6725d6e0b15e200ef0fddea069c6f3435e1e6acda6da063b2caa
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49476172 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea445cd8a5977acf202de4a5dc3713b702db04de630e3d5aa27e284432b501a3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:50 GMT
ADD file:4bfda8f11e59383c37bfa6038bb5d22f58b9724d249a62568a2e7d2908821cd3 in / 
# Tue, 04 Jul 2023 01:21:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:23:44 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:6efa412d16b908043ec01c4ac9ff7323c0f488dd5d7ea4b9b086f1e80350b9b3`  
		Last Modified: Tue, 04 Jul 2023 01:27:46 GMT  
		Size: 49.5 MB (49475944 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d5938779f7041d2eed56d07a832f8b26100c8c792d5c70dbd81ee104db155b96`  
		Last Modified: Tue, 04 Jul 2023 01:30:18 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:8431f23413f063c89ca1c5c6bc53816226370f5d807f28478533251d83a23ca3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322701 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c31435e717296b9e24b2c39649540a77f07be5b3086d293441493344b7841809`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:11 GMT
ADD file:e6181e27889c016519d8d701345aafff7138df96a76fb6a29403c8a33a3365fe in / 
# Tue, 04 Jul 2023 00:49:11 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:50:28 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:667272d8fe1bc81ce33874dbc84976c86d6b95f50198465a6ab672c71d87f666`  
		Last Modified: Tue, 04 Jul 2023 00:53:33 GMT  
		Size: 47.3 MB (47322473 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2190885d65eb97f69c0c7c0c158851e72bdf60e2cfdb2b1136c8de3f6dd6c37f`  
		Last Modified: Tue, 04 Jul 2023 00:56:15 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:7ab5870e0bdbdeeecef461a5b5fbb2906ea9ee2ef4c5da1c1b8998deed1c8a18
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45124201 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5bfc027409f8fba31974738384e2dfa1f2a1a243b3f73b6fd5e1b69f93300462`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:44 GMT
ADD file:5be8eee7722dc860882f26018d8a0de49a4db6fcaa0f35d1dfbb74eff22929cb in / 
# Tue, 04 Jul 2023 00:59:45 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:01:33 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:42cc2becb0b735f0260d9f678fb1201bbe47c438212c5faf04879fab6a2dd73f`  
		Last Modified: Tue, 04 Jul 2023 01:05:43 GMT  
		Size: 45.1 MB (45123975 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1ce68afc01eaea6b552daf2fa88d7caafdad8a2a5bd322b779b58e945322376f`  
		Last Modified: Tue, 04 Jul 2023 01:08:25 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:74c35a88542099de6bc301d5b0a41b91f478e0a136693a73b6b828c07870f328
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482793 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c7dc11db86b8fe46faa58399faa8901211dd196a273b814306fc856887243c3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:49 GMT
ADD file:2c2c0588b93f9af7d93f19033795edf9e61a692ff56e65d9e9500915276782c9 in / 
# Tue, 04 Jul 2023 01:58:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 02:00:05 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d1b52151d5bb774f3680f8697c343bae1b64b7308354db57dc6bbeb7d65d0964`  
		Last Modified: Tue, 04 Jul 2023 02:03:59 GMT  
		Size: 49.5 MB (49482570 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d40b228ca99236a2bb77881dfde126d9e56af64935e3319b6656cb719d0eee3f`  
		Last Modified: Tue, 04 Jul 2023 02:06:25 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; 386

```console
$ docker pull debian@sha256:c959765e87be9a204eac6d8182448e6bba3f6609eb0400c88e076fa894e29e19
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50506052 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:509ad128766c27fb0dd4f93bcb68388825c995b17b3ad06da40fc6b11fdd05f7`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:19 GMT
ADD file:565f8487c71b5de967f1bf16d4bd86291107b68f4152a61e35a8ab86e9f83b7b in / 
# Tue, 04 Jul 2023 01:40:20 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:42:05 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:46685592106ba6554716942e180f6b4cd8a73f1ec934554b40aee89b47118f5e`  
		Last Modified: Tue, 04 Jul 2023 01:46:32 GMT  
		Size: 50.5 MB (50505825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4d74b547e670384f06e7000b09b5fc6ab9f25571d708e00fec2c351140f5d3b4`  
		Last Modified: Tue, 04 Jul 2023 01:49:23 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:e22162bafbe5ff7e199da64bdbb05a5e1a9d73af060a0cec82099bc3e314584e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:551188c58f26c946df80410bde70b326be018cf165af6aaa6c09ab618459e2ed`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:32 GMT
ADD file:ab2873cb088258c79fab5dde2d754d1967ca11e58adcf6a08811206759a323d5 in / 
# Tue, 04 Jul 2023 01:12:35 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:18:03 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:796c2f1009ce9a433d10a1f4708cbb79ef733dcb0ca992041a6aa621e85d8410`  
		Last Modified: Tue, 04 Jul 2023 01:23:15 GMT  
		Size: 49.5 MB (49455398 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:619a4904f147ba7ddbe54aa9fa10e34000a998f58d5372d5f112bd3a1bbdc738`  
		Last Modified: Tue, 04 Jul 2023 01:28:45 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:7f4da65286d7e5db96f60b1cb22b5c07676ee5b56162370fe299912a934f2d1e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474639 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea78d3f7f329c36df0d2b483eda4e8cb72aed603a61fd9a9408b594257a6e396`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:33 GMT
ADD file:4c1d6f76704d32b9b503bd61c4b485555ec28037fb500403ffb5cbb102cf4509 in / 
# Tue, 04 Jul 2023 01:19:36 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:43 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3e4e6efa69b637e0e8ca00ddcae1aed23aef874a92a2904bdc6c9fa5b3212b8a`  
		Last Modified: Tue, 04 Jul 2023 01:27:02 GMT  
		Size: 53.5 MB (53474414 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7222666516418e1bc453a1d0aa1c322401427d980c317645d0ced94a082bcf88`  
		Last Modified: Tue, 04 Jul 2023 01:31:11 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20230703` - linux; s390x

```console
$ docker pull debian@sha256:ab30641b065b5a26db630781063daaa6bee194f8a3d62c163f1f05dd6ec5780a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8f5e583fbbb5738a6f1ec6510b1001913887d33ac5540dbdce71a8a1d000a762`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:27 GMT
ADD file:9c12086273bf86ac1b3f72beebd17cc357bb749a95ac64d0f5ecdbbe7631cec4 in / 
# Tue, 04 Jul 2023 01:33:29 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:35:49 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3dcb3b9eb8abea3e1994db2e9f2073832965384334d3e204d1df1e3dcd09cffa`  
		Last Modified: Tue, 04 Jul 2023 01:38:16 GMT  
		Size: 47.9 MB (47890767 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0e2e9a34da129975469d0fb165471084cb9ad56a688067ce780bfbe786020397`  
		Last Modified: Tue, 04 Jul 2023 01:40:13 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid`

```console
$ docker pull debian@sha256:ed955d4461b4f20cf6410dc48867eb1e3fa548462238dadbe8dc7f3db3491a11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid` - linux; amd64

```console
$ docker pull debian@sha256:527d1900b08e4e9954342af54c86ca48b719a57863fea4598969179b7217cde9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49475944 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0352bedc8566b8bfc33118027c509852fafb085a76cd46482285b0b33104f86c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:50 GMT
ADD file:4bfda8f11e59383c37bfa6038bb5d22f58b9724d249a62568a2e7d2908821cd3 in / 
# Tue, 04 Jul 2023 01:21:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6efa412d16b908043ec01c4ac9ff7323c0f488dd5d7ea4b9b086f1e80350b9b3`  
		Last Modified: Tue, 04 Jul 2023 01:27:46 GMT  
		Size: 49.5 MB (49475944 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v5

```console
$ docker pull debian@sha256:98db827b3315e6af024e1d0f4f03622efe269c17a186f8ff174154700dc402d1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322473 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3a8ce255ed47173a6384bafe68a5e15b4a0ae3404e7067f092495af7d2d09cdc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:11 GMT
ADD file:e6181e27889c016519d8d701345aafff7138df96a76fb6a29403c8a33a3365fe in / 
# Tue, 04 Jul 2023 00:49:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:667272d8fe1bc81ce33874dbc84976c86d6b95f50198465a6ab672c71d87f666`  
		Last Modified: Tue, 04 Jul 2023 00:53:33 GMT  
		Size: 47.3 MB (47322473 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v7

```console
$ docker pull debian@sha256:fb1bb948f398032422680e00b121042c96b9f514c9d7d06e729794235e56dade
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45123975 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7bf5aaa59e410a8a2a6ced815ef08afaaed20351f8d65f09d212bc449880f18a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:44 GMT
ADD file:5be8eee7722dc860882f26018d8a0de49a4db6fcaa0f35d1dfbb74eff22929cb in / 
# Tue, 04 Jul 2023 00:59:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cc2becb0b735f0260d9f678fb1201bbe47c438212c5faf04879fab6a2dd73f`  
		Last Modified: Tue, 04 Jul 2023 01:05:43 GMT  
		Size: 45.1 MB (45123975 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0bfce521b58361c7c8c9d58aac2e07bbb7a6055e1e0e864d762a3d646209f6ea
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482570 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:17af57cd5d36addfd9b4035dcc305565bf8268c54f7d3e2c975fff005fd2a7ac`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:49 GMT
ADD file:2c2c0588b93f9af7d93f19033795edf9e61a692ff56e65d9e9500915276782c9 in / 
# Tue, 04 Jul 2023 01:58:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d1b52151d5bb774f3680f8697c343bae1b64b7308354db57dc6bbeb7d65d0964`  
		Last Modified: Tue, 04 Jul 2023 02:03:59 GMT  
		Size: 49.5 MB (49482570 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; 386

```console
$ docker pull debian@sha256:8be7b9b4a2318d91ae6628a709a8bc59f292e888ab8bf4d784bfc074d9e84634
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50505825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe593256ea621e26e0ffc9dc84143ba336a3a3b98542b657e863de46eb337d6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:19 GMT
ADD file:565f8487c71b5de967f1bf16d4bd86291107b68f4152a61e35a8ab86e9f83b7b in / 
# Tue, 04 Jul 2023 01:40:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:46685592106ba6554716942e180f6b4cd8a73f1ec934554b40aee89b47118f5e`  
		Last Modified: Tue, 04 Jul 2023 01:46:32 GMT  
		Size: 50.5 MB (50505825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; mips64le

```console
$ docker pull debian@sha256:78e9e938637443d40d19670a1799c8bb0430f20fa5d13b2b85a621ecfa80da64
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455398 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af176b0443ef6dc99919608339d4f3fa67b396e5cddbe4ce0ac2998ea87dbdfe`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:32 GMT
ADD file:ab2873cb088258c79fab5dde2d754d1967ca11e58adcf6a08811206759a323d5 in / 
# Tue, 04 Jul 2023 01:12:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:796c2f1009ce9a433d10a1f4708cbb79ef733dcb0ca992041a6aa621e85d8410`  
		Last Modified: Tue, 04 Jul 2023 01:23:15 GMT  
		Size: 49.5 MB (49455398 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; ppc64le

```console
$ docker pull debian@sha256:d23a299e9b85d70427a73932df37395effe6c513882855f32cbdd2421f84e75f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474414 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d481733fcdd11dca699ebafb8d1d99a5ece02cacb92e8782d58cb9d111a9f364`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:33 GMT
ADD file:4c1d6f76704d32b9b503bd61c4b485555ec28037fb500403ffb5cbb102cf4509 in / 
# Tue, 04 Jul 2023 01:19:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e4e6efa69b637e0e8ca00ddcae1aed23aef874a92a2904bdc6c9fa5b3212b8a`  
		Last Modified: Tue, 04 Jul 2023 01:27:02 GMT  
		Size: 53.5 MB (53474414 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; riscv64

```console
$ docker pull debian@sha256:c2a971817bdf88b769ca1709245c327bebc67a6f9f1f2a28f8833ea629749e27
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695454 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e59f47ee47d835089785a5eb9de25f51970032e7f071b48d1bcc71c32989d552`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:05 GMT
ADD file:dc2a1586d12e833e5dc300033838059f3946ddddccde6c62d17cba40536ead15 in / 
# Tue, 04 Jul 2023 01:09:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d273b82777a979179469448ba9e255d7f7e1671f28d5610f47e8530093aaa04e`  
		Last Modified: Tue, 04 Jul 2023 01:12:27 GMT  
		Size: 45.7 MB (45695454 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; s390x

```console
$ docker pull debian@sha256:ccaa7bfd3e25f69a5828afd30905e922bbadefedde1b2a680a51114a3dfa0b89
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890767 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7999368901e4cbd916a71a0db886a9da34cbbea445162dc4fffc992d2a20d247`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:27 GMT
ADD file:9c12086273bf86ac1b3f72beebd17cc357bb749a95ac64d0f5ecdbbe7631cec4 in / 
# Tue, 04 Jul 2023 01:33:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3dcb3b9eb8abea3e1994db2e9f2073832965384334d3e204d1df1e3dcd09cffa`  
		Last Modified: Tue, 04 Jul 2023 01:38:16 GMT  
		Size: 47.9 MB (47890767 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20230703`

```console
$ docker pull debian@sha256:ed955d4461b4f20cf6410dc48867eb1e3fa548462238dadbe8dc7f3db3491a11
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20230703` - linux; amd64

```console
$ docker pull debian@sha256:527d1900b08e4e9954342af54c86ca48b719a57863fea4598969179b7217cde9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49475944 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0352bedc8566b8bfc33118027c509852fafb085a76cd46482285b0b33104f86c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:50 GMT
ADD file:4bfda8f11e59383c37bfa6038bb5d22f58b9724d249a62568a2e7d2908821cd3 in / 
# Tue, 04 Jul 2023 01:21:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6efa412d16b908043ec01c4ac9ff7323c0f488dd5d7ea4b9b086f1e80350b9b3`  
		Last Modified: Tue, 04 Jul 2023 01:27:46 GMT  
		Size: 49.5 MB (49475944 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:98db827b3315e6af024e1d0f4f03622efe269c17a186f8ff174154700dc402d1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322473 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3a8ce255ed47173a6384bafe68a5e15b4a0ae3404e7067f092495af7d2d09cdc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:11 GMT
ADD file:e6181e27889c016519d8d701345aafff7138df96a76fb6a29403c8a33a3365fe in / 
# Tue, 04 Jul 2023 00:49:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:667272d8fe1bc81ce33874dbc84976c86d6b95f50198465a6ab672c71d87f666`  
		Last Modified: Tue, 04 Jul 2023 00:53:33 GMT  
		Size: 47.3 MB (47322473 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:fb1bb948f398032422680e00b121042c96b9f514c9d7d06e729794235e56dade
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45123975 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7bf5aaa59e410a8a2a6ced815ef08afaaed20351f8d65f09d212bc449880f18a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:44 GMT
ADD file:5be8eee7722dc860882f26018d8a0de49a4db6fcaa0f35d1dfbb74eff22929cb in / 
# Tue, 04 Jul 2023 00:59:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:42cc2becb0b735f0260d9f678fb1201bbe47c438212c5faf04879fab6a2dd73f`  
		Last Modified: Tue, 04 Jul 2023 01:05:43 GMT  
		Size: 45.1 MB (45123975 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0bfce521b58361c7c8c9d58aac2e07bbb7a6055e1e0e864d762a3d646209f6ea
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482570 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:17af57cd5d36addfd9b4035dcc305565bf8268c54f7d3e2c975fff005fd2a7ac`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:49 GMT
ADD file:2c2c0588b93f9af7d93f19033795edf9e61a692ff56e65d9e9500915276782c9 in / 
# Tue, 04 Jul 2023 01:58:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d1b52151d5bb774f3680f8697c343bae1b64b7308354db57dc6bbeb7d65d0964`  
		Last Modified: Tue, 04 Jul 2023 02:03:59 GMT  
		Size: 49.5 MB (49482570 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; 386

```console
$ docker pull debian@sha256:8be7b9b4a2318d91ae6628a709a8bc59f292e888ab8bf4d784bfc074d9e84634
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50505825 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe593256ea621e26e0ffc9dc84143ba336a3a3b98542b657e863de46eb337d6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:19 GMT
ADD file:565f8487c71b5de967f1bf16d4bd86291107b68f4152a61e35a8ab86e9f83b7b in / 
# Tue, 04 Jul 2023 01:40:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:46685592106ba6554716942e180f6b4cd8a73f1ec934554b40aee89b47118f5e`  
		Last Modified: Tue, 04 Jul 2023 01:46:32 GMT  
		Size: 50.5 MB (50505825 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:78e9e938637443d40d19670a1799c8bb0430f20fa5d13b2b85a621ecfa80da64
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455398 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af176b0443ef6dc99919608339d4f3fa67b396e5cddbe4ce0ac2998ea87dbdfe`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:32 GMT
ADD file:ab2873cb088258c79fab5dde2d754d1967ca11e58adcf6a08811206759a323d5 in / 
# Tue, 04 Jul 2023 01:12:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:796c2f1009ce9a433d10a1f4708cbb79ef733dcb0ca992041a6aa621e85d8410`  
		Last Modified: Tue, 04 Jul 2023 01:23:15 GMT  
		Size: 49.5 MB (49455398 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:d23a299e9b85d70427a73932df37395effe6c513882855f32cbdd2421f84e75f
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474414 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d481733fcdd11dca699ebafb8d1d99a5ece02cacb92e8782d58cb9d111a9f364`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:33 GMT
ADD file:4c1d6f76704d32b9b503bd61c4b485555ec28037fb500403ffb5cbb102cf4509 in / 
# Tue, 04 Jul 2023 01:19:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e4e6efa69b637e0e8ca00ddcae1aed23aef874a92a2904bdc6c9fa5b3212b8a`  
		Last Modified: Tue, 04 Jul 2023 01:27:02 GMT  
		Size: 53.5 MB (53474414 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; riscv64

```console
$ docker pull debian@sha256:c2a971817bdf88b769ca1709245c327bebc67a6f9f1f2a28f8833ea629749e27
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695454 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e59f47ee47d835089785a5eb9de25f51970032e7f071b48d1bcc71c32989d552`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:05 GMT
ADD file:dc2a1586d12e833e5dc300033838059f3946ddddccde6c62d17cba40536ead15 in / 
# Tue, 04 Jul 2023 01:09:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d273b82777a979179469448ba9e255d7f7e1671f28d5610f47e8530093aaa04e`  
		Last Modified: Tue, 04 Jul 2023 01:12:27 GMT  
		Size: 45.7 MB (45695454 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703` - linux; s390x

```console
$ docker pull debian@sha256:ccaa7bfd3e25f69a5828afd30905e922bbadefedde1b2a680a51114a3dfa0b89
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890767 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7999368901e4cbd916a71a0db886a9da34cbbea445162dc4fffc992d2a20d247`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:27 GMT
ADD file:9c12086273bf86ac1b3f72beebd17cc357bb749a95ac64d0f5ecdbbe7631cec4 in / 
# Tue, 04 Jul 2023 01:33:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3dcb3b9eb8abea3e1994db2e9f2073832965384334d3e204d1df1e3dcd09cffa`  
		Last Modified: Tue, 04 Jul 2023 01:38:16 GMT  
		Size: 47.9 MB (47890767 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20230703-slim`

```console
$ docker pull debian@sha256:b257de913a92758b494666a8666b8488ef2fcf73edd6fa8a99a338fbf6338b20
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:0f5968f2f4baa6cdf74a7cfdece3fee99230ac2fc8007ddb427d5d9f4032ee68
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095988 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6762df5b728ea5688a6e5e297ab239d01e88268af7d3e3ca251772f9bb9731a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:59 GMT
ADD file:e74345927b6edb39fe0c4423514889e8695d61e850a899668c64cfceb0ef647f in / 
# Tue, 04 Jul 2023 01:21:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9f5c278176eebbc57aa00f26af876d1bd6ea8252f3662ecef8e4b4be2260ede2`  
		Last Modified: Tue, 04 Jul 2023 01:27:59 GMT  
		Size: 29.1 MB (29095988 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8d7739b49a5b122c4236823b37854a86c846e2d64117eddb2df8524eedca55a8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26933710 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f83635f1b06f5e36adaa19d4660899c65e577524f9eb41182ffd0cd74fa1e5a4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:17 GMT
ADD file:656b550b6289c76ca8e65b335d822f01181d7ace77390728151dbf2b5b5c1b2c in / 
# Tue, 04 Jul 2023 00:49:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24028cddddcc82e9355f4a064cdb83e9712b60df2ac37408f0a494554a37d89c`  
		Last Modified: Tue, 04 Jul 2023 00:53:46 GMT  
		Size: 26.9 MB (26933710 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:a5445483dc95a4eb1b1b036a7fb8465806713e063f828aa20374fd189b7348ba
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24742801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a21a91361060cc4379af324f0c5576c67d46ea480d5c1271202e4659bf7920c4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:56 GMT
ADD file:5b470fc68c9bd98b417c76f8a0e5c883f989893a7a5d4489d2564d822b6786a9 in / 
# Tue, 04 Jul 2023 00:59:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e83f279dd8c24d7d6141c04fdbc96addf38612e77227662a88744b7f99beddec`  
		Last Modified: Tue, 04 Jul 2023 01:05:56 GMT  
		Size: 24.7 MB (24742801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c7a2d803db4f1c5d32e1b2b8d959d3fc252d3d9425543d48f1eb285d58cb1ad7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29097291 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2a484449ad431f526162c18c083dcf992b353c1236d0ea294af75bbfd2653422`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:56 GMT
ADD file:c3248f5b00ce46f2049c697c9aa74dd59ef4e415e7d36c386a81b735da30aef8 in / 
# Tue, 04 Jul 2023 01:58:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:66151c8a7c0663c327d2b568a70bbe8b1be13231fd76749fd83bb7a99718f73c`  
		Last Modified: Tue, 04 Jul 2023 02:04:11 GMT  
		Size: 29.1 MB (29097291 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:0c9870b7a3f4d0db8aeab75163e73757effe8af5c4c6643d8b0ccabc4e887148
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30118475 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:782b7df845ad377a60ef0457449ec2e2302a190dc44ba96ab31727845cbd573f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:29 GMT
ADD file:69af2942f3d527cb9dffea34a7af31559c7d6256a93ef79d153694b76a3b16e1 in / 
# Tue, 04 Jul 2023 01:40:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f7038651ba3296c4da4f74f8874c6f609275fcb762a3efccc7998fac335635b7`  
		Last Modified: Tue, 04 Jul 2023 01:46:46 GMT  
		Size: 30.1 MB (30118475 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:b7963d6a6dba5d9521a0a43079e0a63054af9a2f035310848bef140a3ee72b74
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29070950 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:38b1e6779e0fa63b65dc3c001b6cee64b7661f557e63da7d8554359435659de5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:57 GMT
ADD file:11fd9108dc41ea3a02674ae5ddd26cbf495636f01ed76a6bd0e5c335c390d125 in / 
# Tue, 04 Jul 2023 01:13:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0cb1354af6125ba4aed642b5383bd78cca2d442f2f07610b861f4bdbdc14d8c4`  
		Last Modified: Tue, 04 Jul 2023 01:23:44 GMT  
		Size: 29.1 MB (29070950 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:41f2ef23403d301625a8475bf1f81e87c18439e964175ddbfe49d4e935741033
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33087101 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a2f482039788542fdd10ded32fd2c35ccb5e07507d0fa0335519123c87621bc3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:52 GMT
ADD file:cc760f64003e064d558ec3dc9c2bbd9b084a7c9aae376f4633198f4c1109a686 in / 
# Tue, 04 Jul 2023 01:19:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a43337364ea5e70339c4a721d25428ff09d4449f634f33298213de41305b9d13`  
		Last Modified: Tue, 04 Jul 2023 01:27:22 GMT  
		Size: 33.1 MB (33087101 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; riscv64

```console
$ docker pull debian@sha256:a3a43e113492d6646c36667b0696fb931ba38c470956383082567070366992ed
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25314953 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a63142a3a199026120871ba7515f04d5ff3b9c5ca19fb21cca431784c5177412`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:42 GMT
ADD file:f2fbcf6c02be778d097ed6765ed6914786b1a53c290b5c7c2810269959d54f52 in / 
# Tue, 04 Jul 2023 01:09:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e8451a164603a999d21ce87536f889bf4f2145656dcfae1cbb331bf5705b193f`  
		Last Modified: Tue, 04 Jul 2023 01:13:04 GMT  
		Size: 25.3 MB (25314953 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:83fe883cb6aa71745e610ad6680556f47ec1f58632f1eafc8e38c8a371de5d8d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27507851 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d59a71a87c8a580bc94699888411d54cc8776c4811040e04c767f83ce3903c69`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:38 GMT
ADD file:0e2fd05065cb53802e060b997cec02f8acfdda2e928eabae806c0a8d939064aa in / 
# Tue, 04 Jul 2023 01:33:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0f5438d2f555db76c0dac486353eaedce110465f65af064e8f821341674fff41`  
		Last Modified: Tue, 04 Jul 2023 01:38:25 GMT  
		Size: 27.5 MB (27507851 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-slim`

```console
$ docker pull debian@sha256:b257de913a92758b494666a8666b8488ef2fcf73edd6fa8a99a338fbf6338b20
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-slim` - linux; amd64

```console
$ docker pull debian@sha256:0f5968f2f4baa6cdf74a7cfdece3fee99230ac2fc8007ddb427d5d9f4032ee68
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095988 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6762df5b728ea5688a6e5e297ab239d01e88268af7d3e3ca251772f9bb9731a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:59 GMT
ADD file:e74345927b6edb39fe0c4423514889e8695d61e850a899668c64cfceb0ef647f in / 
# Tue, 04 Jul 2023 01:21:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9f5c278176eebbc57aa00f26af876d1bd6ea8252f3662ecef8e4b4be2260ede2`  
		Last Modified: Tue, 04 Jul 2023 01:27:59 GMT  
		Size: 29.1 MB (29095988 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8d7739b49a5b122c4236823b37854a86c846e2d64117eddb2df8524eedca55a8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26933710 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f83635f1b06f5e36adaa19d4660899c65e577524f9eb41182ffd0cd74fa1e5a4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:17 GMT
ADD file:656b550b6289c76ca8e65b335d822f01181d7ace77390728151dbf2b5b5c1b2c in / 
# Tue, 04 Jul 2023 00:49:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24028cddddcc82e9355f4a064cdb83e9712b60df2ac37408f0a494554a37d89c`  
		Last Modified: Tue, 04 Jul 2023 00:53:46 GMT  
		Size: 26.9 MB (26933710 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:a5445483dc95a4eb1b1b036a7fb8465806713e063f828aa20374fd189b7348ba
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24742801 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a21a91361060cc4379af324f0c5576c67d46ea480d5c1271202e4659bf7920c4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:59:56 GMT
ADD file:5b470fc68c9bd98b417c76f8a0e5c883f989893a7a5d4489d2564d822b6786a9 in / 
# Tue, 04 Jul 2023 00:59:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e83f279dd8c24d7d6141c04fdbc96addf38612e77227662a88744b7f99beddec`  
		Last Modified: Tue, 04 Jul 2023 01:05:56 GMT  
		Size: 24.7 MB (24742801 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c7a2d803db4f1c5d32e1b2b8d959d3fc252d3d9425543d48f1eb285d58cb1ad7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29097291 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2a484449ad431f526162c18c083dcf992b353c1236d0ea294af75bbfd2653422`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:58:56 GMT
ADD file:c3248f5b00ce46f2049c697c9aa74dd59ef4e415e7d36c386a81b735da30aef8 in / 
# Tue, 04 Jul 2023 01:58:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:66151c8a7c0663c327d2b568a70bbe8b1be13231fd76749fd83bb7a99718f73c`  
		Last Modified: Tue, 04 Jul 2023 02:04:11 GMT  
		Size: 29.1 MB (29097291 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; 386

```console
$ docker pull debian@sha256:0c9870b7a3f4d0db8aeab75163e73757effe8af5c4c6643d8b0ccabc4e887148
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30118475 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:782b7df845ad377a60ef0457449ec2e2302a190dc44ba96ab31727845cbd573f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:29 GMT
ADD file:69af2942f3d527cb9dffea34a7af31559c7d6256a93ef79d153694b76a3b16e1 in / 
# Tue, 04 Jul 2023 01:40:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f7038651ba3296c4da4f74f8874c6f609275fcb762a3efccc7998fac335635b7`  
		Last Modified: Tue, 04 Jul 2023 01:46:46 GMT  
		Size: 30.1 MB (30118475 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; mips64le

```console
$ docker pull debian@sha256:b7963d6a6dba5d9521a0a43079e0a63054af9a2f035310848bef140a3ee72b74
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29070950 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:38b1e6779e0fa63b65dc3c001b6cee64b7661f557e63da7d8554359435659de5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:12:57 GMT
ADD file:11fd9108dc41ea3a02674ae5ddd26cbf495636f01ed76a6bd0e5c335c390d125 in / 
# Tue, 04 Jul 2023 01:13:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0cb1354af6125ba4aed642b5383bd78cca2d442f2f07610b861f4bdbdc14d8c4`  
		Last Modified: Tue, 04 Jul 2023 01:23:44 GMT  
		Size: 29.1 MB (29070950 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:41f2ef23403d301625a8475bf1f81e87c18439e964175ddbfe49d4e935741033
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33087101 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a2f482039788542fdd10ded32fd2c35ccb5e07507d0fa0335519123c87621bc3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:19:52 GMT
ADD file:cc760f64003e064d558ec3dc9c2bbd9b084a7c9aae376f4633198f4c1109a686 in / 
# Tue, 04 Jul 2023 01:19:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a43337364ea5e70339c4a721d25428ff09d4449f634f33298213de41305b9d13`  
		Last Modified: Tue, 04 Jul 2023 01:27:22 GMT  
		Size: 33.1 MB (33087101 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; riscv64

```console
$ docker pull debian@sha256:a3a43e113492d6646c36667b0696fb931ba38c470956383082567070366992ed
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25314953 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a63142a3a199026120871ba7515f04d5ff3b9c5ca19fb21cca431784c5177412`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:09:42 GMT
ADD file:f2fbcf6c02be778d097ed6765ed6914786b1a53c290b5c7c2810269959d54f52 in / 
# Tue, 04 Jul 2023 01:09:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e8451a164603a999d21ce87536f889bf4f2145656dcfae1cbb331bf5705b193f`  
		Last Modified: Tue, 04 Jul 2023 01:13:04 GMT  
		Size: 25.3 MB (25314953 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; s390x

```console
$ docker pull debian@sha256:83fe883cb6aa71745e610ad6680556f47ec1f58632f1eafc8e38c8a371de5d8d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27507851 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d59a71a87c8a580bc94699888411d54cc8776c4811040e04c767f83ce3903c69`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:38 GMT
ADD file:0e2fd05065cb53802e060b997cec02f8acfdda2e928eabae806c0a8d939064aa in / 
# Tue, 04 Jul 2023 01:33:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0f5438d2f555db76c0dac486353eaedce110465f65af064e8f821341674fff41`  
		Last Modified: Tue, 04 Jul 2023 01:38:25 GMT  
		Size: 27.5 MB (27507851 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable`

```console
$ docker pull debian@sha256:c0d85a08079edd9dca318a67ef6aaf015f916e571390c833572331aeab90bd75
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable` - linux; amd64

```console
$ docker pull debian@sha256:f0b8edb2e4436c556493dce86b941231eead97baebb484d0d5f6ecfe4f7ed193
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551410 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5752abdb2b0fde7b678439cc83166fbd510267f67f0dc3f0fbddead9a6b04c0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:10 GMT
ADD file:2de2d7c41413b3d58634c92893c3e99f0d1464f1ab02785208509245497be121 in / 
# Tue, 04 Jul 2023 01:22:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b5be7a721d800e1ee6428736cfb46f85cb70c4f1a7c06d144f60f929ceaaf5fc`  
		Last Modified: Tue, 04 Jul 2023 01:28:13 GMT  
		Size: 49.6 MB (49551410 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v5

```console
$ docker pull debian@sha256:c64378de8d67d9aef16075f4fcfdb892e52facf13da106ebc2490d928ae960e1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402733 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:87519a00604159fbf6dc659925d691a51412c2c97796cc24a853f7678e1c77fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:24 GMT
ADD file:4560973449ae95ba135b1d2a4749ce4d278066080d92407b8fac46bc17e195db in / 
# Tue, 04 Jul 2023 00:49:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a1441b766c0384e637ebb6bcf7156542cf9e9a87c03d36822c1081d39c369d79`  
		Last Modified: Tue, 04 Jul 2023 00:54:02 GMT  
		Size: 47.4 MB (47402733 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v7

```console
$ docker pull debian@sha256:cf5fd794d8cd27576abe7badea45524410c8245456dbde1a113240d3c324d355
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9b0f6dce9fb809ebc0061c10c48ac49f31c63e4628dc617856531905a6a7407`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:12 GMT
ADD file:79a37c77c46d6158e0050823174255b2fbd19b7b2a6321b195fc725519a2e82b in / 
# Tue, 04 Jul 2023 01:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fbd4a02f7e9523396cfcce42fa09a0e010c6757aac4ee88a5814ba28b86e098`  
		Last Modified: Tue, 04 Jul 2023 01:06:13 GMT  
		Size: 45.2 MB (45236225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f27a374b0e1cb1a5e15d8e6ca7af31d1d719f338f14f1fb14c914f97aef38795
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572742 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11dbfaeeb8395db5bb882cde976864dc7f38b0a7fd32b7cf97e18397453f0089`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:04 GMT
ADD file:79e09e609b23970908767f4254ae6453b2f2cb678a7639912cf4ae8823781aa0 in / 
# Tue, 04 Jul 2023 01:59:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cb8a2ff9f84a753ab1cef3aebe7f6b4f4fe4dd799c343046673687ad1e123b1`  
		Last Modified: Tue, 04 Jul 2023 02:04:24 GMT  
		Size: 49.6 MB (49572742 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; 386

```console
$ docker pull debian@sha256:bec5d72bad648ea5a2b75a7c4e34dacb8bf15995e920bf93ff59801e7cb853d0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562395 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:91a45b20ad6e978e06e66c84a02a462d33b416e6074fa92748549c1b5c539a81`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:40 GMT
ADD file:7b0bf2dc9519a1e02da67dbad17f5970b273f7d8e8616afb763c8f3707d1d4e9 in / 
# Tue, 04 Jul 2023 01:40:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ab8c19d79aca410e867102a952a06f0eff14354ef8cda03aa635a7355236a4e5`  
		Last Modified: Tue, 04 Jul 2023 01:47:03 GMT  
		Size: 50.6 MB (50562395 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; mips64le

```console
$ docker pull debian@sha256:333590af9483bdf4f1fd0f18e08741b0e79ba5b3c864e4cb4bd6f6594a7f755c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542171 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:577a779a63b6fe8b3afbdb607ca351d5e30cabb383f267d4b52702032ed7b59e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:13:31 GMT
ADD file:b3dc9cc8ccd9367e0ad281da5eb70296ffac2f063ef44d86aba57316b35d4cb4 in / 
# Tue, 04 Jul 2023 01:13:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eefef2cdc8981171afeb848988d75520dd7c7b357a56c47f68f8bb38b6b8df54`  
		Last Modified: Tue, 04 Jul 2023 01:24:22 GMT  
		Size: 49.5 MB (49542171 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; ppc64le

```console
$ docker pull debian@sha256:5e2ba741719a69728cf9aaf197e323d2b2665b5701d0829ce5ea4992212136bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:556b9dc1cf380b594edc80bcad0da740792984bf3433eeb174cac55861899898`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:068cd55f1f48b9f120a5ab83c28b77cac6aca0c6d09a83d7d69f35e9d4511d19 in / 
# Tue, 04 Jul 2023 01:20:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0b83f823e9aba68f5128f9af5447ebbcdb46e043ff0e79fca69f4ab36adbf7e0`  
		Last Modified: Tue, 04 Jul 2023 01:27:47 GMT  
		Size: 53.5 MB (53536738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; s390x

```console
$ docker pull debian@sha256:26436f8e83dc16fec03c6de812ed7806ecbdbf046ab70101be71e0b204b0fc56
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921726 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:52d07b2f887b3889c9dbc09e5f084b2d3b2bcc0c5ea7bf373240391b5006e3cb`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:50 GMT
ADD file:9788171ce0f336dccbeeac19aa7fb65ec93cf7774a2893c0532b3ddcadaee9c6 in / 
# Tue, 04 Jul 2023 01:33:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8a02e2c90ded98ee4201c0b941febbd8d4e1ba1a83af3c904c62f9b019abfa9f`  
		Last Modified: Tue, 04 Jul 2023 01:38:36 GMT  
		Size: 47.9 MB (47921726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20230703`

```console
$ docker pull debian@sha256:c0d85a08079edd9dca318a67ef6aaf015f916e571390c833572331aeab90bd75
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20230703` - linux; amd64

```console
$ docker pull debian@sha256:f0b8edb2e4436c556493dce86b941231eead97baebb484d0d5f6ecfe4f7ed193
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551410 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5752abdb2b0fde7b678439cc83166fbd510267f67f0dc3f0fbddead9a6b04c0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:10 GMT
ADD file:2de2d7c41413b3d58634c92893c3e99f0d1464f1ab02785208509245497be121 in / 
# Tue, 04 Jul 2023 01:22:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b5be7a721d800e1ee6428736cfb46f85cb70c4f1a7c06d144f60f929ceaaf5fc`  
		Last Modified: Tue, 04 Jul 2023 01:28:13 GMT  
		Size: 49.6 MB (49551410 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:c64378de8d67d9aef16075f4fcfdb892e52facf13da106ebc2490d928ae960e1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402733 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:87519a00604159fbf6dc659925d691a51412c2c97796cc24a853f7678e1c77fa`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:24 GMT
ADD file:4560973449ae95ba135b1d2a4749ce4d278066080d92407b8fac46bc17e195db in / 
# Tue, 04 Jul 2023 00:49:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a1441b766c0384e637ebb6bcf7156542cf9e9a87c03d36822c1081d39c369d79`  
		Last Modified: Tue, 04 Jul 2023 00:54:02 GMT  
		Size: 47.4 MB (47402733 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:cf5fd794d8cd27576abe7badea45524410c8245456dbde1a113240d3c324d355
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236225 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9b0f6dce9fb809ebc0061c10c48ac49f31c63e4628dc617856531905a6a7407`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:12 GMT
ADD file:79a37c77c46d6158e0050823174255b2fbd19b7b2a6321b195fc725519a2e82b in / 
# Tue, 04 Jul 2023 01:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fbd4a02f7e9523396cfcce42fa09a0e010c6757aac4ee88a5814ba28b86e098`  
		Last Modified: Tue, 04 Jul 2023 01:06:13 GMT  
		Size: 45.2 MB (45236225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f27a374b0e1cb1a5e15d8e6ca7af31d1d719f338f14f1fb14c914f97aef38795
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572742 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11dbfaeeb8395db5bb882cde976864dc7f38b0a7fd32b7cf97e18397453f0089`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:04 GMT
ADD file:79e09e609b23970908767f4254ae6453b2f2cb678a7639912cf4ae8823781aa0 in / 
# Tue, 04 Jul 2023 01:59:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cb8a2ff9f84a753ab1cef3aebe7f6b4f4fe4dd799c343046673687ad1e123b1`  
		Last Modified: Tue, 04 Jul 2023 02:04:24 GMT  
		Size: 49.6 MB (49572742 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; 386

```console
$ docker pull debian@sha256:bec5d72bad648ea5a2b75a7c4e34dacb8bf15995e920bf93ff59801e7cb853d0
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562395 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:91a45b20ad6e978e06e66c84a02a462d33b416e6074fa92748549c1b5c539a81`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:40 GMT
ADD file:7b0bf2dc9519a1e02da67dbad17f5970b273f7d8e8616afb763c8f3707d1d4e9 in / 
# Tue, 04 Jul 2023 01:40:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ab8c19d79aca410e867102a952a06f0eff14354ef8cda03aa635a7355236a4e5`  
		Last Modified: Tue, 04 Jul 2023 01:47:03 GMT  
		Size: 50.6 MB (50562395 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:333590af9483bdf4f1fd0f18e08741b0e79ba5b3c864e4cb4bd6f6594a7f755c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542171 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:577a779a63b6fe8b3afbdb607ca351d5e30cabb383f267d4b52702032ed7b59e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:13:31 GMT
ADD file:b3dc9cc8ccd9367e0ad281da5eb70296ffac2f063ef44d86aba57316b35d4cb4 in / 
# Tue, 04 Jul 2023 01:13:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eefef2cdc8981171afeb848988d75520dd7c7b357a56c47f68f8bb38b6b8df54`  
		Last Modified: Tue, 04 Jul 2023 01:24:22 GMT  
		Size: 49.5 MB (49542171 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:5e2ba741719a69728cf9aaf197e323d2b2665b5701d0829ce5ea4992212136bc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536738 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:556b9dc1cf380b594edc80bcad0da740792984bf3433eeb174cac55861899898`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:068cd55f1f48b9f120a5ab83c28b77cac6aca0c6d09a83d7d69f35e9d4511d19 in / 
# Tue, 04 Jul 2023 01:20:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0b83f823e9aba68f5128f9af5447ebbcdb46e043ff0e79fca69f4ab36adbf7e0`  
		Last Modified: Tue, 04 Jul 2023 01:27:47 GMT  
		Size: 53.5 MB (53536738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703` - linux; s390x

```console
$ docker pull debian@sha256:26436f8e83dc16fec03c6de812ed7806ecbdbf046ab70101be71e0b204b0fc56
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921726 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:52d07b2f887b3889c9dbc09e5f084b2d3b2bcc0c5ea7bf373240391b5006e3cb`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:50 GMT
ADD file:9788171ce0f336dccbeeac19aa7fb65ec93cf7774a2893c0532b3ddcadaee9c6 in / 
# Tue, 04 Jul 2023 01:33:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8a02e2c90ded98ee4201c0b941febbd8d4e1ba1a83af3c904c62f9b019abfa9f`  
		Last Modified: Tue, 04 Jul 2023 01:38:36 GMT  
		Size: 47.9 MB (47921726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20230703-slim`

```console
$ docker pull debian@sha256:bfb338bbc84031a32bb0d4ba61025728ac725968b562707716c0fcc42775d26b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:9071e1878844a004088f3c3f4db1dc54421dac00f15437424533c91c81108469
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124826 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:550e10a6cc336273c25449cf252a5b898520036850a92801afa409e935d2df81`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:24 GMT
ADD file:95a4ddc3a846636c52c273cac4e2e0d75225cde59612235c1480201b14581d9a in / 
# Tue, 04 Jul 2023 01:22:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d77d3f401459c6fad6e1ae2adaa16d8a98f64eb3160eed1394e75c5fa9ac866`  
		Last Modified: Tue, 04 Jul 2023 01:28:31 GMT  
		Size: 29.1 MB (29124826 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6bf6e865be394a1255bb2baea047dd85844b8403374e62bbc8c98597863c8881
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8fe07e8fc3a67d3f387a0e3c5718edfd1cbc0f9fb644ec2e1df5fe8423ba758`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:32 GMT
ADD file:e4919c9818c06e214f730c6f4ac75d78d3672a9b047fc64d62e3fd3032638649 in / 
# Tue, 04 Jul 2023 00:49:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0f366de28ca403569b225b3565bb4aff4f11c00e950f938192192decb9d1ba4b`  
		Last Modified: Tue, 04 Jul 2023 00:54:20 GMT  
		Size: 27.0 MB (26982179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c21ec169d75be0c90409fed3c75e81c23678917e4051262c7fb2a1cc9bfc0191
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801288 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:deed7f4f7b6bc48d52c9aa3fe184adb979c544ca5172a53199253809d31c0cc5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:24 GMT
ADD file:72e6589569be67cdaec528a16610fbc01a3570a71b85f29c6086ddcceed9251e in / 
# Tue, 04 Jul 2023 01:00:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:825fb521b00a79d8916b0d0570f2f0564c52b2b18e1a017f5072e9bad3d0bb0e`  
		Last Modified: Tue, 04 Jul 2023 01:06:34 GMT  
		Size: 24.8 MB (24801288 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f73f45543a3709cb45c168712997954ea805ab774f7160bda7e2b44c0235d012
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152438 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fdbde89f2ae949a03d19c42dda472f3956ed191202fb566e0b6ebd47fd9a4d10`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:12 GMT
ADD file:cd3e62dc01880223fb857ab6ec4cdb803acfd9df0ff5b7d50f1747426e3d8f91 in / 
# Tue, 04 Jul 2023 01:59:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6244b9232be94cf8cbf7648479e9c5a6e5832c22e10d15f7e438b7aabeceb280`  
		Last Modified: Tue, 04 Jul 2023 02:04:42 GMT  
		Size: 29.2 MB (29152438 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:01c5d32b6e4e616628af20f6dfb0aff9eb9ea169418aa633910f71ff95b9696b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:71afe5b6945b5727de7a8b2c9cf9ff221f8bd70f9a2534bc664844a8364442c6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:52 GMT
ADD file:e846f21bfb4fb8481fd5a3dbc52b3ab9d826364f9dfea16f683f6b6a2b30b455 in / 
# Tue, 04 Jul 2023 01:40:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8edcbd9e2e5e80909d91623b51d8f7ca14bef4290cf0438f180a1915e8efbe11`  
		Last Modified: Tue, 04 Jul 2023 01:47:24 GMT  
		Size: 30.1 MB (30140730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:ffabc956fed9253dd86a8082a4d4b8425ad3945975692c4b0149e37350bdac4b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ff69a52ab9162b7251cfeff47839bc31889b8869fcf9bf2b17e573688e27392`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:14:14 GMT
ADD file:db4b142cd0a5e1f591e7b6628af9f9daf66e4b7313cf48eb4775bf5849ab6b87 in / 
# Tue, 04 Jul 2023 01:14:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:77899e0939be5d159267133f41559a7cc65fe238f8560ac44bb8c1a25d13963d`  
		Last Modified: Tue, 04 Jul 2023 01:24:57 GMT  
		Size: 29.1 MB (29118184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:9992a7f3bf00111c871ec90e0d8de4896db58c2fd511b66038ed2e2d0343a4c6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116692 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:317c0e2d15529743365f9a139bdcd8364fc3d9ba1456042fcd4e16642b5af45f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:31 GMT
ADD file:86156e39ddbd48dc437d05ee8b863361e5210c8c016ad1e2a73d9fc044a3a10b in / 
# Tue, 04 Jul 2023 01:20:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7040e23c63251effd5c819571b127f6d423762e7e2688ddd57a62388f3d04d59`  
		Last Modified: Tue, 04 Jul 2023 01:28:15 GMT  
		Size: 33.1 MB (33116692 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:5647e246c60ed34df9e7aff3cb0da9dd0dd668ddf3b8e9a6e2d3002d380653ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487971 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:483fe9dae7a64bf6b6dfa43b86f3c23db2d17802e5cb904f17df0fa288ae403a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:08 GMT
ADD file:a4d8ee7ccffa1d8666399d83a2f1cf1aee2ee0e2d1efb4e1e3d5d1a7a78bb986 in / 
# Tue, 04 Jul 2023 01:34:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6d20ee9220aea72ae6ea08f6740e910bac3000d62c16ec97f42e5aa8327b841`  
		Last Modified: Tue, 04 Jul 2023 01:38:49 GMT  
		Size: 27.5 MB (27487971 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-backports`

```console
$ docker pull debian@sha256:b58027974f91ddc38dd3c9df75cd4509ed813a57fa645a9c54e45d1739885b51
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-backports` - linux; amd64

```console
$ docker pull debian@sha256:ab4b74aa336d446dfd67d592f71b8a5880ca4e91279ccd7c70f20fae0144ac42
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49551632 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78ffbc19732f9a6d6685627ceeef9cba3a8d01d8448f4a7e2896a03db1af77a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:10 GMT
ADD file:2de2d7c41413b3d58634c92893c3e99f0d1464f1ab02785208509245497be121 in / 
# Tue, 04 Jul 2023 01:22:11 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:15 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b5be7a721d800e1ee6428736cfb46f85cb70c4f1a7c06d144f60f929ceaaf5fc`  
		Last Modified: Tue, 04 Jul 2023 01:28:13 GMT  
		Size: 49.6 MB (49551410 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:05229969914fdc6497982179b7fde002dd1b5025f5928b59355b1b7d61de2e88`  
		Last Modified: Tue, 04 Jul 2023 01:28:22 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:babf895d3ea280fe191ecdf7a9932bcd705890639af081a7f569aaf341afbf1d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47402957 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2a2ac8f1bbe838c17afe4be2928b58147acb8ffff5d2749f24d5ab0a5d211217`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:24 GMT
ADD file:4560973449ae95ba135b1d2a4749ce4d278066080d92407b8fac46bc17e195db in / 
# Tue, 04 Jul 2023 00:49:25 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:49:27 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a1441b766c0384e637ebb6bcf7156542cf9e9a87c03d36822c1081d39c369d79`  
		Last Modified: Tue, 04 Jul 2023 00:54:02 GMT  
		Size: 47.4 MB (47402733 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:827eacccd25a8db7c434c06efb03b120ad7f207524d6fe2551274faf715d3b74`  
		Last Modified: Tue, 04 Jul 2023 00:54:11 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:26ae0aaa0539df8eee4df5f60c72cdbb324536d7b7180001f89699cbe8855f0b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45236446 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cc8b5302cd8505b9d6fde743fb043d478cd468be6175f97f074cec44409a3f47`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:12 GMT
ADD file:79a37c77c46d6158e0050823174255b2fbd19b7b2a6321b195fc725519a2e82b in / 
# Tue, 04 Jul 2023 01:00:13 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:00:17 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0fbd4a02f7e9523396cfcce42fa09a0e010c6757aac4ee88a5814ba28b86e098`  
		Last Modified: Tue, 04 Jul 2023 01:06:13 GMT  
		Size: 45.2 MB (45236225 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c724686e9580217c87bac7f99dd29b7e9ff7ec915783bfb903225ef27183802c`  
		Last Modified: Tue, 04 Jul 2023 01:06:24 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:cc6469b6f5189d5f18edf3d7b9f616bf6ea16747c7320df9df8d8949b660cf28
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49572965 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7d235017a1f10a213eb0843f71a980519387cd032a509bbeb00ba91bb09512a6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:04 GMT
ADD file:79e09e609b23970908767f4254ae6453b2f2cb678a7639912cf4ae8823781aa0 in / 
# Tue, 04 Jul 2023 01:59:04 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:59:07 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8cb8a2ff9f84a753ab1cef3aebe7f6b4f4fe4dd799c343046673687ad1e123b1`  
		Last Modified: Tue, 04 Jul 2023 02:04:24 GMT  
		Size: 49.6 MB (49572742 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:408b4bb8dbbbb9bee90e757119f781bb71929f810dba5f6cdae9a13af86c4288`  
		Last Modified: Tue, 04 Jul 2023 02:04:32 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; 386

```console
$ docker pull debian@sha256:06ea90f3196b5a18d1259fae567079fa5b8d7e7f186ade3e792ecd085a65d49a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50562618 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:08564de710239fbd3efcb5f72f8c8efb2ff76d043c045a9c6e3ce51c0e539131`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:40 GMT
ADD file:7b0bf2dc9519a1e02da67dbad17f5970b273f7d8e8616afb763c8f3707d1d4e9 in / 
# Tue, 04 Jul 2023 01:40:41 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:40:44 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ab8c19d79aca410e867102a952a06f0eff14354ef8cda03aa635a7355236a4e5`  
		Last Modified: Tue, 04 Jul 2023 01:47:03 GMT  
		Size: 50.6 MB (50562395 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c38be7543a670d269201c15aa919443e6d4c4b1efe95eb5db14818b29eaccd95`  
		Last Modified: Tue, 04 Jul 2023 01:47:12 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:7daf2a257cbdc97e5d9c32c40839b4205722a3c6db1e52af651aa154e36cfdb2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542395 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:93ed6d8cdb64c939560e5971216daab2b6c184b2b9e8749267eb14164a5df140`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:13:31 GMT
ADD file:b3dc9cc8ccd9367e0ad281da5eb70296ffac2f063ef44d86aba57316b35d4cb4 in / 
# Tue, 04 Jul 2023 01:13:37 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:13:50 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:eefef2cdc8981171afeb848988d75520dd7c7b357a56c47f68f8bb38b6b8df54`  
		Last Modified: Tue, 04 Jul 2023 01:24:22 GMT  
		Size: 49.5 MB (49542171 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2d61d5db650c9c31dcaebf07697eea7a8a16dc977e1a50072a851b3cfe8be01a`  
		Last Modified: Tue, 04 Jul 2023 01:24:32 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:3dbb7864f84410db3d33ad2ee533acbb588124e0cb71938cef6cbdef2042fcd1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53536962 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1d7c4cdacfedc01abbd14f76b626844bbc8b6b0e6f865f35506535e59e5ec581`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:09 GMT
ADD file:068cd55f1f48b9f120a5ab83c28b77cac6aca0c6d09a83d7d69f35e9d4511d19 in / 
# Tue, 04 Jul 2023 01:20:13 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:20:18 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0b83f823e9aba68f5128f9af5447ebbcdb46e043ff0e79fca69f4ab36adbf7e0`  
		Last Modified: Tue, 04 Jul 2023 01:27:47 GMT  
		Size: 53.5 MB (53536738 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9fe7ea9ff408b8f4cbbefd7a104a7d931d80d984e6535163fa5353003ec71ce2`  
		Last Modified: Tue, 04 Jul 2023 01:27:59 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; s390x

```console
$ docker pull debian@sha256:10217786d71e03c38589592e5fe8bc14ca29edd6da25818de3da37846ceb4db3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47921947 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b01a7df9ed46601329ae17b23f5c9ce0ce4275dfd32f751211a76dd4cd36aee`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:33:50 GMT
ADD file:9788171ce0f336dccbeeac19aa7fb65ec93cf7774a2893c0532b3ddcadaee9c6 in / 
# Tue, 04 Jul 2023 01:33:52 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:34:00 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8a02e2c90ded98ee4201c0b941febbd8d4e1ba1a83af3c904c62f9b019abfa9f`  
		Last Modified: Tue, 04 Jul 2023 01:38:36 GMT  
		Size: 47.9 MB (47921726 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0f23ed1db943771efea04e1c1da4614765e2b28822fe90f2ad636bb11410b4e2`  
		Last Modified: Tue, 04 Jul 2023 01:38:42 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-slim`

```console
$ docker pull debian@sha256:bfb338bbc84031a32bb0d4ba61025728ac725968b562707716c0fcc42775d26b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-slim` - linux; amd64

```console
$ docker pull debian@sha256:9071e1878844a004088f3c3f4db1dc54421dac00f15437424533c91c81108469
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29124826 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:550e10a6cc336273c25449cf252a5b898520036850a92801afa409e935d2df81`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:24 GMT
ADD file:95a4ddc3a846636c52c273cac4e2e0d75225cde59612235c1480201b14581d9a in / 
# Tue, 04 Jul 2023 01:22:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d77d3f401459c6fad6e1ae2adaa16d8a98f64eb3160eed1394e75c5fa9ac866`  
		Last Modified: Tue, 04 Jul 2023 01:28:31 GMT  
		Size: 29.1 MB (29124826 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6bf6e865be394a1255bb2baea047dd85844b8403374e62bbc8c98597863c8881
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26982179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8fe07e8fc3a67d3f387a0e3c5718edfd1cbc0f9fb644ec2e1df5fe8423ba758`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:32 GMT
ADD file:e4919c9818c06e214f730c6f4ac75d78d3672a9b047fc64d62e3fd3032638649 in / 
# Tue, 04 Jul 2023 00:49:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0f366de28ca403569b225b3565bb4aff4f11c00e950f938192192decb9d1ba4b`  
		Last Modified: Tue, 04 Jul 2023 00:54:20 GMT  
		Size: 27.0 MB (26982179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c21ec169d75be0c90409fed3c75e81c23678917e4051262c7fb2a1cc9bfc0191
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24801288 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:deed7f4f7b6bc48d52c9aa3fe184adb979c544ca5172a53199253809d31c0cc5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:24 GMT
ADD file:72e6589569be67cdaec528a16610fbc01a3570a71b85f29c6086ddcceed9251e in / 
# Tue, 04 Jul 2023 01:00:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:825fb521b00a79d8916b0d0570f2f0564c52b2b18e1a017f5072e9bad3d0bb0e`  
		Last Modified: Tue, 04 Jul 2023 01:06:34 GMT  
		Size: 24.8 MB (24801288 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f73f45543a3709cb45c168712997954ea805ab774f7160bda7e2b44c0235d012
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29152438 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fdbde89f2ae949a03d19c42dda472f3956ed191202fb566e0b6ebd47fd9a4d10`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:12 GMT
ADD file:cd3e62dc01880223fb857ab6ec4cdb803acfd9df0ff5b7d50f1747426e3d8f91 in / 
# Tue, 04 Jul 2023 01:59:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6244b9232be94cf8cbf7648479e9c5a6e5832c22e10d15f7e438b7aabeceb280`  
		Last Modified: Tue, 04 Jul 2023 02:04:42 GMT  
		Size: 29.2 MB (29152438 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; 386

```console
$ docker pull debian@sha256:01c5d32b6e4e616628af20f6dfb0aff9eb9ea169418aa633910f71ff95b9696b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30140730 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:71afe5b6945b5727de7a8b2c9cf9ff221f8bd70f9a2534bc664844a8364442c6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:40:52 GMT
ADD file:e846f21bfb4fb8481fd5a3dbc52b3ab9d826364f9dfea16f683f6b6a2b30b455 in / 
# Tue, 04 Jul 2023 01:40:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8edcbd9e2e5e80909d91623b51d8f7ca14bef4290cf0438f180a1915e8efbe11`  
		Last Modified: Tue, 04 Jul 2023 01:47:24 GMT  
		Size: 30.1 MB (30140730 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:ffabc956fed9253dd86a8082a4d4b8425ad3945975692c4b0149e37350bdac4b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29118184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ff69a52ab9162b7251cfeff47839bc31889b8869fcf9bf2b17e573688e27392`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:14:14 GMT
ADD file:db4b142cd0a5e1f591e7b6628af9f9daf66e4b7313cf48eb4775bf5849ab6b87 in / 
# Tue, 04 Jul 2023 01:14:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:77899e0939be5d159267133f41559a7cc65fe238f8560ac44bb8c1a25d13963d`  
		Last Modified: Tue, 04 Jul 2023 01:24:57 GMT  
		Size: 29.1 MB (29118184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:9992a7f3bf00111c871ec90e0d8de4896db58c2fd511b66038ed2e2d0343a4c6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33116692 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:317c0e2d15529743365f9a139bdcd8364fc3d9ba1456042fcd4e16642b5af45f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:31 GMT
ADD file:86156e39ddbd48dc437d05ee8b863361e5210c8c016ad1e2a73d9fc044a3a10b in / 
# Tue, 04 Jul 2023 01:20:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7040e23c63251effd5c819571b127f6d423762e7e2688ddd57a62388f3d04d59`  
		Last Modified: Tue, 04 Jul 2023 01:28:15 GMT  
		Size: 33.1 MB (33116692 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; s390x

```console
$ docker pull debian@sha256:5647e246c60ed34df9e7aff3cb0da9dd0dd668ddf3b8e9a6e2d3002d380653ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27487971 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:483fe9dae7a64bf6b6dfa43b86f3c23db2d17802e5cb904f17df0fa288ae403a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:08 GMT
ADD file:a4d8ee7ccffa1d8666399d83a2f1cf1aee2ee0e2d1efb4e1e3d5d1a7a78bb986 in / 
# Tue, 04 Jul 2023 01:34:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6d20ee9220aea72ae6ea08f6740e910bac3000d62c16ec97f42e5aa8327b841`  
		Last Modified: Tue, 04 Jul 2023 01:38:49 GMT  
		Size: 27.5 MB (27487971 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing`

```console
$ docker pull debian@sha256:43599ea130ecd5f424a1f80500c5b9efa1e8043a6e0bef700d87085c437c04bf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:1d9176cfc4a534ff7c68bdbdd1a4af4cc8dcc4fb8130944d79f6ae990d3404ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49523824 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6dc2a17df90421a4847a15be1ecca1fa4462a2b680b925346fc1a507281429e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:36 GMT
ADD file:de5374a9ef9a0a868df3aca8ce7b7d50abbcf762f41fef9c58012f36d97f78ad in / 
# Tue, 04 Jul 2023 01:22:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dfc7daecfaf18ac4d0f8113f6edff692b509a3dfec300e2dc347fc813f7fd887`  
		Last Modified: Tue, 04 Jul 2023 01:28:45 GMT  
		Size: 49.5 MB (49523824 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:046109a5e35b7397237d2e2343585705cc2255a58313bd53b4d1a45a0dca5750
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360077 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:67d5eb0dae1ac8bfd41b9e40fc4310d708fd69099c5ee4bf97c42bc6dbdd4f85`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:41 GMT
ADD file:37d9b5c589e088f6abc6b26d8bf3a37c115e8539418cf66fb14b429429a8e241 in / 
# Tue, 04 Jul 2023 00:49:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dca32106e2136c2ebbdad8bd8bebe2d128536b0b1925018c65c0a100df0c7dea`  
		Last Modified: Tue, 04 Jul 2023 00:54:36 GMT  
		Size: 47.4 MB (47360077 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:4d5e935efacad8f8c191a84f3d24eb877eec0ad06480ba7f6251d73b052e7c75
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45182839 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:102360eb564e7ddfaf0cf00fbe9f091c303cf23bd1b169fc20e95d90d73fb95f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:35 GMT
ADD file:bb6a04281258377dba4024648f4859eb16850c1884664013cf4c68e2f64ee71d in / 
# Tue, 04 Jul 2023 01:00:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2dcfcdb1f06b29ac822690bc8922ffadb1885001e45609e1b1e7c951d8aac132`  
		Last Modified: Tue, 04 Jul 2023 01:06:49 GMT  
		Size: 45.2 MB (45182839 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:585e41f47ab38e8d24bae9289e16e172fb302428d2e7623ada5c4a165afd7d2b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524494 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b4cddc9090721d745e7fc710c60e115dfec75c0e5d53efe3bb2405c70fe13a03`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:20 GMT
ADD file:7c88bfb3f20f1d350105594d828661ca616d541d9efd2337a981f3da607d714a in / 
# Tue, 04 Jul 2023 01:59:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c47644f3190914c8cddc6aa8fbe2992624219125ffe676d8186f7a2492ede06`  
		Last Modified: Tue, 04 Jul 2023 02:04:55 GMT  
		Size: 49.5 MB (49524494 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:369770ae079bb62f9a125743cc27c224b92e252df907e8df35b41ede787bd251
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539391 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f7166c23ddd01c9e1eca83aa0a1846f59e1bd337fcecbc0065ce54b8fc0ebc93`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:04 GMT
ADD file:800f4f1bd5ad2906c772ce8b268570bef9103addd86518d8f307f8237a90353a in / 
# Tue, 04 Jul 2023 01:41:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:87b0c9e90e45a821072212b669ce2d860a42c15267266ee4d5cccc60e216d7b7`  
		Last Modified: Tue, 04 Jul 2023 01:47:40 GMT  
		Size: 50.5 MB (50539391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:98e43603b3355befe074791b57d4f6cad3f211b489ff3d6c5ea9e4c535a7b2db
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49496985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2bbcb0b75e5585304c15d3128b1846f0d5a1b1d65c0b575fe687b56ff90e6f6e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:14:46 GMT
ADD file:dcc9571f3f9ebdedc190ed4e00a06ec3448554688dd0f962e043ee5021f2ac48 in / 
# Tue, 04 Jul 2023 01:14:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3c1f5b710b54bc6dd95bc402e09e8bbba2eb71932091a3431e091fb6d6d3ff09`  
		Last Modified: Tue, 04 Jul 2023 01:25:35 GMT  
		Size: 49.5 MB (49496985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:6f84e8cad2a4910a221a65c895bc85cf050a98495205ca35016531cf778f417a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510470 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e5cbf8b8b2d5152a4dd673dd870e64cc7713507c8bd344a4784ecd2eb974e0c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:51 GMT
ADD file:655c8220697661701581fae778e1d70fbf0e03ef7ef1eb7d69e9fc161234d8ce in / 
# Tue, 04 Jul 2023 01:20:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:becb5953410e743a3e23e594ddf8545a47fd1ff0109bff6b5da8fae334557515`  
		Last Modified: Tue, 04 Jul 2023 01:28:38 GMT  
		Size: 53.5 MB (53510470 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:5e1f55632173e2d133ac74dcb052f477d5b484da1368b81519dc7dad18d198d6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47949790 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:672099fd6939a37b424f81fb52911ffa49261a0f66d4bffe6d27745e33e90e21`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:19 GMT
ADD file:905d0d0e6d4f57fc2258b13e088f9d03967c81aa9affec0d939ac1dc063668f4 in / 
# Tue, 04 Jul 2023 01:34:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:91571ef190c1d77037a9711c8341236d7f9955149bcb264c8c332f20daf0519e`  
		Last Modified: Tue, 04 Jul 2023 01:39:01 GMT  
		Size: 47.9 MB (47949790 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20230703`

```console
$ docker pull debian@sha256:43599ea130ecd5f424a1f80500c5b9efa1e8043a6e0bef700d87085c437c04bf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20230703` - linux; amd64

```console
$ docker pull debian@sha256:1d9176cfc4a534ff7c68bdbdd1a4af4cc8dcc4fb8130944d79f6ae990d3404ce
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49523824 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6dc2a17df90421a4847a15be1ecca1fa4462a2b680b925346fc1a507281429e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:36 GMT
ADD file:de5374a9ef9a0a868df3aca8ce7b7d50abbcf762f41fef9c58012f36d97f78ad in / 
# Tue, 04 Jul 2023 01:22:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dfc7daecfaf18ac4d0f8113f6edff692b509a3dfec300e2dc347fc813f7fd887`  
		Last Modified: Tue, 04 Jul 2023 01:28:45 GMT  
		Size: 49.5 MB (49523824 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:046109a5e35b7397237d2e2343585705cc2255a58313bd53b4d1a45a0dca5750
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360077 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:67d5eb0dae1ac8bfd41b9e40fc4310d708fd69099c5ee4bf97c42bc6dbdd4f85`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:41 GMT
ADD file:37d9b5c589e088f6abc6b26d8bf3a37c115e8539418cf66fb14b429429a8e241 in / 
# Tue, 04 Jul 2023 00:49:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dca32106e2136c2ebbdad8bd8bebe2d128536b0b1925018c65c0a100df0c7dea`  
		Last Modified: Tue, 04 Jul 2023 00:54:36 GMT  
		Size: 47.4 MB (47360077 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:4d5e935efacad8f8c191a84f3d24eb877eec0ad06480ba7f6251d73b052e7c75
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45182839 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:102360eb564e7ddfaf0cf00fbe9f091c303cf23bd1b169fc20e95d90d73fb95f`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:35 GMT
ADD file:bb6a04281258377dba4024648f4859eb16850c1884664013cf4c68e2f64ee71d in / 
# Tue, 04 Jul 2023 01:00:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2dcfcdb1f06b29ac822690bc8922ffadb1885001e45609e1b1e7c951d8aac132`  
		Last Modified: Tue, 04 Jul 2023 01:06:49 GMT  
		Size: 45.2 MB (45182839 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:585e41f47ab38e8d24bae9289e16e172fb302428d2e7623ada5c4a165afd7d2b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524494 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b4cddc9090721d745e7fc710c60e115dfec75c0e5d53efe3bb2405c70fe13a03`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:20 GMT
ADD file:7c88bfb3f20f1d350105594d828661ca616d541d9efd2337a981f3da607d714a in / 
# Tue, 04 Jul 2023 01:59:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6c47644f3190914c8cddc6aa8fbe2992624219125ffe676d8186f7a2492ede06`  
		Last Modified: Tue, 04 Jul 2023 02:04:55 GMT  
		Size: 49.5 MB (49524494 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; 386

```console
$ docker pull debian@sha256:369770ae079bb62f9a125743cc27c224b92e252df907e8df35b41ede787bd251
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539391 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f7166c23ddd01c9e1eca83aa0a1846f59e1bd337fcecbc0065ce54b8fc0ebc93`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:04 GMT
ADD file:800f4f1bd5ad2906c772ce8b268570bef9103addd86518d8f307f8237a90353a in / 
# Tue, 04 Jul 2023 01:41:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:87b0c9e90e45a821072212b669ce2d860a42c15267266ee4d5cccc60e216d7b7`  
		Last Modified: Tue, 04 Jul 2023 01:47:40 GMT  
		Size: 50.5 MB (50539391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:98e43603b3355befe074791b57d4f6cad3f211b489ff3d6c5ea9e4c535a7b2db
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49496985 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2bbcb0b75e5585304c15d3128b1846f0d5a1b1d65c0b575fe687b56ff90e6f6e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:14:46 GMT
ADD file:dcc9571f3f9ebdedc190ed4e00a06ec3448554688dd0f962e043ee5021f2ac48 in / 
# Tue, 04 Jul 2023 01:14:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3c1f5b710b54bc6dd95bc402e09e8bbba2eb71932091a3431e091fb6d6d3ff09`  
		Last Modified: Tue, 04 Jul 2023 01:25:35 GMT  
		Size: 49.5 MB (49496985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:6f84e8cad2a4910a221a65c895bc85cf050a98495205ca35016531cf778f417a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510470 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e5cbf8b8b2d5152a4dd673dd870e64cc7713507c8bd344a4784ecd2eb974e0c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:51 GMT
ADD file:655c8220697661701581fae778e1d70fbf0e03ef7ef1eb7d69e9fc161234d8ce in / 
# Tue, 04 Jul 2023 01:20:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:becb5953410e743a3e23e594ddf8545a47fd1ff0109bff6b5da8fae334557515`  
		Last Modified: Tue, 04 Jul 2023 01:28:38 GMT  
		Size: 53.5 MB (53510470 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703` - linux; s390x

```console
$ docker pull debian@sha256:5e1f55632173e2d133ac74dcb052f477d5b484da1368b81519dc7dad18d198d6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47949790 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:672099fd6939a37b424f81fb52911ffa49261a0f66d4bffe6d27745e33e90e21`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:19 GMT
ADD file:905d0d0e6d4f57fc2258b13e088f9d03967c81aa9affec0d939ac1dc063668f4 in / 
# Tue, 04 Jul 2023 01:34:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:91571ef190c1d77037a9711c8341236d7f9955149bcb264c8c332f20daf0519e`  
		Last Modified: Tue, 04 Jul 2023 01:39:01 GMT  
		Size: 47.9 MB (47949790 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20230703-slim`

```console
$ docker pull debian@sha256:cbda229bd9d9c676f9a9e8386bef8157ff1e58bbc2520ee87283978fcd5cc384
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:a26fcbd3acb060c9b6ac87b3b9685b3840c3546344a42960a6be581c8d8abf9d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29083901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c801bb633d5e7d11a755f2e835e978546b865c8b05692602b68d576572bc8338`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:50 GMT
ADD file:148e30fd0b7426d111a2c43d0fdfe66516e8802b247ceb8d2c37853d3e5094e6 in / 
# Tue, 04 Jul 2023 01:22:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4caeadce2d32bbc31e3848526a0ece9a1c849b347b9399882701c86e8d0540d2`  
		Last Modified: Tue, 04 Jul 2023 01:29:03 GMT  
		Size: 29.1 MB (29083901 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8f62ff5478e721e94fe77b09052f6b469c3ac40860345efa3012e647c500ef47
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26930329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0952939abe94b1687b69da6608cca7709e0f9fe09d40207e2246786105312c16`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:50 GMT
ADD file:4098c55ccaed91df58658e4466254ce59829057c2fe0b8868fcde7ef0c1d5f9f in / 
# Tue, 04 Jul 2023 00:49:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba3f896b85ffed9c9bc03b33940f9110b354cb818ba401f65cdfe92cf678b59e`  
		Last Modified: Tue, 04 Jul 2023 00:54:56 GMT  
		Size: 26.9 MB (26930329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:2dcf570e1aa74e3f3f21f65247f954772cb608886e935d1ce46b872ea0962201
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24736888 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:00c7db39c33a37effcd1fee09ec3ad4381804616635b152b849b730b1efb6d28`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:49 GMT
ADD file:1684521f3f7d5362297dfdf5eba2b697f97cc3761a9a5a40926da2ca2d25aaed in / 
# Tue, 04 Jul 2023 01:00:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:68557ba5459efc702392b669e78c2d5c014ba445d974fa41d75444a76bb6f954`  
		Last Modified: Tue, 04 Jul 2023 01:07:09 GMT  
		Size: 24.7 MB (24736888 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e75a6520408c29016c6ecabcf61cb977e65cf2bdb255702481fc78b389f7eab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5cc0c48b677eb20634d849986008ec195ea75650c1c1cd0875e731069d929e1c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:29 GMT
ADD file:a3e3eb7d79b7701aa1d3bc9b83c08c265a4c3a2498f3a91c8b516159fe5ca08b in / 
# Tue, 04 Jul 2023 01:59:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:853c1f19ba2d9a97041bf110976be65d8f5bd703ef6a3f922b093bbd625b9b57`  
		Last Modified: Tue, 04 Jul 2023 02:05:12 GMT  
		Size: 29.1 MB (29095143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:1183a15c8fb87ebd945583aeb5bb239f984bd652cc37a6a53b89c49b051854d2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30110805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea82aa04e74f2c79b8dbf05268e8dc624cdcd09065ded79758bf7019a4f6e5a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:15 GMT
ADD file:5f8cdc6409fb125db6ff3c7b7e407081a308c6e007e923375d28933123224aa1 in / 
# Tue, 04 Jul 2023 01:41:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d15fc372bc5e49103fe42fe33f5e14f8f97ea302fe6d20b621e36f8dc9658650`  
		Last Modified: Tue, 04 Jul 2023 01:47:59 GMT  
		Size: 30.1 MB (30110805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:891643d05d5a91241269c1aad21a89200fa8cf61788e64999e7527f2a3660a6b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29061940 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e466186ca50dc0caa3d8d363dc6d13920b3fe4aaa7052655d1f8504f1782d8e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:15:21 GMT
ADD file:fa728a2d74e3f59ed7775ea288ba83c3850dd69782ae1381d6727907e6ebba2a in / 
# Tue, 04 Jul 2023 01:15:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b16c19b449f09c60b1f0b798657a756c4a2ab6f043b188fe4bfc56aaf9521da3`  
		Last Modified: Tue, 04 Jul 2023 01:26:09 GMT  
		Size: 29.1 MB (29061940 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:6688c08c1b4790dc74037ab51105d0737f42a25da9a3a049079f6754e0eaa2fc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33076699 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65f808f262bc882b476aa772afe0f6aa070ded72df1e96e18c50ff7125bdb318`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:13 GMT
ADD file:a10e15ca7b767bf0c29453a2ab8f29dbc9bf381d4c28d6f42c27898755525ce5 in / 
# Tue, 04 Jul 2023 01:21:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:076f30df4b8b68c6e711f590c5f76befaa0a2452606c383408f7b819699ba547`  
		Last Modified: Tue, 04 Jul 2023 01:29:06 GMT  
		Size: 33.1 MB (33076699 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:f052ee2ed4c3b213ced76f6ffe7071d12f6a72e95760f58ac1a0347c243b2f73
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27506191 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b5fa73551b37ef328bd87efc53e6fe402d487af2664d5b72d20bcd5804aa3133`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:40 GMT
ADD file:458e7599a9782672d3ecac58fc7acafa0699a3f2a026b6ee51805f28b19d10c9 in / 
# Tue, 04 Jul 2023 01:34:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:92a374c1e05d1dd0a35bda545f7a8f1de30757d2528523a21b56156cabd32f16`  
		Last Modified: Tue, 04 Jul 2023 01:39:14 GMT  
		Size: 27.5 MB (27506191 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-backports`

```console
$ docker pull debian@sha256:75fc784b19ee6b137e8c8cc7280c34f3951cfd138cc0d6222f528fd696b24609
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-backports` - linux; amd64

```console
$ docker pull debian@sha256:dac77ef4aae9f9f109a76dcb3cfa5595e759fb529606c7ab619af0e85e638482
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524045 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec6093b45f2296271d9d9b360f460bfa69f54cf76e4ee59edfd8cd8f05357f6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:36 GMT
ADD file:de5374a9ef9a0a868df3aca8ce7b7d50abbcf762f41fef9c58012f36d97f78ad in / 
# Tue, 04 Jul 2023 01:22:36 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:22:41 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:dfc7daecfaf18ac4d0f8113f6edff692b509a3dfec300e2dc347fc813f7fd887`  
		Last Modified: Tue, 04 Jul 2023 01:28:45 GMT  
		Size: 49.5 MB (49523824 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5e9fed4b63c8938203bf3163a65164dedb2fcb2489973aafcaf6b0dbdc8b3394`  
		Last Modified: Tue, 04 Jul 2023 01:28:53 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:b2352720303c39a846d93cce5b5f0682c2c8ba26a65aa76707a1297e729a548e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360298 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:72ea8f8432e5742e56b05e6c6e5ec85af71be2862afc4793bd16a3a9c32f5a3c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:41 GMT
ADD file:37d9b5c589e088f6abc6b26d8bf3a37c115e8539418cf66fb14b429429a8e241 in / 
# Tue, 04 Jul 2023 00:49:42 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:49:45 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:dca32106e2136c2ebbdad8bd8bebe2d128536b0b1925018c65c0a100df0c7dea`  
		Last Modified: Tue, 04 Jul 2023 00:54:36 GMT  
		Size: 47.4 MB (47360077 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4e3cb3d074c15c2f15486611dfc3adf08691b42ab68c316ae42f2b6987bb0a8a`  
		Last Modified: Tue, 04 Jul 2023 00:54:45 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:6c16a88dca2d739fca98e1ed2e9a8fa410aee03e760669231a771e40b2cc44b1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45183060 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f1916f5b52c41748cbe1b8296ab771f892a0ed4db5c3881dabaf00ececcbfce9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:35 GMT
ADD file:bb6a04281258377dba4024648f4859eb16850c1884664013cf4c68e2f64ee71d in / 
# Tue, 04 Jul 2023 01:00:37 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:00:41 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2dcfcdb1f06b29ac822690bc8922ffadb1885001e45609e1b1e7c951d8aac132`  
		Last Modified: Tue, 04 Jul 2023 01:06:49 GMT  
		Size: 45.2 MB (45182839 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:adab0aa5ce5beff54e97f4a0c85cd86c0fb45400fce6edbe1c7267c554e95463`  
		Last Modified: Tue, 04 Jul 2023 01:06:59 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ef3100ef4124e8d0802130ea08b95c1b661f0ccd40295dc3491758306c7e184e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524715 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e96ade264ad6ef6a74e49b0e168125e0815824fed5d4ebefed655487abbf9f5d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:20 GMT
ADD file:7c88bfb3f20f1d350105594d828661ca616d541d9efd2337a981f3da607d714a in / 
# Tue, 04 Jul 2023 01:59:21 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:59:23 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6c47644f3190914c8cddc6aa8fbe2992624219125ffe676d8186f7a2492ede06`  
		Last Modified: Tue, 04 Jul 2023 02:04:55 GMT  
		Size: 49.5 MB (49524494 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bcbb618a03e1c123aeaa5b98f535fb6296e87cbdd574f7b331998188fd4909b8`  
		Last Modified: Tue, 04 Jul 2023 02:05:03 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; 386

```console
$ docker pull debian@sha256:6a099eed248af642536848d0f579520cb1c08c956cb5d7ca54c0c3be8f5e0285
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539612 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:29bbc6ee51b7181b1c4bdd1cf827ee85b4b8ee1a97461be7e2d39b8e9cc597d5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:04 GMT
ADD file:800f4f1bd5ad2906c772ce8b268570bef9103addd86518d8f307f8237a90353a in / 
# Tue, 04 Jul 2023 01:41:04 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:41:07 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:87b0c9e90e45a821072212b669ce2d860a42c15267266ee4d5cccc60e216d7b7`  
		Last Modified: Tue, 04 Jul 2023 01:47:40 GMT  
		Size: 50.5 MB (50539391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6004532d2d9d8cb23425e74d0c9341c5a8d65799f75b7daa853e74d98460b222`  
		Last Modified: Tue, 04 Jul 2023 01:47:48 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; mips64le

```console
$ docker pull debian@sha256:ef1d942f7ae0ab03159830456013fc125b9360020f426a1e6b7bef6f34183463
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49497209 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2b2d2ca38aba81c88c4ade90bfdddd3d45d25e526d6fc074fdbbcf94ca28d302`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:14:46 GMT
ADD file:dcc9571f3f9ebdedc190ed4e00a06ec3448554688dd0f962e043ee5021f2ac48 in / 
# Tue, 04 Jul 2023 01:14:50 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:14:59 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3c1f5b710b54bc6dd95bc402e09e8bbba2eb71932091a3431e091fb6d6d3ff09`  
		Last Modified: Tue, 04 Jul 2023 01:25:35 GMT  
		Size: 49.5 MB (49496985 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9394a6bbf60dc3d29dae2c4844ba91a7177b0b8521c28f3f3651ac20799f1b82`  
		Last Modified: Tue, 04 Jul 2023 01:25:45 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:30944a09c2e687aac8795292591ca6d52ea170dc39c3e1d1e2c23e75c9e02af2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510693 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53bdeb8bae017d5fd68d82ade20d7a11b73c5169354e477e135509852d77a4e5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:20:51 GMT
ADD file:655c8220697661701581fae778e1d70fbf0e03ef7ef1eb7d69e9fc161234d8ce in / 
# Tue, 04 Jul 2023 01:20:55 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:21:00 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:becb5953410e743a3e23e594ddf8545a47fd1ff0109bff6b5da8fae334557515`  
		Last Modified: Tue, 04 Jul 2023 01:28:38 GMT  
		Size: 53.5 MB (53510470 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1faffff9d1c0247ac92887572b457d8ab4467225894f18e64d4003bfd206006e`  
		Last Modified: Tue, 04 Jul 2023 01:28:50 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; s390x

```console
$ docker pull debian@sha256:30578f954d75117d6c735a90e2c7bf3aba614fad8761c7a290930b690d740b80
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.0 MB (47950010 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:245f0396152c7f548c48c1dee9d35f4d1071f8ce5cd6066e4ee5c636f60450c8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:19 GMT
ADD file:905d0d0e6d4f57fc2258b13e088f9d03967c81aa9affec0d939ac1dc063668f4 in / 
# Tue, 04 Jul 2023 01:34:22 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:34:30 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:91571ef190c1d77037a9711c8341236d7f9955149bcb264c8c332f20daf0519e`  
		Last Modified: Tue, 04 Jul 2023 01:39:01 GMT  
		Size: 47.9 MB (47949790 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:63aee5761630d0c58da6ea8114d07db3d1a2024787d734933d1fc5c16fc6d227`  
		Last Modified: Tue, 04 Jul 2023 01:39:07 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-slim`

```console
$ docker pull debian@sha256:cbda229bd9d9c676f9a9e8386bef8157ff1e58bbc2520ee87283978fcd5cc384
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-slim` - linux; amd64

```console
$ docker pull debian@sha256:a26fcbd3acb060c9b6ac87b3b9685b3840c3546344a42960a6be581c8d8abf9d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29083901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c801bb633d5e7d11a755f2e835e978546b865c8b05692602b68d576572bc8338`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:50 GMT
ADD file:148e30fd0b7426d111a2c43d0fdfe66516e8802b247ceb8d2c37853d3e5094e6 in / 
# Tue, 04 Jul 2023 01:22:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4caeadce2d32bbc31e3848526a0ece9a1c849b347b9399882701c86e8d0540d2`  
		Last Modified: Tue, 04 Jul 2023 01:29:03 GMT  
		Size: 29.1 MB (29083901 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8f62ff5478e721e94fe77b09052f6b469c3ac40860345efa3012e647c500ef47
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26930329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0952939abe94b1687b69da6608cca7709e0f9fe09d40207e2246786105312c16`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:50 GMT
ADD file:4098c55ccaed91df58658e4466254ce59829057c2fe0b8868fcde7ef0c1d5f9f in / 
# Tue, 04 Jul 2023 00:49:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba3f896b85ffed9c9bc03b33940f9110b354cb818ba401f65cdfe92cf678b59e`  
		Last Modified: Tue, 04 Jul 2023 00:54:56 GMT  
		Size: 26.9 MB (26930329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:2dcf570e1aa74e3f3f21f65247f954772cb608886e935d1ce46b872ea0962201
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24736888 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:00c7db39c33a37effcd1fee09ec3ad4381804616635b152b849b730b1efb6d28`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:49 GMT
ADD file:1684521f3f7d5362297dfdf5eba2b697f97cc3761a9a5a40926da2ca2d25aaed in / 
# Tue, 04 Jul 2023 01:00:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:68557ba5459efc702392b669e78c2d5c014ba445d974fa41d75444a76bb6f954`  
		Last Modified: Tue, 04 Jul 2023 01:07:09 GMT  
		Size: 24.7 MB (24736888 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e75a6520408c29016c6ecabcf61cb977e65cf2bdb255702481fc78b389f7eab
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095143 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5cc0c48b677eb20634d849986008ec195ea75650c1c1cd0875e731069d929e1c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:29 GMT
ADD file:a3e3eb7d79b7701aa1d3bc9b83c08c265a4c3a2498f3a91c8b516159fe5ca08b in / 
# Tue, 04 Jul 2023 01:59:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:853c1f19ba2d9a97041bf110976be65d8f5bd703ef6a3f922b093bbd625b9b57`  
		Last Modified: Tue, 04 Jul 2023 02:05:12 GMT  
		Size: 29.1 MB (29095143 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; 386

```console
$ docker pull debian@sha256:1183a15c8fb87ebd945583aeb5bb239f984bd652cc37a6a53b89c49b051854d2
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30110805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ea82aa04e74f2c79b8dbf05268e8dc624cdcd09065ded79758bf7019a4f6e5a1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:15 GMT
ADD file:5f8cdc6409fb125db6ff3c7b7e407081a308c6e007e923375d28933123224aa1 in / 
# Tue, 04 Jul 2023 01:41:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d15fc372bc5e49103fe42fe33f5e14f8f97ea302fe6d20b621e36f8dc9658650`  
		Last Modified: Tue, 04 Jul 2023 01:47:59 GMT  
		Size: 30.1 MB (30110805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; mips64le

```console
$ docker pull debian@sha256:891643d05d5a91241269c1aad21a89200fa8cf61788e64999e7527f2a3660a6b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29061940 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e466186ca50dc0caa3d8d363dc6d13920b3fe4aaa7052655d1f8504f1782d8e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:15:21 GMT
ADD file:fa728a2d74e3f59ed7775ea288ba83c3850dd69782ae1381d6727907e6ebba2a in / 
# Tue, 04 Jul 2023 01:15:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b16c19b449f09c60b1f0b798657a756c4a2ab6f043b188fe4bfc56aaf9521da3`  
		Last Modified: Tue, 04 Jul 2023 01:26:09 GMT  
		Size: 29.1 MB (29061940 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:6688c08c1b4790dc74037ab51105d0737f42a25da9a3a049079f6754e0eaa2fc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33076699 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65f808f262bc882b476aa772afe0f6aa070ded72df1e96e18c50ff7125bdb318`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:13 GMT
ADD file:a10e15ca7b767bf0c29453a2ab8f29dbc9bf381d4c28d6f42c27898755525ce5 in / 
# Tue, 04 Jul 2023 01:21:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:076f30df4b8b68c6e711f590c5f76befaa0a2452606c383408f7b819699ba547`  
		Last Modified: Tue, 04 Jul 2023 01:29:06 GMT  
		Size: 33.1 MB (33076699 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; s390x

```console
$ docker pull debian@sha256:f052ee2ed4c3b213ced76f6ffe7071d12f6a72e95760f58ac1a0347c243b2f73
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27506191 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b5fa73551b37ef328bd87efc53e6fe402d487af2664d5b72d20bcd5804aa3133`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:40 GMT
ADD file:458e7599a9782672d3ecac58fc7acafa0699a3f2a026b6ee51805f28b19d10c9 in / 
# Tue, 04 Jul 2023 01:34:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:92a374c1e05d1dd0a35bda545f7a8f1de30757d2528523a21b56156cabd32f16`  
		Last Modified: Tue, 04 Jul 2023 01:39:14 GMT  
		Size: 27.5 MB (27506191 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie`

```console
$ docker pull debian@sha256:2b0b82571047dcda91d6ffe7fb92c49e512da19a3e9b3e0d1159884667a418ab
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:trixie` - linux; amd64

```console
$ docker pull debian@sha256:52ef66c5ba5418242998a69f535b16c920521524ee9a19e0f0cbbdc4720a89b1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49523846 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af227ed3112f258fbdd6a0e6bb6b942efa086ccc63006ea97afb9b9ecd6122af`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:01 GMT
ADD file:69ddb6bf8a54cc4c13fa6dfc75c953d997384f31131296eba0fe4670ab3281bb in / 
# Tue, 04 Jul 2023 01:23:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7b026d231b019b0ddfd89687759b2889cdfa3c816e7bb7bff4316db584f02cb3`  
		Last Modified: Tue, 04 Jul 2023 01:29:17 GMT  
		Size: 49.5 MB (49523846 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm variant v5

```console
$ docker pull debian@sha256:578e1fcc7f9a1e2d24a813fd995ce39b3f60bace96e284074afb29660458d8fe
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360068 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c2324f07fff520073e5ebd945fbecd665d1f6fd38e821a30325c83eb802adc48`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:59 GMT
ADD file:0ffd95af69945bc7c3ba2733bffd453f61419b678dcc1b0bf13261d9a2d147f7 in / 
# Tue, 04 Jul 2023 00:49:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:059b0228783b958bcd75527dc7506c02509e9b3a009b9e7f5b944545d6d86929`  
		Last Modified: Tue, 04 Jul 2023 00:55:11 GMT  
		Size: 47.4 MB (47360068 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm variant v7

```console
$ docker pull debian@sha256:2fcf3d318417cebc0938155ad92b3d17564c4687674cda3b820de2b3be435f0c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45182820 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:070c07920f5742adcc92d329f9c9cfc7b6178c9f8adf4cfea97792db4a39bfd0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:59 GMT
ADD file:d1d870c89e7d21b3459cde2dabaf0d8fbc038324334dfb59648191201c0881a6 in / 
# Tue, 04 Jul 2023 01:00:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:de599662316fbbb0e5a107c34fc84f0bd6cce1eae2febdee5f9a166bcbe6f898`  
		Last Modified: Tue, 04 Jul 2023 01:07:24 GMT  
		Size: 45.2 MB (45182820 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:17e7312995fec6b1f93efe9198152fe8b7b4ea5431fb74b9c779ff321f123d23
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524491 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2381580c83b3b661fc6ff4f946ff0fa5f99f31d3802ad2336fbfd8246aa34dca`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:36 GMT
ADD file:e83c96cb391bb6f4a4236baff0c8be47ff4ea09826f95481497333dddc3537ea in / 
# Tue, 04 Jul 2023 01:59:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d728e68b504cba85127d7d3e5493d24223ae3af314b4e73d6b74bdebf7911f05`  
		Last Modified: Tue, 04 Jul 2023 02:05:27 GMT  
		Size: 49.5 MB (49524491 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; 386

```console
$ docker pull debian@sha256:cc57a2ad9e17f5b3467c79fead68375494044d7f82880f8be59b59626e993964
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539409 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bebd9a94548ed4e16626e3fd624aec4881bc04c73fea9d71c91322655b1a4bde`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:27 GMT
ADD file:034bf0762755f9dc5cb0cea6768f68025634eda8045c112ef95777d21df6ecf3 in / 
# Tue, 04 Jul 2023 01:41:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb3d19d1083945d3c3a14221887db9fd1e087f333da06937d0ec9bfe0c79e18`  
		Last Modified: Tue, 04 Jul 2023 01:48:17 GMT  
		Size: 50.5 MB (50539409 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; mips64le

```console
$ docker pull debian@sha256:c0743a46f445a30189704f274fde97c17f8deda554570feaf15bcf69362757cc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49496987 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ae4f0ac032ffa3c306d89c3c66379f31a67d128b59bdeff408cfb86cc815e1d1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:15:54 GMT
ADD file:ae6c6291352d35c9053a1b89fa15bd2eb76add307e5059d83474f17b2e138b31 in / 
# Tue, 04 Jul 2023 01:15:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:67e71ca2fad81efa04d26e36aee19d65985a3b0a387729559324ca4e38246677`  
		Last Modified: Tue, 04 Jul 2023 01:26:47 GMT  
		Size: 49.5 MB (49496987 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; ppc64le

```console
$ docker pull debian@sha256:08f7c6c26af7129b07626a4188c7da1c3070a43f1614c955c506211bad9bd9f8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a21b9cc7e6bef17a2b58cfd4fce725c4fe88d0ea922964cdd5963febd4831200`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:33 GMT
ADD file:0ceaf5be903c5d350c3e3aa5ba446b3e6ef07b387c44d4fcfe5a4ff0d3c9dafc in / 
# Tue, 04 Jul 2023 01:21:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a29695f02132f5ced01dc6d2db95e88b5cc475771c1b689b45229c55d07594e`  
		Last Modified: Tue, 04 Jul 2023 01:29:35 GMT  
		Size: 53.5 MB (53510478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; s390x

```console
$ docker pull debian@sha256:6379fa32f8afe9fffb987bc0e248f64dfadd2546baf3350de9b8aa7ef40c46bb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47949785 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37a61100328c8f55a68ae64aa592b0a751f1ce0775e2c874441bffe612464194`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:52 GMT
ADD file:9ec04d3ce271a93cb5627cb0001517d58e07016d24e0e80cbdcb8bda54e69f8d in / 
# Tue, 04 Jul 2023 01:34:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56cf8724500ec05fe7063f36d4625803ce264bb1a82d2faaabba9d1be0008b8b`  
		Last Modified: Tue, 04 Jul 2023 01:39:27 GMT  
		Size: 47.9 MB (47949785 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-20230703`

```console
$ docker pull debian@sha256:2b0b82571047dcda91d6ffe7fb92c49e512da19a3e9b3e0d1159884667a418ab
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:trixie-20230703` - linux; amd64

```console
$ docker pull debian@sha256:52ef66c5ba5418242998a69f535b16c920521524ee9a19e0f0cbbdc4720a89b1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49523846 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af227ed3112f258fbdd6a0e6bb6b942efa086ccc63006ea97afb9b9ecd6122af`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:01 GMT
ADD file:69ddb6bf8a54cc4c13fa6dfc75c953d997384f31131296eba0fe4670ab3281bb in / 
# Tue, 04 Jul 2023 01:23:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7b026d231b019b0ddfd89687759b2889cdfa3c816e7bb7bff4316db584f02cb3`  
		Last Modified: Tue, 04 Jul 2023 01:29:17 GMT  
		Size: 49.5 MB (49523846 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:578e1fcc7f9a1e2d24a813fd995ce39b3f60bace96e284074afb29660458d8fe
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360068 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c2324f07fff520073e5ebd945fbecd665d1f6fd38e821a30325c83eb802adc48`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:59 GMT
ADD file:0ffd95af69945bc7c3ba2733bffd453f61419b678dcc1b0bf13261d9a2d147f7 in / 
# Tue, 04 Jul 2023 00:49:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:059b0228783b958bcd75527dc7506c02509e9b3a009b9e7f5b944545d6d86929`  
		Last Modified: Tue, 04 Jul 2023 00:55:11 GMT  
		Size: 47.4 MB (47360068 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:2fcf3d318417cebc0938155ad92b3d17564c4687674cda3b820de2b3be435f0c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45182820 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:070c07920f5742adcc92d329f9c9cfc7b6178c9f8adf4cfea97792db4a39bfd0`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:59 GMT
ADD file:d1d870c89e7d21b3459cde2dabaf0d8fbc038324334dfb59648191201c0881a6 in / 
# Tue, 04 Jul 2023 01:00:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:de599662316fbbb0e5a107c34fc84f0bd6cce1eae2febdee5f9a166bcbe6f898`  
		Last Modified: Tue, 04 Jul 2023 01:07:24 GMT  
		Size: 45.2 MB (45182820 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:17e7312995fec6b1f93efe9198152fe8b7b4ea5431fb74b9c779ff321f123d23
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524491 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2381580c83b3b661fc6ff4f946ff0fa5f99f31d3802ad2336fbfd8246aa34dca`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:36 GMT
ADD file:e83c96cb391bb6f4a4236baff0c8be47ff4ea09826f95481497333dddc3537ea in / 
# Tue, 04 Jul 2023 01:59:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d728e68b504cba85127d7d3e5493d24223ae3af314b4e73d6b74bdebf7911f05`  
		Last Modified: Tue, 04 Jul 2023 02:05:27 GMT  
		Size: 49.5 MB (49524491 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; 386

```console
$ docker pull debian@sha256:cc57a2ad9e17f5b3467c79fead68375494044d7f82880f8be59b59626e993964
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539409 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bebd9a94548ed4e16626e3fd624aec4881bc04c73fea9d71c91322655b1a4bde`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:27 GMT
ADD file:034bf0762755f9dc5cb0cea6768f68025634eda8045c112ef95777d21df6ecf3 in / 
# Tue, 04 Jul 2023 01:41:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:efb3d19d1083945d3c3a14221887db9fd1e087f333da06937d0ec9bfe0c79e18`  
		Last Modified: Tue, 04 Jul 2023 01:48:17 GMT  
		Size: 50.5 MB (50539409 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:c0743a46f445a30189704f274fde97c17f8deda554570feaf15bcf69362757cc
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49496987 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ae4f0ac032ffa3c306d89c3c66379f31a67d128b59bdeff408cfb86cc815e1d1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:15:54 GMT
ADD file:ae6c6291352d35c9053a1b89fa15bd2eb76add307e5059d83474f17b2e138b31 in / 
# Tue, 04 Jul 2023 01:15:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:67e71ca2fad81efa04d26e36aee19d65985a3b0a387729559324ca4e38246677`  
		Last Modified: Tue, 04 Jul 2023 01:26:47 GMT  
		Size: 49.5 MB (49496987 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:08f7c6c26af7129b07626a4188c7da1c3070a43f1614c955c506211bad9bd9f8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a21b9cc7e6bef17a2b58cfd4fce725c4fe88d0ea922964cdd5963febd4831200`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:33 GMT
ADD file:0ceaf5be903c5d350c3e3aa5ba446b3e6ef07b387c44d4fcfe5a4ff0d3c9dafc in / 
# Tue, 04 Jul 2023 01:21:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0a29695f02132f5ced01dc6d2db95e88b5cc475771c1b689b45229c55d07594e`  
		Last Modified: Tue, 04 Jul 2023 01:29:35 GMT  
		Size: 53.5 MB (53510478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703` - linux; s390x

```console
$ docker pull debian@sha256:6379fa32f8afe9fffb987bc0e248f64dfadd2546baf3350de9b8aa7ef40c46bb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47949785 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37a61100328c8f55a68ae64aa592b0a751f1ce0775e2c874441bffe612464194`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:52 GMT
ADD file:9ec04d3ce271a93cb5627cb0001517d58e07016d24e0e80cbdcb8bda54e69f8d in / 
# Tue, 04 Jul 2023 01:34:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56cf8724500ec05fe7063f36d4625803ce264bb1a82d2faaabba9d1be0008b8b`  
		Last Modified: Tue, 04 Jul 2023 01:39:27 GMT  
		Size: 47.9 MB (47949785 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-20230703-slim`

```console
$ docker pull debian@sha256:de2f41651b404e585f9f464ca429eaf71e430232190a8c2424bd3fc0726f34fa
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:trixie-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:18a4052db8a763e9f3728a15a31e0bfd7ef540f6b72d4bd12060fb3829955fbb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29083922 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ab8e5d4ba4ef9e002e5fd05603e9846ea149b379a49229fbdb87e4f7f46b7fc1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:15 GMT
ADD file:32c59baeac5a7bc66e6b48cd8c866ff7460abba3ab2177a69f5f6bc9a08433da in / 
# Tue, 04 Jul 2023 01:23:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:47d3952ec00d0e1235bfb6da06c4d1974d119cd6028e01c230bc757a4d066f59`  
		Last Modified: Tue, 04 Jul 2023 01:29:34 GMT  
		Size: 29.1 MB (29083922 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:e5d347897f2dec9374237715196682e50370c05fdb1afa7f475d1b0769fa7126
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26930301 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e580f4558bfee0fcedda7e741b24411b5c81da618240379f751d5ee49bc9c7fc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:07 GMT
ADD file:03fd2f2141c7872d74490a4748f2970ab64326df6c020165a9407e9c212f0185 in / 
# Tue, 04 Jul 2023 00:50:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:54b30bafb0963ab0a92a2f1c87ac2b8346f4d2659d56119eb2017059b20a4fd2`  
		Last Modified: Tue, 04 Jul 2023 00:55:29 GMT  
		Size: 26.9 MB (26930301 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:1f404ef1a2762801788c05d1850647699110a9de97ab02a2150d2a6f3b659f0c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24736899 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a564d672632a86cb7319fa9ace42ea73c8a8a84df0d3de595b1e84e5307cd37e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:11 GMT
ADD file:21cacbbf34967722df04a0d5f2bac36b819511b668f50142735549a7a8fc3aac in / 
# Tue, 04 Jul 2023 01:01:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:287dcd5c6c32306a507d2099594f3707aeeedd0686882efbd2d7d42a5afdef0e`  
		Last Modified: Tue, 04 Jul 2023 01:07:41 GMT  
		Size: 24.7 MB (24736899 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d8151683a429d6582b688ed9c5e10a2c40a8100427c54d5b6c69a0aa40c0b3c1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095157 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:32c04b6f2e4fccd8acae6855d4a54fec7c91ea85242330a7df5a929b193b89f4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:45 GMT
ADD file:3ef75d2ee5bfb316fde68c102c90d99b813d60ed2036ebf024ee04847b80faae in / 
# Tue, 04 Jul 2023 01:59:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a3439e345c4edbaf201ba7df4510471a2550cdc6c172d2c74a46cc824215fc43`  
		Last Modified: Tue, 04 Jul 2023 02:05:44 GMT  
		Size: 29.1 MB (29095157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:b93c6f7a306a0575999ea0dd14d13ebb8c02e93070add6c0b58e33a37a9fb28b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30110819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:79b6699a6007f0e477843b19fbb9765c0ff501bee1a25b9c60fb2b40b521f51d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:38 GMT
ADD file:3b615710d3e5a97accabfb1967830a69cb1cf7b1fdf99cafbd1912b77ea5abb3 in / 
# Tue, 04 Jul 2023 01:41:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f986ec429baf00fa4e92d90f1171dfa90bd12fdbad15be16e690dc7a34f642d6`  
		Last Modified: Tue, 04 Jul 2023 01:48:37 GMT  
		Size: 30.1 MB (30110819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:012f13596f32658ccf89196a83dcbff22127a95f9b2d965adefd45b6efaf6370
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29061932 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9ab2bb16784ce026d4fe5891f6462340e4bb9319322e48a275794aededed5c2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:16:36 GMT
ADD file:5b1cba878f69cfc12e3ebe79275b700d1fe681a6abe8465b445970cc541d170b in / 
# Tue, 04 Jul 2023 01:16:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6d3fae4ee012fe87d67d18a3c7ac1db12ec60c21c93d56e6277096f867b3bd4`  
		Last Modified: Tue, 04 Jul 2023 01:27:21 GMT  
		Size: 29.1 MB (29061932 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:d3c8bcdb2ee97c08cdac1cb1c682f4f0024f452b16cf5c577b739587b94daae4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33076718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:030b92616f8d190c7825d8e51c45ae397c720b8ffcf628179d68e5d6f0171822`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:58 GMT
ADD file:759c60a31e3160b75d0d87f10e3bd000f8e7cd7ac11748335409124685f086b4 in / 
# Tue, 04 Jul 2023 01:22:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbc87d0073c4719d9465b775cd439f616ed36c4312c97c3928ecea48ea91df4e`  
		Last Modified: Tue, 04 Jul 2023 01:30:04 GMT  
		Size: 33.1 MB (33076718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:40af1ae528f0739df4799d3486e8340a2ca9d2115a46c5506f12bc4cac70c715
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27506168 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd7eddfaa6e3b4d5412162e93c35165c30b4f4c168cdfda350ef1ff777e17d77`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:11 GMT
ADD file:5e5e51beffa02c87c2ece8148385618b52b0af921bb02479385c71f9cc834854 in / 
# Tue, 04 Jul 2023 01:35:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a00a9d8e6e6876672c3b3994a619741c17b2b985648b72315ae2450912ae42e`  
		Last Modified: Tue, 04 Jul 2023 01:39:40 GMT  
		Size: 27.5 MB (27506168 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-backports`

```console
$ docker pull debian@sha256:353db64a0662fa98e11baa69b2d68031e61a2a6f12359581284dcf01e92e79e4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:trixie-backports` - linux; amd64

```console
$ docker pull debian@sha256:5ae29e839124df52a631357d8e4ea09cdc75337a2176ee0a98be9cde2a7eec5c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524069 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3eeec5e1575d2be00f72da3684ea61c06a1afef0a097e7287bae17d898b551b9`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:01 GMT
ADD file:69ddb6bf8a54cc4c13fa6dfc75c953d997384f31131296eba0fe4670ab3281bb in / 
# Tue, 04 Jul 2023 01:23:02 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:23:06 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7b026d231b019b0ddfd89687759b2889cdfa3c816e7bb7bff4316db584f02cb3`  
		Last Modified: Tue, 04 Jul 2023 01:29:17 GMT  
		Size: 49.5 MB (49523846 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bc46b360b0a56bc5e5d179eaa095c07bd7d2e02b033040aada5a6699db028794`  
		Last Modified: Tue, 04 Jul 2023 01:29:25 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:e81d3682081739607a458f874cdf09e6f82488329bbc3adb6658669b11e1b2da
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47360291 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ae32a37a97d059a474f4b4c1bfb7aed1b42298cbc7ec86161fa6fe78f69b4e75`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:49:59 GMT
ADD file:0ffd95af69945bc7c3ba2733bffd453f61419b678dcc1b0bf13261d9a2d147f7 in / 
# Tue, 04 Jul 2023 00:49:59 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 00:50:02 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:059b0228783b958bcd75527dc7506c02509e9b3a009b9e7f5b944545d6d86929`  
		Last Modified: Tue, 04 Jul 2023 00:55:11 GMT  
		Size: 47.4 MB (47360068 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:800e40cbc2bdb55b4e850e1de91b839cf32b2fe10878fa6e6bfb56e2245fb5e6`  
		Last Modified: Tue, 04 Jul 2023 00:55:19 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:470115159880b4f4ae66a971819f3fd22f86f726a586d6732ce135e980bcedb1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45183044 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:10edec909c5814fbc1eec58d5f35fc76c37103c447f59b87481bce5752a709c6`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:00:59 GMT
ADD file:d1d870c89e7d21b3459cde2dabaf0d8fbc038324334dfb59648191201c0881a6 in / 
# Tue, 04 Jul 2023 01:00:59 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:01:03 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:de599662316fbbb0e5a107c34fc84f0bd6cce1eae2febdee5f9a166bcbe6f898`  
		Last Modified: Tue, 04 Jul 2023 01:07:24 GMT  
		Size: 45.2 MB (45182820 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5670106917a9946afac9ad64e88b57f1d74426abd2e7fb9c5a1690541d0674ac`  
		Last Modified: Tue, 04 Jul 2023 01:07:33 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:8bbc37cb7755b44e0509a294e381749150c8afb18b22dffbb49c4ca0b5edb55c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49524712 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ab30828c59efc7b5b188a8fad2f79d62a74b226ca2d3bce5339f16d9db71104b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:36 GMT
ADD file:e83c96cb391bb6f4a4236baff0c8be47ff4ea09826f95481497333dddc3537ea in / 
# Tue, 04 Jul 2023 01:59:37 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:59:40 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d728e68b504cba85127d7d3e5493d24223ae3af314b4e73d6b74bdebf7911f05`  
		Last Modified: Tue, 04 Jul 2023 02:05:27 GMT  
		Size: 49.5 MB (49524491 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d96724154c094afab3d9be4ffa4d0084c788c6cd10f3a427b919a6360ae610c3`  
		Last Modified: Tue, 04 Jul 2023 02:05:35 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; 386

```console
$ docker pull debian@sha256:ba31134ada6ef14473e8ed19d30d2657df5a50fc7558e867adc9a70a5e6f3839
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50539630 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e660080e5694a4f09be0f2e019e3364036d887965eb04fae883db78dadfed872`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:27 GMT
ADD file:034bf0762755f9dc5cb0cea6768f68025634eda8045c112ef95777d21df6ecf3 in / 
# Tue, 04 Jul 2023 01:41:27 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:41:30 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:efb3d19d1083945d3c3a14221887db9fd1e087f333da06937d0ec9bfe0c79e18`  
		Last Modified: Tue, 04 Jul 2023 01:48:17 GMT  
		Size: 50.5 MB (50539409 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0fc339673cb7adea6360b02bf6a64935dc01ab1ea025daf75d817586e45465d0`  
		Last Modified: Tue, 04 Jul 2023 01:48:26 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; mips64le

```console
$ docker pull debian@sha256:a5b9a3e900baa05439aaca266817d49d7ff2da69cab32b9b6e05e3bb435a6251
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49497212 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3a0c7887109ce3a92d8c3fa5cfb0a7fdb7cdb5d775ec6594b947811d036b2aa4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:15:54 GMT
ADD file:ae6c6291352d35c9053a1b89fa15bd2eb76add307e5059d83474f17b2e138b31 in / 
# Tue, 04 Jul 2023 01:15:59 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:16:13 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:67e71ca2fad81efa04d26e36aee19d65985a3b0a387729559324ca4e38246677`  
		Last Modified: Tue, 04 Jul 2023 01:26:47 GMT  
		Size: 49.5 MB (49496987 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:464ca92ca112ca9ef44793598b225b1d8646aa78347045e9ae71982e45ed5ea9`  
		Last Modified: Tue, 04 Jul 2023 01:26:56 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:a33f752d2bf7e0e56ca0059f5010413456e67f9e27a6c058a35e68f3e38ea350
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53510703 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c03510c953e92fe5f5f7609533f15debefd498f7491ebeea3b4091a1d5c8ee12`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:33 GMT
ADD file:0ceaf5be903c5d350c3e3aa5ba446b3e6ef07b387c44d4fcfe5a4ff0d3c9dafc in / 
# Tue, 04 Jul 2023 01:21:38 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:21:46 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0a29695f02132f5ced01dc6d2db95e88b5cc475771c1b689b45229c55d07594e`  
		Last Modified: Tue, 04 Jul 2023 01:29:35 GMT  
		Size: 53.5 MB (53510478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c33912edaabc2ceaaa6173c2a26b4a9a7c5b4909ab28efc7a2402629e6c04f4c`  
		Last Modified: Tue, 04 Jul 2023 01:29:48 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; s390x

```console
$ docker pull debian@sha256:05c18702c98eebf3d9503da696dec797788964b51a836f2d5fc507d1849e9525
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.0 MB (47950007 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e7d6457b344c1d4ed9feaf5f9feba7c6039b6f3748942082d8cfd44f81f80689`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:34:52 GMT
ADD file:9ec04d3ce271a93cb5627cb0001517d58e07016d24e0e80cbdcb8bda54e69f8d in / 
# Tue, 04 Jul 2023 01:34:55 GMT
CMD ["bash"]
# Tue, 04 Jul 2023 01:35:02 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:56cf8724500ec05fe7063f36d4625803ce264bb1a82d2faaabba9d1be0008b8b`  
		Last Modified: Tue, 04 Jul 2023 01:39:27 GMT  
		Size: 47.9 MB (47949785 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:da86eb9d735bfc2a334b19d19061c3401d6c7b1bb8369c5e07768ba511ae4b4f`  
		Last Modified: Tue, 04 Jul 2023 01:39:32 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-slim`

```console
$ docker pull debian@sha256:de2f41651b404e585f9f464ca429eaf71e430232190a8c2424bd3fc0726f34fa
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:trixie-slim` - linux; amd64

```console
$ docker pull debian@sha256:18a4052db8a763e9f3728a15a31e0bfd7ef540f6b72d4bd12060fb3829955fbb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29083922 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ab8e5d4ba4ef9e002e5fd05603e9846ea149b379a49229fbdb87e4f7f46b7fc1`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:15 GMT
ADD file:32c59baeac5a7bc66e6b48cd8c866ff7460abba3ab2177a69f5f6bc9a08433da in / 
# Tue, 04 Jul 2023 01:23:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:47d3952ec00d0e1235bfb6da06c4d1974d119cd6028e01c230bc757a4d066f59`  
		Last Modified: Tue, 04 Jul 2023 01:29:34 GMT  
		Size: 29.1 MB (29083922 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:e5d347897f2dec9374237715196682e50370c05fdb1afa7f475d1b0769fa7126
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26930301 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e580f4558bfee0fcedda7e741b24411b5c81da618240379f751d5ee49bc9c7fc`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:07 GMT
ADD file:03fd2f2141c7872d74490a4748f2970ab64326df6c020165a9407e9c212f0185 in / 
# Tue, 04 Jul 2023 00:50:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:54b30bafb0963ab0a92a2f1c87ac2b8346f4d2659d56119eb2017059b20a4fd2`  
		Last Modified: Tue, 04 Jul 2023 00:55:29 GMT  
		Size: 26.9 MB (26930301 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:1f404ef1a2762801788c05d1850647699110a9de97ab02a2150d2a6f3b659f0c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24736899 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a564d672632a86cb7319fa9ace42ea73c8a8a84df0d3de595b1e84e5307cd37e`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:11 GMT
ADD file:21cacbbf34967722df04a0d5f2bac36b819511b668f50142735549a7a8fc3aac in / 
# Tue, 04 Jul 2023 01:01:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:287dcd5c6c32306a507d2099594f3707aeeedd0686882efbd2d7d42a5afdef0e`  
		Last Modified: Tue, 04 Jul 2023 01:07:41 GMT  
		Size: 24.7 MB (24736899 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d8151683a429d6582b688ed9c5e10a2c40a8100427c54d5b6c69a0aa40c0b3c1
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095157 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:32c04b6f2e4fccd8acae6855d4a54fec7c91ea85242330a7df5a929b193b89f4`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:45 GMT
ADD file:3ef75d2ee5bfb316fde68c102c90d99b813d60ed2036ebf024ee04847b80faae in / 
# Tue, 04 Jul 2023 01:59:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a3439e345c4edbaf201ba7df4510471a2550cdc6c172d2c74a46cc824215fc43`  
		Last Modified: Tue, 04 Jul 2023 02:05:44 GMT  
		Size: 29.1 MB (29095157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; 386

```console
$ docker pull debian@sha256:b93c6f7a306a0575999ea0dd14d13ebb8c02e93070add6c0b58e33a37a9fb28b
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30110819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:79b6699a6007f0e477843b19fbb9765c0ff501bee1a25b9c60fb2b40b521f51d`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:38 GMT
ADD file:3b615710d3e5a97accabfb1967830a69cb1cf7b1fdf99cafbd1912b77ea5abb3 in / 
# Tue, 04 Jul 2023 01:41:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f986ec429baf00fa4e92d90f1171dfa90bd12fdbad15be16e690dc7a34f642d6`  
		Last Modified: Tue, 04 Jul 2023 01:48:37 GMT  
		Size: 30.1 MB (30110819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; mips64le

```console
$ docker pull debian@sha256:012f13596f32658ccf89196a83dcbff22127a95f9b2d965adefd45b6efaf6370
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29061932 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9ab2bb16784ce026d4fe5891f6462340e4bb9319322e48a275794aededed5c2`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:16:36 GMT
ADD file:5b1cba878f69cfc12e3ebe79275b700d1fe681a6abe8465b445970cc541d170b in / 
# Tue, 04 Jul 2023 01:16:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6d3fae4ee012fe87d67d18a3c7ac1db12ec60c21c93d56e6277096f867b3bd4`  
		Last Modified: Tue, 04 Jul 2023 01:27:21 GMT  
		Size: 29.1 MB (29061932 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:d3c8bcdb2ee97c08cdac1cb1c682f4f0024f452b16cf5c577b739587b94daae4
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33076718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:030b92616f8d190c7825d8e51c45ae397c720b8ffcf628179d68e5d6f0171822`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:21:58 GMT
ADD file:759c60a31e3160b75d0d87f10e3bd000f8e7cd7ac11748335409124685f086b4 in / 
# Tue, 04 Jul 2023 01:22:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bbc87d0073c4719d9465b775cd439f616ed36c4312c97c3928ecea48ea91df4e`  
		Last Modified: Tue, 04 Jul 2023 01:30:04 GMT  
		Size: 33.1 MB (33076718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; s390x

```console
$ docker pull debian@sha256:40af1ae528f0739df4799d3486e8340a2ca9d2115a46c5506f12bc4cac70c715
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27506168 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd7eddfaa6e3b4d5412162e93c35165c30b4f4c168cdfda350ef1ff777e17d77`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:11 GMT
ADD file:5e5e51beffa02c87c2ece8148385618b52b0af921bb02479385c71f9cc834854 in / 
# Tue, 04 Jul 2023 01:35:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9a00a9d8e6e6876672c3b3994a619741c17b2b985648b72315ae2450912ae42e`  
		Last Modified: Tue, 04 Jul 2023 01:39:40 GMT  
		Size: 27.5 MB (27506168 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable`

```console
$ docker pull debian@sha256:002f0db93b881359b3b6b5f0f90efb149cbb28e49932fc90e3f44be1bcab9c08
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable` - linux; amd64

```console
$ docker pull debian@sha256:23f93573b8848e30cbfc4071890cd84ccb72e0c64b760fb7c846f61f8a5c3927
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49475947 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d996720ac4212fe93abd3b40f6cf2419c107a0e2271f9274303d66b4de39f1b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:27 GMT
ADD file:30c3621d27a742c3c20865fd70d8aa41d70be0f58eeeb81aaa9dd1298ecb1c1d in / 
# Tue, 04 Jul 2023 01:23:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:52ab46b1411e2672033a4f02a5e7df836c372c7d9366d0f10b15751b45a6a217`  
		Last Modified: Tue, 04 Jul 2023 01:29:49 GMT  
		Size: 49.5 MB (49475947 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:c89d2f654b3fa3e080ccccfbafc7251c2c2ab536a38d74e65639e344f45817c8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322476 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ba3f9ff86537c36c5583237ec62e4f1b34fde1977fc2f5226d127df74bfb9108`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:15 GMT
ADD file:0d73a7183a38c56858e61e53f0c64ae6eaf9d58311df1be7af3b37f1e1769dd2 in / 
# Tue, 04 Jul 2023 00:50:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb46e2b513bf023b4297bc46404f96d3a604f8f913dffe3d1f9e5c5c530e9caf`  
		Last Modified: Tue, 04 Jul 2023 00:55:44 GMT  
		Size: 47.3 MB (47322476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:2caf69b0a2501f1251df3ca6d378672f0e7122addbdb4cd71f59fafeb0a41baf
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45123977 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c81b1dfedc12626621c8a67db7d9060b04c6361321fa36c8f595033d87e6703`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:20 GMT
ADD file:a7c3220804fb0e3fd658c0492b5338ba9fc6c132f28282486bbae9622138fd0e in / 
# Tue, 04 Jul 2023 01:01:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0807315011437359206e59c75af82e26e89ba45ae9281fa33df5e8dda8de17a2`  
		Last Modified: Tue, 04 Jul 2023 01:07:56 GMT  
		Size: 45.1 MB (45123977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:845e92adf4fbce2c3a145a951769a54efa9c283115469990b6fae14d33ab8098
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482573 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e1005902651f75c3e87c51188e1c28daf37eca279a2f6817271711bd67b09e53`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:53 GMT
ADD file:bf1bc62117abf0b13f8992acfed17c5505f88edece9bdb42c246d3801869a909 in / 
# Tue, 04 Jul 2023 01:59:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d63178d4b98b5ec11651f31157085c27b04daa9271f1b4a8be773b8ad11c4264`  
		Last Modified: Tue, 04 Jul 2023 02:05:57 GMT  
		Size: 49.5 MB (49482573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; 386

```console
$ docker pull debian@sha256:bada749b08a4347e19353c06e37937a06a437019d91cd60f28612b230eebd1ae
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50505827 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3bc02c62d06edb08ca82ce3b9dc9d3b3f20bf03d34fcb765d95751017241047a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:50 GMT
ADD file:2f42b2e397b1e8bc6959dbfa501dddc91fd18f0d3ef0fd9ea0663092286e1429 in / 
# Tue, 04 Jul 2023 01:41:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d6a8ba1eaf3690a5e9b7f2debed87f9cfac7972349e43494cf2b7feffb143629`  
		Last Modified: Tue, 04 Jul 2023 01:48:54 GMT  
		Size: 50.5 MB (50505827 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; mips64le

```console
$ docker pull debian@sha256:300e485d9dc733bae77702214b2ac592ab6cc2688fb120a46878932e5f7cba00
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455401 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc13d238fe77a4017c7f72c7723d32bab09013575b1addd9c4ffa08cf8b60235`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:08 GMT
ADD file:20580fa5e83e1873d44c46cb2f9ac1fe39a42c62c9d4e3756723283f13add821 in / 
# Tue, 04 Jul 2023 01:17:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:78d3f505f592dc293a56dbf6460a5f68fd63cf76766388835e32dcbc804fc180`  
		Last Modified: Tue, 04 Jul 2023 01:27:58 GMT  
		Size: 49.5 MB (49455401 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; ppc64le

```console
$ docker pull debian@sha256:4c7600787f8ab33ff9fe1365fda2c93bc960c87555f8cc7933b9269db6c4d3f5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474416 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8c185ac9101fd73796f85b34e3e01a850b841be962dd616e6b8bcc7e54b725`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:15 GMT
ADD file:d5d2b24ead06183959c90a6751c80a887c556ca4f0fdc7d287812576eafb1a60 in / 
# Tue, 04 Jul 2023 01:22:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af5bbdab36f44ac3e6bea7dc4ed1c8b357c4862d3c25a3ec040d72e425ef159c`  
		Last Modified: Tue, 04 Jul 2023 01:30:28 GMT  
		Size: 53.5 MB (53474416 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; riscv64

```console
$ docker pull debian@sha256:b53b908faa69e244c6522b64e15eaaa7e785c4eb76152afabfb17c0bf4ca841b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695485 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0612707a0db67a8659629fb72e3d5ed8943bfe251982adf2afd04d1d4264ae3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:32 GMT
ADD file:bacf5c7bb4f9db293679962e5936b1037a72a2e667da59236cda267c866ad8e2 in / 
# Tue, 04 Jul 2023 01:10:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:892c64510625aa4b95222221c8d62ebbab44da937d39c14956f6e6697a5c4c80`  
		Last Modified: Tue, 04 Jul 2023 01:13:59 GMT  
		Size: 45.7 MB (45695485 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; s390x

```console
$ docker pull debian@sha256:df4bf7f997401f92eaed8dde8a207e53df65e61c52f60940db04c6c53d148059
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890770 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:730f03ad9335bb156c3ab9695a00924262c3cc1b426b096e54b4c8ed3216a712`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:23 GMT
ADD file:aac2397e3a90e63ce8e13930d00bcbee000d5f021a613a196e495f1c36aeeef1 in / 
# Tue, 04 Jul 2023 01:35:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4385878fa0020ea8459bcfa951b29983c0576d085fba63f0b00e83da8014aa50`  
		Last Modified: Tue, 04 Jul 2023 01:39:52 GMT  
		Size: 47.9 MB (47890770 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20230703`

```console
$ docker pull debian@sha256:002f0db93b881359b3b6b5f0f90efb149cbb28e49932fc90e3f44be1bcab9c08
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20230703` - linux; amd64

```console
$ docker pull debian@sha256:23f93573b8848e30cbfc4071890cd84ccb72e0c64b760fb7c846f61f8a5c3927
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49475947 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6d996720ac4212fe93abd3b40f6cf2419c107a0e2271f9274303d66b4de39f1b`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:27 GMT
ADD file:30c3621d27a742c3c20865fd70d8aa41d70be0f58eeeb81aaa9dd1298ecb1c1d in / 
# Tue, 04 Jul 2023 01:23:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:52ab46b1411e2672033a4f02a5e7df836c372c7d9366d0f10b15751b45a6a217`  
		Last Modified: Tue, 04 Jul 2023 01:29:49 GMT  
		Size: 49.5 MB (49475947 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; arm variant v5

```console
$ docker pull debian@sha256:c89d2f654b3fa3e080ccccfbafc7251c2c2ab536a38d74e65639e344f45817c8
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47322476 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ba3f9ff86537c36c5583237ec62e4f1b34fde1977fc2f5226d127df74bfb9108`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:15 GMT
ADD file:0d73a7183a38c56858e61e53f0c64ae6eaf9d58311df1be7af3b37f1e1769dd2 in / 
# Tue, 04 Jul 2023 00:50:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb46e2b513bf023b4297bc46404f96d3a604f8f913dffe3d1f9e5c5c530e9caf`  
		Last Modified: Tue, 04 Jul 2023 00:55:44 GMT  
		Size: 47.3 MB (47322476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; arm variant v7

```console
$ docker pull debian@sha256:2caf69b0a2501f1251df3ca6d378672f0e7122addbdb4cd71f59fafeb0a41baf
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45123977 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5c81b1dfedc12626621c8a67db7d9060b04c6361321fa36c8f595033d87e6703`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:20 GMT
ADD file:a7c3220804fb0e3fd658c0492b5338ba9fc6c132f28282486bbae9622138fd0e in / 
# Tue, 04 Jul 2023 01:01:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0807315011437359206e59c75af82e26e89ba45ae9281fa33df5e8dda8de17a2`  
		Last Modified: Tue, 04 Jul 2023 01:07:56 GMT  
		Size: 45.1 MB (45123977 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:845e92adf4fbce2c3a145a951769a54efa9c283115469990b6fae14d33ab8098
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49482573 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e1005902651f75c3e87c51188e1c28daf37eca279a2f6817271711bd67b09e53`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:59:53 GMT
ADD file:bf1bc62117abf0b13f8992acfed17c5505f88edece9bdb42c246d3801869a909 in / 
# Tue, 04 Jul 2023 01:59:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d63178d4b98b5ec11651f31157085c27b04daa9271f1b4a8be773b8ad11c4264`  
		Last Modified: Tue, 04 Jul 2023 02:05:57 GMT  
		Size: 49.5 MB (49482573 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; 386

```console
$ docker pull debian@sha256:bada749b08a4347e19353c06e37937a06a437019d91cd60f28612b230eebd1ae
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.5 MB (50505827 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3bc02c62d06edb08ca82ce3b9dc9d3b3f20bf03d34fcb765d95751017241047a`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:50 GMT
ADD file:2f42b2e397b1e8bc6959dbfa501dddc91fd18f0d3ef0fd9ea0663092286e1429 in / 
# Tue, 04 Jul 2023 01:41:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d6a8ba1eaf3690a5e9b7f2debed87f9cfac7972349e43494cf2b7feffb143629`  
		Last Modified: Tue, 04 Jul 2023 01:48:54 GMT  
		Size: 50.5 MB (50505827 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; mips64le

```console
$ docker pull debian@sha256:300e485d9dc733bae77702214b2ac592ab6cc2688fb120a46878932e5f7cba00
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49455401 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc13d238fe77a4017c7f72c7723d32bab09013575b1addd9c4ffa08cf8b60235`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:08 GMT
ADD file:20580fa5e83e1873d44c46cb2f9ac1fe39a42c62c9d4e3756723283f13add821 in / 
# Tue, 04 Jul 2023 01:17:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:78d3f505f592dc293a56dbf6460a5f68fd63cf76766388835e32dcbc804fc180`  
		Last Modified: Tue, 04 Jul 2023 01:27:58 GMT  
		Size: 49.5 MB (49455401 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; ppc64le

```console
$ docker pull debian@sha256:4c7600787f8ab33ff9fe1365fda2c93bc960c87555f8cc7933b9269db6c4d3f5
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53474416 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8c185ac9101fd73796f85b34e3e01a850b841be962dd616e6b8bcc7e54b725`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:15 GMT
ADD file:d5d2b24ead06183959c90a6751c80a887c556ca4f0fdc7d287812576eafb1a60 in / 
# Tue, 04 Jul 2023 01:22:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af5bbdab36f44ac3e6bea7dc4ed1c8b357c4862d3c25a3ec040d72e425ef159c`  
		Last Modified: Tue, 04 Jul 2023 01:30:28 GMT  
		Size: 53.5 MB (53474416 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; riscv64

```console
$ docker pull debian@sha256:b53b908faa69e244c6522b64e15eaaa7e785c4eb76152afabfb17c0bf4ca841b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.7 MB (45695485 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0612707a0db67a8659629fb72e3d5ed8943bfe251982adf2afd04d1d4264ae3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:10:32 GMT
ADD file:bacf5c7bb4f9db293679962e5936b1037a72a2e667da59236cda267c866ad8e2 in / 
# Tue, 04 Jul 2023 01:10:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:892c64510625aa4b95222221c8d62ebbab44da937d39c14956f6e6697a5c4c80`  
		Last Modified: Tue, 04 Jul 2023 01:13:59 GMT  
		Size: 45.7 MB (45695485 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703` - linux; s390x

```console
$ docker pull debian@sha256:df4bf7f997401f92eaed8dde8a207e53df65e61c52f60940db04c6c53d148059
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47890770 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:730f03ad9335bb156c3ab9695a00924262c3cc1b426b096e54b4c8ed3216a712`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:23 GMT
ADD file:aac2397e3a90e63ce8e13930d00bcbee000d5f021a613a196e495f1c36aeeef1 in / 
# Tue, 04 Jul 2023 01:35:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4385878fa0020ea8459bcfa951b29983c0576d085fba63f0b00e83da8014aa50`  
		Last Modified: Tue, 04 Jul 2023 01:39:52 GMT  
		Size: 47.9 MB (47890770 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20230703-slim`

```console
$ docker pull debian@sha256:82cd14da53e6dab12a2e345247ab1f5ea620a58a5d209ba01f8a5498f5f6b14f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20230703-slim` - linux; amd64

```console
$ docker pull debian@sha256:93ed8e0ad8d031b4f6bc0af2117aee9e45641324414868073947ac11984e9050
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:595acc5a06a66d7c0a8162185b9579bfa9aa22e8ec60837ad57a224bbac5c040`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:38 GMT
ADD file:b70532b9bcef1c1b07e2e7f509c686a5c68957b4dff0d485326f109ec03f4879 in / 
# Tue, 04 Jul 2023 01:23:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85020adf059e0866f5ceb87e829c11370c2224fc5bd2bc9baee4235f4e401cee`  
		Last Modified: Tue, 04 Jul 2023 01:30:00 GMT  
		Size: 29.1 MB (29095991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:68960d8cc280d1249e47ef018dd9b7e52f2404dc19abbe55c0fa448ef232ffcb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26933713 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a912f258ce1c48c8efc1782a2270f77f975499c1c9f7e89ed8a5f29e8ab3e36`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:22 GMT
ADD file:35ff94123d2605f7472dc7acc4073c77d0c3f1c0758c0b11f76090f05d96675a in / 
# Tue, 04 Jul 2023 00:50:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:077db62d8affa7ef0b36edcff86e0b8c02aa574db52e9476d5732ad4e0858944`  
		Last Modified: Tue, 04 Jul 2023 00:55:58 GMT  
		Size: 26.9 MB (26933713 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9cb30385f6afad177d001b0f038371304883f135671c57b7c25a0b5bfa488b4c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24742804 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a722fad8a2244ea59d76733037d7f75de0650854452836966c956bbb3e073e91`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:28 GMT
ADD file:030c0693ba3302c32c4ba875b2512a3ac1b323a1f20417ced419ec6e442ac934 in / 
# Tue, 04 Jul 2023 01:01:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c62b6e10684803c96014108d545402ef1f4e155d78014d44eb2204564175971`  
		Last Modified: Tue, 04 Jul 2023 01:08:09 GMT  
		Size: 24.7 MB (24742804 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d6ce3d88c2de07c526e721af45d8b8468c511510b762e9234880eeca640dc702
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29097293 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1cd28b77a40b21ba019934686dab3ea0f03560f984021b6357bfcd5ee4c28bd5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 02:00:00 GMT
ADD file:729363e1486e89cc64540ba4759f1f43fc671303ae20cf83d70714fab0d7e1b3 in / 
# Tue, 04 Jul 2023 02:00:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a4340eb10c2941bfc5bb886616c452c36bcba5f1fd138ce65ee97cf8baa07c4`  
		Last Modified: Tue, 04 Jul 2023 02:06:09 GMT  
		Size: 29.1 MB (29097293 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; 386

```console
$ docker pull debian@sha256:065806643ce35e4682de51dfa5394a8b81ba1798121cdf4ef3a04be914c065e6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30118477 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0429a6c501d0120707158a1cede70ccde1b40125c9b5fd053736dbc8599b1330`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:59 GMT
ADD file:284de0668da266634b91f13c99a88b5bc74ae6e4f15bdea9fa35f871b8187833 in / 
# Tue, 04 Jul 2023 01:41:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1bfbda6fa5d07afc929e41c621d3dea0671b55a89729d285ccc0d2614b6246b9`  
		Last Modified: Tue, 04 Jul 2023 01:49:08 GMT  
		Size: 30.1 MB (30118477 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; mips64le

```console
$ docker pull debian@sha256:d3f89383c9598f68ab1a03e094f071cbaa7ae5ec60363c5d55336f1733474d0a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29070952 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b09d675e813da4d9820f32479e6497f9b490a2f5d9d53963b4d40ac1a7e104ad`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:39 GMT
ADD file:22429d002ffe912f9f81f192771c371e48e3f9b24f3495d1460f6363b9c7b2a4 in / 
# Tue, 04 Jul 2023 01:17:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:888c64d9caaf77f9325d48747d650355ba26728922be55b102c2d70678a1c3b0`  
		Last Modified: Tue, 04 Jul 2023 01:28:26 GMT  
		Size: 29.1 MB (29070952 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:5c55a80d1f31f24fd34854154104fddb322b7b293360d62e49d685bc1b97b5cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33087104 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:90062c935071fee060ec4ca201c0ec84d158f43e4a406758adc8ab8f135a100c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:34 GMT
ADD file:aaeaeb8eba9c01e1df0b06ea286f522ec9de7ca1c26891b3a6c56fa7f80c3313 in / 
# Tue, 04 Jul 2023 01:22:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:33bf4d0a87975b9968ab328bed700fc300a3e7535005fa7459080f7ba12d6639`  
		Last Modified: Tue, 04 Jul 2023 01:30:48 GMT  
		Size: 33.1 MB (33087104 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; riscv64

```console
$ docker pull debian@sha256:c6cea56c4b672c5bc3b43eeeebc3080ee1046834b359d1536a48b7a67dbee64b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25314924 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cdd9aef2d34a2639c7e1f0bca8bf0c15bb82a0df17369e81c3364c338b532f36`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:08 GMT
ADD file:e1e38a0b53672223ba7be59ef84533686660c09e6ac74fe7888e887705d9a64a in / 
# Tue, 04 Jul 2023 01:11:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a0de749effce117899cbe73a9788553112d395de2ba0aa315291084489f26adb`  
		Last Modified: Tue, 04 Jul 2023 01:14:34 GMT  
		Size: 25.3 MB (25314924 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20230703-slim` - linux; s390x

```console
$ docker pull debian@sha256:7d6364618dbd1ed3b7e7cf26385fc0fe2d3cb560e93affd08641ffca86c31932
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27507854 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5465c34044084b63f672f66d33da1df82e941ea3482da7976270df5d07d627f5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:35 GMT
ADD file:148745685132442f87405284ed4fde41c2203410a2d8e66864cf8a3292e720b3 in / 
# Tue, 04 Jul 2023 01:35:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4bc2d7ed37dfa5598973e636d61df58b8fc8e2a19618f4b309717401f9b22a82`  
		Last Modified: Tue, 04 Jul 2023 01:40:02 GMT  
		Size: 27.5 MB (27507854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-slim`

```console
$ docker pull debian@sha256:82cd14da53e6dab12a2e345247ab1f5ea620a58a5d209ba01f8a5498f5f6b14f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:93ed8e0ad8d031b4f6bc0af2117aee9e45641324414868073947ac11984e9050
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29095991 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:595acc5a06a66d7c0a8162185b9579bfa9aa22e8ec60837ad57a224bbac5c040`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:23:38 GMT
ADD file:b70532b9bcef1c1b07e2e7f509c686a5c68957b4dff0d485326f109ec03f4879 in / 
# Tue, 04 Jul 2023 01:23:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:85020adf059e0866f5ceb87e829c11370c2224fc5bd2bc9baee4235f4e401cee`  
		Last Modified: Tue, 04 Jul 2023 01:30:00 GMT  
		Size: 29.1 MB (29095991 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:68960d8cc280d1249e47ef018dd9b7e52f2404dc19abbe55c0fa448ef232ffcb
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26933713 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a912f258ce1c48c8efc1782a2270f77f975499c1c9f7e89ed8a5f29e8ab3e36`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 00:50:22 GMT
ADD file:35ff94123d2605f7472dc7acc4073c77d0c3f1c0758c0b11f76090f05d96675a in / 
# Tue, 04 Jul 2023 00:50:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:077db62d8affa7ef0b36edcff86e0b8c02aa574db52e9476d5732ad4e0858944`  
		Last Modified: Tue, 04 Jul 2023 00:55:58 GMT  
		Size: 26.9 MB (26933713 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9cb30385f6afad177d001b0f038371304883f135671c57b7c25a0b5bfa488b4c
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24742804 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a722fad8a2244ea59d76733037d7f75de0650854452836966c956bbb3e073e91`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:01:28 GMT
ADD file:030c0693ba3302c32c4ba875b2512a3ac1b323a1f20417ced419ec6e442ac934 in / 
# Tue, 04 Jul 2023 01:01:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c62b6e10684803c96014108d545402ef1f4e155d78014d44eb2204564175971`  
		Last Modified: Tue, 04 Jul 2023 01:08:09 GMT  
		Size: 24.7 MB (24742804 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:d6ce3d88c2de07c526e721af45d8b8468c511510b762e9234880eeca640dc702
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29097293 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1cd28b77a40b21ba019934686dab3ea0f03560f984021b6357bfcd5ee4c28bd5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 02:00:00 GMT
ADD file:729363e1486e89cc64540ba4759f1f43fc671303ae20cf83d70714fab0d7e1b3 in / 
# Tue, 04 Jul 2023 02:00:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4a4340eb10c2941bfc5bb886616c452c36bcba5f1fd138ce65ee97cf8baa07c4`  
		Last Modified: Tue, 04 Jul 2023 02:06:09 GMT  
		Size: 29.1 MB (29097293 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; 386

```console
$ docker pull debian@sha256:065806643ce35e4682de51dfa5394a8b81ba1798121cdf4ef3a04be914c065e6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30118477 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0429a6c501d0120707158a1cede70ccde1b40125c9b5fd053736dbc8599b1330`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:41:59 GMT
ADD file:284de0668da266634b91f13c99a88b5bc74ae6e4f15bdea9fa35f871b8187833 in / 
# Tue, 04 Jul 2023 01:41:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1bfbda6fa5d07afc929e41c621d3dea0671b55a89729d285ccc0d2614b6246b9`  
		Last Modified: Tue, 04 Jul 2023 01:49:08 GMT  
		Size: 30.1 MB (30118477 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:d3f89383c9598f68ab1a03e094f071cbaa7ae5ec60363c5d55336f1733474d0a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29070952 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b09d675e813da4d9820f32479e6497f9b490a2f5d9d53963b4d40ac1a7e104ad`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:17:39 GMT
ADD file:22429d002ffe912f9f81f192771c371e48e3f9b24f3495d1460f6363b9c7b2a4 in / 
# Tue, 04 Jul 2023 01:17:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:888c64d9caaf77f9325d48747d650355ba26728922be55b102c2d70678a1c3b0`  
		Last Modified: Tue, 04 Jul 2023 01:28:26 GMT  
		Size: 29.1 MB (29070952 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:5c55a80d1f31f24fd34854154104fddb322b7b293360d62e49d685bc1b97b5cd
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33087104 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:90062c935071fee060ec4ca201c0ec84d158f43e4a406758adc8ab8f135a100c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:22:34 GMT
ADD file:aaeaeb8eba9c01e1df0b06ea286f522ec9de7ca1c26891b3a6c56fa7f80c3313 in / 
# Tue, 04 Jul 2023 01:22:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:33bf4d0a87975b9968ab328bed700fc300a3e7535005fa7459080f7ba12d6639`  
		Last Modified: Tue, 04 Jul 2023 01:30:48 GMT  
		Size: 33.1 MB (33087104 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; riscv64

```console
$ docker pull debian@sha256:c6cea56c4b672c5bc3b43eeeebc3080ee1046834b359d1536a48b7a67dbee64b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.3 MB (25314924 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cdd9aef2d34a2639c7e1f0bca8bf0c15bb82a0df17369e81c3364c338b532f36`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:11:08 GMT
ADD file:e1e38a0b53672223ba7be59ef84533686660c09e6ac74fe7888e887705d9a64a in / 
# Tue, 04 Jul 2023 01:11:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a0de749effce117899cbe73a9788553112d395de2ba0aa315291084489f26adb`  
		Last Modified: Tue, 04 Jul 2023 01:14:34 GMT  
		Size: 25.3 MB (25314924 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:7d6364618dbd1ed3b7e7cf26385fc0fe2d3cb560e93affd08641ffca86c31932
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27507854 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5465c34044084b63f672f66d33da1df82e941ea3482da7976270df5d07d627f5`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 04 Jul 2023 01:35:35 GMT
ADD file:148745685132442f87405284ed4fde41c2203410a2d8e66864cf8a3292e720b3 in / 
# Tue, 04 Jul 2023 01:35:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4bc2d7ed37dfa5598973e636d61df58b8fc8e2a19618f4b309717401f9b22a82`  
		Last Modified: Tue, 04 Jul 2023 01:40:02 GMT  
		Size: 27.5 MB (27507854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
